/*****************************************************************************
 ...
 @(#) $Id: fasthdlc.h,v 0.9.2.3 2008-04-28 23:39:55 brian Exp $
...
#ifndef __LOCAL_FASTHDLC_H__
#define __LOCAL_FASTHDLC_H__

#ident "@(#) $RCSfile: fasthdlc.h,v $ $Name:  $($Revision: 0.9.2.3 $) Copyright (c) 2001-2008 OpenSS7 Corporation."
/*
...
static inline char
hdlc_search_precalc(unsigned char c)
{
	int x, p = 0;
	...
	/* If we get here, we must have at least one zero in us but we're not the flag.  So, start
	   at the end (LSB) and work our way to the top (MSB) looking for a zero.  The position of
	   that 0 is most optimistic start of a real frame header */
	x = 1;
	p = 7;
	while (p && (c & x)) {
		x <<= 1;
...
#ifdef DEBUG_PRECALC
static inline void
hdlc_search_print(char c, char r)
{
	int x = 0x80;
...
hdlc_frame_precalc(unsigned char x, unsigned short c)
{
	/* Assume we have seen 'x' one's so far, and have read the bottom 10 bytes of c (MSB
	   first).  Now, we HAVE to have a byte of data or a frame or something.  We are assumed to 
	   be at the beginning of a byte of data or something */
		...
		if (ones == 5) {
			...
			if (c & 0x0200) {
				...
				if ((!(c & 0x0100)) && (bits == 6)) {
					...
					/* Yuck! It's something else... Abort this entire frame,
					   and start looking for a good frame */
...
static inline void
hdlc_frame_print(unsigned char x, unsigned short c, unsigned int res)
{
	int z = 0x0200;
	char *status[] = {
		"Control",
...
static inline unsigned int
hdlc_encode_precalc(int x, unsigned char y)
{
	int bits = 0;
	int ones = x;
...
#ifdef DEBUG_PRECALC
static inline void
hdlc_encode_print(int x, unsigned char y, unsigned int val)
{
	unsigned int z;
	unsigned short c;
...
fasthdlc_precalc(void)
{
	...
	for (x = 0; x < 6; x++) {
		/* Given the # of preceeding ones, process the next byte of input (up to 10 actual
		   bits) */
		for (y = 0; y < 1024; y++) {
...
static inline void
fasthdlc_init(struct fasthdlc_state *h)
{
...
static inline int
fasthdlc_tx_load_nocheck(struct fasthdlc_state *h, unsigned char c)
{
	unsigned int res;
...
static inline int
fasthdlc_tx_load(struct fasthdlc_state *h, unsigned char c)
{
...
static inline int
fasthdlc_tx_frame_nocheck(struct fasthdlc_state *h)
{
	h->ones = 0;
...
static inline int
fasthdlc_tx_frame(struct fasthdlc_state *h)
{
	if (h->bits > 24)
		return -1;
...
static inline int
fasthdlc_tx_run_nocheck(struct fasthdlc_state *h)
{
	unsigned char b;
...
static inline int
fasthdlc_tx_run(struct fasthdlc_state *h)
{
	if (h->bits < 8)
		return -1;
...
static inline int
fasthdlc_rx_load_nocheck(struct fasthdlc_state *h, unsigned char b)
{
...
static inline int
fasthdlc_rx_load(struct fasthdlc_state *h, unsigned char b)
{
...
static inline int
fasthdlc_rx_run(struct fasthdlc_state *h)
{
	unsigned short next;

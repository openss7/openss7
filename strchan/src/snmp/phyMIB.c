/*****************************************************************************
 ...
 @(#) $RCSfile: phyMIB.c,v $ $Name:  $($Revision: 0.9.2.6 $) $Date: 2008-12-24 13:40:57 $
...
#ident "@(#) $RCSfile: phyMIB.c,v $ $Name:  $($Revision: 0.9.2.6 $) $Date: 2008-12-24 13:40:57 $"

static char const ident[] = "$RCSfile: phyMIB.c,v $ $Name:  $($Revision: 0.9.2.6 $) $Date: 2008-12-24 13:40:57 $";
...
refresh_phyMIB(void)
{
	if (phyMIBStorage == NULL) {
		...
		if ((StorageNew = SNMP_MALLOC_STRUCT(phyMIB_data)) == NULL)
			return;
...
physicalEntityTable_add(struct physicalEntityTable_data *thedata)
{
	...
	/* add the index variables to the varbind list, which is used by header_complex to index
	   the data */
	/* physicalEntityId */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_OCTET_STR, (uint8_t *) thedata->physicalEntityId, thedata->physicalEntityIdLen);
...
parse_physicalEntityTable(const char *token, char *line)
{
	...
	line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->physicalEntityId, &tmpsize);
	StorageTmp->physicalEntityIdLen = tmpsize;
	if (StorageTmp->physicalEntityId == NULL) {
		config_perror("invalid specification for physicalEntityId");
		return;
	}

	line = read_config_read_data(ASN_OBJECT_ID, line, &StorageTmp->physicalEntityLocalSapNames, &tmpsize);
	StorageTmp->physicalEntityLocalSapNamesLen = tmpsize;
	...
	line = read_config_read_data(ASN_OBJECT_ID, line, &StorageTmp->physicalEntityTitles, &tmpsize);
	StorageTmp->physicalEntityTitlesLen = tmpsize;
...
/*   XXX:  if (StorageTmp->physicalEntityTableStorageType == ST_NONVOLATILE) { */
		...
		cptr = line + strlen(line);
		tmpsize = StorageTmp->physicalEntityIdLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->physicalEntityId, &tmpsize);
		tmpsize = StorageTmp->physicalEntityLocalSapNamesLen;
		cptr = read_config_store_data(ASN_OBJECT_ID, cptr, &StorageTmp->physicalEntityLocalSapNames, &tmpsize);
		cptr = read_config_store_data(ASN_INTEGER, cptr, &StorageTmp->physicalEntityOperationalState, &tmpsize);
		tmpsize = StorageTmp->physicalEntityTitlesLen;
		cptr = read_config_store_data(ASN_OBJECT_ID, cptr, &StorageTmp->physicalEntityTitles, &tmpsize);
...
physicalSAPTable_add(struct physicalSAPTable_data *thedata)
{
	...
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_OCTET_STR, (uint8_t *) thedata->physicalEntityId, thedata->physicalEntityIdLen);
	/* physicalSAPsapId */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_OCTET_STR, (uint8_t *) thedata->physicalSAPsapId, thedata->physicalSAPsapIdLen);
...
parse_physicalSAPTable(const char *token, char *line)
{
	...
	line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->physicalSAPsapId, &tmpsize);
	StorageTmp->physicalSAPsapIdLen = tmpsize;
	...
	line = read_config_read_data(ASN_OBJECT_ID, line, &StorageTmp->physicalSAPuserEntityNames, &tmpsize);
	StorageTmp->physicalSAPuserEntityNamesLen = tmpsize;
...
/*   XXX:  if (StorageTmp->physicalSAPTableStorageType == ST_NONVOLATILE) { */
		...
		tmpsize = StorageTmp->physicalEntityIdLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->physicalEntityId, &tmpsize);
		tmpsize = StorageTmp->physicalSAPsapIdLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->physicalSAPsapId, &tmpsize);
		cptr = read_config_store_data(ASN_UNSIGNED, cptr, &StorageTmp->physicalSAPsap1Address, &tmpsize);
		tmpsize = StorageTmp->physicalSAPuserEntityNamesLen;
		cptr = read_config_store_data(ASN_OBJECT_ID, cptr, &StorageTmp->physicalSAPuserEntityNames, &tmpsize);
...
dataCircuitTable_add(struct dataCircuitTable_data *thedata)
{
	...
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_OCTET_STR, (uint8_t *) thedata->dataCircuitCoProtocolMachineId, thedata->dataCircuitCoProtocolMachineIdLen);
...
parse_dataCircuitTable(const char *token, char *line)
{
	...
	line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->dataCircuitCoProtocolMachineId, &tmpsize);
	StorageTmp->dataCircuitCoProtocolMachineIdLen = tmpsize;
	...
	line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->dataCircuitBitErrorsThreshold, &tmpsize);
	...
	line = read_config_read_data(ASN_OBJECT_ID, line, &StorageTmp->dataCircuitPhysicalInterfaceStandard, &tmpsize);
	...
	line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->dataCircuitTransmissionRate, &tmpsize);
...
/*   XXX:  if (StorageTmp->dataCircuitTableStorageType == ST_NONVOLATILE) { */
		...
		tmpsize = StorageTmp->physicalEntityIdLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->physicalEntityId, &tmpsize);
		tmpsize = StorageTmp->dataCircuitCoProtocolMachineIdLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->dataCircuitCoProtocolMachineId, &tmpsize);
		...
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->dataCircuitBitErrorsThreshold, &tmpsize);
		...
		cptr = read_config_store_data(ASN_OBJECT_ID, cptr, &StorageTmp->dataCircuitPhysicalInterfaceStandard, &tmpsize);
		...
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->dataCircuitTransmissionRate, &tmpsize);
...
physicalConnectionTable_add(struct physicalConnectionTable_data *thedata)
{
	...
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_OCTET_STR, (uint8_t *) thedata->physicalConnectionId, thedata->physicalConnectionIdLen);
...
parse_physicalConnectionTable(const char *token, char *line)
{
	...
	line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->physicalConnectionId, &tmpsize);
	StorageTmp->physicalConnectionIdLen = tmpsize;
	if (StorageTmp->physicalConnectionId == NULL) {
		config_perror("invalid specification for physicalConnectionId");
		return;
	}

	line = read_config_read_data(ASN_OBJECT_ID, line, &StorageTmp->physicalConnectionUnderlyingConnectionNames, &tmpsize);
	StorageTmp->physicalConnectionUnderlyingConnectionNamesLen = tmpsize;
	...
	line = read_config_read_data(ASN_OBJECT_ID, line, &StorageTmp->physicalConnectionSupportedConnectionNames, &tmpsize);
	StorageTmp->physicalConnectionSupportedConnectionNamesLen = tmpsize;
...
/*   XXX:  if (StorageTmp->physicalConnectionTableStorageType == ST_NONVOLATILE) { */
		...
		tmpsize = StorageTmp->dataCircuitCoProtocolMachineIdLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->dataCircuitCoProtocolMachineId, &tmpsize);
		tmpsize = StorageTmp->physicalConnectionIdLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->physicalConnectionId, &tmpsize);
		tmpsize = StorageTmp->physicalConnectionUnderlyingConnectionNamesLen;
		cptr = read_config_store_data(ASN_OBJECT_ID, cptr, &StorageTmp->physicalConnectionUnderlyingConnectionNames, &tmpsize);
		tmpsize = StorageTmp->physicalConnectionSupportedConnectionNamesLen;
		cptr = read_config_store_data(ASN_OBJECT_ID, cptr, &StorageTmp->physicalConnectionSupportedConnectionNames, &tmpsize);
...
var_physicalConnectionTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	case PHYSICALCONNECTIONUNDERLYINGCONNECTIONNAMES:
		...
		*var_len = StorageTmp->physicalConnectionUnderlyingConnectionNamesLen;
		return (uint8_t *) StorageTmp->physicalConnectionUnderlyingConnectionNames;
	case PHYSICALCONNECTIONSUPPORTEDCONNECTIONNAMES:
		*write_method = NULL;	/* read-only */
		*var_len = StorageTmp->physicalConnectionSupportedConnectionNamesLen;
		return (uint8_t *) StorageTmp->physicalConnectionSupportedConnectionNames;
	case PHYSICALCONNECTIONENDPOINTIDENTIFIER:
		...
		*var_len = StorageTmp->physicalConnectionEndpointIdentifierLen;
		return (uint8_t *) StorageTmp->physicalConnectionEndpointIdentifier;
	case PHYSICALCONNECTIONPORTNUMBER:
		*write_method = NULL;	/* read-only */
		*var_len = sizeof(StorageTmp->physicalConnectionPortNumber);
		return (uint8_t *) &StorageTmp->physicalConnectionPortNumber;
	case PHYSICALCONNECTIONROWSTATUS:
		...
		*var_len = sizeof(StorageTmp->physicalConnectionRowStatus);
		return (uint8_t *) &StorageTmp->physicalConnectionRowStatus;
...
int
write_dataCircuitBitErrorsThreshold(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct dataCircuitTable_data *StorageTmp = NULL;
...
int
write_physicalConnectionEndpointIdentifier(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct physicalConnectionTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in string for you to use, and you
		...
		memdup((void *) &StorageTmp->physicalConnectionEndpointIdentifier, var_val, var_val_len);
...
int
write_physicalSAPRowStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	struct physicalSAPTable_data *StorageTmp = NULL;
	...
	case COMMIT:
		...
		if (StorageDel != NULL) {
			...
			} else if (StorageTmp && StorageTmp->physicalSAPRowStatus == RS_CREATEANDWAIT) {
...
int
write_dataCircuitRowStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	struct dataCircuitTable_data *StorageTmp = NULL;
	...
	case COMMIT:
		...
		if (StorageDel != NULL) {
			...
			} else if (StorageTmp && StorageTmp->dataCircuitRowStatus == RS_CREATEANDWAIT) {
...
int
write_physicalConnectionRowStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	struct physicalConnectionTable_data *StorageTmp = NULL;
	...
	case COMMIT:
		...
		if (StorageDel != NULL) {
			...
			} else if (StorageTmp && StorageTmp->physicalConnectionRowStatus == RS_CREATEANDWAIT) {

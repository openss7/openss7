/*****************************************************************************
 ...
 @(#) $RCSfile: dlmdata.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2008-05-25 12:46:57 $
...
#ident "@(#) $RCSfile: dlmdata.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2008-05-25 12:46:57 $"

static char const ident[] = "$RCSfile: dlmdata.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2008-05-25 12:46:57 $";
...
main(int argc, char *argv[])
{
	...
	if ((physlen = stringtoaddr(argv[++optind], phys)) != ETHERADDRL) {
		if (output > 0 || debug > 0)
			(void) fprintf(strerr,
				       "%s: physlen must be size of ethernet address (6 bytes)\n",
				       argv[0]);
		goto bad_option;
	}
	 ...
	 */
	if (debug > 0)
		fprintf(stderr, "%s: getting physical address\n", argv[0]);
	dlphysaddrreq(fd, DL_CURR_PHYS_ADDR);
	if (debug > 1)
		fprintf(stderr, "%s: expecting DL_PHYSADDR_ACK\n", argv[0]);
	...
	char *av0;
{
	...
	(void) fprintf(stderr, "Usage:  %s device ppa localsap destphys destsap n size\n", av0);
	(void) exit(1);
}

#!/bin/sh
#
# @(#) $RCSfile: strxnet.sh,v $ $Name:  $($Revision: 0.9.2.10 $) $Date: 2008-10-21 22:42:22 $
...
#
# strxnet:	start and stop strxnet subsystem
# update-rc.d:	start 33 S . stop 33 0 6 .
# config:	/etc/default/strxnet
# probe:	false
# hide:		false
# license:	GPL
...
desc="the STREAMS XNET subsystem"
mknod="${name}_mknod"

[ -e /proc/modules ] || exit 0

if test -z "$STRXNET_MKNOD" ; then
    for STRXNET_MKNOD in /sbin/${mknod} /usr/sbin/${mknod} /bin/${mknod} /usr/bin/${mknod} ; do
...
start() {
	...
	echo -n "Making STREAMS XNET devices: "
	$STRXNET_MKNOD
	if [ $? -eq 0 ] ; then
	    echo "."
	else
...
stop() {
	...
	echo -n "Removing STREAMS XNET devices: "
...
# -----------------------------------------------------------------------------
#
# Last Modified $Date: 2008-10-21 22:42:22 $ by $Author: brian $
#
# -----------------------------------------------------------------------------
#
# $Log: strxnet.sh,v $
# Revision 0.9.2.10  2008-10-21 22:42:22  brian
# - handle verbose better in debian initscripts
#
# Revision 0.9.2.9  2008-04-28 18:38:42  brian
# - header updates for release
#
# Revision 0.9.2.8  2007/08/14 04:01:30  brian
# - GPLv3 header update
#
# =============================================================================

...
int
RoBeginRequest(called, data, roc, roi)
...
static int
RoBeginRequestAux(called, data, roc, roi)
	struct RoSAPaddr *called;
	PE data;
	struct RoSAPconnect *roc;
	struct RoSAPindication *roi;
{
	...
	if (called->roa_addr.sa_selectlen == 0) {
		if ((is = getisoserventbyname("ros", "ssap")) == NULL) {
			result = rosaplose(roi, ROS_ADDRESS, NULLCP, "ssap/ros: unknown entity");
			goto out2;
		}
...
#undef	dotoken
	...
	if (paccpt->pUserData->member_OACS_5->offset == type_OACS_ConnectionData_open)
		roc->roc_data = pe_expunge(pe, paccpt->pUserData->member_OACS_5->un.open);
	else
		roc->roc_data = NULLPE;

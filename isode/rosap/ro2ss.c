...
/* begin AbortInformation PSDU (pseudo) */
	if (pe = pe_alloc(PE_CLASS_UNIV, PE_FORM_CONS, PE_CONS_SET)) {
		...
		PLOGP(rosap_log, OACS_AbortInformation, pe, "AbortInformation", 0);
...
int
ro2sswait(acb, invokeID, secs, roi)
	register struct assocblk *acb;
	int *invokeID, secs;
	...
	struct RoSAPindication *roi;
{
	...
	for (;;) {
		...
		case OK:
			...
			(void) ropktlose(acb, roi, ROS_PROTOCOL, NULLCP, "%s", ps_error(result));
		      bad_sx:;
			SXFREE(sx);
			goto out;
...
int
ro2sswrite(acb, pe, fe, priority, roi)
	register struct assocblk *acb;
	register PE pe;
	PE fe;
	int priority;
	...
	struct RoSAPindication *roi;
{
	...
	if (result != PS_ERR_NMEM)
		return rosapreject(acb, roi, ROS_GP_STRUCT, NULLCP, "%s", ps_error(result));

	(void) ropktlose(acb, roi, ROS_CONGEST, NULLCP, "%s", ps_error(result));
	...
	struct RoSAPindication *roi;
{
	int result;
	register PE pe;
	struct type_OACS_AbortInformation *pabort = (struct type_OACS_AbortInformation *) 0;
	int acsap_abort = -1;
	...
	if (result != NOTOK && (rosap_log->ll_events & LLOG_PDUS))
		pvpdu(rosap_log, print_OACS_AbortInformation_P, pe, "AbortInformation", 1);
#endif

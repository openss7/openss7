...
{
	int i, fflag;
	char *logname, buffer[BUFSIZ], *vec[NVEC + 1];
	FILE *fp;
	...
	for (i = 1; i < argc; i++) {
		if (peerhost[0] == NULL && (*argv[i] != '-')) {
			...
			adios("usage: %s [-g] [-D] [-B] [-f] [-F logfile] [hostname]", myname);
	}
	...
	int error;
{
	...
	if (!connected) {
		...
		advise(LLOG_NOTICE, NULLCP, "already associated with terminal service");
		return error;
	}
	...
	register char *name;
{
	...
	default:
		...
		advise(LLOG_NOTICE, NULLCP, "ambiguous operation, it could be one of:%s", buffer);
		return NULL;
	}
}
...
{
	if (*++vec == NULL) {
		if (getline("host: ", line) == NOTOK || str2vecX(line, vec, 0, NULLIP, NULL, 0) < 1)
			return NOTOK;
	}
	...
	char **vec;
{
	...
	/* read network events until the release sequence reached the point where the other side
	   shuts down */
	...
	char **vec;
{
	...
	if (*++vec == NULL) {
		if (getline("new escape character: ", line) == NOTOK || str2vec(line, vec) < 1)
			return NOTOK;
	}
	...
	char **choices;
{
	...
	default:
		...
		advise(LLOG_NOTICE, NULLCP, "ambiguous value, it could be one of:%s", buffer);
		return NULL;
	}
}
	...
	register char *name;
{
	...
	default:
		...
		advise(LLOG_NOTICE, NULLCP, "ambiguous variable, it could be one of:%s", buffer);
		return NULL;
	}
}
	...
	FILE *fp;
{
	...
	if (ioctl(fileno(fp), TIOCGWINSZ, (char *) &win) != NOTOK && (i = win.ws_col) > 0)
		return i;
#endif
	...
	int s;
{
	...
	for (;;) {
		...
		if (FD_ISSET(s, &ibits) && data_pending()) {
			...
			result = xselect(nfds, &ibits, &obits, (fd_set *) NULL, NOTOK);
			if (result == -1)
				adios("failed", "xselect");
		}
		if (!FD_ISSET(s, &ibits)
		...
		if (FD_ISSET(tin, &ibits)) {
			...
			if (tcc < 0 && errno == EWOULDBLOCK)
				...
				if (tcc <= 0) {
					advise(LLOG_NOTICE, NULLCP,
					       "error: read from terminal returned %d", tcc);
					break;
				}
				tbp = tibuf;
			}
	...
	int dd;
{
	register char *cp;
	int n, i, j;
	int nl_flag;			/* If current PDU includes newline, follow it with a
					   Deliver Request */

...
#undef PTYBUG			/* When testing Break and demon not started from rc.local, this
				   turns off resetting local echo so ioctl to pty will not cause
				   demon to hang up. */
	...
	int result;
{
	...
	my_version = 0x01;
	if (G_Func_Units & destBreak)
		do_break = 1;
	else
		do_break = 0;
	my_fu = G_Func_Units & destBreak;	/* VT-Break is only Functional Unit we will accept */
	bzero((char *) &ud, sizeof ud);
	ud.valid_reason = 0;
	ud.result = result;
	ud.valid_imp = 0;
	ud.valid_coll = 1;
	if (vcwa == TRUE)
		ud.coll_winner = ACCEPTOR;
	else
		ud.coll_winner = INITIATOR;
	ud.version.bitstring = my_version;
	ud.version.bitcount = 1;
	if (!strcmp(vtp_profile.profile_name, "telnet")) {
		...
		ud.arg_list.cds_val[0].x_dim.window = vtp_profile.arg_val.tel_arg_list.x_window;
	...
	if (build_ASRPDU_ASRpdu((PE *) & a_resp, 1, NULL, NULLCP, (PEPYPARM) & ud) == NOTOK)
		advise(LLOG_NOTICE, NULLCP, "ASR build failure (%s) -- continuing", PY_pepy);
	...
	TEXT_UPDATE ud;
{
	...
	if (p_ondq == NULLPE) {	/* Nothing waiting to be sent */
		...
		if (seq_add(p_ondq, vtsdip, -1) == NOTOK)
			adios(NULLCP, "NDQ build failure (%s)", pe_error(p_ondq->pe_errno));
	}
}

...
getch()
{
	...
	cbuf.queued--;
	if (debug > 1)
		advise(LLOG_DEBUG, NULLCP, "normal return from getch, c is %c,queued is %d", c,
		       cbuf.queued);
	return (c);
}
...
data_pending()
{
	...
	/* if there is no data left and get_event resulted in the association being released */
	if (!connected) {
		(void) putch(EOF);
		return (TRUE);
	}
	...
	char c;
{
	if (debug > 1) {
		...
		advise(LLOG_DEBUG, NULLCP, "cbuf.buf is %d, cbuf.head is %d, cbuf.tail is %d",
		       (int) cbuf.buf, (int) cbuf.head, (int) cbuf.tail);
	...
	int ack;
{
	...
	if (ack)
		adios(NULLCP, "ACK requests in deliver PDUs not supported at this time");
...

read_asq(pe)			/* Unwrap ASQ PDU.  Use information it contains to fill in some
				   global values (profile_id,G_Func_Units,vcwa). Return 0 if ASQ is 
				   improperly formatted or missing a required field.  For now, only 
				   the more obvious fields are checked and only transparent and
				   telnet profiles are handled.  Return PROFILE_NG if profile is
				   not supported.  Return 1 if ASQ is valid. */
	PE pe;
{
	...
	if (!oid_cmp(ud.asq_profile.prof_oid, ode2oid("telnet"))) {
		...
		if (!ud.asq_profile.cds_offer_list[D].valid_rep_list) {
			...
			if (ud.asq_profile.cds_offer_list[D].rep_offer.num_reps > MAXREPS) {
				advise(LLOG_DEBUG, NULLCP, "ASQ with too many repertoires");
				return (0);
			}
			for (i = 0; i < ud.asq_profile.cds_offer_list[D].rep_offer.num_reps; i++) {
				if (ud.asq_profile.cds_offer_list[D].rep_offer.repertoire[i].
				    rep_type != 2)
					continue;
				if (!strncmp
				    (ud.asq_profile.cds_offer_list[D].rep_offer.repertoire[i].
				     rep_assign, ascii_go_repertoire,
				     sizeof(ascii_go_repertoire))) {
				...
				if (!strncmp
				    (ud.asq_profile.cds_offer_list[D].rep_offer.repertoire[i].
				     rep_assign, full_ascii_repertoire,
				     sizeof(full_ascii_repertoire))) {
					vtp_profile.arg_val.tel_arg_list.full_ascii = 1;
					break;
				}
			}
		...
		if (ud.asq_profile.cds_offer_list[D].x_dim.window.type == 0) {	/* If single value */
			vtp_profile.arg_val.tel_arg_list.x_window =
			...
			/* If range */
		{
			if ((ud.asq_profile.cds_offer_list[D].x_dim.window.min_val
			     <= 80) &&
			    (ud.asq_profile.cds_offer_list[D].x_dim.window.max_val >= 80)) {
		...
		if (vtp_profile.arg_val.tel_arg_list.full_ascii < 0) {
			advise(LLOG_DEBUG, NULLCP,
			       "Using Default for ASCII repertoire (Full ASCII)");
			vtp_profile.arg_val.tel_arg_list.full_ascii = 1;
		}
	} else {
	...
	PE pe;
{
	...
	if (!strcmp(vtp_profile.profile_name, "transparent")) {
		if (!udr.arg_list.cds_val[0].valid_rep_list) {	/* No repertoires */
			if (strcmp(vtp_profile.arg_val.tr_arg_list.cur_rep, TRANSPARENT))
		...
		{
			advise(LLOG_DEBUG, NULLCP,
			       "ASR--Invalid repertoire for transparent profile");
			return (NOTOK);
		}
...
*/
	...
	if (AcAssocRequest(ctx, NULLAEI, aei, NULLPA, pa,
			   &vclist, pci,
			   0, srequirements, SERIAL_MIN, 0, sf, &data, 1, &qos, acc, aci) == NOTOK)
...
#ifdef	DEBUG
	{
		...
		advise(LLOG_DEBUG, NULLCP, "context: %s", oid2ode(acc->acc_context));

		advise(LLOG_DEBUG, NULLCP,
		       "responding AE title: %s, responding PSAP address: %s",
		       sprintaei(&acc->acc_respondtitle), paddr2str(&pc->pc_responding, NULLNA));
	...
	char **argv;
{
	...
	advise(LLOG_NOTICE, NULLCP,
	       "  %s, %d, %d>", sprintb(ps->ps_srequirements, RMASK), ps->ps_isn, ps->ps_ssdusize);

	(void) strcpy(peerhost, na2str(ps->ps_calling.pa_addr.sa_addr.ta_addrs));
	...
	PE brk_pe;
{
	...
	if (telnet_profile) {
		...
		ptyecho(0);
#else
		setmode(0, ECHO);	/* Return to Local Echo.  This call is not the same for
					   user (vtp) and server (vtpd) so for now, VT-BREAK can
					   only be requested at user side. */
#endif

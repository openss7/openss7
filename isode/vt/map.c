...
map(ndq)			/* Parse the given NDQ (could contain several updates). Pass
				   individual updates to appropriate processing routine. */
	PE ndq;
{
	...
	DO_UPDATE *doptr;
{
	...
	case DO_NEXT_X:
		...
		if (my_right == INITIATOR) {
			if (putch('\n') == NOTOK) {	/* Current Telnet only gives CR to PTY */
				advise(LLOG_NOTICE, NULLCP, "DROPPED CHAR");
				return;
			}
		}
	...
	case DO_PTR_REL:	/* Ignore for TELNET since next update must be erase. */
		break;

	case DO_PTR_ABS:	/* Ignore for TELNET since must have been preceeded by erase line. */
		break;
		...
		if ((doptr->do_cmd.erase.start_erase.ptr_type == 0) &&
			...
			if (my_right == ACCEPTOR) {
				...
				if (tcgetattr(pty, &term) == -1)
					...
					(void) putch(erase_char = term.c_cc[VERASE]);	/* XXX what 
											   if
											   _POSIX_VDISABLE 
											 */
#else
			...
			if (my_right == ACCEPTOR) {
				...
				if (tcgetattr(pty, &term) == -1)
					...
					(void) putch(erase_line = term.c_cc[VKILL]);	/* XXX what 
											   if
											   _POSIX_VDISABLE 
											 */
#else
	...
	CO_UPDATE *coptr;
{
	...
	if (!telnet_profile) {
		if ((my_right == INITIATOR) && (!strcmp(coptr->co_name, "E")))
			...
			advise(LLOG_NOTICE, NULLCP,
			       "Received Invalid CO Update under Default Profile\n");
		return;
	}
	...
	if (coptr->co_cmd.bool_update.mask_count == 0)
		...
		active = *coptr->co_cmd.bool_update.mask;
				...
				/* Update to Echo Control Object */
			{
					...
					/* Request from Server for Remote Echo */
				{
					...
					if (ECHO_OBJ & nego_state) {	/* If now in Remote Echo */
						if (ni_image & ECHO_OBJ) {	/* No request
										   outstatnding */
							if (showoptions)
								(void)
								    printf
								    ("Server Request ignored--Now in Remote echo\r\n");
						} else {
							if (showoptions)
								(void)
								    printf
								    ("Request for Local Echo Denied by Server\r\n");
							ni_image |= ECHO_OBJ;
						}
					} else {	/* Else Not in Remote Echo */

						if (ni_image & ECHO_OBJ)
						{	/* I Requested Remote Echo */
							/* This must be confirmation */
							if (showoptions)
								(void)
								    printf
								    ("Server agreed to do Remote Echo\r\n");
						} else {	/* Request to do Remote Echo */

							if (showoptions)
								(void)
								    printf
								    ("Server Requested Remote Echo\r\n");
							ni_image |= ECHO_OBJ;
							vt_set_nego(ni_image, ECHO_OBJ);	/* Respond 
												   "WILL" */
						}
						(void) tmode(1);
						nego_state |= ECHO_OBJ;
					...
					if (nego_state & ECHO_OBJ) {	/* If now in Remote Echo */
						...
						{	/* If no request pending */
							...
							if (showoptions)
								(void)
								    printf
								    ("Server requested Local Echo -- O.K.\r\n");
							ni_image &= ~ECHO_OBJ;
							vt_set_nego(ni_image, ECHO_OBJ);	/* Respond 
												   "WILL" */
						} else {
							if (showoptions)
								(void)
								    printf
								    ("User request for Local Echo Accepted\r\n");
						}
						nego_state &= ~ECHO_OBJ;
						/* sb = ottyb; /* sb.sg_flags |= ECHO|CRMOD|CBREAK;
						...
						{	/* If requeset pending */
							...
							if (showoptions)
								(void)
								    printf
								    ("Request for Remote Echo Denied by Server\r\n");
						} else {	/* Else no request pending */

							if (showoptions)
								(void)
								    printf
								    ("Server Request Ignored--Now in Local Echo\r\n");
						}
					}
				}
			}
				...
				/* Update to Suppress Go Ahead Control Object */
			{
				if (SUP_GA & *coptr->co_cmd.bool_update.value) {
						...
						/* If no outstanding request from User */
					{
						...
						{
							ni_image |= SUP_GA;
							vt_set_nego(ni_image, SUP_GA);	/* Reply
											   "Will" */
						}
					}
						...
						/* Must be request from Server */
					{
						...
						if (showoptions)
							(void)
							    printf
							    ("Server refuses to Suppress Go Ahead\r\n");
				...
				/* Update to WACA Binary Repertoire */
			{
				if (DISP_BIN & *coptr->co_cmd.bool_update.value) {
					if (showoptions)
						(void)
						    printf
						    ("WACA requested Binary Repertoire on DI\r\n");
					if ((ni_image & DISP_BIN) == (nego_state & DISP_BIN))
						/* No request outstanding from Initiator */
					{
						if (!(nego_state & DISP_BIN)) {	/* If not now
										   binary */
							ni_image |= DISP_BIN;
							vt_set_nego(ni_image, DISP_BIN);	/* Send 
												   "Will" */
						}
					}
					nego_state |= DISP_BIN;
					...
					if (showoptions)
						(void)
						    printf
						    ("WACA requested ASCII Repertoire on DI\r\n");
					if ((ni_image & DISP_BIN) == (nego_state & DISP_BIN)) {
				...
				/* Update to WACI Binary Repertoire */
			{
				if (KBD_BIN & *coptr->co_cmd.bool_update.value) {
					if (showoptions)
						(void)
						    printf
						    ("WACA requested Binary Repertoire on KB\r\n");
					if ((ni_image & KBD_BIN) == (nego_state & KBD_BIN))
						/* If no initiator request outstanding */
					{
						if (!(nego_state & KBD_BIN)) {	/* If not now
										   binary */
							ni_image |= KBD_BIN;
							vt_set_nego(ni_image, KBD_BIN);	/* Reply
											   "Will" */
							switch_rep(2);
							/* Send Attribute update to use Binary
							   Repertoire */
						}
					...
					if (showoptions)
						(void)
						    printf
						    ("Acceptor requested ASCII Repertoire on KB\r\n");
					if ((ni_image & KBD_BIN) == (nego_state & KBD_BIN))
			...
			/* Server receives updates to the Keyboard */
		{
				...
				/* If This CO contains potential update to Are You There bit */
			{
					...
					/* If this bit was toggled */
				{
					...
					if (vt_text
					    ("[associated with terminal service on ",
					     strlen("[associated with terminal service on ")) != OK)
				...
				/* Update to Echo Control Object */
			{
					...
					/* Request from User for Remote Echo */
				{
					...
					if (showoptions)
						(void) printf("Remote Echo Update Received\n");
					if (ECHO_OBJ & nego_state) {	/* If now in Remote Echo */
						if (na_image & ECHO_OBJ)	/* No request
										   outstatnding */
							advise(LLOG_NOTICE, NULLCP,
							       "User Request ignored--Now in Remote echo");
						else {	/* Must be user's response to a request */

							if (showoptions)
								(void)
								    printf
								    ("Request for Local Echo Denied by User\n");
							na_image |= ECHO_OBJ;
						}
					} else {	/* Else Not in Remote Echo */

						if (na_image & ECHO_OBJ)
						{	/* I Requested Remote Echo */
							/* This must be confirmation */
							if (showoptions)
								(void)
								    printf
								    ("User agreed to do Remote Echo\n");
						} else {	/* Request to do Remote Echo */

							if (showoptions)
								(void)
								    printf
								    ("User Requested Remote Echo--O.K.\n");
							na_image |= ECHO_OBJ;
							vt_set_nego(na_image, ECHO_OBJ);	/* Respond 
												   "WILL" */
						}
#ifdef TERMIOS
						realptyecho(1);
...
#endif
						nego_state |= ECHO_OBJ;
						cur_emode = NOT_ECHO_NOW;	/* Don't Want user
										   to Echo */
					}
					...
					if (nego_state & ECHO_OBJ) {	/* If now in Remote Echo */
						...
						{	/* If no request pending */
							...
							if (showoptions)
								(void)
								    printf
								    ("User requested Local Echo -- Denied\n");
#endif
							...
							if (showoptions)
								(void)
								    printf
								    ("Server request for Local Echo Accepted\n");
							nego_state &= ~ECHO_OBJ;
#ifdef TERMIOS
							ptyecho(0);
#else
						...
						{	/* If requeset pending */
							...
							if (showoptions)
								(void)
								    printf
								    ("Request for Remote Echo Denied by User\n");
						} else {	/* Else no request pending */

							if (showoptions)
								(void)
								    printf
								    ("User Request Ignored--Now in Local Echo\n");
						}
					}
				}
			}
				...
				/* Update to Suppress Go Ahead Control Object */
			{
				if (SUP_GA & *coptr->co_cmd.bool_update.value) {
						...
						/* If no request from Acceptor outstanding */
					{
						...
						{
							na_image |= SUP_GA;
							vt_set_nego(na_image, SUP_GA);	/* Reply
											   "Will" */
						}
					}
						...
						/* Must be request from Initiator */
					{
						...
						if (showoptions)
							(void)
							    printf
							    ("User refuses to Suppress Go Ahead\n");
				...
				/* Update to WACI Binary Repertoire */
			{
				if (DISP_BIN & *coptr->co_cmd.bool_update.value) {
					if (showoptions)
						(void)
						    printf
						    ("Initiator requested Binary Repertoire on DI\n");
						...
						/* No request outstanding from Acceptor */
					{
						if (!(nego_state & DISP_BIN)) {	/* If not now
										   binary */
							na_image |= DISP_BIN;
							vt_set_nego(na_image, DISP_BIN);	/* Send 
												   "Will" */
							switch_rep(2);
						}
					...
					if (showoptions)
						(void)
						    printf
						    ("Initiator requested ASCII Repertoire on DI\n");
					if ((na_image & DISP_BIN) == (nego_state & DISP_BIN)) {
						if (nego_state & DISP_BIN) {	/* If not now ASCII */
							na_image &= ~DISP_BIN;
				...
				/* Update to WACI Binary Repertoire */
			{
				if (KBD_BIN & *coptr->co_cmd.bool_update.value) {
					if (showoptions)
						(void)
						    printf
						    ("Initiator requested Binary Repertoire on KB\n");
						...
						/* If no Acceptor request outstanding */
					{
						if (!(nego_state & KBD_BIN)) {	/* If not now
										   binary */
							na_image |= KBD_BIN;
							vt_set_nego(na_image, KBD_BIN);	/* Reply
											   "Will" */
						}
					}
					na_image |= KBD_BIN;
					...
					if (showoptions)
						(void)
						    printf
						    ("Initiator requested ASCII Repertoire on KB\n");
					if ((na_image & KBD_BIN) == (nego_state & KBD_BIN))
	...
	if (!strcmp(coptr->co_name, "SY")) {	/* SYNCHRONIZE CO can be written by Initiator or
						   Acceptor */
		if (active & SYNC)
			/* Potential Update to Synch */
		{
			if ((SYNC & *coptr->co_cmd.bool_update.value) != (SYNC & sync_image)) {
	...
	if (!strcmp(coptr->co_name, "GA")) {
		...
		{
			if ((GO_AHEAD & *coptr->co_cmd.bool_update.value) != (GO_AHEAD & ga_image)) {
				if (debug)
		...
		/* If switching repertoires */
	{
			...
			/* If Modal extent */
		{
			if (doptr->do_cmd.wrt_attrib.attr_val == 1) {
				...
				(void) printf("Attribute for unavailable repertoire\n");
		} else
			(void) printf("Attribute update with invalid extent (%d)\n",
...
kill_proc()
{				/* Terminate current UNIX process using UNIX interrupt char */
#ifdef TERMIOS
	struct termios term;

...
int
a1_0(pe)
		    /* NDQ-ntr in states 50B or 51Q (Release awaiting peer) */
 /* Also RTQ in state 51T (Release awaiting peer) */
	PE pe;
{
	/* If quarantined data delivery were supported, we would do: /* ++vnt; /* enque(pe); /*For
	   now, only supporting simple or no delivery control, so give pe /*to application here. */
...
a1_2(pe)				/* VASSreq from user in state 01 (No Association) */
	PE pe;
{
	dr_pm_st = DEFINED;	/* Draft-VTE param. status = Defined /*(SetVTPmS(P) in 9041) */
	/* Set draft-VTE parameter values per profile (SetVTPmV(P) in 9041) */

	asq(pe);		/* Send the ASQ PDU */

	vsmd = 0;		/* Very TEMPORARY -- A-Mode only Should be done in VT-user or
				   application call */
	if (vsmd)
		state = S1_02S;
	else
...

int
a1_3(pe)				/* VASSrsp from user in state 03B (Associate -- Awaiting
					   user) */
	PE pe;
{
	PE p;
	...
	if (vrsl == FAILURE) {
		...
		/* Set draft-VTE parameters according to list in primitive /* (SetVTPmV(L) in 9041)
		   /*Set status of draft-VTE parameters above to defined /* (SetVTPmSDe(L) /*Set
		   current VTE from draft VTE (SetCuVTE) */
...

int
a1_4(pe)				/* VASSrsp from user in state 03S (Associate -- Awaiting
					   user) */
	PE pe;
{
	PE p;
...
/* ARGSUSED */
int
a1_5(pe)				/* RTQ (Token Request) in state 10B (Environment not
					   agreed) */
	PE pe;
{
...
int
a1_7(pe)
		    /* VRELreq from user in state 10B (Env. not agreed) */
 /* GTQ in 50B */
	PE pe;
{
	if (vtok) {
...
a1_10(pe)				/* VRQTreq(request token) n state 10N */
	PE pe;
{
	request_token();	/* TEMP -- Need an ISODE call to really do this since there is no
				   VTP PDU */

/*Probably need to free the NullPE that triggered this*/

	state = S1_10N;		/* Should be here already.  Do this to follow the spec literally */
	return (OK);
}

/* ARGSUSED */
	...
	PE pe;
{
	...
	if (vrsl == FAILURE) {
		...
		/* Set draft-VTE param. according to list in primitive or protocol /*element
		   (SetVTPmV(L)). /*Set status of draft-VTE params. listed in primitive or protocol
		   /*element to defined (SetVTPmSDe(L)) /*Set current-VTE from draft-VTE (SetCuVTE) */
	...
	PE pe;
{
	...
	if (vsmd)
		state = S1_03S;	/* If S-Mode */
	else
		state = S1_03B;
	result = vassind(pe);	/* doesn't really use pe but for consistency with version 1 */
	return (result);
}

int
a1_18(pe)				/* UDQ (uncontrolled data) in 51T (Release Awaiting Peer) */
	PE pe;
{
	vdatind(SEQUENCED, pe);	/* Want to do VDATind-h but this is all that's now available */
	return (OK);
}

/* ARGSUSED */
...

int
a1_20(pe)				/* RLR (Release Response) in 51Q or 51T (Release Awaiting
					   Peer) */
	PE pe;
{
	...
	if (vrsl == SUCCESS) {
		/* VRELcnf -- Confirm the release to user -- for now, use the original mechanism
		   (closing TELNET) -- should be changed especially for forms mode */

		vrelcnf();
		if (vnt > 0)
			/* Should never happen until Quarantined Delivery supported */
		{
			/* VDATind-n(Vnt) */
			vnt = 0;
		}
...

int
a1_21(pe)				/* DLQ (Deliver Request) in 50B or 51Q (Release Awaiting
					   Peer) */
	PE pe;
{
	...
	if (vra) {
		vrsl = FAILURE;
		vrea = COLL_DET;
		/* VRELcnf required in spec but there's really nothing to tell the user */
	}
	...
	PE pe;
{
	...
	vrsl = FAILURE;
	vrea = COLL_DET;
	/* VRELcnf -- Confirm to user telling of failure due to collision -- but user can't do
	   anything now anyway. */
...
a1_23(pe)				/* SNQ (Start negotiation) in 50B */
	PE pe;
{
	/* Switch Negotiation not implemented. Should probably send back negative acknowledgement. */
...
a1_24(pe)				/* SPQ (Switch Profile Request) in state 50B */
	PE pe;
{
	/* Profile Switch not implemented. Should probably send back negative acknowledgement. */
	...
	PE pe;
{
	...
	/* Probably some ISODE call to give token directly instead of telling user */
...
a1_28(pe)				/* SNQ (Start Negotiation) in 10N */
	PE pe;
{
	/* MIN not implemented. Need to return NAK. */
...
a1_29(pe)				/* SNQ (Start Negotiation) in 10B */
	PE pe;
{
	/* MIN not implemented. Need to send NAK to peer. */
...
a1_30(pe)				/* SPQ (Switch Profile Request) in 10B & 10N */
	PE pe;
{
	/* Switch Profile not implemented. Should send NAK to peer. */
	...
	PE pe;
{
	...
	/* 1 is value for Local Error.  0 if for Protocol Error.  Assume 1 for now. */
	if (pe_apq == NULLPE)

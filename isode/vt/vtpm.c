	...
	PE *pe;
{
	...
	case NOTOK:
		...
		if ((pi.pi_type == PI_ABORT) && PC_FATAL(pi.pi_abort.pa_reason))
	...
	case DONE:
		if (pi.pi_type == PI_FINISH) {
			...
			adios(NULLCP, "PReadRequest returned DONE, but event unknown (%d)",
			      pi.pi_type);
		break;
	case OK:
		if (px.px_ninfo > 1)
		...
		case NDQ_PDU:
		{
			...
			/* We're supposed to find out if the NDQ contains an update to a triggered
			   control object or not to determine what kind of event we have.  Right
			   now we'll assume that we do have such an update in all cases.  Note that 
			   this may be a problem if we use quarantine delivery control in the
			   future.

			   for each update, find out if the update is for a display object or for a 
			   control object. if it's a control object get the name of it and find
			   out if it has a trigger
		...
		{
			if (debug)
				advise(LLOG_NOTICE, NULLCP, "Got HDQ");
			event = HDQ;
			break;
		}
...
s1_02B,
	    s1_02S, s1_03B, s1_03S, s1_10B, s1_10N, s1_10T, s1_50B, s1_51Q, s1_51R, s1_51N, s1_51T};
...
int (*s5[]) () = {
s5_400B, s5_402B, s5_420B, s5_422B, s5_40N, s5_40T, s5_42N, s5_42T, s5_61, s5_62};

int ((**sectors[])()) = {
s0, s1, s2, s3, s4, s5};

unsigned state = 0, sector = 1;
	...
	PE pe;
{
	if (debug)
		advise(LLOG_DEBUG, NULLCP,
		       ...
		       sector, state, event,
		       event >= 0 && event < sizeof eventname / sizeof eventname[0]
...

send_bad_asr(reason)
    /* Compose and send ASR with result = failure.  Encode ASR-FailureReason using the reason
       parameter (0 means no reason). */
	int reason;
{
...

send_rlr(pe)			/* Send RLR (Release Response) PDU to peer.  The RLR is built by
				   vrelrsp().  It is sent by a call to Association Control. */
	PE pe;
{
	pe->pe_context = 1;
...

clear_vte()
{				/* Clear VT Environment.  */

	/* Nothing to do for now since we have no formalized environment and we exit VTP when
	   association ends. */
}

vgvt_ind()
{				/* Indication to User that peer has given the token */

	/* Don't know how to indicate this to user yet */
}

vrtq_ind()
{				/* Indicate to User that peer has requested token */

	/* Don't know how to give indication to user. Synchronous? Asynch interrupt??? */
}

give_token()
{				/* Transfer Token to peer.  For VTP, all tokens are given at once
				   so no need to discriminate between them. */
...

send_all()
{				/* TEMP -- Should be supplied by Sector 5 actions */
	...
	char *event;
{
	...
	if (aa->aca_cc > 0)
		(void) sprintf(buffer, "[%s] %*.*s",
			       AcErrString(aa->aca_reason), aa->aca_cc, aa->aca_cc, aa->aca_data);
	else
		(void) sprintf(buffer, "[%s]", AcErrString(aa->aca_reason));

	advise(LLOG_NOTICE, NULLCP, "%s: %s (source %d)", event, buffer, aa->aca_source);
}
	...
	char *event;
{
	...
	if (pab->pa_cc > 0)
		(void) sprintf(buffer, "[%s] %*.*s",
			       PErrString(pab->pa_reason), pab->pa_cc, pab->pa_cc, pab->pa_data);
	else

...
#ifndef __VT_VTPM_H__
#define __VT_VTPM_H__
...
PE pre, pwe;
PE mkdeliver();
int fd, readfds, writefds, exfds, sd, connected, netfile;
...
int vns, allpmde,			/* all draft VTE parameters defined */
 ...
 dr_pm_st,				/* draft parameter status: DEFINED, UNDEFINED, OFI,
					   (offered, initiator) OFR, (offered, responder) COFI,
					   (counter-offered, initiator) COFR, (counter-offered,
					   responder) INVI, (invited, initiator) INVR (invited,
					   responder) */
 pracc, vtpma, vcwa,			/* whether the collision winner right is owned */
 ...
 vacp,					/* action proposal: switch, restore, or responder choice */
 vrea,					/* failure reason: collision detected or profile not
					   supplied */
 vrjo,					/* rejection option: retain, discard, or responder choice */
...
s1_02B(),
s1_02S(),
s1_03B(), s1_03S(), s1_10B(), s1_10N(), s1_10T(), s1_50B(), s1_51Q(), s1_51R(), s1_51N(), s1_51T();

int s5_400B(),
s5_402B(), s5_420B(), s5_422B(), s5_40N(), s5_40T(), s5_42N(), s5_42T(), s5_61(), s5_62();
...
int connected;				/* TEMP -- for sector 1 testing only -- will be supplied by 
					   VTP */

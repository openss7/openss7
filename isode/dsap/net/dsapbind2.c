	...
	struct DSAPindication *di;
{
	...
	case DS_CTX_X500_DAP:
		if ((ds->ds_pctx_id = check_dap_ctxlist(&(acs->acs_start.ps_ctxlist))) == NOTOK) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("Unacceptable Abstract Syntaxes for X.500 DAP"));
	...
	case DS_CTX_X500_DSP:
		if ((ds->ds_pctx_id = check_dsp_ctxlist(&(acs->acs_start.ps_ctxlist))) == NOTOK) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("Unacceptable Abstract Syntaxes for X.500 DSP"));
	...
	case DS_CTX_QUIPU_DSP:
		if ((ds->ds_pctx_id = check_qsp_ctxlist(&(acs->acs_start.ps_ctxlist))) == NOTOK) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("Unacceptable Abstract Syntaxes for QUIPU DSP"));
	...
	case DS_CTX_INTERNET_DSP:
		if ((ds->ds_pctx_id = check_isp_ctxlist(&(acs->acs_start.ps_ctxlist))) == NOTOK) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("Unacceptable Abstract Syntaxes for Internet DSP"));
	...
	if (decode_DAS_DirectoryBindArgument(acs->acs_info[0], 1, NULLCP, NULLIP, &bind_arg) != OK) {
...
int
DBindResult(sd, context, respondtitle,
	...
	struct DSAPindication *di;
{
	...
	if (encode_DAS_DirectoryBindResult(&(bindrespe), 1, NULLIP, NULLCP, bind_res) != OK) {
		/* RoBindReject ?? */
...
int
DBindError(sd, context, respondtitle, respondaddr, ctxlist,
	   defctxresult, prequirements, srequirements, isn, settings, ref, bind_err, pctx_id, di)
	int sd;
	OID context;
	AEI respondtitle;
	struct PSAPaddr *respondaddr;
	...
	struct DSAPindication *di;
{
	...
	if (encode_DAS_DirectoryBindError(&(binderrpe), 1, NULLIP, NULLCP, bind_err) != OK) {
		/* RoBindReject ?? */
	...
	result = RoBindError(sd, context,
			     ...
			     prequirements, srequirements, isn, settings, ref, binderrpe, rni);
	watch_dog_reset();
...
int
DBindReject(ds, status, reason, di)
	struct DSAPstart *ds;
	int status;
	int reason;
	struct DSAPindication *di;

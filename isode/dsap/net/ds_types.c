	...
	struct security_parms *sp;
{
	...
	if ((ret =
	     (struct security_parms *) smalloc(sizeof(struct security_parms))) ==
	...
	struct random_number *rand;
{
	...
	if ((ret =
	     (struct random_number *) smalloc(sizeof(struct random_number))) ==
	    (struct random_number *) NULL)
		return ((struct random_number *) NULL);
	...
	struct signature *sig;
{
	...
	if ((ret =
	     (struct signature *) smalloc(sizeof(struct signature))) == (struct signature *) NULL)
		return ((struct signature *) NULL);
	...
	struct extension *ext;
{
	...
	if ((ret =
	     (struct extension *) smalloc(sizeof(struct extension))) == (struct extension *) NULL)
		return ((struct extension *) NULL);
	...
	struct s_filter *flt;
{
	...
	if ((ret =
	     (struct s_filter *) smalloc(sizeof(struct s_filter))) == (struct s_filter *) NULL)
		return ((struct s_filter *) NULL);
	...
	case FILTER_NOT:
		...
		else if ((ret->flt_un.flt_un_filter =
			  filter_cpy(flt->flt_un.flt_un_filter)) == (struct s_filter *) NULL)
			return ((struct s_filter *) NULL);
		break;
	...
	struct filter_item *tgt;
{
	...
	case FILTERITEM_SUBSTRINGS:
		if (fi_sub_dup(&(src->fi_un.fi_un_substrings), &(tgt->fi_un.fi_un_substrings)) !=
		    OK)
			return (NOTOK);
		break;
	...
	default:
		DLOG(log_dsap, LLOG_DEBUG,
		     ("filter_item_dup(): unknown filter_item type %d", src->fi_type));
		return (NOTOK);
	}
	...
	struct entrymod *em;
{
	...
	if ((ret =
	     (struct entrymod *) smalloc(sizeof(struct entrymod))) == (struct entrymod *) NULL)
		return ((struct entrymod *) NULL);

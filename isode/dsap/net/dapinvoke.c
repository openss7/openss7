...
int
DapInvokeReqAux(sd, id, op, pe, di, asyn)
	int sd;
	int id;
	int op;
	PE pe;
	...
	int asyn;
{
	...
	default:
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("DapInvokeReqAux(): asyn has unknown value: %d", asyn));
		return (daplose(di, DP_INVOKE, NULLCP, "Unknown synchronicity"));
	}
}

int
DapSyncInvokeRequest(sd, id, op, pe, di)
	int sd;
	int id;
	int op;
	PE pe;
	struct DAPindication *di;
{
	...
	result = RoInvokeRequest(sd, op, ROS_SYNC, pe, id, NULLIP, ROS_NOPRIO, roi);
	...
	if (result != OK) {
		if (roi->roi_type != ROI_PREJECT) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("DapSyncInvokeRequest(): Failed without rejection"));
			return (daplose
				(di, DP_INVOKE, NULLCP, "RoInvokeRequest inconsistent result"));
		}

		if (ROS_FATAL(rop->rop_reason) || (rop->rop_reason == ROS_PARAMETER)) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("DapSyncInvokeRequest(): Fatal rejection"));
			return (daplose(di, DP_INVOKE, NULLCP, "RoInvokeRequest failed"));
		} else {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("DapSyncInvokeRequest(): Non-Fatal rejection"));
...
int
DapIntrInvokeRequest(sd, id, op, pe, di)
	int sd;
	int id;
	int op;
	PE pe;
	struct DAPindication *di;
{
	...
	if (result != OK) {
		if (roi->roi_type != ROI_PREJECT) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("DapIntrInvokeRequest(): Failed without rejection"));
			return (daplose
				(di, DP_INVOKE, NULLCP, "RoInvokeRequest inconsistent result"));
		}
		...
		if (ROS_FATAL(rop->rop_reason) || (rop->rop_reason == ROS_PARAMETER)) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("DapIntrInvokeRequest(): Fatal rejection"));
			return (daplose(di, DP_INVOKE, NULLCP, "RoInvokeRequest failed"));
		} else {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("DapIntrInvokeRequest(): Non-Fatal rejection"));
			return (dapreject(di, DP_INVOKE, id, NULLCP, "RoInvokeRequest failed"));
		}
	}
	...
	case ROI_UREJECT:
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("DapIntrInvokeRequest: Operation (%d) user rejected (%d)",
		      roi->roi_ureject.rou_id, roi->roi_ureject.rou_reason));
		return (ros2dapreject(di, "ROI_UREJECT", &(roi->roi_ureject)));

	case ROI_PREJECT:
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("DapIntrInvokeRequest: Operation (%d) provider rejected",
		      roi->roi_preject.rop_id));
		return (ros2daplose(di, "ROI_PREJECT", &(roi->roi_preject)));
...
int
DapAsynInvokeRequest(sd, id, op, pe, di)
	int sd;
	int id;
	int op;
	PE pe;
	struct DAPindication *di;
{
	...
	result = RoInvokeRequest(sd, op, ROS_ASYNC, pe, id, NULLIP, ROS_NOPRIO, roi);
	...
	if (result != OK) {
		if (roi->roi_type != ROI_PREJECT) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("DapAsynInvokeRequest(): Failed without rejection"));
			return (daplose
				(di, DP_INVOKE, NULLCP, "RoInvokeRequest inconsistent result"));
		}
		...
		if (ROS_FATAL(rop->rop_reason) || (rop->rop_reason == ROS_PARAMETER)) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("DapAsynInvokeRequest(): Fatal rejection"));
			return (daplose(di, DP_INVOKE, NULLCP, "RoInvokeRequest failed"));
		} else {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("DapAsynInvokeRequest(): Non-Fatal rejection"));
			return (dapreject(di, DP_INVOKE, id, NULLCP, "RoInvokeRequest failed"));
		}
	}
	...
	struct DAPindication *di;
{
	...
	if (encode_DAS_AbandonArgument(&ab_req_pe, 1, 0, NULLCP, &ab_arg) != OK) {
		...
		return (dapreject
			(di, DP_INVOKE, old_id, NULLCP,
			 "DapInterrupt: Abandon argument encoding failed"));
	} else {
		DLOG(log_dsap, LLOG_DEBUG, ("Abandon invoke request"));

		ret1 =
		    RoInvokeRequest(sd, OP_ABANDON, ROS_SYNC, ab_req_pe, new_id, NULLIP, ROS_NOPRIO,
				    roi1);
		...
		case OK:
			...
			case ROI_RESULT:
				if (roi1->roi_result.ror_id == old_id) {
					...
					LLOG(log_dsap, LLOG_EXCEPTIONS,
					     ("ARRGH! Abandon sent: event for neither op nor abandon op returned!!"));
					return (dapreject
						(di, DP_INVOKE, roi1->roi_result.ror_id, NULLCP,
			...
			case ROI_ERROR:
				...
				if (roi1->roi_error.roe_id == old_id) {
					...
					if (roi1->roi_error.roe_error != DSE_ABANDON_FAILED) {
						LLOG(log_dsap, LLOG_EXCEPTIONS,
						     ("Failed to abandon correctly"));
						return (dapreject
							(di, DP_INVOKE, roi1->roi_error.roe_id,
							 NULLCP, "Error mistyped for abandon"));
					} else {
						struct DSE_abandon_fail *af;

						if (decode_DAS_AbandonFailedParm
						    (roi1->roi_error.roe_param, 1, NULLIP, NULLVP,
						     &af) != OK) {
							LLOG(log_dsap, LLOG_EXCEPTIONS,
							     ("Failed to decode abandonFailed"));
							return (dapreject
								(di, DP_INVOKE, old_id, NULLCP,
								 "Abandon error decoding failed"));
						} else {
							LLOG(log_dsap, LLOG_NOTICE,
							     ("Abandon failed error!!!"));
							ab_err.dse_un.dse_un_abandon_fail = *af;	/* struct 
													   copy 
													 */
							free((char *) af);
					...
					LLOG(log_dsap, LLOG_EXCEPTIONS,
					     ("Abandon sent : event for neither op nor abandon op returned!!"));
					return (dapreject
						(di, DP_INVOKE, roi1->roi_error.roe_id, NULLCP,
						 "Unrecognised op id"));
				}
				break;
			...
			default:
				LLOG(log_dsap, LLOG_FATAL,
				     ("Unexpected roi_type : %d", roi1->roi_type));
		...
		case NOTOK:
			...
			if (ROS_FATAL(rop2->rop_reason)) {
				...
				return (dapreject
					(di, DP_ROS, -1, NULLCP, "DapInterrupt: Non-fatal reject"));
			}
		...
		default:
			LLOG(log_dsap, LLOG_FATAL,
			     ("Unknown return from RoInvokeRequest : %d", ret1));
		...
		case OK:
			...
			case ROI_RESULT:
				if (roi2->roi_result.ror_id == old_id) {
					...
					LLOG(log_dsap, LLOG_EXCEPTIONS,
					     ("ARRGH! Abandon sent and event for neither op nor abandon op returned!!"));
					return (dapreject
						(di, DP_INVOKE, roi1->roi_result.ror_id, NULLCP,
						 "Unexpected operation identifier"));
				}
				...
				if (roi2->roi_error.roe_id == old_id) {
					...
					if (roi2->roi_error.roe_error != DSE_ABANDON_FAILED) {
						LLOG(log_dsap, LLOG_EXCEPTIONS,
						     ("Failed to abandon correctly"));
						return (dapreject
							(di, DP_INVOKE, roi2->roi_error.roe_id,
							 NULLCP, "Error mistyped for abandon"));
					} else {
						...
						if (decode_DAS_AbandonFailedParm
						    (roi2->roi_error.roe_param, 1, NULLIP, NULLVP,
						     &af) != OK) {
							LLOG(log_dsap, LLOG_EXCEPTIONS,
							     ("Failed to decode abandonFailed"));
							return (dapreject
			...
			default:
				LLOG(log_dsap, LLOG_FATAL,
				     ("Unexpected roi_type : %d", roi2->roi_type));
				return (dapreject(di, DP_INVOKE, -1, NULLCP, "Unrecognised event"));
			}
			break;
		...
		default:
			LLOG(log_dsap, LLOG_FATAL,
			     ("Unknown return from RoInvokeRequest : %d", ret2));
			return (daplose(di, DP_ROS, NULLCP, "RoInvokeRequest error"));
		}
	}

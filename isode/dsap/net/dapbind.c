...
int
DapAsynBindReqAux(callingtitle, calledtitle, callingaddr,
...
int
DapAsynBindRequest(calledaddr, bindarg, dc, di, async)
	struct PSAPaddr *calledaddr;
	struct ds_bind_arg *bindarg;
	struct DAPconnect *dc;
	struct DAPindication *di;
...
int
DapAsynBindRetry(sd, do_next_nsap, dc, di)
	int sd;
	int do_next_nsap;
	struct DAPconnect *dc;
	struct DAPindication *di;
	...
	case ACS_ACCEPT:
		...
		if ((acc->acc_ninfo == 1) && (acc->acc_info[0] != NULLPE)) {
			...
			if (decode_DAS_DirectoryBindResult
				...
				dc->dc_un.dc_bind_err.dbe_type = DBE_TYPE_SERVICE;
				dc->dc_un.dc_bind_err.dbe_value = DSE_SV_UNAVAILABLE;
				(void) sprintf(dc->dc_un.dc_bind_err.dbe_data,
					       "decoding bind result failed");
				dc->dc_un.dc_bind_err.dbe_cc =
				    strlen(dc->dc_un.dc_bind_err.dbe_data);

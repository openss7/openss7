	...
	int sd;
{
	...
	if (cc)
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("%s: cn=%d, op=%d %s (%s) %s", str, sd, op, xtype, err, data));
	else
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("%s: cn=%d, op=%d %s (%s)", str, sd, op, xtype, err));
	...
	struct DSAPindication *di;
{
	...
	if (result == NOTOK) {
		...
		if (ROS_FATAL(roi->roi_preject.rop_reason)) {
			...
			return (dsapreject
				(di, DP_ROS, -1, NULLCP, "DspRespWaitRequest: Non-fatal reject"));
		}
	}
	...
	struct DSAPindication *di;
{
	...
	if (rox->rox_nolinked == 0) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("DapDecodeInvoke: Linked operation (%d) %d", sd, rox->rox_linkid));
	...
	default:
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("DapDecodeInvoke(%d): op id %d unknown!", sd, rox->rox_op));
	...
	struct DSAPindication *di;
{
	...
	if (rox->rox_nolinked == 0) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("DspDecodeInvoke: Linked operation (%d) %d", sd, rox->rox_linkid));
		DRejectRequest(sd, ROS_IP_LINKED, rox->rox_id);
		ROXFREE(rox);
		return (dsapreject(di, DP_INVOKE, -1, NULLCP, "Link op"));
	}
	...
	struct DSAPindication *di;
{
	...
	default:
		success = NOTOK;
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("DspDecodeResult (%d): op id %d unknown!", ror->ror_op, sd));
	...
	if (success == NOTOK) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("DspDecodeResult (%d): Unable to parse argument", sd));
	...
	struct DSAPindication *di;
{
	...
	if (rox->rox_nolinked == 0) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("QspDecodeInvoke: Linked operation (%d) %d", sd, rox->rox_linkid));
		DRejectRequest(sd, ROS_IP_LINKED, rox->rox_id);
		ROXFREE(rox);
		return (dsapreject(di, DP_INVOKE, -1, NULLCP, "Link op"));
	}
	...
	default:
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("QspDecodeInvoke (%d): op id %d unknown!", sd, rox->rox_op));
		DRejectRequest(sd, ROS_IP_UNRECOG, rox->rox_id);
		ROXFREE(rox);
		return (dsaplose(di, DP_INVOKE, NULLCP, "Unknown operation identifier"));
	}
	...
	if (success == NOTOK) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("QspDecodeInvoke (%d): Unable to parse argument", sd));
		DRejectRequest(sd, ROS_IP_MISTYPED, rox->rox_id);
		ROXFREE(rox);
		return (dsaplose(di, DP_INVOKE, NULLCP, "Undecodable argument"));
	}
	...
	struct DSAPindication *di;
{
	...
	default:
		success = NOTOK;
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("QspDecodeResult (%d): op id %d unknown!", sd, ror->ror_op));
		DRejectRequest(sd, ROS_RRP_UNRECOG, ror->ror_id);
		RORFREE(ror);
		return (dsaplose(di, DP_RESULT, NULLCP, "Unknown operation identifier"));
	}
	...
	if (success == NOTOK) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("QspDecodeResult (%d): Unable to parse argument", sd));
		DRejectRequest(sd, ROS_RRP_MISTYPED, ror->ror_id);
		RORFREE(ror);
		return (dsaplose(di, DP_RESULT, NULLCP, "Undecodable argument"));
	}
	...
	struct DSAPindication *di;
{
	...
	default:
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("DDecodeError (%d): op id %d unknown!", sd, roe->roe_error));
	...
	if (success == NOTOK) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("DDecodeError (%d): Unable to parse argument", sd));
...
/* ARGSUSED */
static SFD
slack_watch_dog_activate(sd)
	int sd;
{
	LLOG(log_dsap, LLOG_EXCEPTIONS,
	     ("Watchdog: blocking in %s, trying again...", watch_dog_where));

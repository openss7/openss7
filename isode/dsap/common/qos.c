	...
	int format;
{
	...
	if (a->dsa_description)
		ps_printf(ps, format == READOUT ? " (%s)" : " # %s", a->dsa_description);
}
...
{
	struct ditQoS *b;
	register struct attrsQoS *p, **q, *r;
	...
	struct ditQoS *b;
{
	...
	for (p = a->dit_attrs, r = b->dit_attrs; p && r; p = p->dit_next, r = r->dit_next)
	...
	int format;
{
	...
	ps_printf(ps, format == READOUT
		  ? "%s, completeness %s"
		  : "%s + %s", p ? p : "UNKNOWN !!!", q ? q : "UNKNOWN !!!");
}

static
ditQoS_print(ps, a, format)
	PS ps;
	struct ditQoS *a;
	int format;
{
	...
	if (a->dit_description) {
		if (format == READOUT)
			ps_printf(ps, "\n%*s(%s)", qos_indent, a->dit_description);
		else
	...
	char *str;
{
	...
	bzero((char *) a, sizeof *a);
	if (!(ptr = index(str, '#'))) {
		parse_error("missing namespace-completeness in DataQualitySyntax %s", str);
	      out:;
		ditQoS_free(a);
		return NULL;
	}
	...
	*ptr = '#';
	...
	while (*qtr) {
		...
		*q = p, q = &p->dit_next, *q = NULL;
		...
		if (!(rtr = index(qtr, '+'))) {
			parse_error("missing AttributeQuality in attributeQuality %s", qtr);
		      out2:;
			if (ttr)
				*ttr = '$';
			if (ptr)
		...
		*rtr = NULL;
		for (utr = qtr + strlen(qtr) - 1; utr >= qtr && isspace((u_char) *utr); utr--)
			continue;
		utr++;
		...
		*rtr = '+';
		if (!p->dit_type) {
			parse_error("unknown AttributeType in attributeQuality %s", qtr);
			goto out2;
		}
...
QoS_syntax()
{
	(void) add_attribute_syntax("DSAQualitySyntax",
				    ...
				    dsaQoS_print,
				    (IFP) dsaQoS_cpy,
				    dsaQoS_cmp, dsaQoS_free, NULLCP, NULLIFP, TRUE);
	(void) add_attribute_syntax("DataQualitySyntax",
				    ...
				    ditQoS_print,
				    (IFP) ditQoS_cpy,
				    ditQoS_cmp, ditQoS_free, NULLCP, NULLIFP, TRUE);
}

	...
	Saclinfo b;
{
	...
	if (a->sac_zeroifexceeded != b->sac_zeroifexceeded)
		return (a->sac_zeroifexceeded < b->sac_zeroifexceeded ? -1 : 1);
	...
	char *str;
{
	...
	*s = '\0';
	...
	if (new->sac_selector == ACL_GROUP || new->sac_selector == ACL_PREFIX) {
		/* skip past next # */
		*s++ = save;
		str = s;
		if ((s = index(str, '#')) == NULL) {
	...
	int format;
{
	...
	if (format == READOUT)
		...
		ps_printf(ps, "%s # ", acl_sel[acl->sac_selector]);
	if (acl->sac_selector == ACL_GROUP || acl->sac_selector == ACL_PREFIX) {
		if (format == READOUT)
			ps_printf(ps, "( ");
	...
	/* max results */
	if (format == READOUT)
		ps_printf(ps, "\n\t\t\t(limit %d matches", acl->sac_maxresults);
	else
	...
	int format;
{
	...
	if (acl->sac_maxresults > 0) {
		if (format == READOUT)
			ps_printf(ps, " (limit %d entries)", acl->sac_maxresults);
		else
			ps_printf(ps, " # %d", acl->sac_maxresults);
	}

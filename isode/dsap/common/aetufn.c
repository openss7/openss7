...
static PE
name2value_ufn(name, context, ontty, userdn, passwd, real_name)
	...
	PE *real_name;
{
	...
	*real_name = NULLPE;
	...
	if (!aet_match(n, v, ontty ? ufn_interact : just_say_no, &dns, el, context)) {
		if (PY_pepy[0] == NULL) {
...
set_el()
{
	...
	*ep = en, ep = &en->Next;
	en->Dns = dn_seq_push(local_dn, dn_seq_push(c_dn, dn_seq_push(NULLDN, NULLDNSEQ)));
	en->Upper = en->Lower = 1;

	if ((en = (envlist) calloc(1, sizeof **ep)) == NULL)
		goto no_mem;
	*ep = en, ep = &en->Next;
	en->Dns = dn_seq_push(c_dn, dn_seq_push(local_dn, dn_seq_push(NULLDN, NULLDNSEQ)));
	en->Upper = en->Lower = 2;

	if ((en = (envlist) calloc(1, sizeof **ep)) == NULL)
		goto no_mem;
	*ep = en, ep = &en->Next;
	en->Dns = dn_seq_push(NULLDN, dn_seq_push(c_dn, dn_seq_push(local_dn, NULLDNSEQ)));
	...
	char **argv;
{
	...
	if (pe = name2value_ufn(buffer, "iso ftam", ontty, &title)) {
		...
		if (parse_DSE_PSAPaddr(pe, 1, NULLIP, NULLVP, (char *) &pas)
		    == NOTOK)
			fprintf(stderr, "parse of presentation address failed: %s", PY_pepy);
		else
			(void) printf("%s\n", paddr2str(&pas));
	} else

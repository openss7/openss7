	...
	for (;;) {
		if (buflen <= MAXLINE) {
			if (parse_len <= 0) {
				...
				buflen += PARSE_INCR;
				if ((parse_buffer =
				     realloc(parse_buffer, (unsigned) parse_len)) == NULLCP)
					exit(2);	/* ??? */
	...
	char *str;
{
	...
	if (str && str > parse_buffer) {
		/* size will be negative if a long line has been trimmed back beyond the last
		   buffer length */
...
Entry
get_entry_aux(file, parent, dtype)
#ifdef TURBO_DISK
	GDBM_FILE file;
#else
	FILE *file;
	...
	DN dn;
{
	...
	if (database_root == NULLENTRY || database_root->e_children == NULLAVL) {
		...
		for (; dn != NULLDN; dn = dn->dn_parent) {
			...
			(void) avl_insert(&ptr->e_children, (caddr_t) new, entry_cmp,
					  avl_dup_error);
		...
		for (;;) {	/* return out */
			if ((ptr = (Entry) avl_find(kids, (caddr_t) b_rdn, entryrdn_cmp))
			    == NULLENTRY) {
				for (; dn != NULLDN; dn = dn->dn_parent) {
					if ((new = new_constructor(parent)) == NULLENTRY)
						return NULLENTRY;
					new->e_name = rdn_cpy(dn->dn_rdn);
					(void) avl_insert(&parent->e_children, (caddr_t) new,
							  entry_cmp, avl_dup_error);
					parent =
					    (Entry) avl_find(parent->e_children,

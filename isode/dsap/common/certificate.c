	...
	struct certificate *parm;
{
	...
	result->valid.not_before = strdup(parm->valid.not_before);
	result->valid.not_after = strdup(parm->valid.not_after);
	result->key.n_bits = parm->key.n_bits;
	result->key.value = malloc((unsigned) (parm->key.n_bits + 7) / 8);
	bcopy(parm->key.value, result->key.value, (parm->key.n_bits + 7) / 8);
	...
	struct certificate *a, *b;
{
	...
	/* issuer, subject, version and serial should uniquely identify the certificate. */

...
index_cmp(a, b)
	...
	Index_node *b;
{
	return (AttrV_cmp((AttributeValue) a->in_value, (AttributeValue) b->in_value));
}
...
substring_prefix_cmp(val, node, len)
	...
	int len;
{
	return (strncmp(val, (char *) (((AttributeValue) node->in_value)->av_struct), len));
}
...
substring_prefix_tel_cmp(val, node, len)
	...
	int len;
{
	return (telncmp(val, (char *) (((AttributeValue) node->in_value)->av_struct), len));
}
...
substring_prefix_case_cmp(val, node, len)
	...
	int len;
{
	return (lexnequ(val, (char *) (((AttributeValue) node->in_value)->av_struct), len));
}
	...
	struct entry *e;
{
	...
	if (pindex->i_nonlocalaliases == (struct entry **) 0) {
		pindex->i_nonlocalaliases = (struct entry **) malloc(sizeof(struct entry *) * 2);
		pindex->i_nonlocalaliases[0] = (struct entry *) 0;
	}

	/* first, check for duplicates */
	for (i = 0, tmp = pindex->i_nonlocalaliases; *tmp != (struct entry *) 0; tmp++, i++) {
		if (*tmp == e)
			return;
	}

	pindex->i_nonlocalaliases = (struct entry **) realloc((char *) pindex->i_nonlocalaliases,
	...
	struct entry *e;
{
	...
	/* first, check for duplicates */
	for (i = 0, tmp = pindex->i_nonleafkids; *tmp != NULLENTRY; tmp++, i++) {
		if (*tmp == e) {
			return;
		}
	}

	pindex->i_nonleafkids = (struct entry **) realloc((char *) pindex->i_nonleafkids,
							  (unsigned) (sizeof(struct entry *) *
								      (i + 2)));
	...
	Index *pindex;
{
	...
	for (j = i + 1; pindex->i_nonlocalaliases[j]; j++)
		pindex->i_nonlocalaliases[j - 1] = pindex->i_nonlocalaliases[j];
...
static
turbo_attr_insert(pindex, e, attr, values)
	Index *pindex;
	Entry e;
	AttributeType attr;
	AV_Sequence values;
{
	...
	for (av = values; av != NULLAV; av = av->avseq_next) {
		...
		imem->in_entries = (struct entry **) malloc(sizeof(struct entry *));
		...
		if (avl_insert(&pindex[i].i_root, (caddr_t) imem, index_cmp, index_dup) == OK) {
			pindex[i].i_count++;
			imem = NULLINDEXNODE;
	...
	Entry e;			/* the entry these attrs belong to */
{
	...
	for (as = e->e_attributes; as != NULLATTR; as = as->attr_link) {
		...
		/* sibling index */
		if (sibindex) {
			(void) turbo_attr_insert(sibindex, e, as->attr_type, as->attr_value);
		}
		...
		while (dn->dn_parent != NULLDN) {
			...
			for (prevdn = NULLDN, tmpdn = dn; tmpdn->dn_parent != NULLDN; prevdn = tmpdn, tmpdn = tmpdn->dn_parent) ;	/* NULL 
																	 */
...
static
turbo_attr_delete(pindex, e, attr, values)
	Index *pindex;
	Entry e;
	AttributeType attr;
	AV_Sequence values;
{
	...
	for (av = values; av != NULLAV; av = av->avseq_next) {
		...
		if (node == NULLINDEXNODE) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("Optimized attribute value not found! (%s)\n", attr->oa_ot.ot_name));
			continue;
		}
		...
		if (--(node->in_num) == 0) {
			...
			for (k = j; k < node->in_num; k++)
				node->in_entries[k] = node->in_entries[k + 1];
		...
		for (word = first_word((char *) av->avseq_av.av_struct);
			...
			if ((imem = (Index_node *) avl_find(pindex[i].i_sroot,
							    code,
							    index_soundex_cmp)) == NULLINDEXNODE) {
				free(code);
				continue;
			}
			...
			if (--(imem->in_num) == 0) {
				...
				for (k = j; k < imem->in_num; k++)
					imem->in_entries[k] = imem->in_entries[k + 1];
	...
	Entry e;
{
	...
	for (as = e->e_attributes; as != NULLATTR; as = as->attr_link) {
		...
		/* sibling index */
		if (sibindex) {
			(void) turbo_attr_delete(sibindex, e, as->attr_type, as->attr_value);
		}
	...
	if (sibindex && e->e_alias && th_prefix(sibindex->i_dn, e->e_alias) != -2)
	...
	char *attr;
{
	...
	if (subtree_index || sibling_index)
		fatal(-99,
		      "optimized attributes MUST be specified before subtree or sibling index");

	if (turbo_index_types == (AttributeType *) 0)
		turbo_index_types = (AttributeType *) malloc(sizeof(AttributeType *));
	else
		turbo_index_types = (AttributeType *) realloc((char *) turbo_index_types,
	...
	char *tree;
{
	...
	if (turbo_index_num == 0) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("WARNING: cannot index subtree - no attributes have been optimized"));
		return;
	}
	...
	char *parent;
{
	...
	if (turbo_index_num == 0) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("WARNING: cannot index siblings - no attributes have been optimized"));
		return;
	}

...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/dsap/common/RCS/photo_dec.c,v 9.0 1992/06/16 12:12:39 isode Rel";
#endif
	...
	int size;
{
	...
	if ((unsigned char) *data != 0xa3 && *data != 0x03) {
		if (((unsigned char) *data == 0x05) && ((unsigned char) *(data + 1) == 0x00)) {
	...
	if (pe->pe_class == PE_CLASS_UNIV && pe->pe_form == PE_FORM_PRIM && pe->pe_id == PE_PRIM_BITS) {	/* old 
														   BIT 
														   STRING-like 
														   form 
														 */
	...
	} else if (pe->pe_class == PE_CLASS_CONT && pe->pe_form == PE_FORM_CONS && pe->pe_id == 3) {
		...
		for (member = first_member(seq); member; member = next_member(seq, member)) {
			if (member->pe_class != PE_CLASS_UNIV ||
			    member->pe_form != PE_FORM_PRIM || member->pe_id != PE_PRIM_BITS) {
				(void) fprintf(stderr, "%s: is not a fax image\n", name);
				pe_free(pe);
				ps_free(ps);
				return -1;
...
int
decode_t4_aux(inbuf, winname, length, twoDimensional)
	char *inbuf;
	char *winname;
	int length;
	int twoDimensional;
...

{
	...
	while (get_bit(t4_lineptr) != 01) ;	/* skip fill characters */
...

{
	...
	do {
		...
		case -1:
			colour = undo_uncompressed_mode(code_lineptr, t4_lineptr, colour, 1);
			if (colour < 0)
				return (colour);
			break;
	...
	while (get_bit(t4_lineptr) != 1) ;	/* skip fill characters */
...

{
	...
	if (ref_colour == colour) {
		...
		/* special case when b1 = a0, and the colours are different, move b1 to the next
		   change of same colour, this must be allowed at the beginning of a line to get a
		   run of zero, as every line must start with a white element */

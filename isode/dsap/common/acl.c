...
static int
acl_attr_cmp(acl_attr1, acl_attr2)
...
static int
acl_attr_comp_cmp(acl_attr1, acl_attr2)
	struct acl_attr *acl_attr1;
	struct acl_attr *acl_attr2;
{
	int i;
...
int
acl_info_cmp(acl_info1, acl_info2)
...
static int
acl_info_comp_cmp(acl_info1, acl_info2)
	struct acl_info *acl_info1;
	struct acl_info *acl_info2;
{
	int i;
	...
	char *str;
{
	...
	if (newacl->ac_attributes != NULLACL_ATTR)
		aclptr->ac_attributes =
		    acl_attr_merge(aclptr->ac_attributes, newacl->ac_attributes);
}
	...
	register int format;
{
	if (format == READOUT) {
		...
		case ACL_GROUP:
			...
			break;
		default:
			ps_printf(ps, "%s can %s ", acl_sel[aclptr->acl_selector_type],
				  acl_cat[aclptr->acl_categories]);
		}
	} else {
		switch (aclptr->acl_selector_type) {
		case ACL_PREFIX:
		case ACL_GROUP:
			...
			break;
		default:
			ps_printf(ps, "%s # %s ", acl_sel[aclptr->acl_selector_type],
				  acl_cat[aclptr->acl_categories]);
		}
	}
}
...
acl_syntax()
{
	...
	acl_sntx = add_attribute_syntax("acl",
					...
					(IFP) acl_cpy, acl_cmp, acl_free, NULLCP, NULLIFP, TRUE);

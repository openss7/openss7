...
#ifdef STRICT_X500
						   NULLIFP,	/* Not allowed if syntax unknown
								   (X.501 9.6.2) */
#else
...
short
add_attribute_syntax(sntx, enc, dec, parse, print, cpy, cmp, sfree, print_pe, approx, multiline)
	char *sntx;
	...
	char multiline;
{
	...
	set_attribute_syntax(num_syntax, enc, dec, parse, print, cpy, cmp, sfree, print_pe, approx,
			     multiline);
...
short
modify_av_printer(at, sntx, print)
	AttributeType at;
	char *sntx;
	IFP print;
{
	...
	for (; (a != NULLRDN) && (b != NULLRDN); a = a->rdn_next, b = b->rdn_next) {
		...
		if (syntax_table[a->rdn_av.av_syntax].s_compare == NULLIFP)
			return (2);	/* can't compare */
			  ...
			  (*syntax_table[a->rdn_av.av_syntax].s_compare) (a->rdn_av.av_struct,
									  b->rdn_av.av_struct)) !=
			 0)
			return i;
	}
	...
	register AttributeValue x, y;
{
	...
	if (x->av_syntax == AV_FILE)
		return (file_cmp
			((struct file_syntax *) x->av_struct, (struct file_syntax *) y->av_struct));

	...
	PE pe;
{
	...
	if (strlen(a) != 2) {
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("Country code size wrong: \"%s\"", a));
	      losing:;
		free(a);
		return (NULLCP);
	}
	...
	LLOG(log_dsap, LLOG_EXCEPTIONS, ("Invalid country code: \"%s\"", a));
	goto losing;
}
	...
	PE pe;
{
	...
	if (PE_ID(pe->pe_class, pe->pe_id) == PE_ID(PE_CLASS_UNIV, PE_PRIM_OCTS))
		return (prim2qb(pe));
	else
	...
	PE pe;
{
	...
	if (PE_ID(pe->pe_class, pe->pe_id) == PE_ID(PE_CLASS_UNIV, PE_DEFN_IA5S))
		return (TidyString2(prim2str(pe, &z)));
	else
		return (NULLCP);
}
	...
	PE pe;
{
	...
	if (PE_ID(pe->pe_class, pe->pe_id) == PE_ID(PE_CLASS_UNIV, PE_DEFN_NUMS))
		return (TidyString2(prim2str(pe, &z)));
	else
		return (NULLCP);
}
	...
	for (; *str != 0; str++) {
		...
		default:
			LLOG(log_dsap, LLOG_NOTICE,
			     ("character '%c' not in printablestring", *str));
			char_failed = *str;
			return (0);
		}
	}
	...
	char *str;
{
	...
	if (t61_flag) {
		...
		LLOG(log_dsap, LLOG_NOTICE,
		     ("auto-convert to T.61 for '%s' ('%c' not allowed)", str, char_failed));
...
string_syntaxes()
{
	/* Don't change ordering here unless you know the side effects !!! */
	...
	exct = add_attribute_syntax("caseexactstring",
				    ...
				    (IFP) strdup, tpstrcmp, sfree, NULLCP, soundex_match, TRUE);

	tel_sntx = add_attribute_syntax("TelephoneNumber",
					(IFP) strenc, (IFP) prtsdec,
					(IFP) prtparse, strprint,
					(IFP) strdup, telcmp, sfree, NULLCP, soundex_match, TRUE);

	(void) add_attribute_syntax("printablestring",
				    (IFP) strenc, (IFP) prtsdec,
				    (IFP) prtparse, strprint,
				    (IFP) strdup, pstrcmp, sfree, NULLCP, soundex_match, TRUE);
	...
	(void) add_attribute_syntax("countrystring",
				    ...
				    (IFP) strdup, lexequ, sfree, NULLCP, soundex_match, TRUE);
	...
	(void) add_attribute_syntax("caseignorestring",
				    (IFP) strenc, (IFP) t61dec,
				    (IFP) t61parse, strprint,
				    (IFP) strdup, tlexequ, sfree, NULLCP, soundex_match, TRUE);
	...
	(void) add_attribute_syntax("numericstring",
				    ...
				    (IFP) strdup, strprint,
				    (IFP) strdup, pstrcmp, sfree, NULLCP, NULLIFP, FALSE);
	...
	(void) add_attribute_syntax("password",
				    ...
				    (IFP) strdup, passwdcmp, sfree, NULLCP, NULLIFP, TRUE);
}

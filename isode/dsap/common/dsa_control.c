...
dsa_control_syntax()
{
	(void) add_attribute_syntax("DSAControl", (IFP) dsa_control_enc, (IFP) dsa_control_decode,	/* Encoder 
													   and 
													   decoder 
													 */
				    ...
				    (IFP) dsa_control_cpy, dsa_control_cmp,	/* copy and compare 
										 */
				    (IFP) dsa_control_free, NULLCP,	/* structure_free, print -
									   READOUT */
				    NULLIFP, TRUE);	/* Approx matching, multiline output */
}

static PE
dsa_control_enc(control_option)
...
/*	format: number $ string */
	...
	case (CONTROL_RESYNCH):
	{
		struct optional_dn *tmp_opt_dn =
		    (struct optional_dn *) malloc(sizeof(struct optional_dn));
...
quipu_call_syntax()
{
	(void) add_attribute_syntax("Call", (IFP) quipu_call_enc, (IFP) quipu_call_decode,	/* Encoder 
												   and 
												   decoder 
												 */
				    ...
				    (IFP) quipu_call_cpy, quipu_call_cmp,	/* copy and compare 
										 */
				    (IFP) quipu_call_free, NULLCP,	/* structure_free, print -
									   READOUT */
				    NULLIFP, TRUE);	/* Approx matching, multiline output */
}

static PE
quipu_call_enc(control_option)
...
/* ARGSUSED */
static struct quipu_call *
str2quipu_call_aux(str, item)
	char *str;
	struct quipu_call *item;
{
	/* SPT: Forget it! The structure is horrible and should be filled in by hand! Read only
	   structure. */
	...
	struct quipu_call *item;
{
	...
	if (item->authorizationLevel != (struct auth_level *) 0) {
		tmp_item->authorizationLevel =
		    (struct auth_level *) malloc(sizeof(struct auth_level));

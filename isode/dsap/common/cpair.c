	...
	char *str;
{
	...
	/* If there isn't a '|', the pair is technically illegal. However, allow this case to mean
	   : "the string contains a certificate, which is the forward cross certificate". */
	...
	/* Need to cook up a quick test for whether a string contains a certificate or whitespace.
	   A certificate will always contain a '#', so use this. */
...
{
	(void) add_attribute_syntax("CertificatePair",
				    (IFP) cpair_enc, (IFP) cpair_dec,
				    (IFP) str2cpair, printcpair,
				    (IFP) cpair_cpy, cpair_cmp, cpair_free, NULLCP, NULLIFP, TRUE);
}

	...
	int sizelimit;
{
	...
	for (i = 0, sub = cache->list_subs; sub != NULLSUBORD; i++, sub = sub->sub_next) ;
	cache->list_count = i;
	...
	char vals;
{
	...
	if ((current_entry = local_find_entry_aux(current_dn, FALSE)) != NULLENTRY) {
		...
		if ((vals && complete) || ((current_entry->e_data == E_TYPE_CACHE_FROM_MASTER)
					   && (time((time_t *) 0) - current_entry->e_age >
					       cache_timeout))) {
			...
			current_entry->e_attributes =
			    as_merge_aux(current_entry->e_attributes, as_cpy(ptr->ent_attr));
...
Entry
local_find_entry_aux(object, deref)
	DN object;
	char deref;
{
	Entry the_entry;

	...
	char *table;
{
	...
	while (1) {		/* break out */
		...
		if (!add_entry(name, GEN)) {
		      bad_entry:;
			LLOG(log_dsap, LLOG_EXCEPTIONS, ("Can't add entry to oid table"));
			return NOTOK;
		}
	...
	char *d;
{
	...
	for (p = Pbuckets[PHASH(a)]; p && lexequ(p->p_name, a); p = p->p_chain) ;	/* NO-OP */
	...
	if ((p = (struct pair *) calloc(1, sizeof *p)) == NULL) {
		SLOG(log_dsap, LLOG_EXCEPTIONS, NULLCP, ("calloc of oid hash structure failed"));
		return FALSE;
	}
	...
	if (d != NULLCP) {
		for (p = Pbuckets[PHASH(d)]; p && lexequ(p->p_name, d); p = p->p_chain) ;	/* NO-OP 
												 */
	...
	int towho;
{
	...
	*nptr = 0;
	...
	case ATTR:
		...
		Current->ot_name = strdup(newname);
		if (!add_entry_aux
		    (Current->ot_name, (caddr_t) &attrOIDTable[attrNumEntries], ATTR, alias))
			return FALSE;
		break;
	case OC:
		...
		Current->ot_name = strdup(newname);
		if (!add_entry_aux
		    (Current->ot_name, (caddr_t) &ocOIDTable[ocNumEntries], OC, alias))
			return FALSE;
		break;
	}
	*nptr = SEPERATOR;
				...
				*ptr++ = 0;
				if (lexequ(ptr, "FILE") != 0) {
					LLOG(log_dsap, LLOG_FATAL,
					     ("FILE syntax expected, '%s' found", ptr));
					return FALSE;
				} else {
	...
	char *nodename;
{
	...
	for (p = Pbuckets[PHASH(nodename)]; p && lexequ(p->p_name, nodename); p = p->p_chain) ;	/* NO-OP 
												 */
	...
	do {
		...
		if (*buf != 0) {
			...
			if (*ptr == '\\') {
				...
				if (left <= 20) {
					buflen += LINESIZE;
					if ((buffer = realloc(buffer, (unsigned) buflen)) == NULL)
						return (NULL);
					size = strlen(buffer);
					buf = buffer + size - 1;
					left = buflen - size;
	...
	int format;
{
	...
	for (i = 0; i < NumEntries; i++, ptr++) {
		if ((oid_cmp(ptr->ot_oid, oid) == 0) || oid_cmp(ptr->ot_aliasoid, oid) == 0) {
			switch (format) {
			case OIDFULL:
				return (full_gen(ptr));
			case OIDNUM:
	...
	register char *str;
{
	...
	for (p = Pbuckets[PHASH(str)]; p && lexequ(p->p_name, str); p = p->p_chain) ;	/* NO-OP */
...
oid_syntax()
{
	(void) add_attribute_syntax("oid",
				    ...
				    (IFP) oid_cpy, oid_cmp, oid_free, NULLCP, NULLIFP, FALSE);
}

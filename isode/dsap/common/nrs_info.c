...
#ifndef lint
static char *rcsid =
    "Header: /xtel/isode/isode/dsap/common/RCS/nrs_info.c,v 9.0 1992/06/16 12:12:39 isode Rel";
#endif
...
static
str_seq_print(ps, strseq, format)
	register PS ps;
	...
	struct addr_info *arg;
{
	...
	default:
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("addr_info_cpy(): Unknown addressing info type %d", arg->addr_info_type));
		break;
	}
	...
	struct addr_info *arg2;
{
	...
	case ADDR_INFO_OSI_ADDRESSING:
		...
		if ((ret = quipu_pe_cmp(arg1->application_title, arg2->application_title)) != 0)
			return (ret);
		if ((ret =
		     quipu_pe_cmp(arg1->per_app_context_info, arg2->per_app_context_info)) != 0)
			return (ret);
		break;
	case ADDR_INFO_OSI_NSAP_ONLY:
		if ((ret = lexequ(arg1->nsap, arg2->nsap)) != 0)
	...
	default:
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("addr_info_cmp(): Unknown addressing info type %d", arg1->addr_info_type));
		break;
	}
...
static
addr_info_print(ps, info, format)
	register PS ps;
	...
	int format;
{
	...
	default:
		...
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("addr_info_print(): Unknown addressing info type %d", info->addr_info_type));
		break;
	}
}
...
static
nrs_routes_print(ps, routes, format)
	register PS ps;
...
static
addr_sp_id_print(ps, asi, format)
	register PS ps;
	int asi;
	int format;
{
...
nrs_info_syntax()
{
	(void) add_attribute_syntax("NRSInformation",
				    ...
				    nrs_info_free, NULLCP, NULLIFP, TRUE);
}

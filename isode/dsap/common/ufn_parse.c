	...
	DNS a, b;
{
	...
	for (c = a; c->dns_next != NULLDNS; c = c->dns_next) ;	/* Nothing */
	...
	char *s;
{
	...
	for (; dn->dn_parent != NULLDN; dn = dn->dn_parent) ;	/* Nothing */
	...
	DNS el;
{
		...
		NOTIFY(("DAP Search returned an error"))
			...
			case DSE_REFERRAL:
				if (err.ERR_REFERRAL.DSE_ref_candidates
				    && err.ERR_REFERRAL.DSE_ref_candidates->cr_reftype
	...
	if ((result.CSR_limitproblem != LSR_NOLIMITPROBLEM)
		...
		if (!ufn_bad_dsa && result.CSR_cr)
			ufn_bad_dsa = dn_cpy(result.CSR_cr->cr_accesspoints->ap_name);
	...
	if (!result.CSR_entries
	    && check_dnseq(ufn_partials, base) == NOTOK && present(base, at_Organisation))
...
static
rootSearch(s, interact, el, result)
	char *s;
	...
	DNS *result;
{
	...
	if (check_3166(s)) {
		if ((filta = strfilter(at_CountryName, s, FILTERITEM_EQUALITY)) == NULLFILTER)
			return 0;
		if ((filtb =
		     strfilter(at_FriendlyCountryName, s, FILTERITEM_EQUALITY)) == NULLFILTER)
			return 0;
		...
		if ((ufn_flags & UFN_APPROX) && !index(s, '*')) {
			if ((filtb =
			     strfilter(at_FriendlyCountryName, s, FILTERITEM_APPROX)) == NULLFILTER)
				return 0;
			filtb->flt_next = filte;
			if ((filtd =
			     strfilter(at_Organisation, s, FILTERITEM_APPROX)) == NULLFILTER)
				return 0;
...
static
intSearch(base, s, interact, el, result)
	DN base;
	char *s;
...
static
leafSearch(base, s, subtree, interact, el, result)
	DN base;
	char *s;
	char subtree;
...
static
keyedSearch(base, t, v, interact, el, result)
	DN base;
	char *t, *v;
	...
	DNS *result;
{
	...
	return ufn_search(base, base && base->dn_parent
			  && present(base, at_Organisation), filt, result, v, interact, el);
}

static
purportedMatch(base, c, v, interact, el, result)
	DN base;
	int c;
	char **v;
	...
	DNS *result;
{
	...
	if (c == 1) {
		...
		if ((ptr = index(s, '=')) != NULLCP) {
			*ptr++ = 0;
			matches =
			    keyedSearch(base, SkipSpace(s), SkipSpace(ptr), interact, el, result);
			*(--ptr) = '=';
			return matches;
		} else if (base == NULLDN) {
		...
		} else if (base->dn_parent == NULLDN || !present(base, at_Organisation)) {
			...
			if (*result != NULLDNS)
				...
				return leafSearch(base, s,
						  base->dn_parent == NULLDN, interact, el, result);
		} else {
	...
	if ((ptr = index(s, '=')) != NULLCP) {
		*ptr++ = 0;
		if (!
		    (matches =
		     keyedSearch(base, SkipSpace(s), SkipSpace(ptr), interact, el, &root))) {
			*(--ptr) = '=';
			return FALSE;
		}
		*(--ptr) = '=';
...
static
envMatch(c, v, el, interact, result)
	int c;
	char **v;
	DNS el;
...
static
friendlyMatch_aux(c, v, el, interact, result)
	int c;
	char **v;
	envlist el;
...
read_envlist()
{
	...
	while (fgets(buffer, LINESIZE, file) != 0) {
		...
		if (isspace(*p)) {
			...
			if (!dtail) {
				(void) sprintf(PY_pepy,
					       "Unexpected blank at start of line %d in %s", i,
					       opened);
				(void) fclose(file);
				return NULLEL;
			}
			...
			if (*p == '-')
				...
				(void) sprintf(PY_pepy, "Bad DN in environment file (%s) line %d",
					       opened, i);
				(void) fclose(file);
				return NULLEL;
			}
		...
		if ((ptr = index(p, ':')) == NULLCP) {
			(void) sprintf(PY_pepy, "':' missing in environment file (%s) line %d",
				       opened, i);
			(void) fclose(file);
			return NULLEL;
		}
		...
		*ptr++ = 0;
		...
		if (*ptr == '-')
			...
			(void) sprintf(PY_pepy, "Bad DN in environment file (%s) line %d", opened,
				       i);
			(void) fclose(file);
			return NULLEL;
		}

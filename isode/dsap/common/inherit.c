...
static
inherit_print(ps, inherit, format)
	register PS ps;
	InheritAttr inherit;
	int format;
{
	if (format == READOUT) {
		if (inherit->i_oid) {
			if (inherit->i_always) {
				ps_printf(ps, "Object class %s, ",
					  oid2name(inherit->i_oid, OIDPART));
				ps_print(ps, "ALWAYS:\n");
...
static char *
getInheritAttrs(asptr, needsoc, str)
	Attr_Sequence *asptr;
	char needsoc;
	char *str;
{
	...
	while (*ptr != 0) {
		...
		if (*ptr == ')') {
			...
			if (needsoc && as_find_type(as, octype) == NULLATTR) {
				parse_error("Required inherited attribute objectClass missing",
					    NULLCP);
				as_free(as);
				return (NULLCP);
			}
...
inherit_syntax()
{
	...
	inherit_sntx = add_attribute_syntax("InheritedAttribute",
					    ...
					    inherit_free, NULLCP, NULLIFP, TRUE);
}

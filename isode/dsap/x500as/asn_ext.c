		...
		for (; avs_temp != NULLAV;) {
			{
				...
				case 1:	/* initial */
				{
					if (encode_IF_AttributeValue
					    (p93, 0, 0, NULLCP, &avs_temp->avseq_av) == NOTOK)
						return NOTOK;
					...
					{	/* initial TAG PUSHDOWN */
						...
						if ((*p95 =
						     pe_alloc(PE_CLASS_CONT, PE_FORM_CONS,
							      0)) == NULLPE) {
							advise(NULLCP, "initial: %s",
							       PEPY_ERR_NOMEM);
							return NOTOK;
						}
						(*p95)->pe_cons = (*p93);
	...
	PE pe;
{
...

	{
		...
		if (p113->pe_class != PE_CLASS_UNIV
		    || p113->pe_form != PE_FORM_CONS || p113->pe_id != PE_CONS_SEQ) {
...

		{
			...
			if ((p116 = first_member(p113)) != NULLPE) {
				...
				{	/* type */
					...
					if (decode_IF_AttributeType
					    (p116, 1, (int *) 0, NULLVP,
					     &subs_next->fi_sub_type) == NOTOK)
						return NOTOK;
				}
			} else {
...

		{
			...
			if ((p117 =
			     (p113 != p115 ? next_member(p113, p115) : first_member(p113))) !=
			    NULLPE) {
				p115 = p117;
				...
				{	/* strings */
					...
					if (p117->pe_class != PE_CLASS_UNIV
						...
						advise(NULLCP,
						       "strings bad class/form/id: %s/%d/0x%x",
						       pe_classlist[p117->pe_class], p117->pe_form,
						       p117->pe_id);
						return NOTOK;
					}
					...
					for (p118 = first_member(p117); p118;
					     p118 = next_member(p117, p118)) {
						{
#ifdef DEBUG
...
*/
							...
							default:
								advise(NULLCP,
								       "element %s%s/%d/0x%x",
								       PEPY_ERR_UNKNOWN_CHOICE,
								       pe_classlist[p118->pe_class],
								       p118->pe_form, p118->pe_id);
								return NOTOK;
							}
						}
					}
	...
	PE pe;
{
	...
	if (explicit) {
		if (pe->pe_class != PE_CLASS_UNIV
		    || pe->pe_form != PE_FORM_CONS || pe->pe_id != PE_CONS_SET) {
	...
	if (p34 = set_find(pe, PE_CLASS_CONT, 1)) {
		...
		{		/* mandatoryObjectClasses TAG PULLUP */
			...
			if ((p36 = prim2set(p35)) == NULLPE) {
				advise(NULLCP,
				       "mandatoryObjectClasses %smandatoryObjectClasses: %s",
				       PEPY_ERR_BAD, pe_error(p35->pe_errno));
				return NOTOK;
			}
			if (p36->pe_cardinal != 1) {
				advise(NULLCP,
				       "mandatoryObjectClasses %s mandatoryObjectClasses: %d",
				       PEPY_ERR_TOO_MANY_TAGGED, p36->pe_cardinal);
				return NOTOK;
			}
		...
		}
		{
			...
			if (p35->pe_class != PE_CLASS_UNIV
			    || p35->pe_form != PE_FORM_CONS || p35->pe_id != PE_CONS_SET) {
				advise(NULLCP,
				       "mandatoryObjectClasses bad class/form/id: %s/%d/0x%x",
			...
			for (p37 = first_member(p35); p37; p37 = next_member(p35, p37)) {
				...
				if (p37->pe_class != PE_CLASS_UNIV
				    || p37->pe_form != PE_FORM_PRIM || p37->pe_id != PE_PRIM_OID) {
					advise(NULLCP, "member bad class/form/id: %s/%d/0x%x",
					       pe_classlist[p37->pe_class], p37->pe_form,
					       p37->pe_id);
					return NOTOK;
				}
...
*/
				oid_tmp = p38;
				{
					...
					if (is_first != 0) {
						(*parm) = tree_struct_alloc();
						if (((*parm)->tree_object =
						     oid2oc(oid_tmp)) == NULLOBJECTCLASS)
							(*parm)->tree_object = oc_add(oid_tmp);
						is_first = 0;
					} else {
						LLOG(log_dsap, LLOG_EXCEPTIONS,
						     ("Multiple mandatory object classes"));
					}
		...
		advise(NULLCP, "mandatoryObjectClasses %s mandatoryObjectClasses member",
		       PEPY_ERR_MISSING);
		return NOTOK;
	}
	...
	if (p34 = set_find(pe, PE_CLASS_CONT, 2)) {
		...
		{		/* optionalObjectClasses TAG PULLUP */
			...
			if ((p40 = prim2set(p39)) == NULLPE) {
				advise(NULLCP, "optionalObjectClasses %soptionalObjectClasses: %s",
				       PEPY_ERR_BAD, pe_error(p39->pe_errno));
				return NOTOK;
			}
			if (p40->pe_cardinal != 1) {
				advise(NULLCP, "optionalObjectClasses %s optionalObjectClasses: %d",
				       PEPY_ERR_TOO_MANY_TAGGED, p40->pe_cardinal);
				return NOTOK;
			}
		...
		}
		{
			...
			if (p39->pe_class != PE_CLASS_UNIV
			    || p39->pe_form != PE_FORM_CONS || p39->pe_id != PE_CONS_SET) {
				advise(NULLCP,
				       "optionalObjectClasses bad class/form/id: %s/%d/0x%x",
			...
			for (p41 = first_member(p39); p41; p41 = next_member(p39, p41)) {
				...
				if (p41->pe_class != PE_CLASS_UNIV
				    || p41->pe_form != PE_FORM_PRIM || p41->pe_id != PE_PRIM_OID) {
					advise(NULLCP, "member bad class/form/id: %s/%d/0x%x",
					       pe_classlist[p41->pe_class], p41->pe_form,
					       p41->pe_id);
					return NOTOK;
				}
	...
	if (p34 = set_find(pe, PE_CLASS_CONT, 3)) {
		...
		{		/* permittedRDNs TAG PULLUP */
			...
			if (p43->pe_cardinal != 1) {
				advise(NULLCP, "permittedRDNs %s permittedRDNs: %d",
				       PEPY_ERR_TOO_MANY_TAGGED, p43->pe_cardinal);
				return NOTOK;
			}
		...
		}
		{
			...
			if (p42->pe_class != PE_CLASS_UNIV
			    || p42->pe_form != PE_FORM_CONS || p42->pe_id != PE_CONS_SET) {
			...
			for (p44 = first_member(p42); p44; p44 = next_member(p42, p44)) {
				...
				if (p44->pe_class != PE_CLASS_UNIV
				    || p44->pe_form != PE_FORM_CONS || p44->pe_id != PE_CONS_SET) {
					advise(NULLCP, "member bad class/form/id: %s/%d/0x%x",
					       pe_classlist[p44->pe_class], p44->pe_form,
					       p44->pe_id);
					return NOTOK;
				}
				...
				for (p45 = first_member(p44); p45; p45 = next_member(p44, p45)) {
					...
					if (decode_IF_AttributeType
					    (p45, 1, (int *) 0, NULLVP, &at_tmp) == NOTOK)
						return NOTOK;
				}
			}
		}
	...
	PE *pe;
{
	...
	for (ent_tmp = (Entry) avl_getfirst(parm->gr_edb);
	     ent_tmp != NULLENTRY; ent_tmp = (Entry) avl_getnext()) {
	...
	PE pe;
{
	...
	if (pe->pe_class != PE_CLASS_UNIV
	    || pe->pe_form != PE_FORM_CONS || pe->pe_id != PE_CONS_SEQ) {
	...
	if ((p46 = prim2seq(pe)) == NULLPE) {
		advise(NULLCP, "EntryDataBlock %s%s", PEPY_ERR_BAD_SEQ, pe_error(pe->pe_errno));
		return NOTOK;
	}
	...
	for (p46 = first_member(pe); p46; p46 = next_member(pe, p46)) {
		...
		if (avl_insert(tree, tmp, entry_cmp, avl_dup_error) == NOTOK)

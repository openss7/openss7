	...
	PElementID id;
{
	...
	if (o == NULLOID
	    || o->oid_nelem <= 1
	    || o->oid_elements[0] > 2 || (o->oid_elements[0] < 2 && o->oid_elements[1] > 39))
		return NULLPE;
	...
	for (i = n = 0, ip = o->oid_elements, mp = np; i < o->oid_nelem; i++, ip++) {
	...
	dp = pe->pe_prim;
	for (i = 0, ip = o->oid_elements, mp = np; i < o->oid_nelem; i++, ip++) {
		if (ip == o->oid_elements)
			j = *ip++ * 40, i++, j += *ip;
		else
			j = *ip;

...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/ftam/RCS/ftamattr.c,v 9.0 1992/06/16 12:14:55 isode Rel";
#endif
	...
	struct FTAMindication *fti;
{
	...
	if (fa->fa_present & FA_FILENAME) {
		register struct type_FTAM_Filename__Attribute *fn, **fc;

		if (fa->fa_novalue & FA_FILENAME) {
			(void) ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "filename not present");
			goto out;
		}

		if (fa->fa_nfile > NFFILE) {
			(void) ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "too many filenames");
			goto out;
		}
		...
		for (ap = fa->fa_files, i = fa->fa_nfile - 1; i >= 0; ap++, i--) {
			if (*ap == NULL) {
				(void) ftamlose(fti, FS_GEN(fsb), 0, NULLCP,
						"empty filename at slot %d", fa->fa_nfile - i - 1);
				goto out;
			}
	...
	if (fa->fa_present & FA_CONTENTS) {
		if (fa->fa_novalue & FA_CONTENTS) {
			(void) ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "contents-type not present");
			goto out;
		}
		if (fa->fa_contents == NULLOID) {
			(void) ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "missing contents-type");
			goto out;
		}
		if ((fpm->contents__type = (struct type_FTAM_Contents__Type__Attribute *)
	...
	if (fa->fa_present & FA_ACCOUNT) {
		if (fa->fa_account == NULL) {
			(void) ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "missing account");
			goto out;
		}
		if ((fpm->storage__account = (struct type_FTAM_Account__Attribute *)
		...
		if (fa->fa_novalue & FA_ACCOUNT)
			...
			fpm->storage__account->offset = type_FTAM_Account__Attribute_actual__values;
...
#define	dodate(flag,field,tag) \
	...
	dodate(FA_DATE_CREATE, &fa->fa_date_create, date__and__time__of__creation);
	dodate(FA_DATE_MODIFY, &fa->fa_date_modify, date__and__time__of__last__modification);
	dodate(FA_DATE_READ, &fa->fa_date_read, date__and__time__of__last__read__access);
...
#define	douser(flag,field,tag,name) \
	...
	douser(FA_ID_CREATE, fa->fa_id_create, identity__of__creator, "identity-of-creator");
	douser(FA_ID_MODIFY, fa->fa_id_modify,
	       identity__of__last__modifier, "identity-of-last-modifier");
	douser(FA_ID_READ, fa->fa_id_read, identity__of__last__reader, "identity-of-last-reader");
	douser(FA_ID_ATTR, fa->fa_id_attribute,
	       identity__of__last__attribute__modifier, "identity-of-last-attribute-modifier");
#undef	douser

	if (fa->fa_present & FA_AVAILABILITY) {
		if ((fpm->file__availability = (struct type_FTAM_File__Availability__Attribute *)
		...
		if (fa->fa_novalue & FA_AVAILABILITY)
			...
			case FA_AVAIL_IMMED:
			case FA_AVAIL_DEFER:
				fpm->file__availability->un.actual__values = fa->fa_availability;
				break;
...
#undef	dosize
	...
	if (fa->fa_present & FA_CONTROL) {
		if ((fpm->access__control = (struct type_FTAM_Access__Control__Attribute *)
	...
	if (fa->fa_present & FA_PRIVATE) {
		(void) ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "private-use attribute not supported");
		goto out;
	}
	...
	struct FTAMindication *fti;
{
	...
	if (fpm->contents__type) {
		...
		if (fpm->contents__type->parameter
		    && (fa->fa_parameter = pe_cpy(fpm->contents__type->parameter))
		    == NULLPE)
			goto no_mem;
	}
	...
	if (fpm->storage__account) {
		...
		if (fpm->storage__account->offset
			...
			if ((fa->fa_account = qb2str(fpm->storage__account->un.actual__values))
			    == NULL)
				goto no_mem;
...
#undef	douser
	...
	if (fpm->file__availability) {
		...
		if (fpm->file__availability->offset ==
			...
			fa->fa_availability = fpm->file__availability->un.actual__values;
	}
#define	dosize(flag,field,tag) \
    if (fpm -> tag) { \
	fa -> fa_present |= flag; \
...
#undef	dosize
	...
	if (fpm->access__control) {
		...
		else if (fpm2acl(fsb, fpm->access__control->un.actual__values,
	...
	if (fpm->legal__qualification) {
		...
		else if ((fa->fa_legal = qb2str(fpm->legal__qualification->un.actual__values))
			 == NULL)
			goto no_mem;
	}

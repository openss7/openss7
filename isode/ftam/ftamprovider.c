...
struct pair fname_pairs[] = {
	...
	FA_DATE_ATTR,
	    bit_FTAM_Attribute__Names_read__date__and__time__of__last__attribute__modification,
	...
	struct FTAMindication *fti;
{
		...
		case FSB_DATAREAD:
			if (!(fsb->fsb_flags & FSB_INIT)) {
				...
				(void) fpktlose(fsb, fti, FS_PRO_ERR, NULLCP,
						"unexpected FADU; state=0x%x", fsb->fsb_state);
				goto out;
			}
			break;
	...
	for (pep = px->px_info, i = px->px_ninfo - 1; i >= 0; pep++, i--) {
		...
		case type_FTAM_PDU_f__begin__group__request:
			...
			ftg->ftg_threshold = pdu->un.f__begin__group__request->parm;
			goto do_begin;
		...
		case type_FTAM_PDU_f__select__request:
			...
			ftg->ftg_flags |= FTG_SELECT;
			{
				...
				if (fpm2attr(fsb, req->attributes, &ftse->ftse_attrs, fti) == NOTOK)
					goto out;
				...
				if (req->shared__ASE__information
					...
					goto out;
				if (req->account && (ftse->ftse_account = qb2str(req->account))
				    == NULL) {
				      no_mem:;
			...
			next = FTG_RDATTR | FTG_CHATTR | FTG_OPEN | FTG_DESELECT | FTG_DELETE;
			break;
		...
		case type_FTAM_PDU_f__select__response:
			...
			ftg->ftg_flags |= FTG_SELECT;
			{
				...
				ftse->ftse_state = rsp->state__result
				    ? rsp->state__result->parm : int_FTAM_State__Result_success;
				ftse->ftse_action = rsp->action__result
				    ? rsp->action__result->parm : int_FTAM_Action__Result_success;
				if (fpm2attr(fsb, rsp->attributes, &ftse->ftse_attrs, fti) == NOTOK)
					goto out;
				ftse->ftse_attrs.fa_present &= FA_SEL_ATTRS;
				...
				if (rsp->diagnostic
				    && fpm2diag(fsb, rsp->diagnostic,
						ftse->ftse_diags, &ftse->ftse_ndiag, fti) == NOTOK)
					goto out;
			}
		...
		case type_FTAM_PDU_f__create__request:
			...
			ftg->ftg_flags |= FTG_CREATE;
			{
				...
				if (req->shared__ASE__information
					...
					goto out;
				if (req->account && (ftce->ftce_account = qb2str(req->account))
				    == NULL)
					goto no_mem;
			}
			next = FTG_RDATTR | FTG_CHATTR | FTG_OPEN | FTG_DESELECT | FTG_DELETE;
		...
		case type_FTAM_PDU_f__create__response:
			...
			ftg->ftg_flags |= FTG_CREATE;
			{
				...
				if (rsp->diagnostic
				    && fpm2diag(fsb, rsp->diagnostic,
						ftce->ftce_diags, &ftce->ftce_ndiag, fti) == NOTOK)
					goto out;
			}
			next = FTG_RDATTR | FTG_CHATTR | FTG_OPEN | FTG_DESELECT
			    | FTG_DELETE | FTG_END;
		...
		case type_FTAM_PDU_f__read__attrib__request:
			...
			next = FTG_CHATTR | FTG_DESELECT | FTG_DELETE |
			    (ftg->ftg_flags & (FTG_SELECT | FTG_CREATE) ? FTG_OPEN : FTG_NULL);
			break;
		...
		case type_FTAM_PDU_f__read__attrib__response:
			...
			ftg->ftg_flags |= FTG_RDATTR;
			{
				...
				if (rsp->diagnostic
				    && fpm2diag(fsb, rsp->diagnostic,
						ftra->ftra_diags, &ftra->ftra_ndiag, fti) == NOTOK)
					goto out;
			}
			next = FTG_CHATTR | FTG_DESELECT | FTG_DELETE |
			    (ftg->ftg_flags & (FTG_SELECT | FTG_CREATE) ? FTG_OPEN : FTG_NULL);
		...
		case type_FTAM_PDU_f__change__attrib__response:
			...
			ftg->ftg_flags |= FTG_CHATTR;
			{
				...
				if (rsp->diagnostic
				    && fpm2diag(fsb, rsp->diagnostic,
						ftca->ftca_diags, &ftca->ftca_ndiag, fti) == NOTOK)
					goto out;
				if (!(fsb->fsb_attrs & FATTR_STORAGE))
					ftca->ftca_attrs.fa_present &= ~FA_STORAGE;
				if (!(fsb->fsb_attrs & FATTR_SECURITY))
		...
		case type_FTAM_PDU_f__open__request:
			...
			ftg->ftg_flags |= FTG_OPEN;
			{
				...
				if (req->contents__type->offset == choice_FTAM_0_proposed) {
					...
					*proposed = req->contents__type->un.proposed;
					...
					if (proposed->parameter
					    && (ftop->ftop_parameter = pe_cpy(proposed->parameter))
					    == NULLPE)
						goto no_mem;
				}
		...
		case type_FTAM_PDU_f__open__response:
			...
			ftg->ftg_flags |= FTG_OPEN;
			{
				...
				*proposed = rsp->contents__type;
				...
				if (rsp->diagnostic
				    && fpm2diag(fsb, rsp->diagnostic,
						ftop->ftop_diags, &ftop->ftop_ndiag, fti) == NOTOK)
					goto out;
			}
			next = FTG_END;
			break;
		...
		case type_FTAM_PDU_f__close__response:
			...
			{	/* F-CLOSE-response is identical... */
				...
				ftcl->ftcl_action = req->action__result
				    ? req->action__result->parm : int_FTAM_Action__Result_success;
				if (req->shared__ASE__information
				    && fpm2shared(fsb,
						  req->shared__ASE__information,
				...
				if (req->diagnostic
				    && fpm2diag(fsb, req->diagnostic,
						ftcl->ftcl_diags, &ftcl->ftcl_ndiag, fti) == NOTOK)
					goto out;
			}
		...
		case type_FTAM_PDU_f__deselect__request:
			...
			ftg->ftg_flags |= FTG_DESELECT;
			{
				...
				if (req
				    && fpm2shared(fsb, req, &ftde->ftde_sharedASE, fti) == NOTOK)
					goto out;
			}
			next = FTG_END;
			break;
		...
		case type_FTAM_PDU_f__deselect__response:
			...
			ftg->ftg_flags |= FTG_DESELECT;
			{
				...
				if (rsp->diagnostic
				    && fpm2diag(fsb, rsp->diagnostic,
						ftde->ftde_diags, &ftde->ftde_ndiag, fti) == NOTOK)
					goto out;
			}
			next = FTG_END;
			break;
		...
		case type_FTAM_PDU_f__delete__request:
			...
			ftg->ftg_flags |= FTG_DELETE;
			{
				...
				if (req
				    && fpm2shared(fsb, req, &ftxe->ftxe_sharedASE, fti) == NOTOK)
					goto out;
			}
			next = FTG_END;
			break;
		...
		case type_FTAM_PDU_f__delete__response:
			...
			ftg->ftg_flags |= FTG_DELETE;
			{
				...
				if (rsp->diagnostic
				    && fpm2diag(fsb, rsp->diagnostic,
						ftxe->ftxe_diags, &ftxe->ftxe_ndiag, fti) == NOTOK)
					goto out;
			}
			next = FTG_END;
			break;
		...
		case type_FTAM_PDU_f__locate__request:
			if ((fsb->fsb_flags & FSB_INIT)
			    || fsb->fsb_state != FSB_DATAIDLE || next != FTG_BEGIN)
				goto unexpected_fpdu;
		...
		case type_FTAM_PDU_f__locate__response:
			if (!(fsb->fsb_flags & FSB_INIT)
			    || fsb->fsb_state != FSB_LOCATE || next != FTG_BEGIN)
				goto unexpected_fpdu;
			if (!(fsb->fsb_units & FUNIT_ACCESS))
				goto no_access;
			fti->fti_type = FTI_ACCESS;
			{
				...
				if (rsp->diagnostic
				    && fpm2diag(fsb, rsp->diagnostic,
						ftac->ftac_diags, &ftac->ftac_ndiag, fti) == NOTOK)
					goto out;
			}
			next = 0;
			break;
		...
		case type_FTAM_PDU_f__erase__response:
			if (!(fsb->fsb_flags & FSB_INIT)
			    || fsb->fsb_state != FSB_ERASE || next != FTG_BEGIN)
				goto unexpected_fpdu;
			if (!(fsb->fsb_units & FUNIT_ACCESS))
				goto no_access;
			fti->fti_type = FTI_ACCESS;
		...
		case type_FTAM_PDU_f__read__request:
			if ((fsb->fsb_flags & FSB_INIT)
			    || next != FTG_BEGIN || fsb->fsb_state != FSB_DATAIDLE)
				goto unexpected_fpdu;
			...
			fti->fti_type = FTI_READWRITE;
			{
				...
				if (fpm2faduid(fsb,
					...
					goto out;
				if ((ftrw->ftrw_context = req->access__context->access__context)
				    == FA_ACC_FL) {
		...
		case type_FTAM_PDU_f__write__request:
			...
			fti->fti_type = FTI_READWRITE;
			{
				...
				ftrw->ftrw_operation = req->file__access__data__unit__operation;
				if (fpm2faduid(fsb,
					       req->file__access__data__unit__identity,
					       &ftrw->ftrw_identity, fti) == NOTOK)
					goto out;
		...
		case type_FTAM_PDU_f__data__end__request:
			...
			default:
			      unexpected_data_end:;
				(void) fpktlose(fsb, fti, FS_PRO_ERR, NULLCP,
						"unexpected data end; state=0x%x", fsb->fsb_state);
				goto out;
			}
			if (next != FTG_BEGIN)
				goto unexpected_data_end;
			...
			fti->fti_type = FTI_DATAEND;
			{
				...
				if (req->diagnostic
				    && fpm2diag(fsb, req->diagnostic,
						ftda->ftda_diags, &ftda->ftda_ndiag, fti) == NOTOK)
					goto out;
			}
			next = 0;
			break;
		...
		case type_FTAM_PDU_f__cancel__response:
			...
			{	/* F-CANCEL-response is identical... */
				...
				if (req->diagnostic
				    && fpm2diag(fsb, req->diagnostic,
						ftcn->ftcn_diags, &ftcn->ftcn_ndiag, fti) == NOTOK)
					goto out;
			}
			next = 0;
			break;
		...
		case type_FTAM_PDU_f__transfer__end__request:
			...
			fti->fti_type = FTI_TRANSEND;
			{
				...
				if (req
				    && fpm2shared(fsb, req, &ftre->ftre_sharedASE, fti) == NOTOK)
					goto out;
			}
			next = 0;
			break;
		...
		case type_FTAM_PDU_f__transfer__end__response:
			if (!(fsb->fsb_flags & FSB_INIT)
			    || next != FTG_BEGIN || fsb->fsb_state != FSB_DATAFIN2)
				goto unexpected_fpdu;
			fti->fti_type = FTI_TRANSEND;
			{
				register struct FTAMtransend *ftre = &fti->fti_transend;
				...
				if (rsp->diagnostic
				    && fpm2diag(fsb, rsp->diagnostic,
						ftre->ftre_diags, &ftre->ftre_ndiag, fti) == NOTOK)
					goto out;
			}
			next = 0;
			break;
	...
	if (next) {
		(void) fpktlose(fsb, fti, FS_PRO_ERRPROC, NULLCP,
				"missing FPDU(s) in group; expecting one of 0x%x next", next);
		goto out;
	}
	...
	case FTI_ACCESS:
		fsb->fsb_state = (fsb->fsb_flags & FSB_INIT) ? FSB_DATAIDLE
		    : fti->fti_access.ftac_operation == FA_OPS_LOCATE ? FSB_LOCATE : FSB_ERASE;
		goto done;

	case FTI_READWRITE:
		fsb->fsb_state = fti->fti_readwrite.ftrw_operation
		    == FA_OPS_READ ? FSB_DATAREAD : FSB_DATAWRITE;
		goto done;
	...
	case FTI_TRANSEND:
		fsb->fsb_state = (fsb->fsb_flags & FSB_INIT) ? FSB_DATAIDLE : FSB_DATAFIN2;
		goto done;
		...
		if ((fsb->fsb_flags & FSB_INIT)
			...
			if (fsb->fsb_flags & FSB_INIT) {
				...
				if ((ftg->ftg_flags & FTG_SELECT)
				    ? ftg->ftg_select.ftse_account : ftg->ftg_create.ftce_account)
	...
	if (ftg->ftg_flags & FTG_CLOSE) {
      ...
      unexpected_group:;
	(void) fpktlose(fsb, fti, FS_PRO_ERR, NULLCP,
			"unexpected group type 0x%x; state=0x%x", ftg->ftg_flags, fsb->fsb_state);
	goto out;
	...
	struct FTAMindication *fti;
{
	...
	if ((fsb->fsb_flags & FSB_INIT) || fsb->fsb_state != FSB_INITIALIZED) {
		(void) fpktlose(fsb, fti, FS_ACS_MGMT, NULLCP, "association management botched");
		PFFREE(pf);
		goto out1;
	}
	...
	if (pdu->offset != type_FTAM_PDU_f__terminate__request) {
		(void) fpktlose(fsb, fti, FS_PRO_ERRPROC, NULLCP,
				"expecting F-TERMINATE-request, got %d", pdu->offset);
		goto out2;
	}
	req = pdu->un.f__terminate__request;

	fsbtrace(fsb, (fsb->fsb_fd, "A-RELEASE.INDICATION", "F-TERMINATE-request", pe, 1));
...
int
FSetIndications(sd, indication, fti)
	int sd;
	IFP indication;
	struct FTAMindication *fti;
{
...
int
acs2ftamlose(fsb, fti, event, aca)
	register struct ftamblk *fsb;
	struct FTAMindication *fti;
	char *event;
	register struct AcSAPabort *aca;
{
	...
	if (fsb && fsb->fsb_trace && event) {
		...
		if (aca->aca_cc > 0) {
			cp += strlen(cp);
			(void) sprintf(cp, " [%*.*s]", aca->aca_cc, aca->aca_cc, aca->aca_data);
		}
	...
	struct FTAMindication *fti;
{
	...
	if (decode_FTAM_PDU(pe, 1, NULLIP, NULLVP, &pdu) == NOTOK) {
		(void) ftamlose(fti, FS_PRO_ERRMSG, 1, NULLCP, "unable to parse PDU: %s", PY_pepy);
		goto out;
	}
	switch (pdu->offset) {
	...
	if (req->diagnostic)
		(void) fpm2diag(fsb, req->diagnostic, fta->fta_diags, &fta->fta_ndiag, fti);
	...
	register struct PSAPabort *pa;
{
	...
	if (fsb && fsb->fsb_trace && event) {
		...
		if (pa->pa_cc > 0) {
			cp += strlen(cp);
			(void) sprintf(cp, " [%*.*s]", pa->pa_cc, pa->pa_cc, pa->pa_data);
		}
	...
	register struct ftamblk *fsb;
{
	...
	if (fsb->fsb_fd != NOTOK) {
		...
		fsbtrace(fsb, (fsb->fsb_fd, "A-ABORT.REQUEST(discard)", NULLCP, NULLPE, 0));
	...
	for (pp = fsb->fsb_contexts.pc_ctx, i = fsb->fsb_contexts.pc_nctx - 1; i >= 0; pp++, i--) {
		if (pp->pc_asn)
			oid_free(pp->pc_asn);
	}
	...
	for (fcont = fsb->fsb_contents.fc_contents,
	     i = fsb->fsb_contents.fc_ncontent - 1; i >= 0; fcont++, i--) {
		if (fcont->fc_dtn)

...
static int
FGroupResponse(sd, ftg, type, state, fti)
	int sd;
	struct FTAMgroup *ftg;
	int type, state;
	struct FTAMindication *fti;
...
static int
FGroupResponseAux(fsb, ftg, state, fti)
	register struct ftamblk *fsb;
...
{
	register int i;
	int did_loop, npdu, result, okstate;
	char **txp, *texts[NPDATA];
	PE pe, *pep, info[NPDATA];
	struct PSAPindication pis;
	struct PSAPindication *pi = &pis;
	struct PSAPabort *pa = &pi->pi_abort;
	struct type_FTAM_PDU **pdup, *pdus[NPDATA];
	...
	if ((result = frgrp2pdus(fsb, ftg, pdus, texts, &npdu, fti)) == NOTOK)
		goto out;
	for (pdup = pdus, pep = info, txp = texts, i = npdu - 1; i >= 0; pdup++, pep++, txp++, i--) {
		pe = NULLPE;
	...
	result = PDataRequest(fsb->fsb_fd, info, npdu, pi);
      ...
      out:;
	for (pdup = pdus, pep = info, i = NPDATA - 1; i >= 0; pdup++, pep++, i--) {
		if (*pep)
			pe_free(*pep);
		if (*pdup)
			free_FTAM_PDU(*pdup);
	...
	case FSB_DATAIDLE:
		...
		if (okstate != FSTATE_SUCCESS || ftg->ftg_open.ftop_state != FSTATE_SUCCESS) {
	...
	struct FTAMindication *fti;
{
	...
	if (ftg->ftg_flags & FTG_SELECT) {
		...
		if (ftse->ftse_ndiag > NFDIAG)
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "too many select diagnostics");
	}
	...
	if (ftg->ftg_flags & FTG_CREATE) {
		...
		if (!(ftce->ftce_attrs.fa_present & FA_FILENAME))
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "filename not present");
		if (!(ftce->ftce_attrs.fa_present & FA_ACTIONS))
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP,
					"permitted-actions not present");
		if (!(ftce->ftce_attrs.fa_present & FA_CONTENTS))
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "contents-type not present");
		if (ftce->ftce_attrs.fa_present & ~FA_CRE_ATTRS)
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "illegal attributes present");
		...
		if (ftce->ftce_ndiag > NFDIAG)
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "too many create diagnostics");
	}
	...
	if (ftg->ftg_flags & FTG_CLOSE) {
		...
		if (ftcl->ftcl_ndiag > NFDIAG)
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "too many close diagnostics");
	}
	...
	if (ftg->ftg_flags & FTG_DESELECT) {
		...
		if (ftde->ftde_charges.fc_ncharge > NFCHRG)
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "too many deselect charges");
	...
	if (ftg->ftg_flags & FTG_DELETE) {
		...
		if (ftxe->ftxe_charges.fc_ncharge > NFCHRG)
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "too many delete charges");
		if (ftxe->ftxe_ndiag > NFDIAG)
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "too many delete diagnostics");
	}
	...
	if (ftg->ftg_flags & FTG_OPEN) {
		...
		if (ftop->ftop_ndiag > NFDIAG)
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "too many open diagnostics");
	}
...
static int
frgrp2pdus(fsb, ftg, pdus, texts, npdu, fti)
	register struct ftamblk *fsb;
	register struct FTAMgroup *ftg;
...
#define	new_action(a) \
	...
	if (flags & FTG_SELECT) {
		...
		if (ftse->ftse_ndiag > 0
		    && (req->diagnostic = diag2fpm(fsb, 0, ftse->ftse_diags, ftse->ftse_ndiag, fti))
		    == NULL)
			return NOTOK;
	}
	...
	if (flags & FTG_CREATE) {
		...
		if ((req->initial__attributes = attr2fpm(fsb, &ftce->ftce_attrs, fti)) == NULL)
			return NOTOK;
		...
		if (ftce->ftce_ndiag > 0
		    && (req->diagnostic = diag2fpm(fsb, 0, ftce->ftce_diags, ftce->ftce_ndiag, fti))
		    == NULL)
			return NOTOK;
	}
	...
	if (flags & FTG_CLOSE) {
		...
		new_pdu(type_FTAM_F__CLOSE__response,
			type_FTAM_PDU_f__close__response, f__close__response, "F-CLOSE-response");
		...
		if (ftcl->ftcl_ndiag > 0
		    && (req->diagnostic = diag2fpm(fsb, 0, ftcl->ftcl_diags, ftcl->ftcl_ndiag, fti))
		    == NULL)
			return NOTOK;
	}
	...
	if (flags & FTG_RDATTR) {
		...
		if (ftra->ftra_attrs.fa_present
		    && (req->attributes = attr2fpm(fsb, &ftra->ftra_attrs, fti)) == NULL)
			return NOTOK;
		if (ftra->ftra_ndiag > 0
		    && (req->diagnostic = diag2fpm(fsb, 0, ftra->ftra_diags, ftra->ftra_ndiag, fti))
		    == NULL)
			return NOTOK;
	}
	...
	if (flags & FTG_CHATTR) {
		...
		if (ftca->ftca_attrs.fa_present
		    && (req->attributes = attr2fpm(fsb, &ftca->ftca_attrs, fti)) == NULL)
			return NOTOK;
		if (ftca->ftca_ndiag > 0
		    && (req->diagnostic = diag2fpm(fsb, 0, ftca->ftca_diags, ftca->ftca_ndiag, fti))
		    == NULL)
			return NOTOK;
	}
	...
	if (flags & FTG_DESELECT) {
		...
		if (ftde->ftde_charges.fc_ncharge
		    && (req->charging = chrg2fpm(fsb, &ftde->ftde_charges, fti)) == NULL)
			return NOTOK;
		if (ftde->ftde_ndiag > 0
		    && (req->diagnostic = diag2fpm(fsb, 0, ftde->ftde_diags, ftde->ftde_ndiag, fti))
		    == NULL)
			return NOTOK;
	}
	...
	if (flags & FTG_DELETE) {
		...
		if (ftxe->ftxe_charges.fc_ncharge
		    && (req->charging = chrg2fpm(fsb, &ftxe->ftxe_charges, fti)) == NULL)
			return NOTOK;
		if (ftxe->ftxe_ndiag > 0
		    && (req->diagnostic = diag2fpm(fsb, 0, ftxe->ftxe_diags, ftxe->ftxe_ndiag, fti))
		    == NULL)
			return NOTOK;
	}
	...
	if (flags & FTG_OPEN) {
		...
		new_pdu(type_FTAM_F__OPEN__response,
			type_FTAM_PDU_f__open__response, f__open__response, "F-OPEN-response");
		...
		if ((proposed->document__type__name = oid_cpy(ftop->ftop_contents)) == NULLOID)
			goto no_mem;
		if (proposed->parameter = ftop->ftop_parameter)
			proposed->parameter->pe_refcnt++;
		if (conctl_present(&ftop->ftop_conctl)
		    && (req->concurrency__control = conctl2fpm(fsb, &ftop->ftop_conctl, fti))
		    == NULL)
			return NOTOK;
		...
		if (ftop->ftop_ndiag > 0
		    && (req->diagnostic = diag2fpm(fsb, 0, ftop->ftop_diags, ftop->ftop_ndiag, fti))
		    == NULL)
			return NOTOK;
	}

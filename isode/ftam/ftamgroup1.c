...
static int
FGroupRequest(sd, ftg, type, state, fti)
	int sd;
	struct FTAMgroup *ftg;
	int type, state;
	struct FTAMindication *fti;
...
static int
FGroupRequestAux(fsb, ftg, state, fti)
	register struct ftamblk *fsb;
	...
	struct FTAMindication *fti;
{
	...
	default:
	      bad_class:;
		return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "file management not permitted");
	}
	...
	case FTI_BULKBEGIN:
		...
		if (!(ftg->ftg_flags & FTG_OPEN))
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "missing open request");
	...
	case FTI_BULKEND:
		...
		if ((ftg->ftg_flags & FTG_CLOSE) == 0)
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "missing close request");
	...
	if (!(fsb->fsb_units & FUNIT_GROUPING)
		...
		return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "grouping not permitted");
	...
	if (ftg->ftg_flags & FTG_CREATE) {
		...
		default:
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "bad value for creation mode");
		}
	...
	if (ftg->ftg_flags & FTG_OPEN) {
		...
		if ((request = ftop->ftop_mode) & ~FA_MODE_MASK)
			return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "bad open mode");
#ifdef	notdef
...
static int
figrp2pdus(fsb, ftg, pdus, texts, npdu, fti)
	register struct ftamblk *fsb;
	register struct FTAMgroup *ftg;
...
#define	new_action(a) \
	...
	if (ftg->ftg_flags & FTG_SELECT) {
		...
		new_pdu(type_FTAM_F__SELECT__request,
			type_FTAM_PDU_f__select__request, f__select__request, "F-SELECT-request");
		...
		if (passes_present(&ftse->ftse_pwds)
		    && (req->access__passwords = pass2fpm(fsb, &ftse->ftse_pwds, fti)) == NULL)
			return NOTOK;
		if (conctl_present(&ftse->ftse_conctl)
		    && (req->concurrency__control = conctl2fpm(fsb, &ftse->ftse_conctl, fti))
		    == NULL)
			return NOTOK;
		...
		if (ftse->ftse_account
		    && (req->account = str2qb(ftse->ftse_account, strlen(ftse->ftse_account), 1))
		    == NULL)
			goto no_mem;
	}
	...
	if (ftg->ftg_flags & FTG_CREATE) {
		...
		new_pdu(type_FTAM_F__CREATE__request,
			type_FTAM_PDU_f__create__request, f__create__request, "F-CREATE-request");
		...
		if (passes_present(&ftce->ftce_pwds)
		    && (req->access__passwords = pass2fpm(fsb, &ftce->ftce_pwds, fti)) == NULL)
			return NOTOK;
		if (conctl_present(&ftce->ftce_conctl)
		    && (req->concurrency__control = conctl2fpm(fsb, &ftce->ftce_conctl, fti))
		    == NULL)
			return NOTOK;
		...
		if (ftce->ftce_account
		    && (req->account = str2qb(ftce->ftce_account, strlen(ftce->ftce_account), 1))
		    == NULL)
			goto no_mem;
	}
	...
	if (ftg->ftg_flags & FTG_CLOSE) {
		...
		if (ftcl->ftcl_sharedASE
		    && (req->shared__ASE__information = shared2fpm(fsb, ftcl->ftcl_sharedASE, fti))
		    == NULL)
			return NOTOK;
	...
	if (ftg->ftg_flags & FTG_RDATTR) {
		...
		if ((pdu->un.f__read__attrib__request =
		     bits2fpm(fsb, fname_pairs, ftra->ftra_attrnames, fti)) == NULLPE)
			return NOTOK;
	}
	...
	if (ftg->ftg_flags & FTG_OPEN) {
		...
		if (ftop->ftop_mode != FA_PERM_READ
		    && (req->processing__mode = bits2fpm(fsb, fmode_pairs,
							 ftop->ftop_mode, fti)) == NULL)
			return NOTOK;
		if ((req->contents__type = (struct choice_FTAM_0 *)
		...
		if (fsb->fsb_units & FUNIT_FADULOCK)
			req->enable__fadu__locking = ftop->ftop_locking
			    ? int_FTAM_FADU__Lock_on : int_FTAM_FADU__Lock_off;
	}

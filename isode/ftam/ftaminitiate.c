...
int
FInitializeRequest(context, callingtitle, calledtitle, callingaddr,
	...
	int manage, class, units, attrs, fqos, passlen;
	PE sharedASE;
	struct FTAMcontentlist *contents;
	char *initiator, *account, *password;
	struct QOStype *qos;
	IFP tracing;
	...
	struct FTAMindication *fti;
{
	...
	if ((class & FCLASS_TRANSFER && ((units & FUNITS_TRANSFER) != FUNITS_TRANSFER))) {
	      not_enough:;
		return ftamlose(fti, FS_GEN_NOREASON, 1, NULLCP,
	...
	if ((class & FCLASS_MANAGE) && ((units & FUNITS_MANAGE) != FUNITS_MANAGE))
		goto not_enough;
	if (!(class &= (FCLASS_TRANSFER | FCLASS_TM | FCLASS_MANAGE | FCLASS_ACCESS)))
	...
	if (contents && contents->fc_ncontent > NFCONT)
		return ftamlose(fti, FS_GEN_NOREASON, 1, NULLCP, "too many content types");
	if (password == NULL)
		passlen = 0;
#ifdef	notdef
	missingP(qos);
	...
	result = FInitializeRequestAux(context, callingtitle, calledtitle,
				       ...
				       sharedASE, fqos, contents, initiator, account, password,
				       passlen, qos, tracing, ftc, fti);
...
static int
FInitializeRequestAux(context, callingtitle, calledtitle,
		      callingaddr, calledaddr, manage, class, units, attrs, sharedASE, fqos,
		      contents, initiator, account, password, passlen, qos, tracing, ftc, fti)
	OID context;
	AEI callingtitle, calledtitle;
	struct PSAPaddr *callingaddr, *calledaddr;
	int manage, class, units, attrs, fqos, passlen;
...
{
	register int i;
	int bits, rcvd_bits, idc, result, settings;
	long isn;
	PE pe;
	OID ctx, pci;
	struct SSAPref *sr;
	...
	if (contents) {
		...
		for (fx = contents->fc_contents, i = contents->fc_ncontent - 1; i >= 0; fx++, i--) {
	...
	if (class != FCLASS_TRANSFER
	    && (req->service__class = bits2fpm(fsb, fclass_pairs, class, fti)) == NULLPE)
		goto out1;
	if ((req->functional__units = bits2fpm(fsb, funit_pairs, units, fti))
	    == NULLPE)
		goto out1;
	if (attrs && (req->attribute__groups = bits2fpm(fsb, fattr_pairs, attrs, fti)) == NULLPE)
		goto out1;
	if (sharedASE && (req->shared__ASE__information = shared2fpm(fsb, sharedASE, fti)) == NULL)
		goto out1;
	if ((req->ftam__quality__of__service = (struct type_FTAM_FTAM__Quality__Of__Service *)
	...
	if (initiator && (req->initiator__identity = str2qb(initiator, strlen(initiator), 1))
	    == NULL)
		goto out1;
	if (account && (req->account = str2qb(account, strlen(account), 1))
	    == NULL)
		goto out1;
	if (password) {
	...
	isn = (fsb->fsb_srequirements & (SR_MINORSYNC | SR_RESYNC)) ? 1L : SERIAL_NONE;
...
#undef	dotoken
	...
	fsbtrace(fsb, (fsb->fsb_fd, "A-ASSOCIATE.REQUEST", "F-INITIALIZE-request", pe, 0));

	result = AcAssocRequest(ctx, callingtitle, calledtitle, callingaddr, calledaddr, pl, NULLOID	/* pci 
													 */ , fsb->fsb_prequirements,
				fsb->fsb_srequirements, isn, settings, sr, &pe, 1, qos, acc, aci);
	...
	if (pdu->offset != type_FTAM_PDU_f__initialize__response) {
		result = fpktlose(fsb, fti, FS_PRO_ERRPROC, NULLCP,
				  "expecting F-INITIALIZE-response, got %d", pdu->offset);
		goto out2;
	}
	rsp = pdu->un.f__initialize__response;

	fsbtrace(fsb, (fsb->fsb_fd, "A-ASSOCIATE.CONFIRMATION", "F-INITIALIZE-response", pe, 1));
	...
	case ACS_ACCEPT:
		if (ftc->ftc_state != FSTATE_SUCCESS || ftc->ftc_action != FACTION_SUCCESS) {
			result = fpktlose(fsb, fti, FS_PRO_ERRPROC, NULLCP,
...
#undef	dotoken
		...
		if (pl->pc_nctx > 0 && pl->pc_ctx[0].pc_result != PC_ACCEPT) {
			result = fpktlose(fsb, fti, FS_PRO_ERRPROC, NULLCP, "FTAM PCI rejected");
			goto out2;
		}
		...
		if (rsp->service__class) {
			rcvd_bits = fpm2bits(fsb, fclass_pairs, rsp->service__class, &bits, fti);
			if (rcvd_bits == NOTOK)
				goto out2;
			else if (rcvd_bits > 1) {
				result = fpktlose(fsb, fti, FS_PRO_ERRPROC, NULLCP,
		...
		if (fpm2bits(fsb, funit_pairs, rsp->functional__units, &bits, fti) == NOTOK)
			goto out2;
		bits |= i;	/* conservative... */
...
/* check for illegal and mandatory functional units */
		...
		if (rsp->attribute__groups) {
			if (fpm2bits(fsb, fattr_pairs, rsp->attribute__groups, &bits, fti) == NOTOK)
				goto out2;
		} else
			bits = 0;
		if (bits & ~attrs) {
	...
	if (rsp->shared__ASE__information
	    && fpm2shared(fsb, rsp->shared__ASE__information, &ftc->ftc_sharedASE, fti) == NOTOK)
		goto out2;
	...
	if (contents) {
		...
		for (dtn = rsp->contents__type__list; dtn; dtn = dtn->next) {
			for (fx2 = ftc->ftc_contents.fc_contents,
			     i = ftc->ftc_contents.fc_ncontent - 1; i >= 0; fx2++, i--)
	...
	if (rsp->diagnostic)
		(void) fpm2diag(fsb, rsp->diagnostic, ftc->ftc_diags, &ftc->ftc_ndiag, fti);

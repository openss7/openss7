...
int
FInit(vecp, vec, fts, tracing, fti)
	int vecp;
	char **vec;
...
#undef	dotoken
	...
	if (acs->acs_ninfo < 1 || (pe = acs->acs_info[0]) == NULLPE) {
		(void) ftamoops(fti, FS_PRO_ERR, 1, EREF_RFPM, EREF_IFPM, NULLCP, NULLCP);
		goto out2;
	}
	...
	if (pdu->offset != type_FTAM_PDU_f__initialize__request) {
		(void) ftamoops(fti, FS_PRO_ERRPROC, 1, EREF_RFPM, EREF_RFPM,
				NULLCP, "expecting F-INITIALIZE-request, got %d", pdu->offset);
		goto out3;
	}
	req = pdu->un.f__initialize__request;

	fsbtrace(fsb, (fsb->fsb_fd, "A-ASSOCIATE.INDICATION", "F-INITIALIZE-request", pe, 1));
	...
	if (req->service__class) {
		if (fpm2bits(fsb, fclass_pairs, req->service__class, &fsb->fsb_class, fti) == NOTOK)
			goto out3;
	} else
		fsb->fsb_class = FCLASS_TRANSFER;

	if (fpm2bits(fsb, funit_pairs, req->functional__units, &fsb->fsb_units, fti) == NOTOK)
		goto out3;
	/* conservative... */
	...
	if (!(fsb->fsb_class &= (FCLASS_TRANSFER | FCLASS_TM | FCLASS_MANAGE | FCLASS_ACCESS))) {
		(void) ftamoops(fti, FS_ACS_CLASS, 1, EREF_RFPM, EREF_IFPM, NULLCP, NULLCP);
		goto out3;
	}
	...
	if (!(fsb->fsb_units & FUNIT_GROUPING)) {	/* XXX: should be OPTIONAL */
		(void) ftamoops(fti, FS_PRO_ERRFUNIT, 1, EREF_RFPM, EREF_IFPM,
				NULLCP, "insufficient functional units for service class");
		goto out3;
	}
	if (req->attribute__groups
	    && fpm2bits(fsb, fattr_pairs, req->attribute__groups, &fsb->fsb_attrs, fti) == NOTOK)
		goto out3;
#ifdef	notdef
	if ((fsb->fsb_attrs & FATTR_SECURITY)
	    && !(fsb->fsb_attrs & FATTR_STORAGE)) {
		(void) ftamoops(fti, FS_ACS_GRP, 1, EREF_RFPM, EREF_IFPM, NULLCP, NULLCP);
		goto out3;
	}
#endif
	...
	if (req->shared__ASE__information
	    && fpm2shared(fsb, req->shared__ASE__information, &fts->fts_sharedASE, fti) == NOTOK)
		goto out3;
...
#define	PC_XXX	(-2)		/* unique code */
		...
		register struct FTAMcontent *fx, *fx2;
		...
		for (px = fsb->fsb_contexts.pc_ctx,
		     i = fsb->fsb_contexts.pc_nctx - 1; i >= 0; px++, i--)
			if (px->pc_id != fsb->fsb_id
			    && px->pc_id != acsid && px->pc_result == PC_ACCEPT)
				px->pc_result = PC_XXX;
		...
		for (dtn = req->contents__type__list; dtn; dtn = dtn->next) {
			...
			for (px = fsb->fsb_contexts.pc_ctx,
			     i = fsb->fsb_contexts.pc_nctx - 1; i >= 0; px++, i--) {
				if (px->pc_id == fsb->fsb_id
				    || px->pc_id == acsid || oid_cmp(id->id_abstract, px->pc_asn))
					continue;
				break;
			}
			...
			if ((fx2->fc_dtn = oid_cpy(dtn->Document__Type__Name))
			    == NULLOID || (fx->fc_dtn = oid_cpy(dtn->Document__Type__Name))
			    == NULLOID)
				goto no_mem;
...
#undef	PC_XXX
	...
	if (req->initiator__identity && (fts->fts_initiator = qb2str(req->initiator__identity))
	    == NULL)
		goto no_mem;
	if (req->account && (fts->fts_account = qb2str(req->account)) == NULL)
		goto no_mem;
	...
	if (rsp->ftam__quality__of__service = (struct type_FTAM_FTAM__Quality__Of__Service *)
...
int
FInitializeResponse(sd, state, action, context, respondtitle,
		    respondaddr, manage, class, units, attrs, sharedASE, fqos, contents,
		    diag, ndiag, fti)
	int sd;
	int state, action, manage, class, units, attrs, fqos;
	OID context;
	AEI respondtitle;
	...
	struct FTAMindication *fti;
{
	...
	default:
		return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "bad value for state parameter");
	}
	switch (action) {
	...
	default:
		return ftamlose(fti, FS_GEN(fsb), 0, NULLCP, "bad value for action parameter");
	}
#ifdef	notdef
	missingP(context);
	...
	if (contents) {
		...
		(void) AcFindPCI(fsb->fsb_fd, &acsid, aci);
		for (px = pl->fc_contents, i = pl->fc_ncontent - 1; i >= 0; px++, i--) {
			if (fx->fc_id != px->fc_id)
				return ftamlose(fti, FS_GEN(fsb), 0, NULLCP,
						"bad context id %d at offset %d (wanted %d)",
						fx->fc_id, fx - contents->fc_contents, px->fc_id);
			switch (fx->fc_result) {
			case PC_ACCEPT:
			case PC_REJECTED:
	...
	if (fsb->fsb_class != FCLASS_TRANSFER
	    && (rsp->service__class = bits2fpm(fsb, fclass_pairs, fsb->fsb_class, fti))
	    == NULLPE)
		goto out;
	if ((rsp->functional__units = bits2fpm(fsb, funit_pairs, fsb->fsb_units, fti))
	    == NULLPE)
		goto out;
	if (fsb->fsb_attrs
	    && (rsp->attribute__groups = bits2fpm(fsb, fattr_pairs, attrs, fti)) == NULLPE)
		goto out;
	...
	fsbtrace(fsb, (fsb->fsb_fd, "A-ASSOCIATE.RESPONSE", "F-INITIALIZE-response", pe, 0));
	...
	result = AcAssocResponse(fsb->fsb_fd, status, status != ACS_ACCEPT
				 ...
				 fsb->fsb_ssn, fsb->fsb_settings, &fsb->fsb_connect, &pe, 1, aci);

	...
	struct PSAPindication *pi;
{
	...
	if (data[0]->pe_context != PCI_ROSE)
		return psaplose(pi, PC_PARAMETER, NULLCP, "wrong context for user data");
	missingP(pi);
	...
	struct PSAPindication *pi;
{
	...
	for (;;) {
		...
		if (ps_prime(ps, 1) == OK && (*pb->pb_selectfnx) (nfds, &ifds, NULLFD, &efds, secs)
		    <= OK) {
		...
		if (FD_ISSET(pb->pb_fd, &ifds) || FD_ISSET(pb->pb_fd, &efds))
			if ((result = PReadRequestAux(pb, px, pi)) != NOTOK
			    || secs != NOTOK || pa->pa_reason != PC_TIMER)
				break;
	}
	...
	struct PSAPindication *pi;
{
	...
	case type_PS_PDUs_releaseRequest:
	{
		...
		register struct type_PS_ReleaseRequest__PDU *rr = pdu->un.releaseRequest;

		if (pb->pb_reliability == LOW_QUALITY
		    && refcmp(pb->pb_reference, (pref = rr->reference))) {
		      bad_ref1:;
			(void) ppktlose(pb, pi, PC_SESSION, pref, NULLCP, "reference mismatch");
		      bad_ref2:;
			if (pdu)
				free_PS_PDUs(pdu);
	...
	{
		register struct type_PS_CL__UserData__PDU *cl = pdu->un.cL__userData;
	...
	register struct psapblk *pb;
{
	...
	for (qp = pb->pb_contexts, i = pb->pb_ncontexts - 1; i >= 0; qp++, i--) {
		if (qp->pc_asn)
			oid_free(qp->pc_asn);
		if (qp->pc_atn)
			oid_free(qp->pc_atn);
	...
	register struct type_PS_SessionConnectionIdentifier *ref1, *ref2;
{
	...
	if (qb_cmp(ref1->callingSSUserReference, ref2->callingSSUserReference)
	    ...
	    || qb_cmp(ref1->additionalReferenceInformation, ref2->additionalReferenceInformation)) {
...
static int
qb_cmp(qb1, qb2)
	register struct qbuf *qb1, *qb2;
{
	register int i, len1, len2;
	register char *cp1, *cp2;
	register struct qbuf *qp1, *qp2;
	...
	register struct type_PS_SessionConnectionIdentifier *ref;
{
	...
	pdu2sel(sf->sr_udata, &i, sizeof sf->sr_udata, ref->callingSSUserReference);
	sf->sr_ulen = i;

	pdu2sel(sf->sr_cdata, &i, sizeof sf->sr_cdata, ref->commonReference);
	sf->sr_clen = i;

	pdu2sel(sf->sr_adata, &i, sizeof sf->sr_adata, ref->additionalReferenceInformation);
	sf->sr_alen = i;

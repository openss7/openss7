...
int
udpopen(pb, calling, called, pi, async)
	...
	int async;
{
	...
	if (called->na_port == 0)
		return psaplose(pi, PC_ADDRESS, NULLCP, "UDP port of called address not specified");
	else
	...
	struct PSAPindication *pi;
{
	...
	if ((pb->pb_stream = ps_alloc(dg_open)) == NULLPS
	    || dg_setup(pb->pb_stream, pb->pb_fd, MAXDGRAM,
			read_udp_socket, write_udp_socket, check_udp_socket) == NOTOK)
	...
	struct PSAPindication *pi;
{
	...
	PLOGP(psap2_log, PS_PDUs, pb->pb_retry,
	      reason == PC_REFUSED ? "ConnectRequest-PDU" : "ReleaseRequest-PDU", 0);
	...
	if (select_udp_socket(nfds, &ifds, NULLFD, NULLFD, WAITRIES) < 1) {
		...
		if (reason == PC_REFUSED) {
			...
			(void) ppktlose(pb, pi, reason, pb->pb_reference, NULLCP, NULLCP);

	...
	struct PSAPindication *pi;
{
	...
	default:
		(void) psaplose(pi, PC_PARAMETER, NULLCP,
				"unknown transport type: 0x%x (%c)", *vec[0], *vec[0]);
		goto out;
	}
	*vec = NULL;
	...
	struct PSAPindication *pi;
{
	...
	if (pdu->offset != type_PS_PDUs_connectRequest) {
		...
		if (pdu->offset != type_PS_PDUs_abort)
			...
			result = psaplose(pi, PC_SESSION, NULLCP, "unexpected PDU %d", pdu->offset);
		goto out;
	}
	...
	if ((oid = oid_cpy(oid)) == NULL) {
		result = ppktlose(pb, pi, PC_CONGEST, cr->reference, NULLCP, NULLCP);
		goto out;
	}
...
/* should check version number here... */
	...
	pdu2sel(ps->ps_calling.pa_selector,
		&ps->ps_calling.pa_selectlen, sizeof ps->ps_calling.pa_selector, cr->calling);

	ta = &pb->pb_responding.pa_addr.sa_addr;
	ta->ta_naddr = 1;
	ta->ta_addrs[0] = pb->pb_initiating;	/* struct copy */
	pdu2sel(pb->pb_responding.pa_selector,
		&pb->pb_responding.pa_selectlen, sizeof pb->pb_responding.pa_selector, cr->called);
...
int
PConnResponse(sd, status, responding, ctxlist, defctxresult,
	      prequirements, srequirements, isn, settings, ref, data, ndata, pi)
	int sd;
	struct PSAPaddr *responding;
	int status, prequirements, srequirements, settings, ndata;
	long isn;
	...
	struct PSAPindication *pi;
{
	...
	if (ref->sr_ulen > SREF_USER_SIZE
	    || ref->sr_clen > SREF_COMM_SIZE || ref->sr_alen > SREF_ADDT_SIZE || ref->sr_vlen > 0)
	...
	if (status == PC_REJECTED) {
		...
		pdu->reason->parm = int_PS_Rejection__reason_rejected__by__responder;
	} else
		pdu->reason = NULL;

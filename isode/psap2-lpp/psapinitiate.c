...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/psap2-lpp/RCS/psapinitiate.c,v 9.0 1992/06/16 12:31:57 isode Rel";
#endif
...
int
PAsynConnRequest(calling, called, ctxlist, defctxname, prequirements,
		 srequirements, isn, settings, ref, data, ndata, qos, pc, pi, async)
	struct PSAPaddr *calling, *called;
	...
	struct PSAPindication *pi;
{
	...
	if (ctxlist == NULL || ctxlist->pc_nctx != NPCTX_PS)
		return psaplose(pi, PC_PARAMETER, NULLCP,
				"exactly %d proposed presentation contexts supported", NPCTX_PS);
#ifdef	notdef
	if (defctxname)
		return psaplose(pi, PC_PARAMETER, NULLCP, "default context name not allowed");
#endif
	...
	if (isn != SERIAL_NONE)
		return psaplose(pi, PC_PARAMETER, NULLCP, "initial serial number not permitted");
	if (settings != 0)	/* not really an accurate test... */
		return psaplose(pi, PC_PARAMETER, NULLCP, "initial token settings not permitted");
	missingP(ref);
	if (ref->sr_ulen > SREF_USER_SIZE
	    || ref->sr_ulen <= 2
	    || ref->sr_clen > SREF_COMM_SIZE
	    || ref->sr_clen <= 2
	    || ref->sr_alen > SREF_ADDT_SIZE || ref->sr_alen == 1 || ref->sr_vlen > 0)
	...
	if (data[0]->pe_context != PCI_ACSE)
		return psaplose(pi, PC_PARAMETER, NULLCP, "wrong context for initial user data");
	missingP(pc);
	missingP(pi);
...
static int
PConnRequestAux(calling, called, ctxlist, ref, data, qos, pc, pi, async)
	struct PSAPaddr *calling, *called;
	struct PSAPctxlist *ctxlist;
	struct SSAPref *ref;
	PE data;
	...
	int async;
{
	...
	pdu = NULL;
	if ((pref->callingSSUserReference = str2qb(ref->sr_udata + 2, (int) ref->sr_ulen - 2, 1))
	    == NULL
	    || (pref->commonReference = str2qb(ref->sr_cdata + 2, (int) ref->sr_clen - 2, 1))
	    == NULL) {
	      no_mem:;
		(void) psaplose(pi, PC_CONGEST, NULLCP, "out of memory");
		goto out2;
	...
	if (ref->sr_alen > 0) {
		if ((pref->additionalReferenceInformation
		     = str2qb(ref->sr_adata + 2, (int) ref->sr_alen - 2, 1))
		    == NULL)
			goto no_mem;
	} else
		...
		for (pp = ctxlist->pc_ctx, qp = pb->pb_contexts; i >= 0; i--, pp++, qp++) {
			...
			default:
				(void) psaplose(pi, PC_PARAMETER, NULLCP,
						"illegal value for PCI (%d)", pp->pc_id);
				goto out2;
			}
	...
	if (calling && calling->pa_selectlen > 0) {
		if ((pdu->calling = str2qb(calling->pa_selector, calling->pa_selectlen, 1)) == NULL)
			goto no_mem;
	} else
		pdu->calling = NULL;

	if (called->pa_selectlen > 0) {
		if ((pdu->called = str2qb(called->pa_selector, called->pa_selectlen, 1)) == NULL)
			goto no_mem;
	} else
		pdu->called = NULL;
	...
	pb->pb_retry = NULLPE;
	result = encode_PS_ConnectRequest__PDU(&pb->pb_retry, 1, 0, NULLCP, pdu);
	...
	if ((result = PConnRequestAux2(pb, calling ? &calling->pa_addr.sa_addr
				       : NULLTA,
				       &called->pa_addr.sa_addr, qos, pi, async)) == NOTOK)
		goto out1;
...
HIGH_QUALITY, NA_TSET_TCP, tcpopen, LOW_QUALITY, NA_TSET_UDP, udpopen, NULL};

static int
PConnRequestAux2(pb, calling, called, qos, pi, async)
	struct psapblk *pb;
	...
	int async;
{
	...
	for (; n >= 0; na++, n--) {
		...
		for (ns = nsaps; ns->ns_open; ns++)
			if (ns->ns_reliability == reliability && (ns->ns_tset & na->na_tset))
				break;
		if (!ns->ns_open)
			continue;

		if (calling) {
			for (l = calling->ta_naddr - 1, la = calling->ta_addrs; l >= 0; la++, l--) {
	...
	struct PSAPindication *pi;
{
	...
	if (pb->pb_flags & PB_CONN) {
		(void) sigiomask(smask);
		return psaplose(pi, PC_OPERATION, NULLCP, "presentation descriptor connected");
	}
	...
	struct PSAPindication *pi;
{
	...
	{
		register struct type_PS_ConnectResponse__PDU *cr = pdu->un.connectResponse;

		if (pb->pb_reliability == LOW_QUALITY && refcmp(pb->pb_reference, cr->reference)) {
		...
		pdu2sel(pb->pb_responding.pa_selector,
			...
			sizeof pb->pb_responding.pa_selector, cr->responding);
	...
	case type_PS_PDUs_abort:
	{
		...
		if (pb->pb_reliability == LOW_QUALITY && refcmp(pb->pb_reference, ab->reference)) {
			result = psaplose(pi, PC_SESSION, NULLCP, "reference mismatch");
			goto out;
		}

	...
	char **argv, **envp;
{
	...
	(void) sprintf(autogen, "%*.*s", cp - rosyversion, cp - rosyversion, rosyversion);
	...
	char *id;
{
	...
	if (!Pepsyflag || Defsflag) {
		...
		if (yp = yo->yo_result)
			...
			(void) fprintf(fdef, "NULLIFP\n");
		(void) fprintf(fdef, "#define\t%s_result\t", modsym(mymodule, yo->yo_name, "free"));
		if (yp = yo->yo_result)
			(void) fprintf(fdef, "%s\n",
				       modsym(yp->yp_module, yp->yp_identifier, "free"));
		else
	...
	char *id;
{
	...
	(void) fprintf(fstb, "IFP\trfx,\n\tefx;\nstruct RoSAPindication *roi;\n");
	(void) fprintf(fstb, "{\n    return RyStub (sd, table_%s_Operations, ", mymodaux);
	(void) fprintf(fstb,
	...
	if (yp = yo->yo_arg)
		...
		(void) fprintf(fstb, "caddr_t");
	(void) fprintf(fstb, " in;\n");
	(void) fprintf(fstb, "caddr_t *out;\nint    *rsp;\nstruct RoSAPindication *roi;\n");
	(void) fprintf(fstb, "{\n    return RyOperation (sd, table_%s_Operations, ", mymodaux);
	...
	if (Pepsyflag) {
		...
		(void) fprintf(ftbl, "\t%d, %s_result,\n",
			       yo->yo_result ? 1 : 0, modsym(mymodule, yo->yo_name, "encode"));
		(void) fprintf(ftbl, "\t   %s_result,\n", modsym(mymodule, yo->yo_name, "decode"));
		(void) fprintf(ftbl, "\t   %s_result,\n", modsym(mymodule, yo->yo_name, "free"));
	}

	if (yo->yo_errors)
		(void) fprintf(ftbl, "\terrors_%s", modsym(mymodule, yo->yo_name, NULLCP));
	else
		(void) fprintf(ftbl, "\tNULL");
	(void) fprintf(ftbl, ",\n\n");
}
	...
	char *id;
{
	...
	if (yp->yp_flags & YP_ID) {
		...
		if (!(yp->yp_flags & YP_TAG)) {
			(void) printf("\n%*s", ++level * 4, "");
			if (!dflag && !(yp->yp_flags & YP_PULLEDUP)
			    && yp->yp_action1) {
	...
	case YP_ENUMLIST:
		...
		for (yv = yp->yp_value; yv; yv = yv->yv_next) {
			...
			(void) printf("%*s%s(%d)", level * 4, "", yv->yv_named, val2int(yv));
	...
	case YP_BITLIST:
		if (!dflag && yp->yp_strexp)
			(void) printf("BIT STRING\n%*s[[x %s$%s]]\n%*s{\n",
				      level * 4, "", yp->yp_strexp, yp->yp_intexp, level * 4, "");
		else
			(void) printf("BIT STRING {\n");
		level++;
		for (yv = yp->yp_value; yv; yv = yv->yv_next) {
	...
	case YP_SEQLIST:
		...
		for (y = yp->yp_type; y; y = y->yp_next) {
			do_type(y, level + ((y->yp_flags & (YP_ID | YP_TAG)) ? 1 : 2), "element");
			(void) printf("%s\n", y->yp_next ? ",\n" : "");
		}
	...
	case YP_SETLIST:
		...
		if (!dflag && !(yp->yp_flags & YP_PULLEDUP) && yp->yp_action1)
			...
			(void) printf(yp->yp_type ? " {\n" : " {");
		for (y = yp->yp_type; y; y = y->yp_next) {
			do_type(y, level + ((y->yp_flags & (YP_ID | YP_TAG)) ? 1 : 2), "member");
			(void) printf("%s\n", y->yp_next ? ",\n" : "");
		}
		(void) printf(yp->yp_type ? "%*s}" : "}", level * 4, "");
		break;
	...
	case YP_CHOICE:
		...
		(void) printf("{\n");
		for (y = yp->yp_type; y; y = y->yp_next) {
			do_type(y, level + ((y->yp_flags & (YP_ID | YP_TAG)) ? 1 : 2), "choice");
			(void) printf("%s\n", y->yp_next ? ",\n" : "");
		}
		(void) printf("%*s}", level * 4, "");
		break;
	...
	char *id;
{
	...
	*yp = z;
	...
	pass1_type(yyencpref, yydecpref, yyprfpref, mymodule, new_string(buffer), y);
}
 ...
 */
static char *
cmodsym(module, id, prefix, realid)
	register char *module, *id, *realid;
	char *prefix;
{
	char buf1[BUFSIZ], buf2[BUFSIZ], buf3[BUFSIZ];
	register struct triple *t;

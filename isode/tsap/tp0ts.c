	...
	struct TSAPdisconnect *td;
{
	...
	case TPDU_CC:
		...
		if (!(tb->tb_flags & TB_TCP) || t->t_tpdusize) {
			...
			else if (t->t_tpdusize > SIZE_MAXTP0 && !(tb->tb_flags & TB_TCP))
	...
	case TPDU_DR:
		...
		if (t->t_qbuf) {
			copyTSAPdata(t->t_qbuf->qb_data, t->t_qbuf->qb_len, td);
		} else
			td->td_cc = 0;
		goto out;
	case TPDU_ER:
...
#ifndef CCUR_X25		/* Since the MORE bit will have been set on what we have sent, and
				   will continue to be set on any more fragments that we might
				   send, and is not needed for the last fragment, the fact that so
				   far we have only managed part of the NSDU is not signigicant.
				   This is probably true for other X.25 providers too. */
				else if (nc > 0 && (tb->tb_flags & TB_X25)) {
					SLOG(tsap_log, LLOG_EXCEPTIONS, NULLCP,
					     ("partial write (%d of %d octets) to X.25", nc, cc));
					goto losing;
				}
#endif				/* CCUR_X25 */
	...
	while ((qb = tb->tb_qwrites.qb_forw) != &tb->tb_qwrites) {
		if ((nc = (*wfnx) (tb->tb_fd, qb->qb_data, qb->qb_len))
			...
			if (nc == NOTOK) {
				...
				SLOG(tsap_log, LLOG_EXCEPTIONS, NULLCP,
				     ("partial write (%d of %d octets) to X.25", nc, qb->qb_len));

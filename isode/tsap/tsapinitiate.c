...
int
TAsynConnRequest(calling, called, expedited, data, cc, qos, tc, td, async)
	struct TSAPaddr *calling, *called;
	int expedited, cc, async;
	char *data;
	struct QOStype *qos;
	...
	struct TSAPdisconnect *td;
{
	...
	if ((n = called->ta_naddr) <= 0)
		return tsaplose(td, DR_PARAMETER, NULLCP, "no NSAP addresses in called parameter");
	if (n > NTADDR)
	...
	result = TConnRequestAux(calling, called, expedited, data, cc, qos, tc, td, async);
...
static int
TConnRequestAux(calling, called, expedited, data, cc, qos, tc, td, async)
	struct TSAPaddr *calling, *called;
	char *data;
	int expedited, cc, async;
	struct QOStype *qos;
	...
	int async;
{
	...
	didone = 0;
	for (na = called->ta_addrs, n = called->ta_naddr - 1; n >= 0; na++, n--) {
		register int *ip;
		register char **ap;
		...
		for (la = calling->ta_addrs, l = calling->ta_naddr - 1; l >= 0; la++, l--)
			if (la->na_community == na->na_community)
				break;
		if (l < 0)
			la = NULLNA;

		if (realna->na_stack == NA_NSAP && realna->na_addr_class == NAS_UNKNOWN) {
			struct NSAPinfo *nsi;

			if ((nsi = getnsapinfo(realna)) == NULLNI)
				realna->na_addr_class = nsap_default_stack;
			else
				realna->na_addr_class = nsi->is_stack;
		}

		for (ns = nsaps; ns->ns_open; ns++)
			if (ns->ns_type == realna->na_stack && (ns->ns_stack & ts_stacks)) {
				if (realna->na_stack == NA_NSAP &&
				    ns->ns_stack == TS_X2584 && realna->na_addr_class != NAS_CONS)
					continue;
				break;
			}
		if (!ns->ns_open)
		...
		case TS_TP4:
			if ((result = (*ns->ns_open) (tb, calling, la,
						      realcalled, realna, te, async)) == NOTOK) {
				te = &tds;
				continue;
			}
			break;
	...
	struct TSAPdisconnect *td;
{
	...
	if (tb->tb_flags & TB_CONN) {
		(void) sigiomask(smask);
		return tsaplose(td, DR_OPERATION, NULLCP, "transport descriptor connected");
	}
	...
	case NOTOK:		/* try next nsap in list */
		...
		*tb->tb_called = *newtaddr(ta, &ta->ta_addrs[1], ta->ta_naddr - 1);	/* struct
											   copy */
	...
	struct TSAPdisconnect *td;
{
...
#else				/* Been told to move on by upper layers - must do what they say! */
	tb->tb_flags &= ~TB_CONN;
#endif
	...
	*tb->tb_called = *newtaddr(ta, &ta->ta_addrs[1], ta->ta_naddr - 1);	/* struct copy */
	...
	register struct TSAPaddr *ta;
{
	...
	static struct TSAPaddr tzs;
	register struct TSAPaddr *tz = &tzs;
	register struct NSAPaddr *na, *ca;

	SLOG(addr_log, LLOG_TRACE, NULLCP, ("ta2norm %s", taddr2str(ta)));
	...
	bzero((char *) tz, sizeof *tz);
	bcopy(ta->ta_selector, tz->ta_selector, tz->ta_selectlen = ta->ta_selectlen);
	ca = tz->ta_addrs;
	...
	SLOG(addr_log, LLOG_TRACE, NULLCP, ("ta2norm returns %s", taddr2str(tz)));
	...
	struct TSAPaddr *ta;
{
	...
	*nta = *taz;		/* struct copy */
	...
	bcopy(ta->ta_selector, &nta->ta_selector[2 + nna->na_addrlen], ta->ta_selectlen);

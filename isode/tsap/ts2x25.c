...
{
	if (close(fd) != 0) {
		LLOG(x25_log, LLOG_EXCEPTIONS, ("Close %d failed: %d", fd, errno));
	}
}
#endif
#endif
...
int
x25open(tb, local, remote, td, async)
	...
	struct TSAPdisconnect *td;
{
	...
	if (join_x25_server(fd, remote) == NOTOK) {
		if (async)
			...
			case EINPROGRESS:
				...
				if (peers[fd] == NULL && (peers[fd] = (struct NSAPaddr *)
							  malloc(sizeof **peers))
				    == NULL) {
		...
		LLOG(x25_log, LLOG_NOTICE, ("connection to %s failed", na2str(remote)));
		(void) close_x25_socket(fd);
		return (tb->tb_fd = NOTOK);
	}
      done:;
	...
	LLOG(x25_log, LLOG_NOTICE, ("connection %d to %s", fd, na2str(remote)));
...
#ifdef ULTRIX_X25_DEMSA
		(void) sprintf(buffer, "%c%d %d %s %s", NT_X2584, fd, vci, tbuf1, tbuf2);
#else
		(void) sprintf(buffer, "%c%d %s %s", NT_X2584, fd, tbuf1, tbuf2);
#endif
		break;
	case NA_X25:
#ifdef ULTRIX_X25_DEMSA
...
int
x25restore(tb, buffer, td)
	register struct tsapblk *tb;
	char *buffer;
	struct TSAPdisconnect *td;
{
	int fd;
	char dte1[NASIZE + 1], dte2[NASIZE + 1];
...
#ifdef AEF_NSAP
int
x25nsaprestore(tb, buffer, td)
	register struct tsapblk *tb;
	char *buffer;
	struct TSAPdisconnect *td;
{
	int fd;
	char dte1[NASIZE * 2 + 1], dte2[NASIZE * 2 + 1];
	register struct NSAPaddr *na;
	register struct tsapADDR *ta;

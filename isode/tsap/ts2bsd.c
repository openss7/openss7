	...
	char *data;
{
	...
	/* this is sleazy; we assume that tp4getCmsg is being called to find out what went wrong if 
	   errno is set to something */
	...
	if (result >= 0) {
		...
		if ((n > 0) && oc->ocm_control.ocm_cmhdr.cmsg_type == TPOPT_DISC_REASON) {
			tp4_disconnect_reason = ((struct tp_disc_reason *) oc)->dr_reason;
			oc = (union osi_control_msg *) (1 + (struct tp_disc_reason *) oc);
			n = oc->ocm_control.ocm_cmhdr.cmsg_len - sizeof(oc->ocm_control.ocm_cmhdr);
		}
#endif
		if (cc && type && data && n > 0) {
	...
	struct TSAPdisconnect *td;
{
	...
	if (gen2tp4X(&tb->tb_responding, &sock) == NOTOK)
		return tsaplose(td, DR_ADDRESS, NULLCP, "unable to parse remote address");
	len = sizeof *p;
	if (getsockopt(tb->tb_fd, SOL_TRANSPORT, TPOPT_PARAMS, (char *) p, &len)
		...
		if (p->p_xpd_service ? !expedited : expedited) {
			...
			if (setsockopt(tb->tb_fd, SOL_TRANSPORT, TPOPT_PARAMS,
				...
				SLOG(tsap_log, LLOG_EXCEPTIONS, "TPOPT_PARAMS", ("unable to set"));
		}
		if (expedited)
			tb->tb_flags |= TB_EXPD;
		if (p->p_tpdusize > 0) {
	...
	if (data && sendCmsg(tb->tb_fd, data, cc, TPOPT_CONN_DATA) == NOTOK)
 ...
 */
	if (connect(tb->tb_fd, (struct sockaddr *) ifaddr, ifaddr->siso_len)
		...
		if (!tp4_disconnect_reason)
			return tsaplose(td, DR_REFUSED, "connection", "unable to establish");
	...
	struct TSAPdisconnect *td;
{
	...
	case 0xffff:
		...
		if (connect(tb->tb_fd, (struct sockaddr *) ifaddr, ifaddr->siso_len) == NOTOK)
			switch (errno) {
			case EINPROGRESS:
				return CONNECTING_1;
			case EISCONN:
	...
	if (reason == NOTOK) {
		...
		len = sizeof *p;
		if (getsockopt(tb->tb_fd, SOL_TRANSPORT, TPOPT_PARAMS, (char *) p, &len) == NOTOK)
			SLOG(tsap_log, LLOG_EXCEPTIONS, "TPOPT_PARAMS", ("unable to get"));
		else {
			if (!p->p_xpd_service)
				tb->tb_flags &= ~TB_EXPD;
	...
	for (;;) {
		...
		if (msg->msg_controllen) {
			...
			if (oc->ocm_control.ocm_cmhdr.cmsg_type == TPOPT_DISC_REASON) {
				td->td_reason = ((struct tp_disc_reason *) oc)->dr_reason;
	...
	struct TSAPdisconnect *td;
{
	...
	if (sendCmsg(tb->tb_fd, cc, TPOPT_DISC_DATA, data) == NOTOK)
		result = tsaplose(td, DR_CONGEST, "TPOPT_DISC_DATA", "unable to send");
	else
		result = OK;
	freetblk(tb);
	return result;
	...
	struct TSAPdisconnect *td;
{
	...
	if (sendmsg(tb->tb_fd, msg, 0) == NOTOK)
		SLOG(tsap_log, LLOG_EXCEPTIONS, "TPOPT_DISC_DATA", ("unable to send"));
}
	...
	struct TSAPdisconnect *td;
{
	...
	if (getsockname(fd, (struct sockaddr *) ifaddr, &len) == NOTOK) {
		(void) tsaplose(td, DR_CONGEST, "listen", "getsockname failed for tp4restore");
		(void) close(fd);
		return NOTOK;
	}
	ifaddr->siso_len = len;
	...
	if (getpeername(fd, (struct sockaddr *) ifaddr, &len) == NOTOK) {
		(void) tsaplose(td, DR_CONGEST, "listen", "getpeername failed for tp4restore");
		(void) close(fd);
		return NOTOK;
	}
	ifaddr->siso_len = len;
	...
	struct TSAPdisconnect *td;
{
	...
	if (na->na_addrlen == 0) {	/* unique listen */
		...
		if (getsockname(sd, (struct sockaddr *) ifaddr, &len) == NOTOK) {
			(void) tsaplose(td, DR_CONGEST, "listen", "getsockname failed for unique");
			(void) close(sd);
			return NOTOK;
		}
		ifaddr->siso_len = len;
...
int
join_tp4_client(fd, remote_ta, td)
	int fd;

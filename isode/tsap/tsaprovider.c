	...
	struct TSAPdisconnect *td;
{
	...
	missingP(data);
	if (cc <= 0)
		return tsaplose(td, DR_PARAMETER, NULLCP, "illegal value for TSDU length (%d)", cc);
	missingP(td);
	...
	struct TSAPdisconnect *td;
{
	...
	if (cc <= 0)
		return tsaplose(td, DR_PARAMETER, NULLCP, "illegal value for XSDU length (%d)", cc);
	missingP(td);
	...
	if (tb->tb_flags & TB_EXPD)
		...
		result = tsaplose(td, DR_OPERATION, NULLCP, "expedited service unavailable");
	...
	struct TSAPdisconnect *td;
{
	...
	if ((is = getisoserventbyname("isore", "tsap")) == NULL)
		return tsaplose(td, DR_CONGEST, NULLCP, "ISO service tsap/isore not found");
	...
	struct TSAPdisconnect *td;
{
	...
	if (tb->tb_flags & TB_ASYN) {
		...
		if (ioctl(tb->tb_fd, I_SETSIG, result) == NOTOK)
			return tsaplose(td, DR_CONGEST, "failed", "ioctl I_SETSIG 0x%x", result);
#else

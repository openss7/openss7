	...
	int (*initfnx) (), readfnx;
{
	...
	case TPDU_CC:
		if (t->t_li < TPDU_MINLEN(t, CR))
			return DR_LENGTH;
		if (readx(fd, (char *) &t->t_cr, CR_SIZE(t), readfnx) != CR_SIZE(t))
			return DR_NETWORK;
		if ((vlen = t->t_vlen = t->t_li - TPDU_MINLEN(t, CR))) {
			if ((vptr = t->t_vdata = malloc((unsigned) vlen)) == NULL)
				return DR_CONGEST;
			if (readx(fd, t->t_vdata, t->t_vlen, readfnx) != t->t_vlen)
				return DR_NETWORK;
			for (; vlen > 0; vptr += len, vlen -= len) {
				...
				default:	/* IS 8073 says to ignore it on CRs */
					SLOG(tsap_log, LLOG_EXCEPTIONS, NULLCP,
					     ("unknown option 0x%x (length 0x%x) in %s TPDU", code,
					      len, TPDU_CODE(t) == TPDU_CR ? "CR" : "CC"));
	...
	case TPDU_DR:
		if (t->t_li < TPDU_MINLEN(t, DR))
			return DR_LENGTH;
		if (readx(fd, (char *) &t->t_dr, DR_SIZE(t), readfnx) != DR_SIZE(t))
			return DR_NETWORK;
	...
	case TPDU_DT:
		if (t->t_li < TPDU_MINLEN(t, DT))
			return DR_LENGTH;
		if (readx(fd, (char *) &t->t_dt, DT_SIZE(t), readfnx) != DT_SIZE(t))
			return DR_NETWORK;
	...
	case TPDU_ED:
		if (t->t_li < TPDU_MINLEN(t, ED))
			return DR_LENGTH;
		if (readx(fd, (char *) &t->t_ed, ED_SIZE(t), readfnx) != ED_SIZE(t))
			return DR_NETWORK;
	...
	case TPDU_ER:
		if (t->t_li < TPDU_MINLEN(t, ER))
			return DR_LENGTH;
		if (readx(fd, (char *) &t->t_er, ER_SIZE(t), readfnx) != ER_SIZE(t))
			return DR_NETWORK;
	...
	if ((len = TPDU_USRLEN(t))) {
		...
		if (readx
		    (fd, t->t_qbuf->qb_data = t->t_qbuf->qb_base, t->t_qbuf->qb_len =
		     len, readfnx) != len)
			return DR_NETWORK;
	}
	return OK;

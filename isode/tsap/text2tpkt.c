	...
	int isread;
{
	...
	case TPDU_DT:
	case TPDU_ED:
		TPKT_TYPE(TPDU_CODE(t) == TPDU_DT ? "DATA TRANSFER" : "EXPEDITED DATA TRANSFER");
		(void) ll_printf(lp, "%sSEQUENCE/ %s0x%x\n", rw,
				 t->t_dt.dt_nr & DT_EOT ? "<EOT>+" : "", t->t_dt.dt_nr & ~DT_EOT);
		break;
	case TPDU_ER:
		TPKT_TYPE("ERROR");
	...
	if (t->t_qbuf && t->t_qbuf->qb_data)
		type_data(lp, "QBUF", rw, t->t_qbuf->qb_len, t->t_qbuf->qb_data);
	...
	register struct tsapkt *t;
{
	...
	*bptr;				/* Pointer to our buffer */
	...
	while (isspace((u_char) *bptr) && (*bptr != '\0'))
		++bptr;
	if (toupper(*bptr) == 'C') {
		if (toupper(*(bptr + 1)) == 'R') {
			data = 0xE0;
	...
	case TPDU_CC:
		...
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
			t->t_li = data;
			(void) printf("Destination Reference [%04x]: ", data = ntohs(t->t_cr.cr_dstref));
			(void) fflush(stdout);
		}
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
		...
		if (fgets(buffer, sizeof buffer, stdin)) {
			...
			(void) printf("TPDU size [%02x]: ", data = t->t_cr.cr_tpdusize);
			(void) fflush(stdout);
		}
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
		...
		if (fgets(buffer, sizeof buffer, stdin)) {
			...
			(void) printf("Alternate classes [%02x]: ", data = t->t_cr.cr_alternate);
			(void) fflush(stdout);
		}
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
	...
	case TPDU_DR:
		...
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
			t->t_li = data;
			(void) printf("Destination Reference [%04x]: ", data = ntohs(t->t_dr.dr_dstref));
			(void) fflush(stdout);
		}
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
	...
	case TPDU_ER:
		...
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
			t->t_li = data;
			(void) printf("Destination Reference [%04x]: ", data = ntohs(t->t_er.er_dstref));
			(void) fflush(stdout);
		}
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
	...
	default:
		...
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
			t->t_li = data;
			(void) printf("Octets 3-4 [%04x]: ", data = ntohs(t->t_cr.cr_dstref));
			(void) fflush(stdout);
		}
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
			t->t_cr.cr_dstref = htons((u_short) data);
			(void) printf("Octets 5-6 [%04x]: ", data = ntohs(t->t_cr.cr_srcref));
			(void) fflush(stdout);
		}
		if (fgets(buffer, sizeof buffer, stdin)) {
			(void) sscanf(buffer, "%x", &data);
	...
	(void) printf("Calling TSAP ID [%d]: ", ntohs(t->t_calling));
	(void) fflush(stdout);
	if (fgets(buffer, sizeof buffer, stdin)) {
		(void) sscanf(buffer, "%d", &data);
		t->t_calling = htons((u_short) data);
		(void) printf("Called TSAP ID [%d]: ", ntohs(t->t_called));
		(void) fflush(stdout);
	}
	if (fgets(buffer, sizeof buffer, stdin)) {
		(void) sscanf(buffer, "%d", &data);

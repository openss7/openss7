	...
	int (*magic) ();
{
	...
	if ((n = ta->ta_naddr) > NTADDR)
		return tsaplose(td, DR_PARAMETER, NULLCP, "illegal number of network addresses");
	...
	if (n == 0) {
		...
		if ((n = ta->ta_naddr = na - ta->ta_addrs) == 0)
			return tsaplose(td, DR_PARAMETER, NULLCP, "no transport stacks active!?!");
	}
#ifdef	MGMT
	...
	(void) tsaplose(td, DR_PARAMETER, NULLCP, "no supported network addresses");
	...
	struct TSAPdisconnect *td;
{
	...
	bzero((char *) &tas, sizeof tas);
	bcopy(ta->ta_selector, tas.ta_selector, tas.ta_selectlen = ta->ta_selectlen);
	if (na) {
		tas.ta_naddr = 1;
	...
	struct TSAPdisconnect *td;
{
	...
	bcopy(tas.ta_selector, ta->ta_selector, ta->ta_selectlen = tas.ta_selectlen);
			...
			for (lb = LHead->lb_forw; lb != LHead;) {
				...
				if (FD_ISSET(fd, &ifds)) {
					...
					case LB_ACCEPT:
					{
						...
						if (newfd)
							*newfd = fd;
						if (magicfnx && (*magicfnx) (vecp, vec, td)
						    == NOTOK) {
							(void) (*closefnx) (fd);
							return NOTOK;
						}
...
int
TNetCheck(vecp, vec, ifds, ofds, nfds, td)
	int *vecp;
	char **vec;
	fd_set *ifds, *ofds;
	int nfds;
...
TNetClose(ta, td)
	...
	struct TSAPdisconnect *td;
{
	register struct listenblk *lb, *lp;
	...
	if (ta == NULLTA) {
		...
		if ((lb = findlblk(ta, LB_LISTEN)) == NULLLBP)
			return tsaplose(td, DR_PARAMETER, NULLCP, "no such transport addressess");
...
/* only tasks of child process have to be done      */
	...
	case NOTOK:
		(void) tsaplose(td, DR_CONGEST, "connection", "unable to fork, so rejecting");
	default:
#ifndef	LPP
	{
		...
		if (TInit(vecp, vec, ts, td) == NOTOK) {
			SLOG(tsap_log, LLOG_EXCEPTIONS, NULLCP,
			     ("TNetFork: TInit returns [%s]", TErrString(td->td_reason)));
			return pid;
		}

		if ((tb = findtblk(ts->ts_sd)) == NULL) {
			SLOG(tsap_log, LLOG_EXCEPTIONS, NULLCP, ("TNetFork: findtblk fails"));
			return pid;
		}
	...
	struct TSAPdisconnect *td;
{
	...
	if (na->na_domain[0]) {
		if ((hp = gethostbystring(na->na_domain)) == NULL)
			return tsaplose(td, DR_ADDRESS, NULLCP, "%s: unknown host", na->na_domain);
	} else
		hp = NULL;
...
tcpaccept1(lb, td)
	...
	struct TSAPdisconnect *td;
{
	int fd, tset = lb->lb_addr.ta_addrs->na_tset;
	...
	struct TSAPdisconnect *td;
{
	...
	(void) sprintf(buffer1, "%s+%d",
		       inet_ntoa(lb->lb_rem_isock.sin_addr), ntohs(lb->lb_rem_isock.sin_port));
...
#ifndef LPP
	if ((t = fd2tpkt(fd, tb->tb_initfnx, tb->tb_readfnx)) == NULL || t->t_errno != OK) {
		(void) tpktlose(tb, td, t ? t->t_errno : DR_CONGEST, NULLCP, NULLCP);
		goto out;
	}
	...
	if (lb->lb_addr.ta_selectlen > 0
		...
		|| bcmp(lb->lb_addr.ta_selector, t->t_called, lb->lb_addr.ta_selectlen))) {
	...
	struct TSAPdisconnect *td;
{
	...
	if ((fd = start_x25_server(ta->ta_addrs, SOMAXCONN, _listen_opts, SO_KEEPALIVE)) == NOTOK)
	...
	struct TSAPdisconnect *td;
{
	...
	if ((fd = join_x25_client(lb->lb_fd, &lb2->lb_rem_xsock, &lb2->lb_loc_xsock)) == NOTOK) {
	...
	struct TSAPdisconnect *td;
{
	...
	if ((vec[1] = x25save(fd, &lb->lb_rem_xsock, &lb->lb_loc_xsock, td)) == NULL)
		goto out;
	if ((vec[2] = tpkt2str(t)) == NULL) {
		(void) tsaplose(td, DR_CONGEST, NULLCP, NULLCP);
		goto out;
	...
	struct TSAPdisconnect *td;
{
	...
	cc = TS_SIZE;
	if ((fd = join_tp4_client(lb->lb_fd, &lb2->lb_rem_tsock, udata, &cc, &seq, &exp, td))
	    == NOTOK) {
		freelblk(lb2);
		return NOTOK;
	}
...
tp4accept2(lb, vecp, vec, td)
	...
	struct TSAPdisconnect *td;
{
	int cc, cmsgtype, fd = lb->lb_fd, len;
	char udata[TS_SIZE];
	register struct tsapblk *tb = lb->lb_tb;
	...
	if (getsockname(fd, (struct sockaddr *) ifaddr, &len) != NOTOK) {
		...
		SLOG(tsap_log, LLOG_EXCEPTIONS, "failed", ("getsockname on incoming connection"));
...
tp4accept2(lb, vecp, vec, td)
	...
	struct TSAPdisconnect *td;
{
	int cc, fd = lb->lb_fd, header_len, len;
	char data[TC_SIZE];
	register struct tsapblk *tb = lb->lb_tb;
	...
	if ((cc = recvfrom(fd, data, sizeof data, 0, (struct sockaddr *) tp, &header_len)) == NOTOK) {

	...
	struct t_info *info;
{
	...
	if ((td_current = (struct td_info *) calloc(1, sizeof(struct td_info))) == NULL) {
		LLOG(tsap_log, LLOG_EXCEPTIONS, ("Malloc failed in T_setinfo"));
		return NOTOK;
	}
	td_current->fd = fd;
	...
	int fd;
{
	...
	if (td_early == NULLTDINFO) {
		LLOG(tsap_log, LLOG_EXCEPTIONS, ("No transport descriptor to delete"));
		return NOTOK;
	}
	if (fd == td_head->fd) {
	...
	LLOG(tsap_log, LLOG_EXCEPTIONS, ("Couldn't find transport descriptor %d to delete", fd));
	return NOTOK;
}
	...
	int fields;
{
	...
	case T_CALL_STR:
		...
		if ((get_call->udata.buf =
		     (char *) calloc(1, sizeof(char) * max(info.tsdu, info.etsdu))) == NULL)
			break;
	...
	struct TSAPdisconnect *td;
{
	...
	if ((sndcall = (struct t_call *) T_alloc(tb->tb_fd, T_CALL_STR, T_ALL)) == NULL)
		return tli_lose(td, NOTOK, DR_CONGEST, "T_alloc");
	if ((rcvcall = (struct t_call *) T_alloc(tb->tb_fd, T_CALL_STR, T_ALL)) == NULL) {
	...
	struct TSAPdisconnect *td;
{
	...
	if (tb->tb_cc == -1) {	/* call not yet connected */
		...
		if (call->udata.len > 0)
			bcopy(call->udata.buf, tc->tc_data, tc->tc_cc = (int) call->udata.len);
		if (call->opt.len == sizeof(struct isoco_options)) {
			struct isoco_options *isoco_options =
			    (struct isoco_options *) call->opt.buf;
			if (isoco_options->expd == T_YES)
				tb->tb_flags |= TB_EXPD;
			else
				tb->tb_flags &= ~TB_EXPD;
	...
	struct TSAPdisconnect *td;
{
	...
	for (; uv->uv_base; uv++) {
		...
		if (t_snd(tb->tb_fd, uv->uv_base, uv->uv_len, flags | (more ? T_MORE : 0)) == NOTOK) {
			if (t_errno == TLOOK && t_look(tb->tb_fd) == T_DISCONNECT)
				(void) tp4getdis(tb->tb_fd, td);
			else
				(void) tsaplose(td, DR_CONGEST, "failed", "sendmsg");
	...
	struct TSAPdisconnect *td;
{
	...
	(void) sprintf(buffer, "%c%d.%d.%d.%ld.%ld.%ld.%ld.%ld.%ld.%ld %s %s", NT_XTI, fd, seq, exp,
		       info->addr, info->options, info->tsdu, info->etsdu, info->connect,
		       info->discon, info->servtype, taddr2str(&calling), taddr2str(&called));
	...
	struct netbuf *specific;
{
	...
	if (generic->ta_selectlen = xtb->ts_sellen)
		/* assign */

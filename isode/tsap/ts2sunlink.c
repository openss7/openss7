	...
	struct TSAPdisconnect *td;
{
	...
	if (gen2tp4X(&tb->tb_responding, &t->tp4_called,
		     tb->tb_initiating.ta_present ? &tb->tb_initiating.ta_addr : NULLNA) == NOTOK) {
		result = tsaplose(td, DR_ADDRESS, NULLCP, "unable to parse remote address");
		goto out;
	}
#ifdef SUNLINK_7_0		/* 13-Jun-91: added by Dave D. for SunNet OSI 7.0 bug. */
	...
	if (gen2tp4X(&tb->tb_initiating, &t->tp4_calling,
		     tb->tb_responding.ta_present ? &tb->tb_responding.ta_addr : NULLNA) == NOTOK) {
		result = tsaplose(td, DR_ADDRESS, NULLCP, "unable to parse local address");
		goto out;
	}
	...
	if (sendto(tb->tb_fd, data, cc, 0, (struct sockaddr *) t, sizeof(TP_MSG_CONNECT)) == NOTOK)
	...
	struct TSAPdisconnect *td;
{
	...
	default:
		(void) tsaplose(td, DR_NETWORK, NULLCP,
				"expecting 0x%x, got 0x%x", TP_CONNECT_CONF, t->tp4_event);
		break;
	}
	...
	struct TSAPdisconnect *td;
{
	...
	if (i > 0) {
		if (i > 2 * TS_SIZE) {
			result = tsaplose(td, DR_CONNECT, NULLCP, "too much initial user data");
			goto out;
		}
	...
	struct TSAPdisconnect *td;
{
	...
	if (responding)
		(void) gen2tp4X(&tb->tb_responding, &tp->tp4_called,
				tb->tb_initiating.ta_present ? &tb->tb_initiating.ta_addr : NULLNA);

	if (tb->tb_flags & TB_EXPD)
		tp->tp4_expedited = 1;

	pstat = signal(SIGPIPE, SIG_IGN);
	if (sendto(tb->tb_fd, data, cc, 0, (struct sockaddr *) tp, sizeof(TP_MSG_CONNECT)) == NOTOK)
		if (errno == EPIPE)
	...
	int async, oob;
{
	...
	for (;;) {
		...
		default:
			(void) tsaplose(td, DR_NETWORK, NULLCP,
					"unexpected response 0x%x", (int) (tp->tp4_event));
			break;
		}
	...
	struct TSAPdisconnect *td;
{
	...
	if ((cc = recvfrom(tb->tb_fd, td->td_data, sizeof(td->td_data),
			   MSG_OSI_OOB, (struct sockaddr *) tp, &header_len)) == NOTOK) {
		(void) tsaplose(td, DR_CONGEST, "failed", "recvfrom");
	} else if (tp->tp_event != TP_DISCONNECT_IND) {
		(void) tsaplose(td, DR_NETWORK, NULLCP,
				"unexpected non-DR 0x%x", (int) (tp->tp_event));
	} else {
...
int
start_tp4_server(sock, backlog, opt1, opt2, td)
...
#ifdef	notdef
	/* Ouch!! Trying to get the remote address off the socket only works for local connections. 
	   Non-local connections causes a core dump when I try to convert. */
...
static int
gen2tp4(generic, specific, template)
	...
	struct NSAPaddr *template;
{
	...
	if (generic->ta_naddr > 0) {
		...
		if (na->na_addrlen > 0)
			...
			case 0x47:
				if (na->na_addrlen < 3
				    || na->na_address[1] != 0x00 || na->na_address[2] != 0x04)
					break;
...
#ifndef	SUNLINK_5_2
		/* The SunLink OSI I'm using seems to require something although the ``manual''
		   says I don't.  Hmmmm, I wonder if I still need the OSI_ADDR_INIT? BTW,
		   osi_set_sap is one of the two functions I link from the -losi library.  When I
		   get the source for SunLink (maybe before I retire) I'll write my own set/get sap 
		   function and punt libosi.a */
...
static int
gen2tp4X(generic, specific, template)
	struct tsapADDR *generic;
	OSI_ADDR *specific;
	struct NSAPaddr *template;
{
	...
	OSI_ADDR *specific;
{
	...
	paddr_type = 0;
	if ((len = osi_get_sap(specific, buffer, sizeof buffer, OSI_NSAP, &paddr_type)) <= 0)
		return NOTOK;

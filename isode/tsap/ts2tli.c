	...
	int context;
{
	...
	if (ta && (ta->ta_naddr > 0 || ta->ta_selectlen > 0)) {
		...
		if ((qlen && bound->qlen < 1) ||
		    ...
		    memcmp(ta->ta_selector, bound_ta.ta_selector, bound_ta.ta_selectlen) != 0) {
			char buf[BUFSIZ];
	...
	struct TSAPdisconnect *td;
{
	...
	if ((sndcall = (struct t_call *) t_alloc(tb->tb_fd, T_CALL, T_ALL)) == NULL)
		return tli_lose(td, NOTOK, DR_CONGEST, "t_alloc");
	if ((rcvcall = (struct t_call *) t_alloc(tb->tb_fd, T_CALL, T_ALL)) == NULL) {
	...
	if (!expedited) {	/* expedited negotiated by default */
		...
		if ((def = (struct t_optmgmt *) t_alloc(tb->tb_fd, T_OPTMGMT, T_ALL))
		    == NULL) {
			t_free((char *) sndcall, T_CALL);
	...
	if (t_connect(tb->tb_fd, sndcall, rcvcall) == NOTOK) {
		...
		case TBADADDR:
			return tli_lose(td, NOTOK, DR_ADDRESS, "unable to establish connection");

		default:
			return tli_lose(td, NOTOK, DR_REFUSED, "unable to establish connection");
		}
	...
	struct TSAPdisconnect *td;
{
	...
	if (tb->tb_cc == -1) {	/* call not yet connected */
		...
		if (call->udata.len > 0)
			bcopy(call->udata.buf, tc->tc_data, (int) (tc->tc_cc = call->udata.len));
#ifdef RTnet_R02
	...
	struct TSAPdisconnect *td;
{
	...
	if (cp && ((i = strlen(cp)) > 0)) {
		if (i > 2 * TS_SIZE)
			return tsaplose(td, DR_CONNECT, NULLCP, "too much initial user data");
	...
	struct TSAPdisconnect *td;
{
	...
	DLOG(tsap_log, LLOG_TRACE, ("t_accept(%d, %d): seq=%d", tb->tb_fd, result, call->sequence));
	if (t_accept(tb->tb_fd, result, call) == NOTOK) {
		DLOG(tsap_log, LLOG_DEBUG,
		     ("t_accept fail: t_errno=%d, t_look=%d", t_errno, t_look(tb->tb_fd)));
	...
	struct TSAPdisconnect *td;
{
	...
	while (uv->uv_base != NULLCP) {
		...
		for (d = uv->uv_base, cc = 0, xv = uv; xv->uv_base != NULLCP; xv++) {
		...
		DLOG(tsap_log, LLOG_TRACE,
		     ("t_snd(fd=%d, buf=0x%x, len=%d, %s%s)",
		      tb->tb_fd, d, cc,
		      (expedited ? "EXPEDITED, " : ""), (more ? "MORE" : "NO_MORE")));
	...
	struct TSAPdisconnect *td;
{
	...
	while ((qb = tb->tb_qwrites.qb_forw) != &tb->tb_qwrites) {
		...
		if ((nc = t_snd(tb->tb_fd, qb->qb_data, qb->qb_len, 0)) != qb->qb_len) {
			if (nc == NOTOK) {
				if (t_errno == TLOOK && t_look(tb->tb_fd) == T_DISCONNECT) {
	...
	for (;;) {
		...
		if ((cc = t_rcv(tb->tb_fd, qb->qb_data, qb->qb_len, &flags))
			...
			case TLOOK:
				...
				default:
					(void) tsaplose(td, DR_CONGEST, NULLCP,
							"unexpected event %d", t_look(tb->tb_fd));
					break;
				}
				goto out;
			default:
	...
	struct TSAPdisconnect *td;
{
	...
	if ((call = (struct t_call *) t_alloc(tb->tb_fd, T_CALL, T_ALL)) == NULL)
		SLOG(tsap_log, LLOG_EXCEPTIONS, "TLose", ("unable to allocate"));
	else {
		call->sequence = tb->tb_flags & TB_CONN ? -1 : tb->tb_seq;
		if (t_snddis(tb->tb_fd, call) == NOTOK)
			SLOG(tsap_log, LLOG_EXCEPTIONS, "t_snddis", ("unable to send"));
	...
	struct TSAPdisconnect *td;
{
	...
	if (sscanf(buffer, "%d.%d.%d %s %s",
		   &fd, &tb->tb_seq, &exp, calling_buf, called_buf) != 5 || fd < 0)
...
/* ARGSUSED */
int
gen2tp4(generic, specific, context)	/* dependant on Addressing */
			...
			*cp++ = (char) (na->na_addrlen);
			...
			if (context != TLI_CTX_LISTEN && na) {
				...
				if ((dte_nsap = getisosnpa(na)) == NULLNA)
					...
					LLOG(tsap_log, LLOG_EXCEPTIONS,
					     ("ts2tli: No DTE in mapping"));
				else if (dte_nsap->na_dtelen <= 0)
					LLOG(tsap_log, LLOG_EXCEPTIONS,
					     ("ts2tli: DTE zero length"));
				else {
					int dlen;

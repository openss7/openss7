...
PE
name2value_dase(name, context, ontty, userdn, passwd, real_name)
	char *name, *context, *userdn, *passwd;
	int ontty;
	PE *real_name;
{
	int done, err, nfds, vecp;
	fd_set ifds;
	char *vec[NVEC + 1];
	...
	*real_name = NULLPE;
	...
	if (*name == '@') {
		vec[vecp = 0] = name;
		vecp++;
	} else if ((vecp = sstr2arg(name, NVEC, vec, ",")) == NOTOK) {
	...
	{
		register char **vp;
		register struct element_DASE_0 *dl, **dp;
	...
	if ((parm->context = str2qb(context, strlen(context), 1)) == NULL)
		goto no_mem;
	if (userdn && (parm->userdn = str2qb(userdn, strlen(userdn), 1)) == NULL)
		goto no_mem;
	if (passwd && (parm->passwd = str2qb(passwd, strlen(passwd), 1)) == NULL)
		goto no_mem;;
	...
	if ((err = pe2ps(ps, pe)) == NOTOK) {
		PY_advise(NULLCP, "unable to write query [%s]", ps_error(ps->ps_errno));
		goto out;
	}
	...
	for (done = 0; !done;) {
		...
		if ((in = ps2pe(ps)) == NULLPE) {
			PY_advise(NULLCP, "unable to read response [%s]", ps_error(ps->ps_errno));
			err++;
			break;
		}
		...
		case type_DASE_Provider__RSP_answer:
		{
			...
			register struct type_DASE_Query__RSP *ans = rsp->un.answer;
			...
			*real_name = ans->name, ans->name = NULLPE;
			...
			if (q = ans->diagnostic)
				for (p = q->qb_forw;
				     p != q && pp < ep; pp += p->qb_len, p = p->qb_forw)
		...
		default:
			PY_advise(NULLCP, "unexpected response from nameservice: %d", rsp->offset);
			goto you_lose;
		}
...
dase_init()
{
	...
	if ((tz = str2taddr(ns_address)) == NULL) {
		PY_advise(NULLCP, "unable to parse nameservice address \"%s\"", ns_address);
	      out:;
		SLOG(addr_log, LLOG_EXCEPTIONS, NULLCP, ("%s", PY_pepy));
		return NOTOK;
	}
	...
	register struct type_DASE_Callback__REQ *arg;
{
	...
	if (i > 10) {
		(void) printf("%d imprecise matches for '", i);
		print_qb(arg->string);
		(void) printf("', select fron them [y/n] ? ");
		if (yesno() != OK)
			goto send_rsp;
	} else {
		(void) printf("Please select from the following %d match%s for '",
			      i, i != 1 ? "es" : "");
		print_qb(arg->string);
		(void) printf("':\n");
	}
	...
	for (choice = arg->choices; choice; choice = choice->next) {
		...
		if ((j++ % 10) == 0 && choice->next) {
			(void) printf("Continue (%d more) [y/n] ? ", i - j + 1);
			if (yesno() != OK)
				break;
		}
	}
	...
	if ((result = pe2ps(ps, pe)) == NOTOK)
		PY_advise(NULLCP, "unable to write callback [%s]", ps_error(ps->ps_errno));
	else {
		PLOGP(addr_log, DASE_Message, pe, "message", 0);
...
read_el()
{
	register int i;
	register char *bp, *cp;
	char buffer[BUFSIZ], ufnrc[BUFSIZ];
	FILE *fp;
	...
	for (i = 0; fgets(bp = buffer, sizeof buffer, fp); i++) {
		...
		if (!isspace(*bp)) {
			...
			*cp++ = NULL;
			...
			PY_advise(NULLCP, "%s: unexpected blank at start of line %d", ufnrc, i);
			goto out;
		}

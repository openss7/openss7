...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/acsap/RCS/acsaprespond.c,v 9.0 1992/06/16 12:05:59 isode Rel";
#endif
	...
	struct AcSAPindication *aci;
{
	...
	pdu = NULL;
	if (ps->ps_ninfo < 1) {
		(void) acsaplose(aci, ACS_PROTOCOL, NULLCP, "no user-data on P-CONNECT");
		goto out2;
	}

	result = decode_ACS_ACSE__apdu(pe = ps->ps_info[0], 1, NULLIP, NULLVP, &pdu);
...

	{
		...
		if ((oid = AC_ASN_OID) == NULLOID) {
			(void) acsaplose(aci, ACS_PARAMETER, NULLCP, "%s: unknown", AC_ASN);
			goto out2;
		}

		for (pp = ps->ps_ctxlist.pc_ctx, i = ps->ps_ctxlist.pc_nctx - 1; i >= 0; pp++, i--)
			if (pp->pc_id == ctx) {
		...
		if (acb->acb_id == PE_DFLT_CTX) {
			(void) acsaplose(aci, ACS_PROTOCOL, NULLCP, "unable to find PCI for ACSE");
			goto out2;
		}
	}
	...
	if (apdu2info(acb, aci, aarq->user__information, acs->acs_info, &acs->acs_ninfo) == NOTOK)
		goto out2;
	...
	acs->acs_callingtitle.aei_ae_qualifier = aarq->calling__AE__qualifier;
	aarq->calling__AE__qualifier = NULLPE;
	if (aarq->calling__AP__invocation__id) {
		acs->acs_callingtitle.aei_ap_id = aarq->calling__AP__invocation__id->parm;
		acs->acs_callingtitle.aei_flags |= AEI_AP_ID;
	}
	if (aarq->calling__AE__invocation__id) {
		acs->acs_callingtitle.aei_ae_id = aarq->calling__AE__invocation__id->parm;
	...
	acs->acs_calledtitle.aei_ae_qualifier = aarq->called__AE__qualifier;
	aarq->called__AE__qualifier = NULLPE;
	if (aarq->called__AP__invocation__id) {
		acs->acs_calledtitle.aei_ap_id = aarq->called__AP__invocation__id->parm;
		acs->acs_calledtitle.aei_flags |= AEI_AP_ID;
	}
	if (aarq->called__AE__invocation__id) {
		acs->acs_calledtitle.aei_ae_id = aarq->called__AE__invocation__id->parm;
...
int
AcAssocResponse(sd, status, reason, context, respondtitle,
	...
	int prequirements, srequirements, settings, ndata;
	long isn;
	...
	struct AcSAPindication *aci;
{
	...
	default:
		return acsaplose(aci, ACS_PARAMETER, NULLCP, "bad value for status parameter");
	}
#ifdef	notdef
	missingP(context);
...
/* let presentation provider catch errors in presentation parameters */
	...
	pdu->result__source__diagnostic->un.acse__service__user = reason - ACS_USER_BASE;
	if (respondtitle) {
		...
		if (respondtitle->aei_flags & AEI_AP_ID)
			pdu->responding__AP__invocation__id = (struct type_ACS_AP__invocation__id *)
			    &respondtitle->aei_ap_id;
		if (respondtitle->aei_flags & AEI_AE_ID)
			pdu->responding__AE__invocation__id = (struct type_ACS_AE__invocation__id *)

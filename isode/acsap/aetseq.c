...
int
str2aet_seq(designator, qualifier, iep)
	char *designator, *qualifier;
	struct isoentity *iep;
{
	int hitdes, hitqual;
	char descriptor[BUFSIZ], desdflt[BUFSIZ], qualdflt[BUFSIZ];
	register struct isoentity *ie;
	struct isoentity ids, iqs;
	...
	if (!ie && hitqual) {
		...
		*ie = iqs;	/* struct copy */
		...
		if (hitdes) {
			...
			ie->ie_addr.pa_addr.sa_addr.ta_naddr = ids.ie_addr.pa_addr.sa_addr.ta_naddr;
		}
	}

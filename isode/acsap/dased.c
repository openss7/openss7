...
bind_to_dsa()
{
	...
	PY_advise(NULLCP, "unable to bind to directory (%s)",
		  be.dbe_type == DBE_TYPE_SECURITY ? "security error" : "DSA unavailable");
	...
	char *s;
{
	...
	if (encode_DASE_Callback__REQ(&pe, 1, NULL, NULLCP, req) == NOTOK) {
		advise(LLOG_EXCEPTIONS, NULLCP, "encode_DASE_Callback__REQ: %s", PY_pepy);
		goto out;
	}
	if (pe2ps(ps, pe) == NOTOK)
		adios(NULLCP, "pe2ps: %s", ps_error(ps->ps_errno));
	...
	if (decode_DASE_Callback__RSP(pe, 1, NULLIP, NULLVP, &rsp) == NOTOK) {
		advise(LLOG_EXCEPTIONS, NULLCP, "decode_DASE_Callback__RSP: %s", PY_pepy);
		goto out;
	}
	PLOGP(pgm_log, DASE_Message, pe, "message", 1);
	...
	DN dn;
{
	...
	if (result.rdr_entry.ent_attr == NULLATTR) {
		PY_advise(NULLCP, "no '%s' attribute in entry '%s'", DSAADDRESS_OID, dn2str(dn));
		return NULLPE;
	}

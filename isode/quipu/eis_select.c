...
Attr_Sequence
eis_select(eis, entryptr, dn, qctx, node)
	...
	DN node;
{
	...
	if (eis.eis_allattributes || qctx) {
		...
		if (entryptr->e_iattr) {
			for (temp = entryptr->e_iattr->i_default; temp != NULLATTR;
			     temp = temp->attr_link) {
				if (!as_find_type(entryptr->e_attributes, temp->attr_type)) {

					if (!(qctx && (AttrT_cmp(at_acl, temp->attr_type) == 0)))
						if ((temp->attr_acl != NULLACL_INFO) &&
						    (check_acl(dn, ACL_READ, temp->attr_acl, node)
						     != OK))
							continue;
			...
			if (entryptr->e_iattr->i_always) {
				if ((eis.eis_infotypes == EIS_ATTRIBUTETYPESONLY) && !qctx)
					...
					temp =
					    as_cpy_enc(entryptr->e_iattr->i_always, dn, node, qctx);
		...
		for (eptr = EIS_SELECT; eptr != NULLATTR; eptr = eptr->attr_link) {
			...
			if ((temp =
			     as_find_type(entryptr->e_attributes, eptr->attr_type)) == NULLATTR) {
				if (entryptr->e_iattr) {
					if (entryptr->e_iattr->i_default)
						if ((temp =
						     as_find_type(entryptr->e_iattr->i_default,
			...
			if (entryptr->e_iattr) {
				if ((temp =
				     as_find_type(entryptr->e_iattr->i_always,
						  eptr->attr_type)) != NULLATTR) {
...
Attr_Sequence
attr_eis_select(eis, as, dn, node)
	EntryInfoSelection eis;
	Attr_Sequence as;
	DN dn;
	DN node;
...
Attr_Sequence
dsa_eis_select(eis, entryptr, dn, qctx, node)
	EntryInfoSelection eis;
	Entry entryptr;
	DN dn;
	char qctx;			/* If TRUE - it is a Quipu context association */
	...
	DN node;
{
	...
	for (eptr = EIS_SELECT; eptr != NULLATTR; eptr = eptr->attr_link) {
		...
		if ((temp = as_find_type(entryptr->e_attributes, eptr->attr_type)) == NULLATTR)
			if ((temp = as_find_type(dsa_pseudo_attr, eptr->attr_type)) == NULLATTR)
				continue;
	...
	DN dn;
{
	...
	if (eis.eis_allattributes) {
		...
		for (temp = EIS_SELECT; temp != NULLATTR; temp = temp->attr_link) {
			if ((as =
			     as_find_type(entryptr->e_attributes, temp->attr_type)) == NULLATTR)
				continue;

	...
	char authtype;
{
	...
	if (!dsp && arg->mea_changes && (arg->mea_changes->em_type == EM_ADDATTRIBUTE)) {
#ifdef QUIPU_CONSOLE
	...
	authp = real_entry->e_authp ? real_entry->e_authp->ap_modification : AP_SIMPLE;
	if (!manager(binddn))
	...
	for (eptr = arg->mea_changes; eptr != NULLMOD; eptr = eptr->em_next) {
		...
		case EM_ADDATTRIBUTE:
			if (add_attribute(entryptr, eptr->em_what, error, effdn, target) != OK) {
				entry_free(entryptr);
				return (DS_ERROR_REMOTE);
			}
			break;
	...
	if (unravel_attribute(entryptr, error) != OK) {
		...
		if ((acl_list != entryptr->e_acl)
			...
			if ((as = entry_find_type(entryptr, at_acl)) == NULLATTR) {
				LLOG(log_dsap, LLOG_EXCEPTIONS, ("Attempt to remove ACL"));
				error->dse_type = DSE_SERVICEERROR;
	...
	register Attr_Sequence as;
{
	...
	if (as->attr_type == 0) {
		/* X.501 9.6.2 ! */
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("Remove of unknown attribute type"));
	...
	DN requestor, dn;
{
	...
	for (rmavs = rmas->attr_value; rmavs != NULLAV; rmavs = rmavs->avseq_next) {
		...
		/* Removing the nonLeafObject objectclass makes it a leaf again, if this is
		   allowed. */

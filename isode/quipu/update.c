	...
	Entry myentry;
{
	...
	if ((myentry->e_parent != NULLENTRY) && (myentry->e_parent->e_leaf)) {
		...
		if (rename(newfile, filename) == NOTOK) {
			...
			if (link(savefile, filename) == NOTOK && !fileexists(filename))
	...
	int fd;
{
	...
	if (akid == NULLENTRY || (akid->e_data != E_DATA_MASTER && akid->e_data != E_TYPE_SLAVE)) {
		error->dse_type = DSE_SERVICEERROR;
		error->ERR_SERVICE.DSE_sv_problem = DSE_SV_DITERROR;
		return (DS_X500_ERROR);
	}
	...
	for (avs = avs_head; avs != NULLAV; avs = avs->avseq_next) {
		...
		if ((eptr = local_find_entry(dsainfo->edb_name, FALSE)) == NULLENTRY) {
			...
			if ((version = eptr->e_edbversion) == NULLCP) {
				LLOG(log_dsap, LLOG_TRACE, ("update_aux: edbversion was NULLCP"));
				version = "0000000000Z";
			}
		}
	...
	Avlnode *oldkids;
{
	...
	if (avl_apply(e->e_children, quick_unrav, (caddr_t) &error, NOTOK, AVL_PREORDER) == NOTOK) {
		log_ds_error(&error);
		return NOTOK;
	}
	...
	struct oper_act **newop;
{
	...
	if (result->gr_pe == NULLPE) {
		DLOG(log_dsap, LLOG_TRACE,
		     ("  EDBs are the same (%d): %s", on->on_id, on->on_getedb_ver));
		goto out;
	}
	...
	g_entry_cnt = 0;
	if (avl_apply(newkids, link_child, (caddr_t) eptr->e_children, NOTOK, AVL_INORDER) == NOTOK) {
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("Child problem in new EDB - continuing with old"));
		/* ??? should we free newkids here ??? */
		goto out;
	}
	...
	/* Arrange for connection to shut in 30 seconds unless used by something else in that time
...
static
edb_start(arg, error, result, binddn, fd)
	struct getedb_arg *arg;
	struct DSError *error;
	struct getedb_result *result;
	DN binddn;
	int fd;
{
	...
	/* Make file name - this is where we could be clever an pick up old files etc.  Worry about 
	   optimization later. */

	(void) sprintf(buffer, "%s/%s.XXXXXX", edbtmp_path, result->gr_version);
	if ((fname = mktemp(strdup(buffer))) == NULLCP) {
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("Too many getedbs at once '%s'", fname));
		goto out;
	}
	...
	if ((fptr = fopen(fname, "w")) != NULL) {
		...
		if ((fps = ps_alloc(std_open)) == NULLPS) {
			LLOG(log_dsap, LLOG_EXCEPTIONS, ("Could not alloc PS file '%s'", fname));
			goto out;
		}
		if ((std_setup(fps, fptr)) == NOTOK) {
			ps_free(fps);
			LLOG(log_dsap, LLOG_EXCEPTIONS, ("Could not open PS file '%s'", fname));
			goto out;
		}
	} else {
		(void) umask(um);
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("Could not open EDB/PE file '%s'", fname));
		goto out;
	}
	...
	if ((spe = prim2seq(pe)) == NULLPE) {
		ps_free(fps);
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("How comes its not a seq !?!"));
		goto out;
	}
	...
	if (fclose(fptr) != 0) {
		ps_free(fps);
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("Could not close EDB/PE file '%s'", fname));
		fptr = (FILE *) NULL;
		goto out;
	}
	ps_free(fps);
	...
	if ((fptr = fopen(fname, "r")) != NULL) {
		if ((fps = ps_alloc(std_open)) == NULLPS) {
			LLOG(log_dsap, LLOG_EXCEPTIONS, ("Could not alloc PS file (R)'%s'", fname));
			goto out;
		}
		if ((std_setup(fps, fptr)) == NOTOK) {
			ps_free(fps);
			LLOG(log_dsap, LLOG_EXCEPTIONS, ("Could not open PS file (R)'%s'", fname));
			goto out;
		}
	} else {
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("Could not open EDB/PE file (R)'%s'", fname));
		goto out;
	}
...
static
edb_continue(arg, error, result, binddn, fd)
	struct getedb_arg *arg;
	struct DSError *error;
	struct getedb_result *result;
	DN binddn;
	int fd;
{
	...
	/* step through the list to find oper... */
	for (nextop = edbops; nextop != NULLEDBOP; trail = nextop, nextop = nextop->next) {
...
*/
	...
	case OK:
		...
		result->gr_pe = pe;
		result->gr_nextEntryPos = arg->ga_nextEntryPos + arg->ga_maxEntries;
	...
	struct oper_act **newop;
{
	...
	narg->ga_previous = (struct getedb_result *) smalloc(sizeof(struct getedb_result));
	...
	struct getedb_result *result;
{
	...
	if (loop = arg->ga_previous) {	/* ASSIGN */
		...
		if ((spe = prim2seq(pe)) == NULLPE) {
			LLOG(log_dsap, LLOG_EXCEPTIONS, ("First EDB bit is not a seq !?!"));
			pe_free(pe);
			return FALSE;
		}
	...
	int n;
{
	...
	for (i = 0; i < n; i++) {
		if ((pe = ps2pe(ps)) == NULLPE) {
			if ((ps->ps_errno == PS_ERR_EOF) || (ps->ps_errno == 0))
				return DONE;

			pe_free(*pep);
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("EDB/PE read failed (%s)", ps_error(ps->ps_errno)));
			return NOTOK;

	...
	if (di->di_type == DI_INVOKE) {
		...
		while (tmp_avs != (AV_Sequence) 0) {
			if ((((struct quipu_call *) tmp_avs->avseq_av.av_struct)->assoc_id ==
				...
				(char *) 0)) {
				/* Same Assoc Number and the connection has not been finished!
				   Match? */
				break;
			}
		...
		/* Now we have the appropriate quipu_call structure, we can start filling out the
		   op_list */
		/* Note that we add them to the pending operations. Move them over to invoked
		   operations */
		/* in task_select() once they are chosen... */

		/* Before that, move to the end of a possibly null op_list and add another op onto
		   the end. */
		...
		case OP_SEARCH:
		{
			tmp_op_list->operation_list->base_object =
			    dn_cpy(arg->arg_sr.sra_baseobject);
			break;
		}
		case OP_ADDENTRY:
		{
		...
		tmp_op_list->operation_list->start_time = strdup(utct2str(&ut));
		tmp_op_list->operation_list->finish_time = (char *) 0;
	} else if ((di->di_type == DI_FINISH) || (di->di_type == DI_ABORT)) {
		tmp_avs = open_call_avs;

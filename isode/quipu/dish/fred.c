	...
	char **argv;
{
	...
	if (test_arg(argv[1], "-display", 7)) {
		...
		if (s_photo)
			set_av_pe_print(s_photo, strdup(isodefile("g3fax/Xphoto", 1)));
		return;
	}
	...
	char **vec;
{
	int seqno;
	char *cp, mbox[BUFSIZ];
	register struct dn_seq *dlist, *dp;
	...
	struct dn_seq *dlist;
{
	...
	for (;;) {
		...
		for (ptr = sr->CSR_entries; ptr; ptr = ptr->ent_next)
			cache_entry(ptr, sa->sra_eis.eis_allattributes, sa->sra_eis.eis_infotypes);
...
struct dn_seq *expand_full(), *expand_partial();
	...
	if (ptr = find_list_cache(la->lsa_object = dn, SVC_NOSIZELIMIT)) {
		...
		*complete = 1;
		...
		for (;;) {
			...
			la->lsa_object = le->ERR_REFERRAL.DSE_ref_candidates->cr_name;
		}

		cache_list(lr->lsr_subordinates, lr->lsr_limitproblem, dn, SVC_NOSIZELIMIT);
...
static
build_ufnrc(argc, argv)
	int argc;
	char **argv;
{
	envlist el;
	register envlist en, *ep;
	...
	char *s;
{
	int i, j;
	char buffer[BUFSIZ];
	struct dn_seq *result = NULL, *ptr, *tmp;
	static PS nps = NULL;

	if (dns == NULL)
		return NULL;
	if (nps == NULLPS
	    && ((nps = ps_alloc(str_open)) == NULLPS || str_setup(nps, NULLCP, 0, 0) == NOTOK)) {
		if (nps)
			ps_free(nps);
		return dns;
	}
...
#define	NCHOICES	25
	...
	if (i > 10) {
		if (s)
			...
			(void) sprintf(buffer, "%d imprecise matches, select one of them [y/n] ? ");
		if (yesno(buffer)) {
			dn_seq_free(dns);
			return NULLDNSEQ;
		}
	...
	char *fancy;
{
	...
	if (dns == NULL) {
		if (ufn_partials) {
			if (fred_list && frompipe && ufn_partials->dns_next) {
				...
				send_pipe_aux
				    ("mUnable to resolve name, partial matches follow...\n");
				if (read_pipe_aux(buffer, sizeof buffer) < 1) {
					(void) fprintf(stderr, "read failure\n");
					remote_prob = TRUE;
					goto losing;
	...
	if (i == 1 && !fancy && fred_list && frompipe && rps->ps_byteno == 0 && opt->ps_byteno == 0 && fdx_reset(rps) == OK) {	/* MAJOR 
																   HACK 
																 */
...
static struct template {
	...
	"title", NULL, 0, NULL, "documentTitle", NULL, 0, NULL,
#define	LEVEL_POSTAL	0
	    ...
	    3, NULL, "documentLocation", "Location:  ", 3, NULL,
#define	LEVEL_MBOX	4
	    "rfc822Mailbox", "  Mailbox: ",
	    4, NULL, "otherMailbox", "  ", 4, NULL, "textEncodedORaddress", "  ", 4, NULL,
#ifdef USE_PP
	    "mhsORAddresses", "  X.400:   ", 4, NULL,
#endif
	    ...
	    7, NULL, "owner", "Owner:       ", 7, NULL, "documentAuthor", "Author:      ", 7, NULL,
#define	LEVEL_AUTHOR	7
	    ...
	    10, NULL,
	    "favouriteDrink", "Drinks:       ", 10, NULL, "photo", "Picture:      ", 10, NULL,
#ifdef	sparc
	    "audio", "Audio:        ", 10, NULL,
#endif
NULL};
...
showfred(mydn, islong, subdisplay)
	DN mydn;
	char islong, subdisplay;
{
	int didtime, hasauthor, haspost, level, nchild, pos, seqno;
	register struct template *t;
	register Attr_Sequence eptr;
	register AV_Sequence avs, avp;
	AttributeType rdn_at, inf_at;
 ...
 */
			...
			if (AttrT_cmp(eptr->attr_type, t->t_at) == 0) {
				...
				if (t->t_level == LEVEL_AUTHOR)
					if (AttrT_cmp(eptr->attr_type, t_authorCN) == 0) {
					    ...
					    if (hasauthor && AttrT_cmp(eptr->attr_type, t_authorSN)
						== 0)
						continue;
				...
				if (avs = eptr->attr_value) {
					...
					showfredattr(&avs->avseq_av);
					ps_print(RPS, "\n");
					for (avp = avs->avseq_next; avp; avp = avp->avseq_next) {
						if (t->t_prefix)
							ps_printf(RPS, "%*s", i, "");
						else
							ps_print(RPS, "\n");
	...
	if (subdisplay) {
		...
		list_arg.lsa_common.ca_servicecontrol.svc_options |= SVC_OPT_DONTDEREFERENCEALIAS;

		if (ptr = find_list_cache(list_arg.lsa_object = mydn, SVC_NOSIZELIMIT)) {
			if (ptr->list_subs)
				nchild = fred_children(mydn, ptr->list_subs, ptr->list_problem);
		...
		for (;;) {
			...
			list_arg.lsa_object = list_error.ERR_REFERRAL.DSE_ref_candidates->cr_name;
		}
...
static
fred_children(parentdn, ptr, prob)
	DN parentdn;
...
"aliasedObjectName", &t_alias,
	    ...
	    "slaveDSA", &t_slave, "surName", &t_surname, "title", &t_title, NULL};
	...
	char islong;
{
	...
	if ((newentry = local_find_entry(read_arg.rda_object = adn,
					 FALSE)) == NULLENTRY
	    || !newentry->e_lock || (islong && !newentry->e_complete)) {
		if (rebind() != OK)
			return newentry;
	...
	int islong;
{
	...
	if ((nps = ps_alloc(str_open)) == NULLPS || str_setup(nps, NULLCP, 0, 0) == NOTOK) {
		if (nps)
			ps_free(nps);
	...
	for (eptr = theEntry->e_attributes; eptr; eptr = eptr->attr_link)
		if (eptr->attr_type->oa_syntax == s_dn) {
			...
			&& (eptr->attr_type->oa_syntax == s_photo
			    || eptr->attr_type->oa_syntax - AV_WRITE_FILE == s_photo)) {
			int avsno;

			for (avs = eptr->attr_value, avsno = 1; avs; avs = avs->avseq_next, avsno++) {
				int cc, i, j;
				register char *cp, *dp;
				char buffer[BUFSIZ], faxtopbm[BUFSIZ], tmp1[BUFSIZ], tmp2[BUFSIZ];
				AttributeValue av;
				FILE *fp;
				PE pe;
				PS ps;
				...
				(void) sprintf(buffer, "%s < %s > %s", faxtopbm, tmp1, tmp2);
				...
				if (i
				    ...
				    || fstat(fileno(fp), &st) == NOTOK || (cc = st.st_size) == 0) {
					if (!i && fp)
						(void) fclose(fp);
					(void) unlink(tmp2);
				...
				if (avs != eptr->attr_value || avs->avseq_next)
					(void) sprintf(buffer + strlen(buffer), "#%d", avsno);
...
fred_as()
{
	...
	if (!as) {
		...
		if (at = t_mbox)
			as = as_merge(as, as_comp_new(AttrT_cpy(at), NULLAV, NULLACL_INFO));
		if (at = t_phone)
			as = as_merge(as, as_comp_new(AttrT_cpy(at), NULLAV, NULLACL_INFO));
		if (at = t_surname)
			as = as_merge(as, as_comp_new(AttrT_cpy(at), NULLAV, NULLACL_INFO));
...
fred_full()
{
	...
	if (t_photo) {
		...
		if (dad_flag || (s_photo && (s = get_syntax_table(s_photo))
				 && s->s_pe_print))
			as = as_merge(as, as_comp_new(AttrT_cpy(t_photo), NULLAV, NULLACL_INFO));
		else {
			int i;
			register Attr_Sequence ptr, *pptr;

			for (pptr = &as, ptr = *pptr; ptr; pptr = &ptr->attr_link, ptr = *pptr) {

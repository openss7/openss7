		...
		for (; temp != NULLATTR; temp = temp->attr_link)
			if (AttrT_cmp(as->attr_type, temp->attr_type) == 0) {
				...
				if (avs_cmp(as->attr_value, temp->attr_value) != 0)
					emnew =
					    modify_avs(as->attr_value, temp->attr_value,
						       as->attr_type);
				break;
			}
	...
	AttributeType at;
{
	...
	for (x = b; x != NULLAV; x = x->avseq_next) {
		...
		if (y == NULLAV) {
			...
			emnew->em_what =
			    as_comp_new(at, avs_comp_new(AttrV_cpy(&x->avseq_av)), NULLACL_INFO);
			emnew->em_next = NULLMOD;
		} else
			removed_all = FALSE;
		if (emnew != NULLMOD)
	...
	char **argv;
{
	...
	mod_arg.mea_changes->em_what =
	    as_comp_new(at, avs_comp_new(str_at2AttrV(buffer, at)), NULLACL_INFO);
	...
	while (start) {
		...
		if (*ptr == 0) {
			if ((a_t = AttrT_new(start->mod)) == NULLAttrT) {
				ps_printf(OPT, "invalid attribute type '%s'", start->mod);
				return (NOTOK);
			}
			emnew->em_what = as_comp_new(a_t, NULLAV, NULLACL_INFO);

		} else if ((emnew->em_what = str2as(start->mod)) == NULLATTR) {
			ps_printf(OPT, "invalid attribute '%s' ", start->mod);
			return (NOTOK);
		}
				...
				if (emnew->em_type == -1) {
					if (start->add == 0) {	/* Removing... */
						if (emnew->em_what->attr_value == NULLAV) {
							...
							if (attr_val_match == 0) {
								ps_printf(OPT,
									  "Can't remove value that is not present.\n");
								return (NOTOK);
							}
						}
					} else

	...
	CommonArgs *ca;
{
	...
	else if ((read_arg.rda_common.ca_servicecontrol.
		  svc_options & SVC_OPT_DONTDEREFERENCEALIAS) == 0)
		deref = TRUE;
	...
	if (do_read)
		if (noread_flag)
			if (read_entry == NULLENTRY) {
				ps_print(OPT,
					 "No data in cache, but '-cache' prevents me reading it!\n");
				return (-1);
			} else
				ps_print(OPT,
					 "Read required, but '-cache' specified,\nproceeding using cache...\n");
		else {
			...
			if (read_arg.rda_common.ca_security != (struct security_parms *) 0) {
			...
			cache_entry(&(result.rdr_entry), read_arg.rda_eis.eis_allattributes,
				    value_flag);
		...
		if (test_arg(argv[x], "-all", 1)) {
			...
			for (x++; x < argc;) {
				...
				if ((at = AttrT_new(argv[x])) != NULLAttrT)
					as_flag =
					    as_merge(as_flag,
						     as_comp_new(AttrT_cpy(at), NULLAV,
								 NULLACL_INFO));
				else
					break;
				shuffle_up(argc--, argv, x);

		...
		*local = st_alloc();
		...
		if (check_filter_presrch(arg->sra_filter, error, target) != OK) {
...
#ifdef TURBO_INDEX
			(*local)->st_optimized = optimized_filter(arg->sra_filter);
#endif
			...
			*filter_ps->ps_ptr = 0;
			...
			case SRA_ONELEVEL:
				LLOG(log_stat, LLOG_TRACE,
				     ("Search onelevel %s", filter_ps->ps_base));
				break;
			case SRA_WHOLESUBTREE:
				LLOG(log_stat, LLOG_TRACE,
				     ("Search subtree %s", filter_ps->ps_base));
				break;
			default:
			...
			if (!((*local)->st_optimized) && optimized_only
			    && !ismanager && arg->sra_subset == SRA_WHOLESUBTREE) {
				LLOG(log_dsap, LLOG_EXCEPTIONS,
				     ("Non-optimized filter not allowed"));
			...
			if ((arg->sra_subset == SRA_ONELEVEL) ||
				...
				switch (find_child_entry
					((*local)->st_baseobject, &(arg->sra_common), dnbind,
					 NULLDNSEQ, FALSE, &(entryptr), error, di_p)) {
				case DS_OK:
				...
				default:
					/* SCREAM */
					LLOG(log_dsap, LLOG_EXCEPTIONS,
					     ("do_ds_search() - find_child_entry failed 1"));
					st_comp_free(*local);
					*local = NULL_ST;
					return (DS_ERROR_LOCAL);
				}
				...
				switch (find_entry
					((*local)->st_baseobject, &(arg->sra_common), dnbind,
					 NULLDNSEQ, FALSE, &(entryptr), error, di_p, OP_SEARCH)) {
				case DS_OK:
					/* Filled out entryptr - carry on */
					break;
				case DS_CONTINUE:
				...
				default:
					/* SCREAM */
					LLOG(log_dsap, LLOG_EXCEPTIONS,
					     ("do_ds_search() - find_entry failed 1"));
					st_comp_free(*local);
					*local = NULL_ST;
					return (DS_ERROR_LOCAL);
				}
			...
			if (arg->sra_subset == SRA_WHOLESUBTREE) {
				...
				for (dn = (*local)->st_baseobject; dn != NULLDN;
				     dn = dn->dn_parent, x++) ;
				if (x < search_level) {
					if (!ismanager) {
						/* Too high */
				...
				if (entryptr->e_data != E_TYPE_CONSTRUCTOR) {
					...
					if ((*local)->st_baseobject != NULLDN) {
						if ((result->CSR_entries =
						     filterentry(arg, entryptr, (*local)->st_bind,
								 authtype, &saclerror, (*local),
								 1)) != NULLENTRYINFO)
							(*local)->st_size--;
						else if (saclerror < 0) {
							error->dse_type = DSE_SECURITYERROR;
		...
		if (size < 0) {
			...
			result->CSR_limitproblem =
			    ...
			    == SVC_NOSIZELIMIT
			    || arg->sra_common.ca_servicecontrol.svc_sizelimit
			    > admin_size ? LSR_ADMINSIZEEXCEEDED : LSR_SIZELIMITEXCEEDED;
		...
		if (timelimit <= timenow) {
			...
			result->CSR_limitproblem =
			    ...
			    == SVC_NOTIMELIMIT
			    || arg->sra_common.ca_servicecontrol.svc_timelimit
			    > admin_time ? LSR_ADMINSIZEEXCEEDED : LSR_TIMELIMITEXCEEDED;
			/* should fill out a POQ */
			return (DS_OK);
		}
...
static
check_filter_presrch(fltr, error, dn)
...
static
check_filterop_presrch(fltr, error, dn)
	register Filter fltr;
	struct DSError *error;
	DN dn;
{
...
static
check_filteritem_presrch(fitem, error, dn)
	...
	DN dn;
{
	...
	case FILTERITEM_EQUALITY:
		...
		if (fitem->fi_type != FILTERITEM_APPROX)
			if ((fitem->fi_ifp =
			     av_cmp_fn(fitem->UNAVA.ava_type->oa_syntax)) == NULLIFP)
				return (invalid_matching(fitem->UNAVA.ava_type, error, dn));
		...
		if ((fitem->fi_type == FILTERITEM_GREATEROREQUAL
			...
			ftype_add(&ftlist, fitem->UNAVA.ava_type, maxlen, (char *) NULL);
		}
		break;
	case FILTERITEM_SUBSTRINGS:
		...
		if (case_exact_match(av_syntax))
			...
			for (loopavs = fitem->UNSUB.fi_sub_any; loopavs != NULLAV;
			     loopavs = loopavs->avseq_next)
...
static
apply_search(arg, error, result, local, refer, ismanager, authtype, saclerror)
	...
	int *saclerror;
{
	...
	if ((*local)->st_subset == SRA_BASEOBJECT) {
		...
		switch (find_entry
			((*local)->st_baseobject, &(arg->sra_common), (*local)->st_bind, NULLDNSEQ,
			 FALSE, &(entryptr), error, &(di_tmp), OP_SEARCH)) {
		case DS_OK:
			/* Filled out entryptr - carry on */
			break;
		case DS_CONTINUE:
		...
		default:
			/* SCREAM */
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("do_ds_search() - find_child_entry failed 2"));
			return (DS_ERROR_LOCAL);
		}
		/* must be an alias or referral task... */
		if ((*local)->st_baseptr == NULLENTRY)
	...
	*saclerror = 0;
	...
	case SRA_ONELEVEL:
	case SRA_WHOLESUBTREE:
		einfo = filterchildren(arg, entryptr, local, refer, ismanager, authtype, saclerror);
		break;
	default:
	...
	if (einfo != NULLENTRYINFO)
		if (result->CSR_entries == NULLENTRYINFO)
			...
			entryinfo_merge(result->CSR_entries, einfo, !arg->sra_hitalias);
	...
	if (eptr != NULLENTRYINFO && size != -1)
		if (*ska->ska_einfo == NULLENTRYINFO)
			*ska->ska_einfo = eptr;
			...
			entryinfo_merge(*ska->ska_einfo, eptr, !ska->ska_arg->sra_hitalias);
...
*/
				...
				if (ska->ska_domore)
					ska->ska_domore = (ska->ska_tmp < (SEARCH_DELTA_SIZE / 5));
				if (!ska->ska_domore)
					(void) dsa_wait(0);
			}
/*
...
static EntryInfo *
filterchildren(arg, entryptr, local, refer, ismanager, authtype, saclerror)
	struct ds_search_arg *arg;
	Entry entryptr;
	struct ds_search_task **local, **refer;
	int ismanager;
	...
	int *saclerror;
{
	...
	if (check_acl
	    ((*local)->st_bind, ACL_READ, entryptr->e_acl->ac_child,
	     (*local)->st_baseobject) == NOTOK) {
		return (NULLENTRYINFO);
	}
	...
	if ((ptr = entryptr->e_children) == NULLAVL || entryptr->e_allchildrenpresent == FALSE) {
	...
	} else if (arg->sra_subset == SRA_ONELEVEL && get_sibling_index((*local)->st_baseobject)) {
...
EntryInfo *
filterentry(arg, entryptr, binddn, authtype, saclerror, local, dosacl)
	struct ds_search_arg *arg;
	...
	char dosacl;
{
	...
	einfo->ent_attr = eis_select(arg->sra_eis, entryptr, binddn, qctx
				     && arg->sra_eis.eis_allattributes, einfo->ent_dn);
...
static
check_filter(fltr, entryptr, binddn)
	register Filter fltr;
	register Entry entryptr;
	DN binddn;
{
...
static
check_filterop(fltr, entryptr, op, binddn)
	register Filter fltr;
	register Entry entryptr;
	int op;
	DN binddn;
...
static
check_filteritem(fitem, entryptr, binddn)
	register struct filter_item *fitem;
	register Entry entryptr;
	DN binddn;
{
	...
	if ((as = as_find_type(entryptr->e_attributes, at)) == NULLATTR) {
		if (entryptr->e_iattr) {
			for (ptr = entryptr->e_iattr->i_default; ptr != NULLATTR;
			     ptr = ptr->attr_link) {
	...
	register int mode;
{
	...
	for (; avs != NULLAV; avs = avs->avseq_next) {
		switch (((int) (*fitem->fi_ifp)
			 (avs->avseq_av.av_struct, fitem->UNAVA.ava_value->av_struct))) {
		case 0:
			return (OK);
		case 1:
...
static
aux_substr_search(fitem, avs, chrmatch)
		...
		do {
			...
			if (chrmatch[*compstr++] != *temp++) {
				DLOG(log_dsap, LLOG_DEBUG,
				     ("initial failure (%s, %s)", top,
				      (char *) fitem->UNSUB.fi_sub_initial->avseq_av.av_struct));
				return (NOTOK);
			}
		} while (*temp != '\0');
	}
	for (loopavs = fitem->UNSUB.fi_sub_any; loopavs != NULLAV;
	     loopavs = loopavs->avseq_next, compstr += offset)
		if ((offset = attr_substr(compstr, &loopavs->avseq_av, chrmatch)) == -1) {
			DLOG(log_dsap, LLOG_DEBUG,
			     ("any failure (%s, %s)", top, (char *) loopavs->avseq_av.av_struct));
			return (NOTOK);
		}
	if (fitem->UNSUB.fi_sub_final != NULLAV) {
		...
		while (*temp != '\0') {
			if (phoneflag) {
				if (*temp == ' ' || *temp == '-') {
					...
					compstr--;
				while (compstr >= top && (*compstr == ' ' || *compstr == '-'))
					compstr--;
			} else
				compstr--;
			temp++;

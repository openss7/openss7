...
#ifdef QUIPU_CONSOLE
AV_Sequence open_call_avs;		/* SPT: This sequence holds all the current open calls. */
extern PS opt;
	...
	int secs;
{
	...
	if (secs != NOTOK) {
		/* if secs == NOTOK we want to block, otherwise set watchdog, but beware of setting 
		   watchdog off accidentally ! */
	...
	if (TNetAcceptAux(&vecp, vec, &newfd, NULLTA, nads, &iads, &wads, NULLFD, secs, td) ==
	    NOTOK) {
		watch_dog_reset();
 ...
 *	attempt_restart (NOTOK);
	...
	for (cn = connlist; cn != NULLCONN; cn = next_cn) {
		...
		case CN_OPEN:
			if (FD_ISSET(cn->cn_ad, &iads)) {
				DLOG(log_dsap, LLOG_DEBUG,
				     ("Activity on association: %d", cn->cn_ad));
				conn_dispatch(cn);
				result = OK;
		...
		default:
			if (FD_ISSET(cn->cn_ad, &iads)) {
				LLOG(log_dsap, LLOG_EXCEPTIONS,
				     ("Unexpected activity on association %d ... aborting",
				      cn->cn_ad));
#ifdef DEBUG
...
/* SPT: The sub parts to build an openCall attribute type. */
	...
	while (tmp_avs != NULLAV) {
		...
		if ((tmp_oc->assoc_id == cn->cn_ad) && (tmp_oc->finish_time == (char *) 0)) {
...
#ifdef SPT_DEBUG
			fprintf(stderr,
				"Attention. A call is being added when assoc is still open. Why?\n");
#endif
			return;
		}
		tmp_avs = tmp_avs->avseq_next;
...
#ifdef SPT_DEBUG
		fprintf(stderr,
			"dsa_wait.c: Hey, got an closing analyse with no calls present. Error!\n");
#endif
		return;
	}

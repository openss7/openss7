	...
	if ((as = as_find_type(dsa_pseudo_attr, att)) == NULLATTR) {
		...
		if ((AttributeValue) (av = str2AttrV(buffer, att->oa_syntax)) == NULLAttrV) {
			...
			if (dsa_pseudo_attr == NULLATTR)
				...
				dsa_pseudo_attr =
				    as_merge(dsa_pseudo_attr, as_comp_new(att, avs, NULLACL_INFO));
		}
	} else {
		...
		while (tmp_op_list != (struct op_list *) 0) {
			if ((tmp_op_list->operation_list->start_time != (char *) 0) &&
			    (timenow < gtime(ut2tm(str2utct(tmp_op_list->operation_list->start_time,
							    strlen(tmp_op_list->operation_list->
								   start_time)))) + 300)) {
				num_of_ops++;
			}
		...
		while (tmp_op_list != (struct op_list *) 0) {
			if ((tmp_op_list->operation_list->finish_time != (char *) 0) &&
			    (timenow <
			     gtime(ut2tm
				   (str2utct
				    (tmp_op_list->operation_list->finish_time,
...
#endif
		if (as != NULLATTR) {	/* If this attribute is in the entry then remove it *//* Otherwise do nothing. No value & no type so nowt to do */
			if (as == dsa_pseudo_attr) {
				dsa_pseudo_attr = as->attr_link;
				AttrT_free(as->attr_type);
				free((char *) as);
			} else {	/* However, it may be questionable how this piece of code
					   gets activated */
				/* as one must read a pseudo attr and therefore have a connection.
				   But.. */
				Attr_Sequence trail;
	...
	DN dn;
{
	...
	if (as_find_type(dsa_pseudo_attr, at) == NULLATTR)
		...
		dsa_pseudo_attr = as_merge(dsa_pseudo_attr, as_comp_new(at, avs, NULLACL_INFO));
	...
	if (dsa_pseudo_attr) {
		...
		if ((fptr = fopen(filename, "w")) == (FILE *) NULL) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("can't write DSA pseudo entry: \"%s\" (%d)", filename, errno));
		}
		(void) umask(um);
		...
		if (ps->ps_errno != PS_ERR_NONE) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("write DSA ps error: %s", ps_error(ps->ps_errno)));
			(void) fclose(fptr);
			return;
		}
		ps_free(ps);
	...
	if ((fptr = fopen(filename, "r")) == (FILE *) NULL)
		...
		if ((dsa_pseudo_attr = get_attributes(fptr)) == NULLATTR)
			...
			LLOG(log_dsap, LLOG_TRACE,
			     ("Error in DSA pseudo entry: \"%s\" (%d)", filename, errno));
...
#endif
			LLOG(log_dsap, LLOG_TRACE,
			     ("Error in DSA real entry: \"%s\" (%d)", filename, errno));

	...
	Filter f;
{
	...
	case FILTER_ITEM:
		...
		case FILTERITEM_SUBSTRINGS:
			if ((fi->UNSUB.fi_sub_initial == NULLAV && fi->UNSUB.fi_sub_final == NULLAV)
	...
	case FILTER_NOT:
		return (FALSE);
	default:
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("optimized_filter - unknown filter type"));
		return (FALSE);
	}
	/* NOTREACHED */
}
	...
	struct search_kid_arg *ska;
{
	...
	*ska->ska_einfo = NULLENTRYINFO;
	...
	if (e->e_children == NULLAVL) {
		search_refer(ska->ska_arg, e, ska->ska_local, ska->ska_refer, ska->ska_ismanager);
		return;
	}
	...
	if (ska->ska_arg->sra_searchaliases && pindex->i_nonlocalaliases != (Entry *) 0) {
	...
	struct search_kid_arg *ska;
{
	...
	if (ska->ska_arg->sra_searchaliases && pindex->i_nonlocalaliases != (Entry *) 0) {
		for (tmp = pindex->i_nonlocalaliases; *tmp; tmp++) {
			...
			i = th_prefix((*ska->ska_local)->st_originalbase, (*tmp)->e_alias);
			if (i > 0) {
				(void) do_alias(ska->ska_arg, *tmp, ska->ska_local);
			}
		}
	...
	struct search_kid_arg *ska;
{
	...
	for (tmp = pindex->i_nonleafkids; *tmp != NULLENTRY; tmp++) {
		if (((*tmp)->e_children != NULLAVL && (*tmp)->e_allchildrenpresent == FALSE)
			...
			if (check_acl((*ska->ska_local)->st_bind, ACL_READ,
				...
				search_refer(ska->ska_arg, *tmp,
					     ska->ska_local, ska->ska_refer, ska->ska_ismanager);
			}
		}
	}
}
...
static EntryInfo *
turbo_filterkids(e, f, ska, pindex, toplevel)
	Entry e;
	Filter f;
	struct search_kid_arg *ska;
	Index *pindex;
	int toplevel;
{
	...
	default:
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("turbo_filterkids: cannot filter type (%d)", f->flt_type));
		return (NULLENTRYINFO);
	}
	/* NOT REACHED */
}
	...
	EntryInfo **eilist;
{
	...
	for (i = 0; i < node->in_num; i++) {
		if (g_ska->ska_arg->sra_searchaliases && node->in_entries[i]->e_alias != NULLDN)
			continue;
	...
	Index_node *bignode;
{
	...
	for (i = 0; i < node->in_num; i++) {
		...
		if (bignode->in_num >= bignode->in_max) {
			if (bignode->in_max == 0) {
				bignode->in_max = 1;
				bignode->in_entries = (struct entry **) malloc((unsigned)
									       sizeof(struct entry
										      *));
				bignode->in_entries[0] = tmp1;
			} else {
				bignode->in_max *= 2;
				bignode->in_entries = (struct entry **) realloc((char *)
										bignode->in_entries,
										(unsigned)
										(sizeof
										 (struct entry *) *
...
/* ARGSUSED */
static EntryInfo *
turbo_item(e, f, ska, pindex, toplevel)
	Entry e;
	...
	int toplevel;
{
	...
	case FILTERITEM_SUBSTRINGS:
		if (f->UNSUB.fi_sub_initial == NULLAV && f->UNSUB.fi_sub_final == NULLAV) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("turbo_item: non-optimized substring filter"));
			break;
		}
		...
		if (f->UNSUB.fi_sub_initial == NULLAV) {
			...
			thestring = strrev(f->UNSUB.fi_sub_final->avseq_av.av_struct);

			/* no final substring so use initial */
		} else if (f->UNSUB.fi_sub_final == NULLAV) {
			theindex = pindex[i].i_root;
			thestring = strdup(f->UNSUB.fi_sub_initial->avseq_av.av_struct);
			...
			theindex = (flen > ilen ? pindex[i].i_rroot : pindex[i].i_root);
		}
		if (phoneflag)
...
/* ARGSUSED */
static EntryInfo *
turbo_and(e, f, ska, pindex, toplevel)
	Entry e;
	Filter f;
	struct search_kid_arg *ska;
	Index *pindex;
	int toplevel;
{
	...
	if (f == NULLFILTER) {
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("turbo_and - filter has no optimized component"));
		return (NULLENTRYINFO);
	}
...
static EntryInfo *
turbo_or(e, f, ska, pindex, toplevel)
	Entry e;
	Filter f;
	struct search_kid_arg *ska;
	Index *pindex;

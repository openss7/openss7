...
/* If password is present, but zero length, treat as though absent */
	...
	case DBA_AUTH_NONE:
		if (((arg->dba_dn == NULLDN) && auth_bind == 1) || (auth_bind > 1)) {
		      out:;
#ifndef NO_STATS
...
/* Now we're sure dba_dn contains a valid pointer, can decode it */
	...
	case DBA_AUTH_SIMPLE:
		...
		pslog(log_stat, LLOG_NOTICE, buff, (IFP) dn_print, (caddr_t) arg->dba_dn);
#endif
		/* Can't check simple credentials from DSP (livelock risk). Hence treat DSP
		   accesses as unauthenticated. */
	...
	case DBA_AUTH_STRONG:
		...
		pslog(log_stat, LLOG_NOTICE, buff, (IFP) dn_print, (caddr_t) arg->dba_dn);
#endif
		/* Strong authentication is not yet supported. It will eventually be possible to
		   check strong credentials over DSP. For the moment, accept them and treat as NONE 
		   over DSP, but reject over DAP. */
		if (dsap_security && dsap_security->serv_ckpath && dsap_security->serv_cknonce) {
			...
			if (rc != OK) {
				...
				if (dn_cmp(real_name, arg->dba_dn) == OK) {
					...
					(void) sprintf(buff,
						       "User != Authenticated User, ie %s != ",
						       dn2str(arg->dba_dn));
					pslog(log_dsap, LLOG_NOTICE, buff, (IFP) dn_print,
					      (caddr_t) real_name);
					error->dbe_version = DBA_VERSION_V1988;
					error->dbe_type = DBE_TYPE_SECURITY;
					error->dbe_value = DSE_SC_AUTHENTICATION;
...
/* If we fall through to here, credentials are simple or protected simple */
	...
	if ((res =
	     really_find_entry(arg->dba_dn, TRUE, NULLDNSEQ, FALSE, &(entryptr), &(err),
			       &(dsas))) == DS_OK) {
		/* is it really OK ??? */
		if ((entryptr->e_data == E_TYPE_CONSTRUCTOR)
			...
			res =
			    constructor_dsa_info(dn_found, NULLDNSEQ, FALSE, entryptr, &err, &dsas);
			dn_free(dn_found);
		}
	}
	switch (res) {
	...
	case DS_CONTINUE:
		...
		if (arg->dba_auth_type == DBA_AUTH_SIMPLE) {
			...
			cma->cma_purported.ava_value = (AttributeValue) calloc(1, sizeof(attrVal));
			cma->cma_purported.ava_value->av_syntax = str2syntax("protectedPassword");
	...
	if ((as = as_find_type(entryptr->e_attributes,
		...
		/* No password in entry. Simple authentication is not possible for entities without 
		   passwords. Hence, give the `inappropriate authentication' message. */
		error->dbe_version = DBA_VERSION_V1988;
		error->dbe_type = DBE_TYPE_SECURITY;
		error->dbe_value = DSE_SC_AUTHENTICATION;
		return (DS_ERROR_CONNECT);
	...
	struct oper_act *on;
{
	...
	if (on->on_bind_compare == NULLCONN) {
		...
		if (on->on_resp.di_result.dr_res.dcr_dsres.res_cm.cmr_matched) {
			...
			on->on_bind_compare->cn_authen =
			    on->on_bind_compare->cn_start.cs_ds.ds_bind_arg.dba_auth_type;
	...
	register struct connection *conn;
{
	...
	if (fstat(conn->cn_ad, &st) != NOTOK) {
		...
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("*** fd %d: errno=%d ***", conn->cn_ad, errno));
	...
	char *str;
{
	...
	if (reject_prefix_list == NULLDNSEQ)
		...
		for (loop = reject_prefix_list; loop->dns_next != NULLDNSEQ;
		     loop = loop->dns_next) ;
		loop->dns_next = dsa;
	}
}
	...
	char *str;
{
	...
	if (accept_prefix_list == NULLDNSEQ)
		...
		for (loop = accept_prefix_list; loop->dns_next != NULLDNSEQ;
		     loop = loop->dns_next) ;
		loop->dns_next = dsa;
	}
}

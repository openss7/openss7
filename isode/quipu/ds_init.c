...
dsa_init()
{
	...
	if (my_entry->e_dsainfo != NULLDSA) {
		...
		my_entry->e_dsainfo->dsa_version = TidyString(strdup(quipuversion));
		if (as = entry_find_type(my_entry, at_version))
			if (strcmp((char *) as->attr_value->avseq_av.av_struct,
				...
				if (parse_status == 0)
					if (my_entry->e_data == E_DATA_MASTER) {
						if (my_entry->e_parent != NULLENTRY)
							my_entry->e_parent->e_edbversion =
							    new_version();
						...
						if (turbo_write(my_entry) != OK)
							fatal(-33,
							      "self rewrite failed - check database");
#else
						akid =
						    (Entry) avl_getone(my_entry->e_parent->
								       e_children);
						if (journal(akid) != OK)
							fatal(-33,
							      "self rewrite failed - check database");

	...
	struct connection *cn;
{
	...
	if ((ds->ds_start.acs_start.ps_called.pa_selectlen !=
	    ...
	    (dsaladdr->pa_selectlen && bcmp(ds->ds_start.acs_start.ps_called.pa_selector,
					    dsaladdr->pa_selector, dsaladdr->pa_selectlen) == 0)) {

	      out:;
		LLOG(log_dsap, LLOG_EXCEPTIONS, ("Bad Selector (%d): %s",
						 ds->ds_sd,
						 paddr2str(&(ds->ds_start.acs_start.ps_calling),
							   NULLNA)));
	...
	if ((ds->ds_start.acs_start.ps_called.pa_addr.sa_selectlen !=
	    ...
	    (dsaladdr->pa_addr.sa_selectlen
	     && bcmp(ds->ds_start.acs_start.ps_called.pa_addr.sa_selector,
		     dsaladdr->pa_addr.sa_selector, dsaladdr->pa_addr.sa_selectlen) == 0))
		goto out;
	...
	case DS_CTX_X500_DAP:
		LLOG(log_stat, LLOG_NOTICE,
		     ("X500 DAP context association (%d): %s", cn->cn_ad,
		      pa2str(&(ds->ds_start.acs_start.ps_calling))));
		break;
	case DS_CTX_X500_DSP:
		LLOG(log_stat, LLOG_NOTICE,
		     ("X500 DSP context association (%d): %s", cn->cn_ad,
		      pa2str(&(ds->ds_start.acs_start.ps_calling))));
		break;
	case DS_CTX_QUIPU_DSP:
		LLOG(log_stat, LLOG_NOTICE,
		     ("QUIPU DSP context association (%d): %s", cn->cn_ad,
		      pa2str(&(ds->ds_start.acs_start.ps_calling))));
		break;
	case DS_CTX_INTERNET_DSP:
		LLOG(log_stat, LLOG_NOTICE,
		     ("Internet DSP context association (%d): %s", cn->cn_ad,
		      pa2str(&(ds->ds_start.acs_start.ps_calling))));
		break;
	default:
		LLOG(log_stat, LLOG_EXCEPTIONS,
		     ("UNKNOWN context association (%d): %s", cn->cn_ad,
		      pa2str(&(ds->ds_start.acs_start.ps_calling))));
		break;
	}
#endif

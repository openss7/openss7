	...
	char *fname;
{
	...
	if (file == NULL) {
		...
		LLOG(log_dsap, LLOG_NOTICE,
		     ("WARNING - Can't open \"%s\" (%d)- should I be able to ?", fname, errno));
		if (refreshing)
...
/* ARGSUSED */
Avlnode *
get_entries_aux(file, parent, version, dtype, cache_age)
#ifdef TURBO_DISK
	GDBM_FILE file;
#else
	FILE *file;
...
Avlnode *
get_entries(file, parent, version, dtype)
#ifdef TURBO_DISK
	GDBM_FILE file;
#else
	FILE *file;

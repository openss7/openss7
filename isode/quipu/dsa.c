	...
	char **argv;
{
	...
	struct quipu_call *open_call;	/* This bit added for SPT for a spot of debugging. */
	struct quipu_call *oc_tmp;	/* This bit added for SPT for a spot of debugging. */
	struct UTCtime ut;
#endif				/* QUIPU_CONSOLE */
			...
			if (secs != 0 && (log_dsap->ll_events & LLOG_NOTICE)) {
				...
				if (new_size > proc_size) {
					LLOG(log_dsap, LLOG_NOTICE,
					     ("Process grown by %d bytes", new_size - proc_size));
					proc_size = new_size;
				}
			}
#endif
	...
	int secs;
{
	...
	/* Looks like connections are all inactive now. Wait 'till all DSP connections closed. */
...
/* VARARGS */
	...
	static setdsauid() {
		...
		if (setgid(buf.st_gid) == -1)
			LLOG(log_dsap, LLOG_NOTICE,
			     ("Can't set gid %d (database directory \"%s\")", buf.st_uid, treedir));

		if (setuid(buf.st_uid) == -1)
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("Can't set uid %d (database directory \"%s\")", buf.st_uid, treedir));
	}
...

	{
		...
		if (fpid != -1) {
			LLOG(log_dsap, LLOG_FATAL,
			     ("Quipu restart attempted in %d seconds (sig %d)", secs, sig));
#ifndef NO_STATS
			LLOG(log_stat, LLOG_NOTICE,
			     ("RESTARTING with pid %d (%s)", fpid, mydsaname));
#endif
		} else {
#ifndef NO_STATS
	...
	void
	 open_call_avs_clearup() {
		...
		while (tmp_avs != NULLAV) {
			...
			if ((tmp_oc->finish_time != (char *) 0) &&	/* The call session has
									   finished */
			    (timenow > gtime(ut2tm(str2utct(tmp_oc->finish_time, strlen(tmp_oc->finish_time)))) + 300)) {	/* and 
																   past 
																   its 
																   sell 
																   by 
				...
				if ((tmp_avs == open_call_avs) && (tmp_avs->avseq_next == NULLAV)) {
					/* If there is only one item, then remove the attribute
					   completely. */
					Remove_openCall_attribute();
					open_call_avs = NULLAV;	/* Reset this pointer to say that
								   it is empty. */
#ifdef SPT_DEBUG
					fprintf(stderr,
						"There are no more open_call_avs's. All timed out.\n");
#endif
				} else if (tmp_avs == open_call_avs) {
					/* There are many attributes, and we want to remove the
					   first one. */
					...
					at = AttrT_new("openCall");
					dsa_pseudo_attr =
					    as_merge(dsa_pseudo_attr,
						     as_comp_new(at, open_call_avs, NULLACL_INFO));
					AttrT_free(at);
				} else {
					/* There are many attribute values and we want to remove
					   one of them, but not the first */
					t1 = open_call_avs;
					while ((t1 != NULLAV) && (t1->avseq_next != tmp_avs)) {
						t1 = t1->avseq_next;
					}

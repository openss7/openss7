	...
	register struct task_act *tk;
{
	...
	if (quipu_shutdown) {
		...
		if (tk->tk_conn->cn_ctx == DS_CTX_X500_DAP)
			force_close(tk->tk_conn->cn_ad, (struct DSAPindication *) NULL);
		else
...
#ifndef NO_STATS
	if (*local == NULL_ST)
		log_x500_event(arg, tk->tk_conn->cn_ctx, orig, base, tk->tk_conn->cn_ad, tk);
#endif
	...
	case OP_ADDENTRY:
		dsa_ret = do_ds_addentry(&(arg->arg_ad), err, orig, base, &(di), dsp, authtype);
		break;

	case OP_REMOVEENTRY:
		dsa_ret = do_ds_removeentry(&(arg->arg_rm), err, orig, base, &(di), dsp, authtype);
		break;

	case OP_MODIFYENTRY:
		dsa_ret = do_ds_modifyentry(&(arg->arg_me), err, orig, base, &(di), dsp, authtype);
		break;

	case OP_MODIFYRDN:
		dsa_ret = do_ds_modifyrdn(&(arg->arg_mr), err, orig, base, &(di), dsp, authtype);
		break;

	case OP_GETEDB:
		orig = tk->tk_conn->cn_dn;
		dsa_ret = do_get_edb(&(arg->arg_ge), err, &(res->res_ge), orig, tk->tk_conn->cn_ad);
		break;
	...
	case DS_X500_ERROR:
		...
		if (tk->tk_dx.dx_arg.dca_dsarg.arg_type == OP_SEARCH) {
			free((char *) tk->tk_resp.di_result.dr_res.dcr_dsres.res_sr.srr_un.
			     srr_unit);
	...
	case DS_CONTINUE:
		...
		if (task_chain(tk, di) != OK) {
			...
			if (tk->tk_dx.dx_arg.dca_dsarg.arg_type == OP_SEARCH) {
				free((char *) tk->tk_resp.di_result.dr_res.dcr_dsres.res_sr.srr_un.
				     srr_unit);
				free((char *) tk->tk_resp.di_result.dr_res.dcr_dsres.res_sr.
				     srr_next->srr_un.srr_unit);
				free((char *) tk->tk_resp.di_result.dr_res.dcr_dsres.res_sr.
				     srr_next);
			}
			task_extract(tk);
	...
	struct task_act *tk;
{
	...
	if (tk_sr->srr_next == NULLSRR) {
		...
		tk_sr->srr_next =
		    (struct ds_search_result *) calloc(1, sizeof(struct ds_search_result));
		tk_sr->srr_next->srr_correlated = TRUE;
		tk_sr->srr_next->srr_un.srr_unit =
		    (struct ds_search_unit *) calloc(1, sizeof(struct ds_search_unit));
	...
	struct task_act *tk;
{
	...
	if (context == DS_CTX_X500_DAP)
		...
		if (arg->arg_type != OP_GETEDB)
			pslog(log_stat, LLOG_TRACE, "DAP Originator", (IFP) dn_print,
			      (caddr_t) orig);
	}
}
#endif

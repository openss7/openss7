	...
	char authtype;
{
	...
	switch (find_entry
		(target, &(arg->rda_common), binddn, NULLDNSEQ, FALSE, &(entryptr), error, di_p,
		 OP_READ)) {
	case DS_OK:
	...
	/* User can be authenticated by using strong authentication for this operation, or by using 
	   the credntials from the bind. */
	...
	/* entry has got a full list of attributes, eventually select one required */
	if (check_acl((authtype % 3) >= authp ? binddn : NULLDN, ACL_READ,
		      entryptr->e_acl->ac_entry, realtarget) == NOTOK) {
		if (dsp
		    && (check_acl(binddn, ACL_READ, entryptr->e_acl->ac_entry, realtarget) == OK)) {
			error->dse_type = DSE_SECURITYERROR;
	...
	if (entryptr->e_dsainfo && need_pseudo_dsa(entryptr, arg)) {
		...
		/* Its a 7.0 or better DSA. pseudo attributes asked for. special routing req'd */

		if (dn_cmp(realtarget, mydsadn) == 0) {
			/* Its me - generate result */

			if ((result->rdr_entry.ent_attr =
			     dsa_eis_select(arg->rda_eis, entryptr, dsp ? NULLDN : binddn,
	...
	if (cant_use_cache(entryptr, binddn, arg->rda_eis, realtarget)) {
		int res = referral_dsa_info(realtarget, NULLDNSEQ, FALSE, entryptr, error, di_p,
					    arg->rda_common.ca_servicecontrol.
	...
	if ((result->rdr_entry.ent_attr =
	     eis_select(arg->rda_eis, entryptr, dsp ? NULLDN : binddn, quipu_ctx,
			realtarget)) == NULLATTR)
		if (!arg->rda_eis.eis_allattributes && arg->rda_eis.eis_select != NULLATTR) {
			error->dse_type = DSE_ATTRIBUTEERROR;
			error->ERR_ATTRIBUTE.DSE_at_name = get_copy_dn(entryptr);
...
static
cant_use_cache(ptr, dn, eis, target)
	Entry ptr;
	DN dn;
	...
	DN target;
{
	...
	if (eis.eis_allattributes) {
		...
		/* for each attribute in eis.eis_select, see is user entitled to it. */
		...
		for (as = eis.eis_select; as != NULLATTR; as = as->attr_link) {
			if (entry_find_type(ptr, as->attr_type) == NULLATTR)
				if (attribute_not_cached
				    (ptr, dn, grab_oid(as->attr_type), target, ACL_READ, dfltacl))
					return TRUE;
...
static
attribute_not_cached(ptr, dn, at, target, level, dfltacl)
	Entry ptr;
	DN dn;
	OID at;
	DN target;
...
dsa_control_info()
{
	...
	(void) sprintf(buffer,
		       ...
		       local_master_size, master_edbs, local_slave_size, slave_edbs,
		       local_cache_size);

	...
	for (tmp = *l; tmp != NULLFTL; tmp = tmp->ft_next) {
		...
		if (AttrT_cmp(tmp->ft_type, type) == 0) {
			...
			if (inequstr != NULL) {
				...
				tmp->ft_inequstrs[tmp->ft_numstrs - 1] = inequstr;

				/* see if len needs updating */
				for (i = 0; i < tmp->ft_numstrs - 1; i++) {
					if ((plen =
					     common_prefix_len(inequstr, tmp->ft_inequstrs[i]))
					    < tmp->ft_len || tmp->ft_len == -1) {
						tmp->ft_len = plen;
					}
				}
			}
...
static
check_base_sacl(binddn, selfdn, e, local, authtype)
	DN binddn;
	DN selfdn;
	Entry e;
	...
	char authtype;
{
	...
	if (e->e_authp != NULLAUTHP) {
		binddn = (authtype % 3) >= e->e_authp->ap_listandsearch ? binddn : NULLDN;
	}

	/* for each type in the filter */
	for (ft = (Ftypelist) local->st_ftypes; ft != NULLFTL; ft = ft->ft_next) {
...
make_rc(binddn, selfdn, e, scope, local)
	DN binddn;
	DN selfdn;
	Entry e;
	int scope;
...
{
	return (a->rc_base < b->rc_base ? -1 : a->rc_base > b->rc_base ? 1 : 0);
}
	...
	char authtype;
{
	...
	if (ancestor->e_authp != NULLAUTHP) {
		binddn = authtype >= ancestor->e_authp->ap_listandsearch ? binddn : NULLDN;
	}

	if ((rc = (struct result_count *) avl_find((Avlnode *) local->st_sacls,
						   (caddr_t) ancestor,
						   entry_rc_cmp)) == (struct result_count *) 0) {
	...
	for (ft = (Ftypelist) local->st_ftypes; ft != NULLFTL; ft = ft->ft_next) {
		for (i = 0; i < rc->rc_numtypes; i++) {
			if (AttrT_cmp(rc->rc_types[i].td_type, ft->ft_type) == 0)
				break;
		}
		if (i == rc->rc_numtypes) {
			continue;
	...
	int *sizelimit;			/* size limit on acl we found */
{
	...
	*sizelimit = -1;
	...
	for (av = avs; av != NULLAV; av = av->avseq_next) {
		...
		default:
			/* trouble */
			LLOG(log_dsap, LLOG_EXCEPTIONS, ("check_lacl: unknown type"));
			return (NOTOK);
		}
		if (tmp == NULLLISTACL)
			continue;

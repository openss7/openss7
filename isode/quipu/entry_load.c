...
#endif				/* TURBO_DISK */
	{
		if ((newname = rindex(ptr, '#')) == NULLCP) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("Seperator missing in map file \"%s\", line %d", file, parse_line));
			(void) fclose(mapfp);
			return FALSE;
		}
		tmp = newname;
...
static
write_mapped_rdn(aps, name, file)
	PS aps;
	char *name;
	char *file;
{
	...
	*aps->ps_ptr = 0;
	...
	if (mkdir(aps->ps_base, 0700) != 0) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("map rdn mkdir failure \"%s\" (%d)", aps->ps_base, errno));
		return FALSE;
	}
	...
	char make;
{
	...
	*aps->ps_ptr = 0;
	...
	if (fileexists(mapbuf) && read_mapped_rdn(aps, start, mapbuf)) {
		*aps->ps_ptr = 0;
		...
		if (make) {
			if (mkdir(aps->ps_base, 0700) != 0) {
				LLOG(log_dsap, LLOG_EXCEPTIONS,
				     ("dn2file mkdir (mapped) failure \"%s\" (%d)", aps->ps_base,
				      errno));
				return NOTOK;
			}
			LLOG(log_dsap, LLOG_NOTICE,
			     ("WARNING: Needed to make mapped directory \"%s\"", aps->ps_base));
			return OK;
		}
	...
	if (make) {
		...
		if (mkdir(aps->ps_base, 0700) != 0) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("dn2file mkdir failure \"%s\" (%d)", aps->ps_base, errno));
			return NOTOK;
		}
		return OK;
	}
	...
	char make;
{
	if (treedir != NULLCP) {
		...
		if (make) {
			*aps->ps_ptr = 0;
			if ((!dir_exists(aps->ps_base)) && (mkdir(aps->ps_base, 0700) != 0)) {
				LLOG(log_dsap, LLOG_EXCEPTIONS,
				     ("dn2file mkdir failure \"%s\" (%d)", aps->ps_base, errno));
				return NOTOK;
			}
	...
	int offset;
{
	...
	if ((!e->e_external) && (e->e_master == NULLAV) && (e->e_slave == NULLAV)) {
		e->e_leaf = TRUE;
	...
	if (file_check(offset, e) == OK) {
		if ((e->e_children = getentry_block(e, filename)) == NULLAVL) {
			...
			if (e->e_allchildrenpresent != FALSE && e->e_leaf == FALSE) {
				got_all = FALSE;
				return (OK);
			} else
				return (NOTOK);
...
static
entry_load_kids(entryptr, offset)

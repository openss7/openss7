...
net_init()
{
	...
	/* allow 3 fd's for stdio, 2 for logging, 1 emergency, and one for each listen. */
	...
	while (TNetListenAux(&dsaladdr->pa_addr.sa_addr, TMagic, td)
		...
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("TNetListen failed on address %s", paddr2str(dsaladdr, NULLNA)));
		if (td->td_cc > 0) {
			if (ontty)
				(void) fprintf(stderr,
					       "TNetListen: [%s] %*.*s\nAddress: %s\n",
					       TErrString(td->td_reason), td->td_cc,
					       td->td_cc, td->td_data, paddr2str(dsaladdr, NULLNA));
			...
			LLOG(log_dsap, LLOG_FATAL, ("TNetListen: [%s]", TErrString(td->td_reason)));
		}
	...
	for (ip = ts_communities; *ip; ip++) {
		...
		for (na = mydsaaddr->pa_addr.sa_addr.ta_addrs,
		     x = mydsaaddr->pa_addr.sa_addr.ta_naddr - 1; x >= 0; na++, x--)
		...
		if (x < 0)
			LLOG(log_dsap, LLOG_NOTICE,
			     ("Using community %d for external access", *ip));
	}

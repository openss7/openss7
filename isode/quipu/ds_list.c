	...
	char authtype;
{
	...
	switch (find_child_entry
		(target, &(arg->lsa_common), binddn, NULLDNSEQ, FALSE, &(entryptr), error, di_p)) {
	case DS_OK:
	...
	if (!manager(binddn))
		authp = entryptr->e_authp ? entryptr->e_authp->ap_listandsearch : AP_SIMPLE;
	else
		authp = AP_SIMPLE;
	...
	if (check_acl((authtype % 3) >= authp ? binddn : NULLDN,
		...
		if (dsp
		    && (check_acl(binddn, ACL_READ, entryptr->e_acl->ac_child, realtarget) == OK)) {
			error->dse_type = DSE_SECURITYERROR;
	...
	DN dn;
{
	...
	if (g_dsp) {
		...
		if (e->e_lacl != NULLAV) {
			...
			if (check_lacl(NULLDN, g_dn, e->e_lacl, SACL_BASEOBJECT, &dummy) != OK) {

				if (check_lacl(dn, g_dn, e->e_lacl, SACL_BASEOBJECT, &dummy) == OK)
			...
			if (check_acl(NULLDN, ACL_READ, e->e_acl->ac_entry, g_dn) != OK) {
...
static int
build_result(arg, ptr, result, error, binddn, dsp, laclsizelimit)
	register Entry ptr;
	...
	int laclsizelimit;
{
	...
	if (!dsp && manager(binddn))
		...
		if ((size = MIN(laclsizelimit, MIN(admin_size,
						   arg->lsa_common.ca_servicecontrol.
						   svc_sizelimit)))
		    == SVC_NOSIZELIMIT) {
	...
	dn = get_copy_dn(akid);
	for (dnend = dn; dnend->dn_parent != NULLDN; dnend = dnend->dn_parent) ;	/* NO-OP */
	dnrdn = dnend->dn_rdn;
	...
	rc = avl_apply(ptr->e_children, build_list, (caddr_t) binddn, NOTOK, AVL_INORDER);

...
struct connection *
make_conn_block(name, addr, conn_ctx)
	DN name;
	...
	struct oper_act *on;
{
		...
		for (cn = connlist; cn != NULLCONN; cn = cn->cn_next) {
			...
			if (cn->cn_ctx == DS_CTX_X500_DAP) {
				LLOG(log_dsap, LLOG_TRACE,
				     ("link_op_to_conn - open conn has DAP context"));
				continue;
			}
			...
			if ((cn->cn_ctx == DS_CTX_X500_DSP)
				...
				if (!cn->cn_initiator)
					LLOG(log_dsap, LLOG_TRACE,
					     ("link_op_to_conn - open conn has DSP context - we must initiate it"));
				else
					LLOG(log_dsap, LLOG_TRACE,
					     ("link_op_to_conn - open conn has DSP context - QUIPU/Internet context needed"));
				continue;
			}
	...
	for (di = on->on_dsas; di != NULL_DI_BLOCK; di = (*next_di)) {
		...
		if (di->di_state == DI_ACCESSPOINT) {
			/* context problem: if we have not got the entry, we don't know which
			   context it will accept. If the operation is a getedb ASSUME Internet
			   context is OK Could be QUIPU - so we lose out on caching */
			...
			DLOG(log_dsap, LLOG_TRACE,
			     ("link_op_to_conn - make conn block from access point"));

			/* There *should* only be one access point - but QUIPU may give a choice
			   try find one that will work... This is *wrong* if it is a non specific
			   subordinate reference. */
			if (di->di_reftype == RT_NONSPECIFICSUBORDINATE) {
				LLOG(log_dsap, LLOG_EXCEPTIONS,
				     ("Should try each access point - not just one !!!"));
				/* throw back to user */
				(*next_di) = di->di_next;
				di_extract(di);
				continue;
			...
			for (loop_ap = di->di_accesspoints; loop_ap != NULLACCESSPOINT;
			     loop_ap = loop_ap->ap_next)
				if ((cn =
				     make_conn_block(loop_ap->ap_name, loop_ap->ap_address,
						     conn_ctx)) != NULLCONN) {
					if (on->on_relay != 2)
			...
			if (loop_ap == NULLACCESSPOINT) {
				DLOG(log_dsap, LLOG_DEBUG,
				     ("link_op_to_conn - make_conn_block failed 1"));
				(*next_di) = di->di_next;
				di_extract(di);
				continue;
			}
		...
		if (di->di_state == DI_COMPLETE) {
			...
			if ((res = quipu_ctx_supported(di->di_entry)) < 2) {
				if ((on->on_type == ON_TYPE_GET_EDB) || (res == -1)) {
					...
					LLOG(log_dsap, LLOG_TRACE,
					     ("link_op_to_conn - avoiding non-quipu context for GetEDB"));
					(*next_di) = di->di_next;
					di_extract(di);
					continue;
				} else {
					DLOG(log_dsap, LLOG_DEBUG,
					     ("link_op_to_conn - linking to a connection without a quipu context"));
					conn_ctx = DS_CTX_X500_DSP;
				}
			} else {
				if (res == 2) {
					DLOG(log_dsap, LLOG_DEBUG,
					     ("link_op_to_conn - linking to a connection with internet context"));
					conn_ctx = DS_CTX_INTERNET_DSP;

				} else {
					DLOG(log_dsap, LLOG_DEBUG,
					     ("link_op_to_conn - linking to a connection with a quipu context"));
					conn_ctx = DS_CTX_QUIPU_DSP;
				}
			}

			DLOG(log_dsap, LLOG_TRACE,
			     ("link_op_to_conn - make conn block from entry"));

			if ((cn =
			     make_conn_block(di->di_dn, di->di_entry->e_dsainfo->dsa_addr,
					     conn_ctx)) == NULLCONN) {
				DLOG(log_dsap, LLOG_DEBUG,
				     ("link_op_to_conn - make_conn_block failed 2"));
				(*next_di) = di->di_next;
				di_extract(di);
				continue;
		...
		case ON_TYPE_SHADOW:
			do_conn =
			    (conns_used < (MAX_CONNS - CONNS_RESERVED_DI - CONNS_RESERVED_X500));
			break;
		default:
		...
		if (do_conn) {
			DLOG(log_dsap, LLOG_TRACE,
			     ("link_op_to_conn - about to request connection"));
			if (conn_request(cn) != OK) {
				DLOG(log_dsap, LLOG_DEBUG,
				     ("link_op_to_conn - conn_request failed"));
				(*next_di) = di->di_next;
				di_extract(di);
				continue;
			}
	...
	struct oper_act *on;
{
	if (link_op_to_conn(on) == OK) {
		if (on->on_conn == NULLCONN) {
			DLOG(log_dsap, LLOG_DEBUG,
			     ("oper_chain - link_op_to_conn: OK but no conn"));
		} else {
			DLOG(log_dsap, LLOG_DEBUG, ("oper_chain - link_op_to_conn: OK got conn"));

			if (on->on_conn->cn_state == CN_OPEN) {
				DLOG(log_dsap, LLOG_DEBUG,
				     ("oper_chain - link_op_to_conn: OK got open conn"));
				if (oper_send_invoke(on) != OK) {
					LLOG(log_dsap, LLOG_EXCEPTIONS,
					     ("oper_chain - oper_send failed"));
					/* Have another go? */
					return (oper_chain(on));
				} else {
					DLOG(log_dsap, LLOG_DEBUG,
					     ("oper_chain - oper_send succeeded"));
				}
			}
		}
	...
	struct oper_act *on;
{
	...
	if (cref->cr_reftype != RT_NONSPECIFICSUBORDINATE)
		on->on_dsas =
		    ap2di(cref->cr_accesspoints, cref->cr_name, FALSE, DI_OPERATION, on,
			  cref->cr_reftype);
	else {
		on->on_dsas = di_alloc();
		...
		DLOG(log_dsap, LLOG_DEBUG,
		     ("oper_rechain allocates di_block with dn[%x]", on->on_dsas->di_dn));
	...
	DN dsadn;
{
	...
	if (tk->tk_conn->cn_ctx != DS_CTX_X500_DAP) {
		if ((dspchaining != 0)	/* not on */
		    &&(!relayfordsa(tk->tk_conn->cn_dn)))
		{		/* explicitly allowed */
			DLOG(log_dsap, LLOG_DEBUG,
			     ("Not chaining because dspchaining is %s ",
			      dspchaining == 1 ? "off" : "whenNeeded"));
			return (FALSE);
		}
	...
	struct oper_act *on;
{
	...
	if ((tk = on->on_task) == NULLTASK) {
		...
		dsp_cache(&(tk->tk_dx.dx_arg.dca_dsarg),
			  &(tk->tk_result->dcr_dsres), tk->tk_conn->cn_ctx, tk->tk_conn->cn_dn);
	...
	struct oper_act *on;
{
	...
	if ((tk = on->on_task) == NULLTASK) {
		...
		if (!((on->on_resp.di_error.de_err.dse_type == DSE_SECURITYERROR)
		      && (on->on_resp.di_error.de_err.ERR_SECURITY.DSE_sc_problem ==
	...
	struct di_block *di;
{
	...
	if (di2cref(di, &(tk->tk_resp.di_error.de_err), tk->tk_conn->cn_ctx) != OK) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("task_dsa_info_wakeup - reference not acceptable"));
		...
		if (tk->tk_dsas == NULL_DI_BLOCK) {
			...
			tk->tk_resp.di_error.de_err.ERR_SERVICE.DSE_sv_problem =
	...
	char ctx;
{
	...
	case DI_ACCESSPOINT:
		...
		cref = err->ERR_REFERRAL.DSE_ref_candidates =
		    (struct continuation_ref *) calloc(1, sizeof(struct continuation_ref));
	...
	case DI_DEFERRED:
		LLOG(log_dsap, LLOG_TRACE,
		     ("di2cref - Trying to turn deferred di_block into continuation reference!"));
		return (NOTOK);
	default:
	...
	struct task_act *tk;
{
	...
	for (refer = tk->refer_st; refer != NULL_ST; refer = nref) {
		...
		if ((di_tmp = select_refer_dsa(di, tk)) == NULL_DI_BLOCK) {
			/* The remote END is probably unable to follow the referral - chain if
			   allowed */
			for (di_tmp = di; di_tmp != NULL_DI_BLOCK; di_tmp = di_tmp->di_next) {
				if (di_tmp->di_state == DI_DEFERRED)
					continue;
		...
		if (refer->st_alias == NULLDN) {
			if (err.ERR_REFERRAL.DSE_ref_candidates)
				oncha->cha_target =
				    dn_cpy(err.ERR_REFERRAL.DSE_ref_candidates->cr_name);
			else
		...
		oncha->cha_aliasderef =
		    ((oncha->cha_aliasedrdns = di->di_aliasedRDNs) != CR_NOALIASEDRDNS);
		...
		if ((ca->ca_servicecontrol.svc_options & SVC_OPT_CHAININGPROHIBIT)
		    || (oper_chain(on) != OK)) {
			add_cref2poq(&tk->tk_resp.di_result.dr_res.dcr_dsres.res_sr,
	...
	if ((tk->referred_st == NULL_ST) && (tk->tk_state == TK_PASSIVE)
	    && (tk->tk_operlist == NULLOPER)) {
		task_conn_extract(tk);
		task_result(tk);
		task_extract(tk);
	}
	...
	struct oper_act *on;
{
	...
	if ((tk = on->on_task) == NULLTASK) {
		...
		if (st == NULL_ST) {
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("subtask_result_wakeup - subtask lost from referred list"));
		} else {
			/* 
			...
			struct ds_search_result *tk_sr =
			    &(tk->tk_resp.di_result.dr_res.dcr_dsres.res_sr);
			struct ds_search_result *op_sr =
			    &(on->on_resp.di_result.dr_res.dcr_dsres.res_sr);
			...
			if (tk_sr->srr_next == NULLSRR) {
				...
				tk_sr->srr_next =
				    (struct ds_search_result *) calloc(1,
								       sizeof(struct
									      ds_search_result));
				tk_sr->srr_next->srr_correlated = TRUE;
				tk_sr->srr_next->srr_un.srr_unit =
				    (struct ds_search_unit *) calloc(1,
								     sizeof(struct ds_search_unit));
...
*/
		...
		if ((on->on_resp.di_error.de_err.dse_type == DSE_DSAREFERRAL)
			...
			add_cref2poq(&tk->tk_resp.di_result.dr_res.dcr_dsres.res_sr,
				     on->on_resp.di_error.de_err.ERR_REFERRAL.DSE_ref_candidates);
			on->on_resp.di_error.de_err.ERR_REFERRAL.DSE_ref_candidates =
			    NULLCONTINUATIONREF;
		}
	...
	struct di_block *di;
{
	...
	if (di2cref(di, &(tk->tk_resp.di_error.de_err), tk->tk_conn->cn_ctx) != OK) {
		LLOG(log_dsap, LLOG_EXCEPTIONS,
		     ("subtask_dsa_info_wakeup - reference not acceptable"));
		/* Remove di_block which generated unwanted referral wait */
		if (tk->tk_dsas == NULL_DI_BLOCK) {
			/* No more dsas from which to request info to form referral */
			tk->tk_resp.di_error.de_err.dse_type = DSE_SERVICEERROR;
	...
	struct oper_act *on;
{
	...
	for (avs = as->attr_value; avs != NULLAV; avs = avs->avseq_next) {
		...
		switch (get_dsa_info((DN) avs->avseq_av.av_struct, dn_stack, &err, di_trail)) {
		case DS_OK:
		...
		case DS_X500_ERROR:
			...
			DLOG(log_dsap, LLOG_TRACE,
			     ("relay - get_dsa_info (slave) returned X500 ERROR"));
			ds_error_free(&err);
			break;

		default:
			LLOG(log_dsap, LLOG_EXCEPTIONS,
			     ("dsa_info_new - get_dsa_info (master) unexpected return"));
			break;
		}
			...
			while (tmp_op_list != (struct op_list *) 0) {
				if (tmp_op_list->operation_list->invoke_id == task->tk_dx.dx_id) {
					...
					while (tmp_operlist->on_next_conn != (struct oper_act *) 0) {
						...
						new_chain_list =
						    (struct chain_list *)
						    malloc(sizeof(struct chain_list));
						new_sub_ch_list =
						    (struct sub_ch_list *)
						    malloc(sizeof(struct sub_ch_list));

						/* We have now found the right operation in our
						   list */
						/* Now we add the chain request onto the
						   chained_ops list */

						if (tmp_op_list->operation_list->chained_ops ==
						    (struct chain_list *) 0) {
							tmp_op_list->operation_list->chained_ops =
							    new_chain_list;
						} else {
							tmp_chain_list =
							    tmp_op_list->operation_list->
							    chained_ops;
							while (tmp_chain_list->next !=
							       (struct chain_list *) 0) {
								tmp_chain_list =
								    tmp_chain_list->next;
							}
						...
						new_sub_ch_list->assoc_id =
						    tmp_operlist->on_conn->cn_ad;

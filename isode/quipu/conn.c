	...
	int level;
{
	...
	LLOG(log_dsap, level, ("Connection %d, %s %s", conn->cn_ad, cntxt, state));

	if (conn->cn_initiator)
		LLOG(log_dsap, level, ("To: %s", paddr2str(&conn->cn_addr, NULLNA)));
	else
		LLOG(log_dsap, level, ("From: %s",
				       paddr2str(&conn->cn_start.cs_ds.ds_start.acs_start.
						 ps_calling, NULLNA)));

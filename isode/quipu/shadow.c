	...
	Entry eptr;
{
	...
	if (eptr->e_data == E_DATA_MASTER) {
		if ((eptr->e_dsainfo) && (quipu_ctx_supported(eptr) > 2) && (quipu_version_7(eptr))) {
...
shadow_update()
{
		...
		if ((eptr = local_find_entry(dnseq->dns_dn, FALSE)) == NULLENTRY) {
			...
			if ((eptr = local_find_entry(dnseq->dns_dn, TRUE)) == NULLENTRY)
				if ((eptr = make_path(dnseq->dns_dn)) == NULLENTRY) {
					...
					if ((eptr = make_path(eptr->e_alias)) == NULLENTRY) {
						pslog(log_dsap, LLOG_EXCEPTIONS,
						      "Shadowing alias which does not exist !!!",
						      (IFP) dn_print, (caddr_t) eptr->e_alias);
						continue;
					}
		}

		else if ((eptr->e_data == E_TYPE_SLAVE) || (eptr->e_data == E_DATA_MASTER))
			continue;
	...
	for (dnseq = shades; dnseq != NULLDNSEQ; dnseq = dnseq->dns_next) {
		...
		sarg.rda_common.ca_servicecontrol.svc_options |= SVC_OPT_DONTDEREFERENCEALIAS;
		/* would loose track on result - could do more tho! */

		op->on_req.dca_dsarg.arg_rd = sarg;	/* struct copy */

		op->on_req.dca_dsarg.arg_rd.rda_object = dn_cpy(dnseq->dns_dn);

		op->on_req.dca_charg.cha_originator = dn_cpy(mydsadn);

		op->on_req.dca_charg.cha_reftype = RT_SUBORDINATE;

		op->on_req.dca_charg.cha_progress.op_resolution_phase = OP_PHASE_PROCEEDING;

		op->on_req.dca_charg.cha_progress.op_nextrdntoberesolved = -1;
		for (tdn = dnseq->dns_dn; tdn != NULLDN; tdn = tdn->dn_parent)
			op->on_req.dca_charg.cha_progress.op_nextrdntoberesolved++;

		op->on_req.dca_charg.cha_trace =
		    (struct trace_info *) malloc(sizeof(struct trace_info));
		op->on_req.dca_charg.cha_trace->ti_dsa = dn_cpy(mydsadn);
		op->on_req.dca_charg.cha_trace->ti_target = dn_cpy(dnseq->dns_dn);
		op->on_req.dca_charg.cha_trace->ti_progress = op->on_req.dca_charg.cha_progress;
		...
		if ((eptr = local_find_entry_aux(dnseq->dns_dn, FALSE)) == NULLENTRY) {
			...
			aps = ap_cpy((struct access_point *) eptr->e_reference->avseq_av.av_struct);
	...
	struct oper_act *on;
{
	...
	if (on->on_resp.di_type == DI_ERROR) {
		...
		pslog(log_dsap, LLOG_EXCEPTIONS, "Remote shadow error",
		      (IFP) dn_print, (caddr_t) on->on_req.dca_dsarg.arg_rd.rda_object);
	...
	struct oper_act *on;
{
	...
	new_as = on->on_resp.di_result.dr_res.dcr_dsres.res_rd.rdr_entry.ent_attr;
	...
	if (eptr->e_external) {
		...
		if (((as = entry_find_type(eptr, at_subord)) == NULLATTR)
			...
			LLOG(log_dsap, LLOG_EXCEPTIONS, ("external reference missing"));
			goto out;
		}
		new_as = as_merge(new_as, as_comp_cpy(as));

		if ((as = as_find_type(new_as, at_objectclass)) == NULLATTR) {
			LLOG(log_dsap, LLOG_EXCEPTIONS, ("no objectclass in shadow entry"));
			on->on_resp.di_result.dr_res.dcr_dsres.res_rd.rdr_entry.ent_attr = new_as;
			goto out;
		}
	...
	(void) avl_apply(eptr->e_children, inherit_link, (caddr_t) eptr, NOTOK, AVL_PREORDER);

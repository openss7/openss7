...
pass1_type(encpref, decpref, prfpref, mod, id, yp)
	register char *encpref, *decpref, *prfpref, *mod, *id;
	register YP yp;
{
	register SY sy;
...
pass2()
{
	...
	for (sy = mysymbols; sy; sy = sy->sy_next) {
		...
		(void) printf("\n%s", sy->sy_name);
		(void) printf(" [[P struct %s *]]", modsym(mymodule, sy->sy_name, "type"));
		switch (yp->yp_code) {
		case YP_SEQTYPE:
		case YP_SEQLIST:
		case YP_SETTYPE:
		...
		(void) printf(" ::=\n");
		do_type1(yp, 1, (yp->yp_flags & YP_TAG) ? 1 : 2, eval, "parm", NULLCP, YP_ENCODER);
		(void) printf("\n");
		if (ferror(stdout) || ferror(fdef) || (fflag && ferror(fact)))
			myyerror("write error - %s", sys_errname(errno));
	}
	...
	for (sy = mysymbols; sy; sy = sy->sy_next) {
		...
		case YP_SETTYPE:
			if (h2flag)
				xalloc(y, 0, 1, "parm", modsym(mymodule, sy->sy_name, "type"), 1);
			break;
		...
		default:
			xalloc(y, 1, 1, "parm", modsym(mymodule, sy->sy_name, "type"), 1);
			break;
		}
	...
	char *id;
{
	...
	case YP_CHOICE:
	case YP_SETLIST:
		choice_pullup(yp, yp->yp_code == YP_CHOICE ? CH_FULLY : CH_PARTIAL);
		/* and fall */
	case YP_SEQLIST:
	...
	char *id, *pullup;
{
		...
		if (yp->yp_code == YP_BITLIST) {
			...
			for (yv = yp->yp_value; yv; yv = yv->yv_next) {
				...
				(void) fprintf(fdef, "#define\t%s_%s\t%d\n",
					       modsym(mymodule, eval, "bit"), buf1, val2int(yv));
			}
		}
		if (fflag)
	...
	case YP_IDEFINED:
		...
		if (aflag)
			printag(yp, 4, pullup);
		(void) fprintf(fdef, "#define\t%s\t", modsym(mymodule, id, "type"));
		(void) fprintf(fdef, "%s\n", modsym(yp->yp_module, yp->yp_identifier, "type"));
		if (fflag) {
			(void) fprintf(fdef, "#define\t%s\t", modsym(mymodule, id, "free"));
			(void) fprintf(fdef, "%s\n",
	...
	char *id, *pullup;
{
	...
	default:
		...
		if (fflag) {
			if (yp->yp_code != YP_SEQTYPE && yp->yp_code != YP_SETTYPE)
				(void) fprintf(fact, "\n    free ((char *) arg);");
			(void) fprintf(fact, "\n}\n");
		}
...
static
do_type1(yp, top, level, id, var, action2, direction)
	register YP yp;
	int top, level;
	char *id, *var, *action2;
	int direction;
{
	...
	switch (yp->yp_code) {
	case YP_BOOL:
		if ((yp->yp_flags & (YP_OPTIONAL | YP_DEFAULT)) && direction == YP_DECODER) {
	...
	case YP_ENUMLIST:
		if ((yp->yp_flags & (YP_OPTIONAL | YP_DEFAULT)) && direction == YP_DECODER) {
			...
			if (yp->yp_flags & YP_OPTIONAL)
				...
				(void) printf("%%{ %s%s = %d; %%}\n%*s",
					      var, SVAL(yp->yp_varexp), dfl2int(yp), level * 4, "");
		}
		break;
	...
	case YP_ENUMLIST:
		...
		case YP_DECODER:
			(void) printf(top ? "\n%*s[[i %s -> %s ]]\n%*s{\n"
				      : "\n%*s[[i %s%s ]]\n%*s{\n",
				      level * 4, "", var, SVAL(yp->yp_varexp), level * 4, "");
			break;
	...
	case YP_OCT:
		...
		switch (direction) {
		case YP_ENCODER:
			(void) printf("\n%*s[[q %s%s ]]", level * 4, "", var, SVAL(yp->yp_varexp));
			break;
	...
	case YP_ANY:
		...
		case YP_ENCODER:
		case YP_DECODER:
			(void) printf("\n%*s[[a %s%s ]]", level * 4, "", var, SVAL(yp->yp_varexp));
			break;
		}
		break;
	...
	case YP_SETTYPE:
			...
			if (h2flag) {
				...
				if (top)
					...
					(void)
					    printf
					    ("%*s<<%s = %s%s;\n%*s%s;\n%*s%s = %s -> next>>\n",
					     (level + 1) * 4, "", yp->yp_declexp, var,
					     SVAL(yp->yp_varexp), (level + 3) * 4, "",
					     yp->yp_declexp, (level + 3) * 4, "", yp->yp_declexp,
					     yp->yp_declexp);
			}
			break;
		...
		case YP_DECODER:
			if (h2flag) {
				y = yp->yp_type;
				xalloc(y, 0, level + 2, y->yp_declexp, y->yp_declexp, 1);
			} else
				xalloc(yp, 0, level + 1,
	...
	case YP_SETLIST:
		...
		if (direction == YP_DECODER)
			xalloc(yp, 1, level + 2, yp->yp_declexp, yp->yp_declexp, 0);
		for (y = yp->yp_type; y; y = y->yp_next) {
			if (y->yp_declexp)
				switch (direction) {
				case YP_ENCODER:
			...
			if (direction == YP_DECODER && y->yp_flags & YP_DEFAULT) {
	...
	case YP_CHOICE:
		...
		case YP_ENCODER:
			...
			if (*var)
				...
				(void) printf(" <<%s -> offset>>", yp->yp_declexp);
			(void) printf(i ? "\n%*s{\n" : " {\n", level * 4, "");
			break;

		case YP_DECODER:
			(void) printf("\n");
			xalloc(yp, 0, level + 1, yp->yp_declexp, yp->yp_declexp, 1);
			(void) printf("%*s{\n", level * 4, "");
			break;
		...
		for (y = yp->yp_type; y; y = y->yp_next) {
			...
			do_type1(y, top, level + 1, "choice", var, cp ? buffer : NULLCP, direction);
			(void) printf("%s\n", y->yp_next ? ",\n" : "");
		}
		(void) printf("%*s}", level * 4, "");
		break;
	...
	case YP_OID:
		...
		case YP_ENCODER:
		case YP_DECODER:
			(void) printf("\n%*s[[O %s%s ]]", level * 4, "", var, SVAL(yp->yp_varexp));
			break;
		}
		break;
	...
	case YP_IDEFINED:
		...
		switch (direction) {
		case YP_ENCODER:
			(void) printf("\n%*s[[p %s%s ]]", level * 4, "", var, SVAL(yp->yp_varexp));
			break;
	...
	if (yp->yp_flags & YP_OPTIONAL) {
		...
		if (direction == YP_ENCODER)
			...
			case YP_REAL:
				(void) printf(" << %s%s != %g >>",
					      var, SVAL(yp->yp_varexp), val2real(yp->yp_default));
				break;
	...
	if (direction == YP_ENCODER && yp->yp_varexp && (cp = index(yp->yp_varexp, ' '))
	    && strncmp(cp + 1, "-> ", 3) == 0) {
		*cp = NULL;
	...
	char *id, *val, *var;
{
	...
	case YP_ENUMLIST:
		...
		for (yv = yp->yp_value; yv; yv = yv->yv_next) {
			modsym_aux(yv->yv_named, buf1);
			(void) fprintf(fdef, "#define\t%s_%s\t%d\n",
				       modsym(mymodule, top ? eval : id, "int"), buf1, val2int(yv));
		}
		break;
	...
	case YP_BITLIST:
		...
		for (yv = yp->yp_value; yv; yv = yv->yv_next) {
			modsym_aux(yv->yv_named, buf1);
			(void) fprintf(fdef, "#define\t%s_%s\t%d\n",
				       modsym(mymodule, top ? eval : id, "bit"), buf1, val2int(yv));
		}
		break;
	...
	case YP_SETTYPE:
		...
		if ((cp = rindex(buf2, ' ')) && *++cp) {
			if ((dp = rindex(cp, '.')) && *++dp)
				cp = dp;
			(void) sprintf(dp = buf1, "%*.*s", cp - buf2, cp - buf2, buf2);
			dp += strlen(dp);
		} else {
		...
		if (fflag) {
			...
			if (h2flag) {
				...
				(void) fprintf(fact, "%*sstruct %s *f_%s = %s -> next;\n\n",
					       (level + 1) * 4, "",
					       top ? modsym(mymodule, val, "type") : id, id, id);
			}
		}
		level++;
	...
	case YP_SETLIST:
		...
		if (!top) {
			...
			if (fflag) {
				(void) fprintf(fact,
					       "\n%*sif (%s)\n%*sfree ((char *) %s);\n%*s%s = NULL;\n",
					       (level + 1) * 4, "", yp->yp_varexp, (level + 2) * 4,
					       "", yp->yp_varexp, (level + 1) * 4, "",
					       yp->yp_varexp);
				(void) fprintf(fact, "%*s}\n", level * 4, "");
			}
...
static
xalloc(yp, top, level, arg, type, brackets)
	register YP yp;
	int top, level, brackets;
	char *arg, *type;
{
	int didone;
	register YP y;
	...
	if (arg && type) {
		...
		if (h2flag && (yp->yp_code == YP_SEQTYPE || yp->yp_code == YP_SETTYPE)) {
			(void) printf("%*s{\n%*sPE      xx_pe = prim2%s ($$);\n\n",
				      level * 4, "", (level + 1) * 4, "",
	...
	case YP_SETLIST:
		for (y = yp->yp_type; y; y = y->yp_next)
			...
			case YP_SETTYPE:
				if (h2flag && (yp->yp_code == YP_SETLIST ||
					...
					if (y->yp_declexp)
						(void) printf("%*s%s = &(%s);\n", level * 4, "",
							      y->yp_declexp, y->yp_varexp);
					xalloc(y, top, level, y->yp_declexp, y->yp_declexp, 0);
				}
				/* and continue ... */
			case YP_SEQLIST:
			case YP_SETLIST:
			case YP_CHOICE:
				...
				(void) printf("%*s%s = &(%s);\n",
					      level * 4, "", y->yp_declexp, y->yp_varexp);
				break;
			}
		break;
	}
	...
	double r;
{
	...
	(void) printf("{ %s%s, 10, %d }", sign ? "-" : "", sbuf, decpt - strlen(sbuf));
#else
	...
	*cp++ = NULL;
	(void) printf("{ %s%c%s, 10, %d }", sp, *dp, dp + 2, atoi(cp) - strlen(dp + 2));
#endif
}
	...
	register YP yp;
{
	...
	case YV_IDEFINED:
		for (y = yp->yp_value; y; y = y->yv_next)
			if (y->yv_code == YV_NUMBER && (y->yv_flags & YV_NAMED)
	...
	register int level;
{
	...
	(void) fprintf(stderr, "%*scode=0x%x flags=%s direction=0x%x\n", level * 4, "",
		       yp->yp_code, sprintb(yp->yp_flags, YPBITS), yp->yp_direction);
	(void) fprintf(stderr,
...
static SY
new_symbol(encpref, decpref, prfpref, mod, id, type)
	register char *encpref, *decpref, *prfpref, *mod, *id;
	register YP type;
{
	register SY sy;
...
"IA5String", PE_CLASS_UNIV, PE_DEFN_IA5S,
	    ...
	    "ObjectDescriptor", PE_CLASS_UNIV, PE_PRIM_ODE, NULL};

/*  */

static char *
modsym(module, id, prefix)
	register char *module, *id;
	char *prefix;
{
	...
	int level;
{
	...
	case YP_BOOL:
		(void) printf("%*s%s = %d;\n", level * 4, "",
			      SVAL(yp->yp_varexp), val2int(yp->yp_default) ? 1 : 0);
		break;
	...
	case YP_INTLIST:
	case YP_ENUMLIST:
		(void) printf("%*s%s = %d;\n", level * 4, "", SVAL(yp->yp_varexp), dfl2int(yp));
		break;

	case YP_REAL:
		(void) printf("%*s%s = %g;\n", level * 4, "",
			      SVAL(yp->yp_varexp), val2real(yp->yp_default));

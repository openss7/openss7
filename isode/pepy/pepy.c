...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/pepy/RCS/pepy.c,v 9.0 1992/06/16 12:25:01 isode Rel";
#endif
...
"ENCODE", YP_ENCODER, "DECODE", YP_DECODER, "PRINT", YP_PRINTER, NULL};
	...
	char **argv, **envp;
{
	...
	for (argc--, argv++; argc > 0; argc--, argv++) {
		...
		if (strcmp(cp, "-o") == 0) {
			...
			if (bflag) {
			      not_practical:;
				(void) fprintf(stderr, "-b & -o together is not practical\n");
				exit(1);
			}
			argc--, argv++;
		...
		if (strcmp(cp, "-S") == 0) {
			...
			if (!sectp->s_name) {
				(void) fprintf(stderr, "unknown section name \"%s\"\n", cp);
				exit(1);
			}
			continue;
		}
...
prologue()
{
	...
	if (!bflag) {
		(void) printf("#ifndef lint\n");
		(void) printf("static char *pepyid = \"%s\";\n", pepyversion);
		(void) printf("#endif\n\n");
	}
	if (aflag)
	...
	for (sy = mysymbols; sy; sy = sy->sy_next) {
		...
		if (yp->yp_direction & YP_ENCODER) {
			...
			(void) printf("\n/* ARGSUSED */\n\n%sint\t%s ",
				      ...
				      "static " : "",
				      modsym(sy->sy_module, sy->sy_name, YP_ENCODER));
		...
		if (yp->yp_direction & YP_DECODER) {
			...
			(void) printf("\n/* ARGSUSED */\n\n%sint\t%s ",
				      !doexternals && (yp->yp_flags & YP_EXPORTED) ?
				      "static " : "",
				      modsym(sy->sy_module, sy->sy_name, YP_DECODER));
		...
		if (yp->yp_direction & YP_PRINTER) {
			...
			(void) printf("\n/* ARGSUSED */\n\n%sint\t%s ",
				      !doexternals && (yp->yp_flags & YP_EXPORTED) ?
				      "static " : "",
				      modsym(sy->sy_module, sy->sy_name, YP_PRINTER));
	...
	char *arg, *whatsit;
{
	...
	(void) printf("%*sadvise (NULLCP, \"%s %%s%s: %%s\", PEPY_ERR_BAD,\n",
		      (level + 1) * 4, "", id, whatsit);
	(void) printf("%*spe_error (%s -> pe_errno));\n", (level + 3) * 4, "", arg);
	(void) printf("%*sreturn NOTOK;\n%*s}\n", (level + 1) * 4, "", level * 4, "");
	(void) printf("%*sif (%s -> pe_cardinal != 1) {\n", level * 4, "", narg);
	(void) printf("%*sadvise (NULLCP, \"%s %%s %s: %%d\", PEPY_ERR_TOO_MANY_TAGGED,\n",
		      (level + 1) * 4, "", id, whatsit);
	(void) printf("%*s%s -> pe_cardinal);\n", (level + 3) * 4, "", narg);
	(void) printf("%*sreturn NOTOK;\n%*s}\n", (level + 1) * 4, "", level * 4, "");
	(void) printf("%*s%s = first_member (%s);\n%*s}\n",
	...
	case YP_IDEFINED:
		if (yp->yp_flags & YP_BOUND) {
			...
			if (!(y->yp_flags & YP_TAG))
				myyerror("type \"%s\" isn't tagged for binding", yp->yp_identifier);
			yp->yp_flags |= YP_TAG;
...
static YP
lookup_binding(mod, id, binding)
	register char *mod, *id, *binding;
{
	register YP yp, z;
...
check_type(type, level, class, form, id, arg)
	register char *type, *class, *form, *id, *arg;
	register int level;
{
	int explicit;
	...
	(void) printf("%*sif (%s -> pe_class != %s", level * 4, "", arg, class);
	if (form)
		(void) printf("\n%*s|| %s -> pe_form != %s\n%*s",
			      (level + 2) * 4, "", arg, form, (level + 2) * 4 - 1, "");
	(void) printf(" || %s -> pe_id != %s) {\n", arg, id);
	(void) printf("%*sadvise (NULLCP, \"%s bad class/form/id: %%s/%%d/0x%%x\",\n",
		      (level + 1) * 4, "", type);
	...
	if (explicit) {
		...
		if (form) {
			...
			(void) printf("%*sadvise (NULLCP, \"%s bad form: %%d\", %s -> pe_form);\n",
				      (level + 2) * 4, "", type, arg);
			(void) printf("%*sreturn NOTOK;\n%*s}\n",
				      (level + 2) * 4, "", (level + 1) * 4, "");
		}
	...
	for (; yv; yv = yv->yv_next) {
		...
		for (y = yv->yv_next; y; y = y->yv_next)
			if (i == val2int(y)) {
				...
				(void) fprintf(stderr, "\tvalue=%d", i);
				if (yv->yv_flags & YV_NAMED)
					(void) fprintf(stderr, " name1=%s", yv->yv_named);
				if (y->yv_flags & YV_NAMED)
					(void) fprintf(stderr, " name2=%s", y->yv_named);
				(void) fprintf(stderr, "\n");
			}
	}
}
	...
	for (; y != z; y = y->yp_next) {
		...
		for (yp = y->yp_next; yp != z; yp = yp->yp_next) {
			...
			if (id == PE_ID(yt->yt_class, val2int(yt->yt_value))) {
				...
				(void) fprintf(stderr, "\ttag=%s/%d", pe_classlist[yt->yt_class],
					       i);
				if (y->yp_code == YP_IDEFINED)
					(void) fprintf(stderr, " id1=%s", y->yp_identifier);
				if (yp->yp_code == YP_IDEFINED)
					(void) fprintf(stderr, " id2=%s", yp->yp_identifier);
				(void) fprintf(stderr, "\n");
			}
		}
	}
	...
	OID oid;
{
	...
	char buffer[BUFSIZ],
	    file[BUFSIZ], id[BUFSIZ], encpref[BUFSIZ], decpref[BUFSIZ], printpref[BUFSIZ];
	char *p, *ep, *dp, *ppp;
	register FILE *fp;
	register YP yp;
	register YT yt;
	...
	while (fgets(buffer, sizeof buffer, fp)) {
		if (sscanf(buffer, "%d/%d/%d: %s", &class, &value, &direction, id) != 4) {
			myyerror("bad external definition in %s: %s", file, buffer);
			continue;
		}
		ppp = dp = ep = NULLCP;
		if (p = index(buffer, '|')) {
		...
		pass1_type(ep, dp, ppp, new_string(module), new_string(id), yp);
	}
	...
	for (sy = mysymbols; sy; sy = sy->sy_next) {
		...
		if (is_any_type(yp)) {
			...
			(void) fprintf(fp, "%d/%d/%d: %s", yt->yt_class,
				       val2int(yt->yt_value), yp->yp_direction, sy->sy_name);
			(void) fprintf(fp, " |%s %s %s\n", yyencpref, yydecpref, yyprfpref);
		}
	}
...
static FILE *
open_ph_file(fn, fnoid, mode)
	char *fn, *fnoid, *mode;
{
	register char *dst, *path;
	char fnb[BUFSIZ];
	register FILE *fp;
...
init_new_file()
{
	...
	if (freopen(buffer, "w", stdout) == NULL) {
		(void) fprintf(stderr, "unable to write "), perror(buffer);
		exit(1);
	}

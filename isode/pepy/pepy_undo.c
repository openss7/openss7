	...
	int Vflag;
{
	...
	case YP_BOOL:
	case YP_INT:
		if (!Vflag && (dflag || !((level == 1) || yp->yp_action2 || yp->yp_intexp)))
			break;	/* else fall */
	case YP_INTLIST:
	case YP_ENUMLIST:
	...
	case YP_BIT:
		if (!Vflag && (dflag || !((level == 1) || yp->yp_action2 || yp->yp_strexp)))
			break;	/* else fall */
	case YP_BITLIST:
	...
	case YP_OCT:
		if (!dflag && ((level == 1) || yp->yp_action2 || yp->yp_strexp)) {
			narg = gensym();
	...
	case YP_OID:
		if (!Vflag && (dflag || (!yp->yp_action2 && !yp->yp_strexp && level != 1)))
			break;	/* else fall */
	...
	if (level == 1 && (yp->yp_flags & YP_TAG)) {
		...
		for (t = tuples; t->t_type != YP_UNDF; t++)
			if (t->t_type == yp->yp_code) {
				check_type(id, level, t->t_class, t->t_form, t->t_id, arg);
				break;
			}
	}

	if (level == 1 && yp->yp_code != YP_CHOICE && (yp->yp_flags & YP_TAG) == YP_TAG) {
		if ((yp->yp_flags & YP_IMPLICIT) == 0 || is_nonimplicit_type(yp))
		...
		if (yp->yp_flags & YP_ID)
			...
			if (hflag && yp->yp_code == YP_IDEFINED)
				...
				(void) printf("%*svtag (%d, %d);\n", level * 4, "",
					      yp->yp_tag->yt_class, val2int(yp->yp_tag->yt_value));
		}
	}
	...
	case YP_ENUMLIST:
		(void) printf("%*sif ((%s = prim2%snum (%s)) == NOTOK\n",
			      level * 4, "", narg, yp->yp_code == YP_ENUMLIST ? "e" : "", arg);
		(void) printf("%*s&& %s -> pe_errno != PE_ERR_NONE) {\n", (level + 2) * 4, "", arg);
		(void) printf("%*sadvise (NULLCP, \"%s %%s%%s\", PEPY_ERR_BAD_INTEGER,\n",
			      (level + 1) * 4, "", id);
		(void) printf("%*spe_error (%s -> pe_errno));\n", (level + 3) * 4, "", arg);
	...
	case YP_BITLIST:
		...
		if (!rflag) {
			...
			(void)
			    printf
			    ("%*sadvise (NULLCP, \"%s %%s(%d): %%d\", PEPY_ERR_TOO_MANY_BITS,\n",
			     (level + 1) * 4, "", id, i);
	...
	case YP_SEQTYPE:
		...
		if (yp->yp_type) {
			(void)
			    printf
			    ("%*sfor (%s = first_member (%s); %s; %s = next_member (%s, %s)) {\n",
			     level * 4, "", narg, arg, narg, narg, arg, narg);
			if (!dflag && yp->yp_action3) {
				do_action(yp->yp_action3, ++level, arg, yp->yp_act3_lineno);
				(void) printf("%*s{\n", level * 4, "");
			}
			undo_type(yp->yp_type, level + 1, "element", narg, Vflag);
			if (!dflag && yp->yp_action3 && yp->yp_action2)
				do_action(yp->yp_action2, level + 1, arg, yp->yp_act2_lineno);
			if (!dflag && yp->yp_action3)
				(void) printf("%*s}\n", level-- * 4, "");
			(void) printf("%*s}\n", level * 4, "");
		}
	...
	case YP_SEQLIST:
		...
		if (!rflag) {
			...
			(void)
			    printf
			    ("%*sadvise (NULLCP, \"%s %%s(%d): %%d\", PEPY_ERR_TOO_MANY_ELEMENTS,\n",
			     (level + 1) * 4, "", id, i);
	...
	case YP_CHOICE:
		...
		if (yp->yp_type) {
			...
			if (!didefault && !rflag) {
				...
				(void)
				    printf
				    ("%*sadvise (NULLCP, \"%s %%s%%s/%%d/0x%%x\", PEPY_ERR_UNKNOWN_CHOICE,\n",
				     (level + 2) * 4, "", id);
				(void)
				    printf
				    ("%*spe_classlist[%s -> pe_class], %s -> pe_form, %s -> pe_id);\n",
	...
	if (!dflag && yp->yp_action2 && !yp->yp_action3)
		do_action(yp->yp_action2, level, narg ? narg : arg, yp->yp_act2_lineno);
	...
	case YP_OCT:
		if (!dflag && yp->yp_prfexp != 'q' && ((level == 1) || yp->yp_action2)) {
			if (level == 1) {
				(void) printf("%*sif (len)\n", level * 4, "");
...
static
undo_type_element(yp, level, first, last, id, arg, narg, Vflag)
	register YP yp;
	register int level;
	int first, last;
	...
	int Vflag;
{
	...
	undo_type(yp, level, yp->yp_flags & YP_ID ? yp->yp_id : "element", narg2, Vflag);
	...
	if ((yp->yp_flags & YP_DEFAULT) || !(yp->yp_flags & YP_OPTIONAL)) {
		...
		if (yp->yp_flags & YP_DEFAULT)
			...
			(void) printf("%*sadvise (NULLCP, \"%s %%s", (level + 1) * 4, "", id);
			if (yp->yp_flags & YP_ID)
				(void) printf("%s ", yp->yp_id);
...
static
undo_type_member(yp, level, arg, narg, Vflag)
	register YP yp;
	register int level;
	char *arg, *narg;
	int Vflag;
{
	...
	if (yp->yp_flags & YP_TAG)
		(void) printf("%*sif (%s = set_find (%s, PE_CLASS_%s, %d)) {\n",
			      level * 4, "", narg, arg,
			      pe_classlist[yp->yp_tag->yt_class], val2int(yp->yp_tag->yt_value));
	else {
		ype zy;
		register YP y = &zy;
...
static int
undo_type_choice(yp, level, narg, Vflag)
	register YP yp;
	register int level;
	register char *narg;
	int Vflag;
		...
		for (y = y->yp_type; y; y = y->yp_next) {
			if (is_any_type(y)) {
				...
				(void) printf("%*scase PE_ID (PE_CLASS_%s, %d):%s", level * 4, "",
					      pe_classlist[y->yp_tag->yt_class],
					      val2int(y->yp_tag->yt_value), y->yp_next ? "\n" : "");
			}
		}
	} else {
		if (!(yp->yp_flags & YP_TAG))
...
static
undo_components_seq(yp, level, first, last, id, arg, narg, Vflag)
	YP yp;
	...
	int Vflag;
{
	...
	if (!(newyp = lookup_type(yp->yp_module, yp->yp_identifier))) {
		pyyerror(yp, "Can't find referenced COMPONENTS OF %s", yp->yp_identifier);
		return i;
	}
	...
	for (y = newyp->yp_type; y; y = y->yp_next) {
		if (y->yp_flags & YP_COMPONENTS)
			...
			undo_type_element(y, level, first && y == newyp->yp_type,
					  last && y->yp_next == NULLYP, id, arg, narg, Vflag);
			i++;
		}
	}
	return i;
...
static
undo_components_set(yp, level, arg, narg, Vflag)
	register YP yp;
	register int level;
	char *arg, *narg;
	int Vflag;

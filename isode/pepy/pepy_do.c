	...
	register char *id, *arg;
{
	...
	if (level == 1) {
		...
		(void) printf("%sPE     *pe;\nint\texplicit;\n",
			      yp->yp_code != YP_ANY
			      && yp->yp_code != YP_NULL
			      && (yp->yp_code != YP_CHOICE || (yp->yp_flags & YP_CONTROLLED))
			      ? "register " : "");
		(void) printf("int\tlen;\nchar   *buffer;\n%s parm;\n{\n",
	...
	if ((yp->yp_flags & (YP_TAG | YP_IMPLICIT)) == (YP_TAG | YP_IMPLICIT)) {
		(void) sprintf(tbuf2, "PE_CLASS_%s", pe_classlist[yp->yp_tag->yt_class]);
		class = tbuf2;
	...
	case YP_BITLIST:
		...
		if (yp->yp_strexp)
			(void) printf("%*s%s = %s;\n%*s%s = %s;\n", level * 4, "",
				      narg2, yp->yp_strexp, level * 4, "", narg3, yp->yp_intexp);
		else if (level == 1)
	...
	if (!dflag && yp->yp_action05)
		do_action(yp->yp_action05, level, narg ? narg : arg, yp->yp_act05_lineno);
	if (!dflag && yp->yp_action1)
		do_action(yp->yp_action1, level, narg ? narg : arg, yp->yp_act1_lineno);

	switch (yp->yp_code) {
	case YP_BOOL:
		(void) printf("%*sif ((%s = flag2prim (%s, %s, ", level * 4, "", arg, narg, class);
	...
	case YP_ENUMLIST:
		(void) printf("%*sif ((%s = %snum2prim (%s, %s, ",
			      level * 4, "", arg,
			      yp->yp_code == YP_ENUMLIST ? "e" : "", narg, class);
		(void) printf("%s)) == NULLPE) {\n", value);
		(void) printf("%*sadvise (NULLCP, \"%s: %%s\", PEPY_ERR_NOMEM);\n",
			      (level + 1) * 4, "", id);
		(void) printf("%*sreturn NOTOK;\n%*s}\n", (level + 1) * 4, "", level * 4, "");
		...
		for (yv = yp->yp_value; yv; yv = yv->yv_next) {
			...
			if (!dflag && yv->yv_action)
				do_action(yv->yv_action, level + 2, narg, yv->yv_act_lineno);
			(void) printf("%*sbreak;\n", (level + 2) * 4, "");
		}
	...
	case YP_REAL:
		(void) printf("%*sif ((%s = real2prim (%s, %s, ", level * 4, "", arg, narg, class);
		(void) printf("%s)) == NULLPE) {\n", value);
		(void) printf("%*sadvise (NULLCP, \"%s: %%s\", PEPY_ERR_NOMEM);\n",
			      (level + 1) * 4, "", id);
		(void) printf("%*sreturn NOTOK;\n%*s}\n", (level + 1) * 4, "", level * 4, "");
	...
	case YP_BITLIST:
		...
		if (yp->yp_code == YP_BITLIST) {
			...
			if (i >= 0)
				(void)
				    printf
				    ("%*sif (bit_test (%s, %d) == NOTOK)\n%*s(void) bit_off (%s, %d);\n",
				     level * 4, "", narg, i, (level + 1) * 4, "", narg, i);
		}
		...
		if (!dflag)
			for (yv = yp->yp_value; yv; yv = yv->yv_next) {
				...
				if (yv->yv_flags & YV_NAMED)
					(void) printf("\t/* %s */", yv->yv_named);
				(void) printf("\n");
				do_action(yv->yv_action, level + 1, narg, yv->yv_act_lineno);
				(void) printf("%*s}\n", level * 4, "");
			}
		break;
	...
	case YP_SEQLIST:
		for (y = yp->yp_type, i = 0; y; y = y->yp_next, i++) {
			if (y->yp_flags & YP_COMPONENTS)
				do_components_seq(y, level, y->yp_next == NULLYP, id, arg, narg);
			else {
				do_type_element(y, level, y->yp_next == NULLYP, id, narg);
				(void) printf("%*sif (%s != NULLPE)\n", level * 4, "", narg);
				(void) printf("%*sif (seq_add (%s, %s, -1) == NOTOK) {\n",
					      (level + 1) * 4, "", arg, narg);
				(void)
		...
		if (yp->yp_type) {
			for (y = yp->yp_type; y; y = y->yp_next) {
				if (y->yp_flags & YP_COMPONENTS)
					...
					(void)
					    printf
					    ("%*sadvise (NULLCP, \"%s %%s%%s\", PEPY_ERR_BAD_SET,\n",
					     (level + 2) * 4, "", id);
					(void) printf("%*spe_error (%s -> pe_errno));\n",
						      (level + 4) * 4, "", arg);
					(void) printf("%*sreturn NOTOK;\n%*s}\n", (level + 2) * 4,
	...
	(void) printf("\n#ifdef DEBUG\n%*s(void) testdebug (%s, \"", level * 4, "", arg);
	if (level == 1)
...
static
do_type_choice(yp, caseindex, level, narg)
	register YP yp;
	register int caseindex, level;
	register char *narg;
{
...
static
do_type_element(yp, level, last, id, narg)
	register YP yp;
	register int level;
	int last;
	char *id;
	register char *narg;
{
	...
	if (yp->yp_flags & YP_ID)
		(void) printf("\t/* %s */", yp->yp_id);
	(void) printf("\n");
	do_type(yp, level, yp->yp_flags & YP_ID ? yp->yp_id : "element", narg);
...
static
do_components_seq(yp, level, last, id, arg, narg)
	YP yp;
	register int level;
	register char *id, *arg, *narg;
{
	YP newyp, y;
	...
	for (y = newyp->yp_type; y; y = y->yp_next) {
		if (y->yp_flags & YP_COMPONENTS)
			do_components_seq(y, level, last && y->yp_next == NULLYP, id, arg, narg);
		else {
			do_type_element(y, level, last && y->yp_next == NULLYP, id, narg);
			(void) printf("%*sif (%s != NULLPE)\n", level * 4, "", narg);
			(void) printf("%*sif (seq_add (%s, %s, -1) == NOTOK) {\n",
				      (level + 1) * 4, "", arg, narg);
			(void) printf("%*sadvise (NULLCP, \"%s %%s%%s\", PEPY_ERR_BAD_SEQ,\n",
...
static
do_components_set(yp, level, arg, id, narg)
	register YP yp;
	register int level;
	char *narg, *arg, *id;
{

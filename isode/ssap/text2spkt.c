	...
	int read;
{
	...
	LLOG(lp, LLOG_ALL,
	     ("dump of SPDU 0x%x, errno=0x%x mask=0x%x%s",
	      s, s->s_errno, s->s_mask, s->s_mask & SMASK_SPDU_EXPD ? " (expedited)" : ""));
	(void) ll_printf(lp, "%s(\n", rw);
	...
	case SPDU_AC:
		...
		if (s->s_mask & SMASK_CN_OPT)
			type_bits(lp, rw, "OPTIONS", s->s_options, CR_OPT_MASK, OMASK);
		...
		if (s->s_mask & SMASK_CN_REQ)
			type_bits(lp, rw, "REQUIREMENTS", s->s_cn_require, -1, RMASK);
	...
	case SPDU_RF:
		...
		if (s->s_mask & SMASK_RF_DISC)
			type_bits(lp, rw, "DISCONNECT", s->s_rf_disconnect, RF_DISC_MASK, DMASK);
		if (s->s_mask & SMASK_RF_REQ)
			type_bits(lp, rw, "REQUIREMENTS", s->s_rf_require, -1, RMASK);
		if (s->s_mask & SMASK_RF_VRSN)
			type_vrsn(lp, rw, s->s_rf_version);
		if (s->s_rlen > 0) {
			type_reason(lp, rw, *s->s_rdata & 0xff);
			if (s->s_rlen > 1)
				type_data(lp, "REASON", rw, s->s_rlen - 1, s->s_rdata + 1);
		}
		break;
	...
	case SPDU_FN:
		...
		if (s->s_mask & SMASK_FN_DISC)
			type_bits(lp, rw, "DISCONNECT", s->s_fn_disconnect, FN_DISC_MASK, DMASK);
		break;
	...
	case SPDU_MIP:
		...
		if (s->s_mask & SMASK_MIP_SYNC)
			type_bits(lp, rw, "SYNC", s->s_mip_sync, MIP_SYNC_MASK, YMASK);
	...
	case SPDU_AS:
		...
		if (s->s_mask & SMASK_AS_ID) {
			...
			type_info(lp, "ID/ %d", (int) s->s_as_id.sd_len, s->s_as_id.sd_data);
			(void) ll_printf(lp, "\n");
		}
		break;
	...
	case SPDU_AR:
		...
		if (s->s_mask & SMASK_AR_OID) {
			(void) ll_printf(lp, "%s", rw);
			type_info(lp, "OLD ID/ %d", (int) s->s_ar_oid.sd_len, s->s_ar_oid.sd_data);
			(void) ll_printf(lp, "\n");
		}
		...
		if (s->s_mask & SMASK_AR_ID) {
			(void) ll_printf(lp, "%s", rw);
			type_info(lp, "ID/ %d", (int) s->s_ar_id.sd_len, s->s_ar_id.sd_data);
			(void) ll_printf(lp, "\n");
		}
		break;
...
static
type_id(lp, type, rw, selector, len)
	LLog *lp;
	char *type, *rw;
	char *selector;
	int len;
	...
	struct SSAPref *ref;
{
	...
	if (ref->sr_vlen)
		type_info(lp, "<CALLING %d", (int) ref->sr_calling_len, ref->sr_calling);
	else
	...
	if (ref->sr_vlen)
		type_info(lp, ", CALLED %d", (int) ref->sr_called_len, ref->sr_called);
	(void) ll_printf(lp, ">\n");
}

static
type_vrsn(lp, rw, version)
	LLog *lp;
	char *rw;
	u_char version;
{

...
static int si_table[] = {
	...
	PMASK_CN_ID		/* 0x0c: SPDU_RF */
	    | PMASK_TDISC | PMASK_USER_REQ | PMASK_VERSION | PMASK_ENCLOSE | PMASK_REASON,
	PMASK_CN_ID		/* 0x0d: SPDU_CN */
	    | PMASK_CN_ITEMS
	    | PMASK_USER_REQ
	    | PMASK_VERSION | PMASK_SSAP_CALLING | PMASK_SSAP_CALLED | PMASK_UDATA | PMASK_XDATA,
	PMASK_CN_ID		/* 0x0e: SPDU_AC */
	    | PMASK_CN_ITEMS
	    | PMASK_USER_REQ
	    | PMASK_VERSION
	    | PMASK_SSAP_CALLING | PMASK_TOKEN | PMASK_ENCLOSE | PMASK_SSAP_CALLED | PMASK_UDATA,
	...
	PMASK_TDISC		/* 0x19: SPDU_AB & SPDU_AI */
	    | PMASK_REFLECT | PMASK_REASON | PMASK_ENCLOSE | PMASK_UDATA,
	PMASK_NODATA,		/* 0x1a: SPDU_AA & SPDU_AIA */
	PMASK_NOTSUPPORTED,	/* 0x1b */
	PMASK_NOTSUPPORTED,	/* 0x1c */
	PMASK_LINK		/* 0x1d: SPDU_AR */
	    | PMASK_ACT_ID | PMASK_ENCLOSE | PMASK_SERIAL | PMASK_UDATA,
	...
	PMASK_TOKEN_SET		/* 0x22: SPDU_RA */
	    | PMASK_ENCLOSE | PMASK_SERIAL | PMASK_UDATA,
...
static int pi_length[PI_TABLE_LEN] = {
	...
	SREF_USER_SIZE		/* 0x01: Connection ID */
	    + SREF_COMM_SIZE + SREF_ADDT_SIZE + 6,
	0, 0, 0,		/* 0x02-04 */
	...
	2 * SREF_USER_SIZE	/* 0x21: Activity Link */
	    + SREF_COMM_SIZE + SREF_ADDT_SIZE + SID_DATA_SIZE + SIZE_CN_ISN + 6 * 2,
	0, 0, 0, 0, 0, 0, 0,	/* 0x22-28 */
	...
	int *len;
{
	...
	case SPDU_CN:
		...
		If_Set(SMASK_CN_CALLING)
		    put2spdu(PI_SSAP_CALLING, s->s_callinglen, s->s_calling, &c);
		If_Set(SMASK_CN_CALLED)
		    put2spdu(PI_SSAP_CALLED, s->s_calledlen, s->s_called, &c);
		Put_XData(CN_SIZE);
		break;
	...
	If_Set(SMASK_SPDU_AB) {
		...
		If_Set(SMASK_AB_REFL)
		    put2spdu(PI_REFLECT, AB_REFL_SIZE, (char *) s->s_reflect, &c);
		If_Set(SMASK_ENCLOSE)
		    Put_Item(PI_ENCLOSE, (char *) &s->s_enclose);
		Put_UData(AB_SIZE);
		break;
...
/* NB: caller responsible for mapping s -> s_udata to user info */
	...
	case SPDU_AS:
		...
		put2spdu(PI_ACT_ID, (int) s->s_as_id.sd_len, s->s_as_id.sd_data, &c);
		If_Set(SMASK_ENCLOSE)
		    Put_Item(PI_ENCLOSE, (char *) &s->s_enclose);
		Put_UData(AS_SIZE);
		break;
...
{
	register int li;
	int cat0, nread, pktlen, pgilen, pmask, xlen;
	register char *base;
	char *xbase;
	...
	while (pktlen && (s->s_errno == SC_ACCEPT)) {
		...
		default:
			if (code < PI_TABLE_LEN && pi_table[code]
			    && !(pmask & pi_table[code]))
				s->s_errno = SC_PROTOCOL;
			break;
		}

...
int
SSendRequest(sd, data, cc, begin, end, si)
	int sd;
	char *data;
	int cc, begin, end;
	struct SSAPindication *si;
{
	...
	missingP(data);
	if (cc <= 0)
		return ssaplose(si, SC_PARAMETER, NULLCP, "illegal value for SSDU length (%d)", cc);
	missingP(si);
...
int
SDataRequestAux(sb, code, uv, begin, end, si)
	register struct ssapblk *sb;
	int code;
	...
	struct SSAPindication *si;
{
	int cc, j, len, n, result;
	register char *bp, *ep;
	register struct ssapkt *s;
	struct TSAPdisconnect tds;
	register struct TSAPdisconnect *td = &tds;
	struct udvec vvs[NSPUV];
	register struct udvec *vv, *wv;
	struct udvec *xv;
	...
	case SPDU_TD:
		if (!(sb->sb_requirements & SR_TYPEDATA))
			return ssaplose(si, SC_OPERATION, NULLCP, "typed data service unavailable");
		break;
	}
	...
	while (uv->uv_base) {
		...
		if (!sb->sb_tsdu_us && uv->uv_base) {
			(void) ssaplose(si, SC_PARAMETER, NULLCP, "too many vector entries in SDU");
			goto out2;
		}
		vv->uv_base = NULL;
...
static int
SReadRequestAux(sb, sx, secs, si, async, tx)
	register struct ssapblk *sb;
...
/* allows AB SPDUs to have 512, not 9, octets (which is fine by me) */
		...
		if ((s->s_mask & SMASK_ENCLOSE)
			...
			if (sb->sb_version < SB_VRSN2) {
				(void) spktlose(sb->sb_fd, si, SC_PROTOCOL, NULLCP,
						"unexpected segmentation for SPDU 0x%x", s->s_code);
				goto out;
			}
#ifdef notdef
		...
		case SPDU_TD:
			...
			if (sb->sb_tsdu_them) {
				...
				eot = s->s_enclose & ENCL_END;
				   ...
				   (!(s->s_enclose & ENCL_BEGIN) || !(s->s_enclose & ENCL_END))) {
				(void) spktlose(sb->sb_fd, si, SC_PROTOCOL, NULLCP,
			...
			if (!eot)
				goto spin;
			sx->sx_type = sb->sb_code == SPDU_DT ? SX_NORMAL : SX_TYPED;
...
#undef	dotoken
			...
			if ((s = newspkt(SPDU_GTA)) == NULL) {
				(void) spktlose(sb->sb_fd, si, SC_CONGEST, NULLCP, "out of memory");
				break;
			}
		...
		case SPDU_MIA:
			if ((sb->sb_flags & SB_Vsc)
			    || sb->sb_V_A > s->s_mia_serial || s->s_mia_serial >= sb->sb_V_M) {
				freespkt(s);
				goto spin;
			}
...
#undef	dotoken2
		...
		case SPDU_AD:
			...
			si->si_type = SI_ACTIVITY;
			{
				...
				sv->sv_type = s->s_code == SPDU_AI ? SV_INTRIND : SV_DISCIND;
...
#undef	dotoken
			si->si_type = SI_ACTIVITY;
			{
				...
				sv->sv_type = s->s_code == SPDU_AIA ? SV_INTRCNF : SV_DISCCNF;
			}
			freespkt(s);
			return DONE;
...
int
SDoCollideAux(init, localop, localssn, remoteop, remotessn)
	int init, localop, remoteop;
	long localssn, remotessn;
{
	SLOG(ssap_log, LLOG_EXCEPTIONS, NULLCP,
	...
	register struct TSAPdata *ty;
{
	...
	if ((s = tsdu2spkt(&tx->tx_qbuf, tx->tx_cc, (cc = 1, &cc))) == NULL
	    || s->s_errno != SC_ACCEPT) {
		(void) spktlose(sb->sb_fd, si, s ? s->s_errno : SC_CONGEST, NULLCP, NULLCP);
	      bad1:;
		freespkt(s);
		TXFREE(tx);
		return NULL;
	...
	DLOG(ssap_log, LLOG_DEBUG,
	     ("got cat0 SPDU %d, %d octets left in TSDU", s->s_code, tx->tx_cc));
	...
	DLOG(ssap_log, LLOG_DEBUG,
	     ("got cat2 SPDU %d, %d octets left in TSDU", s->s_code, tx->tx_cc));
	...
	register struct TSAPdisconnect *td;
{
	...
	default:
		(void) sprintf(cp = buffer, " (%s at transport)", TErrString(td->td_reason));
	case DR_NETWORK:
		reason = SC_TRANSPORT;
		break;
	}
	...
	register struct ssapblk *sb;
{
	...
	if (sb->sb_fd != NOTOK) {
		...
		if (sb->sb_flags & SB_FINN)
			...
			if (ses_dn_timer >= 0)
				while (TReadRequest(sb->sb_fd, &txs, ses_dn_timer, &tds) != NOTOK) {
					TXFREE(&txs);
				}

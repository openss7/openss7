...
int
SAsynConnRequest(ref, calling, called, requirements, settings, isn, data, cc, qos, sc, si, async)
	struct SSAPref *ref;
	struct SSAPaddr *calling, *called;
	int requirements, settings, cc, async;
	long isn;
	char *data;
	struct QOStype *qos;
	...
	struct SSAPindication *si;
{
	...
	if (requirements & ~SR_MYREQUIRE)
		return ssaplose(si, SC_PARAMETER, NULLCP, "requirements settings not supported");
...
static int
SConnRequestAux(ref, calling, called, requirements, settings, isn, data, cc, qos, sc, si, async)
	struct SSAPref *ref;
	struct SSAPaddr *calling, *called;
	int requirements, settings, cc, async;
	long isn;
	...
	struct SSAPindication *si;
{
	...
	if ((result = TAsynConnRequest(calling ? &calling->sa_addr : NULLTA,
				       &called->sa_addr, (qos ? qos->qos_extended : 0)
	...
	if (qos && qos->qos_sversion < 0) {
		...
		sb->sb_vrsnmask = 1 << (sb->sb_version =
					(cc > SS_SIZE || (qos && qos->qos_sversion > 1))
					? SB_VRSN2 : SB_VRSN1);
	...
	if ((result = SAsynRetryAux1(sb, tc, sc, si)) == DONE && !async)
		result = OK;
	return result;
	...
	struct SSAPindication *si;
{
	...
	if (sb->sb_flags & SB_CONN) {
		(void) sigiomask(smask);
		return ssaplose(si, SC_OPERATION, NULLCP, "session descriptor connected");
	}

	if (sb->sb_flags & SB_TS_CONN)
		result = SAsynRetryAux2(sb, &sb->sb_tc, sc, si);
	else
...
/*
    sb -> sb_retry = NULL;
*/
	sb->sb_flags |= SB_TS_CONN;	/* now got as far as transport */
...
*/
	...
	if (sb->sb_flags & SB_ASYNC_CONN) {
		sb->sb_tc = *tc;	/* struct copy */
		return CONNECTING_2;
	}
	...
	struct SSAPindication *si;
{
...
#ifdef	notdef			/* catch this in the enxt test */
	...
	case SPDU_RF:		/* ignore s -> s_rf_disconnect */
		sc->sc_sd = NOTOK;
		sc->sc_result = s->s_rlen > 0 ? *s->s_rdata : SC_NOTSPECIFIED;

...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/ssap/RCS/ssaprespond.c,v 9.0 1992/06/16 12:39:41 isode Rel";
#endif
	...
	struct SSAPindication *si;
{
	...
	if (vecp == 2 || TInit(vecp, vec, ts, td) != NOTOK) {
		...
		if ((reason = td->td_reason) != DR_PARAMETER
		    ...
		    || (s = str2spkt(vec[1])) == NULL || s->s_errno != SC_ACCEPT) {
			if (s)
				freespkt(s);
			else
				(void) ts2sslose(si, reason != DR_PARAMETER ? "TInit"
						 : "TRestoreState", td);
			(void) ssaplose(si, SC_PARAMETER, NULLCP, "bad initialization vector");
			goto out1;
		}
		bzero(vec[0], strlen(vec[0]));
		bzero(vec[1], strlen(vec[1]));
	...
	sb->sb_fd = ts->ts_sd;
	sb->sb_version = (s->s_mask & SMASK_CN_VRSN)
	    ? ((s->s_cn_version & (1 << SB_VRSN2))
	       ? SB_VRSN2 : SB_VRSN1)
	    : s->s_ulen > SS_SIZE ? SB_VRSN2 : SB_VRSN1;
	if (ts->ts_expedited)
		sb->sb_flags |= SB_EXPD;
	...
	if (s->s_tsdu_init < (sb->sb_tsdu_them = GET_TSDU_SIZE(ts->ts_tsdusize)))
		sb->sb_tsdu_them = s->s_tsdu_init;
	if (s->s_tsdu_resp < (sb->sb_tsdu_us = GET_TSDU_SIZE(ts->ts_tsdusize)))
	...
	if (s->s_mask & SMASK_CN_CALLING) {
		...
		bcopy(s->s_calling, ss->ss_calling.sa_selector, ss->ss_calling.sa_selectlen = len);
	}
	...
	if (s->s_mask & SMASK_CN_CALLED) {
		...
		bcopy(s->s_called, ss->ss_called.sa_selector, ss->ss_called.sa_selectlen = len);
	}
...
int
SConnResponse(sd, ref, responding, status, requirements, settings, isn, data, cc, si)
	int sd;
	struct SSAPref *ref;
	struct SSAPaddr *responding;
	int status, requirements, settings, cc;
	long isn;
	char *data;
	struct SSAPindication *si;
{
	...
	case SC_ACCEPT:
		...
		dotokens();
		if (sb->sb_requirements & (SR_MINORSYNC | SR_MAJORSYNC | SR_RESYNC)) {

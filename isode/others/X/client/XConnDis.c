 ...
 */
	...
	ret = TConnRequest(&(rmpa.pa_addr.sa_addr),
			   &(rpa.pa_addr.sa_addr), 0, 0, NULLCP, qos, tc, td);
 ...
 */
int
_XConnectDisplay(display_name, expanded_name, prop_name, screen_num)
	char *display_name;
 ...
 */
	...
	*(display_ptr++) = '\0';
	...
	/* displaybuf now contains only a null-terminated host name, and display_ptr points to the
	   display number. If the display number is missing there is an error. */
	...
	*screen_num = atoi(dot_ptr + 1);
	...
	if (dnet) {
		 ...
		 */
		if ((fd = dnet_conn(displaybuf, objname, SOCK_STREAM, 0, 0, 0, 0)) < 0)
	...
	{
#ifdef UNIXCONN
		if ((displaybuf[0] == '\0') || (strcmp("unix", displaybuf) == 0)) {
			...
			if (hostinetaddr == -1) {
				...
				/* Only Cray UNICOS3 and UNICOS4 will define this */
				{
					...
					bcopy((char *) host_ptr->h_addr,
					      (char *) &t, sizeof(inaddr.sin_addr));
					inaddr.sin_addr = t;
				}
#else
				bcopy((char *) host_ptr->h_addr,
				      (char *) &inaddr.sin_addr, sizeof(inaddr.sin_addr));
 ...
 */
		...
		BITSET(r_mask, dpy->fd);
		result = TNetAccept(&vecp, vec, dpy->fd + 1, r_mask, NULL, NULL, NOTOK, td);
#else				/* ISOCONN */
		BITSET(r_mask, dpy->fd);

	...
	register Display *dpy;
{
	...
	do {
		...
		/* must read at least one xEvent; if none is pending, then we'll just flush and
	...
	ENDITERATE} while (dpy->head == NULL);
}
	...
	register long size;
{
	...
	while (total) {
		...
		/* You could be very general here and have "in" and "out" iovecs and write a loop
		   without using a macro, but what the heck */
	...
	register xGenericReply *rep;
{
	...
	newseq = (dpy->last_request_read & ~((unsigned long) 0xffff)) | rep->sequenceNumber;
	...
	while (newseq < lastseq) {
		...
		if (newseq > dpy->request) {
			(void) fprintf(stderr,
				       ...
				       newseq, dpy->request, (unsigned int) rep->type);
			newseq -= 0x10000;
			break;
		}
	}
...
{
	/* Pull out the serial number now, so that (currently illegal) requests generated by an
	   error handler don't confuse us. */
	...
	while (1) {
		...
		case X_Reply:
			 ...
			 */
			_XRead(dpy, NEXTPTR(rep, xReply), ((long) rep->generic.length) << 2);
			(*_XIOErrorFunction) (dpy);
			return (0);
	...
	register xEvent *event;
{
	register _XQEvent *qelt;
	 ...
	 /*NOSTRICT*/ if (qelt = _qfree) {
		/* If _qfree is non-NULL do this, else malloc a new one. */
		_qfree = qelt->next;
	} else if ((qelt = (_XQEvent *) Xmalloc((unsigned) sizeof(_XQEvent))) == NULL) {
		/* Malloc call failed! */
		errno = ENOMEM;
		(*_XIOErrorFunction) (dpy);
	}
	...
	register xEvent *event;		/* wire protocol event */
{
	...
	((XAnyEvent *) re)->serial = _SetLastRequestRead(dpy, (xGenericReply *) event);
	((XAnyEvent *) re)->send_event = ((event->u.u.type & 0x80) != 0);
	((XAnyEvent *) re)->display = dpy;

	/* Ignore the leading bit of the event type since it is set when a client sends an event
	...
	case LeaveNotify:
	{
		...
		ev->same_screen = (event->u.enterLeave.flags & ELFlagSameScreen) && True;
	...
	case KeymapNotify:
	{
		...
		bcopy((char *) ((xKeymapEvent *) event)->map,
		      &ev->key_vector[1], sizeof(((xKeymapEvent *) event)->map));
	}
		break;
	case Expose:
	{

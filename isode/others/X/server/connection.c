...
#endif				/* hpux */
#ifdef SO_LINGER
		if (setsockopt(request, SOL_SOCKET, SO_LINGER, (char *) linger, sizeof(linger)))
 ...
 */
	...
	for (ingot = 0, aptr = data, q2data = min(size, result);
	     ingot < q2data; aptr += qcpy, ingot += qcpy) {
	...
	int charsWanted;
{
	...
	setitimer(ITIMER_REAL, &itv, (struct itimerval *) NULL);
	/* If we got here and we didn't time out, then return TRUE, because we must have read what
	   we wanted. If we timed out, return FALSE */
 ...
 */
	*reason = 0;
	...
	if (!ReadBuffer(conn, (char *) &xccp, sizeof(xConnClientPrefix))) {
		/* If they can't even give us this much, just blow them off without an error
		   message */
		*reason = 0;
		return 0;
	}
	if (xccp.byteOrder != whichByteIsFirst) {
		...
		*pswapped = FALSE;
	if ((xccp.majorVersion != X_PROTOCOL) || (xccp.minorVersion != X_PROTOCOL_REVISION)) {
...
#undef STR
	...
	fromlen = sizeof(from);
#ifdef ISOCONN
	if (SGetPeerName(conn, &(from.ts), &fromlen) || InvalidHost(&(from.ts), fromlen))
#else				/* ISOCONN */
	if (getpeername(conn, &from.sa, &fromlen) || InvalidHost(&from.sa, fromlen))
#endif				/* ISOCONN */
	{
...
#undef STR
	...
	/* At this point, if the client is authorized to change the access control list, we should
	   getpeername() information, and add the client to the selfhosts list.  It's not really
	   the host machine, but the true purpose of the selfhosts list is to see who may change
...
#endif				/* ISOCONN */
{
	...
	int swapped;			/* set by ClientAuthorized if connection is swapped */
	char *reason;
	struct iovec iov[2];
 ...
 */
			...
			if ((newconn = accept(curconn,
				...
				if (newconn >= lastfdesc) {
					if (debug_conns)
						ErrorF
						    ("Didn't make connection: Out of file descriptors for connections\n");
#ifdef ISOCONN
					SClose(newconn);
#else				/* ISOCONN */
					close(newconn);
					...
					if (fd2family(newconn) == UNIX_IO) {
						...
						if (!getpeername(newconn, &from.sa, &fromlen)) {
							if (fromlen
								...
								int mi = 1;
								setsockopt(newconn, IPPROTO_TCP,
									   TCP_NODELAY,
									   (char *) &mi,
									   sizeof(int));
							}
					...
					if (ClientAuthorized(newconn, &swapped, &reason)) {
						...
						if (next != (ClientPtr) NULL) {
							...
							priv =
							    (OsCommPtr) xalloc(sizeof(OsCommRec));
							priv->fd = newconn;
...
#ifdef ISOCONN
							(void) SWrite(newconn, (char *) &c,
								      sizeof(xConnSetupPrefix));
#else				/* ISOCONN */
							write(newconn, (char *) &c,
							      sizeof(xConnSetupPrefix));
#endif				/* ISOCONN */
							iov[0].iov_len = c.lengthReason;
							iov[0].iov_base = reason;
							iov[1].iov_len =
							    padlength[c.lengthReason & 3];
							iov[1].iov_base = p;
#ifdef ISOCONN
	...
	for (i = 0; i < mskcnt; i++) {
		while (mask[i]) {
			...
			r = TNetAccept(&vecp, vec,
				       curclient + 1, tmask, (int *) NULL, (int *) NULL, OK, td);
			if (r == NOTOK) {
				Error(TErrString(td->td_reason));
				Error("TNetAccept");
#else				/* ISOCONN */
			r = select(curclient + 1, tmask, (int *) NULL, (int *) NULL, &notime);
			if (r < 0) {
#endif				/* ISOCONN */

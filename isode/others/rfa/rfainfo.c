	...
	struct RfaInfo *rfa;
{
	...
	if (lstat(fn, &st) == -1) {
		advise(LLOG_EXCEPTIONS, NULLCP, "cant stat '%s':%s", fn, sys_errname(errno));
 ...
 *------------------------------------------------------*/
int
getRfaInfoList(dir, rfaHeadp, target, locked)
	char *dir;
	...
	int locked;
{
	...
	if (stat(makeFN(dir), &st) == -1) {
		advise(LLOG_EXCEPTIONS, NULLCP, "cant stat '%s':%s", dir, sys_errname(errno));
	...
	struct RfaInfo *rfa;
{
	...
	for (; rfa; rfa = rfa->ri_next) {
		...
		if (fprintf(le->le_filep, "%s %s %ld %s %ld\n", rfa->ri_filename,
			    ...
			    rfa->ri_lckname ? rfa->ri_lckname : "NONE", rfa->ri_lcksince) == EOF) {

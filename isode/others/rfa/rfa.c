	...
	/*--- check if remote is already master ---*/
	    ...
	    /*--- consistency checks ---*/
		    ...
		    /*--- try to become master ---*/
					...
					if ((rma = (struct type_RFA_RequestMasterArg *)
					     malloc(sizeof(struct type_RFA_RequestMasterArg))) ==
					    NULL) {
						releaseRfaInfoList(dirname(fn), rfalist);
						freeRfaInfoList(remoteRfaList);
						fprintf(err, "*** local error : no memory ***\n");
				...
				if (IS_SLAVE(rrfa->ri_status)) {
					if (interactive) {
						fprintf(err,
							"remote version is already slave, become");
	    ...
	    /*--- consistency checks ---*/
			if (rrfa = findRfaInfo(basename(fn), remoteRfaList)) {
				...
				if (((rfa->ri_mode & S_IFMT & S_IFDIR) == 0)
					...
					releaseRfaInfoList(dirname(fn), rfalist);
					freeRfaInfoList(remoteRfaList);
					fprintf(err, "*** remote master version of %s would be",
						fn);
				...
				releaseRfaInfoList(dirname(fn), rfalist);
				freeRfaInfoList(remoteRfaList);
				fprintf(err, "*** remote site is not master for file %s ***\n", fn);
				CONT(NOTOK_REMOTE_NOT_MASTER);
			}
	...
	char *cmd;
{
	...
	static struct cmd {
		...
		"lock", do_lockFile, "request lock for a local file"}, {
		"unlock", do_unlockFile, "release lock for a local file"}, {
		"rlist", do_listDir, "list files in the remote directory"}, {
		"list", do_localListDir, "list files in a local directory"}, {
		"pwd", do_pwd, "print the current directory path"}, {
		"cd", do_changeDir, "change the current directory path"}, {
		"master", do_master, "make a local file a MASTER version"}, {
		"slave", do_slave, "make a local file a SLAVE version"}, {
		"unregister", do_unregister, "unregister a previously MASTER or SLAVE file"}, {
		"setreq", do_setreq, "set file transfer mode to 'request'"}, {
		"setauto", do_setauto, "set file transfer mode to 'automatic'"}, {
		"syncdir", do_syncdir,
		...
		"timesync", do_timesync, "syncronize local time with masters clock"}, {
		"quit", do_quit, "terminate RFA session"}, {
		"exit", do_quit, "terminate RFA session"}, {
		NULL, NULL, NULL}
	}, *cmdp, *fc;
	...
	char **av;
{
	...
	while ((c = getopt(ac, av, "qu:p:c:h:")) != -1)
		...
		case '?':
			fprintf(stderr,
				"USAGE: %s [-h  hostname] [-u user] [-p password] [-c command] [ -q ]\n",
				basename(myname));
			exit(NOTOK_LOCAL_ERROR);
		}

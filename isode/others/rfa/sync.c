	...
	struct RfaInfo *rfa;
{
	...
	sprintf(fn, "%s%s%s", dir, *(dir + strlen(dir) - 1) == '/' ? "" : "/", rfa->ri_filename);
...
/*--------------------------------------------------------------*/
int
handleDir(dir, localRfaListPtr, rrfa, rec, wrp)
	char *dir;

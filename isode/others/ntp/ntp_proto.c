...
#ifdef notdef
	if (peer->flags & PEER_FL_AUTHENABLE && peer->vers == 2 && authhavekey(peer->keyid)) {
...
/* Not Yet in this version */
	...
	if ((peer->flags & (PEER_FL_BCAST | PEER_FL_REFCLOCK)) == 0) {
		...
		case AF_INET:
			if (send_inet(ap, (char *) pkt, sizeof(*pkt), &peer->src) < 0)
				return;
			break;
		}
#ifdef	REFCLOCK
		...
		if (i && peer->reach == 0) {
			advise(LLOG_NOTICE, NULLCP,
			       "Lost reachability with %.4s", peer->refid.rid_string);
		}
	...
	if (peer->reach == 0) {
		clear(peer);
		if (peer->src.type == AF_INET)
			peer->sock = -1;	/* since he fell off the end of the earth, don't
						   depend on local address any longer */
	}
	...
	int sock;
{
	...
	/* if we're only going to support NTP Version 2 then this stuff isn't necessary, right? */

	if ((peer_mode = pkt->status & MODEMASK) == 0 && dst) {
		/* packet from an older NTP implementation.  Synthesize the correct mode.  The
		   mapping goes like this:

		   pkt source port pkt dst port Mode --------------- ------------ ---- NTP Port NTP 
		   Port symmetric active NTP Port not NTP Port server not NTP Port NTP Port client
		   not NTP Port not NTP Port <not possible>

		   Now, since we only are processing packets with the destination being NTP Port,
		   it reduces to the two cases:

		   pkt source port pkt dst port Mode --------------- ------------ ---- NTP Port NTP 
		   Port symmetric active not NTP Port NTP Port client */
	...
	if (peer_mode < MODE_SYM_ACT || peer_mode > MODE_BROADCAST) {
		TRACE(1, ("Bogus peer_mode %d from %s", peer_mode,
			  (struct ntp_peer *) dst == peer ? "refclock" : paddr(dst)));
#ifdef	DEBUG
		if (debug > 3)
			abort();
#endif
	...
	case ACT_PKT:
		if (!(((peer->flags & PEER_FL_CONFIG) == 0) &&
			...
			process_packet((struct ntp_peer *) dst == peer ?
				       NULL : dst, pkt, tvp, peer);
			break;
		}
		/* Note fall-through */
	case ACT_XMIT:
		process_packet((struct ntp_peer *) dst == peer ? NULL : dst, pkt, tvp, peer);
	...
	struct ntp_peer *peer;
{
	...
	if (peer->src.type == AF_INET) {
		peer->refid.rid_type = peer->stratum == 1 ? RID_STRING : RID_INET;
	...
	struct ntp_peer *peer;
{
	...
	if ((peer == sys.peer) && (sys.hold == 0)) {
		...
		if (peer->flags & PEER_FL_REFCLOCK) {
			/* once we re-map the stratums so that stratum 0 is better than stratum 1,
			   some of this foolishness can go away */
		...
		if (adj_logical(peer->estoffset) > 0) {
			...
			TRACE(1, ("Clockset from %s stratum %d offset %f",
				  paddr(&peer->src), peer->stratum, peer->estoffset));
			...
			if (logstats) {
				advise(LLOG_NOTICE, NULLCP,
				       ...
				       paddr(&peer->src),
				       peer->stratum, peer->estoffset, drift_comp, compliance);
			}
		}
	}
}
...
initialize()
{
	...
	sys.stratum = 0;
	sys.precision = 0;	/* may be specified in the config file; if not, gets set in
				   init_kern_vars() */
	...
	register struct ntp_peer *peer;
{
	...
	TRACE(3, ("clear: emptied filter for %s", paddr(&peer->src)));
	if (peer->reach != 0)
	...
	int new_hpoll;
{
	...
	if (interval < peer->timer) {
		interval = (double) interval *(double) (random() % 100 / 100.0);

		peer->timer = interval;
	}
	TRACE(3, ("poll_update: timer %d, poll=%d", peer->timer, interval));
}

...
select_clock()
{
	...
	if (!candidates) {
		if (sys.peer != NULL) {
			advise(LLOG_NOTICE, NULLCP, "Lost NTP peer %s", paddr(&sys.peer->src));
		}
	...
	TRACE(3, ("select_clock: step2 %d candidates", candidates));

	/* truncate the list at NTP_MAXLIST peers */
	if (candidates > NTP_MAXLIST)
		candidates = NTP_MAXLIST;

	TRACE(3, ("select_clock: step3 %d candidates", candidates));
	...
	TRACE(3, ("select_clock: step4 %d candidates", candidates));
	/* 
	...
	for (i = 0; i < candidates - 1; i++) {
		for (j = i + 1; j < candidates; j++) {
			if ((sel_lst[i].peer->stratum > sel_lst[j].peer->stratum) ||
			    ((sel_lst[i].peer->stratum == sel_lst[j].peer->stratum)
			     && (sel_lst[i].peer->estdelay > sel_lst[j].peer->estdelay))) {
				ptmp = sel_lst[i].peer;
				sel_lst[i].peer = sel_lst[j].peer;
				sel_lst[j].peer = ptmp;
			}
	...
	while (candidates > 1) {
		...
		TRACE(3, ("select_clock: step5 %d candidates", candidates));
		for (i = 0; i < candidates; i++) {
			/* compute dispersion of candidate `i' relative to the rest of the
			   candidates */
			dispersion = 0.0;
			weight = 1.0;
			sel_lst[i].peer->flags |= PEER_FL_CANDIDATE;
			for (j = 0; j < candidates; j++) {
				dtmp = sel_lst[j].peer->estoffset - sel_lst[i].peer->estoffset;
				if (dtmp < 0)
					dtmp = -dtmp;
				dispersion += dtmp * weight;
				weight *= NTP_SELECT;
			}
			/* since we just happen to have this double floating around.. */
			...
			TRACE(4, (" peer %s => disp %f prec_th %f",
				  paddr(&sel_lst[i].peer->src), dispersion, precision_thres));
		}
		/* 
		...
		TRACE(4, (" peer %s => TOSS", paddr(&sel_lst[worst].peer->src)));
		/* 
	...
	TRACE(3, ("select_clock: step6 %d candidates", candidates));
	...
	if (sys.peer != sel_lst[0].peer) {
		if (sys.peer != NULL)
			advise(LLOG_NOTICE, NULLCP,
			       ...
			       sel_lst[0].peer->stratum, paddr(&sys.peer->src), sys.peer->stratum);
		else
			advise(LLOG_NOTICE, NULLCP,
			       "clock: select peer %s stratum %d was UNSYNCED",
			       paddr(&sel_lst[0].peer->src), sel_lst[0].peer->stratum);
...
sanity_check(peer)
	struct ntp_peer *peer;
{
	TRACE(7, ("Checking peer %s stratum %d", paddr(&peer->src), peer->stratum));
	...
	/* Sanity check 2. if peer.stratum is greater than one (synchronized via NTP), peer.refid
	   must not match peer.dstadr */
	...
	if (peer->stratum > 1) {
		...
		for (i = 1; i < nintf; i++) {
			...
			if (addrs[i].addr.type == AF_INET &&
			    ...
			    addrs[i].addr.inet_ad.sin_addr.s_addr == peer->refid.rid_inet)
				return (0);
			if (addrs[i].addr.type == AF_OSI &&
			    peer->refid.rid_type == RID_PSAP &&
			    psapaddr_cmp(&peer->refid.rid_psap, &addrs[i].addr.psap_ad))
				return 0;
		}

	}

	/* Sanity check 3. Both peer.estdelay and peer.estdisp to be less than NTP_MAXWGT, which
	   insures that the filter register at least half full, yet avoids using data from very
	   noisy associations or broken implementations.  */
	if (peer->estdisp > (float) NTP_MAXWGT || peer->estdelay > (float) NTP_MAXWGT)
		return (0);

	/* Sanity check 4. The peer clock must be synchronized... and the interval since the peer
	...
	TRACE(7, ("That one is certainly qualified %s", paddr(&peer->src)));
	return (1);
}

...
receive(), clear(), clock_filter(), select_clock(), poll_update(), adios(), advise();
	...
	char *addr;
{
	...
	if (RyDispatch(NOTOK, table_NTP_Operations, operation_NTP_query, query_func, roi) == NOTOK)
		adios(NULLCP, "RyDispatch failed");
	...
	struct ntp_peer *peer;
{
	...
	TRACE(2, ("Sending OSI packet to %s fd %d if %d", paddr(&ap->addr), ap->fd, ap - addrs));

	if (ap->addr.type != AF_OSI) {
		advise(LLOG_EXCEPTIONS, NULLCP, "Wrong address family in send_osi!");
		return -1;
	}
...
/* really 2 */
	...
	if (i && peer->reach == 0) {
		advise(LLOG_NOTICE, NULLCP, "Lost reachability with %s", paddr(&peer->src));
	}
	...
	struct RoSAPindication *roi;
{
	...
	if (peer == NULL) {
		...
		make_new_peer(peer);
		peer->src = *dst;
		peer->sock = sock;	/* remember which socket we heard this from */
		peer->hmode = MODE_SYM_PAS;
		peer->reach = 0;
		clear(peer);
		/* 
	...
	if (peer_mode < MODE_SYM_ACT || peer_mode > MODE_BROADCAST) {
		TRACE(1, ("Bogus peer_mode %d from %s", peer_mode, paddr(dst)));
		return OK;
	}

	if (peer->hmode < MODE_SYM_ACT || peer->hmode > MODE_BROADCAST) {
		advise(LLOG_EXCEPTIONS, NULLCP,
		       "Bogus hmode %d for peer %s", peer->hmode, paddr(&peer->src));
		abort();
	}
	peer->backoff = 0;
...
static void
process_packet_osi(dst, pkt, tvp, peer)
	struct Naddr *dst;
	...
	struct ntp_peer *peer;
{
	...
	if (bogus || duplicate ||
	    ...
	    (pkt->receiveTimestamp->integer == 0 && pkt->receiveTimestamp->fraction == 0)) {
		peer->pkt_dropped++;
...
/* END Protocol specific stuff */
...
#else
	delay = MAX(delay, (peer->flags & PEER_FL_REFCLOCK) ? NTP_REFMINDIST : NTP_MINDIST);
#endif
	...
	TRACE(1, ("host: %s : %f : %f : %f : %f : %f : %o",
		  ...
		  delay, offset, peer->estdelay, peer->estoffset, peer->estdisp, peer->reach));
	...
	Refid *rid;
{
	...
	case RID_STRING:
		...
		if (rid->rid_type == RID_STRING)
			ci->un.referenceClock = str2qb(rid->rid_string, 4, 1);
		else
	...
	register struct RoSAPindication *roi;
{
	...
	case ROI_RESULT:
	case ROI_ERROR:
		advise(LLOG_EXCEPTIONS, NULLCP, "unexpected indication type=%d", roi->roi_type);
		terminate(ap, roi);
		break;
	...
	case ROI_UREJECT:
	{
		...
		if (rou->rou_noid)
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "RO-REJECT-U.INDICATION/%d: %s", fd, RoErrString(rou->rou_reason));
		else
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "RO-REJECT-U.INDICATION/%d: %s (id=%d)",
			       fd, RoErrString(rou->rou_reason), rou->rou_id);
	}
		break;
	...
	case ROI_FINISH:
	{
		...
		advise(LLOG_EXCEPTIONS, NULLCP, "A-RELEASE.INDICATION/%d: %d", fd, acf->acf_reason);

		result = AcRelResponse(fd, ACS_ACCEPT, ACR_NORMAL, NULLPEP, 0, &acis);
	...
	default:
		advise(LLOG_EXCEPTIONS, NULLCP, "unknown indication type=%d", roi->roi_type);
	}
}
	...
	struct RoSAPindication *roi;
{
	...
	advise(LLOG_NOTICE, NULLCP, "Connection on %d if %d TERMINATED", ap->fd, fd);
}

void
iso_init(vecp, vec, fd)
	int vecp;
	char **vec;
	int fd;
{
	...
	struct intf *ap;
{
	...
	sd = acs->acs_sd;
	if (acs->acs_ninfo > 0) {
		PLOG(pgm_log, print_NTP_BindArgument, acs->acs_info[0], "NTP.BindArgument", 1);
		if (decode_NTP_BindArgument(acs->acs_info[0], 1, NULLIP, NULLVP, &bindarg) == NOTOK) {
			advise(LLOG_EXCEPTIONS, NULLCP, "bind decode failed [%s]", PY_pepy);
			free_NTP_BindArgument(bindarg);
			return bindfailed(ap, acs, int_NTP_reason_badarg, NULLCP);
		}
		...
		if (bit_test(bindarg->version, bit_NTP_version_version__2))
			version = 2;
		else if (bit_test(bindarg->version, bit_NTP_version_version__1))
			version = 1;
		else {
			free_NTP_BindArgument(bindarg);
			return bindfailed(ap, acs, int_NTP_reason_version, "No acceptable version");
		}
		if (bindarg->authentication) {
			advise(LLOG_NOTICE, "Connection specifies authentication");
			free_NTP_BindArgument(bindarg);
		...
		default:
			free_NTP_BindArgument(bindarg);
			return bindfailed(ap, acs, int_NTP_reason_badarg, "Unknown mode");
		}
		free_NTP_BindArgument(bindarg);
		...
		bindresult->mode->parm =
		    mode == PEERMODE_QUERY ? int_NTP_BindMode_query : int_NTP_BindMode_normal;
		if (encode_NTP_BindResult(&pe, 1, 0, NULLCP, bindresult) == NOTOK) {
			advise(LLOG_EXCEPTIONS, NULLCP, "encode failed [%s]", PY_pepy);
			return bindfailed(ap, acs, int_NTP_reason_congested, "Can't build result");
		}
		PLOG(pgm_log, print_NTP_BindResult, pe, "NTP.BindResult", 0);
		pe->pe_context = 3;
	...
	TRACE(2, ("Association accepted from %s sd %d if %d", paddr(adr), sd, ap->inum));
	...
	char *msg;
{
	...
	if (encode_NTP_BindError(&pe, 1, 0, NULLCP, binderr) == NOTOK) {
		advise(LLOG_EXCEPTIONS, NULLCP, "ecode binderror failed [%s]", PY_pepy);
		ACSFREE(acs);
		terminate(ap, roi);
		return NOTOK;
	}
	...
	(void) AcAssocResponse(ap->fd, ACS_REJECT, ACS_USER_NULL,
			       ...
			       ps->ps_prequirements,
			       ps->ps_srequirements, SERIAL_NONE,
			       ps->ps_settings, &ps->ps_connect, &pe, 1, aci);
	pe_free(pe);
	ACSFREE(acs);
	terminate(ap, roi);
	return NOTOK;
	...
	struct RoSAPindication *roi;
{
	...
	if ((ctx = ode2oid(mycontext)) == NULLOID) {
		advise(LLOG_EXCEPTIONS, NULLCP, "%s: unknown object descriptor", mycontext);
		return NOTOK;
	}
	...
	if ((pci = ode2oid(mypci)) == NULLOID) {
		advise(LLOG_EXCEPTIONS, NULLCP, "%s: unknown object descriptor", mypci);
		return NOTOK;
	}
	...
	default:
		advise(LLOG_EXCEPTIONS, NULLCP, "Unknown response (%d)", result);
		break;
	}
	return NOTOK;
}
	...
	struct ntp_peer *peer;
{
	...
	if (acc->acc_ninfo > 0) {
		PLOG(pgm_log, print_NTP_BindResult, acc->acc_info[0], "NTP.BindResult", 1);
		if (decode_NTP_BindResult(acc->acc_info[0], 1, NULLIP, NULLVP, &bindres) == NOTOK) {
			advise(LLOG_EXCEPTIONS, NULLCP, "decode bindresult failed [%s]", PY_pepy);
			terminate(ap, roi);
			return NOTOK;
		}
	...
	TRACE(1, ("CONNECTED to %s on %d if %d",
		  paddr2str(&peer->src.psap_ad, NULLNA), sd, peer->sock));
	return DONE;
}

/* ARGSUSED */
static PE
build_bind_arg(psap, peer)
	...
	struct ntp_peer *peer;
{
	...
	bindarg->mode->parm = int_NTP_BindMode_normal;
	if (encode_NTP_BindArgument(&pe, 1, 0, NULLCP, bindarg) == NOTOK) {
		pe = NULLPE;
		advise(LLOG_EXCEPTIONS, NULLCP, "encode Bindargument failed [%s]", PY_pepy);
	} else {
		pe->pe_context = 3;
		PLOG(pgm_log, print_NTP_BindArgument, pe, "NTP.BindArgument", 0);
	}
	free_NTP_BindArgument(bindarg);
	return pe;
}
	...
	struct RoSAPindication *roi;
{
	...
	default:
		advise(LLOG_EXCEPTIONS, NULLCP, "Bad response from retry %d", result);
		terminate(ap, roi);
		break;
	}
	return NOTOK;
	...
	struct intf *ap;
{
	if (acc->acc_ninfo > 0) {
		...
		PLOG(pgm_log, print_NTP_BindError, acc->acc_info[0], "NTP.BindError", 1);
		if (decode_NTP_BindError(acc->acc_info[0], 1, NULLIP, NULLVP, &binderr) != NOTOK) {
			...
			case int_NTP_reason_refused:
				advise(LLOG_EXCEPTIONS, "connection refused: %s", cp ? cp : "");
				break;
			case int_NTP_reason_validation:
				advise(LLOG_EXCEPTIONS, "validation failure: %s", cp ? cp : "");
				break;
			case int_NTP_reason_version:
				advise(LLOG_EXCEPTIONS, "version mismatch: %s", cp ? cp : "");
				break;
			case int_NTP_reason_badarg:
				advise(LLOG_EXCEPTIONS, "bad connect argument: %s", cp ? cp : "");
				break;
			case int_NTP_reason_congested:
				advise(LLOG_EXCEPTIONS, "congested: %s", cp ? cp : "");
				break;
			default:
				advise(LLOG_EXCEPTIONS, NULLCP,
				       "Unknown reason (%d) %s", binderr->reason, cp ? cp : "");
				break;
			}
			free_NTP_BindError(binderr);
		} else
			advise(LLOG_EXCEPTIONS, NULLCP, "decode bind error failed [%s]", PY_pepy);
	} else
		advise(LLOG_EXCEPTIONS, NULLCP,
		       "Connection failed: %s", AcErrString(acc->acc_result));
	ACCFREE(acc);
	ap->flags = 0;
	return NOTOK;
}
	...
	Refid refid;
{
	...
	case RID_STRING:
		...
		rid->un.referenceClock = str2qb(refid.rid_string, strlen(refid.rid_string), 1);
		break;
	case RID_INET:
	...
	struct RoSAPindication *roi;
{
	...
	if (RyDsResult(sd, rox->rox_id, (caddr_t) clbase, ROS_NOPRIO, roi) == NOTOK)

...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/others/ntp/RCS/ntpd.c,v 9.0 1992/06/16 14:45:01 isode Rel";
#endif	/* lint */
	...
	char *argv[];
{
	...
	initialize();		/* call NTP protocol initialization first, then allow others to
	...
	while ((cc = getopt(argc, argv, "a:c:dD:lstnp:")) != EOF) {
		...
		case 't':
			...
			dotickadj++;
#else
			adios(NULLCP, "%s: not compiled to set tickadj", myname);
#endif
			break;
		...
		case 'n':
			...
			noswap = 1;
#else
			adios(NULLCP, "%s: not compiled for noswap", myname);
#endif
			break;
...
doit()
{
	...
	for (i = 0; i < nintf; i++) {
		...
		if (addrs[i].if_flags & IFF_BROADCAST)
			TRACE(2, ("Addr %d: %s fd %d %s broadcast %s",
				  ...
				  paddr(&addrs[i].addr), ntoa(&addrs[i].bcast)));
		else
			TRACE(2, ("Addr %d: %s fd %d %s", i,
				  addrs[i].name, addrs[i].fd, paddr(&addrs[i].addr)));
	}
	...
	for (;;) {		/* go into a finite but hopefully very long loop */
		...
		(void) TNetAcceptAux(&vecp, vec, &newfd, NULLTA,
				     selfds, &readfds, &writefds, NULLFD, (1 << CLOCK_ADJ), td);
		...
		for (i = 0; i < nintf; i++) {
			...
			if (!FD_ISSET(addrs[i].fd, &readfds) && !FD_ISSET(addrs[i].fd, &writefds))
				continue;
			TRACE(3, ("Activity on if %d fd=%d (%s)", i, addrs[i].fd, addrs[i].name));
			if (addrs[i].flags & INTF_PENDING) {
				struct ntp_peer *p = find_peer(i);

				if (p)
					(void) make_osi_conn(find_peer(i), osiaddress);
				continue;
			}
			...
			default:
				TRACE(1, ("Address family %d not supported", addrs[i].addr.type));
				continue;
			}
		}
		if (ticked)
	...
	int sock;
{
	...
	while (peer != NULL) {
		if (addr_compare(&peer->src, dst) && ((peer->sock == sock) || (peer->sock == -1)))
			return peer;
		peer = peer->next;
	}
	...
	struct ntp_peer *peer;
{
...
#ifdef	DEBUG
	if ((peer->next == NULL && peer->prev == NULL) || l->tail == NULL || l->head == NULL)
		abort();
#endif
	...
	if ((l->members < 0) ||
	    (l->members && l->tail == NULL) || (l->members == 0 && l->tail != NULL)) {
	...
	if (clock_watchdog >= NTP_MAXAGE) {
		...
		if (clock_watchdog < NTP_MAXAGE + n) {
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "logical clock adjust timeout (%d seconds)", NTP_MAXAGE);
		}
	}
#ifdef	DEBUG
	if (debug)
	...
	struct ntp_peer *peer;
{
	...
	if (peer->flags & PEER_FL_CONREQ) {
		...
		default:
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "Bad state flags %d", peer->flags & PEER_FL_CONNSTATE);
			return;
		}
	}
	transmit(peer);
...
#define MAXARGS	10
	...
	if ((fp = fopen(driftcomp_file, "r")) != NULL) {
		if (fscanf(fp, "%lf", &j) == 1 && j > -1.0 && j < 1.0) {
			...
			advise(LLOG_EXCEPTIONS, NULLCP, "init_ntp: bad drift compensation value");
		}
		(void) fclose(fp);
	}
}
	...
	int argc;
{
	...
	case TBL_STRATUM:
		advise(LLOG_NOTICE, NULLCP, "Obsolete command 'stratum'");
		break;
	...
	case TBL_BROADCAST:
		...
		if ((addrs[i].if_flags & IFF_BROADCAST) == 0) {
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "config: %s doesn't support broadcast", argv[1]);
			return NOTOK;
		}
		if (peer = check_peer(&addrs[i].bcast, -1)) {
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "config file: duplicate broadcast for %s", argv[1]);
			return NOTOK;
		}
	...
	case TBL_SERVER:
		...
		if (GetHostName(argv[1], &addr) == NOTOK) {
			advise(LLOG_EXCEPTIONS, NULLCP, "%s: unknown host", argv[1]);
			return NOTOK;
		}
		for (i = 0; i < nintf; i++)
		...
		if (peer != NULL) {
			advise(LLOG_NOTICE, NULLCP,
			       "Duplicate peer %s in in config file", paddr(&addr));
			return NOTOK;
		}
		...
		TRACE(2, ("Peer %s mode %d", paddr(&peer->src), peer->hmode));
		break;
	...
	case TBL_REFCLOCK:
		if (argc < 6)
			...
			make_new_peer(peer);
			(void) strncpy(peer->refid.rid_string, ref_clock, 4);
			...
			TRACE(2, ("Refclock %s mode %d refid %.4s stratum %d precision %d",
				  argv[1], peer->hmode, peer->refid.rid_string, stratum, prec));
	...
	while (argc > 0) {
		...
		case TBLPEER_VERSION:
			...
			if (peer->vers < 1 || peer->vers > 2)
				adios(NULLCP, "Bad version %d", peer->vers);
			break;
...
init_kern_vars()
{
	...
	for (i = 0; i < (sizeof(kern_vars) / sizeof(kern_vars[0])); i++) {
		...
		if ((where = nl[i].n_value) == 0) {
			advise(LLOG_EXCEPTIONS, NULLCP, "Unknown kernal var %s", nl[i].n_name);
			continue;
		}
		if (lseek(kmem, where, L_SET) == -1) {
			advise(LLOG_EXCEPTIONS, "", "lseek for %s fails", nl[i].n_name);
			continue;
		}
		if (read(kmem, (char *) kern_vars[i], sizeof(int)) != sizeof(int)) {
			advise(LLOG_EXCEPTIONS, "", "read for %s fails", nl[i].n_name);
	...
	TRACE(1, ("desired tickadj = %d, dotickadj = %d", tickadj, dotickadj));
	...
	if (sys.precision == 0) {
		if (kern_hz <= 64)
			...
			sys.precision = -11;
		advise(LLOG_NOTICE, NULLCP,
		       "sys.precision set to %d from sys clock of %d HZ", sys.precision, kern_hz);
	}
}
	...
	struct Naddr *addr;
{
	...
	if (addr->type == AF_INET && (hp = gethostbyname(name)) != NULL) {
		if (hp->h_addrtype != AF_INET)
			return NOTOK;
		bcopy((char *) hp->h_addr, (char *) &addr->inet_ad.sin_addr, hp->h_length);
...
hourly()
{
	...
	(void) sprintf(buf, "stats: dc %f comp %f peersw %d inh %d",
		       drift_comp, compliance, peer_switches, peer_sw_inhibited);
	...
	if (sys.peer == NULL) {
		...
		(void) sprintf(buf + p, " off %f SYNC %.4s %d",
			       sys.peer->estoffset, sys.peer->refid.rid_string, sys.peer->stratum);
#endif
	} else {
		p = strlen(buf);
		(void) sprintf(buf + p, " off %f SYNC %s %d",
			       sys.peer->estoffset, paddr(&sys.peer->src), sys.peer->stratum);
	}
	...
	char *name, *type;
{
	...
	if (strcmp(type, "local") == 0) {
		...
		advise(LLOG_EXCEPTIONS, NULLCP, "Unknown reference clock type (%s)", type);
		return (-1);
	}
	if (cfd >= 0) {
...
envinit()
{
	...
	advise(LLOG_NOTICE, NULLCP,
	       "%s starting: version $Revision: 9.0 $ patchlevel %d", myname, PATCHLEVEL);
	...
	struct Naddr *pa1, *pa2;
{
	...
	switch (pa1->type) {
	case AF_INET:
		if (pa1->inet_ad.sin_addr.s_addr != pa2->inet_ad.sin_addr.s_addr)
			return 0;

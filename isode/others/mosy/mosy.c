	...
	char **argv, **envp;
{
	...
	for (argc--, argv++; argc > 0; argc--, argv++) {
		...
		if (sysin) {
		      usage:;
			fprintf(stderr, "usage: mosy [-d] [-o module.defs] [-s] module.my\n");
			exit(1);
		}
	...
	(void) sprintf(autogen, "%*.*s", cp - mosyversion, cp - mosyversion, mosyversion);
...
pass1_obj(mod, id, syntax, value, aname, sname, descr, refer, idx, defval)
	char *mod, *id, *aname, *sname, *descr, *refer;
	YP syntax;
	YV value, idx, defval;
{
	register SY sy;
	...
	char *id;
{
	...
	case YP_SEQTYPE:
		if ((yz = yp->yp_type)->yp_code != YP_IDEFINED
		    || (yz = lookup_type(yz->yp_module,
					 yz->yp_identifier)) == NULL || yz->yp_code != YP_SEQLIST)
			yyerror
	...
	case YP_IDEFINED:
		...
		if (strcmp(id, "Counter") == 0 && yo->yo_name[strlen(yo->yo_name) - 1] != 's')
	...
	if ((yv = yo->yo_value->yv_idlist)
	    ...
	    && yv->yv_next && yv->yv_next->yv_code == YV_NUMBER && !yv->yv_next->yv_next) {
		OT ot;
	...
	char *id;
{
	...
	if ((yv = yt->yt_enterprise->yv_idlist)
	    && yv->yv_code == YV_IDEFINED
	    && !yv->yv_next && !lookup_object(NULLCP, yv->yv_identifier)
	    && !lookup_identifier(NULLCP, yv->yv_identifier)
	    && !importedP(yv->yv_identifier))
		myyerror("value in ENTERPRISE clause is undefined: %s", yv->yv_identifier);
      done_enterprise:;
...
static
check_objects(yv, clause, typesOK)
	register YV yv;
	char *clause;
	int typesOK;
{
	...
	for (yv = yv->yv_idlist; yv; yv = yv->yv_next) {
		...
		case YV_IDEFINED:
			...
			if (lookup_object(yv->yv_module, yv->yv_identifier)
			    == NULL && !importedP(yv->yv_identifier))
	...
	register int level;
{
	...
	case YP_IDEFINED:
		fprintf(stderr, "%*smodule=\"%s\" identifier=\"%s\"\n",
			level * 4, "", yp->yp_module ? yp->yp_module : "", yp->yp_identifier);
		break;

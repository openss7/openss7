...
int
udp_open(tb, local, remote, option, td)
		...
		if (local && local->na_domain[0]) {
			...
			if ((remote && remote->na_domain[0]) &&
				...
				tusaplose(td, DR_ADDRESS, NULLCP,
					  TuErrString(UDERR_ADDR_FAMILY_MISMATCH));
	...
	if (remote && remote->na_domain[0]) {
		...
		if (remote->na_port == 0) {
			if ((sp = getservbyname("tsap", "tcp")) == NULL)
				tusaplose(td, DR_ADDRESS, NULLCP,
					  TuErrString(UDERR_TSAP_SERVICE_UNKNOWN));
 ...
 */
	...
	if (peers == NULL) {
		...
		peers = (struct udpconn *) calloc((unsigned) maxpeers, sizeof(struct udpconn));
 ...
 */
	...
	if (sock->sin_port != 0) {
		...
		printf("\n          addr = %d %d %d %d", sock->sin_addr.s_net,
		       sock->sin_addr.s_host, sock->sin_addr.s_lh, sock->sin_addr.s_impno);
#endif

		if ((sd = socket(SOCK_DGRAM,
				 (struct sockproto *) 0,
				 (struct sockaddr *) sock, SO_LARGE)) != NOTOK)
			/* 
...
int
udp_read_socket(fd, q, secs, fromsock, td)
...
int
udp_write_socket(fd, data, cc, td)
	...
	if (rfds) {
		...
		for (vp = (up = peers) + maxpeers, fd = 0; up < vp; up++, fd++)
			if (up->udp_parent != NOTOK &&
			    FD_ISSET(fd, &jfds) && up->udp_queue.qb_forw != &up->udp_queue) {
				/* 
			...
			for (wp = (vp = peers) + maxpeers; vp < wp; vp++)
				if (vp != up
				    ...
				    && bcmp((char *) &vp->udp_peer, (char *) sock,
					    sizeof *sock) == 0)
					break;
			if (vp >= wp && (vp = &peers[up->udp_parent])->udp_peer.sin_family != 0) {
				free((char *) qb);
				continue;
			}
#ifdef HULADEBUG

...

{
	...
	if ((tb = newtublk()) == NULL)
		return tusaplose(td, DR_CONGEST, NULLCP, TuErrString(UDERR_NO_MEMORY));
	...
	for (; n >= 0; na++, n--) {
		...
		case NA_TCP:
			...
			if ((result = (*tb->tb_UnitDataStart) (tb, na, NULL, TUNITDATA_START, td)
			    ) != OK) {
				continue;
			}
			break;
		...
		default:
			return tusaplose(td,
					 DR_PARAMETER,
					 NULLCP, TuErrString(UDERR_NSAP_NOT_SUPPORTED));
		}
...

{
	...
	if (sd < 0) {
		...
		if (called == NULLTA)
			return tusaplose(td,
					 DR_PARAMETER, NULLCP, TuErrString(UDERR_NO_REMOTE_ADDR));
	...
	if (n = (called->ta_naddr - 1) < 0) {
		...
		if (n = (calling->ta_naddr - 1) < 0) {
			freetublk(tb);
			return tusaplose(td,
					 DR_PARAMETER, NULLCP, TuErrString(UDERR_NO_NSAP_ADDRS));
		} else {
			/* 
	...
	for (na = called->ta_addrs; n >= 0; na++, n--) {
		...
		case NA_TCP:
			...
			if ((result = (*tb->tb_UnitDataStart) (tb, la, na, option), td) != OK) {
				continue;
			}
			break;
		...
		default:
			return tusaplose(td,
					 DR_ADDRESS, NULLCP, TuErrString(UDERR_NSAP_NOT_SUPPORTED));
		}
...

{
	...
	if (vecp < 3)
		return tusaplose(td, DR_PARAMETER, NULLCP, TuErrString(UDERR_BAD_INIT_VECTOR));
	...
	if (sd < 0) {
		...
		case NT_UDP:
			...
			if ((result = (*tb->tb_UnitDataStart) (tb,
							       NULL,
							       NULL, TUNITDATA_START), td) != OK)
#endif
		default:
				(void) tusaplose(td, DR_ADDRESS, NULLCP,
						 "unknown network type: 0x%x (%c)", *vec[0],
						 *vec[0]);
			freetblk(tb);
			return NOTOK;
		}
...
int
TUnitDataRequest(calling, called, qos, uv, td)
 ...
 */
	...
	if (n <= 0)
		return tusaplose(td, DR_PARAMETER, NULLCP, TuErrString(UDERR_ILLEGAL_UD_SIZE));
 ...
 */
	...
	hp = gethostbyaddr((char *) &socket.sin_addr, sizeof socket.sin_addr, socket.sin_family);
	(void) strcpy(na->na_domain, hp ? hp->h_name : inet_ntoa(socket.sin_addr));
...
int
TSetUnitDataIndications(sd, data)
 ...
 */
	...
	if ((tb = findtublk(sd)) == NULL) {
		(void) sigiomask(smask);
		return tusaplose(td, DR_PARAMETER, NULLCP, TuErrString(UDERR_INVALID_XPORT));
	}
	...
	for (;;) {
		...
		for (tb = THead->tb_forw; tb != THead; tb = tb2) {
			...
			if (FD_ISSET(sd = tb->tb_fd, &ifds)) {
				...
				case NOTOK:
					return tusaplose(td, DR_OPERATION, NULLCP,
							 "Unit Read failed");
					break;

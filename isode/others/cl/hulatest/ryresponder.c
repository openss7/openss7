...
ryresponder(argc, argv, host, myservice, mycontext, mypci, dispatches, ops, start, stop)
	int argc;
	char **argv, *host, *myservice, *mycontext, *mypci;
	...
	char *event;
{
	...
	advise(NULLCP, LOG_INFO, "%s: %s (source %d)", event, buffer, aca->aca_source);
}

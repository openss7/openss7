	...
	struct type_PS_User__data *info;
{
	...
	*data = NULLPE;
	...
	switch (info->offset) {
	default:
		return pusaplose(pi, PC_INVALID, NULLCP, "neither Simply- nor Fully-encoded-data");
		...
		for (full = info->un.complex; full; full = full->next) {
			...
			case choice_PS_0_single__ASN1__type:
				if (i++ >= NPDATA)
					return pusaplose(pi, PC_CONGEST, NULLCP,
							 "too much user information");
				pe = pdv->presentation__data__values->un.single__ASN1__type;
				pdv->presentation__data__values->un.single__ASN1__type = NULLPE;
				(*data++ = pe)->pe_context = ctx;
				break;

			case choice_PS_0_octet__aligned:
				qb = pdv->presentation__data__values->un.octet__aligned;
	...
	struct PSAPindication *pi;
{
	...
	if ((qb = qp) == NULL) {
		if ((qb = (struct qbuf *) malloc((unsigned) sizeof *qb + (len = ps_get_abs(pe))))
		    == NULL) {
		      no_mem:;
	...
	if ((ps = ps_alloc(str_open)) == NULLPS)
		goto no_mem;
	if (str_setup(ps, qb->qb_data, qb->qb_len, 1) == NOTOK || pe2ps_aux(ps, pe, 0) == NOTOK) {
	...
	int ndata, ppdu;
{
	...
	register struct type_PS_Fully__encoded__data **complex, *full;
...
/*  there is a context for the AUDT pdu */
	...
	for (d = data, i = 0; i < ndata; i++) {
		...
		default:
			for (j = 0, qp = pb->pb_contexts; j < pb->pb_ncontext; j++, qp++)
		...
		if (pdu->offset == type_PS_User__data_simple) {
			...
			*complex = full;
			...
			if ((full->PDV__list->presentation__data__values = (struct choice_PS_0 *)
	...
	register struct SSAPabort *sa;
{
	...
	if ((psap2level & ISODELOG_EXCEPTIONS)
	    && event && SC_FATAL(sa->sa_reason))
		xsprintf(NULLCP, NULLCP,
			 sa->sa_cc > 0 ? "%s: %s\n\t%*.*s" : "%s: %s", event,
			 SuErrString(sa->sa_reason), sa->sa_cc, sa->sa_cc, sa->sa_data);
	...
	default:
		...
		(void) sprintf(cp = buffer, " (%s at session)", SuErrString(sa->sa_reason));
		break;
	}
	...
	if (sa->sa_cc > 0)
		...
		return pusaplose(pi, reason, NULLCP, "%s", *cp ? cp + 1 : cp);
}
	...
	register struct psapblk *pb;
{
	...
	for (qp = pb->pb_contexts, i = pb->pb_ncontext - 1; i >= 0; qp++, i--) {
		if (qp->pc_asn)
			oid_free(qp->pc_asn);
		if (qp->pc_atn)
			oid_free(qp->pc_atn);

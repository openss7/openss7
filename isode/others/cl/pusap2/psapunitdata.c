...
/*---------------------------------------------------------------------------*/
int
PUnitDataRequest(calling, called, ctxlist, data, ndata, qos, pi)
/*---------------------------------------------------------------------------*/
...
/* let session provider catch errors in session parameters: qos  */
	...
	if (called->pa_selectlen > 0) {
		...
		if (calling->pa_selectlen == 0) {
			calling->pa_port = htons((u_short) (0x8000 | (getpid() & 0x7fff)));
...
/*  Start building ppdu */
	...
	if (calling
	    ...
	    && (pdu->calling = str2qb(calling->pa_selector, calling->pa_selectlen, 1))
	    == NULL)
		goto no_mem;
	if (called->pa_selectlen > 0
	    && (pdu->called = str2qb(called->pa_selector, called->pa_selectlen, 1))
	    == NULL)
		goto no_mem;
...
/*  following validation of ASN.1/BER for the default context */
	...
	if (data && ndata > 0
	    && (pdu->user__data = info2_ppdu(pb, pi, data, ndata, PPDU_UD)) == NULL)
		goto no_good;

	pe = NULLPE;
	if (encode_PS_UD__type(&pe, 1, 0, NULLCP, pdu) == NOTOK) {
		(void) pusaplose(pi, PC_CONGEST, NULLCP, "error encoding PDU: %s", PY_pepy);
		goto no_good;
	}
...
/* let session provider catch errors in session parameters: qos  */
	...
	if (sd == NOTOK) {
		...
		(void) sigiomask(smask);
		return pusaplose(pi, PC_PARAMETER, NULLCP, "invalid presentation descriptor");
	}
...
/*  following validation of ASN.1/BER for the default context */
	...
	if ((result = SUnitDataBind(pb->pb_fd, &calling->pa_addr,
				    called ? &called->pa_addr : NULLSA, qos, si)) == NOTOK) {
	...
	struct PSAPindication *pi;
{
	...
	if (SUnitDataBind(pb->pb_fd, &calling->pa_addr, &called->pa_addr, NULL, si) == NOTOK) {
		(void) ss2pulose(NULLPB, pi, "SUnitDataBind", sa);
		freepublk(pb);
		(void) sigiomask(smask);
		return NOTOK;
	...
	struct PSAPindication *pi;
{
	...
	if (ndata <= 0)
		return pusaplose(pi, PC_PARAMETER, NULLCP, "illegal number of PSDUs (%d)", ndata);
	missingP(pi);
	...
	struct PSAPindication *pi;
{
	...
	if (decode_PS_UD__type(pe, 1, NULLIP, NULLVP, &pdu) == NOTOK) {
		(void) pusaplose(pi, PC_UNRECOGNIZED, NULLCP, "error decoding PDU: %s", PY_pepy);
		goto no_good;
	}
#ifdef	DEBUG
	if (psap2level & ISODELOG_PDUS)
...
/*  build PuSAPstart structure */
	...
	if (pdu->calling) {
		if ((base = qb2str(pdu->calling)) == NULLCP)
			goto no_mem;
		if ((len = pdu->calling->qb_len) > sizeof ps->ps_calling.pa_selector)
	...
	if (pdu->called) {
		if ((base = qb2str(pdu->called)) == NULLCP)
			goto no_mem;
		if ((len = pdu->called->qb_len) > sizeof ps->ps_called.pa_selector)
	...
	if (pdu2contexts(pb, pdu->context__list, &ps->ps_ctxlist) == NOTOK) {
		(void) pusaplose(pi, PC_TRANSFER, NULLCP, "transfer syntax not supported");
		goto no_good;
	}
	...
	struct PSAPindication *pi;
{
	...
	i = nctx - 1;
	for (pp = contexts, qp = pb->pb_contexts; i >= 0; i--, pp++, qp++) {
		...
		if (pp->pc_asn == NULLOID) {
			(void) pusaplose(pi, PC_PARAMETER, NULLCP,
					 "no abstract syntax name given for context %d", pp->pc_id);
			goto out2;
		}
		...
		if ((qp->pc_atn = oid_cpy(pp->pc_atn ? pp->pc_atn : pb->pb_atn)) == NULLOID)
			goto no_mem;
	...
	struct PSAPctxlist *ctxlist;
{
	...
	result = OK;
	i = 0;
	for (lp = ctxdeflist, pp = ctxlist->pc_ctx; lp; lp = lp->next, pp++, i++) {

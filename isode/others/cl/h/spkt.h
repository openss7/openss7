...
#ifndef __CL_H_SPKT_H__
#define __CL_H_SPKT_H__
...
#ifdef HULA
#define SB_CLNS		0x200000	/* Connectionless Service */
#endif

	struct ssapkt *sb_retry;	/* initial spkt */

	long sb_V_A;			/* lowest unconfirmed ssn */
	long sb_V_M;			/* next ssn */
	long sb_V_R;			/* lowest ssn for resynchronization */
	int sb_rs;			/* resynchronization type (an SYNC_xxx code, plus...) */
...
#define	SYNC_DISC	4		/* Activity Discard */
	...
	struct ssapkt *sb_xspdu;	/* saved expedited SPDU (really should be a linked list!) */
...
#ifdef HULA
#define SPDU_UD		0x40		/* UNITDATA TRANSFER, code = 64 */
#endif
...
#define	CR_OPT_EXTD	0x01		/* will receive extended concatenated SPDUs, this
					   implementation DOESN'T; segmenting is enough... */
...
#define	CN_SIZE		512
#define	CONNECT_MAX	10240		/* someday support CDO/OA SPDUs and Data Overflow PI... */
...
#define	AB_DISC_PROTO	0x03		/* protocol error */
#define	AB_DISC_UNKNOWN	0x04		/* no reason */
...
#define EX_BASE_SIZE	0
		...
		/* TYPED DATA SPDU */
#define	TD_SIGHS	65535		/* should be TD_SIZE, but <tsap.h> got there first */
...
#define	SMASK_SPDU_UD	0x0001

#define	SMASK_UD_VERSION 0x0002
			u_char un_ud_version;
#define	SMASK_UD_CALLING 0x0004
			char un_ud_calling[SSSIZE];
			int un_ud_callinglen;
#define	SMASK_UD_CALLED	0x0008
			char un_ud_called[SSSIZE];
			int un_ud_calledlen;
		} un_ud;
#define	UD_MAX_DATA     ((65*1024) - 40)
#define UD_BASE_SIZE	3
...
#define s_ud_version	s_un.un_ud.un_ud_version
#define	s_ud_calling	s_un.un_ud.un_ud_calling
#define	s_ud_callinglen	s_un.un_ud.un_ud_callinglen
#define	s_ud_called	s_un.un_ud.un_ud_called
#define	s_ud_calledlen	s_un.un_ud.un_ud_calledlen
...
#define	ENCLOSE_MAX	65400		/* maximum size of enclosure per segment less slop; slop
					   varies based on SPDU, but we'll always assume the worst
					   case */
#define	SEGMENT_MAX	65528		/* for things other than S-DATA and S-TYPED-DATA under
					   version 2 we allow only ONE enclosure */

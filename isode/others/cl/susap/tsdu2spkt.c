...
static int si_table[] = {
	...
	PMASK_VERSION		/* 0x40: SPDU_UD */
	    | PMASK_SSAP_CALLING | PMASK_SSAP_CALLED | PMASK_UDATA
#endif
};
...
/* NB: caller responsible for mapping s -> s_udata to user info */
	...
	case SPDU_UD:
		...
		If_Set(SMASK_UD_CALLING)
		    put2spdu(PI_SSAP_CALLING, s->s_ud_callinglen, s->s_ud_calling, &c);
		If_Set(SMASK_UD_CALLED)
		    put2spdu(PI_SSAP_CALLED, s->s_ud_calledlen, s->s_ud_called, &c);
/* NB: caller responsible for mapping s -> s_udata to user info */
		break;

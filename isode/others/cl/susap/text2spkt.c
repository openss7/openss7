	...
	int read;
{
	...
	fprintf(fp, "%s(: dump of SPDU 0x%x, errno=0x%x mask=0x%x%s\n",
		rw, s, s->s_errno, s->s_mask, s->s_mask & SMASK_SPDU_EXPD ? " (expedited)" : "");
	...
	case SPDU_AC:
		...
		if (s->s_mask & SMASK_CN_OPT)
			type_bits(fp, rw, "OPTIONS", s->s_options, CR_OPT_MASK, OMASK);
		...
		if (s->s_mask & SMASK_CN_REQ)
			type_bits(fp, rw, "REQUIREMENTS", s->s_cn_require, -1, RMASK);
	...
	case SPDU_UD:
		...
		if (s->s_mask & SMASK_UD_CALLING)
			type_id(fp, "CALLING", rw, s->s_ud_calling, s->s_ud_callinglen);
		if (s->s_mask & SMASK_UD_CALLED)
			type_id(fp, "CALLED", rw, s->s_ud_called, s->s_ud_calledlen);
		break;
#endif
	...
	case SPDU_RF:
		...
		if (s->s_mask & SMASK_RF_DISC)
			type_bits(fp, rw, "DISCONNECT", s->s_rf_disconnect, RF_DISC_MASK, DMASK);
		if (s->s_mask & SMASK_RF_REQ)
			type_bits(fp, rw, "REQUIREMENTS", s->s_rf_require, -1, RMASK);
		if (s->s_mask & SMASK_RF_VRSN)
			type_vrsn(fp, rw, s->s_rf_version);
		if (s->s_rlen > 0) {
			type_reason(fp, rw, *s->s_rdata & 0xff);
			if (s->s_rlen > 1)
				type_data(fp, "REASON", rw, s->s_rlen - 1, s->s_rdata + 1);
		}
		break;
	...
	case SPDU_FN:
		...
		if (s->s_mask & SMASK_FN_DISC)
			type_bits(fp, rw, "DISCONNECT", s->s_fn_disconnect, FN_DISC_MASK, DMASK);
		break;
	...
	case SPDU_MIP:
		...
		if (s->s_mask & SMASK_MIP_SYNC)
			type_bits(fp, rw, "SYNC", s->s_mip_sync, MIP_SYNC_MASK, YMASK);
	...
	case SPDU_AE:		/* aka SPDU_MAP */
		...
		if ((s->s_mask & SMASK_MAP_SYNC)
			...
			type_bits(fp, rw, "SYNC", s->s_map_sync, MAP_SYNC_MASK, SMASK);
		} else
	...
	case SPDU_AS:
		...
		if (s->s_mask & SMASK_AS_ID) {
			fprintf(fp, "%s", rw);
			type_info(fp, "ID/ %d", (int) s->s_as_id.sd_len, s->s_as_id.sd_data);
			fprintf(fp, "\n");
		}
		break;
	...
	case SPDU_AR:
		...
		if (s->s_mask & SMASK_AR_OID) {
			fprintf(fp, "%s", rw);
			type_info(fp, "OLD ID/ %d", (int) s->s_ar_oid.sd_len, s->s_ar_oid.sd_data);
			fprintf(fp, "\n");
		}
		...
		if (s->s_mask & SMASK_AR_ID) {
			fprintf(fp, "%s", rw);
			type_info(fp, "ID/ %d", (int) s->s_ar_id.sd_len, s->s_ar_id.sd_data);
			fprintf(fp, "\n");
		}
		break;
...
static
type_id(fp, type, rw, selector, len)
	FILE *fp;
	char *type, *rw;
	char *selector;
	int len;
	...
	struct SSAPref *ref;
{
	...
	if (ref->sr_vlen)
		type_info(fp, "<CALLING %d", (int) ref->sr_calling_len, ref->sr_calling);
	else
	...
	if (ref->sr_vlen)
		type_info(fp, ", CALLED %d", (int) ref->sr_called_len, ref->sr_called);
	fprintf(fp, ">\n");
}

static
type_vrsn(fp, rw, version)
	FILE *fp;
	char *rw;
	u_char version;
{
...
static
type_reason(fp, rw, reason)
	FILE *fp;
	char *rw;
	int reason;
{
	fprintf(fp, "%sREASON/ 0x%x: %s\n", rw, reason, SErrString((int) reason));
}

static
type_prepare(fp, rw, type)
	FILE *fp;
	char *rw;
	u_char type;
{

...
static struct TSAPaddr  tas[NTADDRS];
    ...
    initbindvars();
*/
	/* if ((sp = getservbyname ("tsap", "tcp")) == NULL) printf ("failed - \ntcp/tsap: unknown
	   service"); */
...
/*-------------------------------------*/
{
	if (mode == CLIENT_MODE) {
		printf("\n Client reading sum\n");
		if (AcUnitDataRead(csd, pcacs, 5, aci) == NOTOK)
			goto rfail;
/*
...
*/
		...
		if (AcUnitDataRead(ssd, psacs, 5, aci) == NOTOK)
			goto rfail;
/*
...
/* cheating til Kurt gets back 
    ...
    if ( AcUnitDataRebind (ssd, pclientaei, cpa, aci) == NOTOK )
		...
		if (AcUnitDataRebind(ssd, &clientaei, &psacs->acs_start.ps_calling, aci) == NOTOK)
		{

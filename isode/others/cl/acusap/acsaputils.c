	...
	int *ndata;
{
	...
	for (i = 0; info; info = info->next, i++) {
		if (i > NACDATA)
			return acusaplose(aci, ACS_CONGEST, NULLCP, "too much user information");
	...
	register struct PSAPabort *pa;
{
	...
	if ((acsaplevel & ISODELOG_EXCEPTIONS) && event)
		xsprintf(NULLCP, NULLCP,
			 pa->pa_cc > 0 ? "%s: %s\n\t%*.*s" : "%s: %s", event,
			 PuErrString(pa->pa_reason), pa->pa_cc, pa->pa_cc, pa->pa_data);
	...
	default:
		(void) sprintf(cp = buffer, " (%s at presentation)", PuErrString(pa->pa_reason));
	case PC_SESSION:
	...
	if (pa->pa_cc > 0)
		...
		return acusaplose(aci, reason, NULLCP, "%s", *cp ? cp + 1 : cp);
}
...
/* VARARGS */
int
acusaplose(aci, reason, what, fmt)
	struct AcSAPindication *aci;
	int reason;
	char *what, *fmt;
{

...
int
AcUnitDataRequest(context, callingtitle, calledtitle,
		  callingaddr, calledaddr, ctxlist, data, ndata, qos, aci)
/*---------------------------------------------------------------------------*/
	OID context;
...
/*  Will change to allow simple encoding later */
	...
	if (data && ndata > 0 && (pdu->user__information = info2_apdu(acb, aci, data, ndata))
	    == NULL)
		goto no_good;

	if (encode_ACS_AUDT__apdu(&pe, 1, 0, NULLCP, pdu) == NOTOK) {
		(void) acusaplose(aci, ACS_CONGEST, NULLCP, "error encoding PDU: %s", PY_pepy);
		goto no_good;
	}
...
#ifdef	DEBUG
	if (acsaplevel & ISODELOG_PDUS)
		ACU_print(pe, "AUDT-apdu", 0);
#endif

	result = PUnitDataRequest(callingaddr, calledaddr, ctxlist, &pe, 1, qos, pi);
...
/*---------------------------------------------------------------------------*/
int
AcUnitDataBind(sd, binding, context, callingtitle, calledtitle,
...
/*  necessary to unhook the AUDT PE from user space. */
...

	{
		...
		result = PUnitDataBind(acb->acb_fd, callingaddr, calledaddr, ctxlist, qos, pi);
	...
	struct AcSAPindication *aci;
{
	...
	if ((acb = findacublk(sd)) == NULL || !(acb->acb_flags & ACB_AUDT)) {
		acusaplose(aci, ACS_PARAMETER, NULLCP, "invalid association descriptor");
		goto no_good;
	}
	...
	struct AcSAPindication *aci;
{
	...
	if ((acb = findacublk(sd)) == NULL || !(acb->acb_flags & ACB_AUDT)) {
		(void) sigiomask(smask);
		return acusaplose(aci, ACS_PARAMETER, NULLCP, "invalid association descriptor");
	}
	...
	struct AcSAPindication *aci;
{
	...
	pdu = NULL;
	if (ps->ps_ninfo < 1) {
		(void) acusaplose(aci, ACS_PROTOCOL, NULLCP, "no user-data on P-UNIT-DATA");
		goto no_good;
	}
...
/************/
	...
	if (apdu2_info(acb, aci, pdu->user__information, acs->acs_info, &acs->acs_ninfo) == NOTOK)
		goto no_good;
...
/*  where AUDT PCI = ACSE PCI since HULA put AUDT in with ACSE */
	...
	if (acb->acb_id == PE_DFLT_CTX) {
		if (i == 0)
			return OK;
		if (i > 1)
			return acusaplose(aci, ACS_PROTOCOL, NULLCP, "contexts not supported");
	}
...
/*  must search through the context list on the indication to get direct ref */
	...
	if (i <= 0)
		return acusaplose(aci, ACS_PROTOCOL, NULLCP, "wrong ASN for ACSE");
	if (pp->pc_result != PC_ACCEPT)
		return acusaplose(aci, ACS_PROTOCOL, NULLCP, "PCI for ACSE not accepted");
	...
	struct AcuSAPstart *acs;
{
	...
	acs->acs_callingtitle.aei_ae_qualifier = pdu->calling__AE__qualifier;
	pdu->calling__AE__qualifier = NULLPE;
	if (pdu->calling__AP__invocation__id) {
		acs->acs_callingtitle.aei_ap_id = pdu->calling__AP__invocation__id->parm;
		acs->acs_callingtitle.aei_flags |= AEI_AP_ID;
	}
	if (pdu->calling__AE__invocation__id) {
		acs->acs_callingtitle.aei_ae_id = pdu->calling__AE__invocation__id->parm;
	...
	acs->acs_calledtitle.aei_ae_qualifier = pdu->called__AE__qualifier;
	pdu->called__AE__qualifier = NULLPE;

	if (pdu->called__AP__invocation__id) {
		acs->acs_calledtitle.aei_ap_id = pdu->called__AP__invocation__id->parm;
		acs->acs_calledtitle.aei_flags |= AEI_AP_ID;
	}
	if (pdu->called__AE__invocation__id) {
		acs->acs_calledtitle.aei_ae_id = pdu->called__AE__invocation__id->parm;

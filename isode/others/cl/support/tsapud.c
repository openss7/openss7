	...
	char **argv, **envp;
{
	...
	for (;;) {
		...
		default:
			(void) tusaplose(td, DR_CONGEST, "unitddata", "unable to fork ");
...
/* end UGLY */
	...
	advise(NULLCP, LOG_INFO,
	       ...
	       sel2str(tud->tud_called.ta_selector, tud->tud_called.ta_selectlen, 1));
	...
	char **vec;
{
	...
	for (vec++; ap = *vec; vec++) {
		if (*ap == '-')
			...
			case 't':
				udpservice = 1;
				tcpservice = x25service = consservice = bridgeservice = tp4service =
				    0;
				continue;
	...
	if (!rflag
	    && getuid() == 0 && stat(ap = isodefile("isoservices"), &st) != NOTOK && st.st_uid != 0)

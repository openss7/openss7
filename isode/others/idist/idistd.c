	...
	char **argv, **envp;
{
	...
	(void) ryresponder(argc, argv, PLocalHostName(), myservice, mycontext,
			   dispatches, table_Idist_Operations, initiate, NULLIFP);
...
{
	char *str;
	register struct type_Idist_InitDir *arg = (struct type_Idist_InitDir *) in;
	...
	if (exptilde(target, str) == NULL)
		return error(sd, error_Idist_badfilename, (caddr_t) ia5list, rox, roi);

	tp = target + strlen(target);

	if (RyDsResult(sd, rox->rox_id, (caddr_t) NULL, ROS_NOPRIO, roi) == NOTOK)
...
op_transfer(sd, ryo, rox, in, roi)
	...
	struct RoSAPindication *roi;
{
	register struct type_Idist_FileSpec *arg = (struct type_Idist_FileSpec *) in;
	...
	default:
		return error(sd, error_Idist_badfiletype, (caddr_t) NULL, rox, roi);
	}
...
op_data(sd, ryo, rox, in, roi)
	...
	struct RoSAPindication *roi;
{
	register struct type_Idist_Data *arg = (struct type_Idist_Data *) in;
	...
	for (qb = arg->qb_forw; qb != arg; qb = qb->qb_forw) {
		if (fwrite(qb->qb_data, sizeof(char), qb->qb_len, cfile) != qb->qb_len)
...
op_query(sd, ryo, rox, in, roi)
	...
	struct RoSAPindication *roi;
{
	register struct type_UNIV_IA5String *arg = (struct type_UNIV_IA5String *) in;
	...
	if (qr == NULL)
		return error(sd, error_Idist_congested, (caddr_t) ia5list, rox, roi);
...
op_terminate(sd, ryo, rox, in, roi)
	...
	struct RoSAPindication *roi;
{
	register struct type_Idist_TermStatus *arg = (struct type_Idist_TermStatus *) in;
	...
	case int_Idist_FileType_regular:
		if (cfile == NULL)
			return i_strerror(sd, error_Idist_protocol, "File not open", rox, roi);
		(void) fflush(cfile);
		if (ferror(cfile))
			return syserror(sd, error_Idist_writeerror, rox, roi);
		(void) fclose(cfile);
		if (fixup() < 0)
			return error(sd, error_Idist_fileproblem, (caddr_t) ia5list, rox, roi);
		break;
		...
		*tp = '\0';
		if (catname <= 0)
			return i_strerror(sd, error_Idist_protocol,
					  "Too many directory levels popped", rox, roi);
		tp = stp[--catname];
		*tp = '\0';
		break;
	...
	case int_Idist_FileType_hardlink:
		return i_strerror(sd, error_Idist_protocol,
				  "Bad file type for terminate operation", rox, roi);
	...
	PE *pe;
{
	...
	*pe = NULLPE;
	...
	if (decode_Idist_Initiate(acs->acs_info[0], 1, NULLIP, NULLVP, &initial) == NOTOK)
		return init_lose(ACS_PERMANENT, pe, "Can't parse initial data");
	...
	if (pw->pw_passwd == NULL || !chkpassword(user, pw->pw_passwd, cp)) {
...
#endif
		advise(LLOG_NOTICE, NULLCP, "Cant set userid %d for %s", userid, user);
...
static struct type_Idist_FileSpec *
cpy_fs(ft)

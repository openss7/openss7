	...
	struct type_Idist_FileSpec *fs;
{
	...
	if (symlink(old, linkname) < 0) {
		if (errno != ENOENT || chkparent(linkname) < 0 || symlink(old, linkname) < 0) {
	...
	if (bit_on(fs->fileopts, bit_Idist_Options_compare)) {
		...
		if ((i = readlink(target, tbuf, BUFSIZ)) >= 0 &&
		    i == fs->filesize && strncmp(old, tbuf, (int) fs->filesize) == 0) {
	...
	char *name;
{
	...
	*cp = '\0';
	if (lstat(name, &stb) < 0) {
		if (errno == ENOENT && chkparent(name) >= 0 && mkdir(name, 0777 & ~oumask) >= 0) {
			*cp = '/';
			return (0);
		}
	...
	struct type_Idist_FileSpec *fs;
{
	...
	if ((cfile = fopen(name, "w")) == NULL) {
		if (errno != ENOENT || chkparent(name) < 0 || (cfile = fopen(name, "w")) == NULL) {
	...
	struct type_Idist_FileSpec *fs;
{
	...
	if (lstat(target, &stb) == 0) {
		...
		if (mode != S_IFREG && mode != S_IFLNK) {
			nadvise(NULLCP, "%s: not a regular file", host, target);
			return NOTOK;
		}
		exists = 1;
	}
...
fixup()
{
	...
	tvp[1].tv_sec = tvp[0].tv_sec = convtime(cfiletype->filemtime);
...
{
	FILE *fp1, *fp2;
	char buf1[BUFSIZ], buf2[BUFSIZ];	/* these two had better be identical */
	int n1, n2;

	...
	char **argv, **envp;
{
	...
	for (vecp = 0; vecp < con_tbl_cnt; vecp++) {
		advise(LLOG_TRACE, NULLCP, "Listening on %s", taddr2str(&con_tbl[vecp].src));
	...
	struct TSAPaddr *ta;
{
	...
	advise(LLOG_NOTICE, NULLCP,
	       ...
	       taddr2str(&ts->ts_called), ts->ts_expedited, ts->ts_tsdusize);
	...
	advise(LLOG_TRACE, NULLCP, "Accepted from address %s", taddr2str(&ctp->src));
	...
	if ((ctp->flags & CONN_TRANS) == 0) {
		ts->ts_expedited = 0;
		if (ts->ts_cc > 0) {
			advise(LLOG_EXCEPTIONS, NULLCP, "%d octets initial user-data", ts->ts_cc);
	...
	advise(LLOG_NOTICE, NULLCP,
	       ...
	       taddr2str(fromta), taddr2str(tota), ts->ts_expedited, ts->ts_data, ts->ts_cc);

	if (TConnRequest(fromta, tota, ts->ts_expedited,
			 ts->ts_data, ts->ts_cc, &ts->ts_qos, tc, td) == NOTOK) {
		(void) TDiscRequest(sd, td->td_data, td->td_cc, td);
		ts_adios(td, "T-CONNECT.REQUEST");
	}
	if (TConnResponse(sd, NULLTA,
			  tc->tc_expedited, tc->tc_data, tc->tc_cc, &tc->tc_qos, td) == NOTOK) {
	...
	int s1, s2;
{
	...
	if (tx->tx_expedited) {
		...
		total = uiocnt = 0;
		for (qb = tx->tx_qbuf.qb_forw; qb != &tx->tx_qbuf; qb = qb->qb_forw) {
		...
		if (tx->tx_cc != total)
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "Mismatch in data %d != %d", tx->tx_cc, total);
	...
	if (result == NOTOK) {
		...
		ts_adios(td, tx->tx_expedited ? "T-EXPEDITED-DATA.REQUEST" : "T-DATA.REQUEST");
	}
}
	...
	if (ctp->flags & CONN_TRANS) {	/* make transparent address */
		*nta = ctp->dest;	/* struct copy */
		if (nta->ta_selectlen == 0) {
			...
			bcopy(ta->ta_selector, nta->ta_selector, ta->ta_selectlen);
		}
		return nta;
	}
...
*/
	if (m > 4 && ta->ta_selector[0] == ta->ta_selector[1] && n > 2 && n <= m - 2) {	/* encoded! 
		...
		if (nta->ta_selectlen > 0)
			bcopy(&ta->ta_selector[n + 2], nta->ta_selector, nta->ta_selectlen);
	...
	ContTbl *ctp;
{
	...
	if (!(ctp->flags & CONN_NOMUNGE) || (ctp->flags & CONN_FORCEMUNGE)) {
		if (isnew == 0) {
			...
			if ((p = _paddr2str(pa, NULLNA, -1)) == NULL ||
				...
				advise(LLOG_NOTICE, NULLCP, "new selector not encodable");
				nta->ta_selectlen = 0;
			} else
			...
			if ((nta->ta_selectlen = 2 + nna->na_addrlen + ta->ta_selectlen) >= TSSIZE)
				nta->ta_selectlen = 0;
			else {
				bcopy(nna->na_address, &nta->ta_selector[2], nna->na_addrlen);
				bcopy(ta->ta_selector,
				      &nta->ta_selector[2 + nna->na_addrlen], ta->ta_selectlen);
	...
	advise(LLOG_NOTICE, NULLCP, "not listening on this network (%d)", type);
	...
	struct TSAPaddr *ta;
{
	...
	for (ctp = con_tbl; ctp < &con_tbl[con_tbl_cnt]; ctp++) {
		if (na2->na_type == NA_NSAP && ctp->src.ta_naddr == 0 &&
		    ...
		    bcmp(ta->ta_selector, ctp->src.ta_selector, ta->ta_selectlen) == 0)
			return ctp;
	...
	char **vec;
{
	...
	if (con_tbl_cnt <= 0) {
		if ((ta = str2taddr(tsb_default_address)) == NULLTA)
			adios(NULLCP, "bad default address \"%s\"", tsb_default_address);
		con_tbl[0].src = *ta;	/* struct copy */
		con_tbl_cnt = 1;
	}
}
	...
	while (fgets(buf, sizeof buf, fp) != NULLCP) {
		...
		for (i = 1; i < vecp; i++) {
			...
			if (*ap == '-') {
				...
				if ((ta = str2taddr(ap)) == NULLTA)
					adios(NULLCP, "Bad address \"%s\" in file %s", ap, file);

	...
	char **argv, **envp;
{
	...
	advise(LLOG_NOTICE, NULLCP,
	       ...
	       ntohs(rts->rts_port), saddr2str(&rts->rts_initiator.rta_addr), rts->rts_data);
	...
	if (decode_RTF_Request(rts->rts_data, 1, NULLIP, NULLVP, &req)
	    == NOTOK) {
		advise(LLOG_EXCEPTIONS, NULLCP, "rejected -- error decoding request: %s", PY_pepy);
		goto reject;
	}
	...
	if (chdir(pw->pw_dir) == NOTOK) {
		advise(LLOG_EXCEPTIONS, pw->pw_dir, "unable to change directory to");
	      no_dice:;
	...
	if (guest && chroot(pw->pw_dir) == NOTOK) {
		advise(LLOG_EXCEPTIONS, pw->pw_dir, "unable to change root to");
		goto no_dice;
	}
	...
	for (;;) {
		...
		case RTI_FINISH:
			adios(NULLCP, "unexpected indication type=%d", rti->rti_type);
...
static int
downtrans(sd, base, len, size, ssn, ack, rti)
	int sd;
	char **base;
	int *len, size;
	long ssn, ack;
	struct RtSAPindication *rti;
{
	...
	if (bp == NULL) {
		...
		if (fstat(fd, &st) == NOTOK)
			return rtsaplose(rti, RTS_TRANSFER, destination, "unable to fstat");
#ifdef	MAXBSIZE
		...
		advise(LLOG_DEBUG, NULLCP,
		       "  based on blksize of %d and RTTR size of %d", bsize, size);
#endif
		bsize = n;
		timer(nbytes = 0);
	}
	...
	struct RtSAPindication *rti;
{
	...
	case SI_SYNC:
	{
		...
		advise(LLOG_DEBUG, NULLCP, "S-MINOR-SYNC.INDICATION: %ld", sn->sn_ssn);
#endif
	}
		break;
	...
	case SI_ACTIVITY:
	{
		...
		case SV_START:
#ifdef	DEBUG
			advise(LLOG_DEBUG, NULLCP, "S-ACTIVITY-START.INDICATION");
#endif
			if ((fd = creat(destination, 0666)) == NOTOK) {
				advise(LLOG_EXCEPTIONS, destination, "unable to create");
		...
		case SV_DISCIND:
			advise(LLOG_EXCEPTIONS, NULLCP,
			       ...
			       : "discarded", SReportString(sv->sv_reason));
			remove(destination);
			break;

		case SV_ENDIND:
#ifdef	DEBUG
			advise(LLOG_DEBUG, NULLCP, "S-ACTIVITY-END.INDICATION");
#endif
			if (close(fd) == NOTOK)
				return rtsaplose(rti, RTS_TRANSFER, destination, "close failed on");
			break;
	...
	case SI_REPORT:
	{
		...
		advise(LLOG_EXCEPTIONS, NULLCP, "exception: %s", SReportString(sp->sp_reason));
		remove(destination);
	}
		break;

	default:
		return rtsaplose(rti, RTS_TRANSFER, NULLCP, "unknown uptrans type=0x%x", type);
	}
	...
	char *file;
{
	...
	if (stat(file, &st) != NOTOK && (st.st_mode & S_IFMT) == S_IFREG && unlink(file) == NOTOK)

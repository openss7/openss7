...
/* create database directory */
	...
	else if (access(pp->p_val, F_OK) == 0)
...
/** INTERNAL OPTIONS -- for the use of wpp-manager */
		...
		else if (wildlife->p_val != (char *) NULL)	/* file already specified? */
			advise(NULLCP,
			       "wildlife directory already specified -- ignoring %s", av[i]);
		else
			wildlife->p_val = av[i];
...
/* open file containing names/identities of regional dsas */
	...
	lncnt = 0;
	for (lncnt = 1, i = 0;
	     i < N_REGIONS && fgets(line, BUFSIZ - 1, fp) != (char *) NULL; lncnt++) {
...
/* tokenize line */
		...
		case 1:	/* syntax error ... */
			advise(NULLCP, "syntax error, line %d: insufficient parameters", lncnt);
			continue;
		...
		default:
			advise(NULLCP, "syntax error, line %d: too many parameters", lncnt);
			continue;
		}		/* end, tokenize line */
...
/* run through each line of configuration file */
	while (fgets(line, BUFSIZ - 1, cfgfp) != (char *) NULL) {
		...
		default:
			advise(NULLCP, "syntax error: multiple values specified, line %d", lncnt);
			continue;
		}		/* end, tokenize line */
...
/* make sure character set used for value is legal */
		if ((pp->p_flags & P_MBOX) == P_MBOX) {
			for (i = 0; toks[1][i] != '\0'; ++i)
				if (!isascii(toks[1][i]) || toks[1][i] == '$' || toks[1][i] == '&') {
					advise(NULLCP,
...
/* make sure all mandatory values have been specified */
	for (i = 0; pairs[i].p_nm != (char *) NULL; ++i)
		if (pairs[i].p_val == (char *) NULL && (pairs[i].p_flags & P_OPT) != P_OPT)
...
/* initialize country value */
	...
	if (pp->p_val == (char *) NULL)
		pp->p_val = strdup("US");
	else if (strlen(pp->p_val) != 2 || strcmp(pp->p_val, "US") != 0)
...
/* check for legality of state value */
	...
	for (i = 0; usstates[i].s_nm != (char *) NULL; ++i)
		if (nmlen == strlen(usstates[i].s_nm) && strcmp(pp->p_val, usstates[i].s_nm) == 0)
			break;

	sp = (struct state *) NULL;
	if (usstates[i].s_nm == (char *) NULL)
		adios(NULLCP, "%s not a legal U.S. state or state equivalent", pp->p_val);
	else
...
/* initialize address, port, on which DSA listens if necessary */
	...
	if ((pp = findpair("port", 0x01)) != (struct pair *) NULL && pp->p_val == (char *) NULL) {
...
/* check to make sure postal address is legal ... */
		...
		for (cp2 = pp->p_val, i = 1; cp = strchr(cp2, (int) '$'); cp2 = cp + 1, ++i) {
			*cp = '\0';	/* null terminate */
	...
	unsigned char postdef;		/* postal address defined? */
{
	...
	if (!postdef)
		advise(NULLCP, "You must explicitly define the 'postaladdress' attribute\n\
...
/* set up location of organization */
	bzero(file, BUFSIZ);
	if ((pp = findpair("orgcode", 0x00)) != (struct pair *) NULL || chartered_by_congress)
...
/* get state, in order to find apppropriate regional DSAs */
	...
	for (i = 0; usstates[i].s_nm != (char *) NULL; ++i)
		if (stnmlen == strlen(pp->p_val) && strcmp(pp->p_val, usstates[i].s_nm) == 0)
			break;
	...
	if ((pp = findpair("owner", 0x00)) == (struct pair *) NULL)
		if ((pp = findpair("userid", 0x00)) == (struct pair *) NULL)
			if ((pw = getpwuid(geteuid())) == (struct passwd *) NULL)
				...
				advise(NULLCP,
				       "Warning: 'owner' and 'userid' not specified -- using %s",
				       pw->pw_name);
				owner = pw->pw_name;
		} else {
			if (getpwnam(pp->p_val) == (struct passwd *) NULL)
				adios(NULLCP, "Account '%s' is not legal -- cannot be owner",
				      pp->p_val);
			owner = pp->p_val;
	} else {
		if (getpwnam(pp->p_val) == (struct passwd *) NULL)
...
/* do chown */
	...
	sprintf(buf, "chmod 755 %s/startup.sh %s/nightly.sh", wildlife, wildlife);
	if (system(buf) != 0)
...
make_usstates()
{
	...
	for (i = 0; usstates[i].s_nm != (char *) NULL; ++i) {
		printf("st=%s\n", usstates[i].s_nm);
		printf
		    ("objectclass= top & locality & quipuObject & quipuNonLeafObject & nadfObject & usStateOrEquivalent\n");

	...
	char **argv, **envp;
{
...
#ifndef	SYS5
	(void) sprintf(buffer, "find %s -exec /etc/chown %d {} \\;", wildlife, uid);
#else
	...
	(void) sprintf(buffer, "find %s -exec chgrp %d {} \\;", wildlife, gid);
	if (debug)
		fprintf(stderr, "%s\n", buffer);
	(void) system(buffer);
...
read_config()
{
	int lineno;
	register char *cp, *dp;
	char buffer[BUFSIZ], file[BUFSIZ], line[BUFSIZ], *vec[NVEC + 1];
	FILE *fp;
	...
	n2p("rootDSA", 1)->p_value = strdup(c->c_root ? c->c_root : c->c_master);
	if (c->c_root)
	...
	if (n2p("postaladdress", 0) == NULL && n2p("street", 0) == NULL && n2p("pob", 0) == NULL)
		adios(NULLCP, "must specify either \"street\" or \"pob\"");

	if ((p = n2p("locality", 1))->p_value == NULL) {
		(void) sprintf(buffer, "%s, %s", n2p("town", 1)->p_value, n2p("state", 1)->p_value);
	...
	if ((p = n2p("middlename", 1))->p_value && (q = n2p("middleinitial", 1))->p_value == NULL) {
	...
	if (dp = (p = n2p("postaladdress", 1))->p_value) {
		...
		if (strlen(dp) > 30) {
		      too_long:;
			adios(NULLCP, "item %d is too long (30 characters maximum): \"%s\"", i, dp);
		}
		if (i > 6)
		...
		if ((i = strlen(cp)) > 30 + 3) {
			advise(NULLCP, "your organization name is longer than 30 characters!");
		      postal_problem:;
			adios(NULLCP,
...
");
		...
		if (q = n2p("pob", 0)) {
			...
			if ((i = strlen(cp)) > 30 + 3) {
				advise(NULLCP, "your street address is longer than 30 characters!");
				goto postal_problem;
			}
			cp += i;
		...
		(void) sprintf(cp, "%s, %s %s $ ",
			       n2p("town", 1)->p_value,
			       n2p("state", 1)->p_value, n2p("zipcode", 1)->p_value);
		if ((i = strlen(cp)) > 30 + 3) {
			advise(NULLCP, "your town/state/zipcode is longer than 30 characters!");
			goto postal_problem;
		}
		cp += i;
	...
	char *code;
{
	int vecp;
	register char *cp, *dp;
	char d, *ep, buffer[BUFSIZ + 1], file[BUFSIZ], *vec[NVEC + NSLACK + 1];
	FILE *fp;
	...
	while (fgets(dp, ep - dp, fp)) {
			...
			default:
				*cp++ = ' ';
				if ((dp = cp) + 1 >= ep) {
					*ep = NULL;
					adios(NULLCP,
					      "virtual line too long in %s: \"%s\"", file, cp);
				}
				continue;
			}
		}
		...
		case 8:
			...
			if (sscanf(vec[1], "%d", &c->c_number) != 1)
				adios(NULLCP,
				      "malformed ISO3166 number for country %s in %s", code, file);
			...
			if (vecp == 8) {
				...
				if (sscanf(cp, bp, &c->c_flags) != 1)
					adios(NULLCP,
					      "malformed entry for country %s in %s", code, file);
			}
			break;
	...
	char *dsa, **addr;
{
	int i;
	register char *cp, *dp;
	char buffer[BUFSIZ], stuff[BUFSIZ];
	FILE *fp;
	...
	int any;
{
	...
	for (p = pairs; p->p_name; p++)
		if (strcmp(p->p_name, name) == 0)
			return (any || (p->p_value && !(p->p_flags & P_ZAP)) ? p : NULL);
...
static char *c_TLC[] = {
	...
	"info= @(firstname) @(lastname) (@(extension)) <@(mailbox)>",
	"",
...
build_TLC()
{
	...
	(void) sprintf(buffer, "%s/c=%s", wildlife, n2p("country", 1)->p_value);
...
build_organization()
{
	...
	(void) sprintf(buffer, "%s/c=%s/o=%s", wildlife,
		       n2p("country", 1)->p_value, n2p("organization", 1)->p_value);
...
build_unit()
{
	...
	(void) sprintf(buffer, "%s/c=%s/o=%s/ou=%s", wildlife,
		       ...
		       n2p("organization", 1)->p_value, n2p("unit", 1)->p_value);
...
static
make_edb(dir, type, date, entries)
	char *dir, *type, *date;
	char *entries[];
{
	char edb[BUFSIZ];
...
static
make_file(infile, outfile, mode, dosed)
	char *infile, *outfile;
	int mode, dosed;
{
	char buffer[BUFSIZ];
...
*/
		...
		(void) sprintf(buffer, "cp templates/%s %s/%s", infile, wildlife, outfile);
...
fudge_file(name)
	char *name;
{
	char buffer[BUFSIZ], file[BUFSIZ], oldfil[BUFSIZ], tmpfil[BUFSIZ];

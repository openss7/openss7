...
static
fetch_face(host, user)
	char *host, *user;
{
	if ((myim = fetch_image(user, host)) == NULL && recording)
		LLOG(pgm_log, LLOG_NOTICE, ("no image for \"%s\" \"%s\"", user, host));
	...
	char **vec;
{
	...
	for (vec++; ap = *vec; vec++)
		if (*ap == '-')
			...
			adios(NULLCP, "usage: %s [switches] host:display", myname);
...
Redisplay()
{
	...
	image = XCreateImage(DISP, DefaultVisual(DISP, SCRN), 1, XYBitmap, 0,
			     ...
			     (unsigned int) myim->width, (unsigned int) myim->height, 8, 0);
	if (image == NULL)
	...
	char *buffer;
{
	...
	for (;;) {
		...
		i = sizeof *isock;
		if ((cc = recvfrom(sd, buffer, BUFSIZ, 0, (struct sockaddr *) isock, &i)) == NOTOK) {
			if (errno == EINTR)
				continue;

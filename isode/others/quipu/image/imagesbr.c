	...
	int stayopen;
{
	...
	if ((local_dn = str2dn(cp = dit ? dit : *local_dit != '@' ? local_dit : local_dit + 1))
	    == NULLDN)
...
static struct aka *
mbox2ak(local, domain)
	char *local, *domain;
{
	register struct aka *ak, *am;
	...
	for (ak = AHead->ak_forw; ak != AHead; ak = ak->ak_forw)
		if (!lexequ(domain, ak->ak_domain)
			...
			if (recording)
				LLOG(pgm_log, LLOG_NOTICE, ("hit \"%s\" \"%s\"", domain, local));

			return ak;
		}

	if ((am = (struct aka *) calloc(1, sizeof *am)) == NULL
	    || (am->ak_domain = strdup(domain)) == NULL || (am->ak_local = strdup(local)) == NULL)
		adios(NULLCP, "out of memory");
	...
	register struct aka *ak;
{
	...
	if (recording) {
		LLOG(pgm_log, LLOG_NOTICE, ("searching for %s %s", ak->ak_domain, ak->ak_local));
	...
	for (dlist = ak->ak_bases; dlist; dlist = dlist->dns_next) {
		...
		fi->FUITEM.UNAVA.ava_value = str2AttrV(ak->ak_local, at->oa_syntax);

		sa->sra_eis.eis_allattributes = FALSE;
		sa->sra_eis.eis_select = as_comp_new(str2AttrT(PHOTO), NULLAV, NULLACL_INFO);
		...
		if (sr->CSR_entries->ent_attr == NULLATTR) {
			...
			AttributeValue av = &sr->CSR_entries->ent_attr->attr_value->avseq_av;

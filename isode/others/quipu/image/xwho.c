	...
	char **vec;
{
	...
	for (vec++; ap = *vec; vec++)
		if (*ap == '-')
			...
			case 's':
				if ((ap = *++vec) == NULL
				    || sscanf(ap, "%d", &sleepsw) != 1 || sleepsw < 1)
	...
	if ((DISP = XOpenDisplay(display)) == NULL)
		adios(NULLCP, "unable to open display \"%s\"", XDisplayName(display));
...
init_X()
{
	...
	myframe.y = 0;
	(void) sprintf(def, "=%dx%d+%d+%d", myframe.width, myframe.height, myframe.x, myframe.y);
	...
	XSetStandardProperties(DISP, mywindow, myname, "X Who", None, (char **) 0, 0, &hints);
		...
		for (fp = hp->h_faces; fp; fp = fp->f_next)
			if (fp->f_imap) {
				if (fp->f_window == NULL
				    ...
				    || fp->f_frame.x != hw || fp->f_frame.y != h) {
	...
	register struct face *fp;
{
	...
	image = XCreateImage(DISP, DefaultVisual(DISP, SCRN), 1, XYBitmap, 0,
			     ...
			     (unsigned int) im->width, (unsigned int) im->height, 8, 0);
	if (image == NULL)
...
read_X()
{
	...
	register struct face *fp, **fpp;
	register struct host *hp, **hpp;
	struct whod wds;
	...
	while (dp = readdir(dd)) {
		...
		for (hp = hosts; hp; hp = hp->h_next)
			if (strncmp(hp->h_name, wd->wd_hostname, sizeof wd->wd_hostname) == 0)
				break;
		if (hp == NULL) {
			...
			(void) strncpy(hp->h_name, wd->wd_hostname, sizeof wd->wd_hostname);
			(void) sprintf(hp->h_string, "%s:", hp->h_name);
			hp->h_frame.width = XTextWidth(myfont, hp->h_string, strlen(hp->h_string));
		}
...

	{
		...
		for (hp = hosts; hp; hp = hp->h_next) {
			...
			if (j > 1) {
				...
				if ((faces = (struct face **)
					...
					continue;
				for (fp = hp->h_faces, fq = faces; *fq = fp; fp = fp->f_next, fq++)
					continue;
				qsort((char *) faces, j, sizeof *faces, (IFP) facecmp);
				for (fq = faces, fpp = &hp->h_faces;
				     *fpp = *fq; fq++, fpp = &(*fpp)->f_next)
					continue;
		...
		if (i > 1) {
			...
			for (hq = hostz, hpp = &hosts; *hpp = *hq; hq++, hpp = &(*hpp)->h_next)
				continue;

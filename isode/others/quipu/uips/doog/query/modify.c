	...
	QCardinal request_id;
{
	...
	if (request == NULLReqRec)
		return NULLModifyResult;
 ...
 */
errorList
do_modify(baseobject, id_ptr, mod_attr_list)
	char *baseobject;
	QCardinal *id_ptr;
	modifyAttr mod_attr_list;
{
	...
	return start_modify(*id_ptr, baseobject, mod_attr_list, &modify_request->task_id);
}				/* do_modify */
 ...
 */
static errorList
start_modify(request_id, baseobject, mod_attr_list, task_id_ptr)
	QCardinal request_id;
	char *baseobject;
	modifyAttr mod_attr_list;
	int *task_id_ptr;
{
	...
	struct entrymod *curr_mod = (struct entrymod *) NULL, *entrymods = (struct entrymod *) NULL;
	...
	for (curr_attr = mod_attr_list; curr_attr != NULLModifyAttr; curr_attr = curr_attr->next) {
 ...
 */
			...
			for (curr_val = curr_attr->values;
			     curr_val != NULLModifyValue; curr_val = curr_val->next) {
 ...
 */
				...
				for (curr_val = curr_attr->values;
					...
					} else {	/* Attribute value has an inappropriate
							   syntax! */

						if (entrymods)
							ems_free(entrymods);
						if (attrVal_seq != NULLAV)
							avs_free(attrVal_seq);
 ...
 */
				...
				if (new_count > 0) {
					...
					if (attrVal_seq == NULLAV) {
						...
						curr_mod->em_what =
						    as_comp_new(curr_attr->attr_type, attrVal_seq,
								NULLACL_INFO);
				}
			} else if (new_count > 0) {
				curr_mod->em_type = EM_ADDVALUES;
					...
					if (curr_val->status == new) {
						...
						if (attr_val != NULLAttrV) {
							if (attrVal_seq != NULLAV)
								...
								attrVal_seq =
								    avs_comp_new(attr_val);
						} else {	/* Value has an inappropriate
								   syntax! */
...
#ifndef NO_STATS
	LLOG(log_stat, LLOG_NOTICE, ("MODIFY +%s, task %d", baseobject, task_id));
#endif
...
/* ARGSUSED */
request_state
process_modify_ds_result(request, task_id, ds_result)
	requestRec request;
	int task_id;
 ...
 */
request_state
process_modify_ds_error(request, task_id, ds_error)
	requestRec request;
	int task_id;
	struct DSError *ds_error;
{
	...
	add_error_to_request_rec(request, task_rec->baseobject, error_type, ds_error);
 ...
 */
makeTemplateResult
get_make_template_result(request_id)
	QCardinal request_id;
{
	requestRec request = _get_request_of_id(request_id);
 ...
 */
QE_error_code
do_make_template(baseobject, id_ptr, attr_list)
	char *baseobject;
	QCardinal *id_ptr;
 ...
 */
	...
	error_code = start_read(*id_ptr, baseobject, NULLStrCell, &template_rec->task_id);
 ...
 */
request_state
process_template_ds_result(request, task_id, ds_result)
	requestRec request;
	int task_id;
	struct DSResult *ds_result;
{
	...
	modifyAttr entry_template = templaterec->include_attrs,
	    ...
	    may_in = NULLModifyAttr, must_out = NULLModifyAttr, may_out = NULLModifyAttr, tmp;
...
#ifndef NO_STATS
	LLOG(log_stat, LLOG_NOTICE, ("READ RESULT +%s, task %d", task_rec->baseobject, task_id));
#endif
 ...
 */
	...
	for (curr_oclass = oclass_attr->values;
	     curr_oclass != NULLModifyValue; curr_oclass = curr_oclass->next) {
 ...
 */
request_state
process_template_ds_error(request, task_id, error)
	requestRec request;
	int task_id;
 ...
 */
static void
get_template_attrs(readattrs, template)
	...
	modifyAttr template;
{
	...
	for (curr_as = readattrs; curr_as != NULLATTR; curr_as = curr_as->attr_link) {
		for (this_attr = template; this_attr != NULLModifyAttr; this_attr = this_attr->next)
				...
				/* The attribute corresponds to one in the template. */
			{
				...
				for (curr_av = curr_as->attr_value;
				     curr_av != NULLAV; curr_av = curr_av->avseq_next) {
					if ((valps = ps_alloc(str_open)) == NULLPS)
						return;
...
void
free_mod_attr_list(mod_attr_list_ptr)
...
void
free_mod_val_list(mod_val_list_ptr)
...
void
free_make_template_result(result_ptr)
...
void
free_make_template_rec(record)
	makeTemplateRec record;
{
	if (record == NULLTemplateRec)
		return;
	...
	attrValList attr_list;
{
	...
	for (curr_attr = attr_list; curr_attr != NULLAVList; curr_attr = curr_attr->next) {
		...
		for (curr_val = curr_attr->val_list, curr_mod_val = NULLModifyValue;
		     curr_val != NULLStrCell; curr_val = curr_val->next) {
 ...
 */
static modifyAttr
remove_common_attrs(primary, secondary)
	...
	modifyAttr primary;
{
	...
	for (curr_primary = primary;
	     curr_primary != NULLModifyAttr; curr_primary = curr_primary->next) {

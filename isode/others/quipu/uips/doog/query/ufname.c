 ...
 */
QE_error_code
do_ufn_resolve(baseobjects, ufname, is_leaf, id_ptr)
	...
	QCardinal *id_ptr;
{
	...
	for (tmp_part = ufname, name_part_count = 1;
	     tmp_part->next != NULLNamePart; tmp_part = tmp_part->next, name_part_count++) ;
	...
	if (baseobjects != NULLEntryList) {
		...
		root_part->good_matches =
		    root_part->poor_matches = root_part->exact_matches = NULLEntryList;
		...
		(void) dn_list_add(path == NULLEntryList ? "" : path->string_dn,
				   &root_part->exact_matches, NULLAttrT);
 ...
 *
 */
static QE_error_code
process_ufn_search(ufnrec)
	ufnameRec ufnrec;
{
	...
	if (prev_comp == NULLNamePart) {
		...
		if ((search_status = directory_search(NULLCP, name_comp, ufnrec))
			...
			add_error_to_request_rec(request,
						 NULLCP, search_status, (struct DSError *) NULL);
			return search_status;
		}
	} else {
		/* Else search against all resolved names. If no exact matches, use good matches.
		   If no good matches use poor matches. Return partial matches if match number
		   exceeds limit */
 ...
 */
request_state
continue_ufn_search(good_matches, request_id)
	...
	QCardinal request_id;
{
	...
	for (last_part = ufnrec->name_parts->next;
	     ...
	     last_part != NULLNamePart; last_part = last_part->next) ;

	if (last_part == NULLNamePart)
		return RQ_error_returned;
	else {
		for (good_match_num = 0, curr_match = good_matches;
		     curr_match != NULLEntryList; curr_match = curr_match->next, good_match_num++) ;
 ...
 */
static QE_error_code
directory_search(base_name, purp_name_comp, ufnrec)
	char *base_name;
	...
	ufnameRec ufnrec;
{
	...
	char *str_oid, *start, *end, *search_value;
	...
	if (purp_name_comp->next == NULLNamePart && purp_name_comp->is_bottom_level == no)
		return QERR_nothing_found;

	if (purp_name_comp->is_bottom_level == might_not)
		purp_name_comp->is_bottom_level = no;

	/* Make a DN struct from given string object name. If not valid then return an error. */
	if (base_name == NULLCP || isnull(*base_name))
		base_dn = NULLDN;
	else if ((base_dn = str2dn(base_name)) == NULLDN)
	...
	if (base_dn != NULLDN)
		while (1) {
			...
			*end = '\0';
			if ((attr_type = AttrT_new(start)) == NULLAttrT)
				return QERR_bad_name;
			*end = '=';
			...
			if (lexequ(str_oid, "2.5.4.7") == 0)
				has_loc = TRUE;
			else if (lexequ(str_oid, "2.5.4.11") == 0)
				has_org_unit = TRUE;
			else if (lexequ(str_oid, "2.5.4.10") == 0)
				has_org = TRUE;
	 ...
	 */
		...
		if ((int) strlen(search_value) > 2) {
			...
			if (make_filter_items(attr_type,
					      search_value,
					      &exact_filter, &approx_filter) != QERR_ok)
				return QERR_bad_value_syntax;
		} else {
	 ...
	 */
	else if (purp_name_comp->is_bottom_level == unknown && level_count >= 2 && has_org == TRUE) {
		...
		if (make_filter_items(attr_type,
				      search_value,
				      &exact_filter->FUFILT, &approx_filter->FUFILT) != QERR_ok) {
			filter_free(exact_filter);
			filter_free(approx_filter);
	 ...
	 */
	for (count = 0;
	     ufnrec->dap_exact_task_ids[count] != NO_TASK && count < MAX_TASKS_PER_REQ; count++) ;
	...
	LLOG(log_stat, LLOG_NOTICE,
	     ("EXACT MATCH +%s, task %d, extent %d, val %s",
	      base_name, task_id, search_arg.sra_subset, search_value));
#endif
 ...
 */
void
ufname_rec_free(record)
	ufnameRec record;
{
	if (record == NULLUfnameRec)
		return;
 ...
 */
void
ufname_result_free(ufn_result)
	ufnResults *ufn_result;
{
	ufnResults result = *ufn_result;

	if (result == NULLUfnResults)
		return;

	error_list_free(&result->errors);
	dn_list_free(&result->matches);

	if (result->unresolved_part != NULLCP)
		free(result->unresolved_part);
	if (result->resolved_part != NULLCP)
		free(result->resolved_part);
	...
	char *str_ufn;
{
	...
	start = str_ufn;
	while (!isnull(*start)) {
		while (isspace(*start) && !isnull(*start))
			start++;

		end = start;
		if (*end++ == '\"') {
			while (*end != '\"' && !isnull(*end))
				end++;

			if (isnull(*end)) {
				if (name_comp != NULLNamePart)
					name_part_free(&name_comp);
				return NULLNamePart;
			}
		}
		...
		*end = '\0';
		...
		name_comp->exact_matches =
		    name_comp->good_matches = name_comp->poor_matches = NULLEntryList;

		name_comp->exact_match_num =
		    name_comp->good_match_num = name_comp->poor_match_num = 0;
 ...
 */
request_state
process_ufn_ds_result(request, task_id, ds_result)
	requestRec request;
	int task_id;
	struct DSResult *ds_result;
{
	...
	for (aindex = 0, ufn_task_array = ufnrec->dap_exact_task_ids;
	     ufn_task_array[aindex] != task_id && aindex < MAX_TASKS_PER_REQ; aindex++) ;
	...
	if (search_result->CSR_entries != NULLENTRYINFO) {
		...
		for (entry_ptr = search_result->CSR_entries;
		     entry_ptr != NULLENTRYINFO; entry_ptr = entry_ptr->ent_next) {
					...
					if (got_attrs != NULLAVList)
						for (curr_av = got_attrs, good_match = FALSE;
							...
							for (curr_attr = curr_av->val_list;
							     curr_attr != NULLStrCell
							     && good_match == FALSE;
							     curr_attr = curr_attr->next) {
								if (is_good_match
								    (match_str, curr_attr->string))
									good_match = TRUE;
					} else
						good_match = FALSE;
				} else
		...
		if (search_result->CSR_limitproblem != LSR_NOLIMITPROBLEM) {
			...
			add_error_to_request_rec(request,
						 task_rec->baseobject,
						 limitproblem, (struct DSError *) NULL);
		}
	}
#ifndef NO_STATS
	LLOG(log_stat, LLOG_NOTICE,
	     ("SEARCH RESULT from +%s, task %d, hits %d",
	      task_rec->baseobject, task_id, hit_count));
#endif
	...
	if (ufnrec->exact_task_count == 0 && ufnrec->approx_task_count == 0) {
		...
		if (name_comp->exact_match_num == 0 &&
		    name_comp->good_match_num == 0 && name_comp->poor_match_num == 0) {
		...
		if (name_comp->next == NULLNamePart) {
			...
			if (name_comp->good_match_num == 0 && name_comp->exact_match_num == 0) {
				if (name_comp->poor_match_num > 0) {
					char resolved_part[LINESIZE],
					    unresolved_part[LINESIZE], buf[LINESIZE];
 ...
 */
request_state
process_ufn_ds_error(request, task_id, error)
	requestRec request;
	int task_id;
	...
	ufnameRec ufnrec;
{
	...
	for (part = ufnrec->name_parts; part != NULLNamePart; part = part->next) {
		part->exact_match_num = part->good_match_num = part->poor_match_num = 0;
	...
	(void) dn_list_add(ufnrec->path->string_dn,
			   &(ufnrec->name_parts->exact_matches), NULLAttrT);
...
void
add_ufn_path_element(lower, upper, path)
	int lower, upper;
	entryList path;
{
...
entryList
get_ufn_path(comp_num)
	int comp_num;
{
	searchPath curr_path;
	int lower, upper;

	for (curr_path = ufnpaths; curr_path != NULLSearchPath; curr_path = curr_path->next) {

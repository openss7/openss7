 ...
 */
QE_error_code
do_read(baseobject, id_ptr, attr_list)
	char *baseobject;
	QCardinal *id_ptr;
 ...
 */
QE_error_code
start_read(request_id, baseobject, attr_list, task_id_ptr)
	QCardinal request_id;
	char *baseobject;
	stringCell attr_list;
	int *task_id_ptr;
...
#ifndef NO_STATS
	LLOG(log_stat, LLOG_NOTICE, ("READ +%s, task %d", baseobject, task_id));
#endif
 ...
 */
request_state
process_read_ds_result(request, task_id, ds_result)
	requestRec request;
	int task_id;
	struct DSResult *ds_result;
{
	...
	get_read_attrs(read_result->rdr_entry.ent_attr, &readrec->results->entry, READOUT);
 ...
 */
request_state
process_read_ds_error(request, task_id, error)
	requestRec request;
	int task_id;
 ...
 */
void
read_rec_free(record)
	readRec record;
{
	if (record == NULLReadRec)
		return;
 ...
 */
void
read_dn_attr_rec_free(record)
	readDnAttrRec record;
{
	if (record == NULLReadDnAttrRec)
		return;

	if (record->base_object != NULLCP)
		free(record->base_object);
	if (record->read_object != NULLCP)
		free(record->read_object);
 ...
 */
void
get_read_attrs(readattrs, entryattrs, format)
	...
	int format;
{
	...
	if (entryattrs == (attrValList *) NULL)
		return;

	for (curr_as = readattrs, curr_attr = NULLAVList;
	     curr_as != NULLATTR; curr_as = curr_as->attr_link) {
		is_photo = FALSE;
		...
		if (format == READOUT) {
			add_string_to_seq(attr2name(curr_as->attr_type, OIDPART), &curr_type);
		} else {
		...
		} else {	/* If not a multi line value */
			if ((valps = ps_alloc(str_open)) == NULLPS)
				return;
	...
	char *name;
{
	...
	if (photo_pass_two == FALSE) {
		...
		photo_ptr = photo_bytes = (unsigned char *) smalloc(byte_width * photo_height);
 ...
 */
QE_error_code
do_dn_attr_read(baseobject, id_ptr, dn_attr)
	char *baseobject;
	QCardinal *id_ptr;
	stringCell dn_attr;
{
 ...
 */
request_state
process_read_dn_attr_result(request, task_id, ds_result)
	requestRec request;
	int task_id;
	struct DSResult *ds_result;
{
 ...
 */
request_state
process_read_dn_attr_error(request, task_id, error)
	requestRec request;
	int task_id;
	struct DSError *error;
{
 ...
 */
readResults
get_read_dn_attr_results(request_id)
	QCardinal request_id;
{
	requestRec request = _get_request_of_id(request_id);

 ...
 */
QE_error_code
_request_invoked(type, return_id_ptr)
	request_type type;
	...
	*return_id_ptr = 0;
	...
	case UFSEARCH:
		...
		for (aindex = 0; aindex < MaxUfsearchTasks; aindex++) {
			ufsrec->exact_task_ids[aindex] = ufsrec->approx_task_ids[aindex] = NO_TASK;
		}
	...
	QCardinal request_id;
{
	...
	if ((old_request = _get_request_of_id(request_id)) == NULLReqRec)
		return;
...
process_dap_result(dap_result, id_ptr)
 ...
 */
static request_state
process_dap_error(error, id_ptr)

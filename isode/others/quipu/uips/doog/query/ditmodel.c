 ...
 */
void
set_relation(parent_type, child_type, need_subtree)
	...
	QBool need_subtree;
{
	...
	for (curr_node = ditmodel; curr_node != NULLDitRelation; curr_node = curr_node->next) {
		if (curr_node->parent_type == parent_type) {
			for (curr_child = curr_node->child_types;
			     curr_child != NULLObjectType; curr_child = curr_child->next)
				if (curr_child->object_type == child_type)
					return;
 ...
 */
objectTypeList
get_search_attrs(attr_type)
	AttributeType attr_type;
{
	searchInfo curr_info;

	for (curr_info = search_type_list; curr_info != NULLSearchInfo; curr_info = curr_info->next)
		if (attr_type == curr_info->object_type)
			return curr_info->search_on_list;
 ...
 */
void
set_search_attr(type_label, obj_type, search_type)
	char *type_label;
	...
	for (curr_info = search_type_list; curr_info != NULLSearchInfo; curr_info = curr_info->next)
		if (obj_type == curr_info->object_type) {
			for (curr_type = curr_info->search_on_list;
			     curr_type != NULLObjectType; curr_type = curr_type->next)
				if (curr_type->object_type == search_type)
					return;
...
test_init_ditmodel()
{
	...
	set_search_attr("Country", country, AttrT_new("0.9.2342.19200300.99.1.8"));
	...
	set_search_attr("Place", loc, AttrT_new("2.5.4.7"));

	set_default_path(org, search_path);

	set_search_attr("Organization", org, AttrT_new("2.5.4.10"));

	set_search_attr("Organization", org, AttrT_new("2.5.4.7"));

	search_path = NULLEntryList;

	set_default_path(ou, search_path);

	set_search_attr("Department", ou, AttrT_new("2.5.4.11"));

	set_search_attr("Department", ou, AttrT_new("2.5.4.7"));
	...
	set_search_attr("Person", person, AttrT_new("2.5.4.3"));
}
 ...
 */
QBool
is_search_attr(octype, srchtype)
	...
	AttributeType at;
{
	...
	for (srchlist = search_type_list;
	     srchlist != NULLSearchInfo && srchlist->object_type != at; srchlist = srchlist->next) ;
 ...
 */
void
set_default_path(object_type, path)
	AttributeType object_type;
	entryList path;
{
	ufsPath curr_path;

	for (curr_path = ufs_paths; curr_path != NULLUfsPath; curr_path = curr_path->next) {

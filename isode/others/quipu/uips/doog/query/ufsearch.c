 ...
 */
QE_error_code
do_ufsearch(target_data, search_data, baselist, id_ptr)
	...
	QCardinal *id_ptr;
{
	...
	QBool have_co = FALSE, have_org = FALSE, have_loc = FALSE;
	...
	for (curr_srch_data = search_data;
	     curr_srch_data != NULLSearchPair; curr_srch_data = curr_srch_data->next) {
	...
	ufsearchRec ufsrec;
{
	...
	if (ufsrec->tried_target_search == FALSE) {
		for (to_follow = ufsrec->to_follow;
		     to_follow != NULLEntryList; to_follow = to_follow->next) {
			...
			for (curr_type = child_types;
				...
				if (qy_in_hierarchy
					...
					if (follow_up(to_follow, ufsrec) == QERR_ok)
						succeeded++;
	...
	for (to_follow = ufsrec->to_follow; to_follow != NULLEntryList; to_follow = to_follow->next)
		if (follow_up(to_follow, ufsrec) == QERR_ok)
			succeeded++;

	/* If trying a search for the target then don't remove list in case search fails to hit and 
	   further intermediate searches are required */
	...
	ufsearchRec ufsrec;
{
	...
	searchPair search_vals = NULLSearchPair,
	    curr_val = NULLSearchPair, search_on = NULLSearchPair;
	...
	for (search_vals = ufsrec->search_data;
	     search_vals != NULLSearchPair; search_vals = search_vals->next) {
		search_vals->checked = FALSE;
		search_val_count++;
	}

	if (ufsrec->trying_target_search == TRUE) {	/* Searching for target */
		for (search_types = get_search_attrs(ufsrec->target_data->type);
		     search_types != NULLObjectType; search_types = search_types->next) {
			...
			for (curr_str_val = ufsrec->target_data->values;
			     curr_str_val != NULLStrCell; curr_str_val = curr_str_val->next)
			...
			if (search_vals->checked == TRUE) {
				for (search_types = get_search_attrs(search_vals->type);
					...
					for (curr_str_val = search_vals->values;
						...
						add_string_to_seq(curr_str_val->string,
								  &curr_val->values);
 ...
 */
static QE_error_code
do_directory_search(baseobject, search_vals, ufsrec)
	char *baseobject;
	...
	ufsearchRec ufsrec;
{
	...
	Filter exact_filter, *exfilt_ptr, tmp_ex_filt, approx_filter, *apfilt_ptr, tmp_ap_filt;
	...
	if (baseobject == NULLCP || isnull(*baseobject))
		base_dn = NULLDN;
	else if ((base_dn = str2dn(baseobject)) == NULLDN)
	...
	if (search_vals->next != NULLSearchPair || search_vals->values->next != NULLStrCell) {
		...
		for (curr_val_list = search_vals,
		     ...
		     curr_val_list != NULLSearchPair; curr_val_list = curr_val_list->next) {
		...
		get_attrs = as_merge(get_attrs,
				     as_comp_new(search_vals->type, NULLAV, NULLACL_INFO));
	}
	...
	for (count = 0;
	     ufsrec->exact_task_ids[count] != NO_TASK && count < MaxUfsearchTasks; count++) ;
	...
	/* Record task invocation for this particular request. */
	for (count = 0;
	     ufsrec->approx_task_ids[count] != NO_TASK && count < MaxUfsearchTasks; count++) ;
 ...
 */
request_state
process_ufs_ds_result(request, task_id, ds_result)
	requestRec request;
	int task_id;
	struct DSResult *ds_result;
{
	...
	for (aindex = 0, ufs_task_array = ufsrec->exact_task_ids;
	     ufs_task_array[aindex] != task_id && aindex < MaxUfsearchTasks; aindex++) ;
	...
	if (search_result->CSR_entries != NULLENTRYINFO) {
		...
		search_vals = ufsrec->trying_target_search == TRUE ?
		    ufsrec->target_data : ufsrec->search_data;
		...
		for (entry_ptr = search_result->CSR_entries;
			...
			/* It is a good_match. If it's objectClass can be found in the `ditmodel'
			   or if it is a match against target data kep it */
			if (good_match == TRUE) {
 ...
 */
request_state
process_ufs_ds_error(request, task_id, error)
	requestRec request;
	int task_id;
 ...
 */
void
ufsearch_result_free(result_ptr)
	ufsearchResult *result_ptr;
{
	ufsearchResult ufs_result = *result_ptr;

	if (ufs_result == NULLUfsearchResult)
		return;

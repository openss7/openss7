 ...
 */
QBool
is_good_match(match_str, dn_str)
	...
	char a, b;
{
	...
	if (isalpha(chr1) && islower(chr1))
		chr1 = toupper(chr1);
	if (isalpha(chr2) && islower(chr2))
		chr2 = toupper(chr2);
 ...
 */
QE_error_code
make_typed_filter_items(filter_str, ex_filter_ptr, ap_filter_ptr)
	char *filter_str;
	...
	Filter *ap_filter_ptr;
{
	...
	*end = save;
	...
	error = make_filter_items(attr_type, str_attr_value, ex_filter_ptr, ap_filter_ptr);
	free(str_attr_type);
	free(str_attr_value);
 ...
 */
QE_error_code
make_filter_items(attr_type, search_value, ex_filter_ptr, ap_filter_ptr)
	AttributeType attr_type;
	char *search_value;
	Filter *ex_filter_ptr;
	Filter *ap_filter_ptr;
{
	...
	approx_filter->FUITEM.UNAVA.ava_type = exact_filter->FUITEM.UNAVA.ava_type = attr_type;

	*ex_filter_ptr = exact_filter;
	*ap_filter_ptr = approx_filter;

	approx_filter->FUITEM.UNAVA.ava_value = exact_filter->FUITEM.UNAVA.ava_value = NULLAttrV;
 ...
 */
char *
get_entry_type_name(entry_name)
	...
	objectclass *a, *b;
{
	...
	if (a == (objectclass *) NULL || b == (objectclass *) NULL)
		return FALSE;

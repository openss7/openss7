 ...
 */
void
ufnresolve(name, baseobjects, is_leaf)
	char *name;
		...
		do {
			do {
				...
				while (select(getdtablesize(),
					      &association,
					      &writefds, &exceptfds, (struct timeval *) NULL) <= 0);
			}
			...
			case PoorComplete:
				...
				if (query_matches(results->matches, &matches) == FALSE) ;
					...
					if (matches != NULLEntryList) {
						...
						request_status =
						    continue_ufn_search(matches, request_id);
					}
				}
	...
	char *params;
{
	...
	for (count = 1, entries = currentlist;
	     count < entry_num && entries != NULLEntryList; count++, entries = entries->next) ;
	...
	do {
		...
		while (select(getdtablesize(),
			      &association, &writefds, &exceptfds, (struct timeval *) NULL) <= 0);
	}
	while (directory_wait(&request_list) == 0);
...
void
print_read_results(results, baseobject)
	readResults results;
	char *baseobject;
{
	stringCell val_list;
	...
	for (entry_attrs = results->entry;
	     entry_attrs != NULLAVList; entry_attrs = entry_attrs->next) {
		for (val_list = entry_attrs->val_list;
		     val_list != NULLStrCell; val_list = val_list->next) {
			(void) sprintf(message,
				       " %-21s- %s\n",
				       entry_attrs->attr_name->string, val_list->string);
			uprint(message);
		}
	}
...
QBool
query_matches(matches, returnlist)
	entryList matches;
	entryList *returnlist;
{
	...
	for (curr_poor_match = matches;
	     curr_poor_match != NULLEntryList; curr_poor_match = curr_poor_match->next) {
		...
		while (1) {
			...
			if ((int) strlen(input) > 1) {
				...
				(void) dn_list_add(curr_poor_match->string_dn,
						   &good_matches, NULLAttrT);
				break;
			} else if (*input == 'q' || *input == 'Q') {
				if (good_matches != NULLEntryList)
					dn_list_free(&good_matches);
				return FALSE;

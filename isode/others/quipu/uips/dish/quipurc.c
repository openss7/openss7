...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/others/quipu/uips/dish/RCS/quipurc.c,v 9.0 1992/06/16 12:44:21 isode Rel";
#endif
...
main()
{
	...
	while (fgets(Read_in_Stuff, LINESIZE, fp_tailor) != 0) {
		if (!strcmp
		    (Read_in_Stuff,
		     "##Anything after this line is copied into the users ~/.quipurc file\n")) {
			break;
		}
...
/* set up the appropriate structures and defaults. */
	...
	if ((search_arg.sra_filter->flt_un.flt_un_item.fi_un.fi_un_ava.ava_type =
	     AttrT_new("userid")) == NULLAttrT) {
		ps_printf(rps, "Oops, userid is not a valid attr type. ABORT!!\n");
		exit(1);
	}
	if ((search_arg.sra_filter->flt_un.flt_un_item.fi_un.fi_un_ava.ava_value =
	     str2AttrV(user_name,
		       search_arg.sra_filter->flt_un.flt_un_item.fi_un.fi_un_ava.ava_type->
		       oa_syntax)) == NULLAttrV) {
 ...
 */
	...
	if (search_result.CSR_entries == NULLENTRYINFO) {
		...
		if ((at = AttrT_new("userPassword")) != NULLAttrT) {
			as_flag =
			    as_merge(as_flag, as_comp_new(AttrT_cpy(at), NULLAV, NULLACL_INFO));
		} else {
			ps_printf(rps,
				  "Oops, Serious error. unknown attribute type 'userPassword'.\n");
			exit(1);
		}
		...
		if ((current_entry = local_find_entry(dn, FALSE)) == NULLENTRY) {
			...
			cache_entry(&(read_result.rdr_entry), read_arg.rda_eis.eis_allattributes,
				    TRUE);
		}
		...
		for (eptr = current_entry->e_attributes; eptr != NULLATTR; eptr = eptr->attr_link) {
			/* Tiptoe through the list of types until one matches, and then print
			   value. */
		...
		if (eptr == NULLATTR) {
			...
			temp =
			    as_comp_new(AttrT_cpy(entry_ptr->e_name->rdn_at), avst, NULLACL_INFO);
			...
			for (as = entry_ptr->e_attributes; as != NULLATTR; as = as->attr_link) {
				...
				if (emnew != NULLMOD) {
					mod_arg.mea_changes =
					    ems_append(mod_arg.mea_changes, emnew);
				}
				as->attr_link = trail;
			}
			...
			while (ds_modifyentry(&mod_arg, &mod_error) != DS_OK) {
				...
				mod_arg.mea_object =
				    mod_error.ERR_REFERRAL.DSE_ref_candidates->cr_name;
			}
	...
	int y;
{
	...
	for (ptr = current_sequence->ds_data;
	     (ptr != NULL_DE) && (x < y); ptr = ptr->de_next, x++) ;
	...
	AttributeType ent_mod_at;
{
	...
	for (x = b; x != NULLAV; x = x->avseq_next) {
		...
		if (y == NULLAV) {
			...
			em_new->em_what =
			    as_comp_new(ent_mod_at, avs_comp_new(&x->avseq_av), NULLACL_INFO);

	...
	char **argv;
{
	...
	/* dn should be set to a: either current location, or b: 2nd arg if specified. */
	...
	/* Is dn a leaf or a non_leaf? */
	{
		...
		compare_arg.cma_common.ca_servicecontrol.svc_options |=
		...
		while (ds_compare(&compare_arg, &compare_error, &compare_result) != DS_OK) {
			...
			compare_arg.cma_object =
			    compare_error.ERR_REFERRAL.DSE_ref_candidates->cr_name;
		}
		...
		if (compare_result.cmr_matched == FALSE) {
			...
			while (ds_read(&read_arg, &read_error, &read_result) != DS_OK) {
				if (dish_error(OPT, &read_error) == 0)
					return (-2);
				read_arg.rda_object =
				    read_error.ERR_REFERRAL.DSE_ref_candidates->cr_name;
			}
	...
	/* Else search the subtree below this current position for ALL aliases, and check each
	   one. */
	...
	EntryInfo *alias_entry;
{
	...
	/* We now have ourselves an entrystruct which is an alias, and we have to check various
	   features!! 1: only the allowed objects. 2: the seeAlso attribute is correct (ie points
	   to a real entry). 3: The objectClass of the real entry fits under where the alias lives. */

	/* Does the alias have the allowed objects and only the allowed? */

	for (tmp_ent_attr = alias_entry->ent_attr; tmp_ent_attr != NULL;
		...
		if (AttrT_cmp(tmp_ent_attr->attr_type, at_ojc) == 0) {
			...
			read_arg.rda_object =
			    dn_cpy((DN) tmp_ent_attr->attr_value->avseq_av.av_struct);
			AlObjNam = TRUE;
	...
	/* and see if it points back to this alias. It should do, but doesn't have to. While we
	   are at it, collect the objectClass of this entry. */

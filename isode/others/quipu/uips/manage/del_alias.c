	...
	char **argv;
{
	...
	if (*objectname >= '0' && *objectname <= '9') {
		...
		if (*objectname == '@') {
			...
			/* oj_dn = dn_cpy(dn) ; dn_append(oj_dn, dn_cpy(str2dn(objectname))) ; */
	...
	/* Now we have the other end of the alias in AttrValue format, convert it into a DN, so we
	   can modify it. */
	...
	*str_ps->ps_ptr = 0;
	...
	/* We now have converted a string to DN and we now have to form the attribute
	   "seeAlso=<DN>", put it into the modify attributes and delete that attribute from the
	   other end of the alias. */

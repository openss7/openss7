...
"From:", &from, "Reply-To:", &reply_to, "Sender:", &sender, "Subject:", &subject, NULL, NULL};
	...
	char **argv, **envp;
{
	int eof, status, vecp;
	register char *cp;
	char address[BUFSIZ], buffer[BUFSIZ], *vec[NVEC + 1];
	struct sockaddr_in in_socket, *isock = &in_socket;
	...
	if (mail) {
		...
		if (subject) {
			...
			(void) ll_log(fred_log, LLOG_NOTICE, NULLCP, "%s asks: %s", ep, buffer);
	...
	if (network) {
		...
		(void) ll_log(fred_log, LLOG_NOTICE, NULLCP, "%s asks: %s", address, buffer);
	...
	if (op) {
		vecp = 0;
		if (nametype > 1 && (strcmp(myname, "whois") == 0 || (*op && **op == 'w'))) {
			register char **pp;
			char *bp = "";
	...
	for (interrupted = 0;; interrupted = 0) {
		...
		if (logging)
			(void) ll_log(fred_log, LLOG_NOTICE, NULLCP, "command: %s", buffer);
...
log_utmp()
{
	...
	while (read(ud, (char *) ut, sizeof *ut) == sizeof *ut) {
		...
		(void) ll_log(fred_log, LLOG_NOTICE, NULLCP,
			      "%.*s on %.*s (%.*s)",
			      NMAX, ut->ut_name, LMAX, ut->ut_line, HMAX, ut->ut_host);
#endif
		break;
	}
	(void) close(ud);

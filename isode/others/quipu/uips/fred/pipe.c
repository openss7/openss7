...
dish(command, silent)
	...
	int silent;
{
	int cc, isarea, isuser, n, sd, status;
	char buffer[BUFSIZ], where[BUFSIZ];
	...
	} else if (dish_running != NOTOK && kill(dish_running, 0) == NOTOK)
		dish_running = NOTOK, boundP = 0;

	if (dish_running == NOTOK && dafd == NOTOK) {
		int vecp;
		char dishname[BUFSIZ], *vec[4];
		...
		if (strcmp(server, "internal")) {
			...
			if ((hp = gethostbystring(server)) == NULL)
				adios(NULLCP,
				      "%s: unknown host for directory assistance server", server);
			if (cp)
				*cp = ':';
			...
			if ((dp = index(cp, ' ')) == NULLCP || sscanf(dp + 1, "%d", &portno) != 1)
			...
			if (cp = getenv("DISPLAY")) {
				...
				*cp_disp++ = '\0';
				if (strcmp(cp_host, "local") &&
				    strcmp(cp_host, "localhost") && strcmp(cp_host, "unix"))
	...
	if (n != cc)
		if (n == NOTOK) {
			...
			adios(NULLCP, "write to DUA truncated, sent %d of %d octets", n, cc);
	...
	for (;;) {
		...
		default:
			advise(NULLCP, "unexpected opcode 0x%x -- contact a camayoc", buffer[0]);
			break;
		}
		break;
	}
...
{
	int cc, i, j, k, n;
	char *cp, *dp, buffer[BUFSIZ], tmpfil[BUFSIZ];
	FILE *fp;
	struct stat st;
	...
	if (n != cc)
		if (n == NOTOK) {
			...
			adios(NULLCP, "write to DUA truncated, sent %d of %d cotets", n, cc);

	if ((fp = fopen(tmpfil, "w")) == NULL)
		adios(tmpfil, "unable to write");

	for (cc = j; j > 0; j -= i)
		switch (i = recv(sd, buffer, j < sizeof buffer ? j : sizeof buffer, 0)) {
		case NOTOK:
			if (!interrupted)
				adios("failed", "read from DUA");
			(void) da_command("INTR");
	...
	(void) sprintf(buffer, "%s %s", _isodefile(isodebinpath, "editentry"), tmpfil);
	if (system(buffer))
		goto all_done;
...
init_ufnrc()
{
	...
	char buffer[BUFSIZ], sync[BUFSIZ], ufnrc[BUFSIZ];
	FILE *fp;
	...
	for (i = 0; fgets(bp = buffer, sizeof buffer, fp); i++) {
		...
		if (!isspace(*bp)) {
			...
			*cp++ = NULL;
			...
			advise(NULLCP, "%s: unexpected blank at start of line %d", ufnrc, i);
			return NOTOK;
		} else
			*ep++ = '$';

	...
	char **vec;
{
	...
	(void) sprintf(buffer, "modify -nocache -dontusecopy -newdraft \"%s\"", mydn);
	dontpage = 1;
	...
	(void) sprintf(buffer, "showentry \"%s\" -fred -nocache -dontusecopy", mydn);
	(void) dish(buffer, 0);
	return OK;
}
	...
	char **vec;
{
	...
	(void) sprintf(bp, " \"%s\" \"%s\"", *vec ? *vec : "White Pages report", cp);
	bp += strlen(bp);
	...
	char **vec;
{
	...
	if (strcmp(cp, "-help") == 0) {
		...
		(void) fprintf(stdfp, "    with one argument, identifies you in the white pages\n");

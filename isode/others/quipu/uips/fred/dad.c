	...
	char **argv, **envp;
{
	...
	if ((sd = start_tcp_server(&lo_socket, SOMAXCONN, debug ? SO_DEBUG : 0, 0)) == NOTOK)
	...
	struct sockaddr_in *isock;
{
	...
	for (;;) {
		...
		ifds = sfds;	/* struct copy */
		switch (xselect(nfds, &ifds, NULLFD, NULLFD, doomsday ? 5 * 60 : NOTOK)) {
		case OK:
			if (doomsday) {
		...
		for (ep = (cp = buffer) + sizeof buffer - 1; cp < ep;) {
			...
			case OK:
				advise(LLOG_NOTICE, NULLCP, "client closed control connection");
				eof = 1;
				break;
	...
	int binding;
{
	...
	(void) strcpy(buffer, _isodefile(isodebinpath, "dish"));
      ...
      fork_again:;
	advise(LLOG_NOTICE, NULLCP, "starting %s on %s", buffer, getenv("DISHPROC"));
...
{
	register char *bp;
	char buffer[BUFSIZ], command[BUFSIZ], *vec[NVEC + 1];
	FILE *fp;
	...
	char **vec;
{
	...
	if ((sp = getservbyname("da", "tcp")) == NULL) {
		advise(LLOG_EXCEPTIONS, NULLCP, "%s/%s: unknown service", "tcp", "da");

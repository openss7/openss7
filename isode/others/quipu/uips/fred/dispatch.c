...
int f_set(), f_help();
int f_alias(), f_area(), f_dish(), f_edit(), f_manual(), f_report(), f_thisis();
int f_bind(), f_quit();
...
/*	(void) printf ("\nversion: %s\n", isodeversion); */
	...
	if (strcmp(*vec, "-help") == 0) {
		(void) fprintf(stdfp, "set [variable [value]]\n");
...
/*	(void) printf ("\nversion: %s\n", isodeversion); */
	...
	if (strcmp(*vec, "-help") == 0) {
		(void) fprintf(stdfp, "help [commands ...]\n");
		(void) fprintf(stdfp,
...
static
snarf(file, name, variable)
	char *file, *name, **variable;
{
	int i;
	register char *bp, *dp, *ep;
	char buffer[BUFSIZ];
	FILE *fp;

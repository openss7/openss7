...
void
put_dn_and_password(dn, password, name)
	char *dn, *password, *name;
{
	str_seq alias_list = alias_seq;
	int seq_num;

	for (seq_num = 1; alias_list != NULLDS; seq_num++, alias_list = alias_list->next)
		if (!lexequ(name, alias_list->dname))
			break;
...
user_tailor()
{
	...
	char *config_dir = "/.duaconfig",
	    ...
	    *options = "/.duarc", *typeDefaults = "/typeDefaults";

	char user_path[BUFSIZ], read_path[BUFSIZ],
	    type_path[BUFSIZ], options_path[BUFSIZ],
	    friendly_path[BUFSIZ], type_defaults_path[BUFSIZ], dua_config_dir[BUFSIZ];
	...
	if ((config_file = fopen(isodefile(tailfile, 0), "r")) == (FILE *) NULL) {
		kill_message();
		message((Widget) NULL, "Cannot open `dsaptailor' file. Attempting to continue.");
	} else {
	...
	while (dir_ent = readdir(config_directory)) {
		if (!(strncmp(dir_ent->d_name, "Type_", 5))) {
			file_names[filt_num] = (char *) malloc((unsigned int)
	...
	if (testing) {
		...
		if (!(config_file = fopen(read_path, "r"))) {
			(void) fprintf(stderr,
				       "File error! Test pod must be run from source directory!\n");
			quit(1);
		}
	} else {
		if (!(config_file = fopen(read_path, "r"))) {
	...
	while (fgets(Read_in_Stuff, STRINGLEN, config_file) != 0) {
		...
		if (*stroid_buf) {
			if (!read_types)
				read_types =
				    as_comp_new(AttrT_new(stroid_buf), NULLAV, NULLACL_INFO);
			else {
				read_types2 =
				    as_comp_new(AttrT_new(stroid_buf), NULLAV, NULLACL_INFO);
	...
	if (!read_types)
		read_types = as_comp_new(AttrT_new("2.6.5.2.6"), NULLAV, NULLACL_INFO);
	else {
		read_types2 = as_comp_new(AttrT_new("2.6.5.2.6"), NULLAV, NULLACL_INFO);
		read_types = as_merge(read_types, read_types2);
	}
			...
			while (*sptr != ':' && !isspace(*sptr) && sptr != '\0') {
				...
				*str = '\0';
				...
				if (!name_map[fname_num]) {
					name_map[fname_num] =
					    (fName) malloc(sizeof(struct friendlyName));
	...
	if (testing) {
		...
		if (!(config_file = fopen(type_defaults_path, "r"))) {
			...
			if (!(config_file = fopen(type_defaults_path, "r"))) {
				(void) fprintf(stderr,
					       "File error! Can't find typeDefaults file.\n");
				quit(1);
			}
		}
	}
	...
	while (fgets(Read_in_Stuff, STRINGLEN, config_file) != 0) {
		...
		*part2++ = '\0';
		...
		while (*part2 != ':') {
			n = 0;
			while (n < filt_num && strncmp(filttype[n], part2, (int) (end - part2)))
				n++;
			...
			if (n == filt_num) {
				...
				count++;
				part2 = end;
				while (!isalpha(*part2) && *part2 != ':' && part2 != '\0')
					part2++;
		...
		if (*end == ':') {
			...
			*end = '\0';
			...
			n = 0;
			while (n < filt_num && strcmp(filttype[n], p))
				n++;
			...
			if (n == filt_num) {
				...
				num = 0;
				while (num < count && n != tempints[num])
					num++;
				if (num == count) {
					(void) fprintf(stderr,
						       "Parsing error in typeDefaults file!");
...
cnnct_bind()
{
	...
	if (namestr[0] != '\0' && passwd[0] == '\0') {
		kill_message();
		message((Widget) NULL, "Enter password at command line!");
	...
	if (dsa_address == NULLCP || *dsa_address == '\0') {
		return ("Cannot find name of DSA to bind to! Click on this window to exit.");
	}
...
set_default_type()
{
	...
	if (*base_path != '\0') {
		...
		for (count = 0;
		     ...
		     strcmp(levels[count], base_name->dn_rdn->rdn_at->oa_ot.ot_stroid); count++) ;
	...
	char *fstr, *rdn;
{
	...
	*end = '\0';
	...
	for (count = 0; count < fname_num && !mapped; count++) {
		seqnum = 0;
		while (!mapped && (string = get_from_seq(seqnum++, name_map[count]->names)))
	...
	char *name;
{
	...
	if ((read_entry = local_find_entry(read_arg.rda_object, FALSE))
	    != NULLENTRY && read_entry->e_data != E_TYPE_CONSTRUCTOR) {
		object_class = get_sorted_attrs(read_entry->e_attributes, oclass);
		entry2str((caddr_t) object_class, entry_str, STRINGLEN);
		as_free(object_class);
		if (issubstr(entry_str, "NonLeaf"))
			return (0);

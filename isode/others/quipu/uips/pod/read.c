...
Attr_Sequence
get_sorted_attrs(entry_attrs, wanted_attrs)
	Attr_Sequence entry_attrs, wanted_attrs;
{
	register Attr_Sequence curr_ent_attr, curr_found, curr_wanted, attrs_found, next;
	...
	Attr_Sequence attrs_to_read;
{
	...
	if ((read_entry = local_find_entry(read_arg.rda_object, FALSE))
		...
		if (attrs_to_read != NULLATTR)
			wanted_attrs = get_sorted_attrs(read_entry->e_attributes, attrs_to_read);
		else
			wanted_attrs = read_entry->e_attributes;

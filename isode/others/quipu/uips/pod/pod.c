...
static Widget standby = 0, error_popup = 0,
    curr_read_popup = 0, curr_list_popup = 0, curr_modify_popup = 0, version_popup = 0;
...
CreateWidgets()
{
	...
	count = 0;
	outer = XtCreateManagedWidget("outer", formWidgetClass, toplevel, args, count);
...
PodLoop()
{
	...
	String window_name = "Directory", icon_name = "Directory";
	...
	XSetWMProperties(dpy, XtWindow(toplevel),
			 &window, &icon, (char **) 0, (int) 0, &hints, &wm_hints, &class_hint);

	PosWindow = XtNameToWidget(outer, "PosForm.PosScrolledWindow.PosWindow");
...
make_photo_widget()
{
	...
	TextForm = XtNameToWidget(curr_read_popup, "ReadForm.TextScrolledWindow.TextForm");
	...
	count++;
	PhotoWindow = XtCreateManagedWidget("PhotoWindow", labelWidgetClass, TextForm, args, count);
}
...
print_photo()
{
	...
	XtAddCallback(PhotoWindow, XtNdestroyCallback, FreeWidgetPixmap, (XtPointer) photo_pixmap);

	photo_pixmap = (Pixmap) 0;
}

 /*ARGSUSED*/ static void
FreeWidgetPixmap(w, closure, calldata)
	Widget w;
...
static void
createList(list_seq, top_mess, lower_mess)
	str_seq list_seq;
	char *top_mess, *lower_mess;
{
	int count = 0;
	Widget shell, swindow, ListForm, ListWindow, closeButton, keepButton, ListTitleView;
	Arg args[MAXARGS];

	if (curr_list_popup) {
		if (!XtIsManaged(curr_list_popup))
			XtManageChild(curr_list_popup);
		shell = curr_list_popup;

		ListForm = XtNameToWidget(shell, "ListForm");
		swindow = XtNameToWidget(shell, "ListForm.ListScrolledWindow");
		ListWindow = XtNameToWidget(shell, "ListForm.ListScrolledWindow.ListWindow");
		...
		count = 0;
		ListWindow = XtCreateWidget("ListWindow", formWidgetClass, swindow, args, count);
		...
		count++;
		XtSetValues(XtNameToWidget(shell, "ListForm.ListTitleView.listTitle"), args, count);

		count = 0;
		XtSetArg(args[count], XtNlabel, lower_mess);
		count++;
		XtSetValues(XtNameToWidget(shell, "ListForm.ListMessage"), args, count);
	...
	count = 0;
	shell = XtCreatePopupShell("ListOutput", topLevelShellWidgetClass, toplevel, args, 0);
	count = 0;
	ListForm = XtCreateManagedWidget("ListForm", formWidgetClass, shell, args, count);
	...
	closeButton = XtCreateManagedWidget("closeButton",
					    commandWidgetClass, ListForm, args, count);

	XtAddCallback(closeButton, XtNcallback, (XtCallbackProc) destroyList, (XtPointer) shell);
	...
	count++;
	keepButton = XtCreateManagedWidget("keepButton", commandWidgetClass, ListForm, args, count);

	XtAddCallback(keepButton, XtNcallback, (XtCallbackProc) keepList, (XtPointer) shell);
	...
	XtSetArg(args[count], XtNlabel, top_mess);
	count++;
	(void) XtCreateManagedWidget("listTitle", labelWidgetClass, ListTitleView, args, count);
	...
	count = 0;
	ListWindow = XtCreateManagedWidget("ListWindow", formWidgetClass, swindow, args, count);

	count = 0;
	XtSetArg(args[count], XtNlabel, lower_mess);
	count++;
	(void) XtCreateManagedWidget("ListMessage", labelWidgetClass, ListForm, args, count);

	XtAddCallback(ListWindow, XtNdestroyCallback, ListDestroy, (XtPointer) list_seq);
	...
	Widget parent;
{
	...
	count = 0;
	PosForm = XtCreateManagedWidget("PosForm", formWidgetClass, parent, args, count);
	count = 0;
	title = XtCreateManagedWidget("PosTitle", commandWidgetClass, PosForm, args, count);
	...
	count = 0;
	(void) XtCreateManagedWidget("PosWindow", formWidgetClass, swindow, args, count);
}
...
static char defaultTranslations[] = "<Key>Return: TSearch() \n\
   Ctrl<Key>M:  TSearch() \n\
   Ctrl<Key>O:  TSearch() \n\
   Ctrl<Key>J:  TSearch() \n\
	...
	Widget parent;
{
	...
	count = 0;
	TypeForm = XtCreateManagedWidget("TypeForm", formWidgetClass, parent, args, count);

	count = 0;
	XtSetArg(args[count], XtNlabel, "Searching For");
	count++;
	(void) XtCreateManagedWidget("SearchValLabel", labelWidgetClass, TypeForm, args, count);

	count = 0;
	XtSetArg(args[count], XtNeditType, XawtextEdit);
	count++;
	SearchVal = XtCreateManagedWidget("SearchVal", asciiTextWidgetClass, TypeForm, args, count);
	...
	count++;
	(void) XtCreateManagedWidget("TypeButton", menuButtonWidgetClass, TypeForm, args, count);
	...
	int seqnum;
{
	...
	history_display = XtNameToWidget(history_form, "ListScrolledWindow.ListWindow");
	...
	count = 0;
	history_display = XtCreateWidget("ListWindow", formWidgetClass, scrolwin, args, count);
	...
	Widget PosWindow;
{
	...
	char *trans_start_btn2 = "None <Btn2Up>:  set() DnList(",
	    *trans_start_btn3 = "None <Btn3Up>:  set() DnMove(",
	    translations[STRINGLEN], trans_buf[SMALLSTRING];
	Dimension width;
	int count;
	Arg args[MAXARGS];
	...
	XtSetArg(args[count], XtNwidth, width);
	count++;
	rdn_window = XtCreateManagedWidget("x", commandWidgetClass, PosWindow, args, count);
 ...
 /*ARGSUSED*/ static void
StartSearch(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
	Arg args[MAXARGS];
	...
	if (mvalue[0] == '\0') {
		kill_message();
		displayError(outer, "You have not specified a search value!\n\
Click on this window to continue");
	...
	if (index(mvalue, '=') == NULLCP)
		...
		(void) sprintf(mess, "Searching with filter `%s'.\nPlease stand by....", mvalue);
	...
	case Okay:
		if (entry_number == 1) {
			...
			if (!is_dit_leaf(entry)) {
				...
				if (curr_read_popup == 0)
					curr_read_popup = createReadPopup();
	...
	Widget parent;
{
	...
	menu_mgr = XtCreatePopupShell("menu", simpleMenuWidgetClass, parent, args, 0);
	...
	while (av_typeindx[n] != -1) {
		...
		button = XtCreateManagedWidget((String) filttype[av_typeindx[n]],
					       smeBSBObjectClass, menu_mgr, args, count);
	...
	XtPointer indx, calldata;
{
	...
	if ((int) indx < 0 || (int) indx >= (int) filt_num)
		return;
	...
	Widget parent;
{
	...
	count = 0;
	form = XtCreateManagedWidget("MainButtonForm", formWidgetClass, parent, args, count);
	...
	count++;
	quitButton = XtCreateManagedWidget("quitButton", commandWidgetClass, form, args, count);

	XtAddCallback(quitButton, XtNcallback, (XtCallbackProc) Quit, (XtPointer) 0);
	...
	count++;
	helpButton = XtCreateManagedWidget("helpButton", commandWidgetClass, form, args, count);

	XtAddCallback(helpButton, XtNcallback, (XtCallbackProc) Help, (XtPointer) 0);
	...
	count++;
	searchButton = XtCreateManagedWidget("searchButton", commandWidgetClass, form, args, count);

	XtAddCallback(searchButton, XtNcallback, (XtCallbackProc) StartSearch, (XtPointer) 0);
	...
	count++;
	listButton = XtCreateManagedWidget("listButton", commandWidgetClass, form, args, count);

	XtAddCallback(listButton, XtNcallback, (XtCallbackProc) List, (XtPointer) 0);
	...
	historyButton = XtCreateManagedWidget("historyButton",
					      commandWidgetClass, form, args, count);

	XtAddCallback(historyButton, XtNcallback, (XtCallbackProc) popupHistory, (XtPointer) 0);
	...
	char *entry_name;
{
	...
	if (curr_read_popup) {
		...
		if (*object != '\0')
			...
			XtAddCallback(modifyButton, XtNcallback, cannotModify, (XtPointer) NULL);

		XtAddCallback(modifyButton, XtNdestroyCallback, freeSpace, (XtPointer) object);
	}
}

 /*ARGSUSED*/ static void
cannotModify(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
...
createReadPopup()
{
	...
	Widget closeButton, keepButton, modifyButton, showAllButton,
	    TextScrolledWindow, AttrWindow, titleView, TextForm, ReadPopup, ReadForm, ButtonForm;

	ReadPopup = XtCreatePopupShell("Directory Entry", topLevelShellWidgetClass,
				       toplevel, args, 0);

	/* Top of popup form. */
	ReadForm = XtCreateWidget("ReadForm", formWidgetClass, ReadPopup, args, 0);

	/* Buttons */
	ButtonForm = XtCreateManagedWidget("ReadButtonForm", formWidgetClass, ReadForm, args, 0);
	...
	XtSetArg(args[count], XtNallowHoriz, TRUE);
	count++;
	titleView = XtCreateManagedWidget("titleView", viewportWidgetClass, ReadForm, args, count);

	count = 0;
	XtSetArg(args[count], XtNlabel, friendly_base_path);
	count++;
	(void) XtCreateManagedWidget("readTitle", commandWidgetClass, titleView, args, count);

	count = 0;
	TextScrolledWindow = XtCreateManagedWidget("TextScrolledWindow",
						   viewportWidgetClass, ReadForm, args, count);
	...
	XtSetArg(args[count], XtNstring, "");
	count++;
	(void) XtCreateManagedWidget("SepWindow", asciiTextWidgetClass, TextForm, args, count);
	...
	XtSetArg(args[count], XtNstring, "");
	count++;
	(void) XtCreateManagedWidget("ValWindow", asciiTextWidgetClass, TextForm, args, count);
	...
	CreateBackgroundPixmap(showAllButton, ShowAll_bits, ShowAll_width, ShowAll_height);

	CreateBackgroundPixmap(modifyButton, Modify_bits, Modify_width, Modify_height);

	if (DefaultDepthOfScreen(screen) == 1) {
		CreateBackgroundPixmap(ReadForm, gray_bits, gray_width, gray_height);

		CreateBackgroundPixmap(ButtonForm, gray_bits, gray_width, gray_height);
	}
...
static void
AddNewList(list_widget, list_seq, list_size)
	Widget list_widget;
	str_seq list_seq;
	unsigned int list_size;
{
	...
	char *trans_start_btn2 = "None <Btn2Up>:  set() ListSelectList(",
	    *trans_start_btn3 = "None <Btn3Up>:  set() ListSelectMove(", translations[STRINGLEN];
	XtTranslations trans_table;
	...
	*name = '\0';
	for (n = 0; n < element_number; n++) {
		...
		count = 0;
		XtSetArg(args[count], XtNlabel, (*friendly_rdn ? friendly_rdn : "The World"));
		count++;
		XtSetArg(args[count], XtNborderWidth, 0);
		count++;
		XtSetArg(args[count], XtNborderColor, WhitePixelOfScreen(screen));
		...
		element = XtCreateManagedWidget((String) name,
						commandWidgetClass, list_widget, args, count);
	...
	char *entry_ptr;
{
	...
	while (str_end != NULLCP && *str_end != '\0') {
		...
		if (*str_end == '\t') {
			...
			*str_end = '\0';
			...
			if (do_mail_reverse == TRUE)
				rfc2greybook(str_start);
			...
			*str_end = '\0';
			...
			if (attr_width > max_attr_width)
				max_attr_width = attr_width;
	...
	if (entry_ptr == NULLCP) {
		...
		max_attr_width = XTextWidth(font, attrs, strlen(attrs)) + FONTWIDTH(font);
	}
	...
	XtPointer closure, calldata;
{
	...
	case timelimit_w_partial:
		...
		if (strlen(base_path) > 3) {
			indx = 0;
			while (friendly_base_path[indx] != ',' && friendly_base_path[indx] != '\0')
				indx++;
			friendly_base_path[indx] = '\0';
			(void) strcat(mess, (char *) friendly_base_path);
		} else
		...
		case adminlimit_w_partial:
			(void) sprintf(string,
				       "%s. %s. %d %s.",
				       "Administrative limit reached",
				       "Partial results only", entry_number, "items displayed");
			break;
		case timelimit_w_partial:
			(void) sprintf(string,
				       "%s. %s. %d %s.",
	...
	XtPointer object, calldata;
{
 ...
 /*ARGSUSED*/ static void
ListSelectMove(w, event, params, num_params)
	Widget w;
	XEvent *event;
	String *params;
	Cardinal num_params;
...
Click on this window to continue");
 ...
 /*ARGSUSED*/ static void
ListSelectList(w, event, params, num_params)
	Widget w;
	XEvent *event;
	String *params;
	Cardinal num_params;
...
Click on this window to continue");
 ...
 /*ARGSUSED*/ static void
DnList(w, event, params, num_params)
	Widget w;
	XEvent *event;
	String *params;
	Cardinal num_params;
{
 ...
 /*ARGSUSED*/ static void
DnMove(w, event, params, num_params)
	Widget w;
	XEvent *event;
	String *params;
	Cardinal num_params;
{
 ...
 /*ARGSUSED*/ static void
DnMoveRead(w, rdnlevel, calldata)
	Widget w;
...
CreateMessagePopup()
{
	...
	standby = XtCreatePopupShell("POD: Working.",
				     transientShellWidgetClass, toplevel, args, count);

	count = 0;
	XtSetArg(args[count], XtNresize, FALSE);
	count++;
	(void) XtCreateManagedWidget("standby", labelWidgetClass, standby, args, count);
...
CreateVersionPopup()
{
	...
	char *version = "Version 2.4 29th May 1991",
	    *title_string = "Pod - POpup Directory", *author_string = "by\n\
...
Brunel University";
	...
	count = 0;
	text_form = XtCreateManagedWidget("verForm", formWidgetClass, version_popup, args, count);
	...
	XtSetArg(args[count], XtNresizable, TRUE);
	count++;
	title = XtCreateManagedWidget("verTitle", labelWidgetClass, text_form, args, count);
	...
	XtSetArg(args[count], XtNresizable, TRUE);
	count++;
	version_label = XtCreateManagedWidget("verLabel", labelWidgetClass, text_form, args, count);
	...
	XtSetArg(args[count], XtNresizable, TRUE);
	count++;
	piccy = XtCreateManagedWidget("teamPic", labelWidgetClass, text_form, args, count);
	...
	XtSetArg(args[count], XtNresizable, TRUE);
	count++;
	(void) XtCreateManagedWidget("authLabel", labelWidgetClass, text_form, args, count);
...
/* ARGSUSED */
static void
ShowVersion(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
...
/* ARGSUSED */
static void
HideVersion(w, event, params, num_params)
	Widget w;
	XEvent *event;
	String *params;
	Cardinal num_params;
...
CreateErrorPopup()
{
	...
	XtSetArg(args[count], XtNresize, FALSE);
	count++;
	text = XtCreateManagedWidget("error_popup", commandWidgetClass, error_popup, args, count);
	...
	dsEnqError status;
{
	...
	switch (status) {
	case timelimit:
		displayError(outer, "Time limit reached.\nClick on this window to continue");
		break;
	case adminlimit:
		displayError
		    (outer, "Administrative limit reached.\nClick on this window to continue");
		break;
	case nothingfound:
		dnseq = 0;
		displayError(outer, "Nothing found!\nClick on this window to continue");
		break;
	case localdsaerror:
		dnseq = 0;
		displayError
	...
	case duaerror:
		dnseq = 0;
		displayError(outer, "Internal error. Sorry!\nClick on this window to continue");
		break;
	case security:
		dnseq = 0;
		displayError(outer, "You do not have the access privileges required\n\
to make this request!\nClick on this window to continue");
	...
	case attributerror:
		dnseq = 0;
		displayError
		    (outer, "Faulty data found in database!\nClick on this window to continue");
		break;
	case serviceerror:
		dnseq = 0;
		displayError(outer, "Directory Service Error!\nClick on this window to continue");
		break;
	case updaterror:
		dnseq = 0;
		displayError(outer, "Update Error!\nClick on this window to continue");
		break;
...
CreateHelpPopup()
{
	...
	popup_quit_button = XtCreateManagedWidget("popup_help_quit",
						  commandWidgetClass, popup_help_form, args, count);
	...
	(void) XtCreateManagedWidget("popup_help_text",
				     asciiTextWidgetClass, popup_help_scrolwin, args, count);
	XtRealizeWidget(popup_help);

	CreateBackgroundPixmap(popup_help_button, About_bits, About_width, About_height);

	CreateBackgroundPixmap(popup_quit_button, Close_bits, Close_width, Close_height);

	if (DefaultDepthOfScreen(screen) == 1) {
		CreateBackgroundPixmap(popup_help_form, gray_bits, gray_width, gray_height);
	}
}

 /*ARGSUSED*/ static void
Help(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
	Widget help_popup;
 ...
 /*ARGSUSED*/ static void
QuitFromHelp(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
	Widget help_popup;
	...
	XtPointer closure, calldata;
{
	...
	scrolwin = XtNameToWidget(toplevel, "Pod Help Screen.popup_help_form.popup_help_scrolwin");
...
static void
CreateBackgroundPixmap(widget, bits, width, height)
	Widget widget;
	char bits[];
	Cardinal width, height;
{
	...
	XtAddCallback(widget, XtNdestroyCallback, FreeWidgetPixmap, (XtPointer) bitmap);
}
	...
	XtPointer calldata, list_seq;
{
 ...
 /*ARGSUSED*/ static void
destroyList(w, shellwidget, calldata)
	Widget w;
	XtPointer shellwidget, calldata;
{
	if (curr_list_popup == (Widget) shellwidget)
		curr_list_popup = 0;
 ...
 /*ARGSUSED*/ static void
keepList(w, shellwidget, calldata)
	Widget w;
...
static void
CreateHistoryPopup(list_seq, mess)
	str_seq list_seq;
	char *mess;
{
	int count = 0;
	...
	count = 0;
	shell = XtCreatePopupShell("Session History", topLevelShellWidgetClass, toplevel, args, 0);
	count = 0;
	HistoryForm = XtCreateManagedWidget("HistoryForm", formWidgetClass, shell, args, count);
	...
	count++;
	closeButton = XtCreateManagedWidget("closeButton",
					    commandWidgetClass, HistoryForm, args, count);

	XtAddCallback(closeButton, XtNcallback, (XtCallbackProc) popdownHistory, (XtPointer) shell);

	count = 0;
	XtSetArg(args[count], XtNlabel, "History Window");
	count++;
	(void) XtCreateManagedWidget("HistoryTitle", labelWidgetClass, HistoryForm, args, count);
	...
	XtSetArg(args[count], XtNlabel, mess);
	count++;
	(void) XtCreateManagedWidget("ListMessage", labelWidgetClass, HistoryForm, args, count);
 ...
 /*ARGSUSED*/ static void
popupHistory(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
 ...
 /*ARGSUSED*/ static void
popdownHistory(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
	Widget history_popup;
	...
	Cardinal num_params;
{
	...
	if (!strcmp(buttonName, "search"))
		CreateBackgroundPixmap(w, SearchPress_bits, SearchPress_width, SearchPress_height);
	else if (!strcmp(buttonName, "help"))
		CreateBackgroundPixmap(w, HelpPress_bits, HelpPress_width, HelpPress_height);
	else if (!strcmp(buttonName, "quit"))
		CreateBackgroundPixmap(w, QuitPress_bits, QuitPress_width, QuitPress_height);
	else if (!strcmp(buttonName, "list"))
		CreateBackgroundPixmap(w, ListPress_bits, ListPress_width, ListPress_height);
		...
		CreateBackgroundPixmap(w, ClosePress_bits, ClosePress_width, ClosePress_height);
	else if (!strcmp(buttonName, "about"))
		CreateBackgroundPixmap(w, AboutPress_bits, AboutPress_width, AboutPress_height);
	else if (!strcmp(buttonName, "keep"))
		CreateBackgroundPixmap(w, KeepPress_bits, KeepPress_width, KeepPress_height);
	else if (!strcmp(buttonName, "modify"))
		CreateBackgroundPixmap(w, ModifyPress_bits, ModifyPress_width, ModifyPress_height);
	...
	if (!strcmp(buttonName, "search"))
		CreateBackgroundPixmap(w, Search_bits, Search_width, Search_height);
	else if (!strcmp(buttonName, "help"))
		CreateBackgroundPixmap(w, Help_bits, Help_width, Help_height);
	else if (!strcmp(buttonName, "quit"))
		CreateBackgroundPixmap(w, Quit_bits, Quit_width, Quit_height);
	else if (!strcmp(buttonName, "list"))
		CreateBackgroundPixmap(w, List_bits, List_width, List_height);
	else if (!strcmp(buttonName, "history"))
		CreateBackgroundPixmap(w, History_bits, History_width, History_height);
	else if (!strcmp(buttonName, "close"))
		CreateBackgroundPixmap(w, Close_bits, Close_width, Close_height);
	else if (!strcmp(buttonName, "about"))
		CreateBackgroundPixmap(w, About_bits, About_width, About_height);
	else if (!strcmp(buttonName, "keep"))
		CreateBackgroundPixmap(w, Kept_bits, Kept_width, Kept_height);
	else if (!strcmp(buttonName, "modify"))
		CreateBackgroundPixmap(w, Modify_bits, Modify_width, Modify_height);
	else if (!strcmp(buttonName, "show_all"))
		CreateBackgroundPixmap(w, ShowAll_bits, ShowAll_width, ShowAll_height);
	else if (!strcmp(buttonName, "submit_modify"))
		CreateBackgroundPixmap(w, Modify_bits, Modify_width, Modify_height);
 ...
 /*ARGSUSED*/ static void
readDestroy(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
	Widget readForm = XtNameToWidget((Widget) closure, "ReadForm");

	PhotoWindow = XtNameToWidget(readForm, "TextScrolledWindow.TextForm.PhotoWindow");

	if (PhotoWindow != (Widget) 0)
		kill_photo();
	...
	XtPointer object, calldata;
{
 ...
 /*ARGSUSED*/ static void
closeModify(w, clientdata, calldata)
	Widget w;
	XtPointer clientdata, calldata;
{
	...
	if (modifyPopup == curr_modify_popup)
		return;
	...
	if (modifyPopup == curr_modify_popup)
		curr_modify_popup = (Widget) 0;
}

 /*ARGSUSED*/ static void
keepModify(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
 ...
 /*ARGSUSED*/ static void
modUpdate(w, attrValue, calldata)
	Widget w;
	XtPointer attrValue, calldata;
{
 ...
 /*ARGSUSED*/ static void
submitModif(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
	...
	XtPointer closure, calldata;
{
	...
	if (curr_val && nextValWidg) {
		...
		if (curr_attr) {
			...
			while (!(nextValWidg = curr_val->text_widg))
				curr_val = curr_val->next;
		...
		curr_val = this_attr->val_seq;
		while (curr_val && !curr_val->text_widg)
			curr_val = curr_val->next;
	...
	if (nextValWidg) {
		...
		if (nextAttrWidg)
			XtSetValues(nextAttrWidg, args, count);
	}
 ...
 /*ARGSUSED*/ static void
addValField(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
	...
	count++;
	modValButton = XtCreateWidget("modValMenu", menuButtonWidgetClass, form, args, count);

	count = 0;
	modValMenu = XtCreatePopupShell("menu", simpleMenuWidgetClass, modValButton, args, count);

	count = 0;
	XtSetArg(args[count], XtNlabel, "Delete this value");
	count++;
	modValMenuButton = XtCreateManagedWidget("delValMenuButton",
						 smeBSBObjectClass, modValMenu, args, count);

	XtAddCallback(modValMenuButton, XtNcallback, deleteVal, (XtPointer) new_val);
	...
	XtAddCallback(modValMenuButton, XtNcallback, UndoValChanges, (XtPointer) new_val);
	...
	XtSetArg(args[count], XtNvertDistance, 10);
	count++;
	newValWidg = XtCreateWidget("valText", asciiTextWidgetClass, form, args, count);
	...
	CreateBackgroundPixmap(modValButton, menuicon_bits, menuicon_width, menuicon_height);

	XawFormDoLayout(form, TRUE);
}

 /*ARGSUSED*/ static void
UndoAttrChanges(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
 ...
 /*ARGSUSED*/ static void
UndoValChanges(w, closure, calldata)
	Widget w;
	XtPointer closure, calldata;
{
	int count;
	...
	Widget thisValWidg, thisValMenuWidg, thisAttrWidg,
	    ...
	    nextValWidg, nextValMenuWidg, nextAttrWidg, modValMenu, modValMenuButton, form;
	int vertDistance;
	modVals this_val = (modVals) closure, next_val, prev_val;
	dirAttrs this_attr = this_val->attr, next_attr;

	if (!this_val->mod_flag && this_val->value)
		return;
	...
	if (this_val->value && *this_val->value != '\0') {
		if (!thisValWidg) {
			...
			while (prev_val && prev_val != this_val) {
				if (prev_val->text_widg)
					prevValWidg = prev_val->text_widg;
			...
			if (!prevValWidg) {
				...
				while (next_val && !(next_val->text_widg))
			...
			count = 0;
			XtSetArg(args[count],
				 XtNwidth, (Dimension) GetTextWidth(thisValWidg, this_val->value));
			count++;
			XtSetArg(args[count],
				 XtNheight,
			...
			count = 0;
			XtSetArg(args[count], XtNwidth, GetTextWidth(thisValWidg, this_val->value));
			count++;
			XtSetArg(args[count],
				 XtNheight, GetTextHeight(thisValWidg, this_val->value));
			count++;
			XtSetArg(args[count], XtNstring, this_val->value);
			count++;
			XtSetValues(thisValWidg, args, count);
...
static dirEntry
createModifyTemplate(modify_form, entry_name)
	Widget modify_form;
	char *entry_name;
{
	...
	modifyScrolledWindow = XtNameToWidget(modify_form, "modifyForm.modifyScrolledWindow");
	...
	for (curr_attrs = entry->attrs; curr_attrs; curr_attrs = curr_attrs->next) {
		...
		XtAddCallback(modAttrMenuButton, XtNcallback, addValField, (XtPointer) curr_attrs);
		...
		for (curr_val = curr_attrs->val_seq; curr_val; curr_val = curr_val->next) {
			count = 0;
	...
	if (curr_modify_popup) {
		for (max_width = 0, curr_attrs = entry->attrs;
		     curr_attrs; curr_attrs = curr_attrs->next) {
			count = 0;
			XtSetArg(args[count], XtNfromHoriz, &modValButton);
			count++;
		...
		XtAddCallback(modifyButton, XtNdestroyCallback, freeEntry, (XtPointer) entry);
	...
	char *entry_name;
{
	...
	count = 0;
	modifyForm = XtCreateManagedWidget("modifyForm", formWidgetClass, modifyPopup, args, count);
	...
	XtAddCallback(closeButton, XtNcallback, closeModify, (XtPointer) modifyPopup);
	...
	(void) XtCreateManagedWidget("modifyScrolledWindow",
				     viewportWidgetClass, modifyForm, args, count);
	...
	for (max_width = 0, curr_attrs = entry->attrs; curr_attrs; curr_attrs = curr_attrs->next) {
		count = 0;
		XtSetArg(args[count], XtNfromHoriz, &modValButton);
		count++;
		XtGetValues(curr_attrs->val_seq->text_widg, args, count);
	...
	if (DefaultDepthOfScreen(screen) == 1) {
		CreateBackgroundPixmap(ButtonForm, gray_bits, gray_width, gray_height);
		CreateBackgroundPixmap(modifyForm, gray_bits, gray_width, gray_height);
	}
	...
	char *string;
{
	...
	if (!string || *string == '\0')
		return (FONTHEIGHT(font) + font->ascent);
...
ConvSel(w, selection, target, type, value, length, format)
	Widget w;
...
static void
PopupMessage(shell, refto, label_widget, label, grab_kind)
	...
	XtGrabKind grab_kind;
{
	...
	if (refto == (Widget) 0 || !XtIsRealized(refto))
		XtMoveWidget(shell, 0, 0);
	else {
	...
	XFillRectangle(dpy, border_pixmap, gc, 0, 0, (unsigned int) width, (unsigned int) height);

	...
	for (attrs = mods->attrs; attrs; attrs = attrs->next) {
		if (attrs->mod_flag) {
			...
			for (curr_val = attrs->val_seq;
			     curr_val && curr_val->mod_flag; curr_val = curr_val->next) ;
			...
			if (!curr_val) {
				...
				if (attrs->in_flag) {
					for (curr_val = attrs->val_seq;
					     curr_val; curr_val = curr_val->next) {
					...
					if (attrVal_seq) {
						curr_mod->em_what =
						    as_comp_new(AttrT_cpy(attr_type), attrVal_seq,
								NULLACL_INFO);
						entrymods = ems_append(entrymods, curr_mod);
						attrVal_seq = 0;
						curr_mod = 0;
	...
	*attrs = 0;
	...
	for (as = read_attrs; as != NULLATTR; as = as->attr_link)
		if (!(AS_SYNTAX(as) == str2syntax("schema") ||
			...
			nas = as_comp_new(AttrT_cpy(as->attr_type),
					  avs_cpy(as->attr_value), NULLACL_INFO);
			if (tas)
	...
	for (; as != NULLATTR; as = as->attr_link)
		if (!(AS_SYNTAX(as) == str2syntax("schema") ||
			...
			if (templ_as)
				templ_as = as_merge(templ_as, nas);
			else
				templ_as = nas;
		}
	...
	dirAttrs *attrs;
{
	...
	while (1) {
		...
		*str = '\0';
		...
		if (mailformat == greybook && !lexequ(curr_attr->attr_name, "mailbox")) {
	...
	AttributeType at;
{
	...
	if (str2syntax("dn") == at->oa_syntax) {
		if (*value == '@')
			(void) strcpy(buffer, (char *) (value + 1));
		return;
	}
	...
	Attr_Sequence as;
{
	...
	while (as != NULLATTR) {
		...
		if (as->attr_value)
			with_vals = as_merge(with_vals, as);
		else
			without_vals = as_merge(without_vals, as);

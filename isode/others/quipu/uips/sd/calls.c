...
void quit(), int_quit();
void help_back(), help_list(), help_up(), help_number(), help_srch(), help_cncs();
void setwidgets();
...
user_tailor()
{
	...
	char *p, *TidyString(), *SkipSpace(), *end;
	...
	if (testing) {
		...
		if (!(config_directory = opendir(type_path))) {
			(void) fprintf(stderr,
				       "File error! Test sd should be run from source directory!\n");
			int_quit(1);
		}
	} else {
		if (!(config_directory = opendir(type_path))) {
	...
	while (dir_ent = readdir(config_directory)) {
		if (!(strncmp(dir_ent->d_name, "Type_", 5))) {
			file_names[filt_num] = (char *) malloc((unsigned)
	...
	if (testing) {
		...
		if (!(config_file = fopen(read_path, "r"))) {
			(void) fprintf(stderr,
				       "File error! Test sd must be run from source directory!\n");
			int_quit(1);
		}
	} else {
		if (!(config_file = fopen(read_path, "r"))) {
...
list_start()
{
	...
	search_arg.sra_baseobject = (*base_path != 'T' ? str2dn(base_path) : NULLDN);
	...
	if (search_arg.sra_filter->flt_un.flt_un_filter->flt_un.flt_un_item.
		...
		if (result.CSR_entries != NULLENTRYINFO) {
			...
			if (entry_number)
				curr_dnseq = SortList(curr_dnseq);
			scrollbar('\0');
		...
		if (result.CSR_limitproblem != LSR_NOLIMITPROBLEM) {
			...
			case LSR_ADMINSIZEEXCEEDED:
				if (entry_number > 0)
					xprint
					    ("(Administrative limit reached. Partial results only.)");
				else {
					free_seq(curr_dnseq);
					curr_dnseq = NULLDS;
...
srch_start()
{
	...
	search_arg.sra_common.ca_servicecontrol.svc_options = SVC_OPT_PREFERCHAIN;
	curr_rdn = search_arg.sra_baseobject = (*base_path != 'T' ? str2dn(base_path) : NULLDN);
	...
	caddr_t ptr;
{
	...
	*ps->ps_ptr = 0;
	...
	str = buffer;
	do {
		for (count = 0, sptr = str; *sptr != '\0' && count < 100; sptr++, count++) ;
		save = *sptr;
		*sptr = 0;
		xprint(str);
		*sptr = save;
	...
	char command;
{
	...
	if (command == '[') {
		...
		for (count = 0; (current_entry - count) > 1 && count < text_height / 2; count++) ;
	...
	if (text_state == DN_LIST)
		if (current_entry >= entry_number)
			xprint("<List finished>");
		else
			xprintint("<Total of %d entries>", entry_number);
	...
	char *name;
{
	...
	if (ds_list(&list_arg, &list_error, &list_result) != DS_OK)
		...
		if (list_result.lsr_subordinates == NULLSUBORD)
			return (1);
		else
			return (0);
	}
	...
	int size;
{
	...
	if ((ps = ps_alloc(str_open)) == NULLPS)
		return;
	if (str_setup(ps, cptr, size, 1) == NOTOK)
		return;
	...
	char *substr;
{
	...
	while (1) {
		...
		for (count = 0; count >= 0 && count < substrlen; count++) {
			if (sptr[count] == '\0')
				return (0);
			else if (substr[count] != sptr[count])
				count = -2;
		}
		if (count == substrlen)
			return (1);

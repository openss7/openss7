...
make_attr_filter()
{
	int match_type;
	char attr_name[1024],
	    attr_val[1024], sub_val_initial[1024], sub_val_final[1024], sub_val_any[1024];
	...
	*end = save;
	...
	case EQUAL:
		...
		if (!sfilt->flt_un.flt_un_item.fi_un.fi_un_ava.ava_type ||
		    sfilt->flt_un.flt_un_item.fi_un.fi_un_ava.ava_type->oa_syntax == 0) {
			cleartext();
	...
	case SUBSTRING:
		...
		sfilt->flt_un.flt_un_item.fi_un.fi_un_substrings.fi_sub_type = AttrT_new(attr_name);
		...
		if (sub_val_initial[0] != '*')
			if ((sfilt->flt_un.flt_un_item.fi_un.fi_un_substrings.fi_sub_initial =
			     avs_comp_new(str2AttrV(sub_val_initial,
						    ...
						    fi_un_substrings.fi_sub_type->oa_syntax)))
			    == NULLAV) {
				cleartext();
				tprint
				    ("Error: Cannot search, invalid value '%s' for attribute type '%s'.\n",
	...
	filt_struct *filt;
{
	...
	case ITEM:
		...
		case SUBSTRING:
			...
			if (sfilt->flt_un.flt_un_item.fi_un.fi_un_substrings.fi_sub_any == NULL) {
				rfilt = sfilt->flt_next;
				sfilt->flt_next = NULLFILTER;
				filter_free(sfilt);
				return rfilt;

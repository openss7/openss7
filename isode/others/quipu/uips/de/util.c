...
void
printLastComponent(indent, dnstr, objectType, printNumber)
	int indent;
	char *dnstr;
	int objectType;
	int printNumber;
	...
	char *str;
{
	if (limitProblem == TRUE) {
		...
		pageprint
		    ("listing of all the entries in the Directory beyond this point.  Try and\n");
		if (strcmp(str, "*") == 0) {
			pageprint
			    ("guess the name of the entry you want.  The directory may find the entry\n");
		...
		if (notAllReached == TRUE) {
			...
			pageprint
			    ("to complete this search - part of the database is currently inaccessible.\n");
...
void
logListSuccess(outcome, objecttype, noMatches)
	char *outcome;
	char *objecttype;
	int noMatches;
{

...
char origDefaultCo[LINESIZE], origDefaultOrg[LINESIZE],
    ...
    matched[LINESIZE], callingDteNumber[LINESIZE];
...
/*  pdu_dump_init("/tmp"); */
	...
	/* exit from this loop (and thus from the program) is handled by the enterString routine */
	for (;;) {
		(void) setjmp(sjbuf);
		highNumber = 0;
		abandoned = FALSE;
		...
		if ((exactMatch != ORG) ||
		    ((strcmp(qinfo[ORG].entered, qinfo[ORG].defvalue) != 0) &&
		     (exactMatch != ORG)) || (searchfail == TRUE)) {
		      entercountry:
			(void) enterString(COUNTRY);
		}

		/* look at the input strings and decide what sort of search or list to do */
	...
	char matchstring[];
{
	...
	for (;;) {
		...
		if (strlen(qinfo[ORG].entered) == 0) {
			...
			resetprint
			    ("an organisation name at the prompt, or press <CR> to start the\n");
		...
		if (qinfo[PERSON].listlen == 0) {
			...
			if (qinfo[PERSON].listlen > maxPersons) {
				if (strcmp(qinfo[PERSON].entered, "*") == 0)
					...
					matchFollowing();
				printNames(ORGUNIT);
				printListPRRs(qinfo[PERSON].entered, qinfo[PERSON].lp, searchparent,
					      FALSE);
	...
	int objectType;
{
	...
	case PERSON:
		...
		/* if a string was entered, but a number could validly have been entered, store
		   value in gotValue for next invocation of this function */
	...
	case ORGUNIT:
		...
		if (strcmp(qinfo[objectType].defvalue, "*") == 0)
			...
			if (strlen(qinfo[objectType].defvalue) == 0) {
				if (strlen(qinfo[PERSON].entered) != 0)
					(void) sprintf(prompt, "%s<CR> to search all depts, ",
						       prompt);
			} else {
	...
	case ORG:
		...
		(void) sprintf(prompt, "%s* to %s, ", prompt, browseMess ? "browse" : "list orgs");
		(void) sprintf(prompt, "%s? for help\n%s", prompt, prstr);
		enterAndValidate(prompt, qinfo[objectType].entered, objectType,
				 qinfo[objectType].defvalue, qinfo[objectType].lp, &numEnt);
		break;
	...
	stopUnbindTimer();
	if (lexnequ(qinfo[objectType].entered, "quit", strlen(qinfo[objectType].entered)) == 0) {
	...
	int *nep;
{
	...
	*nep = 0;
	...
	for (;;) {
		...
		/* if "-" entered, convert this to a null entry, unless entering a country.  In
		   this case, "-" is treated as search international orgs */
		...
		if (*cp == '?') {	/* help on some other specific topic requested */
			...
			if ((lexequ(cp, "?") == 0) || (lexequ(cp, "help") == 0))	/* help
											   about
											   help
											   requested 
											 */
		...
		/* if a number has been entered, check that it is in range, and map the number
...
countryCodeMessage(str)
{
	...
	(void)
	    printf
	    ("Either enter a valid two-letter code, or enter the country name more fully.\n\n",
	     str);
}
...
{
	(void) putchar('\n');
	/* simulate search failure - this ensures that the "country question" is asked */
	searchfail = TRUE;
	longjmp(sjbuf, 0);
}

	...
	char *name;
{
	...
	if (passno == 1) {
		...
		pixmap = XCreatePixmap(dpy, win, winW, winH, DefaultDepthOfScreen(scr));
		if (!pixmap) {
	...
	(void) sprintf(buff, name ? "%s" : "%s (%d)", name ? name : "Photo", getpid());
	XChangeProperty(dpy, win, XA_WM_NAME, XA_STRING, 8, PropModeReplace, buff, strlen(buff));
	XMapWindow(dpy, win);
			...
			if (buttons_down++) {
				if (!ignore_action++) {
					...
					XCopyArea(dpy, pixmap, win,
						  gc, 0, 0, PIC_LINESIZE, NUMLINES, 0, 0);
				}
			} else {
				XSetFunction(dpy, gc, GXxor);
				XFillRectangle(dpy, win, gc, 0, 0, PIC_LINESIZE, NUMLINES);
			}
			continue;
		case ButtonRelease:
			if (!buttons_down)

...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/snmp/RCS/eval.c,v 9.0 1992/06/16 12:38:11 isode Rel";
#endif
	...
	int offset;
{
	...
	case type_SNMP_PDUs_get__request:
		if (oid->oid_nelem != ot->ot_name->oid_nelem + 1)
			return int_SNMP_error__status_noSuchName;
		if ((ifnum = oid->oid_elements[oid->oid_nelem - 1]) == 0 || ifnum > exprNumber)
			return int_SNMP_error__status_noSuchName;
	...
	register struct expr *e;
{
	...
	if (str_setup(ps, e->e_expr, e->e_size, 1) == NOTOK) {
		e->e_status = E_other;
		(void) sprintf(e->e_hints, "str_setup failed: %s", ps_error(ps->ps_errno));
		return NOTOK;
	}
	...
	PElementLen *len;
{
	...
	if (ps_read_id(ps, 0, class, form, id) == NOTOK) {
		e->e_status = E_other;
		(void) sprintf(e->e_hints, "error reading TAG info: %s", ps_error(ps->ps_errno));
		return NOTOK;
	}
	if (ps_read_len(ps, len) == NOTOK) {
		e->e_status = E_other;
		(void) sprintf(e->e_hints, "error reading LEN info: %s", ps_error(ps->ps_errno));
		return NOTOK;
	}
...
static int
read_long(e, base, len, form, result)
	struct expr *e;
	char *base;
	int len;
	PElementForm form;
	...
	OID *ox;
{
	...
	if (form != PE_FORM_PRIM || (dp = (PElementData) base) == NULLPED || len == 0) {
		e->e_status = E_other;
	...
	integer *i;
{
	...
	if ((ot = oi->oi_type) == NULL) {
		e->e_status = E_noSuchName;
		(void) sprintf(e->e_hints, "no object type for variable %s", oid2ode(oid));
		return NOTOK;
	}
	if ((os = ot->ot_syntax) == NULL) {
		e->e_status = E_noSuchName;
		(void) sprintf(e->e_hints, "no object syntax for variable %s", oid2ode(oid));
		return NOTOK;
	}

	if (!os->os_data2) {
		e->e_status = E_notAnInteger;
		(void) sprintf(e->e_hints, "variable %s is not integer-valued", oid2ode(oid));
		return NOTOK;
	}

	if (ot->ot_getfnx == NULL) {
		e->e_status = E_noSuchName;
		(void) sprintf(e->e_hints, "no get method for variable %s", oid2ode(oid));
		return NOTOK;
	}

	if ((v = (struct type_SNMP_VarBind *) calloc(1, sizeof *v)) == NULL
	    || (v->name = oid_cpy(oid)) == NULL
	    || (v->value = pe_alloc(PE_CLASS_UNIV, PE_FORM_PRIM, PE_PRIM_NULL)) == NULL) {
		if (v)
			free_SNMP_VarBind(v);
		e->e_status = E_other;
	...
	int offset;
{
	...
	case type_SNMP_PDUs_rollback:
		if (oid->oid_nelem != ot->ot_name->oid_nelem + 1)
			return int_SNMP_error__status_noSuchName;
		if ((ifnum = oid->oid_elements[oid->oid_nelem - 1]) == 0 || ifnum > exprNumber + 1)
			return int_SNMP_error__status_noSuchName;
		e = exprs + ifnum - 1;
		break;
...
init_eval()
{
	...
	if (ot = text2obj("functSub"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functSub;
	if (ot = text2obj("functMul"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functMul;
	if (ot = text2obj("functDiv"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functDiv;
	if (ot = text2obj("functMod"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functMod;
	if (ot = text2obj("functNeg"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functNeg;
	if (ot = text2obj("functXch"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functXch;
	if (ot = text2obj("functClr"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functClr;
	if (ot = text2obj("functDup"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functDup;
	if (ot = text2obj("functPop"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functPop;
	if (ot = text2obj("functAbs"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functAbs;
	if (ot = text2obj("functSgn"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functSgn;
	if (ot = text2obj("functMin"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functMin;
	if (ot = text2obj("functAnd"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functAnd;
	if (ot = text2obj("functOr"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functOr;
	if (ot = text2obj("functNot"))
		ot->ot_getfnx = o_funct, ot->ot_info = (caddr_t) functNot;

	if (ot = text2obj("exprNumber"))
		ot->ot_getfnx = o_generic, ot->ot_info = (caddr_t) &exprNumber;
	if (ot = text2obj("exprIndex"))
		ot->ot_getfnx = o_expressions, ot->ot_info = (caddr_t) exprIndex;
	if (ot = text2obj("exprEval"))
		ot->ot_getfnx = o_expressions, ot->ot_info = (caddr_t) exprEval;
	if (ot = text2obj("exprExpr"))
		ot->ot_getfnx = o_expressions,
		    ot->ot_setfnx = s_expressions, ot->ot_info = (caddr_t) exprExpr;
	if (ot = text2obj("exprStatus"))
		ot->ot_getfnx = o_expressions, ot->ot_info = (caddr_t) exprStatus;
	if (ot = text2obj("exprHints"))
		ot->ot_getfnx = o_expressions, ot->ot_info = (caddr_t) exprHints;
}

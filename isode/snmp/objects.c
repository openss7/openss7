	...
	char *file;
{
	...
	if (file == NULL)
		file = "objects.defs";
	if ((fp = fopen(file, "r")) == NULL && (fp = fopen(cp = isodefile(file, 0), "r")) == NULL) {
		(void) sprintf(PY_pepy, "unable to read %s: %s", cp, sys_errname(errno));
		return NOTOK;
	}
	...
	if (once_only_Tbuckets == 0) {
		...
		compile_heap1 = NULLOT,
		    compile_heap2 = NULL, compile_heap3 = NULL, compile_heap4 = NULL;

		if (fgets(buffer, sizeof buffer, fp)
		    && strncmp(buffer, "--* compiled", sizeof "--* compiled" - 1) == 0) {
			int i, j, k;
			...
			for (ap = roots + (sizeof roots / sizeof roots[0]) - 1; ap >= roots; ap--) {
	...
	char *name, *value, *hash;
{
	...
	if ((ot->ot_text = strdup(name)) == NULL || (ot->ot_id = strdup(value)) == NULL) {
		(void) sprintf(PY_pepy, "out of memory");
		return NOTOK;
	}
	...
	char **vec;
{
	...
	if ((ot->ot_text = strdup(*vec++)) == NULL || (ot->ot_id = strdup(*vec++)) == NULL) {
		(void) sprintf(PY_pepy, "out of memory");
		return NOTOK;
	}
	...
	Tbuckets[i] = ot;
      ...
      get_rest:;
	if ((ot->ot_syntax = text2syn(*vec)) == NULL && strcmp(*vec, "Aggregate")
	    && debug)
		fprintf(stderr,
...
add_objects_aux()
{
	...
	for (ot = anchor; ot; ot = ot->ot_next) {
		...
		if (ot2) {
			...
			fprintf(stderr, "no distant parent for %s", sprintoid(ot->ot_name));
	}
	...
	char *name;
{
	...
	if (*cp == NULL)	/* name */
		...
		if ((i = str2elem(cp, elements)) < 2)
			goto free_up;
	} else /* name.numbers */ if ((i = str2elem(cp + 1, elements)) < 1)
		goto free_up;
	...
	register OT ot;
{
	...
	if (isdigit(*id)) {
		...
		bcopy((char *) ot2->ot_name->oid_elements,
		      (char *) oid->oid_elements, oid->oid_nelem * sizeof *elements);
	}
	...
	OID oid;
{
	...
	if (oid == NULLOID || oid->oid_nelem < 1 || (i = (ip = oid->oid_elements)[0])
	...
	while (ot) {
		...
		if (bcmp((char *) ip, (char *) (nm->oid_elements + i), (j - i) * sizeof *ip))
			ot = ot->ot_sibling;
		else if (oid->oid_nelem == j || ot->ot_children == NULLOT || ot->ot_smux)
			break;
		else {
			ot = ot->ot_children;
	...
	char *text;
{
	...
	for (ot = Tbuckets[THASH(text)]; ot && strcmp(ot->ot_text, text); ot = ot->ot_chain)
		continue;
	...
	int quoted;
{
	...
	if ((oid->oid_nelem == 2	/* XXX: 0.0 is a special case */
	     && oid->oid_elements[0] == 0 && oid->oid_elements[1] == 0)
	...
	for (ip = oid->oid_elements + (oid2 = ot->ot_name)->oid_nelem,
	     i = oid->oid_nelem - oid2->oid_nelem; i-- > 0; ip++) {

	...
	int offset;
{
	...
	case udpInErrors:
		return o_integer(oi, v, udps->udps_hdrops + udps->udps_badsum + udps->udps_badlen);
	...
	int offset;
{
	...
	case type_SNMP_PDUs_get__request:
		if (oid->oid_nelem != ot->ot_name->oid_nelem + UT_SIZE)
			return int_SNMP_error__status_noSuchName;
		if ((ut = get_udpent(oid->oid_elements + oid->oid_nelem - UT_SIZE, 0)) == NULL)
			return int_SNMP_error__status_noSuchName;
		break;

	case type_SNMP_PDUs_get__next__request:
		if ((i = oid->oid_nelem - ot->ot_name->oid_nelem) != 0 && i < UT_SIZE) {
		...
		if (oid->oid_nelem == ot->ot_name->oid_nelem) {
			...
			if ((ut = get_udpent(ip = oid->oid_elements + oid->oid_nelem - UT_SIZE, 1))
			    == NULL)
				return NOTOK;
...
ut_compar(a, b)
	struct udptab **a, **b;
{
	return elem_cmp((*a)->ut_instance, UT_SIZE, (*b)->ut_instance, UT_SIZE);
}
	...
	int offset;
{
	...
	register struct udptab *us, *up, **usp;
	register struct inpcb *ip;
	struct inpcb *head, udb, zdb;
	struct nlist nzs;
	...
	if (quantum == lastq)
		return OK;
	if (!flush_udp_cache && offset == type_SNMP_PDUs_get__next__request && quantum == lastq + 1) {	/* XXX: 
													   caching! 
													 */
		lastq = quantum;
		return OK;
	...
	while (ip->inp_next != head) {
		...
		*cp++ = ntohs(ip->inp_lport) & 0xffff;
		...
		for (uz = uts; uz; uz = uz->ut_next)
			if (elem_cmp(uz->ut_instance, UT_SIZE, us->ut_instance, UT_SIZE) == 0)
				break;
		if (uz) {
			if (first_time) {
		...
		*usp = us, usp = &us->ut_next, i++;
		...
		if (debug && first_time) {
			oids.oid_elements = us->ut_instance;
			oids.oid_nelem = UT_SIZE;
			advise(LLOG_DEBUG, NULLCP, "add listener: %s", sprintoid(&oids));
		}
	}
	first_time = 0;

	if (i > 1) {
		register struct udptab **base, **use;
...
init_udp()
{
	...
	if (ot = text2obj("udpInDatagrams"))
		ot->ot_getfnx = o_udp, ot->ot_info = (caddr_t) udpInDatagrams;
#endif
#ifdef	udpNoPorts
	if (ot = text2obj("udpNoPorts"))
		ot->ot_getfnx = o_udp, ot->ot_info = (caddr_t) udpNoPorts;
#endif
	if (ot = text2obj("udpInErrors"))
		ot->ot_getfnx = o_udp, ot->ot_info = (caddr_t) udpInErrors;
#ifdef	udpOutDatagrams
	if (ot = text2obj("udpOutDatagrams"))
		ot->ot_getfnx = o_udp, ot->ot_info = (caddr_t) udpOutDatagrams;
#endif

	if (ot = text2obj("udpLocalAddress"))
		ot->ot_getfnx = o_udp_listen, ot->ot_info = (caddr_t) udpLocalAddress;
	if (ot = text2obj("udpLocalPort"))
		ot->ot_getfnx = o_udp_listen, ot->ot_info = (caddr_t) udpLocalPort;

	if (ot = text2obj("unixUdpRemAddress"))
		ot->ot_getfnx = o_udp_listen, ot->ot_info = (caddr_t) unixUdpRemAddress;
	if (ot = text2obj("unixUdpRemPort"))
		ot->ot_getfnx = o_udp_listen, ot->ot_info = (caddr_t) unixUdpRemPort;
	if (ot = text2obj("unixUdpSendQ"))
		ot->ot_getfnx = o_udp_listen, ot->ot_info = (caddr_t) unixUdpSendQ;
	if (ot = text2obj("unixUdpRecvQ"))
		ot->ot_getfnx = o_udp_listen, ot->ot_info = (caddr_t) unixUdpRecvQ;
}

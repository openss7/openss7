...
static int
pw_compar(a, b)
	register struct pw *a, *b;
{
	return elem_cmp(a->pw_instance, a->pw_insize, b->pw_instance, b->pw_insize);
}
	...
	int offset;
{
	...
	if (pw_head
		...
		? pw_fd != NOTOK
		: (stat("/etc/passwd", &st) != NOTOK && pw_st.st_mtime == st.st_mtime))) {
		lastq = quantum;
		return OK;
	}
	...
	if (flock(pw_fd, (offset == type_SNMP_SMUX__PDUs_set__request ? LOCK_EX : LOCK_SH)
		  | LOCK_NB) == NOTOK) {
	...
	for (dp = pw_data, j = cc; j > 0; dp += i, j -= i)
		...
		case OK:
			advise(LLOG_EXCEPTIONS, NULLCP, "premature eof on /etc/passwd");
			goto losing;
		...
		for (;;) {
			if (*dp == ':') {
				...
				*dp++ = NULL;
				...
				for (cp = pw->pw_pw.pw_name, ip = pw->pw_instance, i = 0; *cp; i++)
					*ip++ = *cp++ & 0xff;
				pw->pw_insize = i;
...
static int
gr_compar(a, b)
	register struct gr *a, *b;
{
	return elem_cmp(a->gr_instance, a->gr_insize, b->gr_instance, b->gr_insize);
}
	...
	int offset;
{
	...
	if (gr_head
	    && (offset == type_SNMP_SMUX__PDUs_set__request
		? gr_fd != NOTOK
		: (stat("/etc/group", &st) != NOTOK && gr_st.st_mtime == st.st_mtime))) {
		lastq = quantum;
		return OK;
	}
	...
	if (flock(gr_fd, (offset == type_SNMP_SMUX__PDUs_set__request ? LOCK_EX : LOCK_SH)
		  | LOCK_NB) == NOTOK) {
	...
	for (dp = gr_data, j = cc; j > 0; dp += i, j -= i)
		...
		case OK:
			advise(LLOG_EXCEPTIONS, NULLCP, "premature eof on /etc/group");
			goto losing;
		...
		for (;;) {
			if (*dp == ':') {
				...
				*dp++ = NULL;
				...
				for (cp = gr->gr_gr.gr_name, ip = gr->gr_instance, i = 0; *cp; i++)
					*ip++ = *cp++ & 0xff;
				gr->gr_insize = i;
...
static int
gu_compar(a, b)
	register struct gu *a, *b;
{
	return elem_cmp(a->gu_instance, a->gu_insize, b->gu_instance, b->gu_insize);
}
	...
	int offset;
{
	...
	if ((ifvar = (int) ot->ot_info) == userGroup && get_gr(offset) == NOTOK)
		goto losing;
      try_again:;
	switch (offset) {
	...
	case userPasswd:
		if (strcmp(pwp->pw_passwd, "*") == 0)
			return o_string(oi, v, pwp->pw_passwd, strlen(pwp->pw_passwd));
...
#ifndef BSD44
		if (*pwp->pw_comment)
			return o_string(oi, v, pwp->pw_comment, strlen(pwp->pw_comment));
#endif
		if (offset == type_SNMP_SMUX__PDUs_get__next__request)
			goto try_again;
		return NOTOK;
	...
	if ((pw = get_pwent(ip = oid->oid_elements + ot->ot_name->oid_nelem,
		...
		*cp = NULL;
		...
		for (ip = oid->oid_elements + ot->ot_name->oid_nelem,
		     jp = pw->pw_instance, i = pw->pw_insize; i > 0; i--)
			*jp++ = *ip++;
		pwp = &pw->pw_pw;
		if ((pwp->pw_name = strdup(name)) == NULL || (pwp->pw_passwd = strdup("*")) == NULL
#ifndef BSD44
		    || (pwp->pw_comment = strdup("")) == NULL
#endif
		    || (pwp->pw_gecos = strdup("")) == NULL
		    || (pwp->pw_dir = strdup("/")) == NULL || (pwp->pw_shell = strdup("")) == NULL)
			goto no_mem;
	...
	int offset;
{
	...
	case groupPasswd:
		if (strcmp(grp->gr_passwd, "*") == 0)
			return o_string(oi, v, grp->gr_passwd, strlen(grp->gr_passwd));
		if (offset == type_SNMP_SMUX__PDUs_get__next__request)
			goto try_again;
		return NOTOK;
	...
	int offset;
{
	...
	if ((gr = get_grent(ip = oid->oid_elements + ot->ot_name->oid_nelem,
		...
		*cp = NULL;
		...
		for (ip = oid->oid_elements + ot->ot_name->oid_nelem,
		     jp = gr->gr_instance, i = gr->gr_insize; i > 0; i--)
			*jp++ = *ip++;
		gr->gr_serial = gr - gr_head;
		grp = &gr->gr_gr;
		if ((grp->gr_name = strdup(name)) == NULL || (grp->gr_passwd = strdup("*")) == NULL)
			goto no_mem;
		grp->gr_gid = -1;
	...
	int offset;
{
	...
	if ((gu = get_guent(ip = oid->oid_elements + ot->ot_name->oid_nelem,
	    ...
	    == NULL) {
		register unsigned int *jp;
		char group[GU_SIZE + 1], user[PW_SIZE + 1];
		register struct gr *gr;
		...
		*cp = NULL;
		...
		for (ip = oid->oid_elements + ot->ot_name->oid_nelem,
		     jp = gu->gu_instance, i = gu->gu_insize; i > 0; i--)
			*jp++ = *ip++;
		gu->gu_serial = gr->gr_serial;
		...
		if (!(gu = get_guent(oid->oid_elements
				     + ot->ot_name->oid_nelem,
				     oid->oid_nelem - ot->ot_name->oid_nelem, 0))) {
			advise(LLOG_EXCEPTIONS, NULLCP, "lost newly created group member");
			free_gr();
			return int_SNMP_error__status_noSuchName;
		}
	}
...
init_users()
{
	...
	if (ot = text2obj("userName"))
		ot->ot_getfnx = o_user, ot->ot_info = (caddr_t) userName;
	if (ot = text2obj("userPasswd"))
		ot->ot_getfnx = o_user, ot->ot_setfnx = s_user, ot->ot_info = (caddr_t) userPasswd;
	if (ot = text2obj("userID"))
		ot->ot_getfnx = o_user, ot->ot_setfnx = s_user, ot->ot_info = (caddr_t) userID;
	if (ot = text2obj("userGroup"))
		ot->ot_getfnx = o_user, ot->ot_setfnx = s_user, ot->ot_info = (caddr_t) userGroup;
	if (ot = text2obj("userQuota"))
		ot->ot_getfnx = o_user, ot->ot_setfnx = s_user, ot->ot_info = (caddr_t) userQuota;
	if (ot = text2obj("userComment"))
		ot->ot_getfnx = o_user, ot->ot_setfnx = s_user, ot->ot_info = (caddr_t) userComment;
	if (ot = text2obj("userFullName"))
		ot->ot_getfnx = o_user,
		    ot->ot_setfnx = s_user, ot->ot_info = (caddr_t) userFullName;
	if (ot = text2obj("userHome"))
		ot->ot_getfnx = o_user, ot->ot_setfnx = s_user, ot->ot_info = (caddr_t) userHome;
	if (ot = text2obj("userShell"))
		ot->ot_getfnx = o_user, ot->ot_setfnx = s_user, ot->ot_info = (caddr_t) userShell;
	if (ot = text2obj("userStatus"))
		ot->ot_getfnx = o_user, ot->ot_setfnx = s_user, ot->ot_info = (caddr_t) userStatus;

	if (ot = text2obj("groupName"))
		ot->ot_getfnx = o_group, ot->ot_info = (caddr_t) groupName;
	if (ot = text2obj("groupPasswd"))
		ot->ot_getfnx = o_group,
		    ot->ot_setfnx = s_group, ot->ot_info = (caddr_t) groupPasswd;
	if (ot = text2obj("groupID"))
		ot->ot_getfnx = o_group, ot->ot_setfnx = s_group, ot->ot_info = (caddr_t) groupID;
	if (ot = text2obj("groupStatus"))
		ot->ot_getfnx = o_group,
		    ot->ot_setfnx = s_group, ot->ot_info = (caddr_t) groupStatus;

	if (ot = text2obj("grUserStatus"))
		ot->ot_getfnx = o_gruser,
		    ot->ot_setfnx = s_gruser, ot->ot_info = (caddr_t) grUserStatus;
}
	...
	integer cor;
{
	...
	case int_SNMP_SOutPDU_commit:
		...
		if (fchmod(fileno(fp), (int) (pw_st.st_mode & 0777)) == NOTOK)
			advise(LLOG_EXCEPTIONS, "",
			       "unable to change mode of %s to 0%o", tmpfil, pw_st.st_mode & 0777);
		...
		if (iserr)
			...
			if (access("/etc/passwd.orig", F_OK) == NOTOK
			    && errno == ENOENT && rename("/etc/passwd.old", "/etc/passwd.orig")
			    == NOTOK && errno != ENOENT)
			...
			if (rename(tmpfil, "/etc/passwd") == NOTOK)
				advise(LLOG_FATAL, "/etc/passwd", "unable to rename %s to", tmpfil);
		}
		if (npw > 1)
		...
		if (fchmod(fileno(fp), (int) (gr_st.st_mode & 0777)) == NOTOK)
			advise(LLOG_EXCEPTIONS, "",
			       "unable to change mode of %s to 0%o", tmpfil, gr_st.st_mode & 0777);
		...
		if (iserr)
			...
			if (access("/etc/group.orig", F_OK) == NOTOK
			    && errno == ENOENT && rename("/etc/group.old", "/etc/group.orig")
			    == NOTOK && errno != ENOENT)
			...
			if (rename(tmpfil, "/etc/group") == NOTOK)
				advise(LLOG_FATAL, "/etc/group", "unable to rename %s to", tmpfil);
		}
		if (ngr)

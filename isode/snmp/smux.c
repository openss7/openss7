	...
	int debug;
{
	...
	if (smux_stamp == NULL && (smux_stamp = (struct type_SNMP_TimeTicks *)
	...
	int commlen;
{
	...
	if (identity == NULL || description == NULL || (commname == NULL && commlen != 0))
	...
	struct type_SNMP_SMUX__PDUs *pdu;
{
	...
	if (encode_SNMP_SMUX__PDUs(&pe, 1, 0, NULLCP, pdu) == NOTOK) {
		result = smuxlose(youLoseBig, NULLCP, "encode_SNMP_SMUX__PDUs: %s", PY_pepy);
		goto out;
	}
#ifdef	DEBUG
	if (smux_debug)
	...
	if (pe2ps(ps, pe) == NOTOK) {
		result = smuxlose(youLoseBig, NULLCP, "pe2ps: %s", ps_error(ps->ps_errno));
		goto out;
	}
	...
	int secs;
{
	...
	FD_ZERO(&mask);
	FD_SET(sd, &mask);
	if (ps_prime(ps, 1) == OK && xselect(sd + 1, &mask, NULLFD, NULLFD, secs) <= OK) {
		errno = EWOULDBLOCK;
		return smuxlose(inProgress, NULLCP, NULLCP);
	}

	if ((pe = ps2pe(ps)) == NULLPE) {
		(void) smuxlose(youLoseBig, NULLCP, "ps2pe: %s", ps_error(ps->ps_errno));
		goto out;
	}

	if (decode_SNMP_SMUX__PDUs(pe, 1, NULLIP, NULLVP, event) == NOTOK) {
		(void) smuxlose(youLoseBig, NULLCP, "encode_SNMP_SMUX__PDUs: %s", PY_pepy);
		goto out;
	}
#ifdef	DEBUG
	if (smux_debug)

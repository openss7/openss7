	...
	int offset;
{
	...
	default:
		if (quantum != lastq) {
			...
			if (getkmem(nl + N_ISO_SYSTYPE, (caddr_t) &iso_systype,
			    ...
			    || getkmem(nl + N_CLNP_STAT, (caddr_t) cns, sizeof *cns) == NOTOK)
	...
	switch (ifvar) {
	case clnpForwarding:
		return o_integer(oi, v, iso_systype & SNPA_ES ? FORW_ES : FORW_IS);
	...
	case clnpInHdrErrors:
		return o_integer(oi, v, cns->cns_toosmall
				 + cns->cns_badhlen
				 + cns->cns_badcsum + cns->cns_noseg + cns->cns_badvers);
	...
	case clnpReasmFails:
		return o_integer(oi, v, cns->cns_fragdropped + cns->cns_fragtimeout);
	...
	int offset;
{
	...
	case clnpDefaultLifeTime:
		...
		case type_SNMP_PDUs_set__request:
			...
			if ((*os->os_decode) (&ot->ot_save, v->value)
			    == NOTOK || *((integer *) ot->ot_save) != CLNP_TTL)
	...
	case type_SNMP_PDUs_set__request:
		if (ot->ot_save)
			(*os->os_free) (ot->ot_save), ot->ot_save = NULL;
		if (getkmem(nl + N_ISO_SYSTYPE, (caddr_t) &iso_systype,
			    sizeof iso_systype) == NOTOK || chekmem(nl + N_ISO_SYSTYPE) == NOTOK)
			return int_SNMP_error__status_genErr;
	...
	int offset;
{
	...
	case type_SNMP_PDUs_get__request:
		...
		if ((as = get_addrent(oid->oid_elements
				      ...
				      oid->oid_nelem - ot->ot_name->oid_nelem, afs_iso, 0)) == NULL)
			return int_SNMP_error__status_noSuchName;
		break;
	...
	case type_SNMP_PDUs_get__next__request:
		...
		if (oid->oid_nelem == ot->ot_name->oid_nelem) {
			...
			if ((as = get_addrent(oid->oid_elements
					      + ot->ot_name->oid_nelem,
					      j = oid->oid_nelem
					      - ot->ot_name->oid_nelem, afs_iso, 1)) == NULL)
				return NOTOK;
	...
	switch (ifvar) {
	case clnpAdEntAddr:
		return o_clnpaddr(oi, v, (struct sockaddr_iso *) &as->adr_address);
	...
	int offset;
{
	...
	case type_SNMP_PDUs_get__request:
		...
		if ((rt = get_rtent(oid->oid_elements
				    + ot->ot_name->oid_nelem,
				    oid->oid_nelem - ot->ot_name->oid_nelem, rts_iso, 0)) == NULL)
			return int_SNMP_error__status_noSuchName;
		break;
	...
	case type_SNMP_PDUs_get__next__request:
		if (oid->oid_nelem == ot->ot_name->oid_nelem) {
			...
			if ((rt = get_rtent(oid->oid_elements
					    + ot->ot_name->oid_nelem,
					    j = oid->oid_nelem
					    - ot->ot_name->oid_nelem, rts_iso, 1)) == NULL)
				return NOTOK;
	...
	switch (ifvar) {
	case clnpRouteDest:
		return o_clnpaddr(oi, v, (struct sockaddr_iso *) &rt->rt_dst);
	...
	case clnpRouteNextHop:
		return o_clnpaddr(oi, v, (struct sockaddr_iso *) &rt->rt_gateway);
...
get_arptab(offset)
	int offset;
{
	int adrNumber = 0, rlen, tblsize;
	char *snpac;
	register char *sc, *se;
	register struct adrtab *at, *ap, **base, **afe, **afp;
	register struct interface *is;
	...
	for (se = (sc = snpac) + rlen; sc < se; sc += rtm->rtm_msglen) {
		...
		*afp = at, afp = &at->adn_next, adrNumber++;
		...
		at->adr_type = rtm->rtm_flags & RTF_DYNAMIC ? DYNAMIC_MAPPING : STATIC_MAPPING;

		at->adn_address = nsap;	/* struct copy */
		at->adn_instance[0] = at->adr_index, at->adn_insize = 1;
		at->adn_insize += clnpaddr2oid(at->adn_instance + 1, &at->adn_address);
...
init_clns()
{
	...
	if (ot = text2obj("clnpForwarding"))
		ot->ot_getfnx = o_clnp,
		    ot->ot_setfnx = s_clnp, ot->ot_info = (caddr_t) clnpForwarding;
	if (ot = text2obj("clnpDefaultLifeTime"))
		ot->ot_getfnx = o_clnp,
		    ot->ot_setfnx = s_clnp, ot->ot_info = (caddr_t) clnpDefaultLifeTime;
	if (ot = text2obj("clnpInReceives"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInReceives;
	if (ot = text2obj("clnpInHdrErrors"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInHdrErrors;
	if (ot = text2obj("clnpInAddrErrors"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInAddrErrors;
	if (ot = text2obj("clnpForwPDUs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpForwPDUs;
#ifdef	clnpInUnknownNLPs
	if (ot = text2obj("clnpInUnknownNLPs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInUnknownNLPs;
#endif
	if (ot = text2obj("clnpInUnknownULPs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInUnknownULPs;
#ifdef	clnpInDiscards
	if (ot = text2obj("clnpInDiscards"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInDiscards;
#endif
	if (ot = text2obj("clnpInDelivers"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInDelivers;
	if (ot = text2obj("clnpOutRequests"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutRequests;
	if (ot = text2obj("clnpOutDiscards"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutDiscards;
	if (ot = text2obj("clnpOutNoRoutes"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutNoRoutes;
	if (ot = text2obj("clnpReasmTimeout"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpReasmTimeout;
	if (ot = text2obj("clnpReasmReqds"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpReasmReqds;
	if (ot = text2obj("clnpReasmOKs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpReasmOKs;
	if (ot = text2obj("clnpReasmFails"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpReasmFails;
	if (ot = text2obj("clnpSegOKs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpSegOKs;
	if (ot = text2obj("clnpSegFails"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpSegFails;
	if (ot = text2obj("clnpSegCreates"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpSegCreates;
#ifdef	clnpInOpts
	if (ot = text2obj("clnpInOpts"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInOpts;
#endif
#ifdef	clnpOutOpts
	if (ot = text2obj("clnpOutOpts"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutOpts;
#endif
#ifdef	clnpRoutingDiscards
	if (ot = text2obj("clnpRoutingDiscards"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpRoutingDiscards;
#endif

	if (ot = text2obj("clnpAdEntAddr"))
		ot->ot_getfnx = o_clnp_addr, ot->ot_info = (caddr_t) clnpAdEntAddr;
	if (ot = text2obj("clnpAdEntIfIndex"))
		ot->ot_getfnx = o_clnp_addr, ot->ot_info = (caddr_t) clnpAdEntIfIndex;
	if (ot = text2obj("clnpAdEntReasmMaxSize"))
		ot->ot_getfnx = o_clnp_addr, ot->ot_info = (caddr_t) clnpAdEntReasmMaxSize;

	if (ot = text2obj("clnpRouteDest"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) clnpRouteDest;
	if (ot = text2obj("clnpRouteIfIndex"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) clnpRouteIfIndex;
	if (ot = text2obj("clnpRouteMetric1"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) clnpRouteMetric1;
	if (ot = text2obj("clnpRouteMetric2"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) clnpRouteMetric2;
	if (ot = text2obj("clnpRouteMetric3"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) clnpRouteMetric3;
	if (ot = text2obj("clnpRouteMetric4"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) clnpRouteMetric4;
	if (ot = text2obj("clnpRouteNextHop"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) clnpRouteNextHop;
	if (ot = text2obj("clnpRouteType"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) clnpRouteType;
	if (ot = text2obj("clnpRouteProto"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) clnpRouteProto;
	if (ot = text2obj("clnpRouteAge"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) clnpRouteAge;

	if (ot = text2obj("unixClnpRouteFlags"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) unixClnpRouteFlags;
	if (ot = text2obj("unixClnpRouteRefCnt"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) unixClnpRouteRefCnt;
	if (ot = text2obj("unixClnpRouteUses"))
		ot->ot_getfnx = o_clnp_route, ot->ot_info = (caddr_t) unixClnpRouteUses;

	if (ot = text2obj("clnpNetToMediaIfIndex"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpNetToMediaIfIndex;
	if (ot = text2obj("clnpNetToMediaPhysAddress"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpNetToMediaPhysAddress;
	if (ot = text2obj("clnpNetToMediaNetAddress"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpNetToMediaNetAddress;
	if (ot = text2obj("clnpNetToMediaType"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpNetToMediaType;
#ifdef	clnpNetToMediaAge
	if (ot = text2obj("clnpNetToMediaAge"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpNetToMediaAge;
#endif
#ifdef	clnpNetToMediaHoldTime
	if (ot = text2obj("clnpNetToMediaHoldTime"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpNetToMediaHoldTime;
#endif
	if (ot = text2obj("clnpMediaToNetIfIndex"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpMediaToNetIfIndex;
	if (ot = text2obj("clnpMediaToNetAddress"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpMediaToNetAddress;
	if (ot = text2obj("clnpMediaToNetPhysAddress"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpMediaToNetPhysAddress;
	if (ot = text2obj("clnpMediaToNetType"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpMediaToNetType;
#ifdef	clnpMediaToNetAge
	if (ot = text2obj("clnpMediaToNetAge"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpMediaToNetAge;
#endif
#ifdef	clnpMediaToNetHoldTime
	if (ot = text2obj("clnpMediaToNetHoldTime"))
		ot->ot_getfnx = o_address, ot->ot_info = (caddr_t) clnpMediaToNetHoldTime;
#endif

	if (ot = text2obj("clnpInErrors"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrors;
	if (ot = text2obj("clnpOutErrors"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrors;
	if (ot = text2obj("clnpInErrUnspecs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrUnspecs;
	if (ot = text2obj("clnpInErrProcs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrProcs;
	if (ot = text2obj("clnpInErrCksums"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrCksums;
	if (ot = text2obj("clnpInErrCongests"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrCongests;
	if (ot = text2obj("clnpInErrHdrs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrHdrs;
	if (ot = text2obj("clnpInErrSegs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrSegs;
	if (ot = text2obj("clnpInErrIncomps"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrIncomps;
	if (ot = text2obj("clnpInErrDups"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrDups;
	if (ot = text2obj("clnpInErrUnreachDsts"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrUnreachDsts;
	if (ot = text2obj("clnpInErrUnknownDsts"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrUnknownDsts;
	if (ot = text2obj("clnpInErrSRUnspecs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrSRUnspecs;
	if (ot = text2obj("clnpInErrSRSyntaxes"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrSRSyntaxes;
	if (ot = text2obj("clnpInErrSRUnkAddrs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrSRUnkAddrs;
	if (ot = text2obj("clnpInErrSRBadPaths"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrSRBadPaths;
	if (ot = text2obj("clnpInErrHops"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrHops;
	if (ot = text2obj("clnpInErrHopReassms"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrHopReassms;
	if (ot = text2obj("clnpInErrUnsOptions"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrUnsOptions;
	if (ot = text2obj("clnpInErrUnsVersions"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrUnsVersions;
	if (ot = text2obj("clnpInErrUnsSecurities"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrUnsSecurities;
	if (ot = text2obj("clnpInErrUnsSRs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrUnsSRs;
	if (ot = text2obj("clnpInErrUnsRRs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrUnsRRs;
	if (ot = text2obj("clnpInErrInterferences"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpInErrInterferences;
	if (ot = text2obj("clnpOutErrUnspecs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrUnspecs;
	if (ot = text2obj("clnpOutErrProcs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrProcs;
	if (ot = text2obj("clnpOutErrCksums"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrCksums;
	if (ot = text2obj("clnpOutErrCongests"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrCongests;
	if (ot = text2obj("clnpOutErrHdrs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrHdrs;
	if (ot = text2obj("clnpOutErrSegs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrSegs;
	if (ot = text2obj("clnpOutErrIncomps"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrIncomps;
	if (ot = text2obj("clnpOutErrDups"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrDups;
	if (ot = text2obj("clnpOutErrUnreachDsts"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrUnreachDsts;
	if (ot = text2obj("clnpOutErrUnknownDsts"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrUnknownDsts;
	if (ot = text2obj("clnpOutErrSRUnspecs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrSRUnspecs;
	if (ot = text2obj("clnpOutErrSRSyntaxes"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrSRSyntaxes;
	if (ot = text2obj("clnpOutErrSRUnkAddrs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrSRUnkAddrs;
	if (ot = text2obj("clnpOutErrSRBadPaths"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrSRBadPaths;
	if (ot = text2obj("clnpOutErrHops"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrHops;
	if (ot = text2obj("clnpOutErrHopReassms"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrHopReassms;
	if (ot = text2obj("clnpOutErrUnsOptions"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrUnsOptions;
	if (ot = text2obj("clnpOutErrUnsVersions"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrUnsVersions;
	if (ot = text2obj("clnpOutErrUnsSecurities"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrUnsSecurities;
	if (ot = text2obj("clnpOutErrUnsSRs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrUnsSRs;
	if (ot = text2obj("clnpOutErrUnsRRs"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrUnsRRs;
	if (ot = text2obj("clnpOutErrInterferences"))
		ot->ot_getfnx = o_clnp, ot->ot_info = (caddr_t) clnpOutErrInterferences;

	if (ot = text2obj("esisESHins"))
		ot->ot_getfnx = o_esis, ot->ot_info = (caddr_t) esisESHins;
	if (ot = text2obj("esisESHouts"))
		ot->ot_getfnx = o_esis, ot->ot_info = (caddr_t) esisESHouts;
	if (ot = text2obj("esisISHins"))
		ot->ot_getfnx = o_esis, ot->ot_info = (caddr_t) esisISHins;
	if (ot = text2obj("esisISHouts"))
		ot->ot_getfnx = o_esis, ot->ot_info = (caddr_t) esisISHouts;
	if (ot = text2obj("esisRDUins"))
		ot->ot_getfnx = o_esis, ot->ot_info = (caddr_t) esisRDUins;
	if (ot = text2obj("esisRDUouts"))
		ot->ot_getfnx = o_esis, ot->ot_info = (caddr_t) esisRDUouts;
}
#else

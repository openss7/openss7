...
#ifdef	ipReasmFails
	case ipReasmFails:
		return o_integer(oi, v, ips->ips_fragdropped + ips->ips_fragtimeout);
#endif
	...
	case ipDefaultTTL:
		...
		case type_SNMP_PDUs_set__request:
			...
			if ((*os->os_decode) (&ot->ot_save, v->value)
			    == NOTOK || *((integer *) ot->ot_save) != MAXTTL)
	...
	int offset;
{
	...
	case type_SNMP_PDUs_get__next__request:
		if ((i = oid->oid_nelem - ot->ot_name->oid_nelem) != 0 && i < IFN_SIZE) {
	...
	case ipAdEntBcastAddr:	/* beyond belief! */
	{
		u_long a = (((struct sockaddr_in *) &as->adr_netmask)
			    ->sin_addr.s_addr)
		    & ~(((struct sockaddr_in *) &as->adr_broadaddr)->sin_addr.s_addr);
	...
	int offset;
{
	...
	case type_SNMP_PDUs_get__request:
		...
		if ((rt = get_rtent(oid->oid_elements
				    ...
				    oid->oid_nelem - ot->ot_name->oid_nelem, rts_inet, 0))
		    == NULL)
			return int_SNMP_error__status_noSuchName;
		break;
	...
	case type_SNMP_PDUs_get__next__request:
		...
		if (oid->oid_nelem == ot->ot_name->oid_nelem) {
			...
			if ((rt = get_rtent(ip = oid->oid_elements
					    + ot->ot_name->oid_nelem,
					    j = oid->oid_nelem
					    - ot->ot_name->oid_nelem, rts_inet, 1)) == NULL)
				return NOTOK;
	...
	case ipRouteIfIndex:
	{
		...
		for (is = ifs; is; is = is->ifn_next)
			if ((caddr_t) is->ifn_offset == (caddr_t) rt->rt_rt.rt_ifp) {
				if (is->ifn_ready)
	...
	int offset;
{
	...
	case type_SNMP_PDUs_rollback:
		...
		if (rt = get_rtent(oid->oid_elements
				   + ot->ot_name->oid_nelem,
				   i = oid->oid_nelem - ot->ot_name->oid_nelem, rts_inet, 0))
			break;
		...
		if ((rt = (struct rtetab *) calloc(1, sizeof *rt)) == NULL) {
			advise(LLOG_EXCEPTIONS, NULLCP, "out of memory creating new routing entry");
			return int_SNMP_error__status_genErr;
		}
		...
		bcopy((char *) ip, (char *) rt->rt_instance, (rt->rt_insize = i) * sizeof *ip);

		rt->rt_rt.rt_flags |= RTF_UP;
		{
			register u_char *cp;
			register unsigned int *jp, *kp;
	...
	case type_SNMP_PDUs_set__request:
		...
		case ipRouteDest:
				...
				return int_SNMP_error__status_badValue;
			{
				...
				if (addr.s_addr != ((struct sockaddr_in *) &rt->rt_dst)
				    ->sin_addr.s_addr)
					return int_SNMP_error__status_badValue;
			}
			break;
		...
		if (rt->rt_touched) {
			...
			if (rt->rt_type == TYPE_INVALID) {
				...
				if (((struct sockaddr_in *) &rt->rt_gateway)
				    ->sin_addr.s_addr == 0L || rt->rt_type == TYPE_OTHER) {
...
static int
o_ip_routing_stats(oi, v, offset)
	OI oi;
	register struct type_SNMP_VarBind *v;
	int offset;
{
	...
	int offset;
{
	...
	case type_SNMP_PDUs_get__request:
		...
		if ((at = get_arpent(oid->oid_elements
				     + ot->ot_name->oid_nelem,
				     oid->oid_nelem - ot->ot_name->oid_nelem, isnpa, 0)) == NULL)
			return int_SNMP_error__status_noSuchName;
		break;
	...
	case type_SNMP_PDUs_get__next__request:
		if (oid->oid_nelem == ot->ot_name->oid_nelem) {
			...
			if ((at = get_arpent(oid->oid_elements
					     + ot->ot_name->oid_nelem,
					     j = oid->oid_nelem
					     - ot->ot_name->oid_nelem, isnpa, 1)) == NULL)
				return NOTOK;
			i = isnpa > 0 ? at->adm_insize
			    : isnpa == 0 ? at->adn_insize : at->ada_insize;
	...
	case atPhysAddress:
	case ipNetToMediaPhysAddress:
		return o_string(oi, v, (char *) at->adm_address, (int) at->adm_addrlen);
	...
	int offset;
{
	...
	case type_SNMP_PDUs_rollback:
		...
		if (at = get_arpent(oid->oid_elements
				    + ot->ot_name->oid_nelem,
				    i = oid->oid_nelem - ot->ot_name->oid_nelem, isnpa, 0))
			break;
		if (offset != type_SNMP_PDUs_set__request)
			return int_SNMP_error__status_genErr;
		...
		if ((at = (struct adrtab *) calloc(1, sizeof *at)) == NULL) {
			advise(LLOG_EXCEPTIONS, NULLCP, "out of memory creating new ARP entry");
			return int_SNMP_error__status_genErr;
		}
		...
		bcopy((char *) ip, (char *) (at->adn_instance + 1), IFN_SIZE * sizeof *ip);
		at->adn_insize += IFN_SIZE;

		at->ada_instance[0] = at->adr_index;
		at->ada_instance[1] = 1;
		at->ada_insize = 2;
		bcopy((char *) ip, (char *) (at->ada_instance + 2), IFN_SIZE * sizeof *ip);
				...
				return int_SNMP_error__status_badValue;
			{
				...
				if (len == 0) {
					...
					bcopy((char *) at->adm_address,
					      (char *) at->adr_oldphys,
					      (int) (at->adr_physlen = at->adm_addrlen));
				...
				if (at->adm_addrlen == 0) {
					...
					if (at->adr_physlen > 0) {
						bcopy((char *) at->adr_oldphys,
						      arpreq.arp_ha.sa_data, (int) at->adr_physlen);
						...
						bcopy((char *) at->adm_address,
						      arpreq.arp_ha.sa_data, (int) at->adm_addrlen);
					}
...
get_arptab(offset)
	int offset;
{
	int arptab_size, tblsize;
	register struct arptab *ac, *ae;
	struct arptab *arptab;
	register struct adrtab *at, *ap, **afp;
	register struct interface *is;
	...
	if (quantum == lastq)
		return OK;
	if (!flush_arp_cache && offset == type_SNMP_PDUs_get__next__request && quantum == lastq + 1) {	/* XXX: 
													   caching! 
													 */
		lastq = quantum;
		return OK;
	...
	if (getkmem(nl + N_ARPTAB_SIZE, (caddr_t) &arptab_size, sizeof arptab_size) == NOTOK)
		return NOTOK;
   ...
   tables, so we simply duplicate the arptable for each interface... */
		for (is = ifs; is; is = is->ifn_next) {
			...
			*afp = at, afp = &at->adn_next, adrNumber++;
			...
			at->adn_instance[0] = at->adr_index, at->adn_insize = 1;
			at->adn_insize += ipaddr2oid(at->adn_instance + 1, &at->adn_address);
			...
			bcopy((char *) ac->at_enaddr.ether_addr_octet,
			      (char *) at->adm_address,
			      (int) (at->adm_addrlen = sizeof ac->at_enaddr.ether_addr_octet));
#endif
			at->adm_instance[0] = at->adr_index, at->adm_insize = 1;
			at->adm_insize += mediaddr2oid(at->adm_instance + 1,
						       at->adm_address, (int) at->adm_addrlen, 0);

			at->ada_instance[0] = at->adr_index;
			at->ada_instance[1] = 1;
			at->ada_insize = 2;
			at->ada_insize += ipaddr2oid(at->ada_instance + 2, &at->adn_address);
...
static int
adn_compar(a, b)
	register struct adrtab **a, **b;
{
	return elem_cmp((*a)->adn_instance, (*a)->adn_insize, (*b)->adn_instance, (*b)->adn_insize);
}
...
adm_compar(a, b)
	register struct adrtab **a, **b;
{
	return elem_cmp((*a)->adm_instance, (*a)->adm_insize, (*b)->adm_instance, (*b)->adm_insize);
}
...
ada_compar(a, b)
	register struct adrtab **a, **b;
{
	return elem_cmp((*a)->ada_instance, (*a)->ada_insize, (*b)->ada_instance, (*b)->ada_insize);
}

static
sort_arptab()
{
	register struct adrtab *at, **base, **afe, **afp;
	...
	int isnpa, isnext;
{
	...
	case -1:
		for (at = ada; at; at = at->ada_next)
			switch (elem_cmp(at->ada_instance, at->ada_insize, ip, len)) {
			case 0:
				if (!isnext)
					return at;
...
init_ip()
{
	...
	if (ot = text2obj("ipForwarding"))
		ot->ot_getfnx = o_ip, ot->ot_setfnx = s_ip, ot->ot_info = (caddr_t) ipForwarding;
	if (ot = text2obj("ipDefaultTTL"))
		ot->ot_getfnx = o_ip, ot->ot_setfnx = s_ip, ot->ot_info = (caddr_t) ipDefaultTTL;
#ifdef	ipInReceives
	if (ot = text2obj("ipInReceives"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipInReceives;
#endif
	if (ot = text2obj("ipInHdrErrors"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipInHdrErrors;
#ifdef	ipInAddrErrors
	if (ot = text2obj("ipInAddrErrors"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipInAddrErrors;
#endif
#ifdef	ipForwDatagrams
	if (ot = text2obj("ipForwDatagrams"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipForwDatagrams;
#endif
#ifdef	ipInUnknownProtos
	if (ot = text2obj("ipInUnknownProtos"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipInUnknownProtos;
#endif
#ifdef	ipInDiscards
	if (ot = text2obj("ipInDiscards"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipInDiscards;
#endif
#ifdef	ipInDelivers
	if (ot = text2obj("ipInDelivers"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipInDelivers;
#endif
#ifdef	ipOutRequests
	if (ot = text2obj("ipOutRequests"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipOutRequests;
#endif
#ifdef	ipOutDiscards
	if (ot = text2obj("ipOutDiscards"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipOutDiscards;
#endif
#ifdef	ipOutNoRoutes
	if (ot = text2obj("ipOutNoRoutes"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipOutNoRoutes;
#endif
	if (ot = text2obj("ipReasmTimeout"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipReasmTimeout;
#ifdef	ipReasmReqds
	if (ot = text2obj("ipReasmReqds"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipReasmReqds;
#endif
#ifdef	ipReasmOKs
	if (ot = text2obj("ipReasmOKs"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipReasmOKs;
#endif
#ifdef	ipReasmFails
	if (ot = text2obj("ipReasmFails"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipReasmFails;
#endif
#ifdef	ipFragOKs
	if (ot = text2obj("ipFragOKs"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipFragOKs;
#endif
#ifdef	ipFragFails
	if (ot = text2obj("ipFragFails"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipFragFails;
#endif
#ifdef	ipFragCreates
	if (ot = text2obj("ipFragCreates"))
		ot->ot_getfnx = o_ip, ot->ot_info = (caddr_t) ipFragCreates;
#endif

	if (ot = text2obj("ipAdEntAddr"))
		ot->ot_getfnx = o_ip_addr, ot->ot_info = (caddr_t) ipAdEntAddr;
	if (ot = text2obj("ipAdEntIfIndex"))
		ot->ot_getfnx = o_ip_addr, ot->ot_info = (caddr_t) ipAdEntIfIndex;
	if (ot = text2obj("ipAdEntNetMask"))
		ot->ot_getfnx = o_ip_addr, ot->ot_info = (caddr_t) ipAdEntNetMask;
	if (ot = text2obj("ipAdEntBcastAddr"))
		ot->ot_getfnx = o_ip_addr, ot->ot_info = (caddr_t) ipAdEntBcastAddr;
	if (ot = text2obj("ipAdEntReasmMaxSize"))
		ot->ot_getfnx = o_ip_addr, ot->ot_info = (caddr_t) ipAdEntReasmMaxSize;

	if (ot = text2obj("ipRouteDest"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteDest;
	if (ot = text2obj("ipRouteIfIndex"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteIfIndex;
	if (ot = text2obj("ipRouteMetric1"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteMetric1;
	if (ot = text2obj("ipRouteMetric2"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteMetric2;
	if (ot = text2obj("ipRouteMetric3"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteMetric3;
	if (ot = text2obj("ipRouteMetric4"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteMetric4;
	if (ot = text2obj("ipRouteNextHop"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteNextHop;
	if (ot = text2obj("ipRouteType"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteType;
	if (ot = text2obj("ipRouteProto"))
		ot->ot_getfnx = o_ip_route, ot->ot_info = (caddr_t) ipRouteProto;
#ifdef	ipRouteAge
	if (ot = text2obj("ipRouteAge"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteAge;
#endif
	if (ot = text2obj("ipRouteMask"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteMask;
	if (ot = text2obj("ipRouteMetric5"))
		ot->ot_getfnx = o_ip_route,
		    ot->ot_setfnx = s_ip_route, ot->ot_info = (caddr_t) ipRouteMetric5;
	if (ot = text2obj("ipRouteInfo"))
		ot->ot_getfnx = o_ip_route, ot->ot_info = (caddr_t) ipRouteInfo;

	if (ot = text2obj("unixIpRouteFlags"))
		ot->ot_getfnx = o_ip_route, ot->ot_info = (caddr_t) unixIpRouteFlags;
	if (ot = text2obj("unixIpRouteRefCnt"))
		ot->ot_getfnx = o_ip_route, ot->ot_info = (caddr_t) unixIpRouteRefCnt;
	if (ot = text2obj("unixIpRouteUses"))
		ot->ot_getfnx = o_ip_route, ot->ot_info = (caddr_t) unixIpRouteUses;

	if (ot = text2obj("unixRouteBadRedirects"))
		ot->ot_getfnx = o_ip_routing_stats, ot->ot_info = (caddr_t) unixRouteBadRedirects;
	...
	if (ot = text2obj("unixRouteLookupFails"))
		ot->ot_getfnx = o_ip_routing_stats, ot->ot_info = (caddr_t) unixRouteLookupFails;
	if (ot = text2obj("unixRouteWildcardUses"))
		ot->ot_getfnx = o_ip_routing_stats, ot->ot_info = (caddr_t) unixRouteWildcardUses;

	if (ot = text2obj("atIfIndex"))
		ot->ot_getfnx = o_address,
		    ot->ot_setfnx = s_address, ot->ot_info = (caddr_t) atIfIndex;
	if (ot = text2obj("atPhysAddress"))
		ot->ot_getfnx = o_address,
		    ot->ot_setfnx = s_address, ot->ot_info = (caddr_t) atPhysAddress;
	if (ot = text2obj("atNetAddress"))
		ot->ot_getfnx = o_address,
		    ot->ot_setfnx = s_address, ot->ot_info = (caddr_t) atNetAddress;

	if (ot = text2obj("ipNetToMediaIfIndex"))
		ot->ot_getfnx = o_address,
		    ot->ot_setfnx = s_address, ot->ot_info = (caddr_t) ipNetToMediaIfIndex;
	if (ot = text2obj("ipNetToMediaPhysAddress"))
		ot->ot_getfnx = o_address,
		    ot->ot_setfnx = s_address, ot->ot_info = (caddr_t) ipNetToMediaPhysAddress;
	if (ot = text2obj("ipNetToMediaNetAddress"))
		ot->ot_getfnx = o_address,
		    ot->ot_setfnx = s_address, ot->ot_info = (caddr_t) ipNetToMediaNetAddress;
	if (ot = text2obj("ipNetToMediaType"))
		ot->ot_getfnx = o_address,
		    ot->ot_setfnx = s_address, ot->ot_info = (caddr_t) ipNetToMediaType;
}

...
privs_print(x, os)
	...
	OS os;
{
	printf("%s", sprintb((int) *x, "\020\01get\02get-next\03get-response\04set\05trap"));
}
...
add_null()
{
	(void) add_syntax("NULL", null_encode, null_decode, null_free, null_parse, null_print);
}
	...
	PE pe;
{
	...
	if (decode_SNMP_IpAddress(pe, 1, NULLIP, NULLVP, &ip) == NOTOK)
		return NOTOK;
	if (qb_pullup(ip) == NOTOK || (isock = (struct sockaddr_in *) calloc(1, sizeof *isock))
	    == NULL) {
	...
	bcopy(qb->qb_data, (char *) &isock->sin_addr, sizeof isock->sin_addr);
...
u_long
prim2ulong(pe)				/* also used in SNMP-capable gawk... */
	register PE pe;
{
	register u_long i;
...
PE
ulong2prim(i, class, id)		/* also used in SNMP-capable gawk... */
	register u_long i;
	PElementClass class;
	PElementID id;
{
	...
	PE *pe;
{
	...
	if ((*pe = str2prim(buffer, (int) (x->siso_nlen + 1), PE_CLASS_APPL, 5)) == NULLPE)
		return NOTOK;
	...
	PE pe;
{
	...
	if (decode_SNMP_ClnpAddress(pe, 1, NULLIP, NULLVP, &clnp) == NOTOK)
		return NOTOK;
	if (qb_pullup(clnp) == NOTOK || (isock = (struct sockaddr_iso *) calloc(1, sizeof *isock))
	    == NULL) {
	...
	if ((len = qb->qb_data[0] & 0xff) >= qb->qb_len)
		len = qb->qb_len - 1;
	bcopy(qb->qb_data + 1, isock->siso_data, (int) (isock->siso_nlen = len));
	...
	char *s;
{
	...
	if ((isock = (struct sockaddr_iso *) calloc(1, sizeof *isock)) == NULL)
		return NOTOK;
	isock->siso_family = AF_ISO;
	isock->siso_nlen = implode((u_char *) isock->siso_data, s, strlen(s));
	*x = isock;
	...
	OS os;
{
	...
	buffer[explode(buffer, (u_char *) x->siso_data, (int) x->siso_nlen)] = NULL;
	printf("NS+%s", buffer);
}
...
add_syntax(name, f_encode, f_decode, f_free, f_parse, f_print)
	char *name;
	IFP f_encode, f_decode, f_free, f_parse, f_print;
{
	int i;

...
f_audit(vec)
	char **vec;
{
	int follow, forever, i;
	long longtimeago, now;
	char *cp, *file;
	PE pe, p;
	PS ps2;
	...
	char **vec;
{
	...
	for (result = NOTOK; *vec; vec++) {
		...
		if (et) {
			register OID eid = et->ot_name, oid = ot->ot_name;

			if (eid->oid_nelem != oid->oid_nelem - 1
			    || bcmp((char *) eid->oid_elements,
				    (char *) oid->oid_elements,
				    eid->oid_nelem * sizeof(eid->oid_elements[0])) != 0) {
...
/*	    int	    i; */
			...
			if (et == NULL) {
				advise(NULLCP, "unable to find row object for %s", ot->ot_text);
				goto out;
			}
			if (et->ot_syntax
			    /* || (i = strlen (et -> ot_text)) <= 5 || strcmp (et -> ot_text + i -
			       5, "Entry") */ ) {
		...
		*vp = bind, vp = &bind->next;
		...
		if ((v->name = oid_cpy(ot->ot_name)) == NULLOID
		    || (v->value = pe_alloc(PE_CLASS_UNIV, PE_FORM_PRIM, PE_PRIM_NULL)) == NULLPE)
			adios(NULLCP, "out of memory");
	}
	...
	char **vec;
{
	...
	if (file) {
		...
		if (sp > file && strcmp(sp, ".defs") == 0)
			(void) sprintf(sysout, "%.*s.%s", sp - file, file, source ? "c" : "defs");
		else
			(void) sprintf(sysout, "%s.%s", file, source ? "c" : "defs");
	...
	for (ot = text2obj("ccitt"); ot; ot = ot->ot_next) {
		...
		if (source) {
			...
			fprintf(fp, ", %d, (u_long) 0, %d,\n",
				ot->ot_access & OT_RDWRITE, ot->ot_status & OT_DEPRECATED);
			fprintf(fp,
...
#undef	OT_OFFSET
		...
		if ((os = ot->ot_syntax) || ot->ot_status)
			fprintf(fp, " %-15s %d %d %d",
				...
				ot->ot_status & OT_DEPRECATED, THASH(ot->ot_text));
		else
	...
	if (!fast)
		advise(NULLCP, "%d objects written to %s", i, file ? sysout : "stdout");
	...
	char **vec;
{
	...
	if ((parm = msg->data->un.get__response)->request__id != request_id) {
	...
	for (vp = parm->variable__bindings; vp; vp = vp->next) {
		...
		if (oid
			...
			|| bcmp((char *) oid->oid_elements,
				...
				oid->oid_nelem * sizeof oid->oid_elements[0])))
			goto out;
	...
	int offset;
{
	...
	if (cp == NULL) {
		...
		if ((*os->os_parse) (&value, cp) == NOTOK) {
			advise(NULLCP, "invalid value for variable \"%s\": \"%s\"", ava, cp);
			return NOTOK;
		}
	...
	struct type_SNMP_Message *msg;
{
	...
	for (vp = parm->variable__bindings; vp; vp = vp->next) {
		...
		if ((os = oi->oi_type->ot_syntax) == NULL) {
			advise(NULLCP, "unknown syntax for object \"%s\"", oi->oi_type->ot_text);
			goto no_dice;
		}
		if ((*os->os_decode) (&value, v->value) == NOTOK) {
			advise(NULLCP, "decode error for variable \"%s\"", oid2ode(v->name));
			goto no_dice;
		}
...
"ifType", ifType,
	    ...
	    "unixNetstat", enabled, sizeof enabled / sizeof enabled[0], NULL};
	...
	for (iv = ivars; iv->iv_object; iv++)
		if (ot = text2obj(iv->iv_object)) {
			...
			(void) add_syntax(iv->iv_object, os->os_encode,
					  os->os_decode, os->os_free, os->os_parse, enum_print);
			if ((os = text2syn(iv->iv_object)) == NULL)
				adios(NULLCP, "lost syntax for object \"%s\"", iv->iv_object);
			ot->ot_syntax = os;
			os->os_name = name;
	...
	char **vec;
{
	...
	(void) strncpy(na->na_domain, getlocalhost(), sizeof na->na_domain - 1);
...
#ifdef	TCP
	{
		...
		/* If the request is being sent out on the loopback interface, make sure it appears 
		   to have originated from the loopback interface, rather than the interface used
		   as the default hostname.  If the interface used as the default hostname is a
		   serial link, responses to requests with this origin may be undeliverable.  (EJP) 
		 */

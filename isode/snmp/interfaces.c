	...
	int offset;
{
	...
	case ifOperStatus:
		return o_integer(oi, v, ifn->if_flags & IFF_UP ? OPER_UP : OPER_DOWN);

#ifdef	ifLastChange
	case ifLastChange:
		if ((diff = (ifn->if_lastchange.tv_sec - my_boottime.tv_sec)
		     * 100 + ((ifn->if_lastchange.tv_usec - my_boottime.tv_usec)
			      / 10000))
		    < 0)
			diff = 0;
	...
	int offset;
{
	...
	case type_SNMP_PDUs_commit:
		...
		if (ioctl(nd, SIOCGIFFLAGS, &ifreq) == NOTOK) {
			advise(LLOG_EXCEPTIONS, "failed", "SIOCGIFFLAGS on %s", is->ifn_descr);
			break;
		} else {
			switch (is->ifn_admin) {
			case OPER_UP:
	...
	*cp++ = NULL;
	...
	if (lexequ(ava, "ifType") == 0) {
		if (sscanf(cp, "%d", &i) != 1 || i < TYPE_MIN || i > TYPE_MAX) {
		      malformed:;
			advise(LLOG_EXCEPTIONS, NULLCP, "malformed attribute \"%s=%s\"", ava, cp);
			return;
		}
...
init_interfaces()
{
	...
	struct ifnet *ifnet;
	register OT ot;
	register struct interface *is, **ifp;
	struct nlist nzs;
	...
	for (i = 0; ifnet; i++) {
		...
		nz->n_name = "if_name", nz->n_value = (unsigned long) ifn->if_name;
		if (getkmem(nz, (caddr_t) is->ifn_descr, sizeof is->ifn_descr - 1)
		    == NOTOK)
			goto disabled;
		is->ifn_descr[sizeof is->ifn_descr - 1] = NULL;
		(void) sprintf(is->ifn_descr + strlen(is->ifn_descr), "%d", ifn->if_unit);
	...
	if (ot = text2obj("ifIndex"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifIndex;
	if (ot = text2obj("ifDescr"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifDescr;
	if (ot = text2obj("ifType"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifType;
	if (ot = text2obj("ifMtu"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifMtu;
	if (ot = text2obj("ifSpeed"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifSpeed;
	if (ot = text2obj("ifPhysAddress"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifPhysAddress;
	if (ot = text2obj("ifAdminStatus"))
		ot->ot_getfnx = o_interfaces,
		    ot->ot_setfnx = s_interfaces, ot->ot_info = (caddr_t) ifAdminStatus;
	if (ot = text2obj("ifOperStatus"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifOperStatus;
#ifdef	ifLastChange
	if (ot = text2obj("ifLastChange"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifLastChange;
#endif
#ifdef	ifInOctets
	if (ot = text2obj("ifInOctets"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifInOctets;
#endif
	if (ot = text2obj("ifInUcastPkts"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifInUcastPkts;
#ifdef	ifInNUcastPkts
	if (ot = text2obj("ifInNUcastPkts"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifInNUcastPkts;
#endif
#ifdef	ifInDiscards
	if (ot = text2obj("ifInDiscards"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifInDiscards;
#endif
	if (ot = text2obj("ifInErrors"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifInErrors;
#ifdef	ifInUnknownProtos
	if (ot = text2obj("ifInUnknownProtos"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifInUnknownProtos;
#endif
#ifdef	ifOutOctets
	if (ot = text2obj("ifOutOctets"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifOutOctets;
#endif
	if (ot = text2obj("ifOutUcastPkts"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifOutUcastPkts;
#ifdef	ifOutNUcastPkts
	if (ot = text2obj("ifOutNUcastPkts"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifOutNUcastPkts;
#endif
	if (ot = text2obj("ifOutDiscards"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifOutDiscards;
	if (ot = text2obj("ifOutErrors"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifOutErrors;
	if (ot = text2obj("ifOutQLen"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifOutQLen;
	if (ot = text2obj("ifSpecific"))
		ot->ot_getfnx = o_interfaces, ot->ot_info = (caddr_t) ifSpecific;
}
...
static int
adr_compar(a, b)
	register struct address **a, **b;
{
	int i;

	if ((i = (*a)->adr_address.sa.sa_family - (*b)->adr_address.sa.sa_family))
		return (i > 0 ? 1 : -1);

	return elem_cmp((*a)->adr_instance, (*a)->adr_insize, (*b)->adr_instance, (*b)->adr_insize);
}
	...
	int offset;
{
	int adrNumber = 0;
	register struct interface *is;
	register struct address *as, *ap, **base, **afe, **afp;
	static int first_time = 1;
	static int lastq = -1;

	if (quantum == lastq)
		return OK;
	if (!flush_if_cache && offset == type_SNMP_PDUs_get__next__request && quantum == lastq + 1) {	/* XXX: 
													   caching! 
													 */
		lastq = quantum;
		return OK;
	...
	for (is = ifs; is; is = is->ifn_next) {
...
#ifdef	BSD44
		union sockaddr_un ifsocka, ifsockb;
#endif
		union sockaddr_un ifsockc;
		register union sockaddr_un *ia, *ib;
		...
		if (as = find_address((union sockaddr_un *) &ifn->if_addr))
			...
			*afp = as, afp = &as->adr_next, adrNumber++;
			...
			default:
				bzero((char *) as->adr_instance, sizeof as->adr_instance);
				as->adr_insize = 0;
				break;
			}
		}
		...
		for (ifa = ifn->if_addrlist; ifa; ifa = ifaddr.ifa_next) {
			nz->n_name = "struct ifaddr", nz->n_value = (unsigned long) ifa;
			...
			if (getkmem(nz, (caddr_t) ic, sizeof *ic) == NOTOK)
				continue;
#endif
			if (ia->sa.sa_family == AF_INET && ic->un_in.sin_addr.s_addr == 0) {
			...
			if (as = find_address(ia))
				...
				*afp = as, afp = &as->adr_next, adrNumber++;
				...
				case AF_INET:
					as->adr_insize =
					    ipaddr2oid(as->adr_instance, &ia->un_in.sin_addr);
					if (afs_inet == NULL)	/* needed for find_address */
						afs_inet = as;
					break;
				...
				case AF_ISO:
					as->adr_insize =
					    clnpaddr2oid(as->adr_instance, &ia->un_iso.siso_addr);
	...
	if (debug && first_time) {
		...
		for (as = afs; as; as = as->adr_next) {
			...
			advise(LLOG_DEBUG, NULLCP,
			       ...
			       as->adr_address.sa.sa_family, sprintoid(&oids), as->adr_indexmask);
		}
	}
		...
		for (as = afs_inet; as; as = as->adr_next)
			if (as->adr_address.sa.sa_family != AF_INET)
				break;
				      ...
				      (char *) &as->adr_address.un_in.sin_addr, sizeof *in) == 0)
				return as;
		break;
		...
		for (as = afs_iso; as; as = as->adr_next)
			if (as->adr_address.sa.sa_family != AF_ISO)
				break;
				      ...
				      (char *) &as->adr_address.un_iso.siso_addr, sizeof *iso) == 0)
				return as;

...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/snmp/RCS/view-g.c,v 9.0 1992/06/16 12:38:11 isode Rel";
#endif
	...
	int offset;
{
	...
	case viewPrimTDomain:
		return o_specific(oi, v, (caddr_t) (vu->v_community ? rfc1157Domain : localAgent));
	...
	int offset;
{
	...
	case viewAclPrivileges:
		return o_integer(oi, v, ((c->c_permission & OT_RDONLY) ? 3 : 0)
...
view_compar(a, b)
	struct view **a, **b;
{
	return elem_cmp((*a)->v_instance, (*a)->v_insize, (*b)->v_instance, (*b)->v_insize);
}

static int
comm_compar(a, b)
	struct community **a, **b;
{
	return elem_cmp((*a)->c_instance, (*a)->c_insize, (*b)->c_instance, (*b)->c_insize);
}
...
trap_compar(a, b)
	struct trap **a, **b;
{
	return elem_cmp((*a)->t_instance, (*a)->t_insize, (*b)->t_instance, (*b)->t_insize);
}
...
"defViewWholeRW", NULL,
	    "defViewWholeRO", NULL, "defViewStandardRW", "mgmt", "defViewStandardRO", "mgmt", NULL};

/*  */

init_view()
{
	char buffer[BUFSIZ], *vec[4];
	register OT ot;
	...
	if (ot = text2obj("viewPrimName"))
		ot->ot_getfnx = o_viewPrim, ot->ot_info = (caddr_t) viewPrimName;
	if (ot = text2obj("viewPrimTDomain"))
		ot->ot_getfnx = o_viewPrim, ot->ot_info = (caddr_t) viewPrimTDomain;
	if (ot = text2obj("viewPrimTAddr"))
		ot->ot_getfnx = o_viewPrim, ot->ot_info = (caddr_t) viewPrimTAddr;
	if (ot = text2obj("viewPrimUser"))
		ot->ot_getfnx = o_viewPrim, ot->ot_info = (caddr_t) viewPrimUser;
	if (ot = text2obj("viewPrimCommunity"))
		ot->ot_getfnx = o_viewPrim, ot->ot_info = (caddr_t) viewPrimCommunity;
	if (ot = text2obj("viewPrimType"))
		ot->ot_getfnx = o_viewPrim, ot->ot_info = (caddr_t) viewPrimType;

	if (ot = text2obj("viewAclView"))
		ot->ot_getfnx = o_viewAcl, ot->ot_info = (caddr_t) viewAclView;
	if (ot = text2obj("viewAclCommunity"))
		ot->ot_getfnx = o_viewAcl, ot->ot_info = (caddr_t) viewAclCommunity;
	if (ot = text2obj("viewAclUser"))
		ot->ot_getfnx = o_viewAcl, ot->ot_info = (caddr_t) viewAclUser;
	if (ot = text2obj("viewAclPrivileges"))
		ot->ot_getfnx = o_viewAcl, ot->ot_info = (caddr_t) viewAclPrivileges;
	if (ot = text2obj("viewAclType"))
		ot->ot_getfnx = o_viewAcl, ot->ot_info = (caddr_t) viewAclType;

	if (ot = text2obj("viewTrapView"))
		ot->ot_getfnx = o_viewTrap, ot->ot_info = (caddr_t) viewTrapView;
	if (ot = text2obj("viewTrapGenerics"))
		ot->ot_getfnx = o_viewTrap, ot->ot_info = (caddr_t) viewTrapGenerics;
	if (ot = text2obj("viewTrapSpecifics"))
		ot->ot_getfnx = o_viewTrap, ot->ot_info = (caddr_t) viewTrapSpecifics;
	if (ot = text2obj("viewTrapType"))
		ot->ot_getfnx = o_viewTrap, ot->ot_info = (caddr_t) viewTrapType;
}
...
fin_view()
{
	...
	for (v = VHead->v_forw; v != VHead; v = v->v_forw)
		i++;
	if (i > 0) {
		register struct view **base, **bp, **ep;
		...
		for (v = VHead->v_forw; v != VHead; v = v->v_forw) {
			...
			for (ip = v->v_instance + 1, jp = oid->oid_elements; j > 0; j--)
				*ip++ = *jp++;
	...
	if (i > 0) {
		int j;
		register struct community **base, **bp, **ep;
	...
	for (t = UHead->t_forw; t != UHead; t = t->t_forw)
		i++;
	if (i > 0) {
		register struct trap **base, **bp, **ep;
		...
		for (t = UHead->t_forw; t != UHead; t = t->t_forw) {
			...
			for (ip = t->t_instance + 1, jp = oid->oid_elements; j > 0; j--)
				*ip++ = *jp++;
	...
	char **vec;
{
	...
	if (*vec) {
		...
		else if (lexequ(*vec, "readWrite") == 0)
			c->c_permission = OT_RDWRITE;
		else if (lexequ(*vec, "writeOnly") == 0)
			c->c_permission = OT_WRONLY;
		else if (lexequ(*vec, "none")) {
			advise(LLOG_EXCEPTIONS, NULLCP, "invalid access mode \"%s\"", *vec);
			goto you_lose;
		}
	...
	char **vec;
{
	...
	if (lexequ(*vec, "rfc1157")) {
		advise(LLOG_EXCEPTIONS, NULLCP, "unsupported proxy domain \"%s\"", *vec);
		goto you_lose;
	}
	vec++;
...
{
	char buffer[BUFSIZ];
	register struct subtree *s, *x, *y;
	register struct view *v, *u;

	if (viewmask == 0) {
		advise(LLOG_EXCEPTIONS, NULLCP, "too many views starting with \"%s\"", *vec);
		return NOTOK;
	}
	...
	for (vec++; *vec; vec++) {
		...
		for (x = s->s_forw; x != s; x = y) {
			...
			if (bcmp((char *) x->s_subtree->oid_elements,
				 (char *) name->oid_elements, ((i = x->s_subtree->oid_nelem)
	...
	int proxy;
{
	...
	if (hp = gethostbystring(s)) {
		...
		na->na_community = ts_comm_tcp_default;
		inaddr_copy(hp, &sin);
		(void) strncpy(na->na_domain, inet_ntoa(sin.sin_addr), sizeof na->na_domain - 1);
	} else
#endif
	...
	case NA_TCP:
			...
			goto you_lose;
		{
			...
			sin.sin_port = na->na_port ? na->na_port : proxy ? udport : traport;

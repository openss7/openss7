	...
	struct search *lookat;
{
	...
	if (!ot->ot_getfnx && instance)
		fatal("can't use SNMP scalar variable \"%s\" as control for for-in", ot->ot_text);
...
void
snmp_assoc_next(lookat, alldone)
	struct search *lookat;
	int alldone;
{
	int i;
	...
	if (alldone || snmp_get_next(s) == NOTOK || s->s_reqs[0].r_bindings == NULL) {
		register struct snmp_req *sr;

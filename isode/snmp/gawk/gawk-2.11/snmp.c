...
NODE *AGENT_node, *COMMUNITY_node, *DIAGNOSTIC_node, *ERROR_node, *RETRIES_node, *TIMEOUT_node;
NODE *Ndot_string;
...
snmp_init()
{
	...
	AGENT_node = install(variables, "AGENT",
			     node(make_string(addr, strlen(addr)), Node_var, (NODE *) NULL));
...
"INTEGER", e_integer, d_integer,
	    ...
	    "TimeTicks", e_timeticks, d_ulong, "ClnpAddress", e_clnpaddr, d_clnpaddr, NULL};
...
snmp_onceonly()
{
	...
	for (ot = text2obj("ccitt"); ot; ot = ot->ot_next) {
		if (ot->ot_syntax
		    || (i = strlen(ot->ot_text)) <= 5 || strcmp(ot->ot_text + i - 5, "Entry"))
			continue;
...
int
snmp_get(ptr, instname)
	NODE *ptr;
	char *instname;
{
	int gotone, retries, status = -1;
	struct type_SNMP_Message *msg = &msgs;
	register struct type_SNMP_PDU *parm = msg->data->un.get__request;
	register struct type_SNMP_VarBind *v = parm->variable__bindings->VarBind;
	PE pe = NULLPE, p = NULLPE;
	OID oid;
	OT ot = (OT) ptr->magic;
	NODE *value = NULL;
	...
	if (instname == NULL) {
			...
			for (s = tail; s; s = s->s_prev) {
				...
				if (ot->ot_name->oid_nelem
				    ...
				    || bcmp((char *) ot->ot_name->oid_elements,
					    ...
					    oid->oid_nelem * sizeof ot->ot_name->oid_elements[0]))
					continue;
		...
		if (inst == NULL) {
			snmp_diag(NULLCP, "str2oid: bad instance identifier \"%s\"", instname);
			goto out;
		}
	...
	for (retries = snmp_retries; retries > 0;) {
		...
		if (msg->data->offset != type_SNMP_PDUs_get__response) {
			snmp_diag(NULLCP, "unexpected message type %d", msg->data->offset);
			goto out;
		}
	...
	if (retries <= 0) {
		snmp_diag(NULLCP,
			  ...
			  gotone ? "upto " : "", snmp_timeout, snmp_timeout != 1 ? "s" : "");
		goto out;
	}
	...
	if (parm->variable__bindings == NULL || (v = parm->variable__bindings->VarBind) == NULL) {
...
struct search *
snmp_assoc_scan(symbol, instance)
	NODE *symbol, *instance;
{
	register struct snmp_search *s;
	OID inst;
	register OT ot = (OT) symbol->magic;
	register struct type_SNMP_VarBindList **vp, **vp2;
	...
	for (ot = ot->ot_children; ot; ot = ot->ot_sibling) {
		...
		register struct type_SNMP_VarBind *v, *v2;

		if (!ot->ot_syntax)
			continue;
		emalloc(bind, struct type_SNMP_VarBindList *, sizeof *bind, "snmp_assoc_scan2");
		...
		*vp = bind, vp = &bind->next;
		...
		if (inst) {
			...
			ip = v->name->oid_elements + v->name->oid_nelem - inst->oid_nelem;
			jp = inst->oid_elements;
			for (i = inst->oid_nelem; i > 0; i--)
				*ip++ = *jp++;
		...
		emalloc(bind, struct type_SNMP_VarBindList *, sizeof *bind, "snmp_assoc_scan4");

		*vp2 = bind, vp2 = &bind->next;
		bind->next = NULL;

		emalloc(v2, struct type_SNMP_VarBind *, sizeof *v2, "snmp_assoc_scan5");
	...
	int done;
{
	...
	if (done || snmp_get_next(s) == NOTOK || s->s_reqs[0].r_bindings == NULL) {
		register struct snmp_req *sr;
	...
	if ((v = s->s_reqs[0].r_bindings->VarBind) == NULL || (oid = v->name) == NULL)
		fatal("snmp_assoc_next: internal error2");
	if (ot->ot_name->oid_nelem >= oid->oid_nelem
	    || bcmp((char *) ot->ot_name->oid_elements,
		    (char *) oid->oid_elements,
		    ot->ot_name->oid_nelem * sizeof oid->oid_elements[0]))
...
snmp_get_next(s)
	register struct snmp_search *s;
{
	register struct type_SNMP_VarBindList *vp, *vp2, **vpp, **vpp2;
	register struct snmp_req *sr, *sp;
	...
	for (sr = s->s_reqs; sr->r_bindings; sr++) {
		...
		while ((vp = *vpp) && (vp2 = *vpp2)) {
			OID v = vp->VarBind->name, v2 = vp2->VarBind->name;

			if (v->oid_nelem > v2->oid_nelem
			    || bcmp((char *) v->oid_elements,
				    (char *) v2->oid_elements,
				    v->oid_nelem * sizeof v->oid_elements[0])) {
				*vpp = vp->next;
				vp->next = NULL;
	...
	for (sp = s->s_reqs; sp < sr; sp++) {
		...
		if (sp->r_bindings && sp->r_msg->data->un.get__request->variable__bindings) {
			sp->r_bindings = sp->r_msg->data->un.get__request->variable__bindings;
...
snmp_get_next_aux(s)
	register struct snmp_search *s;
{
	int gotone, result, retries, status = -1;
	...
	for (retries = snmp_retries; retries > 0;) {
		...
		case int_SNMP_error__status_tooBig:
		{
			...
			for (sp = sr + 1; sp->r_bindings; sp++)
				if (sp >= sz) {
					snmp_diag(NULLCP,
						  "too many operations needed (%d max)", NREQ - 1);
					goto out;
				}
			...
			for (vpp = &sr->r_bindings; i-- > 0; vpp = &((*vpp)->next))
				continue;
	...
	snmp_diag(NULLCP,
		  "no %sresponse within %d retries of %s%d second%s each",
		  gotone ? "acceptable " : "", snmp_retries + gotone,
		  gotone ? "up to " : "", snmp_timeout, snmp_timeout != 1 ? "s" : "");
	return NOTOK;
...
snmp_set()
{
	...
	register struct type_SNMP_PDU *parm = msg->data->un.get__request;
	register struct type_SNMP_VarBind *v = parm->variable__bindings->VarBind;
	PE pe = NULLPE, p = NULLPE, *pep = NULLPEP;
	OID oid;
	NODE *ptr = SET_node, *value = NULL;
	OT ot = (OT) ptr->magic;
	...
	PE *pe;
{
	...
	if ((i = strlen(tmp)) == 0 || i % 3 != 2 || !isxdigit(tmp[0])
	    || !isxdigit(tmp[1])
	    || !isxdigit(tmp[i - 2])
	    || !isxdigit(tmp[i - 1]))
	...
	PE *pe;
{
	...
	if (oid == NULLOID) {
	      ...
	      bad_oid:;
#endif
		(void) sprintf(PY_pepy, "str2oid: bad instance identifier \"%s\"", tmp);
		return NOTOK;
	}
#ifndef	original
	if (oid->oid_elements[0] > 2
	    || (oid->oid_nelem > 1 && oid->oid_elements[0] < 2 && oid->oid_elements[1] < 39))
		goto bad_oid;
#endif
	...
	PE pe;
{
	...
	if (qb == NULL || qb_pullup(qb) == NOTOK) {
		(void) strcpy(PY_pepy, qb ? "qb_pullup: out of memory" : pe_error(pe->pe_errno));
		return NOTOK;
	}
	...
	PE pe;
{
	...
	if ((qb = ip->qb_forw)->qb_len != 4) {
		(void) sprintf(PY_pepy, "IpAddress is wrong length (got %d, wanted 4)", qb->qb_len);
		free_SNMP_IpAddress(ip);
		return NOTOK;
	}
	...
	int do_id;
{
	...
	if (snmp_community == NULL || strcmp(snmp_community, tmp->stptr)) {
		...
		emalloc(snmp_community, char *, strlen(tmp->stptr) + 1, "snmp_ready2");

		(void) strcpy(snmp_community, tmp->stptr);
		if ((msg->community = str2qb(snmp_community, strlen(snmp_community), 1)) == NULL) {
	...
	if ((hp = gethostbystring(pp = snmp_agent)) == NULL) {
		...
		if ((ta = str2taddr(snmp_agent))
		    && ta->ta_naddr > 0 && (na = ta->ta_addrs)->na_stack == NA_TCP) {
...
{
	int result = NOTOK;
	u_long hostaddr, netmask, netaddr;
	register char *cp;
	char buffer[BUFSIZ + 1], *vec[NVEC + 1];
	FILE *fp;
	...
	while (fgets(buffer, sizeof buffer, fp)) {
		...
		if (!(netaddr & 0xff000000))
			netaddr <<= (netaddr & 0x00ff0000) ? 8 : (netaddr & 0x0000ff00) ? 16 : 24;
	...
	char *s;
{
	...
	if ((i = str2elem(s, elements)) < 1
	    || elements[0] > 2 || (i > 1 && elements[0] < 2 && elements[1] > 39))
		return NULLOID;
#else
	if ((i = str2elem(s, elements)) < 1)
		return NULLOID;

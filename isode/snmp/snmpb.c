...
struct invocation {
	...
	int i_info;			/* info > 0: response info == 0: request info < 0: not yet
					   ready */
	...
	char *community;
{
	...
	for (timelap = 0; THead->t_forw != THead; timelap++) {
		...
		for (t = THead->t_forw; t != THead; t = u) {
			...
			register struct type_SNMP_VarBindList *vp, *vb2;
			...
			if (oid_cmp
			    (t->t_arg, b =
			     t->t_msg->data->un.get__response->variable__bindings->VarBind->name)
			    >= 0) {
				char buffer[BUFSIZ];
			...
			*bp = bv, bp = &bv->b_next;
			...
			for (vp = t->t_msg->data->un.get__response->variable__bindings, vb2 = vb;
			     vp; vp = vp->next, vb2 = vb2->next) {
				int fixup = 0;
				...
				if (a->oid_nelem > b->oid_nelem
				    || bcmp((char *) a->oid_elements,
					    (char *) b->oid_elements,
					    a->oid_nelem * sizeof a->oid_elements[0])) {
					oid_free(b);
 ...
 */
			...
			if (curthreads < threadlimit
			    && !backoff && (a = oid_median(t->t_arg, t->t_hi))) {
	...
	advise(NULLCP, "messages: %d request%s sent,  along with %d retr%s",
	       totreqs, totreqs != 1 ? "s" : "", totretr, totretr != 1 ? "ies" : "y");
	advise(NULLCP, "          %d response%s rcvd, along with %d duplicate%s",
	       totrsps, totrsps != 1 ? "s" : "", totdups, totdups != 1 ? "s" : "");
...
static int
wait_for_action(sd, ps)
	int sd;
	PS ps;
{
	int backoff, n, nfds, request_id;
	long maxrtt;
	u_long lastime;
	fd_set rfds;
	struct timeval tvs;
	...
	register struct request *r, *u;
	register struct thread *t, *s;
	PE pe;
	...
	default:
		...
		if (msg->data->offset != type_SNMP_PDUs_get__response) {
			advise(NULLCP, "unexpected message type %d", msg->data->offset);
			goto oops;
		}

		request_id = (parm = msg->data->un.get__response)->request__id;
		...
		for (t = THead->t_forw; t != THead; t = s) {
			...
			if (t->t_info) {
				advise(NULLCP, "duplicated response for request-id %d", request_id);
				goto duplicate_id;
			}
			...
			if (parm->error__status != int_SNMP_error__status_noError) {
				if (parm->error__status != int_SNMP_error__status_noSuchName)
					advise(NULLCP, "%s for get-next of %s, thread dead",
					       snmp_error(parm->error__status), oid2ode(t->t_arg));
				free_thread(t);
				dedthreads++;
			}
		...
		for (r = RHead->r_forw; r != RHead; r = u) {
			...
			case int_SNMP_error__status_tooBig:
				advise(NULLCP, "got %s on request of size %d",
				       snmp_error(parm->error__status), r->r_nbound);
			...
			case int_SNMP_error__status_noSuchName:
				...
				{
					register int j;
					register struct bound *b, **bp;
					register struct type_SNMP_VarBindList *v, **vp;

					bp = &r->r_bounds;
					vp = &r->r_msg->data->un.get__request->variable__bindings;
					for (j = parm->error__index;
					     --j > 0; bp = &((*bp)->r_next), vp = &((*vp)->next))
						if (*bp == NULL)
							goto invalid_index;
					advise(NULLCP,
					       "got %s on %s",
			...
			if (i->i_retries == 0) {
				...
				if (debug)
					fprintf(stderr,
						...
						timenow, i->i_lastime, timenow - i->i_lastime);
				backoff = 0;
			} else
				i->i_retries = 0;
			goto next_request;
	...
	if (i && i->i_lastime + timeout < timenow) {
		...
		if (watch) {
			...
			fprintf(stderr,
				...
				i->i_rid, timenow, timenow - i->i_lastime, timeout);
...
static
print_bulk(bl, vb, partial)
	struct binding *bl;
	struct type_SNMP_VarBindList *vb;
	int partial;
{
	int i;
	register struct binding *bv, *bz;
...
new_message(arg, vb, community, next)
	OID arg;
	struct type_SNMP_VarBindList *vb;
	char *community;
	int next;
{
	...
	pdu->offset = next ? type_SNMP_PDUs_get__next__request : type_SNMP_PDUs_get__request;
	...
	for (vp = &parm->variable__bindings; vb; vb = vb->next) {
		...
		*vp = bind, vp = &bind->next;
		...
		bcopy((char *) vb->VarBind->name->oid_elements,
		      ...
		      vb->VarBind->name->oid_nelem * sizeof *v->name->oid_elements);
...
static int
new_thread(ps, vb, community, start, stop)
	PS ps;
	struct type_SNMP_VarBindList *vb;
	char *community;
	OID start, stop;
	...
	char *community;
{
	...
	for (timelap = 0; RHead->r_forw != RHead; timelap++) {
		...
		for (r = RHead->r_forw; r != RHead; r = u) {
			register struct bound *br, *bz, **bb;
			register struct type_SNMP_VarBindList *vr, **vv;
			...
			evalreq = 0;
			while (br = *bb) {
				register struct binding *bv, *bv2;
				struct OIDentifier oids;

				if ((vr = *vv) == NULL) {
					advise(NULLCP, "missing variable in response, continuing");
					dedrequests++;
			...
			if (timelap >= SETTLETIME && evalreq < 0 && boundlimit != MINBOUNDS)
				boundlimit--;
		}

		if (push_requests(ps, community, currequests < threadlimit && !backoff) == NOTOK)
			goto losing;
	}
	...
	advise(NULLCP,
	       ...
	       rows, rows != 1 ? "s" : "", now.tv_sec, now.tv_usec, timelap, timelap2);
	...
	if (timelap2)
		advise(NULLCP,
		       "threads:  at most %d active, total of %d created", maxthreads, tothreads);
	advise(NULLCP, "messages: %d request%s sent,  along with %d retr%s",
	       totreqs, totreqs != 1 ? "s" : "", totretr, totretr != 1 ? "ies" : "y");
	advise(NULLCP, "          %d response%s rcvd, along with %d duplicate%s",
	       totrsps, totrsps != 1 ? "s" : "", totdups, totdups != 1 ? "s" : "");
...
static int
bulk2_aux(ps, sd, bl, vb, community)
	PS ps;
	int sd;
	struct binding *bl;
	register struct type_SNMP_VarBindList *vb;
	char *community;
{
	...
	for (; THead->t_forw != THead; timelap2++) {
		...
		for (t = THead->t_forw; t != THead; t = u) {
			...
			for (vp = t->t_msg->data->un.get__response->variable__bindings;
			     vp; vp = vp->next) {
				if ((bv = (struct binding *) calloc(1, sizeof *bv)) == NULL)
					adios(NULLCP, "out of memory");
				*bz = bv, bz = &bv->b_next;
	...
	char *community;
{
	...
	r->r_info = -1;
	r->r_msg = new_message(NULLOID, (struct type_SNMP_VarBindList *) NULL, community, 1);
...
static int
new_bound(community, start, stop)
	char *community;
	OID start, stop;
{
	register struct request *r;
...
{
	if (debug && b->r_lo) {
		fprintf(stderr, "%d get-nexts on bound: %s to", b->b_gns, oid2ode(b->r_lo));
		fprintf(stderr, " %s\n", oid2ode(b->r_hi));
	}
...
static
push_requests(ps, community, onemore)
	PS ps;
	char *community;
	int onemore;
{
	register int nbound, nrequest, tbound;
	int new, wen;
	register struct request *r;
	struct request *u;
	register struct bound *bn, *bz, **bp;
	struct bound *b;
	register struct type_SNMP_VarBindList *vz, **vp;
	...
	if ((new = (nbound + (boundlimit - 1)) / boundlimit - nrequest) <= 0 && onemore)
		new = 1;
	for (; new-- > 0; nrequest++)
	...
	/* should never get here, since we should run out bounds before running out of requests; if 
	   we do ... ?? */
	...
	OID a, b;
{
	...
	for (i = 1, ap = a->oid_elements, bp = b->oid_elements; i <= b->oid_nelem; i++, ap++, bp++) {
		if (i > a->oid_nelem) {
			for (; *bp == NULL; bp++)
				if (++i > b->oid_nelem)
					goto losing;
		...
		c = oid_copy(a);
		if ((c->oid_elements[(c->oid_nelem = i) - 1] = *ap + ((*bp - *ap) >> 1))
		    == *ap) {
	...
	if (c == NULL) {
		...
		if (debug) {
			(void) strcpy(buffer, sprintoid(a));
			fprintf(stderr, "oid_median(%s, %s) fails", buffer, sprintoid(b));
		}
		return NULL;
	}
	if (oid_cmp(a, c) >= 0) {
		...
		adios(NULLCP, "oid_median(%s, %s) -> %s loses(1)", buf1, buf2, sprintoid(c));
	}
	if (oid_cmp(c, b) >= 0) {
		char buf1[BUFSIZ], buf2[BUFSIZ];
		...
		(void) strcpy(buf1, sprintoid(a));
		(void) strcpy(buf2, sprintoid(b));
		adios(NULLCP, "oid_median(%s, %s) -> %s loses(2)", buf1, buf2, sprintoid(c));
	}

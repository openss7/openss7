...
"snmp", NULLOID, "view", NULLOID, "smux", NULLOID, NULL};
#endif
	...
	char **argv, **envp;
{
...
#else
				advise(LLOG_EXCEPTIONS, NULLCP, "UDP support not configured");
				failed++;
				continue;
#endif
	...
	if (!listening)
		adios(NULLCP, failed ? "no successful listens" : "no network services selected");

#ifndef	SNMPT
	do_trap(int_SNMP_generic__trap_coldStart, 0, (struct type_SNMP_VarBindList *) 0);
#endif
	...
	for (;;) {
...
#ifdef	SMUX
		if (smux != NOTOK && FD_ISSET(smux, &rfds)
		    && (fd = start_smux()) != NOTOK) {
#ifdef sgi
		...
		case AGENT_COLDSTART:
			...
			if (secs) {
				advise(LLOG_EXCEPTIONS, NULLCP,
				       "waiting %d seconds for connections to clear", secs);
		...
		default:
			advise(LLOG_NOTICE, NULLCP, "agentAction reset from %d",
			       *((integer *) * agentAction));
	...
	int pd;
{
	...
	(void) sprintf(source, "Internet=%s+%d+2",
		       cp = inet_ntoa(isock->sin_addr), (int) ntohs(isock->sin_port));
	...
	int pd;
{
	...
	doit_aux(fd, ta->ta_addrs, read_clts_socket, write_clts_socket, check_clts_socket);
#ifndef	SNMPT
	if (pqr)
	...
	IFP rfx, wfx, cfx;
{
	...
	pqr = NULL;
#endif
	if ((ps = ps_alloc(dg_open)) == NULLPS || dg_setup(ps, fd, MAXSNMP, rfx, wfx, cfx) == NOTOK) {
		if (ps == NULLPS)
			advise(LLOG_EXCEPTIONS, NULLCP, "ps_alloc: out of memory (%s)", source);
		else
...
#endif
		advise(LLOG_EXCEPTIONS, NULLCP, "ps2pe: %s (%s)", ps_error(ps->ps_errno), source);
#ifndef	SNMPT
		snmpstat.s_inpkts++;
	...
	if (decode_SNMP_Message(pe, 1, NULLIP, NULLVP, &msg) == NOTOK) {
		advise(LLOG_EXCEPTIONS, NULLCP, "decode_SNMP_Message: %s (%s)", PY_pepy, source);
#ifndef	SNMPT
		snmpstat.s_asnparseerrs++;
#endif
#ifdef	COTS
		if (rfx == ts_read) {
			...
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "clearing connection from %d: %s", fd, taddr2str(taddrs + fd));
	...
	int size;
{
	...
	if (msg->version != int_SNMP_version_version__1) {
		advise(LLOG_EXCEPTIONS, NULLCP, "badVersion: %d (%s)", msg->version, source);
	...
	if ((comm = str2comm(commname, na)) == NULL) {
		advise(LLOG_EXCEPTIONS, NULLCP, "badCommunity: %s (%s)", commname, source);
		snmpstat.s_badcommunitynames++;
		if (*((integer *) snmpstat.s_enableauthentraps) == TRAPS_ENABLED)
	...
	if (encode_SNMP_Message(&pe, 1, 0, NULLCP, msg) != NOTOK) {
		...
		advise(LLOG_EXCEPTIONS, NULLCP, "encode_SNMP_Message: %s (%s)", PY_pepy, source);
	...
	int size;
{
	...
	case type_SNMP_PDUs_get__request:
		...
		if (vu == NULL || !(comm->c_permission & OT_RDONLY)) {
		      access_denied:;
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "authorizationFailure: %d (%s)", pdu->offset, source);
/* no trap for this... */
			offset = pdu->offset;
	...
	default:
		if (vu == NULL)
			goto access_denied;
	      bad_operation:;
		advise(LLOG_EXCEPTIONS, NULLCP, "badOperation: %d (%s)", pdu->offset, source);
		return NOTOK;
	}
...

	{
		...
		opp = &op;
		for (vp = msg->data->un.get__request->variable__bindings; vp; vp = vp->next) {
	...
	default:
	      losing:;
		if (op) {
			free_SNMP_VarBindList(msg->data->un.get__request->variable__bindings);
	...
	struct view *vu;
{
	...
	for (vp = msg->data->un.get__request->variable__bindings; vp; vp = vp->next) {
		...
		if (offset == type_SNMP_PDUs_get__next__request) {
			...
			if ((ot = oi->oi_type)->ot_getfnx == NULLIFP && ot->ot_smux == NULL)
				goto get_next;
		} else {
			if ((oi = name2inst(v->name)) == NULLOI)
				goto no_name;
			...
			if ((offset == type_SNMP_PDUs_get__request
			     ? ot->ot_getfnx : ot->ot_setfnx) == NULLIFP && ot->ot_smux == NULL) {
			      no_name:;
				parm->error__status = int_SNMP_error__status_noSuchName;
				return idx;
			}
...
#ifdef	SMUX
		if (ot->ot_smux)
			status = smux_method(pdu, ot, ((struct smuxTree *) ot->ot_smux)
					     ->tb_peer, v, offset);
		else
#endif
...
#ifdef SMUX
			/* If current object is being handled by a SMUX sub-agent, then step over
			   the entire subtree before proceeding with the 'get-next' search.  This
			   is needed to avoid descending into the portion of the object tree that
			   the SMUX sub-agent has 'mounted over'. (EJP) */
			if (ot->ot_smux) {
	...
	struct community *comm;
{
	...
	if (qb_pullup(msg->community) == NOTOK) {
		advise(LLOG_EXCEPTIONS, NULLCP, "qb_pullup: out of memory (proxy %s)", source);
		return NOTOK;
	}
	if (pqs >= NPQ) {
	...
	pq->pq_ps = psp;
	pq->pq_community.qb_forw = pq->pq_community.qb_back = &pq->pq_community;
	...
	if ((ps = ps_alloc(dg_open)) == NULLPS
		...
		if (ps == NULLPS)
			...
			advise(LLOG_EXCEPTIONS, NULLCP, "dg_setup: %s (proxy %s)",
			       ps_error(ps->ps_errno), source);
	} else if (hack_dgram_socket(udp, &v->v_sa) == NOTOK)
		advise(LLOG_EXCEPTIONS, "failed", "hack_dgram_socket(1) (proxy %s)", source);
	else if (pe2ps(ps, pe) == NOTOK)
...
{
	integer request;
	register struct proxyque *pq, *qp;
	PE pe;
...
start_smux()
{
	...
	/* Format sockets consistantly with other places in this program, with a plus sign between
	   the internet address and port number.  (EJP) */
	(void) sprintf(pb->pb_source, "%s+%d",
		       inet_ntoa(pb->pb_address.sin_addr), (int) ntohs(pb->pb_address.sin_port));
	(void) strcpy(source, pb->pb_source);

	if ((pb->pb_ps = ps_alloc(fdx_open)) == NULLPS || fdx_setup(pb->pb_ps, fd) == NOTOK) {
		if (pb->pb_ps == NULLPS)
			advise(LLOG_EXCEPTIONS, NULLCP,
	...
	/* Insert new smuxPeer structure at the end of the doubly-linked list anchored at PHead and 
	   assign it the next sequential index number for use as smuxPindex in o_smuxPeer() and
	   smuxTindex in o_smuxTree().  (EJP) */
	...
	struct type_SNMP_SMUX__PDUs *pdu;
{
	...
	case type_SNMP_SMUX__PDUs_simple:
		if (pb->pb_identity)
			goto unexpected;
		{
			register struct type_SNMP_SimpleOpen *simple = pdu->un.simple;
			register struct smuxEntry *se;

			if (simple->version != int_SNMP_version_version__1) {
				advise(LLOG_EXCEPTIONS, NULLCP,
				       "badVersion: %d (SMUX %s)", simple->version, source);
				return NOTOK;
			}
			...
			if ((se = getsmuxEntrybyidentity(pb->pb_identity)) == NULL
			    || strcmp(se->se_password, simple->password->qb_forw->qb_data)) {
				advise(LLOG_EXCEPTIONS, NULLCP,
				       "%s: %s (SMUX %s)",
				       se ? "badPassword" : "badIdentity",
				       oid2ode(simple->identity), source);
				if (*((integer *) snmpstat.s_enableauthentraps)
				    == TRAPS_ENABLED)
	...
	case type_SNMP_SMUX__PDUs_close:
		if (!pb->pb_identity)
			goto unexpected;
		advise(LLOG_NOTICE, NULLCP,
		       "SMUX close: %s (%s)", smux_error(pdu->un.close->parm), source);
		return NOTOK;
	...
	case type_SNMP_SMUX__PDUs_registerRequest:
		if (!pb->pb_identity)
			goto unexpected;
		{
			register struct type_SNMP_RReqPDU *rreq = pdu->un.registerRequest;
				...
				if (sr->rb_name
				    && bcmp((char *) sr->rb_name->oid_elements,
					    ...
					    (sr->rb_name->oid_nelem
					     <= oid->oid_nelem
					     ? sr->rb_name->oid_nelem : oid->oid_nelem)
					...
					advise(LLOG_EXCEPTIONS, NULLCP,
					       ...
					       sr->rb_name->oid_nelem
					       <= oid->oid_nelem
					       ? "under" : "contains", sr->rb_text, source);
					goto no_dice;
				}
			...
			if ((ot = name2obj(oid)) == NULLOT) {
				if (rreq->operation == int_SNMP_operation_delete) {
					advise(LLOG_EXCEPTIONS, NULLCP,
					       "noSuchSubTree: %s (SMUX %s)", oid2ode(oid), source);
					goto no_dice;
				}

				if ((ot = (OT) calloc(1, sizeof *ot)) == NULL
				    || (ot->ot_text = ot->ot_id = strdup(sprintoid(oid)))
				    == NULL) {
					advise(LLOG_EXCEPTIONS, NULLCP,
				...
				if (ot->ot_name->oid_nelem > oid->oid_nelem) {
					advise(LLOG_EXCEPTIONS, NULLCP,
					       "badSubTree: %s (SMUX %s)", oid2ode(oid), source);
					ot = NULL;
					goto no_dice;
				}
			...
			if ((tb = (struct smuxTree *) calloc(1, sizeof *tb))
			    == NULL) {
				advise(LLOG_EXCEPTIONS, NULLCP, "out of memory (SMUX %s)", source);
				return NOTOK;
			}
			...
			rrsp.parm = tb ? tb->tb_priority : int_SNMP_RRspPDU_failure;
			...
			if (encode_SNMP_SMUX__PDUs(&pe, 1, 0, NULLCP, &rsp)
				...
				if (pe2ps(pb->pb_ps, pe) == NOTOK) {
					...
					if (tb && rreq->operation != int_SNMP_operation_delete) {
						register int i;
						...
						/* Insert the new element into the single-linked
						   chain of smuxTree structures for this object
						   type that is anchored in the object tree.
						   Elements are chained in order of increasing
						   priority.  (EJP) */
						...
						*qpp = tb;
						...
						/* Fill in the tb_instance field of the new element
						   with the concatenation of the size of the object
						   name followed by the object name itself followed
						   by its priority.  (EJP) */
						...
						*ip++ = tb->tb_priority;
						...
						/* Insert the new element into the doubly-linked
						   chain of all smuxTree structures that is
						   anchored in THead. Elements are chained in
						   lexicographic order of tb_instance so that the
						   get_tbent() function will work correctly for
						   'get next' operations.  (EJP) */

						for (qb = THead->tb_forw;
						     qb != THead; qb = qb->tb_forw)
				...
				advise(LLOG_EXCEPTIONS, NULLCP,
				       "encode_SNMP_SMUX__PDUs: %s (SMUX %s)", PY_pepy, source);
				result = NOTOK;
			}
			...
			goto unexpected;
		{
			...
			if (loopback_addr
			    ...
			    && bcmp(qb->qb_forw->qb_data,
				    loopback_addr->qb_forw->qb_data, qb->qb_len) == 0)
	...
	case type_SNMP_SMUX__PDUs_set__request:
	      unexpected:;
		advise(LLOG_EXCEPTIONS, NULLCP,
		       "unexpectedOperation: %d (SMUX %s)", pdu->offset, source);
		return NOTOK;

	default:
		advise(LLOG_EXCEPTIONS, NULLCP, "badOperation: %d (SMUX %s)", pdu->offset, source);
		return NOTOK;
	}
...
{
	int status, orig_id;
	struct type_SNMP_VarBindList *orig_bindings, vps;
	struct type_SNMP_SMUX__PDUs req, *rsp;
	...
	if (encode_SNMP_SMUX__PDUs(&pe, 1, 0, NULLCP, &req) != NOTOK) {
		...
		if (pe2ps(pb->pb_ps, pe) == NOTOK) {
			advise(LLOG_EXCEPTIONS, NULLCP, "pe2ps: %s (%s, SMUX %s)",
			       ps_error(pb->pb_ps->ps_errno), source, pb->pb_source);
		...
		advise(LLOG_EXCEPTIONS, NULLCP, "encode_SNMP_SMUX__PDUs: %s (%s)", PY_pepy, source);
		status = int_SNMP_error__status_genErr;
	}
	...
	if (status != int_SNMP_error__status_noError
	    || offset == type_SNMP_PDUs_commit || offset == type_SNMP_PDUs_rollback)
		return status;
	...
	if (decode_SNMP_SMUX__PDUs(pe, 1, NULLIP, NULLVP, &rsp) == NOTOK) {
		advise(LLOG_EXCEPTIONS, NULLCP,
		       "decode_SNMP_SMUX__PDUs: %s (%s, SMUX %s)", PY_pepy, source, pb->pb_source);
	...
	if (rsp->offset != type_SNMP_SMUX__PDUs_get__response) {
		if (rsp->offset == type_SNMP_SMUX__PDUs_trap) {
			...
			free_SNMP_SMUX__PDUs(rsp), rsp = NULL;
			pe_free(pe), pe = NULL;
			goto try_it_again;
		}

		advise(LLOG_EXCEPTIONS, NULLCP,
		       "unexpectedOperation: %d (%s, SMUX %s)", rsp->offset, source, pb->pb_source);
	...
	case int_SNMP_error__status_noError:
	{
		...
		if (offset == type_SNMP_PDUs_get__next__request
			...
			|| bcmp((char *) ot->ot_name->oid_elements,
				...
				ot->ot_name->oid_nelem * sizeof ot->ot_name->oid_elements[0]))) {
			status = NOTOK;
			break;
		}
...
{
	register struct smuxTree *tp, **tpp;

	if (tb == NULL)
		return;

	for (tpp = (struct smuxTree **) &tb->tb_subtree->ot_smux; tp = *tpp; tpp = &tp->tb_next)
		if (tp == tb) {
			*tpp = tb->tb_next;
			break;
		}
...
start_view()
{
	...
	/* Initialize the view mask of all objects in the tree, not just the leaves.  This is
	   necessary so that do_pass() will allow SMUX sub-agents to mount their subtrees over
	   interior objects.  (EJP) */
		...
		if ((s = v->v_subtree.s_forw) != &v->v_subtree) {
			for (; s != &v->v_subtree; s = s->s_forw)
				if (inSubtree(s->s_subtree, name))
					goto mark_it;
		} else if (!viewTree || !inSubtree(viewTree, name)) {
		      mark_it:;
			...
			if (c->c_addr.na_stack == NA_TCP
				...
				case NA_X25:
					if (c->c_addr.na_dtelen != na->na_dtelen
					    || bcmp(c->c_addr.na_dte, na->na_dte, na->na_dtelen))
						continue;
					break;
...
start_trap()
{
	...
	if (hp = gethostbystring(myhost)) {
		...
		advise(LLOG_EXCEPTIONS, NULLCP, "%s: unknown host, so no traps", myhost);
		goto out;
	}
	...
	struct type_SNMP_VarBindList *bindings;
{
	...
	if ((ot = text2obj("sysObjectID")) == NULLOT) {
		advise(LLOG_EXCEPTIONS, NULLCP,
		       "unable to send trap: no such object: \"%s\"", "sysObjectID");
		return;
	}
	if ((parm->enterprise = (OID) ot->ot_info) == NULLOID) {
		advise(LLOG_EXCEPTIONS, NULLCP,
		       "unable to send trap: no value defined for object \"%s\"", "sysObjectID");
		return;
	}
	...
	struct NSAPaddr *na;
{
	...
	if ((au->source = str2qb(source, strlen(source), 1)) == NULL) {
	      no_mem:;
		advise(LLOG_EXCEPTIONS, NULLCP, "out of memory for audit (%s)", source);
		goto out;
	}
	(void) time(&now);
	...
	if ((cp = gent2str(ut)) == NULL || (au->dateAndTime = str2qb(cp, strlen(cp), 1)) == NULL)
		goto no_mem;
	...
	if (encode_SNMP_Audit(&pe, 1, 0, NULLCP, au) != NOTOK) {
		...
		advise(LLOG_EXCEPTIONS, NULLCP, "encode_SNMP_Audit: %s (%s)", PY_pepy, source);
	...
	char **vec;
{
...
#else
	bcopy("snmp-trap", tz->ta_selector, tz->ta_selectlen = sizeof "snmp-trap" - 1);
#endif
	tz->ta_naddr = 0;
	...
	for (vec++; ap = *vec; vec++) {
		if (*ap == '-')
			switch (*++ap) {
			case 'b':
				if ((ap = *++vec) == NULL || *ap == '-' || (tooBig = atoi(ap)) <= 128	/* MAXSNMP 
													 */ )
...
#ifdef	TCP
			case 'p':
				if ((ap = *++vec) == NULL || *ap == '-' || (port = atoi(ap)) <= 0)
			...
			case 'a':
				...
				/* This permits listening on a specific protocol id. In fact,
				   SunLink X.25 lets you listen on a protocol id mask, but let's
				   keep it simple. */
			case 'i':
				if ((ap = *++vec) == NULL || *ap == '-')
					adios(NULLCP, "usage: %s -i pid", myname);
				x25_na->na_pidlen = str2sel(ap, -1, x25_na->na_pid, NPSIZE);
				continue;
#endif
	...
	int sig;
{
	...
	advise(LLOG_EXCEPTIONS, NULLCP,
	       "sbrk=0x%x allocs=%d frees=%d most=%d", sbrk(0), pe_allocs, pe_frees, pe_most);

	for (p = pe_active; p; p = p->pe_link) {
		(void) sprintf(buffer, "active PE 0x%x (refcnt %d)", (caddr_t) p, p->pe_refcnt);
...
int f_community(), f_logging(), f_proxy(), f_trap(), f_variable(), f_view(), f_expression();
...
"community", f_community,
	    ...
	    "proxy", f_proxy,
	    "trap", f_trap,
	    "variable", f_variable, "view", f_view, "expression", f_expression, NULL};
...
readconfig()
{
	...
	cp = "snmpc.rc";
#endif
	if ((fp = fopen(cp, "r")) == NULL && (fp = fopen(cp = isodefile(cp, 0), "r")) == NULL)
		adios(cp, "unable to read");

	if (!rflag && getuid() == 0 && fstat(fileno(fp), &st) != NOTOK && st.st_uid != 0)
	...
	while (fgets(buffer, sizeof buffer, fp)) {
		...
		if (!p->p_name)
			advise(LLOG_EXCEPTIONS, NULLCP, "unknown directive: \"%s\"", line);
	}

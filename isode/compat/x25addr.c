...
/* ARGSUSED */
CONN_DB *
gen2if(generic, specific, context)
	...
	int context;
{
...
#ifdef AEF_NSAP
		/* for suns, with sunnet 7.0 or >, if this a real NSAP, we have a NULL X121 and let 
		   the routing tables do the business */
	...
	if (x25_dnic_prefix[0] && *x25_dnic_prefix[0]) {
		...
		if (strncmp(generic->na_dte, x25_dnic_prefix[0],
			...
			if (x25_strip_dnic)
				bcopy(generic->na_dte + i, dte, dtelen = generic->na_dtelen - i);
			else
				bcopy(generic->na_dte, dte, dtelen = generic->na_dtelen);
		} else {
			char **cp;
			char found = 0;
	...
	if ((context == ADDR_LISTEN) && x25_local_dte && *x25_local_dte) {
		...
		if (strncmp(generic->na_dte, x25_local_dte, i = strlen(x25_local_dte)) == 0) {
			bcopy(generic->na_dte + i, dte, dtelen = generic->na_dtelen - i);
			dte[dtelen] = NULL;
		}
	}
	...
	DLOG(x25_log, LLOG_DEBUG,
	     ("gen2if %s -> %s, %d octets; PID %s",
	      generic->na_dte, dte, dtelen, sel2str(generic->na_pid, (int) generic->na_pidlen, 1)));
	...
	if ((specific->xaddr_len = dtelen) != 0) {
		bcopy(dte, specific->xaddr_addr, dtelen);
		specific->xaddr_len = dtelen;
		specific->xaddr_facilities = 0;
		bcopy(generic->na_pid, specific->xaddr_proto, generic->na_pidlen);
		bcopy(generic->na_cudf, specific->xaddr_userdata, generic->na_cudflen);
	}
#endif
	...
	if (specific->cud.len = min((unsigned) generic->na_cudflen, sizeof(specific->cud.buf)))
		bcopy(generic->na_cudf, specific->cud.buf, specific->cud.len);
	else if (specific->cud.len = min((unsigned) generic->na_pidlen, sizeof(specific->cud.buf)))
		bcopy(generic->na_pid, specific->cud.buf, specific->cud.len);
#endif				/* CCUR_X25 */

#ifdef HPUX_X25
	if ((dtelen != 1) || (dte[0] != '0'))
		bcopy(dte, specific->addr.x25_host, specific->addr.x25hostlen = dtelen);
	...
	if (generic->na_pidlen) {	/* non-null PID */
		...
		if (generic->na_cudflen) {
			if (generic->na_cudflen + specific->cudf.x25_cud_len > X25_MAX_CU_LEN) {
				SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP,
		...
		bcopy(generic->na_cudf, specific->addr.x25pid,
		      ...
		      (generic->na_cudflen <= NPSIZE) ? generic->na_cudflen : NPSIZE);
	...
	if (generic->na_pidlen) {	/* non-null PID */
		if (generic->na_pidlen > NPSIZE) {
			...
			bcopy(generic->na_pid, (char *) specific->data, generic->na_pidlen);
			bcopy(generic->na_cudf, (char *) specific->data + NPSIZE,
			      generic->na_cudflen);
			specific->datalen = generic->na_pidlen + generic->na_cudflen;
		}
	} else {		/* Null PID (just copy in CUDF, the first four octets of which will 
				   be the PID in any case) */
		bcopy(generic->na_cudf, (char *) specific->data, generic->na_cudflen);
	...
	case ADDR_LISTEN:
		iov = &(specific->ccl_iovec[0]);
		if (generic->na_pidlen) {	/* listen on a PID */
			register int i;

			iov->iov_base[0] = 'C';
			bcopy(generic->na_pid, iov->iov_base + 1, i = generic->na_pidlen);
			iov->iov_len = i + 1;
		} else if (generic->na_dtelen < 6) {	/* listen on a subaddress */
			register int i;

			iov->iov_base[0] = 'S';
			bcopy(generic->na_dte, iov->iov_base + 1, i = generic->na_dtelen);
			iov->iov_len = i + 1;
		} else		/* full DTE */
			bcopy(dte, iov->iov_base, iov->iov_len = dtelen);
		return (specific);
	}
	/* 
	...
	if (generic->na_dtelen != 0) {
		...
		if (generic->na_pidlen > 0) {
			bcopy(generic->na_pid,
			      specific->x25_cudf, specific->x25_cudflen = generic->na_pidlen);
		}
		if (generic->na_cudflen > 0) {
			bcopy(generic->na_cudf,
			      specific->x25_cudf + generic->na_pidlen, generic->na_cudflen);
			specific->x25_cudflen += generic->na_cudflen;
		}
	}
#else
	if (generic->na_dtelen != 0) {
		specific->na_dtelen = specific->na_pidlen = specific->na_cudflen = 0;
		bcopy(generic->na_dte, specific->na_dte, specific->na_dtelen = generic->na_dtelen);
		/* 
		 * concatenate PID and CUDF into CUDF buffer.
		 */
		if (generic->na_pidlen > 0) {
			bcopy(generic->na_pid,
			      specific->na_cudf, specific->na_cudflen = generic->na_pidlen);
		}
		if (generic->na_cudflen > 0) {
			bcopy(generic->na_cudf,
			      specific->na_cudf + specific->na_pidlen, generic->na_cudflen);
	...
	if (dtelen > 0) {	/* Called X.25 Addr. */
		...
		if (generic->na_pidlen > 0) {	/* Protocol Id */
			...
			bcopy(generic->na_pid, specific->user_data, generic->na_pidlen);
			specific->flags |= X25FLG_USER_DATA;
		}
		if (generic->na_cudflen > 0) {	/* Call User Data */
			specific->user_data_len += generic->na_cudflen;
			bcopy(generic->na_cudf,
			      specific->user_data + generic->na_pidlen, generic->na_cudflen);
			specific->flags |= X25FLG_USER_DATA;
		}
	}
#endif
...
/* ARGSUSED */
struct NSAPaddr *
if2gen(generic, specific, context)
	struct NSAPaddr *generic;
	CONN_DB *specific;
	int context;
{
	...
	if (specific->xaddr_len != 0) {
		...
		bcopy(specific->xaddr_proto, generic->na_pid, sizeof(specific->xaddr_proto));
		generic->na_pidlen = sizeof specific->xaddr_proto;
		bcopy(specific->xaddr_userdata, generic->na_cudf, sizeof(specific->xaddr_userdata));
	...
	if (specific->cud.len > generic->na_pidlen) {
		generic->na_cudflen = min(specific->cud.len - generic->na_pidlen, CUDFSIZE);
	...
	if (specific->datalen > NPSIZE) {	/* have some real user data after the PID */
		bcopy((char *) specific->data, generic->na_pid, generic->na_pidlen = NPSIZE);
	...
	if (specific->addr.x25hostlen)
		bcopy(specific->addr.x25_host, dte, dtelen = specific->addr.x25hostlen);
	else {
		dte[0] = '0';
		dte[1] = NULL;
		dtelen = 1;
	...
	case ADDR_REMOTE:
		...
		if (iov->iov_len) {
			if (*(iov->iov_base) == '#') {
				...
				bcopy((iov->iov_base) + 1, dte, dtelen);
			}
		} else
			dtelen = 0;
		break;
	...
	if (specific->flags & X25FLG_CALLING_ADDR) {
		...
		if ((specific->flags & X25FLG_USER_DATA) && (specific->user_data_len > 0)) {	/* Protocol 
												   Id 
												 */
		...
		if ((specific->flags & X25FLG_USER_DATA) && (specific->user_data_len > 4)) {
			/* Call User Data */
			generic->na_cudflen = specific->user_data_len - 4;
			bcopy(specific->user_data + 4, generic->na_cudf, generic->na_cudflen);
		}
	}
#endif
	...
	if (x25_dnic_prefix[0] && *x25_dnic_prefix[0]) {
		...
		if (x25_intl_zero && dte[0] == '0' && dte[1] != '0')
			i = 1;
			 ...
			 && *x25_dnic_prefix[0]
			 && x25_strip_dnic && dtelen < 12)	/* local call... */
	...
	unsigned char *pkt;
{
	...
	if (flags & (1 << RECV_DIAG)) {
		...
		if (flags & (1 << DIAG_TYPE))	/* cleared */
			SLOG(x25_log, LLOG_EXCEPTIONS, NULLCP, ("cleared 0x%02x", pkt[0]));
		else if (flags & (1 << REST_TYPE))
			SLOG(x25_log, LLOG_EXCEPTIONS, NULLCP, ("restart 0x%02x", pkt[0]));
		else
			SLOG(x25_log, LLOG_EXCEPTIONS, NULLCP, ("reset 0x%02x", pkt[0]));

#else
		SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP,
		     ((flags & (1 << DIAG_TYPE)) ? "cleared 0x%02x" : "reset 0x%02x", pkt[0]));
#endif
		...
		/* In any case, if someone is keen, they can stuff it all into a text file and read 
		   it out */
		...
		default:
			SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP, ("diagnostic: 0x%2x", pkt[1]));
			goto done;
		}
		...
		if (flags & (1 << DIAG_TYPE))	/* cleared */
			SLOG(x25_log, LLOG_EXCEPTIONS, NULLCP,
			     ("clearing source: %s (%d)",
			      pkt[2] ? "X25-Level 2" : "network prov./firmware", pkt[2]));
		else if (flags & (1 << REST_TYPE))
			SLOG(x25_log, LLOG_EXCEPTIONS, NULLCP,
...
#else
	if (flags)
		SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP, ("diag flags: 0x%02x", flags));
#endif

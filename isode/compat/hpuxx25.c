...
#ifndef lint
static char *rcsid =
    "Header: /xtel/isode/isode/compat/RCS/hpuxx25.c,v 9.0 1992/06/16 12:07:00 isode Rel";
#endif
	...
	int backlog, opt1, opt2;
{
	...
	if (!(local->na_dtelen || local->na_pidlen)) {
		...
		if (x25_dnic_prefix[0] && *x25_dnic_prefix[0])
			strcpy(local->na_dte, x25_dnic_prefix[0]);
	...
	if (ioctl(sd, X25_CALL_ACPT_APPROVAL, (char *) &onoff) == NOTOK) {
		SLOG(x25_log, LLOG_EXCEPTIONS, "failed", ("X25_CALL_ACPT_APPROVAL"));
		(void) close_x25_socket(sd);
		return NOTOK;
	}
...
int
set_x25_facilities(sd, coc, caption)
	int sd, coc;
	char *caption;
{
	...
	if (coc == PROBE || !(coc == CALLED || reverse_charge || recvpktsize || sendpktsize || recvwndsize || sendwndsize || recvthruput || sendthruput || cug_req	/* || 
																					   cug_index 
																					 */
	...
	case 2:
		...
		break;
	default:
		SLOG(x25_log, LLOG_EXCEPTIONS, "illegal value", ("fast_select_type"));
	}
	...
	for (;;) {
		if ((buflen = recv(fd, buf, MAX_EVENT_SIZE, MSG_OOB)) == NOTOK) {
			if (x25_log->ll_events & LLOG_NOTICE)
				SLOG(x25_log, LLOG_NOTICE, "failed", ("recv %d (MSG_OOB)", fd));
			clear_sigurg(fd);
			return OK;
		} else if (!buflen)
			return OK;
	...
	int fd;
{
	...
	if ((fdl != NULL) && (fdl->fd == fd)) {
		fdl = fdl->next;
		if (free(fdlp) == NOTOK)
			SLOG(x25_log, LLOG_EXCEPTIONS, "failed", ("free (sigurg-struct)"));
		return;
	} else
		while (fdlp != NULL)
...
static int
log_x25_facilities(fd, coc, caption)
	int fd;
	int coc;
	char *caption;
{
	...
	char *caption;
{
	...
	DLOG(x25_log, LLOG_DEBUG,
	     (hp->t_43[0] == 0 ? "default recv window size"
	      ...
	      ? "recv window size %d" : "invalid recv window size %d", hp->t_43[0]));

	DLOG(x25_log, LLOG_DEBUG,
	     (hp->t_43[1] == 0 ? "default send window size"
	      : 1 <= hp->t_43[1] && hp->t_43[1] <= 127
	      ? "send window size %d" : "invalid send window size %d", hp->t_43[1]));
	...
	default:
		DLOG(x25_log, LLOG_DEBUG, ("invalid recv throughput %d", hp->t_02 >> 4));
		break;
	}
	...
	default:
		DLOG(x25_log, LLOG_DEBUG, ("invalid send throughput %d", hp->t_02 & 0x0f));
		break;
	}

	if (hp->t_03_sel)
		DLOG(x25_log, LLOG_DEBUG, ("closed user group 0x%x (BCD)", hp->t_03));
	else
	...
	case CCITT_FAST_ACPT_CLR:
		DLOG(x25_log, LLOG_DEBUG, ("clear or call accepted is fast select response"));
		break;

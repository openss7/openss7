...
#ifdef X25
#define LOCALDNIC_MAX	10	/* Max number of different DNIC's for 'local' sites (i.e., leading
				   zero not req'd) */
#endif
...
char *x25_dnic_prefix[LOCALDNIC_MAX + 1];
		...
		/* 0 = don't request/allow reverse charging 1 = reverse charge request/allowed */
static char *reverse_charge_default = "0";
unsigned char reverse_charge = 0;

		/* 0= default recv packet size. 16, 32, 64, 128, 256, 512, 1024 ( octets in decimal 
		   ) */
static char *recvpktsize_default = "0";
unsigned short recvpktsize = 0;

		/* same as above, but for send packet size */
static char *sendpktsize_default = "0";
unsigned short sendpktsize = 0;

		/* 0= default recv window size. 7, 127 ( in decimal ) */
static char *recvwndsize_default = "0";
unsigned char recvwndsize = 0;

		/* same as above, but for send window size */
static char *sendwndsize_default = "0";
unsigned char sendwndsize = 0;

		/* 0= default recv throughtput. 3= 75 8= 2400 4= 150 9= 4800 5= 300 10= 9600 6= 600 
		   11= 19200 7= 1200 12= 48000 ( bps in decimal ) */
static char *recvthruput_default = "0";
unsigned char recvthruput = 0;

		/* same as above, but for send throughput */
		/* 1 = use closed user group in cug_index */
static char *sendthruput_default = "0";
unsigned char sendthruput = 0;

		/* 0 = no closed user group. */
		/* 1 = use closed user group in cug_index */
static char *cug_req_default = "0";
unsigned char cug_req = 0;

		/* valid when cug_req= 1. 0x00 ~ 0x99 ( closed user group in BCD) */
static char *cug_index_default = "0";
unsigned char cug_index = 0;

		/* 0= don't use fast select 1= clear is fast select response 2= clear or call
		   accepted is fast select response */
static char *fast_select_type_default = "0";
unsigned char fast_select_type = 0;

		/* 0= no RPOA transit request 1= use RPOA transit request in rpoa */
static char *rpoa_req_default = "0";
unsigned char rpoa_req = 0;

		/* valid when rpoa_req= 1 */
		/* 0x0000 ~ 0x9999 (RPOA transit group in BCD) */
static char *rpoa_default = "0";
unsigned short rpoa = 0;
...
int
isodesetvar(name, value, dynamic)
	char *name, *value;
	int dynamic;
{
...
int macro2comm(char *name, register struct ts_interim *ts);
	...
	char *myname;
{
	...
	{
		register int *ip, *jp, *kp;
		int j;
		register char *cp, *adrp, **ap, **cpp;
		register struct ts_interim *ts;
		static struct ts_interim *te = NULL;
		char buffer[BUFSIZ], *vec[NVEC + NSLACK + 1];
		...
		while ((cp = *ap++)) {
			...
			for (tp = ts_interim; tp < ts; tp++)
				if (tp->ts_length == ts->ts_length
				    && bcmp(tp->ts_prefix, ts->ts_prefix, tp->ts_length) == 0) {
					(void) fprintf(stderr,
		...
		*ip = '\0', *cpp = NULLCP;
		...
		if (addr_log->ll_events & LLOG_DEBUG) {
			LLOG(addr_log, LLOG_DEBUG, ("ts_stacks: %s", sprintb(ts_stacks, TS_MASK)));
			...
			for (ts = ts_interim; ts->ts_name; ts++) {
				...
				buffer[explode(buffer, (unsigned char *) ts->ts_prefix,
					       ts->ts_length)] = '\0';
				LLOG(addr_log, LLOG_DEBUG,
				     ("    prefix \"%s\" (%d octets)", buffer, ts->ts_length));
			}
			...
			LLOG(addr_log, LLOG_DEBUG,
			     ("default communities: nsap=%d x25=%d tcp=%d",
			      ts_comm_nsap_default, ts_comm_x25_default, ts_comm_tcp_default));
...
#ifdef	X25
	reverse_charge = (unsigned char) atoi(reverse_charge_default);
	recvpktsize = (unsigned short) atoi(recvpktsize_default);
	sendpktsize = (unsigned short) atoi(sendpktsize_default);
	recvwndsize = (unsigned char) atoi(recvwndsize_default);
	sendwndsize = (unsigned char) atoi(sendwndsize_default);
	recvthruput = (unsigned char) atoi(recvthruput_default);
	sendthruput = (unsigned char) atoi(sendthruput_default);
	cug_req = (unsigned char) atoi(cug_req_default);
	cug_index = (unsigned char) atoi(cug_index_default);
	fast_select_type = (unsigned char) atoi(fast_select_type_default);
	rpoa_req = atoi(rpoa_req_default);
	rpoa = (unsigned short) atoi(rpoa_default);
	...
	nsap_default_stack = events_value(nsap_pairs, _nsap_default_stack, "nsap_default_stack");

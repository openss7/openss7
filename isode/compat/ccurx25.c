...
#define N_CHUNK_SIZE 1024	/* Cannot write single chunks greater than this and must write a
				   single NSDU in multiples of this, or of the NPDU size (which is
				   always less than or equal). This could be determined by doing an
				   N_INFO_REQ (the NIDU_size field), or the NPDU size could be
				   extracted from the facilities on a connection indication/confirm, 
				   but where would we store such information? We hardly want to do
				   an N_INFO_REQ before every write! */
	...
	int n;
{
	...
	case NBADDESTOFF:
	case NBADRESLENGTH:
		return "bad address size/offset";
	case NNOLINE:
		return "line not up";
	case NBOUND:
		return "address in use";
	case NBADCONINDNUM:
		return "listen concurrency too large";
	case NBADEXPWIN:
		return "expedited window closed";
	case NSYSERR:
		return "";
	case NNOTSUPPORT:
...

int
get_prim(fd, control, data, flags, expected, size, close_fd)
	int fd;
	...
	int expected, size, close_fd;
{
	...
	if (control->len < (int) sizeof(err->PRIM_type)) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "insufficient control data", ("getmsg"));
		if (close_fd != NOTOK)
			(void) close(close_fd);
		return NOTOK;
	}
	...
	if (err->PRIM_type == expected && control->len >= size)
		return OK;
	if (err->PRIM_type == N_ERROR_ACK && control->len >= (int) sizeof(N_error_ack_t)) {
		errno = err->UNIX_error;
		SLOG(compat_log, LLOG_EXCEPTIONS,
		     npierr2str(err->NPI_error), ("getmsg (expected %s)", npiev2str(expected)));
		if (close_fd != NOTOK)
			(void) close(close_fd);
		return NOTOK;
	}
	...
	struct NSAPaddr *local;
{
	...
	/* should use local address to see if sub-address on local bind but cannot bind now as need 
	   to know remote address to determin which line to use. */
	...
	int backlog, opt1, opt2;
{
	...
	control.buf = pbuf;
	if (get_prim(sd, &control, 0, &flags, N_BIND_ACK, (int) sizeof(N_bind_ack_t), sd) == NOTOK)
		return NOTOK;
...

int
join_x25_client(fd, remote, local)
	int fd;
	struct NSAPaddr *remote, *local;
{
	...
	control.maxlen = sizeof(pbuf);
	control.buf = pbuf;
	if (get_prim(fd, &control, 0, &flags, N_OK_ACK, (int) sizeof(N_ok_ack_t), nfd) == NOTOK)
		return NOTOK;
	return nfd;
}

	...
	struct NSAPaddr *remote;
{
	...
	if ((res = getmsg(fd, &control, 0, (flags = 0, &flags))) == NOTOK || (res & MORECTL)
	...
	char *buffer;
{
	...
	DLOG(compat_log, LLOG_DEBUG, ("X25 read, total %d/%d", count, data.maxlen));

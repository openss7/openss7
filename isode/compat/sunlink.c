	...
	int backlog, opt1, opt2;
{
	...
	if (ioctl(sd, X25_CALL_ACPT_APPROVAL, (char *) &onoff) == NOTOK) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("X25_CALL_ACPT_APPROVAL"));
		(void) close_x25_socket(sd);
		return NOTOK;
	}
#ifdef AEF_NSAP
	if (local->na_stack == NA_NSAP) {
		...
		/* if null DTE in /etc/x25params then ALWAYS use null DTE for listen if PID is
		   specified */
		if (xs->hostlen == 0 && local->na_pidlen > 0) {
			...
			*sock = *xs;	/* struct copy */
			...
			bcopy(local->na_cudf, (char *) sock->data + NPSIZE, local->na_cudflen);
			sock->datalen = local->na_pidlen + local->na_cudflen;
		} else
			sock = gen2if(local, sock, ADDR_LISTEN);

		/* Adopt the convention that if a null DTE is given, we should get the one from
		   /etc/x25params */
		if (!local->na_dtelen) {
	...
	register struct NSAPaddr *remote;
{
	...
	if ((nfd = connect(fd, (struct sockaddr *) sock, sizeof(CONN_DB)))
		...
		if (compat_log->ll_events & LLOG_EXCEPTIONS)
			(void) log_cause_and_diag(fd);	/* Sun's documentation throwns no light as
							   to whether, or not this will result in
	...
	struct NSAPaddr *remote;
{
	...
	f.type = T_CALLING_AEF;
	if (ioctl(nfd, X25_GET_FACILITY, &f) == NOTOK || if2nsap(&f.f_calling_aef, remote) == NOTOK)
#endif
	...
	char *caption;
{
	...
	if (coc != CALLED && ioctl(sd, X25_RD_FACILITY, (char *) &facilities) == NOTOK) {
	...
	char *caption;
{
	...
	default:
		DLOG(compat_log, LLOG_DEBUG, ("invalid reverse_charge: %d", f.reverse_charge));
		break;
	}
	...
	case 512:
	case 1024:
		DLOG(compat_log, LLOG_DEBUG, ("recv packet size %d", f.recvpktsize));
		break;

	default:
		DLOG(compat_log, LLOG_DEBUG, ("invalid recv packet size %d", f.recvpktsize));
		break;
	}
	...
	case 512:
	case 1024:
		DLOG(compat_log, LLOG_DEBUG, ("send packet size %d", f.sendpktsize));
		break;

	default:
		DLOG(compat_log, LLOG_DEBUG, ("invalid send packet size %d", f.sendpktsize));
		break;
	}
	...
	DLOG(compat_log, LLOG_DEBUG,
	     (f.recvwndsize == 0 ? "default recv window size"
	      ...
	      ? "recv window size %d" : "invalid recv window size %d", f.recvwndsize));

	DLOG(compat_log, LLOG_DEBUG,
	     (f.sendwndsize == 0 ? "default send window size"
	      : 1 <= f.sendwndsize && f.sendwndsize <= 127
	      ? "send window size %d" : "invalid send window size %d", f.sendwndsize));
	...
	default:
		DLOG(compat_log, LLOG_DEBUG, ("invalid recv throughput %d", f.recvthruput));
		break;
	}
	...
	default:
		DLOG(compat_log, LLOG_DEBUG, ("invalid send throughput %d", f.sendthruput));
		break;
	}
	...
	case 1:
		DLOG(compat_log, LLOG_DEBUG, ("closed user group 0x%x (BCD)", f.cug_req));
		break;

	default:
		DLOG(compat_log, LLOG_DEBUG, ("invalid closed user group %d", f.cug_req));
		break;
	}
	...
	case FAST_ACPT_CLR:
		DLOG(compat_log, LLOG_DEBUG, ("clear or call accepted is fast select response"));
		break;

	default:
		DLOG(compat_log, LLOG_DEBUG, ("invalid fast select type %d", f.fast_select_type));
		break;
	}
	...
	case 1:
		DLOG(compat_log, LLOG_DEBUG, ("RPOA transit request 0x%x", f.rpoa_req));
		break;

	default:
		DLOG(compat_log, LLOG_DEBUG, ("invalid RPOA transit request %d", f.rpoa_req));
	}
}
#endif
#else

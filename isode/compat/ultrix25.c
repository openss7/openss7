	...
	struct NSAPaddr *remote;
{
	...
	if ((stat = getsockopt(nfd, X25_ACCESS, XSO_TAKECALL, enc_buf, &enc_buf_len) < 0)) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("Taking inbound X.25 Call"));
		if (compat_log->ll_events & LLOG_EXCEPTIONS)
			(void) log_call_status(fd);	/* decode useful information */
		return NOTOK;
	}
	...
	if ((stat =
	     X25Decode(enc_buf, enc_buf_len, X25I_CALLINGDTE, sck->na_dte,
		       &(sck->na_dtelen))) < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("Getting remote DTE"));
		if (compat_log->ll_events & LLOG_EXCEPTIONS)
			(void) log_call_status(fd);	/* decode useful information */
		return NOTOK;
	}
	...
	if ((stat =
	     X25Decode(enc_buf, enc_buf_len, X25I_USERDATA, sck->na_cudf,
		       &(sck->na_cudflen))) < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("Getting remote CUDF"));
		if (compat_log->ll_events & LLOG_EXCEPTIONS)
			(void) log_call_status(fd);	/* decode useful information */
		return NOTOK;
	}
	...
	if (setsockopt(nfd, X25_ACCESS, XSO_ACCEPTCALL, (caddr_t) 0, 0) < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("Sending Ultrix X.25 Connect Accept"));
		if (compat_log->ll_events & LLOG_EXCEPTIONS)
			(void) log_call_status(fd);	/* decode useful information */
		return NOTOK;
	}
	...
	struct NSAPaddr *remote;
{
	...
	if ((enc_buf_len = X25Encode(enc_buf, 1024,
				     ...
				     X25I_USERDATA,
				     sck->na_cudflen, sck->na_cudf, X25I_NULL)) < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("Encoding Ultrix X.25 Call Template"));
		return NOTOK;
	}
	...
	int fd;
{
	...
	if ((stat = getsockopt(fd, X25_ACCESS, XSO_SHOWSTATUS, stats, &sbl) < 0)) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("x25 getsockopt(XSO_SHOWSTATUS)"));
		return;
	}
	switch (stats->portState) {
	case X25S_OPEN:
		SLOG(compat_log, LLOG_DEBUG, NULLCP, ("X25S_OPEN: No connection is in Progress."));
		log_call_clear(fd, 0);
		break;
	...
	case X25S_CALLING:	/* Connection in progress */
		SLOG(compat_log, LLOG_DEBUG, NULLCP, ("X25S_CALLING: Connection in progress."));
		log_call_clear(fd, 0);
		break;

	case X25S_CALLED:	/* Call received and taken */
		SLOG(compat_log, LLOG_DEBUG, NULLCP, ("X25S_CALLED: Call received and taken."));
		log_call_clear(fd, 0);
		break;

	case X25S_SYNC:	/* Unconfirmed user reset */
		SLOG(compat_log, LLOG_DEBUG, NULLCP, ("X25S_SYNC: Unconfirmed user reset."));
		log_call_clear(fd, 0);
		break;
	...
	case X25S_CLEARING:	/* User requested clearing */
		SLOG(compat_log, LLOG_DEBUG, NULLCP, ("X25S_CLEARING: User requested clearing."));
		log_call_clear(fd, 0);
		break;

	case X25S_NOCOMM:	/* No communication with net */
		SLOG(compat_log, LLOG_DEBUG, NULLCP, ("X25S_NOCOMM: No communication with net."));
		log_call_clear(fd, 0);
		break;

	case X25S_CLEARBYDIR:	/* Cleared by directive */
		SLOG(compat_log, LLOG_DEBUG, NULLCP, ("X25S_CLEARBYDIR: Cleared by directive."));
		log_call_clear(fd, 0);
		break;

	case X25S_INCALL:	/* Untaken incoming call */
		SLOG(compat_log, LLOG_DEBUG, NULLCP, ("X25S_INCALL: Untaken incoming call."));
		log_call_clear(fd, 0);
		break;

	default:
		SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP,
		     ("unknown return from getsockopt(XSO_SHOWSTATUS)= %d [see /usr/include/netx25/x25.h]",
		      stats->portState));
		SLOG(compat_log, LLOG_DEBUG, NULLCP,
		...
		SLOG(compat_log, LLOG_DEBUG, NULLCP, ("Error BitMap = %2x", stats->errorBitMap));
	}
	return;
}
	...
	int type;
{
	 ...
	 */
	if ((stat = getsockopt(fd, X25_ACCESS, XSO_CLEARDATA, cdata, &cbl) < 0) && errno != ENOMSG) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("x25 getsockopt(XSO_CLEARDATA)"));
		return;
	}
	if (errno == ENOMSG)
		return;
	...
	SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP,
	     ("[Clear origin was %s]",
	      ...
	      cdata->clearOrigin == X25R_ORIGINLOCAL ? "local" : "unknown"));
	SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP,
	     ("and %d bytes of info", cdata->clearDetailsLength));

	dlen = sizeof(dbuf);
	if ((stat = X25Decode(cdata->clearDetails,
			      cdata->clearDetailsLength, X25I_CHARGEMON, dbuf, &dlen)) < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("X25Decode(X25I_CHARGEMON)"));
		return;
	}
	if (stat > 0)
		SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP,
	...
	dlen = sizeof(dbuf);
	if ((stat = X25Decode(cdata->clearDetails,
			      cdata->clearDetailsLength, X25I_CHARGESEG, dbuf, &dlen)) < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("X25Decode(X25I_CHARGESEG)"));
		return;
	}
	if (stat > 0)
		SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP, ("%*s Segments", dlen, dbuf));
	dlen = sizeof(dbuf);
	if ((stat = X25Decode(cdata->clearDetails,
			      cdata->clearDetailsLength, X25I_CHARGETIME, dbuf, &dlen)) < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("X25Decode(X25I_CHARGETIME)"));
		return;
	}
	if (stat > 0)
		SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP, ("%*s Time Units", dlen, dbuf));
	dlen = sizeof(dbuf);
	if ((stat = X25Decode(cdata->clearDetails,
			      cdata->clearDetailsLength, X25I_USERDATA, dbuf, &dlen)) < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("X25Decode(X25I_USERDATA)"));
		return;
	}
	if (stat > 0)
		SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP, ("%d Bytes User Data", dlen));
	return;
}
	...
	char *caption;
{
	...
	if (coc == CALLING) {
		if ((stat = getsockopt(fd,
				       X25_ACCESS, XSO_ACCEPTDETAILS, enc_buf, &enc_buf_len) < 0)) {
			SLOG(compat_log, LLOG_EXCEPTIONS, "failed",
...
X25I_CALLEDEXTISO, EPL_STR, "Address ext for dest (ISO): %s",
	    ...
	    X25I_REDIRECTREASON, EPL_INT, "Reason for call redirection: %d", 0, 0, 0,};
	...
	char *caption;
{
	...
	if ((numitems = X25GetItemList(enc_buf, enc_buf_len, lbuf, &lbl)) < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS, "failed", ("Getting Returned Facilities List"));
	}
	for (i = 0; i < numitems; i++) {
		cbl = sizeof(cbuf);
		if ((stat = X25Decode(enc_buf, enc_buf_len, lbuf[i], cbuf, &cbl)) < 0) {
			SLOG(compat_log, LLOG_EXCEPTIONS, "failed",
	...
	char *s;
{
	if (sd >= our_TABLE_SIZE) {
		SLOG(compat_log, LLOG_EXCEPTIONS, s, ("ultrix25.c:sd >= our_TABLE_SIZE"));
		return NOTOK;
	}
	if (sd < NULL) {
		SLOG(compat_log, LLOG_EXCEPTIONS, s, ("ultrix25.c:sd < NULL"));
		return NOTOK;
	}
...
char *
Kloppenburgs_x25_ErrorCause(what, vci)
...
int
check_x25_event(event_expected, event_received, vci)
	X25vc vci;
	int event_expected;
	int event_received;
{
	...
	case X25_Data:
		DLOG(compat_log, LLOG_DEBUG,
		     ("check_x25_event: complete unqualified packet sequence"));
		break;

	case X25_DataM:
		DLOG(compat_log, LLOG_DEBUG,
		     ("check_x25_event: incomplete unqualified packet sequence"));
		break;

	case X25_QData:
		DLOG(compat_log, LLOG_DEBUG,
		     ("check_x25_event: complete qualified packet sequence"));
		break;

	case X25_QDataM:
		DLOG(compat_log, LLOG_DEBUG,
		     ("check_x25_event: incomplete qualified packet sequence"));
		break;

	case X25_Reset:
		DLOG(compat_log, LLOG_DEBUG,
		     ("check_x25_event: a reset indication packet was received"));
	...
	case X25_ResetConf:
		DLOG(compat_log, LLOG_DEBUG,
		     ("check_x25_event: a previous reset request is acknowlewded"));
		break;
	...
	case X25_InterruptConf:
		DLOG(compat_log, LLOG_DEBUG,
		     ("check_x25_event: a previous interrupt request is confirmed"));
		break;

	case X25_Clear:
		DLOG(compat_log, LLOG_DEBUG,
		     ("check_x25_event: a clear indication packet was received"));
		SLOG(compat_log, LLOG_EXCEPTIONS,
	...
	case X25_ClearConf:
		DLOG(compat_log, LLOG_DEBUG,
		     ("check_x25_event: a previous clear request is acknowledged"));
		break;

	case X25_Accepted:
		DLOG(compat_log, LLOG_DEBUG,
		     ("check_x25_event: an outstanding call accepted after X25Call"));
		break;
	...
	int backlog, opt1, opt2;
{
	...
	if (check_x25_event(X25_Call, event, vci)) {
		...
		if (error < 0) {
			SLOG(compat_log, LLOG_EXCEPTIONS,
			     CAT("start_x25_server: ", X25ErrorText(vci, error)), ("X25Clear"));
	...
	if (opt1)
		error = X25Option(vci, (unsigned int) opt1);
	if (error < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS,
	...
	if (opt2)
		error = X25Option(vci, (unsigned int) opt2);
	if (error < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS,
	...
	struct NSAPaddr *rremote;
{
	...
	if (rremote == NULLNA || rremote->na_stack != NA_X25) {
		SLOG(compat_log, LLOG_EXCEPTIONS, NULLCP, ("Invalid type na%d", rremote->na_stack));
	...
	error = X25Call(vci, rremote->na_dte, demsa_local_subaddr,
			...
			x25_interface.x25_cudf, (int) x25_interface.x25_cudflen, NULL);
	...
	if (check_x25_event(X25_Clear, event, vci)) {
		SLOG(compat_log, LLOG_EXCEPTIONS,
		     Kloppenburgs_x25_ErrorCause(X25_Clear, vci), ("X25_Clear_Event"));
...
/******************************************************************/
	...
	if (X25Init(vcip, (unsigned int) NULL) < NULL) {
		DLOG(compat_log, LLOG_DEBUG, (CAT("start_x25_server:", X25ErrorText(vci, error))));
		return NOTOK;
	}
#ifdef	notyet			/* not sure if these are supported... */
#ifndef	BSD43
...
/* Call available ? */
	...
	if (check_x25_event(X25_CallAvail, event, vci_l)) {
...
#ifdef DEBUG
		DLOG(compat_log, LLOG_DEBUG, ("join_x25_client:Call available so X25TakeCall"));
#endif
		...
		if (check_x25_event(X25_Call, event, vci)) {
			...
			DLOG(compat_log, LLOG_DEBUG, ("join_x25_client:X25Info(user_data_fields)"));
...
int
write_x25_socket(sd, buffer, len)
	int sd;
	int len;
	char *buffer;
{
	...
	if (count < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS,
		     CAT("write_x25_socket: ", X25ErrorText(vci, count)), ("write_x25_socket"));
	...
	int sd;
{
	...
	if (error < 0) {
		SLOG(compat_log, LLOG_EXCEPTIONS,
		     CAT("close_x25_socket: ", X25ErrorText(vci, error)), ("close_x25_socket"));
	}

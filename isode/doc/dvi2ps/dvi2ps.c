...
void MoveDown();
void MoveOver();
int NoSignExtend();			/* see cautionary note in code, re arithmetic vs logical
					   shifts */
void OpenFontFile();
int PixRound();
void PutInt();
...
void SetRule();
void SetString();
int SignExtend();			/* see cautionary note in code, re arithmetic vs logical
					   shifts */
void SkipFontDef();
void Warning();
...
#ifndef USEGLOBALMAG
			if (mag != 1000)
				fprintf(stderr, "Magnification of %d ignored.\n", mag);
#endif
	...
	int unmodsize;			/* actually factor * 1000 */
{
	...
	/* the remaining magnification steps are represented with sufficient accuracy already */
	return (realsize);
}
...
void
DoSpecial(str, n)			/* interpret a \special command, made up of keyword=value
					   pairs */
	char *str;
	int n;
{
...
#ifdef	TWG
	else if (strncmp(str, "pbm ", 4) == 0)
		pbm(str + 3);
#endif
	else
		goto oldstuff;	/* if no tpic stuff do oldstuff */
	...
	struct char_entry *ce;
{
	...
	int nbpl;
#endif	/* ~USEPXL */
	float cw;			/* char width, in "dots" - we rely on PostScript
					   maintaining sufficient accuracy */

	/* Output in PostScript coord system (y +ive up, x +ive right) (0,0) of char bitmap at
	   lower left.  Output scan lines from bottom to top */
...
#ifdef USEPXL
	     nbpl << 3, ce->height, ce->xOffset, (((int) ce->height) - ce->yOffset) - 1, cw, c);
#else	/* ~USEPXL */
...
/********************************************************************* */
	...
	/* extract first keyword-value pair from string (value part may be null) return pointer to
	   remainder of string return NULL if none found */
...
********************************************************************** */
{
	...
	for (current = 1; (current <= nopen) && (pixel_files[current].pixel_file_id != fontptr->font_file_id); ++current) ;	/* try 
																   to 
																   find 
																   file 
																   in 
...
/********************************************************************* */
...

	{
		...
		EmitChar(c, ptr);
#ifdef USEPXL
		/* we should really free the space used by the PXL data after this point, but it is 
		   not large, and besides, we may want to be more clever in the future, about
		   sending bitmaps.  So keep the data around */
#endif	/* USEPXL */
	}
...
/********************************************************************* */
...

	{
		...
		/* NOTE: This code assumes that the right-shift is an arithmetic, rather than
		   logical, shift which will propagate the sign bit right.  According to Kernighan

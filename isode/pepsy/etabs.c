...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/pepsy/RCS/etabs.c,v 9.0 1992/06/16 12:24:03 isode Rel";
#endif
	...
	char *type;
{
	...
	if (yal = yp->yp_bef_alist) {
		yal->yal_type = t;
		if (yal->yal_enc)
			gen_act(fp, yp->yp_bef_alist->yal_enc, yp);
	}
	...
	if ((yp->yp_flags & (YP_OPTIONAL | YP_OPTCONTROL | YP_DEFAULT))
		...
		if (yp->yp_optional_act && yp->yp_optional_act->yal_enc) {
			(void) fprintf(fp, "\t{ BOPTIONAL, %d, 0, FL_USELECT, %s},\n",
				       yp->yp_optional_act->yal_enc->a_num, genstrform(yp));
		} else {
	...
	/* handle explicit tags - one day may have to change this if anyone ever defines a type
	   with more than one explicit tag */
	if (yp->yp_flags & YP_TAG && !(yp->yp_flags & YP_IMPLICIT)) {
		(void) fprintf(fp, "\t{ ETAG, 0, ");
		(void) fprintf(fp, "%s, %s, %s },\n", ec_tag(yp), ec_class(yp), genstrform(yp));
	}
	...
	if (yp->yp_yfn && yp->yp_yfn->yfn_enc) {
		...
		if (yp->yp_aft_alist && yp->yp_aft_alist->yal_enc)
			gen_act(fp, yp->yp_aft_alist->yal_enc, yp);
	...
	case YP_CHOICE:
		...
		if (type == NULL || (type && noindirect(f)))
			...
			prtfield(fp, "CHOICE_START", t, type ? f : NULLCP, NULLCP, c_class(yp), yp);
		if (y = yp->yp_type) {
			char *t1;
			...
			if (yp->yp_control_act && yp->yp_control_act->yal_enc) {
				(void) fprintf(fp, "\t{ SCTRL, %d, 0, FL_USELECT, %s },\n",
					       yp->yp_control_act->yal_enc->a_num, genstrform(yp));
	...
	YP yp;
{
	...
	if (yp->yp_flags & YP_TAG && yp->yp_flags & YP_IMPLICIT) {
		...
		if (yp->yp_code < 0 || yp->yp_code > YP_REAL || yp->yp_code == YP_CHOICE)
			i = 0;
		else
			i = fflags[yp->yp_code];
	...
	char *name;
{
	...
	while (code == YP_IDEFINED) {
		if ((sy = syfind(yp1->yp_identifier)) == NULL) {
			if ((p = univtyp(yp1->yp_identifier)) == NULL || p->univ_type <= YP_UNDF)
				ferrs(1,
...
gdflt(fp, yp, which)
	FILE *fp;
	YP yp;
	int which;			/* Which type of entries to generate G_ENC encode G_DEC
					   decode */
{
	YV yv;
	YV yv1;
	...
	case YP_BITLIST:
		...
		case YV_VALIST:
			(void) fprintf(fp, "\t{ %s,	%d,	%d,	0, %s },\n", ndflt,
				       yp->yp_act0_lineno, addptr(yp->yp_action0), genstrform(yp));
			break;
	...
	case YP_REAL:
		switch (yv->yv_code) {
#ifdef ILLEGAL_DEFAULTS
		case YV_IDEFINED:	/* Illegal according to ASN.1 but we can do it so why not
					   support it */
#endif
		case YV_REAL:
			(void) fprintf(fp, "\t{ %s,	0,	%d,	0, %s },\n", ndflt,
				       addptr(concat("&", yp->yp_action0)), genstrform(yp));
			break;
	...
	int (*fn) ();
{
	...
	/* any explicit tag for original yp type is handled before this routine is called so don't
	   call gen_etag for it here but we do need to initialise pd_yt for the case of an IMPLICIT 
	   TAG. */
	...
	if (yt && yt->yt_value) {
		if ((yp->yp_flags & (YP_TAG | YP_IMPLICIT)) == YP_TAG) {
			...
			(void) fprintf(fp, "\t{ ETAG, 0, %d, %s%s, %s },\n",
				       ...
				       class2str(yt->yt_class), flags, genstrform(yp));
			pd_yt = NULLYT;	/* non't pass on any value */

		} else if ((yp->yp_flags & (YP_TAG | YP_IMPLICIT)) == (YP_TAG | YP_IMPLICIT)) {
			/* otherwise it is implicit and so pass its tag down unless we already have 
			   a tag being passed in which case the passed down tag overrides this
			   current tag */
			if (pd_yt == NULLYT)
				pd_yt = yt;
		}
	...
	char *t, *f;
{
	...
	if (noindirect(f) && s != 'q' && s != 'a')
		ferrs(1, "gen_ventry: must specify a field for primative value- not %s\n", f);
	...
	char *dummy;
{
	...
	(void) fprintf(fp, "\t{ FN_CALL, %d, %s, %s, %s },\n",
		       addptr(fn), c_tag(yp), c_class(yp), genstrform(yp));
}
	...
	char *p1;			/* table entry name */
{
	if (type == NULL || (type && noindirect(f)))
		...
		prtfield(fp, p1, t, type ? f : NULLCP, c_tag(yp), c_class(yp), yp);
}

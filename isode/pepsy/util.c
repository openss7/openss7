	...
	va_dcl
{
	...
	(void) sprintf(PY_pepy, "%s: module %s", buffer, module ? module->md_name : "<none>");
	if (p) {
		...
		(void) sprintf(cp, "encoding %s expecting %s,class=%s/id=%d",
			       pname(p1),
			       pr_petype(p->pe_type),
			       pe_classlist[p->pe_flags & FL_CLASS], p->pe_tag);
	}
	if (pe && ((int) pe->pe_class) < pe_maxclass) {
		cp = PY_pepy + strlen(PY_pepy);
		(void) sprintf(cp, " found %s/%d", pe_classlist[pe->pe_class], pe->pe_id);
	}
	...
	va_dcl
{
	...
	if (pe && ((int) pe->pe_class) < pe_maxclass) {
		cp = PY_pepy + strlen(PY_pepy);
		(void) sprintf(cp, " got %s/%d", pe_classlist[pe->pe_class], pe->pe_id);
	}
	...
	int *popt, *optcnt;
{
	...
	case FN_CALL:		/* function call */
		if ((val = (FN_PTR(mod, p)) (parm, (PE *) 0)) == NOTOK)
			return pepsylose(mod, p, NULLPE, "hasdata:FN_CALL:call failed");
		if (val != OK)
			goto next;
		break;
	...
	modtyp *mod;			/* Module it is from */
{
	...
	case BITSTRING:
		...
		if ((p1 = bitstr2strb(pe, &val)) == NULL) {
			...
			return (NOTOK);
			/* Should really abort encoding here but how can we comunicate this to the
			   routine that calls us? */
		}
	      bstring2:
	...
	case FN_CALL:		/* function call */
		if ((val = (FN_PTR(mod, typ)) (parm, (PE *) 0)) == NOTOK)
			return pepsylose(mod, typ, NULLPE, "same:FN_CALL:call failed");
		if (val != OK)
			val = 1;
		else
			val = 0;

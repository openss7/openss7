	...
	char *type;
{
	...
	case YP_BITLIST:
		ptr_tblidx = addptr(modsym(mymodule, yp->yp_varexp ? yp->yp_varexp : id, "bits"));
		/* fall */
	case YP_BIT:
	...
	int idx;
{
	...
	if (typename) {
		...
		(void) fprintf(fp, "\t{ %s, %d, %s, %s, (char **)&%s%s%s[%d] },\n",
			       type, idx, tag, flags, PREFIX, PTR_TABNAME, tab, pindex);
	} else
	...
	int idx;
{
	...
	if (typename) {
		...
		(void) fprintf(fp, "\t{ %s, OFFSET(%s, %s), %s, %s, (char **)&%s%s%s[%d] },\n",
			       type, t, f, tag, flags, PREFIX, PTR_TABNAME, tab, pindex);
	} else
	...
	char *t, *f;
{
	...
	if (typename) {
		...
		(void) fprintf(fp, "\t{ %s, %s, _Z%s, %s, (char **)&%s%s%s[%d] },\n",
			       type, off, obj, flags, PREFIX, PTR_TABNAME, tab, pindex);
	} else
	...
	char *t, *f;
{
	...
	if (typename) {
		...
		(void) fprintf(fp, "\t{ %s, %s, %d, %s, (char **)&%s%s%s[%d] },\n",
			       type, off, tag, flags, PREFIX, PTR_TABNAME, tab, pindex);
	} else
...
/* can't get id from in here - yet */
	if (yp->yp_code == YP_BITLIST) {
		idx = addptr(modsym(mymodule, yp->yp_varexp ? yp->yp_varexp : id, "bits"));
	} else
#endif
		idx = -1;

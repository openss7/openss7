	...
	char **parm;
{
	...
	DLOG(psap_log, LLOG_DEBUG, ("Decoding %s.%s", mod->md_name, pname(p)));
	...
	if (pr_obj(explicit, pe, parm, p, mod) == NOTOK)
		goto bad;
	DLOG(psap_log, LLOG_DEBUG, ("Decoding %s.%s suceeded", mod->md_name, pname(p)));
	return (OK);
      bad:
	DLOG(psap_log, LLOG_DEBUG, ("Decoding %s.%s failed", mod->md_name, pname(p)));
	return (NOTOK);
}
	...
	modtyp *mod;			/* Module it is from */
{
	...
	while (p->pe_type != PE_END) {
		...
		if (ISDTYPE(p) && expl && CHKTAG(mod, p, pe) == 0) {
			if (DEFAULT(p))
				return pepsylose(mod, p, pe, "pr_obj:Default not implemented");
			else if (OPTIONAL(p)) {
				if (ALLOC_MEM(p, parm))
					fix_mem(parm, p);
				goto next;
	...
	modtyp *mod;			/* Module it is from */
{
	...
	/* there is no such pointer as &(*parm + p->pe_ucode) (ucode non zero) so we fudge it by
	   making a temporary one. As no memory needs to be allocated through it this should work */
	...
	while (p->pe_type != PE_END) {
		...
		if (ISDTYPE(p) && expl && CHKTAG(mod, p, pe) == 0) {
			if (DEFAULT(p)) {
				...
				return pepsylose(mod, p, pe, "pr_type:missing mandatory parameter");
		}
		DLOG(psap_log, LLOG_DEBUG, ("Decode type %s.%s type %s",
					    mod->md_name, pname(p), pr_petype(p->pe_type)));
		...
		case FREE_ONLY:	/* the next entry(s) only for freeing routines so skip
					   this and next entry */
			break;
		...
		case MEMALLOC:
			...
			/* The difference between here and the en_type makes me think that this is
			   never called for SEQ_START ???? */
		case SSEQ_START:
			if (p->pe_ucode > 0) {
				if (*parm == NULLCP)
					return inpm(mod, p);
		...
		case IMP_OBJ:
			...
			if (p->pe_type == EXTOBJ) {
				...
				if (p->pe_ucode > 0) {
					...
					nparm = *parm + p->pe_ucode;
					if (dec_f
					    (p->pe_tag, EXT2MOD(mod, (p + 1)), pe, 0 | F_CI,
					     (int *) 0, (char **) 0, (char **) &nparm) == NOTOK)
						goto bad;
				} else
				    if (dec_f
					(p->pe_tag, EXT2MOD(mod, (p + 1)), pe, 0 | F_CI, (int *) 0,
				...
				if (p->pe_type == SOBJECT) {
					if (p->pe_ucode > 0) {
						...
						nparm = *parm + p->pe_ucode;
						if (pr_obj
						    (0, pe, &nparm, mod->md_dtab[p->pe_tag] + 1,
						     mod)
						    == NOTOK)
							goto bad;
					} else
					    if (pr_obj
						(0, pe, parm, mod->md_dtab[p->pe_tag] + 1, mod)
						== NOTOK)
						goto bad;
		...
		case SOBJECT:
			if (p->pe_ucode > 0) {
				if (*parm == NULLCP)
					return inpm(mod, p);
				nparm = *parm + p->pe_ucode;
				if (pr_obj(expl, pe, &nparm, mod->md_dtab[p->pe_tag] + 1, mod) ==
				    NOTOK)
					goto bad;
			} else if (pr_obj(expl, pe, parm, mod->md_dtab[p->pe_tag] + 1, mod) ==
				   NOTOK)
				goto bad;
			break;
		...
		case INTEGER:
			if (pe != NULLPE) {
				DLOG(psap_log, LLOG_DEBUG, ("Decode INTEGER %s.%s -> %d",
							    mod->md_name, pname(p), prim2num(pe)));
		...
		case REALTYPE:
			if (pe != NULLPE) {
				DLOG(psap_log, LLOG_DEBUG, ("Decode REAL %s.%s -> %g",
							    mod->md_name, pname(p), prim2real(pe)));
		...
		case BOOLEAN:
			if (pe != NULLPE) {
				...
				DLOG(psap_log, LLOG_DEBUG, ("Decode BOOLEAN %s.%s -> %d",
							    mod->md_name, pname(p), prim2flag(pe)));
		...
		case T_STRING:
			if ((*((char **) (*parm + p->pe_ucode)) = prim2str(pe, &len))
				...
				return pepsylose(mod, p, pe, "pr_type:bad octet string %s",
						 pe_error(pe->pe_errno));
			/* undocumented feature of prim2str that it adds a NULL char to the end of
			   the string */
			break;

		case BITSTR_PTR:
			if (p[1].pe_type != BITSTR_LEN)
				return pepsylose(mod, &p[1], NULLPE, "pr_type: missing BITSTR_PTR");
		...
		case OCTET_PTR:
			if (p[1].pe_type != OCTET_LEN)
				return pepsylose(mod, &p[1], NULLPE, "pr_type: missing OCTET_PTR");
			if ((*((char **) (*parm + p->pe_ucode)) =
		...
		case FN_CALL:
			if ((FN_PTR(mod, p)) (parm, pe) == NOTOK)
				return pepsylose(mod, p, NULLPE, "pr_type:FN_CALL:call failed");
			break;
	...
	while (p->pe_type != PE_END) {
		...
		if (ISDTYPE(p) && OPTIONAL(p)) {
			...
			if (DEFAULT(p)) {
				...
				return pepsylose(mod, p, pe, "pr_seq:missing mandatory parameter");
		}
		switch (p->pe_type) {
		case OPTL:
		...
		case SOBJECT:
			if (p->pe_ucode > 0) {
				if (*parm == NULLCP)
					return inpm(mod, p);
				nparm = *parm + p->pe_ucode;
				if (pr_obj(1, pe, &nparm, mod->md_dtab[p->pe_tag] + 1, mod) ==
				    NOTOK)
					goto bad;
		...
		while (p->pe_type != PE_END) {
			...
			case SOBJECT:
				if (p->pe_ucode > 0) {
					...
					if (pr_obj(1, pe, &nparm, mod->md_dtab[p->pe_tag] + 1, mod)
					    == NOTOK)
						goto bad;
				} else if (pr_obj(1, pe, parm, mod->md_dtab[p->pe_tag] + 1, mod) ==
					   NOTOK)
					goto bad;
				break;
			...
			case SEXTOBJ:
				...
				if (p->pe_ucode > 0) {
					...
					nparm = *parm + p->pe_ucode;
					if (dec_f
					    (p->pe_tag, EXT2MOD(mod, (p + 1)), pe, 1 | F_CI,
					     (int *) 0, (char **) 0, &nparm) == NOTOK)
						goto bad;
				} else
				    if (dec_f
					(p->pe_tag, EXT2MOD(mod, (p + 1)), pe, 1 | F_CI, (int *) 0,
					 (char **) 0, parm) == NOTOK)
					goto bad;
				break;
	...
	modtyp *mod;			/* Module it is from */
{
	...
	case FREE_ONLY:	/* the next entry(s) only for freeing routines * so skip this and
				   next entry */
		break;
	...
	case UCODE:
		if (mod->md_ducode == NULLIFP || (*mod->md_ducode) (parm, pe, p, 0) == NOTOK)
			goto bad;
		break;
		...
		if (p->pe_type == EXTOBJ) {
			...
			if (p->pe_type == SOBJECT) {
				if (p->pe_ucode > 0) {
					...
					nparm = *parm + p->pe_ucode;
					if (pr_obj
					    (0, pe, (char **) &nparm, mod->md_dtab[p->pe_tag] + 1,
					     mod) == NOTOK)
						goto bad;
				} else
				    if (pr_obj
					(0, pe, (char **) parm, mod->md_dtab[p->pe_tag] + 1,
					 mod) == NOTOK)
					goto bad;
			} else if (pr_obj(0, pe, (char **) (*parm + p->pe_ucode),
					  mod->md_dtab[p->pe_tag] + 1, mod) == NOTOK)
	...
	case SOCTETSTRING:
		if (pe != NULLPE) {
			if ((*((struct qbuf **) (parm + p->pe_ucode))
			     = prim2qb(pe)) == (struct qbuf *) NULL && pe->pe_errno != PE_ERR_NONE)
	...
	case T_STRING:
		if ((*((char **) (*parm + p->pe_ucode)) = prim2str(pe, &cnt))
			...
			return pepsylose(mod, p, pe, "pr_type:bad octet string %s",
					 pe_error(pe->pe_errno));
		/* undocumented feature of prim2str that it adds a NULL char to the end of the
		   string */
		break;
	...
	case BITSTR_PTR:
		if (p[1].pe_type != BITSTR_LEN)
			return pepsylose(mod, &p[1], NULLPE, "pr_etype: missing BITSTR_PTR");
		pe = prim2bit(pe);
		if ((*((char **) (*parm + p->pe_ucode)) =
		     bitstr2strb(pe, (int *) (*parm + (p + 1)->pe_ucode)))
		    == NULLCP && pe->pe_errno != PE_ERR_NONE)
	...
	default:
		return pepsylose(mod, p, pe, "pr_etype: %d not implemented", p->pe_type);
	}
	...
	modtyp *mod;
{
	...
	case FN_CALL:
		if ((FN_PTR(mod, typ)) (parm, NULLPE) == NOTOK)
			return pepsylose(mod, typ, NULLPE, "setdval:FN_CALL:call failed");
		break;
	...
	default:
		 ...
		 * mod
		 */
		(void) pepsylose(mod, typ, NULLPE, "setdval: %d not implemented", typ->pe_type);
		return NOTOK;
	}
	return OK;
}
...
fix_mem(parm, p)
	...
	ptpe *p;
{
	if (p->pe_type != SOBJECT || p[-1].pe_type != MEMALLOC || p[1].pe_type != PE_END)

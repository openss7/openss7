	...
	char *inc;
{
	...
	/* Only need two files <stdio.h> and our types file which includes everything else we need
	...
	if (start_action) {
		...
		(void) fputs(start_action->a_data, fptab);
	}
	...
	if (final_action) {
		...
		(void) fputs(final_action->a_data, fptab);
	}
	...
	char *inc;
{
	...
	for (sy = mysymbols; sy; sy = sy->sy_next) {
		...
		if (yp->yp_direction & YP_ENCODER || Aflag) {
			...
			(void) fprintf(fph, "    %s(%s%s, ", ENCFNCNAME, PREFIX,
		...
		if (yp->yp_direction & YP_DECODER || Aflag) {
			...
			(void) fprintf(fph, "#define %s", buf);
			(void) fprintf(fph, "(pe, top, len, buffer, parm) \\\n");
			(void) fprintf(fph, "    %s(%s%s, ", DECFNCNAME, PREFIX,
				       proc_name(sy->sy_name, 1));
			(void) fprintf(fph, "&%s%s%s, ", PREFIX, tab, MODTYP_SUFFIX);
			(void) fprintf(fph, "pe, top, len, buffer, (char **) parm)\n\n");
#if ACT_CODE
		...
		if (yp->yp_direction & YP_PRINTER || Aflag) {
			...
			(void) fprintf(fph, "#define %s", buf);
			(void) fprintf(fph, "(pe, top, len, buffer, parm) \\\n");
			(void) fprintf(fph, "    %s(%s%s, ", PRNTFNCNAME, PREFIX,
				       proc_name(sy->sy_name, 1));
			(void) fprintf(fph, "&%s%s%s, ", PREFIX, tab, MODTYP_SUFFIX);
			(void) fprintf(fph, "pe, top, len, buffer)\n");
			(void) fprintf(fph, "#define %s_P", buf);
			(void) fprintf(fph, "    %s%s, ", PREFIX, proc_name(sy->sy_name, 1));
	...
	int f1, f2, f3;
{
	...
	/* action tables */
	if (e_actions > 0)
		(void) fprintf(fp, "\tefn_%s,\n", tab);
	else
		(void) fprintf(fp, "\t0,\n");	/* no action code */

	if (d_actions > 0)
		(void) fprintf(fp, "\tdfn_%s,\n", tab);
	else
		(void) fprintf(fp, "\t0,\n");	/* no action code */

	if (p_actions > 0)
		(void) fprintf(fp, "\tpfn_%s,\n", tab);
	else
		(void) fprintf(fp, "\t0,\n");	/* no action code */
	...
	char *p;
{
	...
	if (ptr_max <= ptr_cnt) {
		if (ptr_max == 0)
			...
			ptr_tab =
			    (char **) realloc((char *) ptr_tab,
					      (unsigned) sizeof(char **) * (ptr_max *= 2));
	}
	if (ptr_tab == NULL) {
	...
	FILE *fp;
{
	...
	/* 
	   Appears to be valid ANSI and K&R c, but gives some compilers a real hard time.  So
	   leave it out, unless really needed. */

	(void) fprintf(fp, "static caddr_t %s%s%s[] = {\n", PREFIX, PTR_TABNAME, tab);
#else

	(void) fprintf(fp, "caddr_t %s%s%s[] = {\n", PREFIX, PTR_TABNAME, tab);
#endif

	for (i = 0; i < ptr_cnt; i++)
		(void) fprintf(fp, "    (caddr_t ) %s,\n", ptr_tab[i]);

	if (ptr_cnt <= 0)
		(void) fprintf(fp, "    (caddr_t ) 0,\n");	/* for fussy C compilers */
	...
	char **pstr;
{
	...
	*pstr = p + 1;		/* have to leave it up to the compiler to verify the constant
				   expression for the bit number */
	return (buf);
}
	...
	int ret;
{
	...
	case GEN_ASSIGN:
		if (control_act(act) == -1)
			(void) fprintf(fp, "\t\t/* ignored - empty expression */\n");
		else
			(void) fprintf(fp, "\t\t(%s) = _val;\n", act->a_data);
		break;
	case GEN_RETURN:
		(void) fprintf(fp, "\t\treturn (%s);\n", act->a_data);
		break;
	default:
		(void) fputs(act->a_data, fp);
		(void) putc(';', fp);
		break;
	}
	...
	YP yp;
{
	...
	if ((s = yp->yp_id) == NULLCP) {
		...
		if (yp->yp_code == YP_IDEFINED &&
		    yp->yp_module != NULL && strcmp(yp->yp_module, mymodule) != 0) {
			(void) sprintf(genbuf, "%s.%s", yp->yp_module, s);
			s = genbuf;
		}
	}
	pindex = addsptr(s);
	(void) sprintf(genbuf, "(char **)&%s%s%s[%d]", PREFIX, PTR_TABNAME, tab, pindex);
	return genbuf;
}

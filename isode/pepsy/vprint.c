...
{
	int ia5ok;
	register int hit, i, j, k;
	register char *bp, *cp, *zp;
	...
	register PE pe;
{
	...
	case PE_FORM_PRIM:
		...
		case PE_ID(PE_CLASS_UNIV, PE_PRIM_REAL):
			if ((j = prim2real(pe)) == NOTOK && pe->pe_errno != PE_ERR_NONE)
				goto bad_pe;
			vprint("%g", j);
			break;
#endif
	...
	case PE_FORM_CONS:
		...
		case PE_ID(PE_CLASS_UNIV, PE_CONS_EXTN):
			(void) print_UNIV_EXTERNAL(pe, 1, NULLIP, NULLVP, NULLCP);
			return;
	...
	int rw;
{
	...
	bp = buffer;
	(void) sprintf(bp, "%s %s", rw ? "read" : "wrote", text ? text : "pdu");
	bp += strlen(bp);
	if (pe->pe_context != PE_DFLT_CTX) {

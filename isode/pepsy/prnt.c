...
/* ARGSUSED */
	...
	PE pe;
	int explicit;			/* nonzero means we are call top level print final \n */
	int *len;
	char **buf;
{
	ptpe *p;
	...
	modtyp *mod;			/* Module it is from */
{
	...
	while (p->pe_type != PE_END) {
		...
		case UCODE:
			if (mod->md_pucode == NULLIFP || (*mod->md_pucode) (pe, p) == NOTOK)
				goto bad;
			break;
	...
	while (p->pe_type != PE_END) {
		...
		case REALTYPE:
			if (pe != NULLPE) {
				...
				if ((r = prim2real(pe)) == NOTOK && pe->pe_errno != PE_ERR_NONE)
					goto bad;
				VPRINT(("%g", r));
			}
			break;
		...
		case SBITSTRING:
			if (pe != NULLPE) {
				...
				if (p->pe_ucode >= 0 && mod->md_ptrtab &&
				    mod->md_ptrtab[p->pe_ucode] && bstr->pe_nbits < LOTSOFBITS)
					VPRINT(("%s", bit2str(bstr, mod->md_ptrtab[p->pe_ucode])));
	...
	modtyp *mod;			/* Module it is from */
{
	...
	while (p->pe_type != PE_END) {
		...
		case SEXTOBJ:
			...
			if (prnt_f(p->pe_tag, EXT2MOD(mod, (p + 1)), pe, 0, NULLIP, (char **) 0) ==
			    NOTOK)
				return (NOTOK);
			break;
	...
	while (pe != NULLPE) {
		while (p->pe_type != PE_END) {
			...
			case SEXTOBJ:
				if (p[1].pe_type != EXTMOD) {
					return (ppepsylose
						(mod, p, pe, "p_pr_seqof:missing EXTMOD"));
				}
				if (prnt_f(p->pe_tag, EXT2MOD(mod, (p + 1)), pe, 0, NULLIP,
					   (char **) 0) == NOTOK)
					goto bad;
	...
	for (pe = first_member(head); pe; pe = next_member(head, pe)) {
		while (p->pe_type != PE_END) {
			...
			case SEXTOBJ:
				if (p[1].pe_type != EXTMOD) {
					return (ppepsylose
						(mod, p, pe, "p_pr_setof:missing EXTMOD"));
				}
				if (prnt_f(p->pe_tag, EXT2MOD(mod, (p + 1)), pe, 0, NULLIP,
					   (char **) 0) == NOTOK)
					goto bad;
	...
	ptpe *p;
{
	...
	level = 0;
	if (p->pe_type == PE_END) {
		(void) ppepsylose(NULLMODTYP, p, NULLPE, "next_ptpe: unexpected PE_END");
		return (NULLPTPE);
	}
	do {
	      again:

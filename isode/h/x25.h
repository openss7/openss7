...
#ifndef __ISODE_X25_H__
#define __ISODE_X25_H__
...
typedef struct NSAPaddr CONN_DB;	/* address translation is delayed until connect()/accept()
					   time and returns as fields in a packed structure. calls
					   to an X25Encode() X25Decode() routine unpacks this
					   directly into the ISODE NSAPaddr buffer. not as simple
					   as some but DEC like to be different (sigh) */

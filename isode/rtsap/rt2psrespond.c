...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/rtsap/RCS/rt2psrespond.c,v 9.0 1992/06/16 12:37:45 isode Rel";
#endif
...
int
RtInit_Aux(vecp, vec, rts, rti, dctx)
	int vecp;
	char **vec;
...
#undef	dotoken
	...
	if ((0 < prtorq->checkpointSize && prtorq->checkpointSize < acb->acb_ckpoint)
	    || acb->acb_ckpoint <= 0)
...

	{
		...
		if (dctx)
			...
			(void) rtsaplose(rti, RTS_PARAMETER, NULLCP, "%s: unknown", RT_ASN);
			free_RTS_RTSE__apdus(rtpdu);
			goto out;
		}
		...
		if (acb->acb_rtsid == PE_DFLT_CTX) {
			(void) rtsaplose(rti, RTS_PROTOCOL, NULLCP, "unable to find PCI for RTSE");
			free_RTS_RTSE__apdus(rtpdu);
			goto out;
		}
	}
	...
	if (prtorq->connectionDataRQ->offset == type_RTS_ConnectionData_open) {
		(void) pe_extract(pe, rts->rts_data = prtorq->connectionDataRQ->un.open);
		if (rts->rts_data)

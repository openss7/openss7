	...
	register struct RtSAPindication *rti;
{
	...
	switch (result) {
	case OK:
		if (PActEndRequest(acb->acb_fd, &acb->acb_ssn, NULLPEP, 0, pi) == NOTOK) {
	...
	register struct RtSAPindication *rti;
{
	...
	for (;;) {
		...
		case DONE:
			switch (pi->pi_type) {
			case PI_TOKEN:
				if ((result = doPStoken(acb, &pi->pi_token, trans, rti)) != OK)
					return result;
				continue;

			case PI_SYNC:
				if ((result = doPSsync(acb, &pi->pi_sync, rti)) != OK || trans)
					return result;
				continue;
...
/* begin RTAB APDU */
	if ((pe = pe_alloc(PE_CLASS_UNIV, PE_FORM_CONS, PE_DEFN_IA5S))
	    && set_add(pe, num2prim((integer) result, PE_CLASS_CONT, RTAB_REASON)) != NOTOK) {
		pe->pe_context = acb->acb_rtsid;
	...
	struct RtSAPindication *rti;
{
	...
	if (acb->acb_len > 0) {
		...
		if (pe->pe_inline) {
			if ((acb->acb_realbase = acb->acb_base = malloc((unsigned int) pe->pe_len))
			    == NULL)
				goto congested;
			bcopy((char *) pe->pe_prim, acb->acb_base, acb->acb_len = pe->pe_len);
		} else {
	...
	if (acb->acb_flags & ACB_TWA)
		...
		case ST_PLEASE:
			...
			if (rtpdu->offset != type_RTS_RTSE__apdus_rttp__apdu) {
				(void) rtpktlose(acb, rti, RTS_PROTOCOL, NULLCP, "unexpected PDU");
				free_RTS_RTSE__apdus(rtpdu);
				goto out;
			}
	...
	struct RtSAPindication *rti;
{
	...
	if (acb->acb_flags & ACB_ACT)
		...
		case SN_MINORIND:	/* always confirm it */
			...
			if (PMinSyncResponse(acb->acb_fd, pn->pn_ssn, NULLPEP, 0, pi) == NOTOK) {
	...
	struct RtSAPindication *rti;
{
	...
	case SV_START:
		...
		if (acb->acb_uptrans) {
			if ((*acb->acb_uptrans) (acb->acb_fd, SI_ACTIVITY,
				...
				if (PUReportRequest(acb->acb_fd, SP_LOCAL, NULLPEP, 0, pi) == NOTOK) {
					(void) ps2rtslose(acb, rti, "PUReportRequest", pa);
					goto out;
				}
				return OK;
	...
	case SV_RESUME:	/* XXX: will support this later */
		if (acb->acb_flags & (ACB_ACT | ACB_TURN))
			break;
		if (PUReportRequest(acb->acb_fd, SP_PROCEDURAL, NULLPEP, 0, pi) == NOTOK) {
			(void) ps2rtslose(acb, rti, "PUReportRequest", pa);
			goto out;
		}
		acb->acb_flags |= ACB_ACT;
	...
	case SV_DISCIND:
		if (!(acb->acb_flags & ACB_ACT)
			...
			break;
		if (acb->acb_uptrans)
			(void) (*acb->acb_uptrans) (acb->acb_fd, SI_ACTIVITY, (caddr_t) pv, rti);
		if ((pv->pv_type == SV_INTRIND ? PActIntrResponse(acb->acb_fd, pi)
...
/* XXX: should try lots of things here, based on pp_reason,
	...
	if (acb->acb_uptrans)
		(void) (*acb->acb_uptrans) (acb->acb_fd, SI_REPORT, (caddr_t) pp, rti);

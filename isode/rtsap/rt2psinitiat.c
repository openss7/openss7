...
int
RtOpenRequest2(mode, turn, context, callingtitle, calledtitle,
	       callingaddr, calledaddr, ctxlist, defctxname, data, qos, tctx, rtc, rti)
	int mode, turn;
	OID context;
...
static int
RtOpenRequestAux(mode, turn, context, callingtitle, calledtitle,
		 callingaddr, calledaddr, ctxlist, defctxname, data, qos, tctx, rtc, rti)
	int mode, turn;
	OID context;
	AEI callingtitle, calledtitle;
...
{
	register int i;
	int result, requirements, offset, rtsid, settings;
	PE pe, p, q;
...
/* begin RTORQ APDU */
	...
	if ((DEFAULT_CKPOINT != PCONN_CK_DFLT
	     && set_add(pe, num2prim((integer) DEFAULT_CKPOINT, PE_CLASS_CONT, RTORQ_CKPOINT))
	     == NOTOK)
	    || (DEFAULT_WINDOW != PCONN_WD_DFLT
		&& set_add(pe, num2prim((integer) DEFAULT_WINDOW, PE_CLASS_CONT, RTORQ_WINDOW))
		== NOTOK)
	    ...
	    || set_add(p, q = pe_alloc(PE_CLASS_CONT, PE_FORM_CONS,
				       RTORQ_CD_OPEN)) == NOTOK
	    || set_add(q, data ? data : pe_alloc(PE_CLASS_CONT, PE_FORM_PRIM, 0)) == NOTOK)
		goto no_mem;
/* end RTORQ APDU */
...
#undef	dotoken
	...
	if (ctxlist->pc_nctx >= NPCTX) {
		result = rtsaplose(rti, RTS_PARAMETER, NULLCP, "too many contexts");
		goto out1;
	}
	{
		register int ctx;
		...
		if (tctx)
			...
			result = rtsaplose(rti, RTS_PARAMETER, NULLCP, "%s: unknown", RT_ASN);
			goto out1;
		}
	...
	if (acc->acc_result != ACS_ACCEPT) {
		...
		if (rtpdu->offset != type_RTS_RTSE__apdus_rtorj__apdu) {
			result = rtpktlose(acb, rti, RTS_PROTOCOL, NULLCP, "Unexpected PDU");
...
#undef	dotoken
	...
	if (pp->pc_id != rtsid) {
		result = rtpktlose(acb, rti, RTS_PROTOCOL, NULLCP, "RTSE PCI not found");
		goto out3;
	}
	if (pp->pc_result != PC_ACCEPT) {
		result = rtpktlose(acb, rti, RTS_PROTOCOL, NULLCP, "RTSE PCI rejected");
		goto out3;
	}

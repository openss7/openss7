...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/rtsap/RCS/rt2ss.c,v 9.0 1992/06/16 12:37:45 isode Rel";
#endif
	...
	register struct RtSAPindication *rti;
{
	...
	if (!(acb->acb_flags & ACB_TWA))
		return rtsaplose(rti, RTS_OPERATION, NULLCP, "mode of association is monologue");
	...
	register struct RtSAPindication *rti;
{
	...
	if (acb->acb_ckpoint == 0) {
		...
		if (data == NULLPE) {
			if ((*acb->acb_downtrans) (acb->acb_fd, &base, &len, size,
						   acb->acb_ssn, acb->acb_ack, rti) == NOTOK)
				goto bad_trans;
			if (len == 0) {
				base = NULL;
				goto done;
		...
		for (dp += cc, len -= cc; data == NULLPE || len > 0; dp += cc, len -= cc) {
	...
	switch (result) {
	case OK:
		if (SActEndRequest(acb->acb_fd, &acb->acb_ssn, NULLCP, 0, si) == NOTOK) {
	...
	register struct RtSAPindication *rti;
{
	...
	for (;;) {
		...
		case DONE:
			switch (si->si_type) {
			case SI_TOKEN:
				if ((result = doSStoken(acb, &si->si_token, trans, rti)) != OK)
					return result;
				continue;

			case SI_SYNC:
				if ((result = doSSsync(acb, &si->si_sync, rti)) != OK || trans)
					return result;
				continue;
...
/* begin AbortInformation PSDU (pseudo) */
	if (pe = pe_alloc(PE_CLASS_UNIV, PE_FORM_CONS, PE_CONS_SET)) {
		...
		PLOGP(rtsap_log, OACS_AbortInformation, pe, "AbortInformation", 0);
	...
	struct RtSAPindication *rti;
{
	...
	if (acb->acb_len > 0) {
		...
		if (acb->acb_realbase) {
			if ((dp = malloc(i)) == NULL) {
			      congested:;
				if (SUReportRequest(acb->acb_fd, SP_LOCAL, NULLCP, 0, si) == NOTOK) {
		...
		cp = dp + acb->acb_len;
		for (qb = sx->sx_qbuf.qb_forw; qb != &sx->sx_qbuf; qb = qb->qb_forw)
			if (qb->qb_len) {
		...
		if ((qb = sx->sx_qbuf.qb_forw) != &sx->sx_qbuf && qb->qb_forw == &sx->sx_qbuf) {
			remque(qb);
	...
	if (acb->acb_flags & ACB_TWA)
		...
		case ST_PLEASE:
			...
			if (result != NOTOK && (rtsap_log->ll_events & LLOG_PDUS))
				pvpdu(rtsap_log, print_OACS_Priority_P, pe, "Priority", 1);
#endif
	...
	struct RtSAPindication *rti;
{
	...
	if (acb->acb_flags & ACB_ACT)
		...
		case SN_MINORIND:	/* always confirm it */
			...
			if (SMinSyncResponse(acb->acb_fd, sn->sn_ssn, NULLCP, 0, si) == NOTOK) {
	...
	struct RtSAPindication *rti;
{
	...
	case SV_RESUME:	/* XXX: will support this later */
		if (acb->acb_flags & (ACB_ACT | ACB_TURN))
			break;
		if (SUReportRequest(acb->acb_fd, SP_PROCEDURAL, NULLCP, 0, si) == NOTOK) {
			(void) ss2rtslose(acb, rti, "SUReportRequest", sa);
			goto out;
		}
		acb->acb_flags |= ACB_ACT;
	...
	case SV_DISCIND:
		if (!(acb->acb_flags & ACB_ACT)
			...
			break;
		if (acb->acb_uptrans)
			(void) (*acb->acb_uptrans) (acb->acb_fd, SI_ACTIVITY, (caddr_t) sv, rti);
		if ((sv->sv_type == SV_INTRIND ? SActIntrResponse(acb->acb_fd, si)
...
/* XXX: should try lots of things here, based on pp_reason,
	...
	if (acb->acb_uptrans)
		(void) (*acb->acb_uptrans) (acb->acb_fd, SI_REPORT, (caddr_t) sp, rti);
	...
	struct RtSAPindication *rti;
{
	...
	if (((acb->acb_flags & ACB_INIT) && (acb->acb_flags & ACB_TWA))
	    || (acb->acb_flags & ACB_TURN)) {
		(void) rtpktlose(acb, rti, RTS_PROTOCOL, NULLCP, "association management botched");
		goto out;
	}

	if (acb->acb_flags & ACB_ACT) {
		(void) rtpktlose(acb, rti, RTS_PROTOCOL, NULLCP, "unexpected release indication");
		goto out;
	}
...
#ifdef	DEBUG
	if (result != NOTOK && (rtsap_log->ll_events & LLOG_PDUS))
		pvpdu(rtsap_log, print_OACS_AbortInformation_P, pe, "AbortInformation", 1);
#endif
	...
	register struct SSAPabort *sa;
{
	...
	if (event)
		SLOG(rtsap_log, LLOG_EXCEPTIONS, NULLCP,
		     (sa->sa_cc > 0 ? "%s: %s [%*.*s]" : "%s: %s", event,
		      SErrString(sa->sa_reason), sa->sa_cc, sa->sa_cc, sa->sa_prdata));
	...
	default:
		(void) sprintf(cp = buffer, " (%s at session)", SErrString(sa->sa_reason));
	case SC_TRANSPORT:
	case SC_ABORT:
		reason = RTS_SESSION;
		break;

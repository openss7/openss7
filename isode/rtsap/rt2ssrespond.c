...
#undef	dotoken
	if (acb->acb_owned != 0 && acb->acb_owned != acb->acb_avail) {
		(void) rtsaplose(rti, RTS_PROTOCOL, NULLCP, "token management botched");
		goto out2;
	}
	if (acb->acb_owned)
	...
	if (pcon->pUserData->member_OACS_2->offset == type_OACS_ConnectionData_recover) {
		(void) rtsaplose(rti, RTS_CONGEST, NULLCP, "rejecting attempted recovery");
		pe_free(pe);
	...
	rts->rts_turn = acb->acb_flags & ACB_TURN ? RTS_RESPONDER : RTS_INITIATOR;
	...
	if (pcon->pUserData->member_OACS_2->offset == type_OACS_ConnectionData_open)
		rts->rts_data = pe_expunge(pe, pcon->pUserData->member_OACS_2->un.open);
	else
		rts->rts_data = NULLPE;
...
int
RtBeginResponse(sd, status, data, rti)
	int sd;
	int status;
	PE data;
	struct RtSAPindication *rti;
{
	...
	if ((acb = findacblk(sd)) == NULL || (acb->acb_flags & ACB_CONN))
		return rtsaplose(rti, RTS_PARAMETER, NULLCP, "invalid association descriptor");
	...
	default:
		return rtsaplose(rti, RTS_PARAMETER, NULLCP, "bad value for status parameter");
	}
	missingP(rti);
...
/* begin PAccept PSDU */
		...
		if (set_add(pe, p = pe_alloc(PE_CLASS_CONT, PE_FORM_CONS,
		    ...
		    || set_add(q, r = pe_alloc(PE_CLASS_CONT, PE_FORM_CONS,
					       CN_OPEN)) == NOTOK
		    || set_add(r, data ? data : pe_alloc(PE_CLASS_UNIV, PE_FORM_PRIM, PE_PRIM_NULL))
		    == NOTOK)
			goto no_mem;
/* end PAccept PSDU */
		status = SC_ACCEPT;
...
/* begin PRefuse PSDU */
		if ((pe = pe_alloc(PE_CLASS_UNIV, PE_FORM_CONS, PE_CONS_SET))
		    == NULLPE
		    || set_add(pe, num2prim((integer) status, PE_CLASS_CONT, PREF_REASON)) == NOTOK)
			goto no_mem;
/* end PRefuse PSDU */
		status = SC_REJECTED;
		break;

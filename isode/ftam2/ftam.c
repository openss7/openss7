	...
	char **argv, **envp;
{
	int eof, status, vecp;
	SFP istat;
	char *bp, buffer[BUFSIZ], *vec[NVEC + 1];
	FILE *fp;
	...
	/* The lower layers get a SIGPIPE if the remote end dies while we are sending. The SIGPIPE
	   is followed by a DISCONNECT Request. */

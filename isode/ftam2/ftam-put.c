	...
	if (src[1] == NULL) {
		...
		if (dst == NULL) {
			...
			dst = str2file(dst);
		      ...
		      ask_it:;
			if (query)
				switch (ask("%s %s %s", append ? "append" : "put", *src, dst)) {
				case NOTOK:
					goto out;
	...
	for (gp = src; *gp && !interrupted; gp++) {
		...
		if (sglobbed) {
			if (query)
				...
				advise(NULLCP, "%s %s %s", append ? "append" : "put", *gp, dst);
		}
...
put(src, dst, append)
	...
	int append;
{
	int bsize, fd, magic, result, size;
	PE pe;
...
#ifdef	BRIDGE
static int
putaux(dst, append, fd, pe, vf, size)
	char *dst;
#else
static int
putaux(src, dst, append, fd, pe, vf, size)
	char *src, *dst;
#endif
	int append, fd;
	PE pe;
...
{
	register int n;
	int cc, effector, gd, nc, reason, result;
	PE de, param;
	...
	param = NULLPE;
	if (vf->vf_parameter
	    && enc_f(vf->vf_number, &_ZDOCS_mod, &param, 1, 0, NULLCP, vf->vf_parameter) == NOTOK) {
		advise(NULLCP, "unable to build document type parameter: %s", PY_pepy);
		return NOTOK;
	}
	...
	if (units & FUNIT_LIMITED) {
		ftg->ftg_flags |= FTG_CREATE;
		{
			...
			fa->fa_permitted = FA_PERM_READ | FA_PERM_REPLACE
			    ...
			    | FA_PERM_CHNGATTR | FA_PERM_DELETE | FA_PERM_TRAV;
		...
		case VFS_UBF:
			...
			if (ftop->ftop_parameter && myvf->vf_number >= 0) {
				...
				if (dec_f(myvf->vf_number, &_ZDOCS_mod,
					  ftop->ftop_parameter, 1, NULLIP, NULLVP, &parm) == NOTOK)
					advise(NULLCP,
			...
			if (debug)
				advise(NULLCP, "effector=%d id=0x%x", effector, pe->pe_id);
			if (myvf != vf || watch) {
...
#else
			advise(src, gd != NOTOK ? "fdopen failed on" : "unable to dup");
#endif
			reason = FS_ACC_LCL;
			goto do_cancel;
		}
	...
	if (FDataEndRequest(ftamfd, FACTION_SUCCESS, (struct FTAMdiagnostic *) 0, 0, fti) == NOTOK) {

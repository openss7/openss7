...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/ftam2/RCS/ftamd-select.c,v 9.0 1992/06/16 12:15:43 isode Rel";
#endif
...
ftam_selection(ftg, ftm)
	register struct FTAMgroup *ftg, *ftm;
{
	int action, state;
	...
	if (ftg->ftg_flags & FTG_SELECT) {
		...
		if (!(fa->fa_present & FA_FILENAME)
		    || fa->fa_nfile != 1 || (myfile = getfile(fa->fa_files[0])) == NULL
#ifdef	BRIDGE
...
/* no account checking */
	...
	if (ftg->ftg_flags & FTG_CREATE) {
		...
		for (vf = vfs; vf->vf_entry; vf++)
			if (vf->vf_oid && oid_cmp(vf->vf_oid, fa->fa_contents) == 0)
				break;
		if (!vf->vf_entry) {
			(void) sprintf(dp->ftd_data,
				       "invalid contents-type %s", sprintoid(fa->fa_contents));
			dp->ftd_cc = strlen(dp->ftd_data);
			dp->ftd_type = DIAG_PERM;
		...
		if (chkattrs(fa, fa->fa_present & ~(FA_FILENAME | FA_CONTENTS), 1, &dp) == NOTOK) {
			state = FSTATE_FAILURE;
			goto done_create;
		}
...
#else
			if (statok && ((myst.st_mode & S_IFMT) == S_IFREG ? unlink(myfile)
				       : rmdir(myfile)) == NOTOK) {
			      bad_override:;
#endif
				dp->ftd_type = DIAG_PERM;
	...
	if (ftg->ftg_flags & FTG_RDATTR && (ftm->ftg_readattr.ftra_action == FACTION_SUCCESS)) {
	...
	if (ftg->ftg_flags & FTG_CHATTR && (ftm->ftg_chngattr.ftca_action == FACTION_SUCCESS)) {
	...
	if (ftg->ftg_flags & FTG_OPEN) {
		...
		if (ftop->ftop_contents == NULL) {
			...
			if (ftg->ftg_flags & FTG_CREATE) {
				...
				advise(LLOG_DEBUG, NULLCP,
				       ...
				       oid2ode(ftop->ftop_contents), ftop->ftop_parameter);
				goto find_myvfs;
			}

			if ((ftg->ftg_flags & FTG_RDATTR)
			    && ftm->ftg_readattr.ftra_action == FACTION_SUCCESS
			    && ((fa = &ftm->ftg_readattr.ftra_attrs)->fa_present & FA_CONTENTS)) {
				ftop->ftop_contents = fa->fa_contents;
				ftop->ftop_parameter = fa->fa_parameter;
				advise(LLOG_DEBUG, NULLCP,
				...
				if (oid_cmp(ftop->ftop_contents, myvf->vf_oid) == 0)
					goto find_param;
				advise(LLOG_DEBUG, NULLCP, "wrong intuition; back to step one");
			...
			if ((myvf = st2vfs(myfd, myfile, &myst, NULLOID, ftamfd))
			    == NULL) {
			      no_ascertain:;
				(void) strcpy(dp->ftd_data, "unable to ascertain contents-type");
				dp->ftd_cc = strlen(dp->ftd_data);
				dp->ftd_type = DIAG_PERM;
			...
			advise(LLOG_DEBUG, NULLCP,
			       "using contents-type from st2vfs: %s", oid2ode(ftop->ftop_contents));
		    ...
		    if ((myvf = st2vfs(myfd, myfile, &myst, ftop->ftop_contents, ftamfd)) == NULL) {
			dp->ftd_cc = 0;
		      bad_param:;
			dp->ftd_type = DIAG_PERM;
		...
		if (ftop->ftop_parameter) {
			if (myvf->vf_number < 0) {
				(void) sprintf(dp->ftd_data, "unexpected document type parameter");
				dp->ftd_cc = strlen(dp->ftd_data);
				goto bad_param;
			}
			myparam = NULL;
			...
			if (myvf->vf_check && (*myvf->vf_check) (myparam, dp->ftd_data)
			    == NOTOK)
				goto bad_param;
		} else if (myvf->vf_mandatory > 0) {
			(void) strcpy(dp->ftd_data, "mandatory document type parameter missing");
			dp->ftd_cc = strlen(dp->ftd_data);
			goto bad_param;
		}
	...
	if (ftg->ftg_flags & FTG_CLOSE && (ftm->ftg_close.ftcl_action == FACTION_SUCCESS)) {
		if (myfd != NOTOK) {
#ifdef	BRIDGE
			(void) close(myfd);
			(void) ftp_reply();
...
static int
chkattrs(fa, present, select, diags)
	register struct FTAMattributes *fa;
	long present;
	int select;
	register struct FTAMdiagnostic **diags;
{
	...
	if (present & FA_FILENAME) {
		if (fa->fa_nfile != 1 || (file = getfile(fa->fa_files[0])) == NULL) {
			dp->ftd_type = DIAG_PERM;
...
int
readattrs(attrnames, fa, proposed, parameter, file, st, diags)
	int attrnames;
	register struct FTAMattributes *fa;
	OID proposed;
	PE parameter;
	...
	if (attrnames & FA_CONTENTS) {
		...
		if (vf = st2vfs(myfd, file, st, proposed, ftamfd)) {
			...
			if (proposed
			    ...
			    && parameter && vf->vf_number >= 0 && vf->vf_check) {
				caddr_t p = NULL;
	...
	register struct FTAMdiagnostic **diags;
{
	...
	if (present & FA_FILENAME) {
		...
		if (ftp_rename(myfile, mvfile) == NOTOK) {
			(void) sprintf(dp->ftd_data, "%s: %s", fa->fa_files[0], ftp_error);
#else
		if (stat(mvfile, &n1) == 0) {	/* file exists */
			advise(LLOG_NOTICE, NULLCP, "file %s already exists", mvfile);
			(void) sprintf(dp->ftd_data, "File \"%s\" already exists", fa->fa_files[0]);
			dp->ftd_cc = strlen(dp->ftd_data);
			dp->ftd_type = DIAG_PERM;
			dp->ftd_identifier = FS_MGT_CHANGE;
			dp->ftd_observer = dp->ftd_source = EREF_RFSU;
			...
			*diags = dp;
			return NOTOK;
		} else if (rename(myfile, mvfile) == NOTOK) {	/* on EXDEV could do gonzo copy,
								   but why bother? */
			(void) sprintf(dp->ftd_data, "%s: %s", fa->fa_files[0], sys_errname(errno));
#endif
			dp->ftd_cc = strlen(dp->ftd_data);
#ifndef	BRIDGE
			goto no_change;
...
#endif
	static int i = 0;
	static char buffer1[MAXPATHLEN], buffer2[MAXPATHLEN];
	...
	int mode;
{
	...
	default:
		while ((i = wait(&status)) != NOTOK && pid != i)
			continue;
		if (i != NOTOK && status) {
			if (stat(dir, &st) == NOTOK || (st.st_mode & S_IFMT) != S_IFDIR) {
				i = NOTOK;
				errno = EACCES;
			} else
				status = OK;
...
int
ftruncate(fd, length)			/* works only 'cause we're lucky */
	int fd, length;
{

	...
	char **argv, **envp;
{
	...
	if (Post_Exec(argc, argv, "FTAM main module") == NOTOK)
		exit(1);
#endif
...
#ifdef	DEBUG
	{
		...
		advise(LLOG_DEBUG, NULLCP,
		       ...
		       sprintaei(&fts->fts_calledtitle), paddr2str(&fts->fts_calledaddr, NULLNA));
		advise(LLOG_DEBUG, NULLCP,
		       " calling AE title: %s, calling PSAP address: %s",
		       sprintaei(&fts->fts_callingtitle), paddr2str(&fts->fts_callingaddr, NULLNA));

		advise(LLOG_DEBUG, NULLCP, " manage: %d, ftam-QoS: %d",
		       fts->fts_manage, fts->fts_fqos);
		advise(LLOG_DEBUG, NULLCP, " units: %s", sprintb(fts->fts_units, UMASK));
		advise(LLOG_DEBUG, NULLCP, " attrs: %s", sprintb(fts->fts_attrs, AMASK));

		for (fx = fts->fts_contents.fc_contents, i = 0;
		     i < fts->fts_contents.fc_ncontent; fx++, i++)
	...
	int ndiag;
{
	...
	for (dp = diag, i = ndiag - 1; i >= 0; dp++, i--) {
		...
		if (dp->ftd_cc > 0) {
			cp += strlen(cp);
			(void) sprintf(cp, ": %*.*s", dp->ftd_cc, dp->ftd_cc, dp->ftd_data);
		}

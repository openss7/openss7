...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/ftam2/RCS/ftamd-trans.c,v 9.0 1992/06/16 12:15:43 isode Rel";
#endif
	...
	struct FTAMgroup *ftg;
{
	...
	if ((state != FSTATE_SUCCESS || ((ftm->ftg_flags & FTG_OPEN)
	...
	struct FTAMreadwrite *ftrw;
{
	...
	if (ftp_type(myvf - vfs) == NOTOK
	    && myoperation == FA_OPS_READ && (mycontext = ftrw->ftrw_context) != myvf->vf_context) {
#else
	...
	case VFS_UTF:
	default:
		if (fa->fa_type != FA_FIRSTLAST || fa->fa_firstlast != FA_FIRST) {
			dp->ftd_type = DIAG_PERM;
	...
	if (fti->fti_type == FTI_CANCEL) {
		...
		advise(LLOG_NOTICE, NULLCP, "F-CANCEL.RESPONSE: %d", ftcn->ftcn_action);
...
{
	register int n;
	int bsize, effector, gd, magic, nc, size, cancelled = OK;
	PE pe, de;
	...
	case VFS_UTF:
		...
		if ((gd = dup(myfd)) == NOTOK || (fp = fdopen(gd, "r")) == NULL) {
			if (gd != NOTOK)
				(void) close(gd);
			if (myfd != NOTOK) {
#ifdef	BRIDGE
	...
	switch (myvf - vfs) {
	case VFS_UTF:
		if (p1 && (p1->optionals & opt_DOCS_FTAM__1__Parameters_maximum__string__length)
		    && p1->maximum__string__length > 0)
			size = p1->maximum__string__length;
		break;

	case VFS_UBF:
		if (p3 && (p3->optionals & opt_DOCS_FTAM__3__Parameters_maximum__string__length)
	...
	for (;;) {
		...
		for (ep = (bp = (char *) pe->pe_prim) + size - (fp ? 2 : 0), nc = 0; bp < ep;) {
			if (fp) {
				register char *cp;
	...
	register struct FTAMdiagnostic **diags;
{
	...
	for (errno = 0; d = readdir(dd); errno = 0) {
		...
		if (debug)
#ifdef	BRIDGE
			advise(LLOG_DEBUG, NULLCP, "len=%d name=\"%s\"", strlen(line), line);
#else
		...
		*diags = dp;
#ifdef	BRIDGE
		(void) readattrs(names, fa, vfs[VFS_UTF].vf_oid, NULLPE, path, &st, diags);
#else
		...
		if (encode_DOCS_NBS__9__Datatype1(&pe, 1, 0, NULLCP, d9) == NOTOK) {
			...
			dp->ftd_observer = dp->ftd_source = EREF_RFSU;
			dp->ftd_delay = DIAG_NODELAY;
			(void) sprintf(dp->ftd_data, "error encoding Datatype1: %s", PY_pepy);
			dp->ftd_cc = strlen(dp->ftd_data);
			dp++;
	...
	int concat;
{
	...
	if (pe == NULLPE) {
		if (concat && ninfo > 0 && FDataRequest(ftamfd, info, ninfo, fti) == NOTOK)
	...
	if (concat) {
		...
		if (size + (n = ps_get_abs(pe) + MAGIC_OCTET2) >= fadusize && ninfo > 0) {
			if (debug)
	...
	if (fti->fti_type == FTI_CANCEL) {
		...
		advise(LLOG_NOTICE, NULLCP, "F-CANCEL.INDICATION: %d", ftcn->ftcn_action);
		ftam_diag(ftcn->ftcn_diags, ftcn->ftcn_ndiag);
		FTCNFREE(ftcn);
	...
	register struct PSAPdata *px;
{
	...
	case VFS_UTF:
	{
		...
		if (p1 && (p1->optionals & opt_DOCS_FTAM__1__Parameters_universal__class__number))
			id = (PElementID) p1->universal__class__number;
		else
			id = PE_DEFN_GFXS;
		switch (id) {
	...
	for (pep = px->px_info, i = px->px_ninfo - 1; i >= 0; pep++, i--) {
		...
		if (FCancelRequest(ftamfd, FACTION_PERM, NULLPE, diags, dp - diags, fti) == NOTOK)
			ftam_adios(&fti->fti_abort, "F-CANCEL-REQUEST");

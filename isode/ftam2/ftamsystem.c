...
int fqos;
int ftam_class;
int units = FUNIT_READ | FUNIT_WRITE | FUNIT_LIMITED | FUNIT_ENHANCED | FUNIT_GROUPING;
	...
	register struct FTAMstart *fts;
{
	...
	for (fx = fts->fts_contents.fc_contents,
	     i = fts->fts_contents.fc_ncontent - 1; i >= 0; fx++, i--) {
		if (fx->fc_result != PC_ACCEPT)
			continue;

		for (vf = vfs; vf->vf_entry; vf++)
			if (vf->vf_oid && oid_cmp(vf->vf_oid, fx->fc_dtn) == 0) {
				vf->vf_flags |= VF_OK;
				vf->vf_id = fx->fc_id;
				break;
			}
				...
				if (!oid_cmp(fx->fc_dtn, str2oid(OLDNBS9_OID)) &&
					...
					if ((vf->vf_oid = oid_cpy(fx->fc_dtn)) == NULLOID)
						adios(NULLCP, "%s: out of memory, cannot overload",
						      vf->vf_entry);
					advise(LLOG_NOTICE, NULLCP,
					       "%s files type OID is overloaded!", vf->vf_entry);
					break;
				}
			}
#endif				/* COMPAT_OLD_NBS9OID */
...
/* scan initiator for remote host */
	if ((RemoteHost = rindex(initiator, '@')) == NULL) {
		advise(LLOG_EXCEPTIONS, NULLCP, "missing remote host name in \"%s\"", initiator);
		seterr(FS_ACS_IDENTITY, EREF_RFSU, EREF_IFSU, "missing remote hostname");
	}
	*RemoteHost++ = '\0';
	...
	advise(LLOG_NOTICE, NULLCP,
	       "attemping connection with TCP host \"%s\" for user \"%s\"", RemoteHost, initiator);
	...
	if ((!guest && fts->fts_password == NULL)
	    || *pw->pw_passwd == NULL
	    || (!guest && !chkpassword(initiator, pw->pw_passwd, fts->fts_password)))
		seterr(FS_ACS_PASSWORD, EREF_RFSU, EREF_IFSU, "");

#ifdef DEBUG
	advise(LLOG_DEBUG, NULLCP, "initiator=%s, account=%s", initiator, fts->fts_account);
#endif
	...
	if ((wtmp = open("/usr/adm/wtmp", O_WRONLY | O_APPEND)) != NOTOK) {
		...
		(void) SCPYN(uts.ut_host, na2str(fts->fts_callingaddr.pa_addr.sa_addr.ta_addrs));
#else
...
#ifndef	BRIDGE
	if (cflag || guest) {
		(void) setisobject(1);	/* for PDU pretty-printing AND for A-ASSOCIATE.RESPONSE!!! */
	...
	if (FInitializeResponse(ftamfd, FSTATE_SUCCESS, FACTION_SUCCESS,
				...
				fqos, &fts->fts_contents, diags, dp - diags, fti) == NOTOK)
	...
	register struct FTAMabort *fta;
{
	...
	if (fta->fta_action != FACTION_PERM && !fta->fta_peer)
		(void) FUAbortRequest(ftamfd, FACTION_PERM, (struct FTAMdiagnostic *) 0, 0, &ftis);

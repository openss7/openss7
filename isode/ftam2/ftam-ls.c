...
f_fls(vec)
	char **vec;
{
	int doingpipe, result;
	SFP pstat;
	char *cp, *pp, buffer[BUFSIZ];
	FILE *fp;
...
#ifdef	BRIDGE
	if ((fd = dataconn("LIST")) == NOTOK || (lsfp = fdopen(fd, "w")) == NULL) {
...
static int
ls(file, entry, top, first, last, invis, multi)
	char *file, *entry;
	int top, first, last, invis, multi;
{
	int recurse;
	long mtime;
	char *s;
	...
	if (ftg->ftg_flags & FTG_RDATTR) {
		...
		if (top && (fa->fa_present & FA_FILENAME)
		...
		if (dashl && !invis && !recurse) {
			...
			ut = (fa->fa_present & FA_DATE_MODIFY) ? &fa->fa_date_modify : NULLUTC;
			...
			if (fa->fa_present & FA_CONTENTS) {
				...
				for (uf = vfs; uf->vf_entry; uf++)
					if (oid_cmp(uf->vf_oid, fa->fa_contents) == 0)
						break;
				(void) fprintf(lsfp, "%c ", uf->vf_entry ? uf->vf_stat : ' ');
			} else
				(void) fprintf(lsfp, "  ");
			(void) fprintf(lsfp, "%-8.8s %-8.8s %8d ", s ? s : "",
			...
			if (ut) {
				...
				if (mtime < longtimeago || mtime > now)
					(void) fprintf(lsfp, "%-7.7s %-4.4s ", s + 4, s + 20);
				else
					(void) fprintf(lsfp, "%-12.12s ", s + 4);
			} else
				(void) fprintf(lsfp, "             ");
		}
		if (!invis) {
	...
	char *file;
{
	...
	register struct filent *fi, **xi, **yi;
	...
	default:
		...
		for (i = 0; i < lines; i++)
			for (j = 0; j < columns; j++) {
				...
				(void) fprintf(lsfp, "%s", fi->fi_entry);
				if (w + lines >= nfilent) {
#ifdef	BRIDGE
					(void) fputc('\r', lsfp);
					(void) fputc('\n', lsfp);
				...
				for (w = strlen(fi->fi_entry); w < width; w = (w + 8) & ~7)
					(void) fputc('\t', lsfp);
			}
		free((char *) xi);
	}

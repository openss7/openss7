...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/ftam2/RCS/ftamuser.c,v 9.0 1992/06/16 12:15:43 isode Rel";
#endif
...
char *rs_unknown = "type of remote realstore is unknown; use \"set realstore\"";
	...
	register char *name;
{
	...
	register char *p, *q;
	char buffer[BUFSIZ];
	register struct dispatch *ds, *fs;
	...
	default:
		...
		advise(NULLCP, "ambiguous operation, it could be one of:%s", buffer);
		return NULL;
	}
}
#endif
	...
	struct var *v;
{
	...
	if ((vf = &vfs[tmode]) != &vfs[VFS_DEF]
		...
		advise(NULLCP, "negotiation prevents transfer of %ss", vf->vf_text);
	...
	char **choices;
{
	...
	default:
		...
		advise(NULLCP, "ambiguous value, it could be one of:%s", buffer);
		return NULL;
	}
}
	...
	register char *name;
{
	...
	default:
		...
		advise(NULLCP, "ambiguous variable, it could be one of:%s", buffer);
		return NULL;
	}
}
	...
	char **vec;
{
	register int i, j, w;
	int columns, width, lines;
	register struct dispatch *ds, *es;
	...
	char *event;
{
	...
	if (fta->fta_peer) {
#ifdef	BRIDGE
		(void) sprintf(ftam_error, "%s: peer aborted association, due to ", event);
#else
		(void) fprintf(stderr, "%s: peer aborted association, due to ", event);
#endif
		switch (fta->fta_action) {
		case FACTION_TRANS:
#ifdef	BRIDGE
			(void) sprintf(ftam_error + strlen(ftam_error), "transient-error");
#else
			(void) fprintf(stderr, "transient-error");
#endif
			break;

		case FACTION_PERM:
#ifdef	BRIDGE
			(void) sprintf(ftam_error + strlen(ftam_error), "permanent-error");
#else
			(void) fprintf(stderr, "permanent-error");
#endif
			break;
...
#else
			(void) fprintf(stderr, "action result %d", fta->fta_action);
#endif
			break;
		}
#ifndef	BRIDGE
...
#endif
	ftam_diag(fta->fta_diags, fta->fta_ndiag, fta->fta_peer, FACTION_PERM);
	...
	register struct FTAMcharging *charges;
{
	...
	for (fc = charges->fc_charges, i = charges->fc_ncharge - 1;
	     i >= 0; fc++, i--, cp = "    %s: %d %s\n")
#ifdef	BRIDGE
		(void) sprintf(ftam_error, cp, fc->fc_resource, fc->fc_value, fc->fc_unit);
	advise(NULLCP, "%s", ftam_error);
#else
		(void) printf(cp, fc->fc_resource, fc->fc_value, fc->fc_unit);
#endif
}
...
void
ftam_diag(diag, ndiag, peer, action)
	...
	int peer, action;
{
	...
	for (dp = diag, i = ndiag - 1; i >= 0; dp++, i--) {
...
#else
			(void) printf("%s", FErrString(dp->ftd_identifier));
#endif
			if (dp->ftd_cc > 0)
#ifdef	BRIDGE
				(void) sprintf(ftam_error + strlen(ftam_error),
					       ": %*.*s", dp->ftd_cc, dp->ftd_cc, dp->ftd_data);
#else
				(void) printf(": %*.*s", dp->ftd_cc, dp->ftd_cc, dp->ftd_data);
#endif
		} else if (dp->ftd_cc > 0)
#ifdef	BRIDGE
			(void) sprintf(ftam_error + strlen(ftam_error),
		...
		case DIAG_INFORM:
			...
			didit++;
#ifdef	BRIDGE
			(void) sprintf(ftam_error + strlen(ftam_error), "    type informative");
#else
			(void) printf("    type informative");
#endif
			break;
		...
		case DIAG_TRANS:
			didit++;
#ifdef	BRIDGE
			(void) sprintf(ftam_error + strlen(ftam_error), "    type transient");
#else
			(void) printf("    type transient");
#endif
			break;
		...
		case DIAG_PERM:
			...
			didit++;
#ifdef	BRIDGE
			(void) sprintf(ftam_error + strlen(ftam_error), "    type permanent");
#else
			(void) printf("    type permanent");
#endif
			break;
		...
		default:
			...
			(void) sprintf(ftam_error + strlen(ftam_error),
				       "    type %d", dp->ftd_type);
#else
			(void) printf("    type %d", dp->ftd_type);
#endif
			break;
		}
...
#ifdef	BRIDGE
			(void) sprintf(ftam_error + strlen(ftam_error),
				       "%sobserver %d", didit++ ? ", " : "    ", dp->ftd_observer);
#else
			(void) printf("%sobserver %d", didit++ ? ", " : "    ", dp->ftd_observer);
#endif
			break;
		}
...
#ifdef	BRIDGE
			(void) sprintf(ftam_error + strlen(ftam_error),
				       "%ssource %d", didit++ ? ", " : "    ", dp->ftd_source);
#else
			(void) printf("%ssource %d", didit++ ? ", " : "    ", dp->ftd_source);
#endif
			break;
		}
		...
		if (dp->ftd_delay != DIAG_NODELAY)
			...
			(void) sprintf(ftam_error + strlen(ftam_error),
				       ...
				       dp->ftd_delay);
#else
			(void) printf("%ssuggested-delay %d", didit++ ? ", " : "    ",
				      dp->ftd_delay);
#endif
	...
	FILE *fp;
{
	...
	if (ioctl(fileno(fp), TIOCGWINSZ, (char *) &ws) != NOTOK && (i = ws.ws_col) > 0)
		return i;
#endif

	...
	char **vec;
{
	...
	if ((aei = _str2aei(host, storename, "iso ftam", ontty,
			    userdn && *userdn ? userdn : NULLCP, buffer[0] ? buffer : NULLCP))
	    == NULLAEI) {
		advise(NULLCP, "unable to resolve service: %s", PY_pepy);
#endif
		return NOTOK;
	...
	units |= FUNIT_READ | FUNIT_WRITE | FUNIT_LIMITED | FUNIT_ENHANCED | FUNIT_GROUPING;
	attrs = FATTR_STORAGE;
	fc->fc_ncontent = 0;
	for (vf = vfs; vf->vf_entry; vf++) {
		if (vf->vf_oid) {
			...
			if (!oid_cmp(vf->vf_oid, str2oid(NEWNBS9_OID)))
				 ...
				 */
				fc->fc_contents[fc->fc_ncontent++].fc_dtn =
				    oid_cpy(str2oid(OLDNBS9_OID));
	...
	case FSTATE_SUCCESS:
		...
		for (fx = ftc->ftc_contents.fc_contents,
		     i = ftc->ftc_contents.fc_ncontent - 1; i >= 0; fx++, i--) {
				...
				if (oid_cmp(vf->vf_oid, fx->fc_dtn) == 0) {
					...
					if (!oid_cmp(fx->fc_dtn, str2oid(OLDNBS9_OID)) &&
						...
						if ((vf->vf_oid = oid_cpy(fx->fc_dtn)) == NULLOID)
							adios(NULLCP,
							      "%s: out of memory, cannot overload",
							      vf->vf_entry);
						if (watch)
							advise(NULLCP,
		...
		if (vf->vf_oid == NULLOID || !(vf->vf_flags & VF_OK))
			advise(NULLCP,
			       "%s files not negotiated, suggest you quit now!", vf->vf_entry);
	...
	ftam_diag(ftc->ftc_diags, ftc->ftc_ndiag,
		  ftamfd != NOTOK || fti->fti_abort.fta_peer, ftc->ftc_action);

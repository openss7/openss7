...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/ftam2/RCS/ftam-get.c,v 9.0 1992/06/16 12:15:43 isode Rel";
#endif
	...
	char **vec;
{
	...
	int sglobbed;
	register char *bp;
	register char *dst, **gp, **src;
	char *freedst = NULL, buffer[BUFSIZ];
	struct stat st;
#endif
	...
	if (vf == &vfs[VFS_DEF]
	    && (!(units & FUNIT_LIMITED)
		|| (ftam_class != FCLASS_MANAGE && ftam_class != FCLASS_TM))) {
		advise(NULLCP, "unable to use \"default\" document type on this association");
		return OK;
	}
#ifdef	BRIDGE
	if (*++vec == NULL)
	...
	if (src[1] == NULL) {
		...
		} else if (stat(dst, &st) != NOTOK && (st.st_mode & S_IFMT) == S_IFDIR) {
#ifdef apollo
			if (*dst == '/')
	...
	IFP wfnx;
{
	...
	if (vf == &vfs[VFS_DEF]) {
		if (!(vf = findvf(src))) {
			vf = &vfs[VFS_UBF];
			advise(NULLCP, "unable to determine document type associated with %s", src);
		...
		} else if (vf == &vfs[VFS_FDF]) {
			advise(NULLCP, "%s is a %s", src, vf->vf_text);
			return NOTOK;
		}
	} else if (vf->vf_oid == NULLOID || !(vf->vf_flags & VF_OK)) {
	...
	if (ftg->ftg_flags & FTG_SELECT) {
		...
		ftam_diag(ftse->ftse_diags, ftse->ftse_ndiag, 1, ftse->ftse_action);
	...
	if (ftg->ftg_flags & FTG_OPEN) {
		...
		ftam_diag(ftop->ftop_diags, ftop->ftop_ndiag, 1, ftop->ftop_action);
	...
	if (dst) {
		...
		if ((fd = open(dst, O_WRONLY | O_CREAT | O_TRUNC, 0666)) == NOTOK) {
			...
			if (fti->fti_type == FTI_CANCEL) {
				...
				ftam_diag(ftcn->ftcn_diags, ftcn->ftcn_ndiag, 1, ftcn->ftcn_action);
				FTCNFREE(ftcn);
			}
	...
	case FTI_TRANSEND:
	{
		...
		ftam_diag(ftre->ftre_diags, ftre->ftre_ndiag, 1, ftre->ftre_action);
		FTREFREE(ftre);
	}
		break;
	...
	if (ftg->ftg_flags & FTG_CLOSE) {
		...
		ftam_diag(ftcl->ftcl_diags, ftcl->ftcl_ndiag, 1, ftcl->ftcl_action);
	}

	if (ftg->ftg_flags & FTG_DESELECT) {
		register struct FTAMdeselect *ftde = &ftg->ftg_deselect;

		ftam_diag(ftde->ftde_diags, ftde->ftde_ndiag, 1, ftde->ftde_action);
	...
	char *file;
{
	...
	if (ftg->ftg_flags & FTG_RDATTR) {
		...
		if (debug)
			ftam_diag(ftra->ftra_diags, ftra->ftra_ndiag, 1, ftra->ftra_action);

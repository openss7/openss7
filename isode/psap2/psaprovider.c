...
static int DATAser(), TOKENser(), SYNCser(), ACTIVITYser(), REPORTser(), FINISHser(), ABORTser();
...
int
PDataRequestAux(sd, data, ndata, pi, dtype, sfunc, stype, text, ppdu)
	int sd;
	PE *data;
	int ndata;
	struct PSAPindication *pi;
	char *dtype, *stype, *text;
	IFP sfunc;
	int ppdu;
{
	SBV smask;
	...
	if (ppdu == PPDU_TTD && !(pb->pb_urequirements & SR_TYPEDATA)) {
		(void) sigiomask(smask);
		return psaplose(pi, PC_OPERATION, NULLCP, "typed data service unavailable");
	}

	if ((result = info2ssdu(pb, pi, data, ndata, &realbase, &base, &len, text, ppdu)) != OK)
		goto out2;
	...
	case DONE:
		...
		default:
			(void) ppktlose(pb, pi, PC_PROTOCOL, PPDU_NONE,
					NULLCP,
					"unknown indication (0x%x) from session", si->si_type);
			break;
		}
		break;
	...
	struct PSAPindication *pi;
{
	...
	result = qbuf2info(pb, pi, &sx->sx_qbuf, sx->sx_cc, px->px_info, &px->px_ninfo, text, ppdu);
	...
	struct PSAPindication *pi;
{
	...
	result = ssdu2info(pb, pi, sn->sn_data, sn->sn_cc, pn->pn_info,
			   ...
			   ? PPDU_RS : sn->sn_type == SN_RESETCNF ? PPDU_RSA : PPDU_NONE);
	...
	struct PSAPindication *pi;
{
	...
	result = ssdu2info(pb, pi, sv->sv_data, sv->sv_cc, pv->pv_info,
			   ...
			   ? "P-ACTIVITY-RESUME user-data" : "P-ACTIVITY-END user-data", PPDU_NONE);
...
int
PSetIndications(sd, data, tokens, sync, activity, report, finish, abort, pi)
	int sd;
	...
	register struct SSAPabort *sa;
{
	...
	if (pb) {
		if (sa->sa_cc > 0)
			...
			return ppktlose(pb, pi, reason, PPDU_NONE, NULLCP, "%s", *cp ? cp + 1 : cp);
	} else {
		if (sa->sa_cc > 0)
			return psaplose(pi, reason, NULLCP, "%*.*s%s",
					sa->sa_cc, sa->sa_cc, sa->sa_prdata, cp);
		else
			return psaplose(pi, reason, NULLCP, "%s", *cp ? cp + 1 : cp);
	}
}
...
info2ppdu(pb, pi, data, ndata, ppdu)
	register struct psapblk *pb;
	struct PSAPindication *pi;
	PE *data;
	int ndata, ppdu;
{
	...
	for (d = data, i = 0; i < ndata; i++) {
		...
		if (pb->pb_ncontext > 0
		    ...
		    if (ppdu == PPDU_CP || (pb->pb_ncontext > 1 && pe->pe_context != PE_DFLT_CTX))
...
int
ppdu2info(pb, pi, info, data, ndata, ppdu)
	register struct psapblk *pb;
	struct PSAPindication *pi;
	...
	*ndata = 0;
	...
	case type_PS_User__data_simple:
		...
		if (result != PS_ERR_EOF)
			return ppktlose(pb, pi, result != PS_ERR_NMEM ? PC_INVALID
					: PC_CONGEST, ppdu, NULLCP, "%s", ps_error(result));
		break;
	...
	case type_PS_User__data_complex:
		...
		if (allctx != NOTOK && allctx != OK &&
		    ((pb->pb_flags & (PB_DFLT | PB_MAGIC)) == PB_DFLT) && pb->pb_dctxid == allctx)
...
int
info2ssdu(pb, pi, data, ndata, realbase, base, len, text, ppdu)
	register struct psapblk *pb;
	struct PSAPindication *pi;
	PE *data;
	int ndata;
	...
	*realbase = *base = NULLCP, *len = 0;
	...
	if (ppdu == PPDU_TTD) {
		...
		if ((result = encode_PS_User__data(&pe, 1, 0, NULLCP, info))
			...
			return psaplose(pi, PC_CONGEST, NULLCP, "error encoding PDU: %s", PY_pepy);
		}
	...
	if (info->offset == type_PS_User__data_simple) {
		...
		*realbase = (char *) qb, *base = qb->qb_base;
		...
		pe = NULLPE;
		if (encode_PS_Fully__encoded__data(&pe, 0, 0, NULLCP, info->un.complex) == NOTOK)
			goto losing;
...
int
ssdu2info(pb, pi, base, len, data, ndata, text, ppdu)
	register struct psapblk *pb;
	struct PSAPindication *pi;
	char *base;
	int len;
	...
	int ppdu;
{
	...
	*ndata = 0;
	...
	if (ppdu == PPDU_RS || ppdu == PPDU_RSA) {
		...
		if ((pe = ssdu2pe(base, len, NULLCP, &result)) == NULLPE)
			return ppktlose(pb, pi, result == PS_ERR_NMEM ? PC_CONGEST
					: PC_PROTOCOL, ppdu, NULLCP, "%s", ps_error(result));
	...
	if (ppdu == PPDU_TE ||
	    ...
	    ((PE_CLASS_APPL << PE_CLASS_SHIFT) | (PE_FORM_PRIM << PE_FORM_SHIFT) | (0))) {
		register struct qbuf *qb;
		...
		if (pe->pe_class != PE_CLASS_APPL || pe->pe_form != PE_FORM_CONS || pe->pe_id != 1) {
			PY_advise(NULLCP,
		...
		if (result == NOTOK) {
			(void) ppktlose(pb, pi, PC_UNRECOGNIZED, ppdu, NULLCP, "%s", PY_pepy);
			goto out;
		}
	}
	...
	if (result == OK
	    && ppdu != PPDU_RS
	    && ppdu != PPDU_RSA
	    && info->offset == type_PS_User__data_simple && (psap2_log->ll_events & LLOG_PDUS)) {
		register int i;
...
int
qbuf2info(pb, pi, qb, len, data, ndata, text, ppdu)
	register struct psapblk *pb;
	struct PSAPindication *pi;
	struct qbuf *qb;
	int len;
	...
	struct PSAPindication *pi;
{
	...
	register struct type_PS_Identifier__list *lp, **mp;
	...
	for (j = 0, qp = pb->pb_contexts; j < pb->pb_ncontext; j++, qp++) {
		...
		*mp = lp;
		...
		if ((lp->element_PS_10 = (struct element_PS_11 *)
		    ...
		    || (lp->element_PS_10->transfer__syntax = oid_cpy(qp->pc_atn)) == NULLOID)
			goto no_mem;

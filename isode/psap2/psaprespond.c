	...
	struct PSAPindication *pi;
{
	...
	if ((pb->pb_asn = DFLT_ASN_OID) == NULLOID) {
		(void) ppktlose(pb, pi, PC_ABSTRACT, PPDU_CP, NULLCP, "%s: unknown", DFLT_ASN);
		goto out1;
	}
	if ((pb->pb_asn = oid_cpy(pb->pb_asn)) == NULLOID)
		goto congest;
	if ((pb->pb_atn = DFLT_ATN_OID) == NULLOID) {
		(void) ppktlose(pb, pi, PC_TRANSFER, PPDU_CP, NULLCP, "%s: unknown", DFLT_ATN);
		goto out1;
	}
...

	{
		...
		for (lp = cp_normal->context__list,
		     ...
		     qp = pb->pb_contexts; lp; lp = lp->next, pp++, qp++, i++) {
	...
	struct PSAPindication *pi;
{
	...
	case PC_REJECTED:
		if (pb->pb_result != PC_ACCEPT)
			return psaplose(pi, PC_PARAMETER, NULLCP,
					"invalid result %d for default context name", defctxresult);
...
/* let session provider catch errors in remainder of session parameters */
	...
	if (pb->pb_ncontext > 0) {
		...
		register struct type_PS_Definition__result__list *cd, **cp;
	...
	if (status == PC_ACCEPT) {
		...
		if (pb->pb_flags & PB_DFLT) {
			if ((cpr_normal->default__context = (struct type_PS_Context__result *)
...
#undef	dotoken
	...
	(void) SConnResponse(pb->pb_fd, ref, NULLSA, result, 0, 0, SERIAL_NONE, base, len, si);

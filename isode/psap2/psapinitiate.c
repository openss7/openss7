...
static int
PConnRequestAux(calling, called, ctxlist, defctxname,
		prequirements, srequirements, isn, settings, ref, data, ndata, qos, pc, pi, async)
	struct PSAPaddr *calling, *called;
	int prequirements, srequirements, settings, ndata, async;
	long isn;
	struct PSAPctxlist *ctxlist;
	...
	struct PSAPindication *pi;
{
	...
	if (calling
	    ...
	    && (normal->calling = str2qb(calling->pa_selector, calling->pa_selectlen, 1))
	    == NULL)
		goto no_mem;
	if (called->pa_selectlen > 0
	    && (normal->called = str2qb(called->pa_selector, called->pa_selectlen, 1))
	    == NULL)
		goto no_mem;
		...
		for (pp = ctxlist->pc_ctx, qp = pb->pb_contexts; i >= 0; i--, pp++, qp++) {
			...
			*cp = cd;
			...
			if ((cd->element_PS_5->abstract__syntax =
			    ...
			    || (cd->element_PS_5->transfer__syntax__list = (struct element_PS_7 *)
	...
	if (defctxname) {
		...
		if ((pb->pb_asn = oid_cpy(defctxname)) == NULLOID
		    ...
		    == NULL
		    || (normal->default__context->abstract__syntax = oid_cpy(pb->pb_asn)) == NULLOID
	...
	if (data && ndata > 0) {
		...
		if ((normal->user__data = info2ppdu(pb, pi, data, ndata, PPDU_CP)) == NULL)
			goto out2;
		pb->pb_flags = dflt;
	}
...
#undef	dotoken
		...
		if (decode_PS_CPA__type(pe, 1, NULLIP, NULLVP, &cpa) == NOTOK) {
			(void) ppktlose(pb, pi, PC_UNRECOGNIZED, PPDU_CPA, NULLCP, "%s", PY_pepy);
			goto out2;
		}
		...
		if (decode_PS_CPR__type(pe, 1, NULLIP, NULLVP, &cpr) == NOTOK) {
			(void) ppktlose(pb, pi, PC_UNRECOGNIZED, PPDU_CPR, NULLCP, "%s", PY_pepy);
			goto out2;
		}
	...
	if (qb = cpa ? cpa_normal->responding : cpr_normal->responding) {
		...
		if ((base = qb2str(qb)) == NULLCP && sc->sc_result == SC_ACCEPT) {
			(void) ppktlose(pb, pi, PC_INVALID, PPDU_CPA, NULLCP,
	...
	{
		register struct PSAPcontext *pp, *qp;
		register struct type_PS_Definition__result__list *lp, *mp;
		...
		for (pp = pc->pc_ctxlist.pc_ctx, qp = pb->pb_contexts, mp = lp;
			...
			case int_PS_result_provider__rejection:
				if ((pp->pc_result = mp->element_PS_12->provider__reason) == 0)
			...
			for (pp = preq_pairs; pp->p_mask; pp++)
				if (!(pb->pb_prequirements & pp->p_mask)) {
					if (bit_test(cpa_normal->presentation__fu,
						     pp->p_bitno) == 1) {
						(void) ppktlose(pb, pi, PC_INVALID, PPDU_CPA,
								NULLCP,
								"presentation-requirements negotiation botched");
						goto out2;
					}
				} else if (bit_test(cpa_normal->presentation__fu, pp->p_bitno) < 1)

...
ss_actindication(), ss_reportindication(), ss_finishindication(), ss_abortindication();
...
ps_actindication(), ps_reportindication(), ps_finishindication(), ps_abortindication();
	...
	register struct TSAPdata *tx;
{
	...
	if (mymode == echo) {
		...
		if ((tx->tx_expedited ? TExpdRequest(sd, p, tx->tx_cc, td)
	...
	char **argv;
{
	...
	advise(LLOG_DEBUG, NULLCP, "new requirements: %s", sprintb(ss->ss_requirements, RMASK));
	dotokens();
	...
	default:
		(void) strcpy(buffer, "entity unknown or unavailable");
		if (SConnResponse(sd, &ss->ss_connect, NULLSA,
				  SC_REJECTED, 0, 0, SERIAL_NONE, buffer, strlen(buffer + 1), si)
		    == NOTOK)
	...
	register struct SSAPdata *sx;
{
	...
	case SX_EXPEDITED:
		advise(LLOG_DEBUG, NULLCP, "expedited data, %d bytes", sx->sx_cc);
		break;
	...
	case SX_CAPDIND:
		advise(LLOG_DEBUG, NULLCP, "capability data, %d bytes", sx->sx_cc);
		break;

	case SX_CAPDCNF:
		advise(LLOG_DEBUG, NULLCP, "capability data ack, %d bytes", sx->sx_cc);

	default:
		advise(LLOG_DEBUG, NULLCP,
		       "unknown data indication type=0x%x, %d bytes", sx->sx_type, sx->sx_cc);
	}
#endif
			...
			if (hx->sx_cc > 0) {
				...
				*hx = *sx;	/* struct copy */
				...
				if (!(owned & ST_DAT_TOKEN)
				    && SPTokenRequest(sd, ST_DAT_TOKEN, NULLCP, 0, si) == NOTOK)
					ss_adios(sa, "S-TOKEN-PLEASE.REQUEST");
			}
		} else if (SDataRequest(sd, p = qb2str(&sx->sx_qbuf), sx->sx_cc, si) == NOTOK)
	...
	case SX_EXPEDITED:
		if (mymode == sink)
			break;
		if (SExpdRequest(sd, p = qb2str(&sx->sx_qbuf), sx->sx_cc, si) == NOTOK)
	...
	case SX_TYPED:
		if (mymode == sink)
			break;
		if (STypedRequest(sd, p = qb2str(&sx->sx_qbuf), sx->sx_cc, si) == NOTOK)
			ss_adios(sa, "S-TYPED-DATA.REQUEST");
		break;

	case SX_CAPDIND:
		if (SCapdResponse(sd, p = qb2str(&sx->sx_qbuf), sx->sx_cc, si) == NOTOK)
	...
	register struct SSAPtoken *st;
{
	...
	default:
		adios(NULLCP, "unknown token indication type=0x%x", st->st_type);
	}
	...
	if ((owned & ST_DAT_TOKEN) && hx->sx_cc > 0) {
		...
		hx->sx_qbuf.qb_forw = hx->sx_qbuf.qb_back = &hx->sx_qbuf;
	}
	...
	register struct SSAPactivity *sv;
{
	...
	case SV_START:
		advise(LLOG_DEBUG, NULLCP,
		       ...
		       sv->sv_id.sd_len, sv->sv_id.sd_len, sv->sv_id.sd_data, sv->sv_cc);
		break;
	...
	case SV_INTRIND:
		advise(LLOG_DEBUG, NULLCP,
		       "activity interrupt indication %d, %d bytes", sv->sv_reason, sv->sv_cc);
		break;

	case SV_INTRCNF:
		advise(LLOG_DEBUG, NULLCP, "activity interrupt confirmation, %d bytes", sv->sv_cc);
		break;
	...
	case SV_DISCCNF:
		advise(LLOG_DEBUG, NULLCP, "activity discard confirmation, %d bytes", sv->sv_cc);
		break;
	...
	case SV_ENDCNF:
		advise(LLOG_DEBUG, NULLCP, "activity end confirmation, %d bytes", sv->sv_cc);
		break;

	default:
		advise(LLOG_DEBUG, NULLCP,
		       "unknown activity indication=0x%x, %d bytes", sv->sv_type, sv->sv_cc);
		break;
	}
#endif
	...
	register struct SSAPfinish *sf;
{
	...
	if (sf->sf_cc > 0)
		advise(LLOG_NOTICE, NULLCP, "S-RELEASE.INDICATION: %d bytes", sf->sf_cc);
	else
	...
	char **argv;
{
	...
	if (isacs) {
		...
		advise(LLOG_NOTICE, NULLCP,
		       "  %s, %d, %d>",
		       sprintb(ps->ps_srequirements, RMASK), ps->ps_isn, ps->ps_ssdusize);
	...
	if (isacs) {
		...
		if (is = getisoserventbyselector("tsap", ta->ta_selector, ta->ta_selectlen))
	...
	register struct PSAPdata *px;
{
	...
	switch (px->px_type) {
	case SX_NORMAL:
		advise(LLOG_DEBUG, NULLCP, "normal data, %d elements", px->px_ninfo);
		break;

	case SX_EXPEDITED:
		advise(LLOG_DEBUG, NULLCP, "expedited data, %d elements", px->px_ninfo);
		break;

	case SX_TYPED:
		advise(LLOG_DEBUG, NULLCP, "typed data, %d elements", px->px_ninfo);
		break;

	case SX_CAPDIND:
		advise(LLOG_DEBUG, NULLCP, "capability data, %d elements", px->px_ninfo);
		break;

	case SX_CAPDCNF:
		advise(LLOG_DEBUG, NULLCP, "capability data ack, %d elements", px->px_ninfo);

	default:
		advise(LLOG_DEBUG, NULLCP,
		       "unknown data indication type=0x%x, %d elements", px->px_type, px->px_ninfo);
	}
#endif
		...
		if (srequirements & SR_HALFDUPLEX) {
			if (ix->px_ninfo) {
				...
				*ix = *px;	/* struct copy */
				...
				if (!(owned & ST_DAT_TOKEN)
				    && PPTokenRequest(sd, ST_DAT_TOKEN, NULLPEP, 0, pi) == NOTOK)
	...
	register struct PSAPtoken *pt;
{
	...
	advise(LLOG_DEBUG, NULLCP, "%s tokens: %s, %d elements",
	       ...
	       sprintb((int) pt->pt_tokens, TMASK), pt->pt_ninfo);
#endif
	...
	default:
		adios(NULLCP, "unknown token indication type=0x%x", pt->pt_type);
	}
	...
	register struct PSAPsync *pn;
{
	...
	switch (pn->pn_type) {
	case SN_MAJORIND:
		advise(LLOG_DEBUG, NULLCP, "majorsync indication %d", pn->pn_ssn);
		break;

	case SN_MAJORCNF:
		advise(LLOG_DEBUG, NULLCP, "majorsync confirmation %d", pn->pn_ssn);
		break;
	...
	case SN_MINORCNF:
		advise(LLOG_DEBUG, NULLCP, "minorsync confirmation %d", pn->pn_ssn);
		break;
	...
	case SN_RESETCNF:
		advise(LLOG_DEBUG, NULLCP, "resync confirmation %d", pn->pn_ssn);
		break;
	...
	register struct PSAPactivity *pv;
{
	...
	case SV_START:
		advise(LLOG_DEBUG, NULLCP, "activity start indication: %*.*s",
		       pv->pv_id.sd_len, pv->pv_id.sd_len, pv->pv_id.sd_data);
		break;
	...
	case SV_INTRIND:
		advise(LLOG_DEBUG, NULLCP, "activity interrupt indication %d", pv->pv_reason);
		break;
	...
	case SV_DISCIND:
		advise(LLOG_DEBUG, NULLCP, "activity discard indication %d", pv->pv_reason);
		break;
	...
	case SV_ENDIND:
		advise(LLOG_DEBUG, NULLCP, "activity end indication %d", pv->pv_ssn);
		break;
	...
	default:
		advise(LLOG_DEBUG, NULLCP, "unknown activity indication=0x%x", pv->pv_type);
		break;
	}
	...
	register struct PSAPreport *pp;
{
	...
	advise(LLOG_NOTICE, NULLCP, "%s report %d, %d elements",
	       pp->pp_peer ? "user" : "provider", pp->pp_reason, pp->pp_ninfo);
#endif
	...
	register struct PSAPfinish *pf;
{
	...
	advise(LLOG_NOTICE, NULLCP, "P-RELEASE.INDICATION: %d elements", pf->pf_ninfo);
	...
	register struct PSAPabort *pa;
{
	...
	advise(LLOG_NOTICE, NULLCP, "P-U-ABORT.INDICATION: %d elements", pa->pa_ninfo);
	PAFREE(pa);
	...
	char *event;
{
	...
	advise(LLOG_NOTICE, NULLCP, "%s: %s (source %d)", event, buffer, aca->aca_source);
}
	...
	char **argv;
{
	...
	if (isacs) {
		...
		advise(LLOG_NOTICE, NULLCP, "RT-OPEN.INDICATION: <%d, %s, %s, 0x%x>",
		       ...
		       rts->rts_turn == RTS_RESPONDER ? "responder" : "initiator", rts->rts_data);
		...
		advise(LLOG_NOTICE, NULLCP,
		       ...
		       ntohs(rts->rts_port),
		       saddr2str(&rts->rts_initiator.rta_addr), rts->rts_data);
	}
	...
	if (isacs) {
		...
		case sink:
			if (rts->rts_mode != RTS_TWA && rts->rts_turn != RTS_INITIATOR)
				goto rtse_reject;
		      rtse_accept:
	...
	char **argv;
{
	...
	advise(LLOG_NOTICE, NULLCP, "RO-BEGIN.INDICATION: <%d, <%d, %s>, 0x%x>",
	       ros->ros_sd,
	       ntohs(ros->ros_port), saddr2str(&ros->ros_initiator.roa_addr), ros->ros_data);
	if (ros->ros_data) {

...
#ifndef	lint
static char *rcsid =
    "Header: /xtel/isode/isode/support/RCS/tsapd.c,v 9.0 1992/06/16 12:40:09 isode Rel";
#endif
	...
	for (ta = tas; ta < tz; ta++) {
		...
		if (ta->ta_naddr) {
			if (((na = ta->ta_addrs)->na_stack < 0
			     || na->na_stack >= sizeof services / sizeof services[0]))
...
#ifdef SYS5
#ifdef HPUX
		(void) signal(SIGCHLD, cldser);
#else
		(void) signal(SIGCLD, cldser);
#endif
#endif
	...
	for (is = iae; is < iz; is++)
		...
		if (is->is_addr.ta_selectlen == ts->ts_called.ta_selectlen
		    && bcmp(is->is_addr.ta_selector,
			    ts->ts_called.ta_selector, is->is_addr.ta_selectlen) == 0)
			break;
#endif
	if (is >= iz) {
#endif
		(void) sprintf(buffer, "OSI service tsap/%s not found",
			       sel2str(ts->ts_called.ta_selector, ts->ts_called.ta_selectlen, 1));
		goto out;
	}
#ifndef IAE
}
...
if (hook == NULLIFP || (is = getisoserventbyname("session", "tsap")) == NULL) {
...
*is->is_tail = NULL;
	...
	default:
		...
		(void) sprintf(buffer, "unable to exec %s: %s", *is->is_vec, sys_errname(errno));
...
/* end UGLY */
	...
	if (strcmp(is->is_entity, "presentation") == 0) {
		if (PExec(ss, &pis, buffer1, buffer2, NULLIFP, setperms) == NOTOK) {
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "service not started at psap: %s", PErrString(pa->pa_reason));
			if (pa->pa_cc > 0)
				advise(LLOG_EXCEPTIONS, NULLCP, "   %*.*s",
	...
	if (strcmp(is->is_entity, "rts") == 0) {
		if (RtExec(ss, &rtis, buffer1, buffer2, NULLIFP, setperms) == NOTOK) {
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "service not started at rtsap: %s", RtErrString(rta->rta_reason));
			if (rta->rta_cc > 0)
				advise(LLOG_EXCEPTIONS, NULLCP, "   %*.*s",
		...
		if (RoExec(ss, &rois, buffer1, buffer2, NULLIFP, setperms) == NOTOK) {
			advise(LLOG_EXCEPTIONS, NULLCP,
			       "service not started at rosap: %s", RoErrString(rop->rop_reason));
			if (rop->rop_cc > 0)
				advise(LLOG_EXCEPTIONS, NULLCP, "   %*.*s",
	...
	char **vec;
{
	...
	if (x25_local_pid && *x25_local_pid)
		x25_na->na_pidlen = str2sel(x25_local_pid, -1, x25_na->na_pid, NPSIZE);
	tz->ta_naddr = 1;
	...
	(void) strcpy(x2584_na->na_address, "00");
	...
	while (is = getisoservent())
		if (strcmp(is->is_provider, "tsap") == 0
			...
			if (tz >= tas + NTADDRS) {
				advise(LLOG_EXCEPTIONS, NULLCP,
				       "too many services, starting with %s", is->is_entity);
				break;
			}
	...
	for (vec++; ap = *vec; vec++) {
		if (*ap == '-')
			...
			case 'a':
			{
				...
				for (loop_na = tp4_na_start; loop_na != tp4_na_end; loop_na++) {
					loop_na->ta_addrs->na_dtelen = strlen(ap);
					(void) strcpy(loop_na->ta_addrs->na_dte, ap);
					loop_na->ta_naddr = 1;
			...
			case 'N':
				...
				x2584_na->na_addrlen =
				    implode((u_char *) x2584_na->na_address, ap, strlen(ap));
				continue;
#endif
#ifdef TP4
				{
	...
	if (!rflag
	    && getuid() == 0
	    && stat(ap = isodefile("isoservices", 0), &st) != NOTOK && st.st_uid != 0)
	...
	char **vec;
{
	int argp, options;
	register char *ap;
	char base[BUFSIZ], **argptr, *args[4];
	...
	for (vec++; ap = *vec; vec++) {
		if (*ap == '-')
			...
			case 'c':
				if ((ap = *++vec) == NULL || *ap == '-')
					adios(NULLCP, "usage: %s -c DSA-name-or-address", pgmname);
				continue;
...

	{
		...
		if ((t_pa = str2AttrT("presentationAddress")) == NULL)
			adios(NULLCP, "unknown attribute type \"%s\"", "presentationAddress");
		if ((t_la = str2AttrT("listenAddress")) == NULL)
			adios(NULLCP, "unknown attribute type \"%s\"", "listenAddress");
		...
		if ((fi->FUITEM.UNAVA.ava_value =
		     str2AttrV("iSODEApplicationEntity", fi->FUITEM.UNAVA.ava_type->oa_syntax))
		    == NULL)
		...
		as = as_merge(as, as_comp_new(AttrT_cpy(t_la), NULLAV, NULLACL_INFO));
	...
	int firstime;
{
	...
	register struct TSAPaddr *ta, *tb, *ty;
	struct TSAPaddr tys[NTADDRS];
	int found_listen;

	advise(LLOG_NOTICE, NULLCP, "searching directory for iSODEApplicationEntity objects");
	...
	for (;;) {
		if (debug) {
			...
			pslog(pgm_log, LLOG_DEBUG, "  for", fi_print, (caddr_t) sa->sra_filter);
		}
		...
		sa->sra_baseobject = se->ERR_REFERRAL.DSE_ref_candidates->cr_name;
	}
	...
	for (ptr = sr->CSR_entries; ptr; ptr = ptr->ent_next) {
		...
		for (eptr = ptr->ent_attr; eptr; eptr = eptr->attr_link) {
			...
			if (AttrT_cmp(eptr->attr_type, t_pa) == 0 && found_listen == 0) {
				if (avs = eptr->attr_value) {
					register struct PSAPaddr *pa =
					    (struct PSAPaddr *) avs->avseq_av.av_struct;
			...
			if (AttrT_cmp(eptr->attr_type, t_ev) == 0) {
				if (avs = eptr->attr_value) {
					int vecp;
					register char **vp;
					char *cp, *evec[NVEC + NSLACK + 1];

					cp = (char *) avs->avseq_av.av_struct;
					if ((iz->is_vector = malloc((unsigned) (strlen(cp) + 1)))
					    == NULL)
					...
					if ((iz->is_vec =
					    ...
					    == NULL)
						adios(NULLCP,
						      "out of memory allocating execVector");
					...
					if ((iz->is_vec[0] = malloc((unsigned) (strlen(cp) + 1)))
					    == NULL)
	...
	if (sr->CSR_cr) {
		...
		advise(LLOG_EXCEPTIONS, NULLCP, "%s limit exceeded",
		       ...
		       ? "time"
		       : sr->CSR_limitproblem == LSR_SIZELIMITEXCEEDED ? "size" : "administrative");
	}

	if (i == 0)
		adios(NULLCP, "search failed to find anything");
	else if (debug)
		advise(LLOG_DEBUG, NULLCP, "%d match%s found", i, i != 1 ? "es" : "");
...
bind_to_directory()
{
	...
	register struct ds_bind_arg *ba = &bind_arg, *br = &bind_result;
...
static int
make_bind_args(ba, br, be)
	register struct ds_bind_arg *ba, *br;
...
{
	if (de->dse_type == DSE_REFERRAL && de->ERR_REFERRAL.DSE_ref_candidates) {
		...
		pslog(pgm_log, LLOG_NOTICE, "referring to", (IFP) dn_print, (caddr_t) ap->ap_name);

	...
	char **argv, **envp;
{
	...
	if (index(argv[3], '/')) {
		...
		if ((is = getisoserventbyname(argv[3], argv[2])) == NULL)
			adios(NULLCP, "%s/%s: unknown provider/entity pair", argv[2], argv[3]);
	...
	char *addr;
{
...
#ifdef	DEBUG
	{
		advise(NULLCP, "responding TSAP address: %s", taddr2str(&tc->tc_responding));
	...
	if (fstat(fileno(stdin), &st) != NOTOK
		...
		for (expedited = 0; fgets(buffer, sizeof buffer, stdin); expedited = !expedited) {
...
static int
ts_datarequest(sd, data, cc, expedited)
	int sd;
	char *data;
	int cc, expedited;
{
...
#undef	dotoken
	...
	if ((i = SAsynConnRequest(sf, NULLSA, sz, requirements, tokens,
				  ISN(requirements), userdata, sizeof userdata /* SS_SIZE */ ,
				  NULLQOS,
				  sc, si, 1))
	    == NOTOK) {
		(void) fprintf(stderr, "failed\n");
	...
	if (sc->sc_result != SC_ACCEPT) {
		...
		if (sc->sc_cc > 0)
			adios(NULLCP, "connection rejected: [%s] %*.*s",
			      SErrString(sc->sc_result), sc->sc_cc, sc->sc_cc, sc->sc_data);
		else
...
static int
ss_datarequest(sd, data, cc, dm, sync)
	int sd;
	char *data;
	int cc, dm, sync;
{
	...
	if (sync && (requirements & SR_MINORSYNC) && !(requirements & SR_ACTIVITY)) {
		...
		ss_waitfor(sd, -1);
	} else if (sync && (requirements & SR_ACTIVITY)
	...
	register struct SSAPindication *si;
{
	...
	case SI_TOKEN:
		...
		case ST_PLEASE:
			if (SGTokenRequest(sd, (int) st->st_tokens, si)
			    == NOTOK)
				ss_adios(sa, "S-TOKEN-GIVE.REQUEST");
			else
				owned &= ~st->st_tokens;
...
#undef	dotoken
	...
	if (isacs) {
		...
		if ((i = AcAsynAssocRequest(ode, NULLAEI, aei, NULLPA, pz, pl, ode,
					    prequirements, srequirements, ISN(srequirements),
					    tokens, sf, udata, NACDATA, NULLQOS, acc, aci,
					    1)) == NOTOK) {
			(void) fprintf(stderr, "failed\n");
		...
		if (PConnRequest(NULLPA, pz, pl, NULLOID, prequirements,
				 ...
				 & (SR_MINORSYNC | SR_MAJORSYNC | SR_RESYNC | SR_ACTIVITY)
				 ? (long) (getpid() % (SERIAL_MAX - SERIAL_MIN + 1)) + SERIAL_MIN
				 : SERIAL_NONE, tokens, sf, udata, NPDATA, NULLQOS, pc,
				 pi) == NOTOK) {
			(void) fprintf(stderr, "failed\n");
		...
		if ((i = PAsynConnRequest(NULLPA, pz, pl, NULLOID, prequirements,
					  srequirements, ISN(srequirements), tokens, sf, udata,
					  NPDATA, NULLQOS, pc, pi, 1)) == NOTOK) {
			(void) fprintf(stderr, "failed\n");
			ps_adios(pa, "P-CONNECT.REQUEST");
		}
		sd = pc->pc_sd, cc = 0;
	...
	if (isacs) {
		...
		if (mode == echo) {
			...
			for (i = 0; i < NACDATA; i++) {
				...
				if ((j = prim2num(pe)) == NOTOK && pe->pe_errno != PE_ERR_NONE)
					adios(NULLCP, "hello %d: %s", i, pe_error(pe->pe_errno));
				if (j != i)
...
#undef	dotoken
	...
	if (fstat(fileno(stdin), &st) != NOTOK
		...
		for (j = l = m = 0; fgets(buffer, sizeof buffer, stdin);) {
			k = j >= nmodes ? SX_EXPEDITED : datamodes[j++ % nmodes];
			if ((cc = strlen(buffer) + 1) > SX_EXSIZE - 7 && k == SX_EXPEDITED) {
				if ((k = datamodes[j++ % nmodes]) == SX_EXPEDITED)
					k = datamodes[j++ % nmodes];
			}
...
#undef	dotoken
	...
	if ((requirements & SR_MAJORSYNC) && !(requirements & SR_ACTIVITY)) {
		if (PMajSyncRequest(sd, &ssn, udata, NPDATA, pi) == NOTOK)
			switch (pa->pa_reason) {
			case PC_OPERATION:
				ps_waitfor(sd, ST_DAT_TOKEN | ST_MIN_TOKEN | ST_MAJ_TOKEN);
	...
	if (isacs) {
		...
		if (!pr->pr_affirmative) {
			...
			adios(NULLCP, "release rejected by peer: %d elements", pr->pr_ninfo);
		}
		PRFREE(pr);
	}
	...
	int dm, sync;
{
			...
			case OK:
				...
				if (pe->pe_context != (*px->px_info)->pe_context) {
					advise(NULLCP, "context mismatch, orig=%d echo=%d",
					       pe->pe_context, (*px->px_info)->pe_context);
					status++;
				}
	...
	if (sync && (srequirements & SR_MINORSYNC) && !(srequirements & SR_ACTIVITY)) {
		if (PMinSyncRequest(sd, SYNC_CONFIRM, &ssn, NULLPEP, 0, pi) == NOTOK)
			switch (pa->pa_reason) {
			case PC_OPERATION:
				ps_waitfor(sd, ST_DAT_TOKEN | ST_MIN_TOKEN);
				if (PMinSyncRequest(sd, SYNC_CONFIRM, &ssn, NULLPEP, 0, pi) == OK)
					break;	/* else fall */
		...
		ps_waitfor(sd, -1);
	} else if (sync && (srequirements & SR_ACTIVITY)
	...
	register struct PSAPindication *pi;
{
	...
	case PI_TOKEN:
		...
		case ST_PLEASE:
			if (PGTokenRequest(sd, (int) pt->pt_tokens, pi)
			    == NOTOK)
				ps_adios(pa, "P-TOKEN-GIVE.REQUEST");
			else
				owned &= ~pt->pt_tokens;
	...
	case PI_SYNC:
		switch (pn->pn_type) {
		case SN_MAJORIND:
			adios(NULLCP, "majorsync indication %d", pn->pn_ssn);
			break;
...
#undef	dotoken
	...
	case PI_ACTIVITY:
		...
		case SV_INTRIND:
			adios(NULLCP, "activity interrupt indication %d", pv->pv_reason);

		case SV_INTRCNF:
			break;

		case SV_DISCIND:
			adios(NULLCP, "activity discard indication %d", pv->pv_reason);

		case SV_DISCCNF:
			break;

		case SV_ENDIND:
			adios(NULLCP, "activity end indication %d", pv->pv_ssn);
	...
	case PI_REPORT:
		advise(NULLCP, "%s report %d", pp->pp_peer ? "user" : "provider", pp->pp_reason);
	...
	char *event;
{
	...
	advise(NULLCP, "%s: %s%s", event, buffer, pa->pa_peer ? " (peer initiated)" : "");
}
	...
	char *addr;
{
	...
	if (isacs) {
		...
		if (mode == echo) {
			if (acr->acr_ninfo != 1)
				advise(NULLCP, "got %d elements returned on close", acr->acr_ninfo);
	...
	register struct RtSAPindication *rti;
{
	...
	default:
		adios(NULLCP, "unknown indication type=0x%x", rti->rti_type);
	}
}
	...
	char *addr;
{
	...
	if (isacs) {
		...
		if (roc->roc_result != ROS_ACCEPT) {
			(void) fprintf(stderr, "failed\n");
			adios(NULLCP, "association rejected: [%s]", RoErrString(roc->roc_result));
		}
		(void) fprintf(stderr, "connected\n");
	...
	PE pe;
{
	...
	switch (result = RoInvokeRequest(sd, op++, ROS_SYNC, pe, ++id, NULLIP, ROS_NOPRIO, roi)) {
	case NOTOK:
	...
	case OK:
		...
		case ROI_ERROR:
		{
			...
			if (roe->roe_id != id) {
				advise(NULLCP, "id mismatch (wanted %d, got %d)", id, roe->roe_id);
				status++;
				if (RoURejectRequest(sd, &roe->roe_id,
						     ROS_REP_UNRECOG, ROS_NOPRIO, roi) == NOTOK)
					ros_adios(rop, "RO-REJECT-U.REQUEST");
			} else if (mode == echo && pe_cmp(pe, roe->roe_param)) {
		...
		case ROI_UREJECT:
		{
			...
			if (!rou->rou_noid && rou->rou_id != id) {
				advise(NULLCP, "id mismatch (wanted %d, got %d)", id, rou->rou_id);
				status++;
			}
		}
			break;
	...
	int cc;
{
	...
	advise(NULLCP, "%d bytes %s in %d.%02d seconds (%.2f Kbytes/s)",
	       cc, mode == echo ? "echoed" : "sunk", td.tv_sec, td.tv_usec / 10000, bs / 1024);
}
	...
	int cc;
{
	...
	advise(NULLCP, "%d bytes %s in %d.%02d seconds (%.2f KBytes/s)",
	       cc, mode == echo ? "echoed" : "sunk", secs, msecs / 10, bs / 1024);
}
#endif
#endif

...
dnl =========================================================================
dnl
dnl @(#) $Id: strconf.m4,v 0.9.2.2 2004/04/25 08:49:17 brian Exp $
dnl
dnl =========================================================================
dnl
...
AC_DEFUN([_STRCONF_OPTIONS], [
    AC_ARG_WITH([base-major],
        AS_HELP_STRING([--with-base-major=MAJOR],
            ...
            numbering major devices @<:@default=180@:>@]),
...
AC_DEFUN([_STRCONF_SETUP], [
    STRCONF_CONFIGS=
    for strconf_tmp in `find $builddir $srcdir -type f -name "lis.conf"`
    do
        if test -r "$strconf_tmp" ; then
            STRCONF_CONFIGS="${STRCONF_CONFIGS}${STRCONF_CONFIGS:+ }$strconf_tmp"
        fi
    done
    STRMAKENODES='ss7makenodes'
    STRCONF_SCRIPT="$ac_aux_dir/strconf-sh"
    STRCONF="$SHELL $STRCONF_SCRIPT"
    STRCONF_INPUT='Config.master'
    STRCONF_MAJBASE=180
    STRCONF_CONFIG='strconf.h'
    STRCONF_MKNODES='ss7makenodes.c'
    STRCONF_CONFMOD='conf.modules'
    STRCONF_MAKEDEV='devices.lst'
dnl STRCONF_MODCONF='modconf.h'
dnl STRCONF_DRVCONF='drvrconf.mk'
dnl STRCONF_STSETUP='strsetup.conf'
dnl STRCONF_STRLOAD='strload.conf'
...
dnl     if test :"${STRCONF_MODCONF:+set}" = :set; then
dnl         AC_MSG_NOTICE([creating $STRCONF_MODCONF from $STRCONF_INPUT])
dnl         eval "$STRCONF -b${STRCONF_MAJBASE} --modconf=$STRCONF_MODCONF $STRCONF_INPUT"
dnl     fi
dnl     if test :"${STRCONF_DRVCONF:+set}" = :set; then
dnl         AC_MSG_NOTICE([creating $STRCONF_DRVCONF from $STRCONF_INPUT])
dnl         eval "$STRCONF -b${STRCONF_MAJBASE} --driverconf=$STRCONF_DRVCONF $STRCONF_INPUT"
dnl     fi
...
AC_DEFUN([_STRCONF_OUTPUT_CONFIG], [
    AC_CONFIG_COMMANDS([strconf],
        ...
        STRCONF_CONFIGS="$STRCONF_CONFIGS" \
...
dnl     STRCONF_MODCONF="$STRCONF_MODCONF" \
dnl     STRCONF_DRVCONF="$STRCONF_DRVCONF" \
dnl     STRCONF_STSETUP="$STRCONF_STSETUP" \
dnl     STRCONF_STRLOAD="$STRCONF_STRLOAD" \
        ])
])# _STRCONF_OUTPUT_CONFIG
# =========================================================================
...
dnl     AC_SUBST([STRCONF_MODCONF])
dnl     AC_SUBST([STRCONF_DRVCONF])
dnl     AC_SUBST([STRCONF_STSETUP])
dnl     AC_SUBST([STRCONF_STRLOAD])
        _STRCONF_OUTPUT_CONFIG
])# _STRCONF_OUTPUT
# =========================================================================

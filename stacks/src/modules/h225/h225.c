/*****************************************************************************
 ...
 @(#) $RCSfile: h225.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:14:49 $
 ...
 Last Modified $Date: 2004/01/17 08:14:49 $ by $Author: brian $

 *****************************************************************************/

#ident "@(#) $RCSfile: h225.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:14:49 $"

static char const ident[] = "$RCSfile: h225.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:14:49 $";
...
#define ISDN_REVISION	"OpenSS7 $RCSfile: h225.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:14:49 $"
...
STATIC void isdn_link_dl(ulong, struct dl *, struct fg *, struct tg *);
...
isdn_recv_unexpected_message(queue_t *q, struct cr *cr, isdn_msg_t * m)
{
	...
	int err;
	if (!cr && !(cr = isdn_alloc_cr(m->cr, NULL, dl->fg.fg, NULL, 0)))
		return (-ENOMEM);
...
STATIC ushort isdn_majors[H225_CMAJORS] = { H225_CMAJOR_0, };
...
STATIC void
isdn_link_dl(ulong id, struct dl *dl, struct fg *fg, struct tg *tg)
{
	dl->id = dl_get_id(id);
	/*
...
isdn_init(void)
{
	...
	for (mindex = 0; mindex < H225_CMAJORS; mindex++) {
		if ((err =
		     lis_register_strdev(isdn_majors[mindex], &isdn_info, H225_CMINORS,
					 ISDN_DRV_NAME)) < 0) {
			if (!mindex) {

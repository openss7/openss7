/*****************************************************************************
 ...
 @(#) $RCSfile: sl.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:22:31 $
...
#ident "@(#) $RCSfile: sl.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:22:31 $"

static char const ident[] = "$RCSfile: sl.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:22:31 $";
...
sl_txc_nack_to_be_sent(queue_t *q, struct sl *sl)
{
	...
	ptrace(("%s: %p: [%x/%x] %d\n", SL_MOD_NAME, sl, sl->statem.tx.N.bib | sl->statem.tx.N.bsn,
...
sl_txc_bsnr_and_bibr(queue_t *q, struct sl *sl)
{
	...
	printd(("%s: %p: F.fsn = %x, R.bsn = %x, mask = %x\n", SL_MOD_NAME, sl, sl->statem.tx.F.fsn,

/*****************************************************************************
 ...
 @(#) $RCSfile: sdt_sctp.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:22:50 $
...
#ident "@(#) $RCSfile: sdt_sctp.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:22:50 $"

static char const ident[] = "$RCSfile: sdt_sctp.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:22:50 $";
...
#define SDT_REVISION	"OpenSS7 $RCSfile: sdt_sctp.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:22:50 $"
...
sdt_daedr_received_bits_ind(sdt_t * sp, ulong count, mblk_t *dp)
{
	...
	if (canputnext(sp->iq)) {
		if ((mp = allocb(sizeof(*p), BPRI_MED))) {
			mp->b_datap->db_type = M_PROTO;
			p = ((sdt_daedr_received_bits_ind_t *) mp->b_wptr)++;
...
sdt_daedr_correct_su_ind(sdt_t * sp, ulong count)
{
	...
	if ((mp = allocb(sizeof(*p), BPRI_MED))) {
		mp->b_datap->db_type = M_PCPROTO;
		p = ((sdt_daedr_correct_su_ind_t *) mp->b_wptr)++;
...
sdt_daedr_su_in_error_ind(sdt_t * sp)
{
	...
	if ((mp = allocb(sizeof(*p), BPRI_MED))) {
		mp->b_datap->db_type = M_PCPROTO;
		p = ((sdt_daedr_su_in_error_ind_t *) mp->b_wptr)++;
...
sdt_daedt_transmission_request_ind(sdt_t * sp)
{
	...
	if ((mp = allocb(sizeof(*p), BPRI_MED))) {
		mp->b_datap->db_type = M_PCPROTO;
		p = ((sdt_txc_transmission_request_ind_t *) mp->b_wptr)++;

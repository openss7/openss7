/*****************************************************************************
 ...
 @(#) $RCSfile: trmod.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2009-03-02 05:15:03 $
...
#ident "@(#) $RCSfile: trmod.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2009-03-02 05:15:03 $"

static char const ident[] = "$RCSfile: trmod.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2009-03-02 05:15:03 $";

/*
 * This is TRMOD, a TCAP Transaction Sublayer (TR) module implementing the TRI
 * interface as described in ITU-T Rec. Q.771 as a module pushed over an SCCP
...
#define TRMOD_REVISION	"$RCSfile: trmod.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2009-03-02 05:15:03 $"
...
#define TRMOD_BANNER	TRMOD_DESCRIP	"\n" \
			TRMOD_EXTRA	"\n" \
			TRMOD_REVISION	"\n" \
			TRMOD_COPYRIGHT	"\n" \
			TRMOD_DEVICE	"\n" \
			TRMOD_CONTACT
#define TRMOD_SPLASH	TRMOD_DESCRIP	"\n" \
			TRMOD_REVISION

#ifdef LINUX
MODULE_AUTHOR(TRMOD_CONTACT);
MODULE_DESCRIPTION(TRMOD_DESCRIP);
MODULE_SUPPORTED_DEVICE(TRMOD_DEVICE);
#ifdef MODULE_LICENSE
MODULE_LICENSE(TRMOD_LICENSE);
#endif				/* MODULE_LICENSE */
#ifdef MODULE_ALIAS
MODULE_ALIAS("streams-trmod");
#endif				/* MODULE_ALIAS */
...
#endif
#ifndef TRMOD_MOD_NAME
#error "TRMOD_MOD_NAME must be defined."
#endif
 ...
 * STREAMS DEFINITIONS
 */
#define MOD_ID		TRMOD_MOD_ID
#define MOD_NAME	TRMOD_MOD_NAME
#ifdef MODULE
#define MOD_BANNER	TRMOD_BANNER
#else				/* MODULE */
#define MOD_BANNER	TRMOD_SPLASH
#endif				/* MODULE */

static struct module_info trmod_minfo = {
	.mi_idnum = MOD_ID,
	.mi_idname = MOD_NAME,
	.mi_minpsz = STRMINPSZ,
	.mi_maxpsz = STRMAXPSZ,
 ...
 */
static streamscall int
trmod_qopen(queue_t *q, dev_t *devp, int oflags, int sflag, cred_t *crp)
{
	unsigned long flags;
	struct priv *priv;
	caddr_t ptr;
	...
	if (!mi_set_sth_hiwat(q, SHEADHIWAT >> 1))
		goto enobufs;
	if (!(priv = (struct priv *) (ptr = mi_open_alloc_cache(priv_cachep, GFP_KERNEL))))
		goto enomem;
...
static streamscall int
trmod_qclose(queue_t *q, int oflags, cred_t *crp)
{
	unsigned long flags;
	struct priv *priv = PRIV(q);
	caddr_t ptr = (caddr_t) priv;
	int err;
...
static struct qinit tr_winit = {
	.qi_putp = tr_wput,
	.qi_srvp = tr_wsrv,
	.qi_minfo = &trmod_minfo,
	.qi_mstat = &tr_mstat,
};
static struct qinit sc_rinit = {
	.qi_putp = sc_rput,
	.qi_srvp = sc_rsrv,
	.qi_qopen = trmod_qopen,
	.qi_qclose = trmod_qclose,
	.qi_minfo = &trmod_minfo,
	.qi_mstat = &sc_mstat,
};

MODULE_STATIC struct streamtab trmodinfo = {
	.st_rdinit = &sc_rinit,
	.st_wrinit = &tr_winit,
};
...
trmodexit(void)
{
	...
	if ((err = unregsiter_strmod(&tr_fmod)) < 0)
...
module_init(trmodinit);
module_exit(trmodexit);

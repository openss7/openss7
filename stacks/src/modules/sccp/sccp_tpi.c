 ...
 */
static inline fastcall int
tp_txprim(struct tp *tp, mblk_t *msg, mblk_t *mp, const t_scalar_t prim)
{
	int priority, rtn;
...
/**
 * sc_t_prim: - translate NPI primitive to TPI primitive
 * @prim: the primitive type to translate
 */
static t_scalar_t
sc_t_prim(np_ulong prim)
{
	switch (prim) {
	case N_CONN_REQ:
		return (T_CONN_REQ);
...
/**
 * sc_t_error: - translate NPI error to TPI error
 * @error: error type to translate
 */
static t_scalar_t
sc_t_error(np_ulong error, np_ulong errno)
{
	switch (error) {
	case NBADADDR:
		return (TBADADDR);
...
/**
 * tp_s_address: - convert a TPI address to an SCCP address
 * @t_add: pointer to TPI address
 * @s_add: pointer to SCCP address
 */
static void
tp_s_address(struct sockaddr_sccp *t_add, struct sccp_addr *s_add, np_ulong *lenp)
{
	if (s_add && t_add) {
		s_add->ni = t_add->sccp_ni;
		s_add->ri = t_add->sccp_ri;
		s_add->pc = t_add->sccp_pc;
		s_add->ssn = t_add->sccp_ssn;
		s_add->gtt = t_add->sccp_gtt;
		s_add->nai = t_add->sccp_nai;
		s_add->es = t_add->sccp_es;
		s_add->nplan = t_add->sccp_nplan;
		s_add->tt = t_add->sccp_tt;
		s_add->alen = t_add->sccp_alen;
		memcpy(s_add->addr, t_add->sccp_addr, s_add->alen);
		if (lenp)
...
/**
 * sc_t_address: - convert an NPI SCCP address to a TPI SCCP address
 * @s_add: pointer to NPI address (or NULL)
 * @t_add: pointer to TPI address (not NULL when s_add not NULL)
 */
static void
sc_t_address(struct sccp_addr *s_add, struct sockaddr_sccp *t_add)
{
	if (s_add && t_add) {
		t_add->sccp_family = AF_SCCP;
		t_add->sccp_ni = s_add->ni;
		t_add->sccp_ri = s_add->ri;
		t_add->sccp_pc = s_add->pc;
		t_add->sccp_ssn = s_add->ssn;
		t_add->sccp_gtt = s_add->gtt;
		t_add->sccp_nai = s_add->nai;
		t_add->sccp_es = s_add->es;
		t_add->sccp_nplan = s_add->nplan;
		t_add->sccp_tt = s_add->tt;
		t_add->sccp_alen = s_add->alen;
		memcpy(t_add->sccp_addr, s_add->addr, t_add->sccp_alen);
...
/**
 * sc_t_options: - convert an NPI SCCP QOS structure to TPI options
 * @qos: pointer to NPI SCCP QOS structure
 * @opt: pointer to TPI SCCP options structure
 */
static void
sc_t_options(N_qos_sccp_t * qos, struct t_sccp_options *opt)
{
	if (qos == NULL)
		return;
...
tp_size_opt(struct tp *tp, N_qos_sccp_t *qos, const t_scalar_t prim)
{
	size_t size = 0;
	ulong flags[4] = { 0, };
	...
	case T_CONN_IND:
		...
		size += t_size_set_opts(flags);
		break;
	case T_CONN_CON:
		if (qos->n_qos_type != N_QOS_SEL_CONN_SCCP)
			break;
		flags[0] = tp->opt.flags[0];
		flags[1] = tp->opt.flags[1];
		flags[2] = tp->opt.flags[2];
		flags[3] = tp->opt.flags[3];
		t_qos_set_flags(qos, flags);
		size += t_size_set_opts(flags);
		break;
	case T_UNITDATA_IND:
		if (qos->n_qos_type != N_QOS_SEL_CONN_DATA)
			break;
		t_qos_set_flags(qos, flags);
		size += t_size_set_opts(flags);
		break;
...
t_read_opts(struct tp *tp, N_qos_sccp_t * qos, unsigned char *ip, size_t ilen)
{
	...
	/* This is only for connect request, response and data requests.  Any value of any option
	   ...
	   provided. */
	bzero(tp->opt.flags, sizeof(tp->opt.flags));
	if (ip == NULL || ilen == 0)
		return (0);
	for (ih = _T_OPT_FIRSTHDR_OFS(ip, ilen, 0); ih; ih = _T_OPT_NEXTHDR_OFS(ip, ilen, ih, 0)) {
		...
		case XTI_GENERIC:
			...
			case XTI_DEBUG:
			{
				...
				if (ih->len >= sizeof(*ih) + 4 * sizeof(t_uscalar_t))
					tp->opt.xti.debug[3] = valp[3];
				else
					tp->opt.xti.debug[3] = 0;
				if (ih->len >= sizeof(*ih) + 3 * sizeof(t_uscalar_t))
					tp->opt.xti.debug[2] = valp[2];
				else
					tp->opt.xti.debug[2] = 0;
				if (ih->len >= sizeof(*ih) + 2 * sizeof(t_uscalar_t))
					tp->opt.xti.debug[1] = valp[1];
				else
					tp->opt.xti.debug[1] = 0;
				if (ih->len >= sizeof(*ih) + 1 * sizeof(t_uscalar_t))
					tp->opt.xti.debug[0] = valp[0];
				else
					tp->opt.xti.debug[0] = 0;
				t_set_bit(_T_BIT_XTI_DEBUG, tp->opt.flags);
				continue;
			}
			case XTI_LINGER:
			{
				...
				if (valp->l_linger != T_INFINITE && valp->l_linger != T_UNSPEC
				    && valp->l_linger < 0)
					goto einval;
				tp->opt.xti.linger = *valp;
				t_set_bit(_T_BIT_XTI_LINGER, tp->opt.flags);
				continue;
			}
			case XTI_RCVBUF:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.xti.rcvbuf = *valp;
				t_set_bit(_T_BIT_XTI_RCVBUF, tp->opt.flags);
				continue;
			}
			case XTI_RCVLOWAT:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.xti.rcvlowat = *valp;
				t_set_bit(_T_BIT_XTI_RCVLOWAT, tp->opt.flags);
				continue;
			}
			case XTI_SNDBUF:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.xti.sndbuf = *valp;
				t_set_bit(_T_BIT_XTI_SNDBUF, tp->opt.flags);
				continue;
			}
			case XTI_SNDLOWAT:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.xti.sndlowat = *valp;
				t_set_bit(_T_BIT_XTI_SNDLOWAT, tp->opt.flags);
				continue;
			}
			}
			continue;
		...
		case T_SS7_MTP:
			...
			case T_MTP_PVAR:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.mtp.pvar = *valp;
				t_set_bit(_T_BIT_MTP_PVAR, tp->opt.flags);
				continue;
			}
			case T_MTP_MPLEV:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.mtp.mplev = *valp;
				t_set_bit(_T_BIT_MTP_MPLEV, tp->opt.flags);
				continue;
			}
			case T_MTP_CLUSTER:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.mtp.cluster = *valp;
				t_set_bit(_T_BIT_MTP_CLUSTER, tp->opt.flags);
				continue;
			}
			case T_MTP_PRIO:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.mtp.prio = *valp;
				t_set_bit(_T_BIT_MTP_PRIO, tp->opt.flags);
				continue;
			}
			}
			continue;
		...
		case T_SS7_SCCP:
			...
			case T_SCCP_PVAR:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.pvar = *valp;
				t_set_bit(_T_BIT_SCCP_PVAR, tp->opt.flags);
				continue;
			}
			case T_SCCP_MPLEV:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.mplev = *valp;
				t_set_bit(_T_BIT_SCCP_MPLEV, tp->opt.flags);
				continue;
			}
			case T_SCCP_CLUSTER:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.cluster = *valp;
				t_set_bit(_T_BIT_SCCP_CLUSTER, tp->opt.flags);
				continue;
			}
			case T_SCCP_PRIO:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.prio = *valp;
				t_set_bit(_T_BIT_SCCP_PRIO, tp->opt.flags);
				continue;
			}
			case T_SCCP_PCLASS:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.pclass = *valp;
				t_set_bit(_T_BIT_SCCP_PCLASS, tp->opt.flags);
				continue;
			}
			case T_SCCP_IMP:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.imp = *valp;
				t_set_bit(_T_BIT_SCCP_IMP, tp->opt.flags);
				continue;
			}
			case T_SCCP_RETERR:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.reterr = *valp;
				t_set_bit(_T_BIT_SCCP_RETERR, tp->opt.flags);
				continue;
			}
			case T_SCCP_CREDIT:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.credit = *valp;
				t_set_bit(_T_BIT_SCCP_CREDIT, tp->opt.flags);
				continue;
			}
			case T_SCCP_DISCON_ADD:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.discon_add = *valp;
				t_set_bit(_T_BIT_SCCP_DISCON_ADD, tp->opt.flags);
				continue;
			}
			case T_SCCP_DISCON_REASON:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.discon_reason = *valp;
				t_set_bit(_T_BIT_SCCP_DISCON_REASON, tp->opt.flags);
				continue;
			}
			case T_SCCP_RESET_REASON:
			{
				...
				if (ih->len - sizeof(*ih) != sizeof(*valp))
					goto einval;
				tp->opt.sccp.reset_reason = *valp;
				t_set_bit(_T_BIT_SCCP_RESET_REASON, tp->opt.flags);
				continue;
			}
			}
			continue;
	...
	if (qos) {
		switch (qos->n_qos_type) {
		case N_QOS_SEL_DATA_SCCP:
			if (t_tst_bit(_T_BIT_SCCP_PCLASS, tp->opt.flags))
				qos->sel_data.protocol_class = tp->opt.sccp.pclass;
			else
				qos->sel_data.protocol_class = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_RETERR, tp->opt.flags))
				qos->sel_data.option_flags = tp->opt.sccp.reterr;
			else
				qos->sel_data.option_flags = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_SEQCTRL, tp->opt.flags))
				qos->sel_data.sequence_selection = tp->opt.sccp.seqctrl;
			else
				qos->sel_data.sequence_selection = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_PRIO, tp->opt.flags))
				qos->sel_data.message_priority = tp->opt.sccp.prio;
			else
				qos->sel_data.message_priority = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_IMP, tp->opt.flags))
				qos->sel_data.importance = tp->opt.sccp.imp;
			else
				qos->sel_data.importance = QOS_UNKNOWN;
			break;
		case N_QOS_SEL_CONN_SCCP:
			if (t_tst_bit(_T_BIT_SCCP_PCLASS, tp->opt.flags))
				qos->sel_conn.protocol_class = tp->opt.sccp.pclass;
			else
				qos->sel_conn.protocol_class = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_RETERR, tp->opt.flags))
				qos->sel_conn.option_flags = tp->opt.sccp.reterr;
			else
				qos->sel_conn.option_flags = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_SEQCTRL, tp->opt.flags))
				qos->sel_conn.sequence_selection = tp->opt.sccp.seqctrl;
			else
				qos->sel_conn.sequence_selection = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_PRIO, tp->opt.flags))
				qos->sel_conn.message_priority = tp->opt.sccp.prio;
			else
				qos->sel_conn.message_priority = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_IMP, tp->opt.flags))
				qos->sel_conn.importance = tp->opt.sccp.imp;
			else
				qos->sel_conn.importance = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_CREDIT, tp->opt.flags))
				qos->sel_conn.credit = tp->opt.sccp.credit;
			else
				qos->sel_conn.credit = QOS_UNKNOWN;
			break;
		case N_QOS_SEL_INFO_SCCP:
			if (t_tst_bit(_T_BIT_SCCP_PCLASS, tp->opt.flags))
				qos->sel_info.protocol_class = tp->opt.sccp.pclass;
			else
				qos->sel_info.protocol_class = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_RETERR, tp->opt.flags))
				qos->sel_info.option_flags = tp->opt.sccp.reterr;
			else
				qos->sel_info.option_flags = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_SEQCTRL, tp->opt.flags))
				qos->sel_info.sequence_selection = tp->opt.sccp.seqctrl;
			else
				qos->sel_info.sequence_selection = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_PRIO, tp->opt.flags))
				qos->sel_info.message_priority = tp->opt.sccp.prio;
			else
				qos->sel_info.message_priority = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_IMP, tp->opt.flags))
				qos->sel_info.importance = tp->opt.sccp.imp;
			else
				qos->sel_info.importance = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_CREDIT, tp->opt.flags))
				qos->sel_info.credit = tp->opt.sccp.credit;
			else
				qos->sel_info.credit = QOS_UNKNOWN;
			break;
		case N_QOS_RANGE_INFO_SCCP:
			if (t_tst_bit(_T_BIT_SCCP_PCLASS, tp->opt.flags))
				qos->range_info.protocol_classes = tp->opt.sccp.pclass;
			else
				qos->range_info.protocol_classes = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_SEQCTRL, tp->opt.flags))
				qos->range_info.sequence_selection = tp->opt.sccp.seqctrl;
			else
				qos->range_info.sequence_selection = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_CREDIT, tp->opt.flags))
				qos->range_info.credit = tp->opt.sccp.credit;
			else
				qos->range_info.credit = QOS_UNKNOWN;
			break;
		case N_QOS_SEL_INFR_SCCP:
			if (t_tst_bit(_T_BIT_SCCP_PCLASS, tp->opt.flags))
				qos->sel_infr.protocol_class = tp->opt.sccp.pclass;
			else
				qos->sel_infr.protocol_class = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_RETERR, tp->opt.flags))
				qos->sel_infr.option_flags = tp->opt.sccp.reterr;
			else
				qos->sel_infr.option_flags = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_SEQCTRL, tp->opt.flags))
				qos->sel_infr.sequence_selection = tp->opt.sccp.seqctrl;
			else
				qos->sel_infr.sequence_selection = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_PRIO, tp->opt.flags))
				qos->sel_infr.message_priority = tp->opt.sccp.prio;
			else
				qos->sel_infr.message_priority = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_IMP, tp->opt.flags))
				qos->sel_infr.importance = tp->opt.sccp.imp;
			else
				qos->sel_infr.importance = QOS_UNKNOWN;
			if (t_tst_bit(_T_BIT_SCCP_CREDIT, tp->opt.flags))
				qos->sel_infr.credit = tp->opt.sccp.credit;
			else
	   ...
	   N_qos_sel_conn_sccp_t *qos, t_uscalar_t seq, mblk_t *dp)
{
	struct T_conn_ind *p;
	mblk_t *mp;
	const size_t olen = tp_size_opt(tp, (N_qos_sccp_t *) qos, T_CONN_IND);
	...
	if (unlikely(!bcanputnext(tp->oq, 0)))
		goto ebusy;
	sc_t_address(rem, &tp->rem, &tp->REMADDR_length);
	sc_t_address(loc, &tp->loc, &tp->LOCADDR_length);
	DB_TYPE(mp) = M_PROTO;
	...
	tp_build_opt((N_qos_sccp_t *) qos, mp->b_wptr, olen, T_CONN_IND);
	mp->b_wptr += olen;
	mp->b_cont = dp;
	tp->state.coninds++;
	tp_set_state(tp, TS_WRES_CIND);
	tp_txprim(tp, msg, mp, T_CONN_IND);
	return (0);
      ebusy:
	freeb(mp);
	return (-EBUSY);
...
t_conn_con(struct tp *tp, queue_t *q, mblk_t *msg, struct sccp_addr *res, N_qos_sel_conn_sccp_t *qos, mblk_t *dp)
{
	struct T_conn_con *p;
	mblk_t *mp;
	const size_t olen = tp_size_opt(tp, (N_qos_sccp_t *) qos, T_CONN_CON);
	const size_t mlen = sizeof(*p) + sizeof(tp->res) + olen;

	if (unlikely(!(mp = mi_allocb(q, mlen, BPRI_MED))))
		goto enobufs;
	sc_t_address(res, &tp->res, &tp->RES_length);
	DB_TYPE(mp) = M_PROTO;
	p = (typeof(p)) mp->b_wptr;
	...
	tp_build_opt(tp, (N_qos_sccp_t *) qos, mp->b_wptr, olen);
	mp->b_wptr += olen;
	mp->b_cont = dp;
	tp_set_state(tp, TS_DATA_XFER);
	tp_txprim(tp, msg, mp, T_CONN_CON);
	return (0);
      enobufs:
	return (-ENOBUFS);
}
	     ...
	     t_scalar_t seq, mblk_t *dp)
{
	...
	tp_txprim(tp, msg, mp, T_DISCON_IND);
	return (0);
      enobufs:
	return (-ENOBUFS);
}
...
t_data_ind(struct tp *tp, queue_t *q, mblk_t *msg, t_scalar_t flag, mblk_t *dp)
{
	...
	mp->b_wptr += sizeof(*p);
	mp->b_cont = dp;
	tp_txprim(tp, msg, mp, T_DATA_IND);
	return (0);
      ebusy:
	freeb(mp);
	return (-EBUSY);
...
t_exdata_ind(struct tp *tp, queue_t *q, mblk_t *msg, t_scalar_t flag, mblk_t *dp)
{
	...
	mp->b_wptr += sizeof(*p);
	mp->b_cont = dp;
	tp_txprim(tp, msg, mp, T_EXDATA_IND);
	return (0);
      ebusy:
	freeb(mp);
	return (-EBUSY);
...
t_info_ack(struct tp *tp, queue_t *q, mblk_t *msg)
{
	...
	*p = tp->CAPABILITY_ack.INFO_ack;
	...
	mp->b_wptr += sizeof(*p);
	tp_txprim(tp, msg, mp, T_INFO_ACK);
	return (0);
      enobufs:
	return (-ENOBUFS);
}
 ...
 */
noinline fastcall int
t_bind_ack(struct tp *tp, queue_t *q, mblk_t *msg, struct sockaddr_sccp *add, t_uscalar_t coninds)
{
	struct T_bind_ack *p;
	mblk_t *mp;
	...
	if (unlikely(!(mp = mi_allocb(q, mlen, BPRI_MED))))
		goto enobufs;
	sc_t_address(add, &tp->add, &tp->ADDR_length);
	DB_TYPE(mp) = M_PCPROTO;
	p = (typeof(p)) mp->b_wptr;
	...
	tp_txprim(tp, msg, mp, T_BIND_ACK);
	return (0);
      enobufs:
	return (-ENOBUFS);
}
...
t_error_ack(struct tp *tp, queue_t *q, mblk_t *msg, t_uscalar_t prim, t_scalar_t error)
{
	...
	tp_txprim(tp, msg, mp, T_ERROR_ACK);
	return (0);
      enobufs:
	return (-ENOBUFS);
}
	       ...
	       N_qos_sel_data_sccp_t *qos, mblk_t *dp)
{
	...
	mp->b_wptr += olen;
	mp->b_cont = dp;
	tp_txprim(tp, msg, mp, T_UNITDATA_IND);
	return (0);
      ebusy:
	freeb(mp);
	return (-EBUSY);
	      ...
	      N_qos_sel_data_sccp_t *qos, t_scalar_t error, mblk_t *dp)
{
	...
	mp->b_wptr += olen;
	mp->b_cont = dp;
	tp_txprim(tp, msg, mp, T_UDERROR_IND);
	return (0);
      ebusy:
	freeb(mp);
	return (-EBUSY);
...
t_datack_ind(struct tp *tp, queue_t *q, mblk_t *msg)
{
	...
	mp->b_wptr += sizeof(*p);
	tp->state.datacks--;
	tp_txprim(tp, msg, mp, T_OPTDATA_IND);
	return (0);
      ebusy:
	freeb(mp);
	return (-EBUSY);
...
t_optmgmt_ack(struct tp *tp, queue_t *q, mblk_t *msg, N_qos_sccp_t * qos, t_scalar_t flags)
{
	...
	if (tp_get_state(tp) == TS_WACK_OPTREQ)
		tp_set_state(tp, TS_IDLE);
	tp_txprim(tp, msg, mp, T_OPTMGMT_ACK);
	return (0);
      enobufs:
	return (-ENOBUFS);
}
...
t_ordrel_ind(struct tp *tp, queue_t *q, mblk_t *msg, mblk_t *dp)
{
	...
	tp_txprim(tp, msg, mp, T_ORDREL_IND);
	return (0);
      ebusy:
	freeb(mp);
	return (-EBUSY);
...
t_addr_ack(struct tp *tp, queue_t *q, mblk_t *msg)
{
	...
	tp_txprim(tp, msg, mp, T_ADDR_ACK);
	return (0);
      enobufs:
	return (-ENOBUFS);
}
...
t_capability_ack(struct tp *tp, queue_t *q, mblk_t *msg)
{
	...
	mp->b_wptr += sizeof(*p);
	tp_txprim(tp, msg, mp, T_CAPABILITY_ACK);
	return (0);
      ebusy:
	freeb(mp);
	return (-EBUSY);
...
t_inform_ind(struct tp *tp, queue_t *q, mblk_t *msg, t_uscalar_t orig, t_uscalar_t reason)
{
	...
	*(t_scalar_t *)(oh + 1) = reason;
	...
	oh->name = T_SCCP_PCLASS;
	oh->len = sizeof(*oh) + sizeof(t_scalar_t);
	*(t_scalar_t *)(oh + 1) = tp->opt.sccp.pclass;
	mp->b_wptr += oh->len;
	oh = (typeof(oh)) mp->b_wptr;
	oh->level = T_SS7_SCCP;
	oh->name = T_SCCP_CREDIT;
	oh->len = sizeof(*oh)+ sizeof(t_scalar_t);
	*(t_scalar_t *)(oh + 1) = tp->opt.sccp.credit;
	mp->b_wptr += oh->len;
...
n_conn_req(struct sc *sc, queue_t *q, mblk_t *msg, struct sockaddr_sccp *rem,
	   struct t_sccp_options *qos, np_ulong flags, mblk_t *dp)
{
	N_conn_req_t *p;
	mblk_t *mp;
	...
	if (unlikely(!bcanputnext(sc->oq, 0)))
		goto ebusy;
	tp_s_address(rem, &sc->rem, &sc->REMADDR_length);
	tp_s_options(qos, &sc->qos, &sc->QOS_length, &flags);
	DB_TYPE(mp) = M_PROTO;
	p = (typeof(p)) mp->b_wptr;
...
n_conn_res(struct sc *sc, queue_t *q, mblk_t *msg, struct sockaddr_sccp *res,
	   struct t_sccp_options *qos, np_ulong token, np_ulong flags, np_ulong seq, mblk_t *dp)
{
	N_conn_res_t *p;
	mblk_t *mp;
	...
	if (unlikely(!bcanputnext(sc->oq, 0)))
		goto ebusy;
	tp_s_address(res, &sc->res, &sc->RES_length);
	tp_s_options(qos, &sc->qos, &sc->QOS_length, &flags);
	DB_TYPE(mp) = M_PROTO;
	p = (typeof(p)) mp->b_wptr;
...
n_bind_req(struct sc *sc, queue_t *q, mblk_t *msg, struct sockaddr_sccp *add, np_ulong coninds, np_ulong flags)
{
	...
	if (unlikely(!bcanputnext(sc->oq, 0)))
		goto ebusy;
	tp_s_address(add, &sc->add, &sc->ADDR_length);
	DB_TYPE(mp) = M_PROTO;
	p = (typeof(p)) mp->b_wptr;
 ...
 */
noinline fastcall int
n_unitdata_req(struct sc *sc, queue_t *q, mblk_t *msg, struct sockaddr_sccp *dst,
	       struct t_sccp_options *qos, mblk_t *dp)
{
	N_unitdata_req_t *p;
	mblk_t *mp;
	...
	if (unlikely(!bcanputnext(sc->oq, 0)))
		goto ebusy;
	tp_s_address(dst, &sc->rem, &sc->REMADDR_length);
	tp_s_options(qos, &sc->qos, &sc->QOS_length);
	DB_TYPE(mp) = M_PROTO;
	p = (typeof(p)) mp->b_wptr;
 ...
 */
noinline fastcall int
n_request_req(struct sc *sc, queue_t *q, mblk_t *msg, struct sockaddr_sccp *loc,
	      struct t_sccp_options *qos, np_ulong sref)
{
	N_request_req_t *p;
	mblk_t *mp;
	...
	if (unlikely(!bcanputnext(sc->oq, 0)))
		goto ebusy;
	tp_s_address(loc, &sc->loc, &sc->LOCADDR_length);
	tp_s_options(qos, &sc->qos, &sc->QOS_length, NULL);
	DB_TYPE(mp) = M_PROTO;
	p = (typeof(p)) mp->b_wptr;
 ...
 */
static int
tp_parse_opt(struct tp *tp, unsigned char *optr, size_t olen, t_uscalar_t flags, t_uscalar_t prim)
{
	int err;
...
tp_conn_req(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	if (unlikely(p->DEST_length == 0))
		goto noaddr;
	if (unlikely(p->DEST_length < sizeof(tp->rem)))
		goto badaddr;
	...
	if (p->OPT_length == 0)
		goto noopt;
	if (p->OPT_length < sizeof(struct t_opthdr) + sizeof(t_uscalar_t))
		goto badopt;
	if ((err = t_parse_opt(tp, mp->b_rptr + p->OPT_offset, p->OPT_length, T_CONN_REQ)))
		goto error;
      noopt:
	if (mp->b_cont && unlikely(msgsize(mp->b_cont) > tp->CAPABILITY_ack.INFO_ack.CDATA_size))
		goto baddata;
	tp_set_state(tp, TS_WACK_CREQ);
	return n_conn_req(tp->sc, q, mp, &tp->rem, &tp->opt, 0 /* FIXME */ , mp->b_cont);
      baddata:
	err = TBADDATA;
	goto error;
      badopt:
...
tp_conn_res(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	bcopy(&tp->add, &tp->res, sizeof(tp->res));
	tp->RES_length = tp->ADDR_length;
	...
	if (unlikely(p->OPT_length < sizeof(struct t_opthdr) + sizeof(t_scalar_t)))
		goto badopt;
	if (unlikely((err = t_parse_opt(tp, mp->b_rptr + p->OPT_offset, p->OPT_length, T_CONN_RES))))
		goto error;
      noqos:
	if (mp->b_cont && unlikely(msgsize(mp->b_cont) > tp->CAPABILITY_ack.INFO_ack.CDATA_size))
		goto baddata;
	tp->state.ACCEPTOR_id = p->ACCEPTOR_id;
	tp_set_state(tp, TS_WACK_CRES);
	return n_conn_res(tp->sc, q, mp, res, &tp->opt, p->ACCEPTOR_id, 0 /* FIXME */ , p->SEQ_number,
			  mp->b_cont);
      baddata:
	err = TBADDATA;
	goto error;
...
tp_discon_req(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	if (mp->b_cont && unlikely(msgsize(mp->b_cont) > tp->CAPABILITY_ack.INFO_ack.DDATA_size))
		goto baddata;
...
tp_data_req(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	if (p->MORE_flag & ~(0x01))
		goto badflag;
	if (mp->b_cont && unlikely(msgsize(mp->b_cont) > tp->CAPABILITY_ack.INFO_ack.TIDU_size))
		goto baddata;
	if (mp->b_cont && unlikely(msgsize(mp->b_cont) > tp->CAPABILITY_ack.INFO_ack.TSDU_size))
		goto baddata;
...
tp_exdata_req(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	if (mp->b_cont && unlikely(msgsize(mp->b_cont) > tp->CAPABILITY_ack.INFO_ack.TIDU_size))
		goto baddata;
	if (mp->b_cont && unlikely(msgsize(mp->b_cont) > tp->CAPABILITY_ack.INFO_ack.ETSDU_size))
		goto baddata;
...
tp_bind_req(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	return n_bind_req(tp->sc, q, mp, add, p->CONIND_number, 0 /* FIXME */ );
      badaddr:
	err = TBADADDR;
	goto error;
      outstate:
	       ...
	       const char *location)
{
	...
	mi_strlog(q, level, flags, "ERROR: %s: %s: %s", location, sc_errname(error),
		  sc_strerror(error));
...
sc_discon_ind(struct sc *sc, queue_t *q, mblk_t *mp)
{
	...
	/* A wrinkle of the impeadance mismatch between NPI and TPI is that if we have not yet
	   issued the T_OK_ACK in response to a T_CONN_REQ and now receive an N_DISCON_IND, we need 
	   to issue the T_OK_ACK before issuing the T_DISCON_IND to meet the TPI user's
	   expectations. */
...
sc_info_ack(struct sc *sc, queue_t *q, mblk_t *mp)
{
	...
	tp->CAPABILITY_ack.INFO_ack.SERV_type = sc_t_serv_type(p->SERV_type);
	tp->CAPABILITY_ack.INFO_ack.CURRENT_state = sc_t_state(p->CURRENT_state);
	...
	if (p->QOS_length) {
		...
		sc->QOS_length = p->QOS_length;
		qos = &sc->qos;
		sc_t_options((N_qos_sccp_t *) qos, &tp->opt);
	}
	if (p->QOS_range_length) {
		...
		qor = &sc->qor;
		sc_t_options((N_qos_sccp_t *) qor, &tp->opt);
	}
	if (p->PROTOID_length) {
		np_ulong *pro;
...
sc_error_ack(struct sc *sc, queue_t *q, mblk_t *mp)
{
	...
	case NS_WCON_CREQ:
		...
		/* In this state we have already issued a T_OK_ACK back in response
		   ...
		   and issued upstream. */
		return t_discon_ind(sc->tp, q, mp, NULL, sc_t_error(error), 0, NULL);
	case NS_WACK_CRES:
		sc_set_state(sc, NS_WRES_CIND);
		break;
	case NS_WCON_RREQ:
	...
	return t_error_ack(sc->tp, q, mp, sc_t_prim(prim), sc_t_error(error));
}
...
sc_ok_ack(struct sc *sc, queue_t *q, mblk_t *mp)
{
	...
	return t_ok_ack(sc->tp, q, mp, sc_t_prim(prim));
}
...
sccp_qopen(queue_t *q, dev_t *devp, int oflags, int sflag, cred_t *credp)
{
	...
	priv->cred = *credp;
	priv->tp.oq = RD(q);
	priv->tp.state.state = TS_UNBND;
	priv->tp.state.datinds = 0;
	priv->tp.state.datacks = 0;
	priv->tp.state.coninds = 0;
	priv->tp.state.infinds = 0;
	priv->tp.CAPABILITY_ack.INFO_ack.PRIM_type = T_INFO_ACK;
	priv->tp.CAPABILITY_ack.INFO_ack.TSDU_size = QOS_UNKNOWN;
	priv->tp.CAPABILITY_ack.INFO_ack.ETSDU_size = QOS_UNKNOWN;
	priv->tp.CAPABILITY_ack.INFO_ack.CDATA_size = QOS_UNKNOWN;
	priv->tp.CAPABILITY_ack.INFO_ack.DDATA_size = QOS_UNKNOWN;
	priv->tp.CAPABILITY_ack.INFO_ack.ADDR_size = sizeof(struct sockaddr_sccp);
	priv->tp.CAPABILITY_ack.INFO_ack.OPT_size = QOS_UNKNOWN;
	priv->tp.CAPABILITY_ack.INFO_ack.TIDU_size = QOS_UNKNOWN;

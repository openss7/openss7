/*****************************************************************************
 ...
 @(#) $RCSfile: test-inet_tcp.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:25:15 $
...
#ident "@(#) $RCSfile: test-inet_tcp.c,v $ $Name:  $($Revision: 0.9 $) $Date: 2004/01/17 08:25:15 $"
...
unsigned short port1 = 10000, port2 = 10001, port3 = 10002;
...
int get_msg(int fd, int wait)
{
	...
	while ((ret = getmsg(fd, &ctrl, &data, &flags)) < 0) {
		if (ret != -1)
			printf("ERROR: getmsg: return value %d\n", ret);
		switch (errno) {
		default:
		case EPROTO:
		case EINVAL:
	...
	if (ret > 0) {
		printf("ERROR: getmsg: return value %d\n", ret);
		if (ret & (MORECTL | MOREDATA)) {
			if (ret & MORECTL)
				printf("ERROR: getmsg: our ctl buffers are too small.\n");
			if (ret & MOREDATA)
				printf("ERROR: getmsg: our dat buffers are too small.\n");
			return (FAILURE);
		}
	}
	if (!ret) {
			...
			if (pfd[0].revents & (POLLIN | POLLPRI)) {
				...
				if (ret > 0) {
					if (ret & (MORECTL | MOREDATA)) {
						if (ret & MORECTL)
							printf
							    ("ERROR: getmsg: our ctl buffers are too small.\n");
						if (ret & MOREDATA)
							printf
							    ("ERROR: getmsg: our dat buffers are too small.\n");
						return (FAILURE);
					}
				}
				printf("ERROR: getmsg: [%d] %s\n", errno, strerror(errno));
...
#define ACCEPTOR_offset ((char *)&cmd.tpi.conn_res.ACCEPTOR_id - (char *)&cmd.tpi.conn_res)
	return put_fdi(fd, fd2, ACCEPTOR_offset, 0);
}
...
int test_case_3(void)
{
	...
	for (;;) {
		...
		inet_discon_req(fd1, 0);
		expect(fd2, LONG_WAIT, T_DISCON_IND);
		return (FAILURE);
	}
}
...
int test_case_3b(void)
{
	...
	for (;;) {
		...
		case 2:
			...
			state = 3;
		case 3:
			if (expect(fd1, 60000, T_DISCON_IND) == FAILURE)
				break;
			state = 4;
		case 4:
...
accomplished in TCP protocol, however the interface should deliver\n\
the data after the connection is established."
...
#define desc_case_6c "\
Test Case 6(c):\n\
Connect and send partial data and expedited data.\n\
Expedited data should be delivered between normal data."
	...
	for (;;) {
		...
		case 0:
			...
			if (inet_optdata_req(fd1, T_MORE, "Hello There.", 0) == FAILURE)
				break;
			opt_data.sid_val = 0;
			if (inet_exdata_req(fd1, 0, "X") == FAILURE)
				break;
			opt_data.sid_val = 0;
			if (inet_optdata_req(fd1, T_MORE, "Hello There.", 0) == FAILURE)
				break;
...
#if 0
	, {
	desc_case_6c, &preamble_2, &test_case_6c, &postamble_3}
	, {

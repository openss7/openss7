/*****************************************************************************
 ...
 @(#) $RCSfile: test-inet_udp.c,v $ $Name:  $($Revision: 0.9.2.7 $) $Date: 2005/05/14 08:31:26 $
 ...
 Last Modified $Date: 2005/05/14 08:31:26 $ by $Author: brian $

 -----------------------------------------------------------------------------

 $Log: test-inet_udp.c,v $
 Revision 0.9.2.7  2005/05/14 08:31:26  brian
 ...
 Revision 0.9.2.3  2004/09/02 09:31:22  brian
...
#ident "@(#) $RCSfile: test-inet_udp.c,v $ $Name:  $($Revision: 0.9.2.7 $) $Date: 2005/05/14 08:31:26 $"
...
struct {
	...
	, T_NO, {
	sizeof(struct t_opthdr) + sizeof(t_uscalar_t), T_INET_SCTP, T_SCTP_ISTREAMS,
		    T_SUCCESS}
	, 1, {
	sizeof(struct t_opthdr) + sizeof(t_uscalar_t), T_INET_SCTP, T_SCTP_OSTREAMS,
		    T_SUCCESS}
, 1};
...
addr_string(char *add_ptr, size_t add_len)
{
	...
	if (add_len) {
		if (add_len != sizeof(*a))
			len +=
			    snprintf(buf + len, sizeof(buf) - len, "Aaarrg! add_len = %d, ",
				     add_len);
		len +=
		    snprintf(buf + len, sizeof(buf) - len, "%d.%d.%d.%d:%d",
			     (a->sin_addr.s_addr >> 0) & 0xff, (a->sin_addr.s_addr >> 8) & 0xff,
	...
	case T_INET_IP:
		...
		case T_IP_ADDR:
			if (oh->len == sizeof(*oh) + sizeof(uint32_t)) {
				...
				snprintf(buf, sizeof(buf), "%d.%d.%d.%d", (addr >> 0) & 0x00ff,
					 (addr >> 8) & 0x00ff, (addr >> 16) & 0x00ff,
					 (addr >> 24) & 0x00ff);
			}
			return buf;
		}
		...
		case T_TCP_MAXSEG:
			if (oh->len == sizeof(*oh) + sizeof(t_uscalar_t))
				snprintf(buf, sizeof(buf), "%lu",
					 (ulong)*((t_uscalar_t *) T_OPT_DATA(oh)));
			return buf;
		case T_TCP_KEEPALIVE:
			return yesno_string(oh);
		case T_TCP_CORK:
...
print_options(int fd, char *opt_ptr, size_t opt_len)
{
	...
	if (verbose < 4)
		return;
	for (oh = _T_OPT_FIRSTHDR_OFS(opt_ptr, opt_len, 0); oh;
	     oh = _T_OPT_NEXTHDR_OFS(opt_ptr, opt_len, oh, 0)) {
#if 0
...
print_event_conn(int fd, int event)
{
	...
	case __EVENT_OPTMGMT_REQ:
		...
		print_options(fd, cmd.cbuf + cmd.tpi.optmgmt_req.OPT_offset,
	...
	case __EVENT_OPTMGMT_ACK:
		...
		print_options(fd, cmd.cbuf + cmd.tpi.optmgmt_ack.OPT_offset,
	...
	case __EVENT_BIND_ACK:
		...
		if (verbose > 1) {
			fprintf(stdout,
				"%-20s|                               |  |                    \n",
				addr_string(cmd.cbuf + cmd.tpi.bind_ack.ADDR_offset,
	...
	case __EVENT_ADDR_ACK:
		...
		if (verbose > 1) {
			fprintf(stdout,
				"%-20s|                               |  |                    \n",
				addr_string(cmd.cbuf + cmd.tpi.addr_ack.LOCADDR_offset,
					    cmd.tpi.addr_ack.LOCADDR_length));
			fprintf(stdout,
				"%-20s|                               |  |                    \n",
				addr_string(cmd.cbuf + cmd.tpi.addr_ack.REMADDR_offset,
	...
	case __EVENT_UNITDATA_REQ:
		...
		if (verbose > 1) {
			fprintf(stdout,
				"%-20s|                               |  |                    \n",
				addr_string(cmd.cbuf + cmd.tpi.unitdata_req.DEST_offset,
					    cmd.tpi.unitdata_req.DEST_length));
			print_options(fd, cmd.cbuf + cmd.tpi.unitdata_req.OPT_offset,
	...
	case __EVENT_UNITDATA_IND:
		...
		if (verbose > 1) {
			fprintf(stdout,
				"%-20s|                               |  |                    \n",
				addr_string(cmd.cbuf + cmd.tpi.unitdata_ind.SRC_offset,
					    cmd.tpi.unitdata_ind.SRC_length));
			print_options(fd, cmd.cbuf + cmd.tpi.unitdata_ind.OPT_offset,
	...
	case __EVENT_CONN_REQ:
		fprintf(stdout,
			"T_CONN_REQ    ----->| - - - - - - ->\\               |  |                    [%d]\n",
			state);
		print_options(fd, cmd.cbuf + cmd.tpi.conn_req.OPT_offset,
	...
	case __EVENT_CONN_IND:
		fprintf(stdout,
			"T_CONN_IND    <-----|<- - - - - - - /               |  |                    [%d]\n",
			state);
		print_options(fd, cmd.cbuf + cmd.tpi.conn_ind.OPT_offset,
	...
	case __EVENT_CONN_RES:
		fprintf(stdout,
			"T_CONN_RES    ----->| - - - - - - ->\\               |  |                    [%d]\n",
			state);
		print_options(fd, cmd.cbuf + cmd.tpi.conn_res.OPT_offset,
	...
	case __EVENT_CONN_CON:
		fprintf(stdout,
			"T_CONN_CON    <-----|<- - - - - - - /               |  |                    [%d]\n",
			state);
		print_options(fd, cmd.cbuf + cmd.tpi.conn_con.OPT_offset,
	...
	case __EVENT_OPTDATA_REQ:
		...
		print_options(fd, cmd.cbuf + cmd.tpi.optdata_req.OPT_offset,
	...
	case __EVENT_OPTDATA_IND:
		if (cmd.tpi.optdata_ind.DATA_flag & T_ODF_EX) {
			...
			if (cmd.tpi.optdata_ind.DATA_flag & T_ODF_MORE)
				fprintf(stdout,
					"T_OPTDATA_IND+<-----|<- -(%03lu:%03lu)- / [%010lu]  |  |                    [%d]\n",
					(ulong) ((rdat_opt_t *) (cmd.cbuf +
								 cmd.tpi.optdata_ind.OPT_offset))->
					sid_val,
					(ulong) ((rdat_opt_t *) (cmd.cbuf +
		...
		print_options(fd, cmd.cbuf + cmd.tpi.optdata_ind.OPT_offset,
...
print_event_resp(int fd, int event)
{
	...
	case __EVENT_OPTDATA_REQ:
		if (cmd.tpi.optdata_req.DATA_flag & T_ODF_EX)
			fprintf(stdout,
				"                    |               /<- - (%03lu:-U-) + -|<--- T_OPTDATA_REQ  [%d]\n",
				(ulong)opt_data.sid_val, state);
		else
			fprintf(stdout,
				"                    |               /<- - (%03lu:---) + -|<--- T_OPTDATA_REQ  [%d]\n",
				(ulong)opt_data.sid_val, state);
		print_options(fd, cmd.cbuf + cmd.tpi.optdata_req.OPT_offset,
			      cmd.tpi.optdata_req.OPT_length);
		break;
	case __EVENT_OPTDATA_IND:
		if (cmd.tpi.optdata_ind.DATA_flag & T_ODF_EX) {
			...
			if (cmd.tpi.optdata_ind.DATA_flag & T_ODF_MORE)
				fprintf(stdout,
					"                    |  [%010lu] \\ - - (%03lu:%03lu) +->|---> T_OPTDATA_IND  [%d]\n",
					(ulong) ((rdat_opt_t *) (cmd.cbuf +
								 cmd.tpi.optdata_ind.OPT_offset))->
					tsn_val,
					(ulong) ((rdat_opt_t *) (cmd.cbuf +
	...
	if (side->preamble && side->preamble(resp_fd) != __RESULT_SUCCESS) {
		...
		case __RESULT_FAILURE:
			if (verbose > 0) {
				lockf(fileno(stdout), F_LOCK, 0);
				fprintf(stdout,
					"                    |XXXXXXXXXXXX FAILED XXXXXXXXXXX|  |XXXXXXXXXXXXXXXXXXX(%d)\n",
					state);
				fflush(stdout);
				lockf(fileno(stdout), F_ULOCK, 0);
			}
	...
	if (side->preamble && side->preamble(list_fd) != __RESULT_SUCCESS) {
		...
		case __RESULT_FAILURE:
			if (verbose > 0) {
				lockf(fileno(stdout), F_LOCK, 0);
				fprintf(stdout,
					"                    |XXXXXXXXXXXX FAILED XXXXXXXXXXX|XX|XXXXXXXXXXXXXXXXXXX(%d)\n",
					state);
				fflush(stdout);
				lockf(fileno(stdout), F_ULOCK, 0);
			}
...
test_run(struct test_side *conn_side, struct test_side *resp_side, struct test_side *list_side)
{
	...
	int got_stat, conn_stat = __RESULT_FAILURE, resp_stat = __RESULT_FAILURE, list_stat = __RESULT_FAILURE;
	start_tt(5000);
	if (conn_side) {
	...
	stop_tt();
	if (conn_stat == __RESULT_FAILURE || resp_stat == __RESULT_FAILURE
	    || list_stat == __RESULT_FAILURE)
		return (__RESULT_FAILURE);
	if (conn_stat == __RESULT_SUCCESS && resp_stat == __RESULT_SUCCESS
	    && list_stat == __RESULT_SUCCESS)
		return (__RESULT_SUCCESS);

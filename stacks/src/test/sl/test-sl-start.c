/*****************************************************************************
 ...
 @(#) $RCSfile: test-sl-start.c,v $ $Name:  $($Revision: 0.8 $) $Date: 2002/10/25 17:45:52 $
 ...
 Last Modified $Date: 2002/10/25 17:45:52 $ by <bidulock@openss7.org>

 *****************************************************************************/

#ident "@(#) $RCSfile: test-sl-start.c,v $ $Name:  $($Revision: 0.8 $) $Date: 2002/10/25 17:45:52 $"
...
static int mymain(int argc, char **argv)
{
	...
	printf("Clearing mux links.\n");
	fflush(stdout);
	if ((ret = ioctl(mux, I_PUNLINK, MUXID_ALL)) < 0) {
		perror(argv[0]);
		exit(1);
	}
	...
	printf("Linking device.\n");
	fflush(stdout);
	if ((ret = ioctl(mux, I_PLINK, fd)) < 0) {
		perror(argv[0]);
		exit(1);
	}
	printf("Muxed under: index = %d\n", ret);
	fd = mux;
	ctrl.maxlen = BUFSIZE;

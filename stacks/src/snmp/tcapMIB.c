/*****************************************************************************
 ...
 @(#) $RCSfile: tcapMIB.c,v $ $Name:  $($Revision: 0.9.2.7 $) $Date: 2009-01-04 13:16:06 $
...
#ident "@(#) $RCSfile: tcapMIB.c,v $ $Name:  $($Revision: 0.9.2.7 $) $Date: 2009-01-04 13:16:06 $"

static char const ident[] = "$RCSfile: tcapMIB.c,v $ $Name:  $($Revision: 0.9.2.7 $) $Date: 2009-01-04 13:16:06 $";
 ...
 */
oid tcapMIB_variables_oid[] = { 1, 3, 6, 1, 4, 1, 29591, 17, 751, 4 };
...
init_tcapMIB(void)
{
	...
	DEBUGMSGTL(("tcapMIB", "initializing...  "));
...
#if defined MODULE
	if (sa_readfd != 0) {
		register_readfd(sa_readfd, tcapMIB_fd_handler, (void *) 0);
		register_exceptfd(sa_readfd, tcapMIB_fd_handler, (void *) 1);
	}
#if defined MASTER
	tcapMIBold_signal_handler = external_signal_handler[0];
	external_signal_handler[0] = &tcapMIB_loop_handler;
...
deinit_tcapMIB(void)
{
	DEBUGMSGTL(("tcapMIB", "deinitializating...  "));
#if defined MODULE
#if defined MASTER
	external_signal_handler[0] = tcapMIBold_signal_handler;
#endif				/* defined MASTER */
	if (sa_readfd != 0) {
...
tcapMIB_create(void)
{
	...
	DEBUGMSGTL(("tcapMIB", "creating scalars...  "));
	if (StorageNew != NULL) {
...
tcapMIB_destroy(struct tcapMIB_data **thedata)
{
	...
	DEBUGMSGTL(("tcapMIB", "deleting scalars...  "));
...
{
	DEBUGMSGTL(("tcapMIB", "adding data...  "));
...
parse_tcapMIB(const char *token, char *line)
{
	...
	DEBUGMSGTL(("tcapMIB", "parsing config...  "));
	if (StorageTmp == NULL) {
...
store_tcapMIB(int majorID, int minorID, void *serverarg, void *clientarg)
{
	...
	DEBUGMSGTL(("tcapMIB", "storing data...  "));
	refresh_tcapMIB();
 ...
 */
void
tcapMIB_loop_handler(int dummy)
{
...
#endif				/* defined MASTER */
/**
 * @fn void tcapMIB_readfd_handler(int fd, void *dummy)
 ...
 */
void
tcapMIB_readfd_handler(int fd, void *dummy)
{
...
char sa_logfile[256] = "/var/log/tcapmib.log";
char sa_pidfile[256] = "/var/run/tcapmib.pid";
char sa_sysctlf[256] = "/etc/tcapmib.conf";
...
Options:\n\
    ...
    -l, --log-file [LOGFILE]\n\
        log to log file name LOGFILE.  [default: /var/log/tcapmib.log]\n\
    -L, --log-stderr\n\
    ...
    -n, --nodaemon\n\
        run in the foreground.\n\
    -n, --name NAME\n\
        use NAME for configuration file base.  [default: tcapmib]\n\
    -p, --port PORTNUM\n\
        listen on port number PORTNUM.  [default: 161]\n\
    -p, --pidfile PIDFILE\n\
        write daemon pid to PIDFILE.  [default: /var/run/tcapmib.pid]\n\
    -P, --pidfile PIDFILE\n\
        write daemon pid to PIDFILE.  [default: /var/run/tcapmib.pid]\n\
    -q, --quiet\n\
...
sa_init_logging(int argc, char *argv[])
{
	...
	if (sa_logsyslog) {
		...
		snmp_enable_syslog_ident("tcapMIB", LOG_DAEMON);
		...
		closelog();
#if defined LOG_PERROR
		openlog("tcapMIB", LOG_PID | LOG_CONS | LOG_NDELAY | (sa_logstderr ? LOG_PERROR : 0), MY_FACILITY(0));
#else				/* defined LOG_PERROR */
		openlog("tcapMIB", LOG_PID | LOG_CONS | LOG_NDELAY, MY_FACILITY(0));
#endif				/* defined LOG_PERROR */
	}
	if (sa_logcallog) {
		snmp_enable_calllog();

/*****************************************************************************
 ...
 @(#) $RCSfile: cdr2MIB.c,v $ $Name:  $($Revision: 0.9.2.8 $) $Date: 2009-01-10 17:09:12 $
...
#ident "@(#) $RCSfile: cdr2MIB.c,v $ $Name:  $($Revision: 0.9.2.8 $) $Date: 2009-01-10 17:09:12 $"

static char const ident[] = "$RCSfile: cdr2MIB.c,v $ $Name:  $($Revision: 0.9.2.8 $) $Date: 2009-01-10 17:09:12 $";
...
#define   CALLDETAILDATAID      1
	{(u_char) CALLDETAILDATAID, ASN_OCTET_STR, RONLY, var_callDetailDataTable, 5, {1, 2, 1, 1, 1}},
...
callDetailLogRecordR2Table_create(void)
{
	...
	if (StorageNew != NULL) {
		...
		StorageNew->sequenceId = 0;
		StorageNew->zoneId = 0;
		StorageNew->recSeqId = 0;
		StorageNew->fileSeqId = 0;
...
{
	if (!StorageTmp || StorageTmp->callDetailDataTable_request == sa_request)
		return;
...
var_callDetailDataTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	*var_len = 0;
	...
	/* This is where we do the value assignments for the mib results. */
	switch (vp->magic) {
	case (u_char) CALLDETAILDATAID:	/* ReadOnly */
		if (!StorageTmp)
			break;
		*var_len = StorageTmp->callDetailDataIdLen;
		rval = (u_char *) StorageTmp->callDetailDataId;
		break;
	default:
		ERROR_MSG("");
	}
...
{
	if (!StorageTmp || StorageTmp->simpleUsageMeteringControlTable_request == sa_request)
		return;
...
{
	if (!StorageTmp || StorageTmp->configurableSimpleUsageMeteringControlTable_request == sa_request)
		return;
...
{
	if (!StorageTmp || StorageTmp->blockGeneratingLogTable_request == sa_request)
		return;
...
{
	if (!StorageTmp || StorageTmp->fileGeneratingLogTable_request == sa_request)
		return;
...
{
	if (!StorageTmp || StorageTmp->callDetailLogRecordTable_request == sa_request)
		return;
...
{
	if (!StorageTmp || StorageTmp->configurableSimpleUsageMeteringControlR2Table_request == sa_request)
		return;
...
{
	if (!StorageTmp || StorageTmp->blockGeneratingLogR2Table_request == sa_request)
		return;
...
{
	if (!StorageTmp || StorageTmp->callDetailLogRecordR2Table_request == sa_request)
		return;

/*****************************************************************************
 ...
 @(#) $RCSfile: ss7MIB.c,v $ $Name:  $($Revision: 0.9.2.11 $) $Date: 2009-01-16 20:45:01 $
...
#ident "@(#) $RCSfile: ss7MIB.c,v $ $Name:  $($Revision: 0.9.2.11 $) $Date: 2009-01-16 20:45:01 $"

static char const ident[] = "$RCSfile: ss7MIB.c,v $ $Name:  $($Revision: 0.9.2.11 $) $Date: 2009-01-16 20:45:01 $";
...
#if defined MODULE
	if (sa_readfd != 0) {
		register_readfd(sa_readfd, ss7MIB_fd_handler, (void *) 0);
		register_exceptfd(sa_readfd, ss7MIB_fd_handler, (void *) 1);
	}
#if defined MASTER
...
ss7MIB_create(void)
{
	...
	DEBUGMSGTL(("ss7MIB", "creating scalars...  "));
	if (StorageNew != NULL) {
...
ss7MIB_destroy(struct ss7MIB_data **thedata)
{
	...
	DEBUGMSGTL(("ss7MIB", "deleting scalars...  "));
...
ss7NetworkTable_create(void)
{
	...
	DEBUGMSGTL(("ss7NetworkTable", "creating row...  "));
	if (StorageNew != NULL) {
...
ss7NetworkTable_duplicate(struct ss7NetworkTable_data *thedata)
{
	...
	DEBUGMSGTL(("ss7NetworkTable", "duplicating row...  "));
	if (StorageNew != NULL) {
	}
      done:
	DEBUGMSGTL(("ss7NetworkTable", "done.\n"));
...
ss7NetworkTable_destroy(struct ss7NetworkTable_data **thedata)
{
	...
	DEBUGMSGTL(("ss7NetworkTable", "deleting row...  "));
	if ((StorageDel = *thedata) != NULL) {
...
var_ss7NetworkTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	StorageTmp = header_complex(ss7NetworkTableStorage, vp, name, length, exact, var_len, write_method);
	refresh_ss7NetworkTable_row(StorageTmp, 0);
	*write_method = NULL;
	*var_len = 0;
	rval = NULL;
	/* This is where we do the value assignments for the mib results. */

...
static const char sa_program[] = "ss7mib";
...
static char sa_logfile[256] = "/var/log/ss7mib.log";
static char sa_pidfile[256] = "/var/run/ss7mib.pid";
static char sa_sysctlf[256] = "/etc/ss7mib.conf";
...
static volatile int ss7MIB_refresh = 1;
static volatile int ss7NetworkTable_refresh = 1;
...
#define   SS7NETWORKID          3
	{SS7NETWORKID, ASN_INTEGER, RWRITE, var_ss7NetworkTable, 5, {1, 2, 1, 1, 1}},
#define   SS7PROTOCOLVARIANT    4
	{SS7PROTOCOLVARIANT, ASN_OBJECT_ID, RWRITE, var_ss7NetworkTable, 5, {1, 2, 1, 1, 2}},
#define   SS7PROTOCOLVERSION    5
	{SS7PROTOCOLVERSION, ASN_INTEGER, RWRITE, var_ss7NetworkTable, 5, {1, 2, 1, 1, 3}},
#define   SS7NETWORKROWSTATUS   6
	{SS7NETWORKROWSTATUS, ASN_INTEGER, RWRITE, var_ss7NetworkTable, 5, {1, 2, 1, 1, 4}},

};

/*    (L = length of the oidsuffix) */

static struct ss7MIB_data *ss7MIBStorage = NULL;

/* global storage of our data, saved in and configured by header_complex() */
static struct header_complex_index *ss7NetworkTableStorage = NULL;
...
refresh_ss7MIB(void)
{
	if (ss7MIBStorage == NULL) {
		if ((ss7MIBStorage = SNMP_MALLOC_STRUCT(ss7MIB_data)) == NULL)
			return;
...
parse_ss7NetworkTable(const char *token, char *line)
{
	...
	line = read_config_read_data(ASN_INTEGER, line, &StorageTmp->ss7NetworkId, &tmpint);
	line = read_config_read_data(ASN_OBJECT_ID, line, &StorageTmp->ss7ProtocolVariant, &StorageTmp->ss7ProtocolVariantLen);
	if (StorageTmp->ss7ProtocolVariant == NULL) {
		config_perror("invalid specification for ss7ProtocolVariant");
		return;
	}

	line = read_config_read_data(ASN_INTEGER, line, &StorageTmp->ss7ProtocolVersion, &tmpint);
	line = read_config_read_data(ASN_INTEGER, line, &StorageTmp->ss7NetworkRowStatus, &tmpint);
...
/*   XXX:  if (StorageTmp->ss7NetworkTableStorageType == ST_NONVOLATILE) { */
		...
		cptr = read_config_store_data(ASN_INTEGER, cptr, &StorageTmp->ss7NetworkId, &tmpint);
		cptr = read_config_store_data(ASN_OBJECT_ID, cptr, &StorageTmp->ss7ProtocolVariant, &StorageTmp->ss7ProtocolVariantLen);
		cptr = read_config_store_data(ASN_INTEGER, cptr, &StorageTmp->ss7ProtocolVersion, &tmpint);
		cptr = read_config_store_data(ASN_INTEGER, cptr, &StorageTmp->ss7NetworkRowStatus, &tmpint);
...
var_ss7NetworkTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	case SS7NETWORKID:
		*write_method = write_ss7NetworkId;
		*var_len = sizeof(StorageTmp->ss7NetworkId);
		return (u_char *) &StorageTmp->ss7NetworkId;

	case SS7PROTOCOLVARIANT:
		*write_method = write_ss7ProtocolVariant;
		*var_len = StorageTmp->ss7ProtocolVariantLen;
		return (u_char *) StorageTmp->ss7ProtocolVariant;

	case SS7PROTOCOLVERSION:
		*write_method = write_ss7ProtocolVersion;
		*var_len = sizeof(StorageTmp->ss7ProtocolVersion);
		return (u_char *) &StorageTmp->ss7ProtocolVersion;
...
int
write_ss7NetworkId(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid * name, size_t name_len)
{
	static long tmpvar;
	struct ss7NetworkTable_data *StorageTmp = NULL;
	...
	DEBUGMSGTL(("ss7MIB", "write_ss7NetworkId entering action=%d...  \n", action));
	...
	case RESERVE1:
		if (var_val_type != ASN_INTEGER) {
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to ss7NetworkId not ASN_INTEGER\n");
	...
	case ACTION:
		/* The variable has been stored in long_ret for you to use, and you have just been
		   ...
		   in the UNDO case */
		tmpvar = StorageTmp->ss7NetworkId;
		StorageTmp->ss7NetworkId = *((long *) var_val);
		break;

	case UNDO:
		/* Back out any changes made in the ACTION case */
		StorageTmp->ss7NetworkId = tmpvar;
		break;
...
int
write_ss7ProtocolVariant(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid * name, size_t name_len)
{
	static oid *tmpvar;
	struct ss7NetworkTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	(void) tmplen;		/* not always used */
	DEBUGMSGTL(("ss7MIB", "write_ss7ProtocolVariant entering action=%d...  \n", action));
	if ((StorageTmp = header_complex(ss7NetworkTableStorage, NULL, &name[sizeof(ss7MIB_variables_oid) / sizeof(oid) + 6 - 1], &newlen, 1, NULL, NULL)) == NULL)
		return SNMP_ERR_NOSUCHNAME;	/* remove if you support creation here */
	...
	case RESERVE1:
		if (var_val_type != ASN_OBJECT_ID) {
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to ss7ProtocolVariant not ASN_OBJECT_ID\n");
			return SNMP_ERR_WRONGTYPE;
		}
		break;
	...
	case ACTION:
		/* The variable has been stored in objid for you to use, and you have just been
		   asked to do something with it.  Note that anything done here must be reversable
		   in the UNDO case */
		tmpvar = StorageTmp->ss7ProtocolVariant;
		tmplen = StorageTmp->ss7ProtocolVariantLen;
		memdup((void *) &StorageTmp->ss7ProtocolVariant, var_val, var_val_len);
		StorageTmp->ss7ProtocolVariantLen = var_val_len / sizeof(oid);
		break;

	case UNDO:
		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->ss7ProtocolVariant);
		StorageTmp->ss7ProtocolVariant = tmpvar;
		StorageTmp->ss7ProtocolVariantLen = tmplen;
		break;
...
int
write_ss7ProtocolVersion(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid * name, size_t name_len)
{
	static long tmpvar;
	struct ss7NetworkTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	(void) tmplen;		/* not always used */
	DEBUGMSGTL(("ss7MIB", "write_ss7ProtocolVersion entering action=%d...  \n", action));
	if ((StorageTmp = header_complex(ss7NetworkTableStorage, NULL, &name[sizeof(ss7MIB_variables_oid) / sizeof(oid) + 6 - 1], &newlen, 1, NULL, NULL)) == NULL)
		return SNMP_ERR_NOSUCHNAME;	/* remove if you support creation here */
	...
	case RESERVE1:
		if (var_val_type != ASN_INTEGER) {
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to ss7ProtocolVersion not ASN_INTEGER\n");
			return SNMP_ERR_WRONGTYPE;
		}
		break;
	...
	case ACTION:
		/* The variable has been stored in long_ret for you to use, and you have just been
		   asked to do something with it.  Note that anything done here must be reversable
		   in the UNDO case */
		tmpvar = StorageTmp->ss7ProtocolVersion;
		StorageTmp->ss7ProtocolVersion = *((long *) var_val);
		break;

	case UNDO:
		/* Back out any changes made in the ACTION case */
		StorageTmp->ss7ProtocolVersion = tmpvar;
		break;

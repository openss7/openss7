/*****************************************************************************
 ...
 @(#) $RCSfile: ss7MIB.c,v $ $Name:  $($Revision: 0.9.2.3 $) $Date: 2008-12-27 15:29:48 $
...
#ident "@(#) $RCSfile: ss7MIB.c,v $ $Name:  $($Revision: 0.9.2.3 $) $Date: 2008-12-27 15:29:48 $"

static char const ident[] = "$RCSfile: ss7MIB.c,v $ $Name:  $($Revision: 0.9.2.3 $) $Date: 2008-12-27 15:29:48 $";
...
/*
 * variable7 ss7MIB_variables:
 *   this variable defines function callbacks and type return information
 *   for the ss7MIB mib section
 */
...
ss7NetworkTable_add(struct ss7NetworkTable_data *thedata)
{
	...
	/* add the index variables to the varbind list, which is used by header_complex to index
	   the data */
	/* ss7NetworkId */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_INTEGER, (uint8_t *) &thedata->ss7NetworkId, sizeof(thedata->ss7NetworkId));
 ...
 *   The workings of this are basically the same as for var_ss7MIB above.
 */
uint8_t *
var_ss7NetworkTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	case SS7NETWORKPROTOCOLVARIANT:
		...
		*var_len = StorageTmp->ss7NetworkProtocolVariantLen;
		return (uint8_t *) StorageTmp->ss7NetworkProtocolVariant;
	case SS7NETWORKPROTOCOLVERSION:
		...
		*var_len = sizeof(StorageTmp->ss7NetworkProtocolVersion);
		return (uint8_t *) &StorageTmp->ss7NetworkProtocolVersion;
...
int
write_ss7NetworkProtocolVariant(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static oid *tmpvar;
	struct ss7NetworkTable_data *StorageTmp = NULL;
	...
	case ACTION:		/* The variable has been stored in objid for you to use, and you
		...
		tmpvar = StorageTmp->ss7NetworkProtocolVariant;
		tmplen = StorageTmp->ss7NetworkProtocolVariantLen;
		memdup((void *) &StorageTmp->ss7NetworkProtocolVariant, var_val, var_val_len);
		StorageTmp->ss7NetworkProtocolVariantLen = var_val_len / sizeof(oid);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->ss7NetworkProtocolVariant);
		StorageTmp->ss7NetworkProtocolVariant = tmpvar;
		StorageTmp->ss7NetworkProtocolVariantLen = tmplen;
		break;
...
int
write_ss7NetworkProtocolVersion(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and
				   ...
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->ss7NetworkProtocolVersion;
		StorageTmp->ss7NetworkProtocolVersion = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->ss7NetworkProtocolVersion = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_ss7NetworkProtocolOptions(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct ss7NetworkTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in string for you to use, and you
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->ss7NetworkProtocolOptions;
		tmplen = StorageTmp->ss7NetworkProtocolOptionsLen;
		memdup((void *) &StorageTmp->ss7NetworkProtocolOptions, var_val, var_val_len);
		StorageTmp->ss7NetworkProtocolOptionsLen = var_val_len;
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->ss7NetworkProtocolOptions);
		StorageTmp->ss7NetworkProtocolOptions = tmpvar;
		StorageTmp->ss7NetworkProtocolOptionsLen = tmplen;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
		SNMP_FREE(tmpvar);
...
int
write_ss7NetworkRowStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	struct ss7NetworkTable_data *StorageTmp = NULL;
	...
	case RESERVE2:
		...
		if (StorageTmp == NULL) {
			/* creation */
			vars = NULL;
			snmp_varlist_add_variable(&vars, NULL, 0, ASN_INTEGER, NULL, 0);	/* ss7NetworkId 
												 */
	...
	case ACTION:
		...
		if (StorageTmp == NULL) {
			...
			StorageTmp->ss7NetworkRowStatus = *((int *) var_val);
		} else {
	...
	case COMMIT:
		...
		if (StorageDel != NULL) {
			...
			if (StorageTmp && StorageTmp->ss7NetworkRowStatus == RS_CREATEANDGO) {
				StorageTmp->ss7NetworkRowStatus = RS_ACTIVE;
			} else if (StorageTmp && StorageTmp->ss7NetworkRowStatus == RS_CREATEANDWAIT) {

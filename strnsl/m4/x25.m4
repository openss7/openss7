...
# =============================================================================
# 
# @(#) $RCSfile: x25.m4,v $ $Name:  $($Revision: 0.9.2.5 $) $Date: 2008/05/03 17:14:19 $
#
# -----------------------------------------------------------------------------
#
...
# -----------------------------------------------------------------------------
#
# Last Modified $Date: 2008/05/03 17:14:19 $ by $Author: brian $
#
# =============================================================================
...
*** ])
	...
	if test :"${x25_cv_includes:-no}" = :no ; then
		...
		if test -r "$x25_dir/$x25_what" ; then
		    ...
		    x25_cv_ldadd="$os7_cv_master_builddir/strx25/libsx25.la"
		    x25_cv_ldadd32="$os7_cv_master_builddir/strx25/lib32/libsx25.la"
	...
	if test :"${x25_cv_includes:-no}" = :no ; then
		...
		if test -d "$x25_dir" ; then
			...
			x25_cv_includes="$x25_inc $x25_bld $x25_dir"
			x25_cv_ldadd=`echo "$x25_bld/../../libsx25.la" |sed -e 's|/[[^/]][[^/]]*/\.\./|/|g;s|/[[^/]][[^/]]*/\.\./|/|g;s|/\./|/|g;s|//|/|g'`
			x25_cv_ldadd32=`echo "$x25_bld/../../lib32/libsx25.la" |sed -e 's|/[[^/]][[^/]]*/\.\./|/|g;s|/[[^/]][[^/]]*/\.\./|/|g;s|/\./|/|g;s|//|/|g'`
	...
	for x25_dir in $x25_cv_includes ; do
		...
		x25_cv_ldadd=`echo "$x25_dir/../../$x25_what" | sed -e 's|/[[^/]][[^/]]*/\.\./|/|g;s|/[[^/]][[^/]]*/\.\./|/|g;s|/\./|/|g;s|//|/|g'`
		break
	    fi
	done
	...
	x25_cv_ldadd32=
	for x25_dir in $x25_cv_includes ; do
	    if test -f "$x25_dir/../../lib32/$x25_what" ; then
		x25_cv_ldadd32=`echo "$x25_dir/../../lib32/$x25_what" | sed -e 's|/[[^/]][[^/]]*/\.\./|/|g;s|/[[^/]][[^/]]*/\.\./|/|g;s|/\./|/|g;s|//|/|g'`
		break
	    fi
	done
...
#
# $Log: x25.m4,v $
# Revision 0.9.2.5  2008/05/03 17:14:19  brian
# - correct heade file for search
#
# Revision 0.9.2.4  2008/05/03 16:54:39  brian
# - updates for libosi
#
# Revision 0.9.2.3  2008/05/03 16:47:13  brian
# - more x25 macro corrections
#
# Revision 0.9.2.2  2008/05/03 16:13:45  brian
# - add libsx25 library
#

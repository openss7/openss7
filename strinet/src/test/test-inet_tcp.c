/*****************************************************************************
 ...
 @(#) $RCSfile: test-inet_tcp.c,v $ $Name:  $($Revision: 0.9.2.19 $) $Date: 2005/06/10 12:21:13 $
 ...
 Last Modified $Date: 2005/06/10 12:21:13 $ by $Author: brian $

 -----------------------------------------------------------------------------

 $Log: test-inet_tcp.c,v $
 Revision 0.9.2.19  2005/06/10 12:21:13  brian
 - some final option tweaking

 Revision 0.9.2.18  2005/06/10 04:03:12  brian
 - more options corrections

 Revision 0.9.2.17  2005/06/09 23:19:44  brian
 - improved options processing

 Revision 0.9.2.16  2005/06/08 09:01:19  brian
 - corrected options processing

 Revision 0.9.2.15  2005/06/08 06:08:25  brian
 ...
 Revision 0.9.2.12  2005/06/07 08:10:45  brian
...
#ident "@(#) $RCSfile: test-inet_tcp.c,v $ $Name:  $($Revision: 0.9.2.19 $) $Date: 2005/06/10 12:21:13 $"

static char const ident[] = "$RCSfile: test-inet_tcp.c,v $ $Name:  $($Revision: 0.9.2.19 $) $Date: 2005/06/10 12:21:13 $";
...
int test_case_1_2(int child)
{
	...
	if (p->info_ack.PRIM_type != T_INFO_ACK)
		goto failure;
	state++;
	if (p->info_ack.TSDU_size != 0)
		goto failure;
	state++;
	if (p->info_ack.ETSDU_size != 1)
		goto failure;
	state++;
	...
	if (p->info_ack.TIDU_size != 65535)
		goto failure;
	state++;
	if (p->info_ack.SERV_type != T_COTS_ORD)
		goto failure;
	state++;
...
int test_case_1_3(int child)
{
	...
	if (p->capability_ack.INFO_ack.PRIM_type != T_INFO_ACK)
		goto failure;
	state++;
	if (p->capability_ack.INFO_ack.TSDU_size != 0)
		goto failure;
	state++;
	if (p->capability_ack.INFO_ack.ETSDU_size != 1)
		goto failure;
	state++;
	...
	if (p->capability_ack.INFO_ack.TIDU_size != 65535)
		goto failure;
	state++;
	if (p->capability_ack.INFO_ack.SERV_type != T_COTS_ORD)
		goto failure;
	state++;
...
struct test_case {
...
} tests[] = {
	{
		...
		numb_case_5_4, tgrp_case_5_4, name_case_5_4, desc_case_5_4, sref_case_5_4, { &test_5_4_conn, &test_5_4_resp, &test_5_4_list }, 0, 0}, {

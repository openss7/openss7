...
# =============================================================================
# 
# @(#) $RCSfile: voip.m4,v $ $Name:  $($Revision: 1.1.2.2 $) $Date: 2009-07-21 11:06:13 $
#
# -----------------------------------------------------------------------------
#
...
AC_DEFUN([_VOIP_OPTIONS], [dnl
	...
	[AS_HELP_STRING([--with-voip=HEADERS],
	    [VOIP header directory @<:@default=INCLUDEDIR/strvoip@:>@])],
	[for s in ${!voip_cv_*} ; do eval "unset $s" ; done])
])# _VOIP_OPTIONS
# =============================================================================
...
*** ])
	...
	if test :"${voip_cv_includes:-no}" = :no ; then
	    ...
	    voip_search_path=`echo "$voip_search_path" | sed -e 's|\<NONE\>||g;s|//|/|g' | awk '{if(!([$]0 in seen)){print[$]0;seen[[$ 0]]=1}}'`
	    voip_cv_includes=
	...
	if test -z "$voip_cv_ldadd" ; then
		...
		${DESTDIR}${libdir}\""
	    voip_search_path=`echo "$voip_search_path" | sed -e 's|\<NONE\>|'$ac_default_prefix'|g;s|//|/|g' | awk '{if(!([$]0 in seen)){print[$]0;seen[[$ 0]]=1}}'`
	    AC_MSG_RESULT([(searching)])
	    for voip_dir in $voip_search_path ; do
		if test -d "$voip_dir" ; then

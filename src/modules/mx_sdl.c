/*****************************************************************************
 ...
 @(#) $RCSfile: mx_sdl.c,v $ $Name:  $($Revision: 1.1.2.2 $) $Date: 2010-11-28 14:22:04 $
...
static char const ident[] = "$RCSfile: mx_sdl.c,v $ $Name:  $($Revision: 1.1.2.2 $) $Date: 2010-11-28 14:22:04 $";
...
#define MX_SDL_REVISION	"LfS $RCSfile: mx_sdl.c,v $ $Name:  $($Revision: 1.1.2.2 $) $Date: 2010-11-28 14:22:04 $"
#define MX_SDL_COPYRIGHT	"Copyright (c) 2008-2010  Monavacon Limited.  All Rights Reserved."
...
STATIC struct streamtab mx_sdlinfo = {
 ...
 */
STATIC INLINE int
mx_optmgmt_ack(queue_t *q, uchar *opt_ptr, size_t opt_len, ulong flags)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
 ...
 */
STATIC INLINE int
mx_error_ack(queue_t *q, ulong prim, long error)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
 ...
 */
STATIC INLINE int
mx_connect_con(queue_t *q, ulong flags, ulong slot)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
 ...
 */
STATIC INLINE int
mx_disconnect_ind(queue_t *q, ulong flags, ulong slot)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
 ...
 */
STATIC INLINE int
mx_disconnect_con(queue_t *q, ulong flags, ulong slot)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
 ...
 */
STATIC INLINE int
mx_disable_ind(queue_t *q, long cause)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
 ...
 */
STATIC INLINE int
lmi_attach_req(queue_t *q, uchar *ppa_ptr, size_t ppa_len)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
	lmi_attach_req_t *p;
 ...
 */
STATIC INLINE int
lmi_optmgmt_req(queue_t *q, uchar *opt_ptr, size_t opt_len, ulong flags)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
	lmi_optmgmt_req_t *p;
 ...
 */
STATIC INLINE int
sdl_bits_for_transmission_req(queue_t *q, mblk_t *dp)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
 ...
 */
STATIC INLINE int
sdl_connect_req(queue_t *q, ulong flags)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
	sdl_connect_req_t *p;
 ...
 */
STATIC INLINE int
sdl_disconnect_req(queue_t *q, ulong flags)
{
	struct mx *mx = MX_PRIV(q);
	mblk_t *mp;
...
lmi_error_ack(queue_t *q, mblk_t *mp)
{
	...
	return mx_error_ack(q, prim, error);
      emsgsize:
	swerr();
	return (-EFAULT);
}
...
sdl_disconnect_ind(queue_t *q, mblk_t *mp)
{
	...
	if (mx->state != MXS_CONNECTED && mx->state != MXS_WACK_CREQ && mx->state != MXS_WCON_CREQ)
		goto outstate;
	return mx_disconnect_ind(q, MXF_BOTH_DIR, 0);
      emsgsize:
	swerr();
	return (-EFAULT);
      outstate:
...
mx_info_req(queue_t *q, mblk_t *mp)
{
	...
	if (mx->state == MXS_UNUSABLE)
		goto outstate;
	return mx_info_ack(q);
      outstate:
	return mx_error_ack(q, p->mx_primitive, MXOUTSTATE);
      emsgsize:
	return mx_error_ack(q, p->mx_primitive, -EMSGSIZE);
}
...
mx_optmgmt_req(queue_t *q, mblk_t *mp)
{
	...
	if (p->mx_opt_length) {
		...
		case MX_PARMS_CIRCUIT:
			...
			case MX_SET_OPT:
				...
				return mx_optmgmt_ack(q, (uchar *) o, sizeof(o->circuit),
						      p->mx_mgmt_flags);
			case MX_DEFAULT:
				/* 
				   set default */
		...
		case MX_GET_OPT:
			...
			return mx_optmgmt_ack(q, (uchar *) o, sizeof(o->circuit), p->mx_mgmt_flags);
		case 0:
		case MX_DEFAULT:
			/* 
			   get default */
	...
	goto badflag;
      badflag:
	return mx_error_ack(q, p->mx_primitive, MXBADFLAG);
      badparmtype:
	return mx_error_ack(q, p->mx_primitive, MXBADPARMTYPE);
      badparm:
	return mx_error_ack(q, p->mx_primitive, MXBADPARM);
      badopt:
	return mx_error_ack(q, p->mx_primitive, MXBADOPT);
      outstate:
	return mx_error_ack(q, p->mx_primitive, MXOUTSTATE);
      emsgsize:
	return mx_error_ack(q, p->mx_primitive, -EMSGSIZE);
...
mx_attach_req(queue_t *q, mblk_t *mp)
{
	...
	if (mx->state != MXS_DETACHED)
		goto outstate;
	return lmi_attach_req(q, mp->b_rptr + p->mx_addr_offset, p->mx_addr_length);
      outstate:
	return mx_error_ack(q, p->mx_primitive, MXOUTSTATE);
      emsgsize:
	return mx_error_ack(q, p->mx_primitive, -EMSGSIZE);
...
mx_connect_req(queue_t *q, mblk_t *mp)
{
	...
	case MXS_CONNECTED:
		...
		if (((mx->flags & p->mx_conn_flags) & MXF_BOTH_DIR))
			goto badflag;
		if ((err = sdl_connect_req(q, p->mx_conn_flags & MXF_BOTH_DIR)))
			return (err);
		/* 
		   fall through */
	case MXS_WCON_CREQ:
		...
		return mx_connect_con(q, p->mx_conn_flags & MXF_BOTH_DIR, p->mx_slot);
	}
	goto outstate;
      badslot:
	return mx_error_ack(q, p->mx_primitive, MXBADSLOT);
      badflag:
	return mx_error_ack(q, p->mx_primitive, MXBADFLAG);
      outstate:
	return mx_error_ack(q, p->mx_primitive, MXOUTSTATE);
...
mx_disconnect_req(queue_t *q, mblk_t *mp)
{
	...
	case MXS_CONNECTED:
		...
		if (!((mx->flags & p->mx_conn_flags) & MXF_BOTH_DIR))
			goto badflag;
		if ((err = sdl_disconnect_req(q, p->mx_conn_flags & MXF_BOTH_DIR)))
			return (err);
		/* 
		   fall through */
	case MXS_WCON_DREQ:
	case MXS_UNUSABLE:
		/* 
		   automatic confirmation */
		return mx_disconnect_con(q, p->mx_conn_flags & MXF_BOTH_DIR, p->mx_slot);
	}
	goto outstate;
      badslot:
	return mx_error_ack(q, p->mx_primitive, MXBADSLOT);
...
mx_w_proto(queue_t *q, mblk_t *mp)
{
	...
	default:
		printd(("%s: %p: -> MX_????\n", MOD_NAME, mx));
		rtn = mx_error_ack(q, prim, MXNOTSUPP);
		break;
	}
	return (rtn);
}
...
mx_r_error(queue_t *q, mblk_t *mp)
{
	...
	default:
		...
		if ((err = mx_disable_ind(q, -mp->b_rptr[0])))
			return (err);
		/* 
		   fall through */
	case MXS_UNUSABLE:
...
#endif
MODULE_PARM_DESC(modid, "Module ID for the MX-SDL module. (0 for allocation.)");

...
#define SM_MOD_REVISION		"LfS $RCSfile: sm_mod.c,v $ $Name:  $($Revision: 0.9.2.22 $) $Date: 2008-09-22 20:31:20 $"
#define SM_MOD_COPYRIGHT	"Copyright (c) 2008-2009  Monavacon Limited.  All Rights Reserved."
...
sm_register_strmod(void)
{
	...
	if ((err = lis_register_module_qlock_option(err, LIS_QLOCK_NONE)) < 0) {
		lis_unregister_strmod(&sm_modinfo);
		return (err);
	}
	return (0);
}

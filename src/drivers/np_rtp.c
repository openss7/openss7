...
static char const ident[] = "$RCSfile: np_rtp.c,v $ $Name:  $($Revision: 1.1.2.5 $) $Date: 2011-09-02 08:46:35 $";
...
struct nc {
	...
	/* Note that we should try to collapse these into a single 32-bit word to save space and
	   increase performance.  The NPI state is never more than 31 and can be represented in 5
	   bits.  BIND_flags and CONN_flags are only a couple of bits.  Accessor functions should
	   be used for these fields so that we can move them around and compress them in various
	   ways. */
	...
	union {
		struct {
			...
			np_ulong CONN_flags:1;		/* connection flags for the NC; zero when
							   disconnected */
		} state;
...
#if 0
	/* These are only required if we are generating connection indications.  I don't think that
	   this is so useful. */
...
struct np_addr_info {
	...
	N_qos_sel_info_rtp_t qos;	/* addresss-related QOS parameters (note that the daddr and
					   dport fields holds the assigned IPv4 address and lowest
...
np_get_port(int index, struct nc *nc, np_ulong SERV_type)
{
	...
	case (N_CONS | N_CLNS):
		if (likely((nb = ni->clns_head) != NULL && (jiffies - nb->stamp) >= ni->clns_guard)) {
			/* Check if there is a suitably aged port number on the free list.  If
			   there is, remove the structure from the free list and assign that port
			   number.  Free the old structure. Free lists for CLNS and CONS are
			   handled differently. */
			...
			/* When there is no available free port number, check whether we still have
			   available port numbers in the range.  If we do, allocate the next one and
			   decrement the allocation mark. */
...
np_put_port(int index, struct nc *nc)
{
	...
	if (likely((nb = xchg(&nc->nb, NULL)) != NULL)) {
		...
		if (ni->clns_hi_port >= nb->port && ni->clns_lo_port <= nb->port) {
			/* When the port number is in the CLNS range, place the NC structure onto
			   the free list for CLNS. */
			*(ni->clns_tail) = nb;
...
/** np_bind_prot -  bind a protocol
  ...
  * on 2.6 and put special code in the handler to give us effectively the old 2.4 approach.  This is
  * also detectable by the fact that inet_add_protocol() returns void on 2.4 and int on 2.6.
  *
...
/** np_bind - bind a Stream to an NSAP
  ...
  * regardless of the number specified.  Binding supports wildcard addresses, both in port number
  * and IP address.  The bind hash contains bind buckets that list Streams that are bound to the
  ...
  * port assignment range, we should just refuse it.  This is because Linux RTP believes that it
  * owns this range and will assign within this range without the user having any control.  If
  * outside this range, it might be an idea to check the Linux RTP hashes to make sure that noone
  * else is using this port at the moment.  It does not keep Linux RTP from assigning it later,
...
/** np_alloc_skb_old - allocate a socket buffer from a message block
  ...
  * referenced by the message block (b_rptr to b_wptr).  Because there is no socket buffer
  * destructor capable of freeing the message block, we steal the kmem_alloc'ed buffer from the
  * message and attach it tot he socket buffer header.  The reference to the message block is
  * consumed unless the function returns NULL.
  *
  * A problem exists in converting mblks to sk_buffs (although visa versa is easy):  sk_buffs put a
  * hidden shared buffer structure at the end of the buffer (where it is easily overwritten on
  * buffer overflows).  There is not necessarily enough room at the end of the mblk to add this
  * structure.  There are several things that I added to the Stream head to help with this:
  *
  ...
  *    requested allocation.  Linux kmalloc() allocates from 2^n size memory caches that are always
  *    SMP cache line aligned.
  *
  ...
  * contain this headroom.  However, again, it might be possible that the message block originated
  * at an intermediate module or was modified by an intermediate module unaware of this policy.  If
  * there is insufficient headroom, again we allocate a new message block large enough to contain
  * the header and make two sk_buffs, one for the header and one for the payload.
  *
  * As a result, we might wind up with three socket buffers: one containing the headroom for the
  * hard header, IP header and RTP header; one containing most of the data payload; and one
  * containing the last fragment of the payload smaller than or equal to sizeof(struct
  * skb_shared_info).  All but the initial socket buffer are placed in the frag_list of the first
  * socket buffer.  Note that only the header need be completed.  If checksum has not yet been
  * performed, it is necessary to walk through the data to generate the checksum.
  */
noinline fastcall __unlikely struct sk_buff *
...
/** np_connect - form a connection
  ...
  *
  * Connect to the specified port and address.  If the connection results in a conflict, TADDRBUSY
  * is returned, zero (0) on success.
  *
  * Destination addresses and port number as well as connection request quality of service
  * parameters should already be stored into the private structure.  Yes, this information will
  * remain if there is an error in the connection request.  When any primitive containing options
  * fails and returns and error, it is the caller's responsibility to set again the values of the
  * options.
  */
STATIC fastcall int
...
/** np_reset_loc - perform a local reset
  ...
  *
  * When completing a local reset, it is necessary to send an ICMP message to the peer.  The
  * attached M_DATA message blocks contain the ICMP message payload.  The @RESET_reason parameter
  * contains the reset reason that translates to an ICMP error code.  The destination for the reset
  * on a multi-homed connection is the current default destination.
  */
STATIC int
	   ...
	   const np_ulong CONN_flags, mblk_t *dp)
{
	...
	if (dp != NULL)
		if (unlikely
		    ((err =
		      np_senddata(np, np->qos.protocol, np->dport, np->qos.daddr,
				  dp)) != QR_ABSORBED))
			goto recover;
	if (SEQ_number != NULL) {
...
/** np_conn_con: perform N_CONN_CON actions
  ...
  * connections are autobound (a null address was specified on N_BIND_REQ and one is only selected
  * on N_CONN_REQ), it would be convenient if the NS-provider would return the bound address so that
  * the NS-user does not have to issue a N_INFO_REQ to obtain the autobound value.  Therefore, the
	    ...
	    struct N_qos_sel_conn_rtp *QOS_buffer, np_ulong CONN_flags)
{
	...
	/* The M_PROTO is set to band 2 to keep expedited data on the same network connection from
...
/** np_conn_ind - generate a N_CONN_IND message
  ...
  * @dp: message containing IP packet
  *
  * We generate connection indications to Streams that are bound as listening to an address
  * including the destination address of the IP packet, where no connection exists for the source
...
/** np_discon_ind - NE_DISCON_IND NC disconnected event
  ...
  */
STATIC INLINE fastcall int
np_discon_ind(queue_t *q, struct sockaddr_in *RES_buffer, socklen_t RES_length,
...
np_discon_ind_icmp(queue_t *q, mblk_t *mp)
{
	...
	if ((err = np_discon_ind(q, RES_buffer, sizeof(*RES_buffer), RESERVED_field, DISCON_orig,
...
np_uderror_ind(queue_t *q, struct sockaddr_in *DEST_buffer, np_ulong RESERVED_field,
...
noinline fastcall __unlikely int
np_uderror_reply(queue_t *q, struct sockaddr_in *DEST_buffer, np_ulong RESERVED_field,
...
/** nc_conn_ind - generate an N_CONN_IND and pass it upstream
  ...
  */
STATIC int
nc_conn_ind(queue_t *q, mblk_t *msg, struct np *np, struct nc *nc,
	    mblk_t *SEQ_number, np_ulong CONN_flags,
	    caddr_t DEST_buffer, size_t DEST_length,
	    caddr_t SRC_buffer, size_t SRC_length,
...
/** nc_conn_con - generate an N_CONN_CON and pass it upstream
  ...
  */
STATIC int
nc_conn_con(queue_t *q, mblk_t *msg, struct np *np, struct nc *nc, np_ulong CONN_flags,
	    caddr_t RES_buffer, size_t RES_length,
	    caddr_t QOS_buffer, size_t QOS_length, mblk_t *dp)
{
	N_conn_con_t *p;
	mblk_t *mp;
	...
	/* The M_CTL/M_PROTO is set to band 2 to keep expedited data on the same network connection
	   from overtaking the connection confirmation. */
...
nc_discon_ind(queue_t *q, mblk_t *msg, struct np *np, struct nc *nc, np_ulong DISCON_orig,
	      np_ulong DISCON_reason, caddr_t RES_buffer, size_t RES_length, mblk_t *SEQ_number,
	      mblk_t *dp)
{
	N_discon_ind_t *p;
	mblk_t *mp;
...
/** nc_unitdata_ind - generate an N_UNITDATA_IND and pass it upstream
  ...
  */
STATIC int
nc_unitdata_ind(queue_t *q, mblk_t *msg, struct np *np, struct nc *nc, caddr_t SRC_buffer,
		size_t SRC_length, caddr_t DEST_buffer, size_t DEST_length, mblk_t *dp)
{
	N_unitdata_ind_t *p;
	mblk_t *mp;
	const size_t plen = sizeof(*p) + (nc ? sizeof(nc) : 0);
...
/** nc_info_ack - generate an N_INFO_ACK and pass it upstream
  ...
  */
STATIC int
nc_info_ack(queue_t *q, mblk_t *msg, struct np *np, struct nc *nc,
	    caddr_t ADDR_buffer, size_t ADDR_length,
	    caddr_t QOS_buffer, size_t QOS_length,
	    caddr_t QOS_range_buffer, size_t QOS_range_length,
...
/** nc_bind_ack - generate an N_BIND_ACK and pass it upstream
  ...
  */
STATIC int
nc_bind_ack(queue_t *q, mblk_t *msg, struct np *np, struct nc *nc,
	    np_ulong SERV_type, np_ulong CONIND_number, np_ulong BIND_flags,
	    caddr_t ADDR_buffer, size_t ADDR_length, caddr_t PROTOID_buffer, size_t PROTOID_length)
{
	N_bind_ack_t *p;
	mblk_t *mp;
	...
	if (likely(nc != NULL)) {
		if (unlikely
		    (!(nc = nc_alloc(np, *(typeof(nc->tag) *) mp->b_wptr, SERV_type,
				     CONIND_number, BIND_flags, ADDR_buffer, ADDR_length,
				     PROTOID_buffer, PROTOID_length))))
			goto enomem;
...
/** nc_error_ack - generate an N_ERROR_ACK and pass it upstream
  ...
  */
STATIC int
nc_error_ack(queue_t *q, mblk_t *msg, struct np *np, struct nc *nc, np_ulong ERROR_prim,
	     np_long error)
{
	N_error_ack_t *p;
	mblk_t *mp;
...
np_conn_res(queue_t *q, mblk_t *mp, struct np *np)
{
	...
	err = np_ok_ack(q, N_CONN_RES, RES_buffer, p->RES_length,
...
np_discon_req(queue_t *q, mblk_t *mp, struct np *np)
{
	...
	err = np_ok_ack(q, N_DISCON_REQ, RES_buffer, RES_length, NULL, SEQ_number, NULL,
			p->DISCON_reason, dp);
	if (unlikely(err != QR_ABSORBED))
		goto error;
	return (QR_TRIMMED);	/* user data is absorbed */
...
ne_write_req(queue_t *q, mblk_t *mp, struct np *np)
{
	...
	if (unlikely
	    ((err =
	      np_senddata(np, np->qos.protocol, np->dport, np->qos.daddr, mp)) != QR_ABSORBED))
		goto error;
...
np_data_req(queue_t *q, mblk_t *mp, struct np *np)
{
	...
	if (unlikely
	    ((err =
	      np_senddata(np, np->qos.protocol, np->dport, np->qos.daddr, dp)) != QR_ABSORBED))
		goto error;
	return (QR_TRIMMED);	/* np_senddata() consumed message blocks */
      discard:
...
nc_data_req(queue_t *q, mblk_t *mp, struct np *np, struct nc *nc)
{
	...
	if (unlikely(nc->SERV_type != N_CONS))
		goto error;
	/* Note: when the interface is in the NS_IDLE or NS_WRES_RIND states when the provider
	   receives the N_DATA_REQ primitive, then the NS provider should discard the request
	   without generating a fatal error. */
	if (nc_chk_state(nc, (NSF_IDLE | NSF_WRES_RIND)))
		/* For multihomed operation, we should not actually discard the N_DATA_REQ if the
		   destination of the request is the address that does not have an outstanding
		   reset indication. */
		goto discard;
	err = NOUTSTATE;
	if (unlikely(np_not_state(np, NSM_OUTDATA)))
	...
	if (unlikely(p->DATA_xfer_flags & (N_MORE_DATA_FLAG | N_RC_FLAG)))
		/* TODO: we should check the N_MORE_DATA_FLAG and see whether this is a complete
		   NSDU or not.  If not, we should accumulate the M_DATA block in a buffer waiting
		   for a findal N_DATA_REQ or delimited message. */
		goto error;
	err = NBADDATA;
	if (unlikely((dp = mp->b_cont) == NULL))
		goto error;
...
nc_exdata_req(queue_t *q, mblk_t *mp, struct np *np, struct nc *nc)
{
	...
	if (unlikely(nc->SERV_type != N_CONS))
		goto error;
	/* Note: when the interface is in the NS_IDLE or NS_WRES_RIND states when the provider
	   receives the N_EXDATA_REQ primitive, the NS provider should discard the requeswt without
	   generating a fatal error. */
...
nc_conn_req(queue_t *q, mblk_t *mp, struct np *np, struct nc *nc)
{
	...
	if (unlikely(nc->SERV_type != N_CONS))
		goto error;
	/* Connection requests are not allowed on a listening Stream.  Note that there is a
	   conflict in the NPI specification here: under the description for N_BIND_REQ, NPI 2.0.0
	   says: "If a Stream is bound as a listener Stream, it will not be able to initiate
	   connect requests. If the NS user attempts to send a N_CONN_REQ primitive down this
	   Stream, an N_ERROR_ACK message will be sent to the NS user by the NS provider with an
	   error value of NACCESS." Then, under N_BIND_ACK, NPI 2.0.0 says: "A Stream with a
	   negotiated CONIND_number greater than zero may generate connect requests or accept
	   connect indications." */
...
/** nc_conn_res - process N_DATA_REQ primitive
  ...
  * @nc: network connection
  *
  * An NE_CONN_IND event is generated when a UDP message arrives for an address to an N_CONS Stream
  * and there is not yet a connection for the source address in the message.  Outstanding connection
  * indications are queued against the virtual Stream with a Sequence number assigned (derived from
  * the address of the connecting message held in the connection indication list).  The NE_CONN_RES
  * event is generated by the user to accept a connection indication event from the connection
  * indication queue and to perform a passive connection (NE_PASS_CON event) on the indicate Stream.
  * In addition, the user can set QOS parameters for the Stream to which a passive connection is
  * made and for any response message (data attached to the N_CONN_RES message). 
  *
  * There is a deviation here from the NPI specifications: the responding address(es) in the
  * N_CONN_RES primitive contains the list of destination address(es) to which to form the
  * connection.  If no responding addresses are provided, then the destination address is the source
...
nc_conn_res(queue_t *q, mblk_t *mp, struct np *np, struct nc *nc)
{
	...
	if ((res_len = p->RES_length)) {
		...
		if (unlikely
		    ((RES_buffer[0].sin_family != AF_INET && RES_buffer[0].sin_family != 0)))
			goto error;
	...
	if ((qos_len = p->QOS_length)) {
		...
		if (unlikely(qos_len < sizeof(QOS_buffer->n_qos_type)))
			goto error;
		/* Cannot be sure that the quality of service parameters are properly aligned, and
		   to keep unscrupulous users from DoS attacks, copy the information into an
		   aligned buffer. */
		bcopy(mp->b_rptr + p->QOS_offset, qos_ptr, sizeof(QOS_buffer->n_qos_type));
		err = NBADQOSTYPE;
		if (unlikely(QOS_buffer->n_qos_type != N_QOS_SEL_CONN_RTP))
	...
	if (p->TOKEN_value != 0) {
		...
		if (nc_get_state(np, TOKEN_value) != NS_IDLE)
			/* Later we could auto bind if in NS_UNBND state.  Note that the Stream to
			   which se do a passive connection could be already connected: we are just
			   adding another address to the multihomed connection.  But this is not as
			   useful as adding or removing an address with N_OPTMGMT_REQ. */
			goto error;
		err = NBADTOKEN;
...
nc_discon_req(queue_t *q, mblk_t *mp, struct np *np, struct nc *nc)
{
	...
	if ((res_len = p->RES_length)) {
		...
		if (unlikely(res_len % sizeof(*RES_buffer) != 0))
			goto error;
		bcopy(mp->b_rptr + p->RES_offset, res_ptr, res_len);
		if (unlikely((RES_buffer[0].sin_family != AF_INET && RES_buffer[0].sin_family !=
			      0)))
			goto error;
		if (unlikely(RES_buffer[0].sin_port == 0))
			goto error;
...
nc_bind_req(queue_t *q, mblk_t *mp, struct np *np, struct nc *nc)
{
	...
	if ((p->BIND_flags & DEFAULT_DEST) || p->CONIND_number != 0) {
		/* When DEFAULT_DEST is specified, the interface is always considered
		   connectionless.  Each protocol id is permitted multiple DEFAULT_DEST interface
		   (one for each IP address and port, and one for the wildcard IP address and
		   port). DEFAULT_DEST interfaces are only considered when there is no match for
		   any other connectionless bound interface that maches the destination adress. */
		SERV_type = N_CLNS;
		CONIND_number = 0;
	} else {
		SERV_type = N_CONS;
	...
	if ((add_len = p->ADDR_length)) {
		...
		if (unlikely
		    (ADDR_buffer[0].sin_family != AF_INET && ADDR_buffer[0].sin_family != 0))
			goto error;
		if (add_num == 1
		    && (ADDR_buffer[0].sin_addr.s_addr == INADDR_ANY
			|| ADDR_buffer[0].sin_port == 0))
	...
	if (unlikely(p->BIND_flags & DEFAULT_DEST) && ADDR_buffer[0].sin_port != 0)
		goto error;
	return nc_bind_ack(q, mp, np, nc, SERV_type, CONIND_number, p->BIND_flags,
			   add_ptr, add_len, pro_ptr, pro_len);
      error:
...
nc_datack_req(queue_t *q, mblk_t *mp, struct np *np, struct nc *nc)
{
	...
	if (unlikely(nc->SERV_type != N_CONS))
		goto error;
	/* Note: If the interface is in the NS_IDLE state when the provider receives the
	   N_DATACK_REQ primitive, the the NS provider should discard the request without
	   generating a fatal error. */
	err = 0;
	if (unlikely(nc_get_state(np, nc) == NS_IDLE))
		goto error;
	...
	if (unlikely(nc_not_state(nc, NSM_CONNECTED)))
		goto error;
	/* Note: If the NS provider had no knowleged of a previous N_DATA_IND with the receipt
	   confirmation flag set, then the NS provider should ignore the request without generating
	   a fatal error. */
...
nc_reset_req(queue_t *q, mblk_t *mp, struct np *np, struct nc *nc)
{
	...
	if (unlikely(nc->SERV_type != N_CONS))
		goto error;
	/* Note: when the interface is in the NS_IDLE state when the provider receives the
	   N_RESET_REQ primitive, then the NS provider should discard the message without
	   generating and error. */
	err = 0;
	state = nc_get_state(np, nc);
...
/** nc_lookup_conn - lookup network connection in the connection hashes
  ...
  *
  * The start of the right kind of code shaping up here.  We provide connection buckets that have IP
  * protocol number, source port and destination port as hash parameters.  Attached to the
...
/** nc_lookup_bind - lookup Stream in the bind/listening hashes
  ...
  *
  * Note that an N_CLNS Stream can bind to either a port number or port number zero. An N_CONS
  * Stream can only bind listening to a non-zero port number, but can bind normal to a zero port
  * number.
  *
  ...
  *
  * NOTICES: There are two types of Streams in the bind hashes, N_CONS and N_CLNS.  When lookup up a
  * Stream for a received packet, we are interested in any N_CLNS Stream that matches or any N_CONS
...
/** np_v4_rcv - process a received IP packet
  ...
  * an IP protocol number to which a Stream is bound.  For NPI-RTP, this is IPPROTO_UDP, however,
  * the possibility is there to permit a specialized IP protocol number for RTP (although one has
  * not been assigned by IANA yet).
  *
  ...
  * address is a unicast address, look up the receiving IP interface based on the protocol number,
  * IP address and UDP port number.  If no receiving interface exists for a unicast packet, or if
  * the packet is a broadcast or multicast packet, pass the packet along to the next handler if any.
  * If there is no next handler and the packet was not sent to any interface, generate an
  ...
  * receive function.
  *
  * If the receiving Stream is flow controlled, simply discard its copy of the IP packet.
  * Otherwise, generate an (internal) M_DATA message and pass it to the Stream.  To avoid dropping
  * messages, use the N_DATA_IND, N_DATACK_REQ mechanism for connected interfaces to permit at least
...
/** np_v4_err - process a received ICMP packet
  ...
  *
  * ICMP packet consists of ICMP IP header, ICMP header, IP header of returned packet, and IP
  * payload of returned packet (up to some number of bytes of total payload).  The passed in sk_buff
  * has skb->data pointing to the ICMP payload which is the beginning of the returned IP header.
  ...
  * np->qlock protects the state of private structure.  np->refs protects the private structure from
...
/** np_alloc_priv - allocate a private structure for the open routine
  ...
  *
  * Allocates a new private structure, initializes it to appropriate values, and then inserts it
  * into the private structure list.
  */
noinline struct np *
...
np_qopen(queue_t *q, dev_t *devp, int oflag, int sflag, cred_t *crp)
{
	...
	if (q->q_ptr) {
		...
		/* FIXME: this is not a clone device and yet it is already open? Check that the
		   opening process has sufficient credentials to open the non-clone device. */
		return (EPERM);
	}
	...
	/* np_lock protects the list of streams.  This is needed for accepting a connection
	   indication from one stream on another. */
	...
	/* There are several groupings of information associated with RTP channels that we will use
	   when sending RTP packets. The base group is global defaults.  Another group is
	   per-Stream.  To acheive a different grouping of per-Stream parameters, open another
	   Stream.  Another group is per-IP address.  We only expect a limited number (16) IP
	   addresses to be used.  The last grouping is per-interface parameters.  Per-interface
	   parameters are limited as much as possible to reduce the size (and thus increase the
	   performance) of the connection-mapping memory. */
	...
	/* We generate a message to the Stream head (that can be intercepted by other modules) that
	   specifies that M_DATA message blocks should be allocated using socket buffers.  Also, set
	   the write offset in the M_DATA messages to accept MAX_HEADER bytes so that we can fill out 
	   the IP and hardware header without reallocating socket buffers on the way out. Set the
	   minimum and maximum packet size for outgoing messages so that the Stream head will do the
	   checking for us where possible.  The high and low water marks for the Stream head are also 
	   set, but this is preliminary as this can be adjusted using the STREAMS MIB.  */
...
np_init_caches(void)
{
	if (np_priv_cachep == NULL) {
		np_priv_cachep =
		    kmem_create_cache("np_priv_cachep", mi_open_size(sizeof(struct np)), 0,
	...
	if (np_prot_cachep == NULL) {
		np_prot_cachep =
		    kmem_create_cache("np_prot_cachep", sizeof(struct nc_prot_bucket), 0,
				      SLAB_HWCACHE_ALIGN, NULL, NULL);
		if (np_prot_cachep == NULL) {
	...
	if (np_conn_cachep == NULL) {
		np_conn_cachep =
		    kmem_create_cache("np_conn_cachep", sizeof(struct nc), 0,
				      SLAB_HWCACHE_ALIGN, NULL, NULL);
		if (np_conn_cachep == NULL) {
...
np_init_hashes(void)
{
	...
	if (nc_bhash == NULL) {
		nc_bhash_order = 0;
		if ((nc_bhash =
		     (struct nc_bhash_bucket *) __get_free_pages(GFP_ATOMIC, nc_bhash_order))) {
			nc_bhash_size =
			    (1 << (nc_bhash_order + PAGE_SHIFT)) / sizeof(struct nc_bhash_bucket);
	...
	if (nc_chash == NULL) {
		nc_chash_order = 0;
		if ((nc_chash =
		     (struct nc_chash_bucket *) __get_free_pages(GFP_ATOMIC, nc_chash_order))) {
			nc_chash_size =
			    (1 << (nc_chash_order + PAGE_SHIFT)) / sizeof(struct nc_chash_bucket);

...
static char const ident[] __attribute__((unused)) = "src/snmp/streamsd.c (" PACKAGE_ENVR ") " PACKAGE_DATE;
...
main(int argc, char *argv[])
{
	for (;;) {
		...
		case 'l':	/* -l, --log-file, --logfile, -Lf, -LF p1[-p2] [LOGFILE] */
			if (optarg != NULL)
				strncpy(sa_logfile, optarg, sizeof(sa_logfile) - 1);
			if (sa_debug)
		...
		case 'P':	/* -p, -P, --pidfile PIDFILE */
			if (optarg) {
				...
				strncpy(sa_pidfile, optarg, sizeof(sa_pidfile) - 1);
			}
			if (sa_debug)
		...
		case 'S':	/* -S, -sysctl-file FILENAME */
			...
			strncpy(sa_sysctlf, optarg, sizeof(sa_sysctlf) - 1);
			break;

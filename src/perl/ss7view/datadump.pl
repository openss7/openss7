...
my $progdir = $0; $progdir =~ s/\/[^\/]*$//; $progdir = '.' unless $progdir;
...
foreach (keys %{$what->{city}}) {
	delete $what->{town}->{$_};
}

my %cmap = ();

open($fh,"<","$datadir/ccodes.txt");
while (<$fh>) { chomp;
	next if /^#/;
	next unless /^[A-Z][A-Z]\t/;
...
open($fh,"<","$datadir/divisions.txt");
while (<$fh>) { chomp;
	next if /^#/;
	next unless /^[A-Z][A-Z]\t/;
	my ($a2,$sts,$locs,$div) = split(/\t/,$_,4);
	if ($locs =~ / \/ /) { $locs = [ split(/ \/ /,$locs) ]; } else { $locs = [ $locs ]; }
	foreach my $loc (@$locs) {
		$smap{$a2}->{$loc} = { a2=>$a2, st=>$sts } unless exists $smap{$a2}->{$loc};
	}
	if (!$sts or $sts eq '--') {
		foreach my $loc (@$locs) {
			push @{$var->{iso3166}->{$a2}->{names}}, $loc;
	...
	if ($sts =~ /,/) { $sts = [ split(/,/,$sts) ]; } else { $sts = [ $sts ]; }
	foreach my $st (@$sts) {
		my $val;
		if (exists $var->{iso3166}->{$a2}->{states}->{$st}) {
			$val = $var->{iso3166}->{$a2}->{states}->{$st};
		} else {
			$val = {};
			$var->{iso3166}->{$a2}->{states}->{$st} = $val;
		}
		foreach my $loc (@$locs) {
			push @{$val->{names}}, $loc;
		}
...
open($fh,"<","$datadir/states.txt");
while (<$fh>) { chomp;
	next if /^#/;
	my ($a2,$st,$loc,$tz) = split(/\t/,$_);
	$smap{$a2}->{$loc} = { a2=>$a2, st=>$st } unless exists $smap{$a2}->{$loc};
	$smap{$a2}->{$loc}->{tz} = $tz if defined $tz;
	unless ($a2 eq 'ZZ' or exists $var->{iso3166}->{$a2}) {
...
open($fh,"<","$datadir/e164d.txt");
while (<$fh>) { chomp;
	next if /^#/;
	if (/^[0-9]+[0-9-]*\t/) {
		my ($code,$loc,$note,$shared) = split(/\t/,$_);
		next if $loc =~ /spare code/i;
		next if $loc =~ /reserved/i;
		my ($a2,$a3,$n3,$geo) = ('ZZ','-','-',0);
		unless ($shared) {
			$geo = 1;
			my $cc = $cmap{$loc};
			unless (defined $cc) {
...
while (<$fh>) { chomp;
	next if /^#/;
	next unless /^[0-9]+\t/;
	my ($npa,$a2,$loc,$shared) = split(/\t/,$_);
	my $state = $smap{$a2}->{$loc} if exists $smap{$a2};
	unless ($state) {
		warn "cannot find state code for '$a2' '$loc'" unless $a2 eq 'ZZ';
		$state = {st=>'--'};
	}
	my $cc = 1;
	#my $a2 = $state->{a2};
	...
	if (exists $var->{e164}->{$cc}->{area}->{$npa}) {
		...
		if ($a2 ne 'US' and $a2 ne 'CA' and $geo) {
	...
	$val->{geo} = $geo unless defined $val->{geo};
	$val->{loc} = $loc unless $val->{loc};
	$val->{idp} = '011' unless defined $val->{idp};
	if ($npa =~ /^[2-9]11$/) {
		$val->{ndp} = ''    unless defined $val->{ndp};
		$val->{digs} = 3    unless defined $val->{digs};
	} else {
	...
	foreach my $loc (@$locs) {
		...
		if ($digs) {
			foreach my $dig (@$digs) {
				...
				my $pref = $num.$dig;
				my $val;
				if (exists $var->{e164}->{$cc}->{area}->{$pref}) {
					warn "record exists for '$pref'";
					$val = $var->{e164}->{$cc}->{area}->{$pref};
				} else {
					$val = {};
					$var->{e164}->{$cc}->{area}->{$pref} = $val;
			...
			if (exists $var->{e164}->{$cc}->{area}->{$num}) {
				warn "record exists for '$num'";
				$val = $var->{e164}->{$cc}->{area}->{$num};
			} else {
				$val = {};
				$var->{e164}->{$cc}->{area}->{$num} = $val;
			}
			$val->{geo} = 1;
			$val->{loc} = $loc;
			$val->{plan} = $plan;
...
sub listem {
	...
	foreach my $code (sort keys %$var) {
		...
		if (defined $ge) {
			printf $of $join."geo=>%d", $ge;
			$join = ',';
		}
		if ($cn and (!$ge or !$a2 or $a2 eq '-' or $a2 eq 'ZZ' or $st eq '--')) {
			$cn =~ s/\\/\\\\/g; $cn =~ s/'/\\'/g;
	...
	foreach (sort keys %{$var->{iso3166}}) {
		...
		if ($val->{states}) {
			...
			foreach my $state (sort keys %{$val->{states}}) {
				...
				printf $of "\t\t\t'%s'=>{names=>[", $state;
				my $join = '';
				foreach my $name (@{$sta->{names}}) {
					$name =~ s/\\/\\\\/g; $name =~ s/'/\\'/g;
					printf $of $join."'%s'", $name;
					$join = ',';
				}
				print $of "]},\n";
			}

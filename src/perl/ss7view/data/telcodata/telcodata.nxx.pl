...
foreach my $npa (keys %db) {
	my $dir = "$rsltdir/$npa";
	my $fil = "$dir/data.html.xz";
	print STDERR "I: fetching $npa ($tot remaining)\n";
	system("mkdir -v -p $dir") unless -d $dir;
	system("wget -O - 'http://www.telcodata.us/search-area-code-exchange-detail?npa=$npa&exchange=' | xz > $fil") unless -f $fil;
	$tot--;
}

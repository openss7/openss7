...
sub dodata {
	...
	my %mapping = (
		...
		'Prefix Type'=>sub{
			...
			mappedfield($dat,'companytype',$val,{
					'CLEC'=>'CLEC',
					'WIRELESS'=>'WIRE',
					'PCS'=>'PCS',
					'RBOC'=>'RBOC',
		...
		'Switch Name'=>\&simplefield,
		'Switch Type'=>sub{
			my ($dat,$fld,$val) = @_;
			simplefield($dat,'switchdesc',$val);
		},
		'LATA'=>sub{
			my ($dat,$fld,$val) = @_;
			$val = undef if $val eq 'N/A';

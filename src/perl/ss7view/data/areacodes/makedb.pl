...
sub dodata {
	...
	my %mapping = (
		...
		'STATE'=>sub{
			...
			simplefield($dat,'region',$val);
		},
		'CITY'=>sub{
			my ($dat,$fld,$val) = @_;
			simplefield($dat,'rccity',$val);
		},
		'COUNTY'=>sub{
			my ($dat,$fld,$val) = @_;
		...
		'LATITUDE'=>sub{
			...
			simplefield($dat,'rclat',$val);
		},
		'LONGITUDE'=>sub{
			my ($dat,$fld,$val) = @_;
			$val = sprintf('%.8f',$val) if $val;
			simplefield($dat,'rclon',$val);
		},
		'LATA'=>sub{
			my ($dat,$fld,$val) = @_;
		...
		'OBSERVES_DST'=>sub{
			my ($dat,$fld,$val) = @_;
			simplefield($dat,'dst',$val);
		},
		'COUNTY_POPULATION'=>sub{
			my ($dat,$fld,$val) = @_;
			simplefield($dat,'population',$val);
		},
		'FIPS_COUNTY_CODE'=>sub{
			my ($dat,$fld,$val) = @_;
			simplefield($dat,'fips',$val);
		},
		'MSA_COUNTY_CODE'=>sub{
			my ($dat,$fld,$val) = @_;
			simplefield($dat,'msa',$val);
		},
		'PMSA_COUNTY_CODE'=>sub{
			my ($dat,$fld,$val) = @_;
			simplefield($dat,'pmsa',$val);
		},
		'CBSA_COUNTY_CODE'=>sub{
			my ($dat,$fld,$val) = @_;
			simplefield($dat,'cbsa',$val);
		},
		'ZIPCODE_POSTALCODE'=>sub{
			my ($dat,$fld,$val) = @_;
			simplefield($dat,'rczip',$val);
		},
		'ZIPCODE_COUNT'=>sub{
			my ($dat,$fld,$val) = @_;
		},
		...
		'SWITCH_CLLI'=>sub{
			my ($dat,$fld,$val) = @_;
			simplefield($dat,'switch',$val);
		},
		'RC_VERTICAL'=>sub{
			my ($dat,$fld,$val) = @_;
			$val = sprintf('%05d',$val) if $val;
			simplefield($dat,'rcv',$val);
		},
		'RC_HORIZONTAL'=>sub{
			my ($dat,$fld,$val) = @_;
			$val = sprintf('%05d',$val) if $val;
			simplefield($dat,'rch',$val);
		},
	);

...
my @dbnxx_keys = qw/NPA NXX X CC ST RG LATA/;
...
while (<$fh>) { chomp;
	...
	my $rg = $data->{REGION}; $rg = $data->{WCRG} unless $rg; $data->{RG} = $rg if $rg;
	if ($npa ne $oldnpa and $rg ne $oldrg) {
		print STDERR "I: processing $npa $rg...\n";
	}
	($oldnpa,$oldrg) = ($npa,$rg);
	my $cc = $data->{RCCC};   $cc = $data->{WCCC} unless $cc; $data->{CC} = $cc if $cc;
	my $st = $data->{RCST};   $st = $data->{WCST} unless $st; $data->{ST} = $st if $st;
	my $lt = $data->{LATA}; $lt = $data->{RCLATA} unless $lt; $lt = $data->{WCLATA} unless $lt;
	{
		$dbnxx{$data->{NPA}}{$data->{NXX}}{$data->{X}} = {} unless exists
		$dbnxx{$data->{NPA}}{$data->{NXX}}{$data->{X}};
		my $rec = $dbnxx{$data->{NPA}}{$data->{NXX}}{$data->{X}};
		foreach my $k (@dbnxx_keys) {
			if (length($data->{$k})) {
...
$fn = "$datadir/db.nxx.nogeo.csv";
print STDERR "I: writing $fn\n";
open($of,">:utf8",$fn) or die "can't write $fn";
print $of '"',join('","',@dbnxx_keys),'"',"\n";
foreach my $npa (sort keys %dbnxx) {
	foreach my $nxx (sort keys %{$dbnxx{$npa}}) {
		foreach my $x (sort keys %{$dbnxx{$npa}{$nxx}}) {
			my $rec = $dbnxx{$npa}{$nxx}{$x};
			my @values = ();
			foreach (@dbnxx_keys) {
				if (exists $rec->{$_}) {

...
sub getrg {
	...
	my $rg = $data->{STATE};
	return $rg unless $rg;
	return undef unless exists $necarg{$rg};
	$rg = $necarg{$rg};
	return $rg;
}
...
while (<$fh>) { chomp;
	...
	if ($cc and $st and $nm) {
		...
		my ($v,$h) = split(/,/,$data->{WCVH}); ($v,$h) = (int($v),int($h));
		...
		if ($v and $h) {
			$data->{WCVH} = sprintf('%05d,%05d',$v,$h);
			$data->{WCLL} = join(',',vh2ll($v,$h));
			unless ($rec or exists $data->{RCGEOID}) {
				my ($vh,$ll,$gn,$fc) = ($data->{WCVH},$data->{WCLL},$nm);

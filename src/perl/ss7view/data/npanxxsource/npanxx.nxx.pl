...
foreach my $file (@files) { chomp $file;
	...
	while (<$fh>) { chomp;
		...
		next unless $npa and $nxx and length($x);
		$tot++ unless exists $db{$npa}{$nxx}{$x};
		$db{$npa}{$nxx}{$x}++;

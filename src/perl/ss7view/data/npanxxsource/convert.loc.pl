...
my %skipnames = (
	'411 Local Directory Assistance'=>1,
	'Advanced Intelligent Network'=>1,
	'Billing Only'=>1,
	'Broadband and IP Services'=>1,
	'Cable Television'=>1,
	'Community Support And Referral Services'=>1,
	'DIRECTORY ASSISTANCE'=>1,
	'Emergency Services'=>1,
	'FGB Access Code'=>1,
	'High Volume'=>1,
	'Information Provider'=>1,
	'Intralata Pic Validation'=>1,
	'Non Emergency Police And Other Governmental Services'=>1,
	'Non-Urgent Health Care Telephone Triage Service'=>1,
	'One Call Services-Pipeline And Utility Safety'=>1,
	'Open Network Architecture'=>1,
	'Rate Center Not Applicable'=>1,
	'Reserved Service Access Code'=>1,
	'Routing Only'=>1,
	'Standard Plant Test Code'=>1,
	'Telco Repair Service'=>1,
	'Telecommunications Relay Service'=>1,
	'Travel Information Services'=>1,
	'Unavailable For Assignment'=>1,
);
my %locs = ();
my %lcas = ();
...
while (<$fh>) { chomp;
	...
	my $loc = $data->{'Rate Center Name'};
	next if exists $skipnames{$loc};
	my $rg = $data->{REGION};
	...
	if ($loc and $cc and $st) {
		...
		$rec->{LOCNAME} = $data->{'Rate Center Name'};
		$rec->{NPA} = $data->{NPA};
		$rec->{NXX} = $data->{NXX};
		$rec->{OCN} = $data->{OCN};
		$rec->{LATA} = $data->{LATA};

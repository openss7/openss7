...
sub doswitchdata {
	...
	foreach $fn (@files) { chomp $fn;
		...
		open($fh,"xzcat $f2 |") or die "can't process $f2";
		my $fdate = stat($fn)->mtime;
		my $sect = $sw;
		my %data = (sect=>$sect,fdate=>$fdate);
		while (<$fh>) { chomp;
			...
			if (/^<\/inputdata>/) {
				...
				if (length($inpt{exch}) and scalar @prefixes) {
					foreach my $rec (@prefixes) {
						...
						$rec->{exchob} = $inpt{exch};
						$rec->{exchib} = $rec->{exch};
						if ($rec->{exchob} ne $rec->{exchib}) {
							if ($inpt{dir} eq 'Inbound') {
								($rec->{exchob},$rec->{exchib}) =
								($rec->{exchib},$rec->{exchob});
							}
							delete $rec->{npa};
							delete $rec->{nxx};
							updatedata($rec,$fdate,$sect);
						}
					}
				}
				last;

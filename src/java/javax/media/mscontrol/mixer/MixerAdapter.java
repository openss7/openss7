/*
 @(#) $RCSfile: MixerAdapter.java,v $ $Name:  $($Revision: 1.1.2.1 $) $Date: 2009-06-21 11:35:26 $ <p>
...
Interface MixerAdapter
   ...
   All Superinterfaces:
          ...
          ResourceContainer<MixerAdapterConfig>, java.io.Serializable,
          StatusEventNotifier
     _________________________________________________________________

public interface MixerAdapter

   extends JoinableContainer, ResourceContainer<MixerAdapterConfig>

   A MixerAdapter is the materialization of a "Mixer port".
   Additionally, it can include media processing resources.
   (The resources cannot be players or recorders).
   In most cases, the application will want a dtmf clamper, and/or a
   volume control.
   This is also the place to set a VideoLayout per mixer port.
   The default behaviour is to let all streams pass through, in both
   directions, without modification.
   This object is built with createMixerAdapter in its different forms..
   It should be released by the application (However the Mixer or the
   MediaSession will clean it up on exit).
     _________________________________________________________________

/*
 @(#) $RCSfile: InformationToRecord.java,v $ $Name:  $($Revision: 1.1.2.1 $) $Date: 2009-06-21 11:34:54 $ <p>
...
public class InformationToRecord implements java.io.Serializable {
    ...
    public void setMessageID(int messageID)
        ...
        if (0 <= messageID && messageID <= 2147483647) {
            this.messageID = messageID;
            isMessageID = true;
            return;
        }
        throw new IllegalArgumentException("Message Id: " + messageID + ", out of range.");
    }
    ...
    public int getMessageDeletionTimeOut()
        throws ParameterNotSetException {
        if (isMessageDeletionTimeOut)
            return messageDeletionTimeOut;
        throw new ParameterNotSetException("Message Deletion Time Out: not set.");
    }
    /** Sets Message Deletion.  */
    public void setMessageDeletionTimeOut(int messageDeletionTimeOut)
        throws IllegalArgumentException {
        if (1 <= messageDeletionTimeOut && messageDeletionTimeOut <= 3600) {
            this.messageDeletionTimeOut = messageDeletionTimeOut;
            isMessageDeletionTimeOut = true;
            return;
        }
        throw new IllegalArgumentException("Message Deletion Time Out: " + messageDeletionTimeOut + ", out of range.");
    }
    ...
    /** Gets Time To Record.  */
    public int getTimeToRecord()
        throws ParameterNotSetException {
        if (isTimeToRecord)
            return timeToRecord;
        throw new ParameterNotSetException("Time To Record: not set.");
    }
    /** Sets Time To Record.  */
    public void setTimeToRecord(int timeToRecord) {
        this.timeToRecord = timeToRecord;
        this.isTimeToRecord = true;
    }

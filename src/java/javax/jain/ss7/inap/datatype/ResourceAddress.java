/*
 @(#) $RCSfile: ResourceAddress.java,v $ $Name:  $($Revision: 1.1.2.1 $) $Date: 2009-06-21 11:34:55 $ <p>
...
public class ResourceAddress implements java.io.Serializable
{
    ...
    public ResourceAddress(int callSegmentIDOrNone, ResourceAddressID resourceAddressID) {
        if (resourceAddressID == ResourceAddressID.NONE)
            setNone(callSegmentIDOrNone);
        else if (resourceAddressID == ResourceAddressID.CALL_SEGMENT_ID)
    ...
    public CalledPartyNumber getIPRoutingAddress()
        ...
        if (resourceAddressChoice == ResourceAddressChoice.IP_ROUTING_ADDRESS)
            return ipRoutingAddress;
        throw new ParameterNotSetException("IP Routing Address: not set.");
    }
    ...
    public LegID getLegID()
        throws ParameterNotSetException {
        if (resourceAddressChoice == ResourceAddressChoice.LEG_ID)
            return legID;
    ...
    public IPAddressAndLegID getIPAddressAndLegID()
        throws ParameterNotSetException {
        if (resourceAddressChoice == ResourceAddressChoice.IP_ADDRESS_AND_LEG_ID)
            return ipAddressAndLegID;
        throw new ParameterNotSetException("IP Address and Leg Id: not set.");
    }
    ...
    public int getNone()
        throws ParameterNotSetException {
        if (resourceAddressChoice == ResourceAddressChoice.NONE)
            return none;
        throw new ParameterNotSetException("None: not set.");
    }
    /** Sets None.  */
    ...
    public int getCallSegmentID()
        throws ParameterNotSetException {
        if (resourceAddressChoice == ResourceAddressChoice.CALL_SEGMENT_ID)
    ...
    public IPAddressAndCallSegmentID getIPAddressAndCallSegmentID()
        throws ParameterNotSetException {
        if (resourceAddressChoice == ResourceAddressChoice.IP_ADDRESS_AND_CALL_SEGMENT_ID)
            return ipAddressAndCallSegmentID;
        throw new ParameterNotSetException("IP Address and Call Segment Id: not set.");
    }

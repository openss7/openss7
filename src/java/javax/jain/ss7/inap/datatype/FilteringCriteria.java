/*
 @(#) $RCSfile: FilteringCriteria.java,v $ $Name:  $($Revision: 1.1.2.1 $) $Date: 2009-06-21 11:34:53 $ <p>
...
public class FilteringCriteria implements java.io.Serializable {
    ...
    public FilteringCriteria(DigitsGenericDigits number, FilterCriteriaID filterCriteriaID) {
        if (filterCriteriaID == FilterCriteriaID.DIALLED_NUMBER)
            setDialledNumber(number);
        else if (filterCriteriaID == FilterCriteriaID.CALLING_LINE_ID)
    ...
    /** Constructor For FilteringCriteria.  */
    public FilteringCriteria(int serviceKey)
    ...
    public DigitsGenericDigits getDialledNumber() throws ParameterNotSetException {
        ...
        throw new ParameterNotSetException("Dialed Number: not set.");
    }
    /** Sets Dialled Number.  */
    ...
    public DigitsGenericDigits getCallingLineID()
        throws ParameterNotSetException {
        if (filteringCriteriaChoice == FilteringCriteriaChoice.CALLING_LINE_ID)
            return callingLineID;
        throw new ParameterNotSetException("Calling Line Id: not set.");
    }
    ...
    public int getServiceKey()
        throws ParameterNotSetException {
        if (filteringCriteriaChoice == FilteringCriteriaChoice.SERVICE_KEY)
            return serviceKey;
    ...
    public AddressAndService getAddressAndService()
        throws ParameterNotSetException {
        if (filteringCriteriaChoice == FilteringCriteriaChoice.ADDRESS_AND_SERVICE)
            return addressAndService;
        throw new ParameterNotSetException("Address and Service: not set.");
    }

...
static char const ident[] = "src/util/mac.c (" PACKAGE_ENVR ") " PACKAGE_DATE;
...
Usage:\n\
    ...
    %1$s {-C|--copying}\n\
Options:\n\
  Command: (-s assumed if none given)\n\
    -s, --stats\n\
        list the MAC statistics\n\
    -h, --help, -?, --?\n\
...
main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		c = getopt_long_only(argc, argv, "snd::v::hVC?W:", long_options, &option_index);
#else				/* _GNU_SOURCE */
		c = getopt(argc, argv, "snqd:vhVC?");
#endif				/* _GNU_SOURCE */
		if (c == -1) {
			if (debug)

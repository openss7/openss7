...
enum MessageType {
	...
	ISUP_MT_CSVQ = 0x25,		// 0b00100101 - CUG Selection and Validation Request (not
	// ANSI)
	ISUP_MT_CSVR = 0x26,		// 0b00100110 - CUG Selection and Validation Response (not
	// ANSI)
...
enum ParameterType {
	...
	ISUP_PT_CGI = 21UL,		// 0x15 - 0b00010101 - Circuit group supervision msg type
	// ind
	ISUP_PT_RS = 22UL,		// 0x16 - 0b00010110 - Range and status
	ISUP_PT_CMI = 23UL,		// 0x17 - 0b00010111 - Call modification indicators (Blue
	// Book)
	ISUP_PT_FACI = 24UL,		// 0x18 - 0b00011000 - Facility indicator
	ISUP_PT_FAII = 25UL,		// 0x19 - 0b00011001 - Facility information indicators
	// (Bcore)
	...
	ISUP_PT_TMRP = 62UL,		// 0x3e - 0b00111110 - Transmission medium requirement
	// prime
	...
	ISUP_PT_SPR = 228UL,		// 0xe4 - 0b11100100 - Special processing request
	// (Bellcore)
	ISUP_PT_CGCI = 229UL,		// 0xe5 - 0b11100101 - Cc't group char ind (ANSI,Bellcore)
	ISUP_PT_CVRI = 230UL,		// 0xe6 - 0b11100110 - Cc't validation resp ind
	// (ANSI,Bellcore)
	ISUP_PT_OTGN = 231UL,		// 0xe7 - 0b11100111 - Outgoing trunk group numb
	// (ANSI,Bellcore)
	ISUP_PT_CIN = 232UL,		// 0xe8 - 0b11101000 - Circuit ident name (ANSI,Bellcore)
	ISUP_PT_CLLI = 233UL,		// 0xe9 - 0b11101001 - Common language loc id
	// (ANSI,Bellcore)
	ISUP_PT_OLI = 234UL,		// 0xea - 0b11101010 - Originating line info
	// (ANSI,Bellcore)
	ISUP_PT_CHGN = 235UL,		// 0xeb - 0b11101011 - Charge number (ANSI,Bellcore)
	ISUP_PT_SVCD = 236UL,		// 0xec - 0b11101100 - Service code indicator
	// (ANSI,Bellcore)
	ISUP_PT_CSEL = 238UL,		// 0xee - 0b11101110 - Carrier selection info
	// (ANSI,Bellcore)
	ISUP_PT_ORI = 243UL,		// 0xf3 - 0b11110011 - Outgoing route identification
	// (Spain)
	ISUP_PT_IRI = 244UL,		// 0xf4 - 0b11110100 - Incoming route identification
	// (Spain)
	ISUP_PT_RATE = 248UL,		// 0xf8 - 0b11111000 - Rate (Spain)
	ISUP_PT_IIC = 249UL,		// 0xf9 - 0b11111001 - Identifier of incoming circuit
	// (Spain)
	ISUP_PT_TOI = 253UL,		// 0xfd - 0b11111101 - Trunk offering information
	// (Singapore)
...
void dump_oparms(unsigned char *p, unsigned char *e)
{
	...
	for (;;) {
		...
		case ISUP_PT_EOP:
			return;
		case ISUP_PT_CLRF:
			fprintf(stdout, ", CLRF: 0x");
			break;
		case ISUP_PT_TMR:
			fprintf(stdout, ", TMR: 0x");
			break;
		case ISUP_PT_ATP:
			fprintf(stdout, ", ATP: 0x");
			break;
		case ISUP_PT_CDPN:
			...
			break;
		case ISUP_PT_SUBN:
			fprintf(stdout, ", SUBN: 0x");
			break;
		case ISUP_PT_NCI:
			fprintf(stdout, ", NCI: 0x");
			break;
		case ISUP_PT_FCI:
			fprintf(stdout, ", FCI: 0x");
			break;
		case ISUP_PT_OFCI:
			fprintf(stdout, ", OFCI: 0x");
			break;
		case ISUP_PT_CPC:
			fprintf(stdout, ", CPC: 0x");
			break;
		case ISUP_PT_CGPN:
			fprintf(stdout, ", CGPN: 0x");
			len = *p++ & 0xff;
			odd = (*p & 0x80) >> 7;
			for (i = 0; i < 2; i++)
				fprintf(stdout, "%02x", *p++);
			...
			continue;
		case ISUP_PT_RDGN:
			fprintf(stdout, ", RDGN: 0x");
			break;
		case ISUP_PT_RDNN:
			fprintf(stdout, ", RDNN: 0x");
			break;
		case ISUP_PT_CONR:
			fprintf(stdout, ", CONR: 0x");
			break;
		case ISUP_PT_INRI:
			fprintf(stdout, ", INRI: 0x");
			break;
		case ISUP_PT_INFI:
			fprintf(stdout, ", INFI: 0x");
			break;
		case ISUP_PT_COTI:
			fprintf(stdout, ", COTI: 0x");
			break;
		case ISUP_PT_BCI:
			fprintf(stdout, ", BCI: 0x");
			break;
		case ISUP_PT_CAUS:
			fprintf(stdout, ", CAUS: 0x");
			break;
		case ISUP_PT_RDI:
			fprintf(stdout, ", RDI: 0x");
			break;
		case ISUP_PT_CGI:
			fprintf(stdout, ", CGI: 0x");
			break;
		case ISUP_PT_RS:
			fprintf(stdout, ", RS: 0x");
			break;
		case ISUP_PT_CMI:
			fprintf(stdout, ", CMI: 0x");
			break;
		case ISUP_PT_FACI:
			fprintf(stdout, ", FACI: 0x");
			break;
		case ISUP_PT_FAII:
			fprintf(stdout, ", FAII: 0x");
			break;
		case ISUP_PT_CUGI:
			fprintf(stdout, ", CUGI: 0x");
			break;
		case ISUP_PT_INDEX:
			fprintf(stdout, ", INDEX: 0x");
			break;
		case ISUP_PT_USI:
			fprintf(stdout, ", USI: 0x");
			break;
		case ISUP_PT_SPC:
			fprintf(stdout, ", SPC: 0x");
			break;
		case ISUP_PT_UUI:
			fprintf(stdout, ", UUI: 0x");
			break;
		case ISUP_PT_CONN:
			fprintf(stdout, ", CONN: 0x");
			break;
		case ISUP_PT_SRIS:
			fprintf(stdout, ", SRIS: 0x");
			break;
		case ISUP_PT_TNS:
			fprintf(stdout, ", TNS: 0x");
			break;
		case ISUP_PT_EVNT:
			fprintf(stdout, ", EVNT: 0x");
			break;
		case ISUP_PT_CSI:
			fprintf(stdout, ", CSI: 0x");
			break;
		case ISUP_PT_ACL:
			fprintf(stdout, ", ACL: 0x");
			break;
		case ISUP_PT_OCDN:
			fprintf(stdout, ", OCDN: 0x");
			break;
		case ISUP_PT_OBCI:
			fprintf(stdout, ", OBCI: 0x");
			break;
		case ISUP_PT_UUIND:
			fprintf(stdout, ", UUIND: 0x");
			break;
		case ISUP_PT_ISPC:
			fprintf(stdout, ", ISPC: 0x");
			break;
		case ISUP_PT_GNOT:
			fprintf(stdout, ", GNOT: 0x");
			break;
		case ISUP_PT_CHI:
			fprintf(stdout, ", CMI: 0x");
			break;
		case ISUP_PT_ADI:
			fprintf(stdout, ", ADI: 0x");
			break;
		case ISUP_PT_NSF:
			fprintf(stdout, ", NSF: 0x");
			break;
		case ISUP_PT_USIP:
			fprintf(stdout, ", USIP: 0x");
			break;
		case ISUP_PT_PROP:
			fprintf(stdout, ", PROP: 0x");
			break;
		case ISUP_PT_ROPS:
			fprintf(stdout, ", ROPS: 0x");
			break;
		case ISUP_PT_SA:
			fprintf(stdout, ", SA: 0x");
			break;
		case ISUP_PT_UTI:
			fprintf(stdout, ", UTI: 0x");
			break;
		case ISUP_PT_TMU:
			fprintf(stdout, ", TMU: 0x");
			break;
		case ISUP_PT_CDI:
			fprintf(stdout, ", CDI: 0x");
			break;
		case ISUP_PT_ECI:
			fprintf(stdout, ", ECI: 0x");
			break;
		case ISUP_PT_MCI:
			fprintf(stdout, ", MCI: 0x");
			break;
		case ISUP_PT_PCI:
			fprintf(stdout, ", PCI: 0x");
			break;
		case ISUP_PT_MLPP:
			fprintf(stdout, ", MLPP: 0x");
			break;
		case ISUP_PT_MCIQ:
			fprintf(stdout, ", MCIQ: 0x");
			break;
		case ISUP_PT_MCIR:
			fprintf(stdout, ", MCIR: 0x");
			break;
		case ISUP_PT_HOPC:
			fprintf(stdout, ", HOPC: 0x");
			break;
		case ISUP_PT_TMRP:
			fprintf(stdout, ", TMRP: 0x");
			break;
		case ISUP_PT_LN:
			fprintf(stdout, ", LN: 0x");
			break;
		case ISUP_PT_RDNR:
			fprintf(stdout, ", RDNR: 0x");
			break;
		case ISUP_PT_FREEP:
			fprintf(stdout, ", FREEP: 0x");
			break;
		case ISUP_PT_GREF:
			fprintf(stdout, ", GREF: 0x");
			break;
		case ISUP_PT_GNUM:
			fprintf(stdout, ", GNUM: 0x");
			break;
		case ISUP_PT_GDIG:
			fprintf(stdout, ", GDIG: 0x");
			break;
		case ISUP_PT_EGRESS:
			fprintf(stdout, ", EGRESS: 0x");
			break;
		case ISUP_PT_JUR:
			fprintf(stdout, ", JUR: 0x");
			break;
		case ISUP_PT_CIDC:
			fprintf(stdout, ", CIDC: 0x");
			break;
		case ISUP_PT_BGROUP:
			fprintf(stdout, ", BGROUP: 0x");
			break;
		case ISUP_PT_NOTI:
			fprintf(stdout, ", NOTI: 0x");
			break;
		case ISUP_PT_SVACT:
			fprintf(stdout, ", SVACT: 0x");
			break;
		case ISUP_PT_TRNSRQ:
			fprintf(stdout, ", TRNSRQ: 0x");
			break;
		case ISUP_PT_SPR:
			fprintf(stdout, ", SPR: 0x");
			break;
		case ISUP_PT_CGCI:
			fprintf(stdout, ", CGCI: 0x");
			break;
		case ISUP_PT_CVRI:
			fprintf(stdout, ", CVRI: 0x");
			break;
		case ISUP_PT_OTGN:
			fprintf(stdout, ", OTGN: 0x");
			break;
		case ISUP_PT_CIN:
			fprintf(stdout, ", CIN: 0x");
			break;
		case ISUP_PT_CLLI:
			fprintf(stdout, ", CLLI: 0x");
			break;
		case ISUP_PT_OLI:
			fprintf(stdout, ", OLI: 0x");
			break;
		case ISUP_PT_CHGN:
			fprintf(stdout, ", CHGN: 0x");
			break;
		case ISUP_PT_SVCD:
			fprintf(stdout, ", SVCD: 0x");
			break;
		case ISUP_PT_CSEL:
			fprintf(stdout, ", CSEL: 0x");
			break;
		default:
			fprintf(stdout, ", [0x%02x]: 0x", pt);
			break;
		}
		len = *p++ & 0xff;
		for (i = 0; i < len; i++)
...
Options:\n\
    ...
    -p, --parfile parfile             (default: none)\n\

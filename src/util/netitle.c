...
static char const ident[] = "src/util/netitle.c (" PACKAGE_ENVR ") " PACKAGE_DATE;
...
static char netitle[BUFSIZ + 1] = "";

static void
do_get(int argc, char *argv[], int start)
{
	(void) argc;
	(void) argv;
	(void) start;
...
static void
do_set(int argc, char *argv[], int start)
{
	(void) argc;
	(void) argv;
	(void) start;
...
Arguments:\n\
    [TITLE]\n\
        the network entity title to set\n\
Options:\n\
  Command:\n\
    -h, --help, -?, --?\n\
...
main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		c = getopt_long_only(argc, argv, "nd::v::hVC?W:", long_options, &option_index);
#else				/* _GNU_SOURCE */
		c = getopt(argc, argv, "nqd:vhVC?");
#endif				/* _GNU_SOURCE */
		if (c == -1) {
			if (debug)
	...
	/* Network Entity Title argument */
	if (debug)
		fprintf(stderr, "%s: testing NE title\n", argv[0]);
	start = optind;
	if (optind < argc) {
		...
		strncpy(netitle, argv[optind], BUFSIZ);
		/* check this later */
		optind++;
		/* check for hexadecimal digits only */
		if ((bad = strspn(netitle, "0123456789abcdefABCDEF")) < len) {
			if (output || debug)
				fprintf(stderr,
	...
	switch (command) {
	case COMMAND_DFLT:
		do_get(argc, argv, start);
		break;
	case COMMAND_TITL:
		do_set(argc, argv, start);
		break;
	case COMMAND_HELP:
		help(argv);
		break;

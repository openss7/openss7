/*****************************************************************************
 ...
 @(#) $RCSfile: dlcommon.c,v $ $Name:  $($Revision: 1.1.2.1 $) $Date: 2009-06-21 11:47:54 $
...
#ident "@(#) $RCSfile: dlcommon.c,v $ $Name:  $($Revision: 1.1.2.1 $) $Date: 2009-06-21 11:47:54 $"

static char const ident[] = "$RCSfile: dlcommon.c,v $ $Name:  $($Revision: 1.1.2.1 $) $Date: 2009-06-21 11:47:54 $";
...
void dlattachreq(int fd, ulong ppa);
void dlenabmultireq(int fd, char *addr, int length);
void dldisabmultireq(int fd, char *addr, int length);
void dlpromisconreq(int fd, ulong level);
void dlpromiscoff(int fd, ulong level);
void dlphysaddrreq(int fd, ulong addrtype);
void dlsetphysaddrreq(int fd, char *addr, int length);
void dldetachreq(int fd);
void dlbindreq(int fd, ulong sap, ulong max_conind, ulong service_mode, ulong conn_mgmt, ulong xidtest);
void dlunitdatareq(int fd, uchar *addrp, int addrlen, ulong minpri, ulong maxpri, uchar *datap, int datalen);
void dlunbindreq(int fd);
void dlokack(int fd, char *bufp);
void dlerrorack(int fd, char *bufp);
...
void printdltestreq(union DL_primitives *dlp);
void printdltestind(union DL_primitives *dlp);
void printdltestres(union DL_primitives *dlp);
void printdltestcon(union DL_primitives *dlp);
void printdlxidreq(union DL_primitives *dlp);
void printdlxidind(union DL_primitives *dlp);
void printdlxidres(union DL_primitives *dlp);
void printdlxidcon(union DL_primitives *dlp);
void printdludqosreq(union DL_primitives *dlp);
void addrtostring(uchar *addr, ulong length, uchar *s);
int stringtoaddr(char *sp, char *addr);
static char hexnibble(char c);
char *dlprim(ulong prim);
char *dlstate(ulong state);
char *dlerrno(ulong errno);
char *dlpromisclevel(ulong level);
char *dlservicemode(ulong servicemode);
char *dlstyle(long style);
char *dlmactype(ulong media);
void err(char *fmt, char *a1, char *a2, char *a3, char *a4);
void syserr(char *s);
void strioctl(int fd, int cmd, int timout, int len, char *dp);
...
expecting(prim, dlp)
	...
	union DL_primitives *dlp;
{
	if (dlp->dl_primitive != (ulong) prim) {
		printdlprim(dlp);
		err("expected %s got %s", dlprim(prim), dlprim(dlp->dl_primitive));
		exit(1);
	}
}
	...
	union DL_primitives *dlp;
{
	...
	default:
		err("printdlprim:  unknown primitive type 0x%x", dlp->dl_primitive);
		break;
	}
}
...
printdlinfoack(dlp)
	union DL_primitives *dlp;
{
	uchar addr[MAXDLADDR];
	uchar brdcst[MAXDLADDR];
	...
	(void) printf("DL_INFO_ACK:  max_sdu %d min_sdu %d\n",
		      dlp->info_ack.dl_max_sdu, dlp->info_ack.dl_min_sdu);
	...
	(void) printf("sap_length %d service_mode %s qos_length %d\n",
		      ...
		      dlservicemode(dlp->info_ack.dl_service_mode), dlp->info_ack.dl_qos_length);
	(void) printf("qos_offset %d qos_range_length %d qos_range_offset %d\n",
		      dlp->info_ack.dl_qos_offset,
		      dlp->info_ack.dl_qos_range_length, dlp->info_ack.dl_qos_range_offset);
	(void) printf("provider_style %s addr_offset %d version %d\n",
		      dlstyle(dlp->info_ack.dl_provider_style),
		      dlp->info_ack.dl_addr_offset, dlp->info_ack.dl_version);
	(void) printf("brdcst_addr_length %d brdcst_addr_offset %d\n",
		      dlp->info_ack.dl_brdcst_addr_length, dlp->info_ack.dl_brdcst_addr_offset);
	(void) printf("addr %s\n", addr);
	...
	union DL_primitives *dlp;
{
	(void) printf("DL_ERROR_ACK:  error_primitive %s errno %s unix_errno %d\n",
		      ...
		      dlerrno(dlp->error_ack.dl_errno), dlp->error_ack.dl_unix_errno);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_ENABMULTI_REQ:  addr_length %d addr_offset %d\n",
		      dlp->enabmulti_req.dl_addr_length, dlp->enabmulti_req.dl_addr_offset);
	(void) printf("addr %s\n", addr);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_DISABMULTI_REQ:  addr_length %d addr_offset %d\n",
		      dlp->disabmulti_req.dl_addr_length, dlp->disabmulti_req.dl_addr_offset);
	(void) printf("addr %s\n", addr);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_PHYS_ADDR_ACK:  addr_length %d addr_offset %d\n",
		      dlp->physaddr_ack.dl_addr_length, dlp->physaddr_ack.dl_addr_offset);
	(void) printf("addr %s\n", addr);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_SET_PHYS_ADDR_REQ:  addr_length %d addr_offset %d\n",
		      dlp->set_physaddr_req.dl_addr_length, dlp->set_physaddr_req.dl_addr_offset);
	(void) printf("addr %s\n", addr);
}
	...
	union DL_primitives *dlp;
{
	(void) printf("DL_BIND_REQ:  sap %d max_conind %d\n",
		      dlp->bind_req.dl_sap, dlp->bind_req.dl_max_conind);
	(void) printf("service_mode %s conn_mgmt %d xidtest_flg 0x%x\n",
		      dlservicemode(dlp->bind_req.dl_service_mode),
		      dlp->bind_req.dl_conn_mgmt, dlp->bind_req.dl_xidtest_flg);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_BIND_ACK:  sap %d addr_length %d addr_offset %d\n",
		      dlp->bind_ack.dl_sap,
		      dlp->bind_ack.dl_addr_length, dlp->bind_ack.dl_addr_offset);
	(void) printf("max_conind %d xidtest_flg 0x%x\n",
		      dlp->bind_ack.dl_max_conind, dlp->bind_ack.dl_xidtest_flg);
	(void) printf("addr %s\n", addr);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_SUBS_BIND_REQ:  subs_sap_offset %d sub_sap_len %d\n",
		      dlp->subs_bind_req.dl_subs_sap_offset, dlp->subs_bind_req.dl_subs_sap_length);
	(void) printf("sap %s\n", sap);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_SUBS_BIND_ACK:  subs_sap_offset %d sub_sap_length %d\n",
		      dlp->subs_bind_ack.dl_subs_sap_offset, dlp->subs_bind_ack.dl_subs_sap_length);
	(void) printf("sap %s\n", sap);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_UNITDATA_REQ:  dest_addr_length %d dest_addr_offset %d\n",
		      dlp->unitdata_req.dl_dest_addr_length, dlp->unitdata_req.dl_dest_addr_offset);
	(void) printf("dl_priority.min %d dl_priority.max %d\n",
		      dlp->unitdata_req.dl_priority.dl_min, dlp->unitdata_req.dl_priority.dl_max);
	(void) printf("addr %s\n", addr);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_UNITDATA_IND:  dest_addr_length %d dest_addr_offset %d\n",
		      dlp->unitdata_ind.dl_dest_addr_length, dlp->unitdata_ind.dl_dest_addr_offset);
	(void) printf("src_addr_length %d src_addr_offset %d\n",
		      dlp->unitdata_ind.dl_src_addr_length, dlp->unitdata_ind.dl_src_addr_offset);
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_UDERROR_IND:  dest_addr_length %d dest_addr_offset %d\n",
		      dlp->uderror_ind.dl_dest_addr_length, dlp->uderror_ind.dl_dest_addr_offset);
	(void) printf("unix_errno %d errno %s\n",
		      dlp->uderror_ind.dl_unix_errno, dlerrno(dlp->uderror_ind.dl_errno));
	(void) printf("addr %s\n", addr);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_TEST_REQ:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		      ...
		      dlp->test_req.dl_dest_addr_length, dlp->test_req.dl_dest_addr_offset);
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_TEST_IND:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		      ...
		      dlp->test_ind.dl_dest_addr_length, dlp->test_ind.dl_dest_addr_offset);
	(void) printf("src_addr_length %d src_addr_offset %d\n",
		      dlp->test_ind.dl_src_addr_length, dlp->test_ind.dl_src_addr_offset);
	(void) printf("dest_addr %s\n", dest);
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_TEST_RES:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		      ...
		      dlp->test_res.dl_dest_addr_length, dlp->test_res.dl_dest_addr_offset);
	(void) printf("dest_addr %s\n", dest);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_TEST_CON:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		      ...
		      dlp->test_con.dl_dest_addr_length, dlp->test_con.dl_dest_addr_offset);
	(void) printf("src_addr_length %d src_addr_offset %d\n",
		      dlp->test_con.dl_src_addr_length, dlp->test_con.dl_src_addr_offset);
	(void) printf("dest_addr %s\n", dest);
	(void) printf("src_addr %s\n", src);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_XID_REQ:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		      dlp->xid_req.dl_flag,
		      dlp->xid_req.dl_dest_addr_length, dlp->xid_req.dl_dest_addr_offset);
	(void) printf("dest_addr %s\n", dest);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_XID_IND:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		      dlp->xid_ind.dl_flag,
		      dlp->xid_ind.dl_dest_addr_length, dlp->xid_ind.dl_dest_addr_offset);
	(void) printf("src_addr_length %d src_addr_offset %d\n",
		      dlp->xid_ind.dl_src_addr_length, dlp->xid_ind.dl_src_addr_offset);
	(void) printf("dest_addr %s\n", dest);
	(void) printf("src_addr %s\n", src);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_XID_RES:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		      dlp->xid_res.dl_flag,
		      dlp->xid_res.dl_dest_addr_length, dlp->xid_res.dl_dest_addr_offset);
	(void) printf("dest_addr %s\n", dest);
}
	...
	union DL_primitives *dlp;
{
	...
	(void) printf("DL_XID_CON:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		      dlp->xid_con.dl_flag,
		      dlp->xid_con.dl_dest_addr_length, dlp->xid_con.dl_dest_addr_offset);
	(void) printf("src_addr_length %d src_addr_offset %d\n",
		      dlp->xid_con.dl_src_addr_length, dlp->xid_con.dl_src_addr_offset);
	(void) printf("dest_addr %s\n", dest);
	(void) printf("src_addr %s\n", src);
}
	...
	union DL_primitives *dlp;
{
	(void) printf("DL_UDQOS_REQ:  qos_length %d qos_offset %d\n",
		      dlp->udqos_req.dl_qos_length, dlp->udqos_req.dl_qos_offset);
}
	...
	ulong servicemode;
{
	...
	default:
		(void) sprintf(servicemodebuf, "unknown provider service mode 0x%x", servicemode);

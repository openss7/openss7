#!/bin/sh
#
# @(#) $RCSfile: striso.sh,v $ $Name:  $($Revision: 0.9.2.9 $) $Date: 2008-10-21 22:42:17 $
...
#		that the STREAMS ISO subsystem is configured and loaded.
#
...
# Short-Description: start and stop STREAMS ISO subsystem
# License: GPL
# Description:	This STREAMS ISO init script is part of Linux Fast-STREAMS.
#	It is reponsible for ensuring that the necessary STREAMS ISO character
#	devices are present in the /dev directory and that the STREAMS ISO
...
desc="the STREAMS ISO subsystem"
mknod="${name}_mknod"

[ -e /proc/modules ] || exit 0

if test -z "$STRISO_MKNOD" ; then
    for STRISO_MKNOD in /sbin/${mknod} /usr/sbin/${mknod} /bin/${mknod} /usr/bin/${mknod} ; do
	if [ -x $STRISO_MKNOD ] ; then
	    break
	else
	    STRISO_MKNOD=
	fi
...
[ -n "$STRISO_PRELOAD"       ] || STRISO_PRELOAD=""
[ -n "$STRISO_DRIVERS"       ] || STRISO_DRIVERS="streams-clns streams-tp streams-x25-lapb streams-x25-plp"
[ -n "$STRISO_MODULES"       ] || STRISO_MODULES="streams-cmot streams-isot streams-itot streams-lpp streams-tcpns streams-xot"
[ -n "$STRISO_MAKEDEVICES"   ] || STRISO_MAKEDEVICES="yes"
[ -n "$STRISO_REMOVEDEVICES" ] || STRISO_REMOVEDEVICES="yes"
...
[ -z "$STRISO_MKNOD" ] && STRISO_MAKEDEVICES="no"
[ -z "$STRISO_MKNOD" ] && STRISO_REMOVEDEVICES="no"
...
start() {
    ...
    RETVAL=0
    modules=
    for module in $STRISO_PRELOAD ; do
    ...
    if [ -n "$STRISO_MKNOD" -a ":$STRISO_MAKEDEVICES" = ":yes" ] ; then
	echo -n "Making STREAMS ISO devices: "
	$STRISO_MKNOD
	if [ $? -eq 0 ] ; then
	    echo "."
	else
...
stop() {
    ...
    RETVAL=0
    if [ -n "$STRISO_MKNOD" -a ":$STRISO_REMOVEDEVICES" = ":yes" ] ; then
	echo -n "Removing STREAMS ISO devices: "
	$STRISO_MKNOD --remove
	if [ $? -eq 0 ] ; then
	    echo "."
	else
    ...
    modules=
    for module in $STRISO_PRELOAD $STRISO_DRIVERS $STRISO_MODULES ; do
...
# -----------------------------------------------------------------------------
#
# Last Modified $Date: 2008-10-21 22:42:17 $ by $Author: brian $
#
# -----------------------------------------------------------------------------
#
# $Log: striso.sh,v $
...
#
# Revision 0.9.2.8  2008-04-29 00:02:06  brian

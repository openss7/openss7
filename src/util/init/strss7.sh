#!/bin/sh
#
# @(#) $RCSfile: strss7.sh,v $ $Name:  $($Revision: 0.9.2.8 $) $Date: 2008-10-21 22:42:13 $
...
#
# strss7:	start and stop strss7 subsystem
# update-rc.d:	start 33 S . stop 33 0 6 .
# config:	/etc/default/strss7
# probe:	false
# hide:		false
# license:	GPL
...
#		that the STREAMS SS7 subsystem is configured and loaded.
#
...
# Short-Description: start and stop STREAMS SS7 subsystem
# License: GPL
# Description:	This STREAMS SS7 init script is part of Linux Fast-STREAMS.
#	It is reponsible for ensuring that the necessary STREAMS SS7 character
#	devices are present in the /dev directory and that the STREAMS SS7
...
desc="the STREAMS SS7 subsystem"
mknod="${name}_mknod"

[ -e /proc/modules ] || exit 0

if test -z "$STRSS7_MKNOD" ; then
    for STRSS7_MKNOD in /sbin/${mknod} /usr/sbin/${mknod} /bin/${mknod} /usr/bin/${mknod} ; do
	if [ -x $STRSS7_MKNOD ] ; then
	    break
	else
	    STRSS7_MKNOD=
	fi
...
[ -n "$STRSS7_PRELOAD"       ] || STRSS7_PRELOAD=""
[ -n "$STRSS7_DRIVERS"       ] || STRSS7_DRIVERS="streams-sl-x400p"
[ -n "$STRSS7_MODULES"       ] || STRSS7_MODULES=""
[ -n "$STRSS7_MAKEDEVICES"   ] || STRSS7_MAKEDEVICES="yes"
[ -n "$STRSS7_REMOVEDEVICES" ] || STRSS7_REMOVEDEVICES="yes"
...
[ -z "$STRSS7_MKNOD" ] && STRSS7_MAKEDEVICES="no"
[ -z "$STRSS7_MKNOD" ] && STRSS7_REMOVEDEVICES="no"
...
start() {
    ...
    RETVAL=0
    modules=
    for module in $STRSS7_PRELOAD ; do
    ...
    if [ -n "$STRSS7_MKNOD" -a ":$STRSS7_MAKEDEVICES" = ":yes" ] ; then
	echo -n "Making STREAMS SS7 devices: "
	$STRSS7_MKNOD
	if [ $? -eq 0 ] ; then
	    echo "."
	else
...
stop() {
    ...
    RETVAL=0
    if [ -n "$STRSS7_MKNOD" -a ":$STRSS7_REMOVEDEVICES" = ":yes" ] ; then
	echo -n "Removing STREAMS SS7 devices: "
	$STRSS7_MKNOD --remove
	if [ $? -eq 0 ] ; then
	    echo "."
	else
...
# -----------------------------------------------------------------------------
#
# Last Modified $Date: 2008-10-21 22:42:13 $ by $Author: brian $
#
# -----------------------------------------------------------------------------
#
# $Log: strss7.sh,v $
# Revision 0.9.2.8  2008-10-21 22:42:13  brian
# - handle verbose better in debian initscripts
#
# Revision 0.9.2.7  2008-04-29 07:11:34  brian
# - updating headers for release
#
# Revision 0.9.2.6  2007/08/12 16:49:59  brian
# - header updates
#
# =============================================================================

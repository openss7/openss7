...
static char const ident[] = "src/util/x400yac.y (" PACKAGE_ENVR " " PACKAGE_DATE ")";
...
device_selection: { $<val>$ = 0; }
    ...
    { if ($<val>2)
	{ $<val>$ = 1; cur.levl = 1; strncpy(cur.device,$<str>2,MAX_BUF_SIZE - 1); }
	else
	{ $<val>$ = 0; }
    }
	...
	while (1) {
		...
		case 'f':	/* -f, --file [FILENAME] */
			if (cmd != COMMAND_NONE)
			    goto bad_option;
			if (optarg != NULL)
			    strncpy(filename,optarg,MAX_BUF_SIZE - 1);
			cmd = COMMAND_FILE;
			break;
		case 'p':	/* -p, --ppa PPA */
		...
		case 'c':	/* -c, --clei CLEI */
			...
			if (optarg == NULL || sel.clli[0] != '\0')
			    goto bad_option;
			strncpy(sel.clli,optarg,MAX_BUF_SIZE - 1);
			break;
		case 'q':	/* -q, --quiet */

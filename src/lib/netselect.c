...
static char const ident[] __attribute__ ((unused)) = "src/lib/netselect.c (" PACKAGE_ENVR ") " PACKAGE_DATE;
...
__nsl_nc_sperror(void)
{
	...
	if (errbuf != NULL) {
		...
		case NC_NOMEM:
		case NC_NOSET:
			(void) strncpy(errbuf, gettext(__nsl_nc_errlist[idx]), NCERR_BUFSZ - 1);
			break;
		case NC_OPENFAIL:
		case NC_NOTFOUND:

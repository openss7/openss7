/*****************************************************************************
 ...
 @(#) $Id: test6.c,v 0.9.2.2 2001/04/17 01:43:57 brian Exp $
 ...
 Last Modified $Date: 2001/04/17 01:43:57 $ by $Author: brian $

 -----------------------------------------------------------------------------

 $Log: test6.c,v $
 Revision 0.9.2.2  2001/04/17 01:43:57  brian
 Initial revision

 Revision 0.1  2001/04/17 01:43:57  brian
 Initial revision

 *****************************************************************************/

static char const ident[] = "$Name:  $($Revision: 0.9.2.2 $) $Date: 2001/04/17 01:43:57 $";
...
//	fprintf(stderr, "Testing single bind:\n");
	if ( !test_multi_bind(6,5,2) ) test_connect(6,5,2);
	if ( !test_multi_bind(5,3,2) ) test_connect(5,3,2);
	if ( !test_multi_bind(4,1,2) ) test_connect(4,1,2);
	if ( !test_multi_bind(3,4,2) ) test_connect(3,4,2);
	if ( !test_multi_bind(2,2,2) ) test_connect(2,2,2);
	if ( !test_multi_bind(1,0,2) ) test_connect(1,0,2);
	if ( !test_multi_bind(0,0,2) ) test_connect(0,0,2);

/*****************************************************************************
 ...
 @(#) $Id: etsi5.c,v 0.9.2.2 2001/04/26 09:04:00 brian Exp $
 ...
 Last Modified $Date: 2001/04/26 09:04:00 $ by $Author: brian $

 -----------------------------------------------------------------------------

 $Log: etsi5.c,v $
 Revision 0.9.2.2  2001/04/26 09:04:00  brian
 Initial revision

 Revision 0.1  2001/04/26 09:04:00  brian
 Initial revision

 *****************************************************************************/

static char const ident[] = "$Name:  $($Revision: 0.9.2.2 $) $Date: 2001/04/26 09:04:00 $";
...
inline int test_send(int n)
{
//	fprintf(stderr, "Sending on accepted socket %d\n", n);
//	if ( send(s[n], "This is a test!", 16, MSG_OOB|MSG_NOSIGNAL) < 0 )
	if ( send(s[n], "This is a test!", 16, MSG_NOSIGNAL|MSG_DONTWAIT) < 0 )
		return show_error(__FUNCTION__);
	return 0;
}

inline int test_read(int n)
{
	int len;
	char *buf[100];
//	fprintf(stderr, "Reading from connected socket %d\n", n);
	if ( (len = recv(s[n], buf, 100, MSG_WAITALL|MSG_NOSIGNAL|MSG_DONTWAIT)) < 0 )
		return show_error(__FUNCTION__);
...
main() {
	...
	inet_aton("192.168.3.1",	    &addr[0].sin_addr);
	inet_aton("192.169.3.1",	    &addr[1].sin_addr);
...
#define TEST_PORT 10000
	addr[0].sin_port = htons(1);
	addr[1].sin_port = htons(1);

/*****************************************************************************
 ...
 @(#) $RCSfile: test-sctps.c,v $ $Name:  $($Revision: 0.9.2.1 $) $Date: 2002/05/20 05:00:03 $
...
#ident "@(#) $RCSfile: test-sctps.c,v $ $Name:  $($Revision: 0.9.2.1 $) $Date: 2002/05/20 05:00:03 $"

static char const ident[] = "$RCSfile: test-sctps.c,v $ $Name:  $($Revision: 0.9.2.1 $) $Date: 2002/05/20 05:00:03 $";
...
//      unsigned char my_msg[] = "This is a good short test message that has some 64 bytes in it.";
	...
	}
	{
		int val = 0;
		int vlen = sizeof(val);
		if (setsockopt(fd, SOL_SCTP, SCTP_NODELAY, &val, vlen) < 0) {
			perror("setsockopt");
			goto dead;
		}
	}

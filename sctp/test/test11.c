/*****************************************************************************
 ...
 @(#) $Id: test11.c,v 0.9.2.3 2002/05/14 09:38:55 brian Exp $
 ...
 Revision 0.1.1.1  2001/04/25 01:51:39  brian
...
#define TEST_PORT 10000
	...
	fprintf(stderr, "Sending data in 10 seconds...\n");
	fflush(stderr);
	sleep(10);
	...
	for (i = 0; i < 10; i++)
      ...
      dead:
	if (s[1] > 0) {
		fprintf(stderr, "Closing connected socket in 10 seconds...\n");
		fflush(stderr);
		sleep(10);
		close(s[1]);
	}
	if (s[0] > 0) {
		fprintf(stderr, "Closing listening socket in 10 seconds...\n");
		fflush(stderr);
		sleep(10);
		close(s[0]);
	}
	if (s[2] > 0) {
		fprintf(stderr, "Closing accepted socket in 10 seconds...\n");
		fflush(stderr);
		sleep(10);
		close(s[2]);
	}

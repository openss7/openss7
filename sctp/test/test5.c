/*****************************************************************************
 ...
 @(#) $Id: test5.c,v 0.9.2.2 2001/04/16 05:04:37 brian Exp $
 ...
 Last Modified $Date: 2001/04/16 05:04:37 $ by $Author: brian $

 -----------------------------------------------------------------------------

 $Log: test5.c,v $
 Revision 0.9.2.2  2001/04/16 05:04:37  brian
 Initial revision

 Revision 0.1  2001/04/16 05:04:37  brian
 Initial revision

 *****************************************************************************/

static char const ident[] = "$Name:  $($Revision: 0.9.2.2 $) $Date: 2001/04/16 05:04:37 $";
...
int test_multi_bind(int n, int a, int m)
{
	...
	for ( i=0; i<m; i++ )
		printf(" %s:%d", inet_ntoa(addr[a+i].sin_addr),
...
//	printf("Testing single bind:\n");
	if ( !test_multi_bind(6,5,2) ) test_listen(6);
	if ( !test_multi_bind(5,3,2) ) test_listen(5);
	if ( !test_multi_bind(4,1,2) ) test_listen(4);
	if ( !test_multi_bind(3,4,2) ) test_listen(3);
	if ( !test_multi_bind(2,2,2) ) test_listen(2);
	if ( !test_multi_bind(1,0,2) ) test_listen(1);
	if ( !test_multi_bind(0,0,2) ) test_listen(0);

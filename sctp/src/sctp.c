/*****************************************************************************
 ...
 @(#) $RCSfile: sctp.c,v $ $Name:  $($Revision: 0.9.2.34 $) $Date: 2006/07/16 12:46:24 $
 ...
 Last Modified $Date: 2006/07/16 12:46:24 $ by $Author: brian $

 *****************************************************************************/

#ident "@(#) $RCSfile: sctp.c,v $ $Name:  $($Revision: 0.9.2.34 $) $Date: 2006/07/16 12:46:24 $"
...
#define SCTP_DESCRIP	"SCTP/IP (RFC 2960) FOR LINUX NET4 $Name:  $($Revision: 0.9.2.34 $)"
#define SCTP_EXTRA	"Part of the OpenSS7 Stack for Linux."
#define SCTP_REVISION	"OpenSS7 $RCSfile: sctp.c,v $ $Name:  $($Revision: 0.9.2.34 $) $Date: 2006/07/16 12:46:24 $"
...
__sctp_inherit_port(struct sock *sk, struct sock *lsk)
{
	...
	if ((sb = SCTP_PROT(lsk)->bindb))
		___sctp_bhash_insert(SCTP_PROT(sk), sb);
	else
		swerr();
...
sctp_recv_abort_listening(struct sock *sk, struct sk_buff *skb)
{
	...
	for (cp = skb_peek(&sp->conq); cp; cp = skb_next(cp)) {
		...
		if (ck->v_tag == sh->v_tag && ck->sport == sh->dest && ck->dport == sh->srce
			...
			printd(("ck->v_tag = %08x, sh->v_tag = %08x\n", (int) ck->v_tag,
				(int) sh->v_tag));
			printd(("ck->sport = %d, sh->dest = %d\n", (int) ntohs(ck->sport),
				(int) ntohs(sh->dest)));
			printd(("ck->dport = %d, sh->srce = %d\n", (int) ntohs(ck->dport),
				(int) ntohs(sh->srce)));
			printd(("ck->saddr = %08x, iph->daddr = %08x\n", (int) ck->saddr,
				(int) iph->daddr));
			printd(("ck->daddr = %08x, iph->saddr = %08x\n", (int) ck->daddr,
				(int) iph->saddr));
		}
	}
	usual(cp);
...
sctp_recv_msg(struct sock *sk, struct sk_buff *skb)
{
	...
	ptrace(("ERROR: message size = %d, chunk length = %d\n", (int) skb->len,
		(int) ntohs(ch->len)));
	goto done;
      error:
	ptrace(("sk = %p, Error %d returned.\n", sk, err));
...
struct {
	...
	[ICMP_PORT_UNREACH]   = { ECONNREFUSED, 1, 1 },
 ...
 *  probably better.
 *
 *  For STREAMS, because we don't want any races here, we place a M_CTL message on the read queue
 *  of the stream to which the message applies.  This distinguishes it from M_DATA messages.  It is
 *  processed within the stream with queues locked by sctp_recv_err when the M_CTL message is

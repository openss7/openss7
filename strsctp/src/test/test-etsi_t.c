...
preamble_iut_cookie_wait_iut(int child)
{
	...
	if (expect(child, INFINITE_WAIT, __TEST_CONN_ACK) != __RESULT_SUCCESS)
		goto failure;
	state++;
	return (__RESULT_SUCCESS);
      failure:
...
{
	if (preamble_pt_cookie_wait_iut(child) != __RESULT_SUCCESS)
		goto failure;
	state++;
...
{
	if (preamble_pt_cookie_wait_pt(child) != __RESULT_SUCCESS)
		goto failure;
	state++;
...
{
	return postamble_iut_establish_iut(child);
}
static int
postamble_pt_established_pt(int child)
{
	return postamble_iut_establish_pt(child);
}

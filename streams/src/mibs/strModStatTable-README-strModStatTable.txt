...
------------------------------------------------------------------------
  ...
  the strModStatTable table.  The agent will start by looking for the right
  ...
  file defaults/table-strModStatTable.m2d, and then re-running mib2c.
    ...
    in the strModStatTable.h header.
    ...
    will be generated:
      ...
      strModStatTable_allocate_data
      strModStatTable_release_data
 ...
 Index(es) for the strModStatTable table
  ------------------------------------------------------------
  The index(es) for the strModStatTable table are:
...
************************************************************************
strModStatTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the strModStatTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: strModStatTable_data_access.[c|h]
------------------------------------------------------------------------
  The strModStatTable_data_access file contains the interface to your data in
  ...
  FUNC : strModStatTable_indexes_set
  WHERE: strModStatTable_data_access.c
  ...
  A simplified version of the pseudo-code looks like this:
     ...
     strModStatTable_loop_init_context(loop)
  ...
  -----------------------------
    TODO : typedef strModStatTable_loop_context
    WHERE: strModStatTable_data_access.h
    ...
    definitions are used in examples later on.
      ...
      Linked list
      -----------
          typedef list_node strModStatTable_loop_context;

      Array
      -----
          typedef integer strModStatTable_loop_context;
  ...
  --------------
    TODO : Initialization
    FUNC : strModStatTable_loop_init_data
    WHERE: strModStatTable_data_access.c

    The strModStatTable_loop_init_data function will be called during startup to
  ...
  ----------------------
    ...
    FUNC : strModStatTable_loop_init_context
    WHERE: strModStatTable_data_access.c
    ...
    strModStatTable_loop_get_first and strModStatTable_loop_get_next.
    ...
    earlier example loop contexts.
      ...
      File
      ----
          ref->loop_ctx = SNMP_MALLOC_TYPEDEF(strModStatTable_loop_context);
          /* error checking here */
  ...
  --------
    ...
    FUNC : strModStatTable_loop_get_first
    WHERE: strModStatTable_data_access.c
    ...
    TODO : return raw data
    FUNC : strModStatTable_loop_get_next
    WHERE: strModStatTable_data_access.c
  ...
  -----------------------------
    ...
    FUNC : strModStatTable_loop_save_position
    WHERE: strModStatTable_data_access.c
  ...
  ---------------------------
    ...
    FUNC : strModStatTable_loop_get_data
    WHERE: strModStatTable_data_access.c
  ...
  --------------------------
    ...
    FUNC : strModStatTable_loop_cleanup_context
    WHERE: strModStatTable_data_access.c
...
File: strModStatTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : strModStatQueues_get
  ...
  -------------------
  TODO : retrieve column data from raw data
  FUNC : strModStatPCnt_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : strModStatScnt_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : strModStatOcnt_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : strModStatCcnt_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : strModStatAcnt_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : strModStatPrivate_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : strModStatFlags_get



File: strModStatTable_data_set.c
------------------------------------------------------------------------
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      strModStatTable table.

      Conversely, the examples below may not include some functions that
      were generated for the strModStatTable table.

To watch the flow of the strModStatTable table, use the
following debug tokens:

        snmp_agent
        helper:table:req
        strModStatTable
        verbose:strModStatTable
        internal:strModStatTable

e.g.
        snmpd -f -Le -DstrModStatTable,verbose:strModStatTable,internal:strModStatTable
...
/*---------------------------------------------------------------------
 * OPENSS7-STREAMS-MIB::strModStatEntry.strModStatIndex
 * strModStatIndex is subid 1 of strModStatEntry.
 * Its status is Current, and its access level is NoAccess.
 * OID: .1.3.6.1.4.1.29591.1.1.1.1.1.3.1.1.1
 * Description:
 ...
 *
 *
 * NOTE: NODE strModStatIndex IS NOT ACCESSIBLE
 *
 *
 */
/*---------------------------------------------------------------------

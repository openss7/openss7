...
streamtab_t *lis_find_strdev(major_t major)
{
	...
	if (major < MAX_CHRDEV) {
		struct cdevsw *cdev;
		if ((cdev = sdev_get(makedevice(major, 0))) != NULL) {
			st = cdev->d_str;
			sdev_put(cdev);
		}
	}
...
int lis_unregister_strmod(struct streamtab *strtab)
{
	...
	int err;
	/* this is ugly, we have to ignore "name" for deregistration */
	/* we should be able to accept a cdev of NULL so that we don't need to export smod_get and
	   smod_put when liscomp is loaded as a module */
	if ((fmod = smod_get(strtab->st_rdinit->qi_minfo->mi_idname)) == NULL)
		return (-ENOENT);
	smod_put(fmod);

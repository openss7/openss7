/*****************************************************************************
 ...
 @(#) $RCSfile: strExtMIB.c,v $ $Name:  $($Revision: 0.9.2.10 $) $Date: 2009-01-16 20:25:49 $
...
#ident "@(#) $RCSfile: strExtMIB.c,v $ $Name:  $($Revision: 0.9.2.10 $) $Date: 2009-01-16 20:25:49 $"

static char const ident[] = "$RCSfile: strExtMIB.c,v $ $Name:  $($Revision: 0.9.2.10 $) $Date: 2009-01-16 20:25:49 $";
...
send_strStrlogRecord_v2trap(struct variable_list *vars)
{
	...
	snmp_varlist_add_variable(&vars,
			snmpTrapOID_oid, sizeof(snmpTrapOID_oid)/sizeof(oid),
			ASN_OBJECT_ID,
			strStrlogRecord_oid, sizeof(strStrlogRecord_oid)/sizeof(oid));
	snmp_varlist_add_variable(&vars,
			strExtStrlogTimeStamp_oid, sizeof(strExtStrlogTimeStamp_oid)/sizeof(oid),
			ASN_UNSIGNED,
			&tmpuint, sizeof(uint));
	tmpuint = log.mid;
	snmp_varlist_add_variable(&vars,
			strExtStrlogMid_oid, sizeof(strExtStrlogMid_oid)/sizeof(oid),
			ASN_UNSIGNED,
			&tmpuint, sizeof(uint));
	tmpuint = log.sid;
	snmp_varlist_add_variable(&vars,
			strExtStrlogSid_oid, sizeof(strExtStrlogSid_oid)/sizeof(oid),
			ASN_UNSIGNED,
			&tmpuint, sizeof(uint));
	tmpuint = log.level;
	snmp_varlist_add_variable(&vars,
			strExtStrlogLevel, sizeof(strExtStrlogLevel)/sizeof(oid),
			ASN_UNSIGNED,
			&tmpuint, sizeof(uint));
	tmpbits = 0;
	if (log.flags & SL_CONSOLE)
		tmpbits |= (1<<(7 - STREXTSTRLOGFLAGS_SLCONSOLE));
	if (log.flags & SL_ERROR)
		tmpbits |= (1<<(7 - STREXTSTRLOGFLAGS_SLERROR));
	if (log.flags & SL_TRACE)
		tmpbits |= (1<<(7 - STREXTSTRLOGFLAGS_SLTRACE));
	if (log.flags & SL_FATAL)
		tmpbits |= (1<<(7 - STREXTSTRLOGFLAGS_SLFATAL));
	if (log.flags & SL_WARN)
		tmpbits |= (1<<(7 - STREXTSTRLOGFLAGS_SLWARN));
	if (log.flags & SL_NOTE)
		tmpbits |= (1<<(7 - STREXTSTRLOGFLAGS_SLNOTE));
	if (log.flags & SL_NOTIFY)
		tmpbits |= (1<<(7 - STREXTSTRLOGFLAGS_SLNOTIFY));
	if (log.flags & SL_NOPUTBUF)
		tmpbits |= (1<<(7 - STREXTSTRLOGFLAGS_SLNOPUTBUF));
	snmp_varlist_add_variable(&vars,
			strExtStrlogFlags, sizeof(strExtStrlogFlags)/sizeof(oid),
			ASN_BIT_STR,
			&tmpbits, 1);
...
strExtMIB_fd_handler(int fd, void *dummy)
{
	...
	struct log_ctl *lc;
	struct variable_list *vars = NULL;
	ulong seq, mid, sid, level;
	uint8_t tmpbits, timestamp[11], flags[1];
	...
	seq = lc->seq_no;
	snmp_varlist_add_variable(&vars,
			strExtStrlogIndex_oid, sizeof(strExtStrlogIndex_oid)/sizeof(oid),
			ASN_UNSIGNED,
			&seq, sizeof(uint));
	snmp_varlist_add_variable(&vars,
			strExtStrlogTimeStamp_oid, sizeof(strExtStrlogTimeStamp_oid) / sizeof(oid),
	...
	level = lc->level;
	snmp_varlist_add_variable(&vars,
			strExtStrlogLevel_oid, sizeof(strExtStrlogLevel_oid)/sizeof(oid),
			ASN_UNSIGNED,
			&level, sizeof(uint));
	flags[0] = 0;
	...
	snmp_varlist_add_variable(&vars,
			strExtStrlogFlags_oid, sizeof(strExtStrlogFlags_oid)/sizeof(oid),
			ASN_BIT_STR,
			flags, 1);
	snmp_varlist_add_variable(&vars,
			strExtStrlogFmtString_oid, sizeof(strExtStrlogFmtString_oid)/sizeof(oid),
			ASN_UNSIGNED,
			&tmpuint, sizeof(uint));

/*****************************************************************************
 ...
 @(#) $RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.3 $) $Date: 2007/10/15 06:47:50 $
 ...
 Last Modified $Date: 2007/10/15 06:47:50 $ by $Author: brian $
...
#ident "@(#) $RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.3 $) $Date: 2007/10/15 06:47:50 $"
...
/**
 * refresh_strMIB: refersh strMIB scalars
 *
 * Normally, the values retrieved from the operating system are cached.  When the agent receives a
 * SIGPOLL from an open STREAMS configuration or administrative driver (sc(4) or sad(4)) Stream, the
 * STREAMS subsystem indicates to the agent that the cache has been invalidated and that it should
 * reread scalars and tables from the STREAMS subsystem.  This function is used when the agent
 * starts for the first time, or after a SIGPOLL has been received (and a scalar has been
 * requested).
 */
void
refresh_strMIB(void)
	   ...
	   WriteMethod ** write_method)
{
	...
	case STRCLTIME:
		*write_method = write_strCltime;	/* read-write */
		return (unsigned char *) &StorageTmp->strCltime;

	case STRMAXAPUSH:
		*write_method = write_strMaxApush;	/* read-write */
		return (unsigned char *) &StorageTmp->strMaxApush;

	case STRMAXMBLK:
		*write_method = write_strMaxMblk;	/* read-write */
		return (unsigned char *) &StorageTmp->strMaxMblk;

	case STRMAXSTRAMOD:
		*write_method = write_strMaxStramod;	/* read-write */
		return (unsigned char *) &StorageTmp->strMaxStramod;

	case STRMAXSTRDEV:
		*write_method = NULL;	/* read-only */
		return (unsigned char *) &StorageTmp->strMaxStrdev;

	case STRMAXSTRMOD:
		*write_method = NULL;	/* read-only */
		return (unsigned char *) &StorageTmp->strMaxStrmod;

	case STRMSGPRIORITY:
		*write_method = write_strMsgPriority;	/* read-write */
		return (unsigned char *) &StorageTmp->strMsgPriority;

	case STRNBAND:
		*write_method = NULL;	/* read-only */
		return (unsigned char *) &StorageTmp->strNband;

	case STRNSTRMSGS:
		*write_method = write_strNstrmsgs;	/* read-write */
		return (unsigned char *) &StorageTmp->strNstrmsgs;

	case STRNSTRPUSH:
		*write_method = write_strNstrpush;	/* read-write */
		return (unsigned char *) &StorageTmp->strNstrpush;

	case STRHIWAT:
		*write_method = write_strHiwat;	/* read-write */
		return (unsigned char *) &StorageTmp->strHiwat;

	case STRLOWAT:
		*write_method = write_strLowat;	/* read-write */
		return (unsigned char *) &StorageTmp->strLowat;

	case STRMAXPSZ:
		*write_method = write_strMaxpsz;	/* read-write */
		return (unsigned char *) &StorageTmp->strMaxpsz;

	case STRMINPSZ:
		*write_method = write_strMinpsz;	/* read-write */
		return (unsigned char *) &StorageTmp->strMinpsz;

	case STRREUSEFMODSW:
		*write_method = write_strReuseFmodsw;	/* read-write */
		return (unsigned char *) &StorageTmp->strReuseFmodsw;

	case STRRTIME:
		*write_method = write_strRtime;	/* read-write */
		return (unsigned char *) &StorageTmp->strRtime;

	case STRSTRHOLD:
		*write_method = write_strStrhold;	/* read-write */
		return (unsigned char *) &StorageTmp->strStrhold;

	case STRSTRCTLSZ:
		*write_method = write_strStrctlsz;	/* read-write */
		return (unsigned char *) &StorageTmp->strStrctlsz;

	case STRSTRMSGSZ:
		*write_method = write_strStrmsgsz;	/* read-write */
		return (unsigned char *) &StorageTmp->strStrmsgsz;

	case STRSTRTHRESH:
		*write_method = write_strStrthresh;	/* read-write */
		return (unsigned char *) &StorageTmp->strStrthresh;

	case STRLOWTHRESH:
		*write_method = NULL;	/* read-only */
		return (unsigned char *) &StorageTmp->strLowthresh;

	case STRMEDTHRESH:
		*write_method = NULL;	/* read-only */
		return (unsigned char *) &StorageTmp->strMedthresh;

	case STRIOCTIME:
		*write_method = write_strIoctime;	/* read-write */
		return (unsigned char *) &StorageTmp->strIoctime;
		     ...
		     u_char *statP, oid * name, size_t name_len)
{
	...
	case RESERVE1:
		...
		default:
			snmp_log(MY_FACILITY(LOG_NOTICE),
				 "write to strMsgPriority: bad value %ld\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
		     ...
		     u_char *statP, oid * name, size_t name_len)
{
	...
	case RESERVE1:
		...
		default:
			snmp_log(MY_FACILITY(LOG_NOTICE),
				 "write to strReuseFmodsw: bad value %ld\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
		 ...
		 u_char *statP, oid * name, size_t name_len)
{
	...
	case COMMIT:
	{
		...
		int clen = 2;
		unsigned long value = StorageTmp->strStrhold * 10;	/* centiseconds to
									   milliseconds */
		    ...
		    u_char *statP, oid * name, size_t name_len)
{
	...
	if (var_val_type != ASN_INTEGER || var_val == NULL) {
		snmp_log(MY_FACILITY(LOG_NOTICE), "write to strApshStatus not ASN_INTEGER\n");
		return SNMP_ERR_WRONGTYPE;
	}
	...
	case RESERVE2:
		...
		if (StorageTmp == NULL) {
			...
			vp = vp->next_variable;
			memdup((void *) &StorageNew->strModInfoIndex, vp->val.string, vp->val_len);
			StorageNew->strModInfoIndexLen = vp->val_len;
			vp = vp->next_variable;
			memdup((void *) &StorageNew->strModStatIndex, vp->val.string, vp->val_len);
			...
			vp = vp->next_variable;
			memdup((void *) &StorageNew->strApshMinor, vp->val.string, vp->val_len);
#if 0

/*****************************************************************************
 ...
 @(#) $RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.30 $) $Date: 2009-01-16 20:25:49 $
...
#ident "@(#) $RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.30 $) $Date: 2009-01-16 20:25:49 $"

static char const ident[] = "$RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.30 $) $Date: 2009-01-16 20:25:49 $";
...
#define MY_FACILITY(__pri)	LOG_ERR
#define MASTER 1
#if !defined MODULE
...
#define   STRMODINFOQUEUES      5
	{(u_char) STRMODINFOQUEUES, ASN_BIT_STR, RONLY, var_strModInfoTable, 6, {1, 1, 2, 1, 1, 2}},
...
#define   STRMODSTATQUEUES      11
	{(u_char) STRMODSTATQUEUES, ASN_BIT_STR, RONLY, var_strModStatTable, 6, {1, 1, 3, 1, 1, 2}},
...
var_strModTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	StorageTmp = header_complex(strModTableStorage, vp, name, length, exact, var_len, write_method);
	refresh_strModTable_row(StorageTmp, 0);
	*write_method = NULL;
	*var_len = 0;
	rval = NULL;
	/* This is where we do the value assignments for the mib results. */
...
var_strModInfoTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	/* This assumes you have registered all your data properly with header_complex_add() somewhere before this. */
	StorageTmp = header_complex(strModInfoTableStorage, vp, name, length, exact, var_len, write_method);
	refresh_strModInfoTable_row(StorageTmp, 0);
	*write_method = NULL;
	*var_len = 0;
	rval = NULL;
	/* This is where we do the value assignments for the mib results. */
...
var_strModStatTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	/* This assumes you have registered all your data properly with header_complex_add() somewhere before this. */
	StorageTmp = header_complex(strModStatTableStorage, vp, name, length, exact, var_len, write_method);
	refresh_strModStatTable_row(StorageTmp, 0);
	*write_method = NULL;
	*var_len = 0;
	rval = NULL;
	/* This is where we do the value assignments for the mib results. */
...
var_strApshTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	/* This assumes you have registered all your data properly with header_complex_add() somewhere before this. */
	StorageTmp = header_complex(strApshTableStorage, vp, name, length, exact, var_len, write_method);
	refresh_strApshTable_row(StorageTmp, 0);
	*write_method = NULL;
	*var_len = 0;
	rval = NULL;
	/* This is where we do the value assignments for the mib results. */
...
refresh_strStatsTable(int force)
{
	...
	/* XXX: Here, update the table as required... */
	{
		...
		if ((fd = open("/dev/nuls", O_RDWR)) < 0) {
			snmp_log_perror("refresh_strStatsTable: open");
			return;
		}
		if (ioctl(fd, I_PUSH, "sc") < 0) {
			snmp_log_perror("refresh_strStatsTable: I_PUSH");
			close(fd);
			return;
		}
...
var_strStatsTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	/* This assumes you have registered all your data properly with header_complex_add() somewhere before this. */
	StorageTmp = header_complex(strStatsTableStorage, vp, name, length, exact, var_len, write_method);
	refresh_strStatsTable_row(StorageTmp, 0);
	*write_method = NULL;
	*var_len = 0;
	rval = NULL;
	/* This is where we do the value assignments for the mib results. */
...
write_strModRowStatus(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid * name, size_t name_len)
{
	...
	case ACTION:
		...
		case RS_ACTIVE:
			...
			if (old_value != RS_ACTIVE) {
				...
				if (!strModTable_consistent(StorageTmp)) {
	...
	case COMMIT:
		...
		case RS_CREATEANDGO:
			...
			if (strModTable_consistent(StorageNew)) {
...
write_strApshRowStatus(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid * name, size_t name_len)
{
	...
	case RESERVE2:
		...
		case RS_CREATEANDWAIT:
			...
			vp = vp->next_variable;
			StorageNew->strApshLastMinor = *vp->val.integer;
			vp = vp->next_variable;
	...
	case ACTION:
		...
		case RS_ACTIVE:
			...
			if (old_value != RS_ACTIVE) {
				/* check that activation is possible */
				if (!strApshTable_consistent(StorageTmp)) {
	...
	case COMMIT:
		...
		case RS_CREATEANDGO:
			/* row creation, set final state */
			/* check if row is ready, otherwise leave at RS_NOTREADY */
			if (strApshTable_consistent(StorageNew)) {
				/* XXX: commit creation to underlying device */
				/* XXX: activate with underlying device */
...
strMIB_loop_handler(int sig)
{
	...
	/* refresh tables on each request */

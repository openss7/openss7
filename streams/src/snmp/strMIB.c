/*****************************************************************************
 ...
 @(#) $RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.19 $) $Date: 2008-12-24 13:42:15 $
 ...
 Last Modified $Date: 2008-12-24 13:42:15 $ by $Author: brian $
...
#ident "@(#) $RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.19 $) $Date: 2008-12-24 13:42:15 $"

static char const ident[] = "$RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.19 $) $Date: 2008-12-24 13:42:15 $";
...
refresh_strMIB(void)
{
	...
	if (strMIBStorage == NULL) {
		...
		if ((StorageNew = SNMP_MALLOC_STRUCT(strMIB_data)) == NULL)
			return;
...
strModTable_add(struct strModTable_data *thedata)
{
	...
	/* strModIdnum */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->strModIdnum, sizeof(thedata->strModIdnum));
...
parse_strModTable(const char *token, char *line)
{
	...
	line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->strModName, &tmpsize);
	StorageTmp->strModNameLen = tmpsize;
...
/*   XXX:  if (StorageTmp->strModTableStorageType == ST_NONVOLATILE) { */
		...
		tmpsize = StorageTmp->strModNameLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->strModName, &tmpsize);
...
strModInfoTable_add(struct strModInfoTable_data *thedata)
{
	...
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->strModIdnum, sizeof(thedata->strModIdnum));
	/* strModInfoIndex */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->strModInfoIndex, sizeof(thedata->strModInfoIndex));
...
parse_strModInfoTable(const char *token, char *line)
{
	...
	line = read_config_read_data(ASN_BIT_STR, line, &StorageTmp->strModInfoQueues, &tmpsize);
	StorageTmp->strModInfoQueuesLen = tmpsize;
...
/*   XXX:  if (StorageTmp->strModInfoTableStorageType == ST_NONVOLATILE) { */
		...
		tmpsize = StorageTmp->strModInfoQueuesLen;
		cptr = read_config_store_data(ASN_BIT_STR, cptr, &StorageTmp->strModInfoQueues, &tmpsize);
...
strModStatTable_add(struct strModStatTable_data *thedata)
{
	...
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->strModIdnum, sizeof(thedata->strModIdnum));
	/* strModStatIndex */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->strModStatIndex, sizeof(thedata->strModStatIndex));
...
parse_strModStatTable(const char *token, char *line)
{
	...
	line = read_config_read_data(ASN_BIT_STR, line, &StorageTmp->strModStatQueues, &tmpsize);
	StorageTmp->strModStatQueuesLen = tmpsize;
	...
	line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->strModStatPrivate, &tmpsize);
	StorageTmp->strModStatPrivateLen = tmpsize;
...
/*   XXX:  if (StorageTmp->strModStatTableStorageType == ST_NONVOLATILE) { */
		...
		tmpsize = StorageTmp->strModStatQueuesLen;
		cptr = read_config_store_data(ASN_BIT_STR, cptr, &StorageTmp->strModStatQueues, &tmpsize);
		...
		tmpsize = StorageTmp->strModStatPrivateLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->strModStatPrivate, &tmpsize);
...
strApshTable_add(struct strApshTable_data *thedata)
{
	...
	/* add the index variables to the varbind list, which is used by header_complex to index the data */
	/* strApshName */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_OCTET_STR, (uint8_t *) thedata->strApshName, thedata->strApshNameLen);
	/* strApshMinor */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->strApshMinor, sizeof(thedata->strApshMinor));
...
parse_strApshTable(const char *token, char *line)
{
	...
	line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->strApshName, &tmpsize);
	StorageTmp->strApshNameLen = tmpsize;
	...
	line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->strApshModules, &tmpsize);
	StorageTmp->strApshModulesLen = tmpsize;
...
/*   XXX:  if (StorageTmp->strApshTableStorageType == ST_NONVOLATILE) { */
		...
		cptr = line + strlen(line);
		tmpsize = StorageTmp->strApshNameLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->strApshName, &tmpsize);
		...
		tmpsize = StorageTmp->strApshModulesLen;
		cptr = read_config_store_data(ASN_OCTET_STR, cptr, &StorageTmp->strApshModules, &tmpsize);
...
strStatsTable_add(struct strStatsTable_data *thedata)
{
	...
	/* add the index variables to the varbind list, which is used by header_complex to index the data */
	/* strStatsStructure */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_INTEGER, (uint8_t *) &thedata->strStatsStructure, sizeof(thedata->strStatsStructure));
...
var_strModStatTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	case STRMODSTATQUEUES:
		...
		*var_len = StorageTmp->strModStatQueuesLen;
		return (uint8_t *) StorageTmp->strModStatQueues;
	case STRMODSTATPCNT:
		*write_method = NULL;	/* read-only */
		*var_len = sizeof(StorageTmp->strModStatPCnt);
		return (uint8_t *) &StorageTmp->strModStatPCnt;
	case STRMODSTATSCNT:
		*write_method = NULL;	/* read-only */
		*var_len = sizeof(StorageTmp->strModStatScnt);
		return (uint8_t *) &StorageTmp->strModStatScnt;
	case STRMODSTATOCNT:
		*write_method = NULL;	/* read-only */
		*var_len = sizeof(StorageTmp->strModStatOcnt);
		return (uint8_t *) &StorageTmp->strModStatOcnt;
	case STRMODSTATCCNT:
		*write_method = NULL;	/* read-only */
		*var_len = sizeof(StorageTmp->strModStatCcnt);
		return (uint8_t *) &StorageTmp->strModStatCcnt;
	case STRMODSTATACNT:
		*write_method = NULL;	/* read-only */
		*var_len = sizeof(StorageTmp->strModStatAcnt);
		return (uint8_t *) &StorageTmp->strModStatAcnt;
	case STRMODSTATPRIVATE:
		*write_method = NULL;	/* read-only */
		*var_len = StorageTmp->strModStatPrivateLen;
		return (uint8_t *) StorageTmp->strModStatPrivate;
	case STRMODSTATFLAGS:
		*write_method = NULL;	/* read-only */
		*var_len = sizeof(StorageTmp->strModStatFlags);
		return (uint8_t *) &StorageTmp->strModStatFlags;
...
var_strStatsTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	case STRSTATSCURRENTALLOCS:
		...
		*var_len = sizeof(StorageTmp->strStatsCurrentAllocs);
		return (uint8_t *) &StorageTmp->strStatsCurrentAllocs;
	case STRSTATSHIGHWATERMARK:
		*write_method = NULL;	/* read-only */
		*var_len = sizeof(StorageTmp->strStatsHighWaterMark);
		return (uint8_t *) &StorageTmp->strStatsHighWaterMark;
...
int
write_strModName(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct strModTable_data *StorageTmp = NULL;
...
int
write_strModInfoMinpsz(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	...
	case FREE:		/* Release any resources that have been allocated */
		break;
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you have just been
...
int
write_strModInfoMaxpsz(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct strModInfoTable_data *StorageTmp = NULL;
	static size_t tmplen;
...
int
write_strModInfoHiwat(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strModInfoHiwat = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strModInfoLowat(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strModInfoTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strModInfoLowat = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strModInfoTraceLevel(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strModInfoTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strModInfoTraceLevel = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strApshModules(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct strApshTable_data *StorageTmp = NULL;
	static size_t tmplen;
...
int
write_strCltime(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	...
	case RESERVE1:
		...
		if ((tmpvar = *((long *) var_val)) < 0 || tmpvar > INT_MAX / 10) {
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to strCltime: bad value %d\n", tmpvar);
...
int
write_strMaxApush(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strMaxApush = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strMaxMblk(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strMaxMblk = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strMaxStramod(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strMaxStramod = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strMsgPriority(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case RESERVE1:
		...
		case TV_FALSE:
			break;
		default:
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to strMsgPriority: bad value %d\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
	case RESERVE2:		/* memory reseveration, final preparation... */
...
int
write_strNstrmsgs(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strNstrmsgs = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strNstrpush(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strNstrpush = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strHiwat(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strHiwat = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strLowat(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strLowat = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strMaxpsz(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case RESERVE1:
		...
		if ((tmpvar = *((int *) var_val)) <= 0) {
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to strMaxpsz: bad value %d\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
	case RESERVE2:		/* memory reseveration, final preparation... */
...
int
write_strMinpsz(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case RESERVE1:
		...
		/* Note: ranges 0..2147482648 */
		if ((tmpvar = *((int *) var_val)) < 0) {
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to strMinpsz: bad value %d\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
	case RESERVE2:		/* memory reseveration, final preparation... */
...
int
write_strReuseFmodsw(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case RESERVE1:
		...
		case TV_FALSE:
			break;
		default:
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to strReuseFmodsw: bad value %d\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
	case RESERVE2:		/* memory reseveration, final preparation... */
...
int
write_strRtime(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case RESERVE1:
		...
		if ((tmpvar = *((int *) var_val)) <= 0 || tmpvar > INT_MAX / 10) {
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to strRtime: bad value %d\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
	case RESERVE2:		/* memory reseveration, final preparation... */
...
int
write_strStrhold(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case RESERVE1:
		...
		case TV_FALSE:
			break;
		default:
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to strStrhold: bad value %d\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
	case RESERVE2:		/* memory reseveration, final preparation... */
...
int
write_strStrctlsz(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case RESERVE1:
		...
		/* Note: default value 4096 */
		/* Note: ranges (none) */
		/* values less than about 1024 are not really usable, and values greater than about 262,144 are not
		   workable. */
		tmpvar = *((ulong *) var_val);
		if (1024 > tmpvar || tmpvar > 262144) {
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to strStrctlsz: bad value %u\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
	case RESERVE2:		/* memory reseveration, final preparation... */
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strStrctlsz = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strStrmsgsz(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case RESERVE1:
		...
		tmpvar = *((ulong *) var_val);
		if (1024 > tmpvar || tmpvar > 262144) {
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to strStrmsgsz: bad value %u\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
	case RESERVE2:		/* memory reseveration, final preparation... */
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strStrmsgsz = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strStrthresh(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strStrthresh = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strLowthresh(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strLowthresh = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strMedthresh(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and you have just been
		...
		StorageTmp->strMedthresh = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
...
int
write_strIoctime(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct strMIB_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case RESERVE1:
		...
		/* Note: ranges 0..2147483647 */
		if ((tmpvar = *((int *) var_val)) <= 0 || tmpvar > INT_MAX / 10) {
			snmp_log(MY_FACILITY(LOG_NOTICE), "write to strIoctime: bad value %d\n", tmpvar);
			return SNMP_ERR_WRONGVALUE;
		}
		break;
	case RESERVE2:		/* memory reseveration, final preparation... */
...
int
write_strModRowStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	struct strModTable_data *StorageTmp = NULL;
...
int
write_strApshRowStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	struct strApshTable_data *StorageTmp = NULL;
...
sa_help_directives(int argc, char *argv[])
{
	...
	/* Unfortunately, read_config_print_usage() uses snmp_log(), meaning that it can only be writen to standard
	   error and not standard output. */
...
sa_hup_action(void)
{
	/* There are several times that we might be sent a SIGHUP.  We might be sent a SIGHUP by logrotate asking us to 
	   close and reopen our log files. */
	sa_hup_signal = 0;
...
sa_init_logging(int argc, char *argv[])
{
	...
	/* The purpose of this function is to bring logging up before forking (and while still in the foreground) so
	   that we can use the snmp_log() function before and during forking if necessary.  Note that the default
	   configuration for snmp_log() is to send all logs to standard error. */
	...
	if (sa_logstderr | sa_logstdout) {
		...
		/* Note that when we have Linux LOG_PERROR, and logs go both to syslog and stderr, it is better to use
		   the LOG_PERROR than to use snmp_log()'s print to stderr, as the former is better formated. */
		if (!sa_logsyslog)
	...
	if (sa_logsyslog) {
		...
		/* Note that the way that snmp sets up the logger is not really the way we want it, so close the log
		   and reopen it the way we want. */
		closelog();
#if defined LOG_PERROR
...
sa_mloop(int argc, char *argv[])
{
	...
	for (;;) {
		...
		/* to use select or poll you need to use the snmp_select_info() to obtain the fd of the agentx socket
		   and add it to the fdset. */
...
main(int argc, char *argv[])
{
	for (;;) {
		...
		case 'L':	/* -L, --log-stderr, -Le, -LE p1[-p2] */
			/* Note that the recent NET-SNMP version of this option is far more complicated: -Le is the
			   same as the old version of the option; -Lf LOGFILE is like the -l option; -Ls is like the -s 
			   option; -Lo logs messages to standard output; -LX p1[-p2] [LOGFILE], where X = E, F, S or O, 
			   logs priority p1 and above to X, or p1 thru p2 to X. */
			if (sa_debug)

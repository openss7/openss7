/*****************************************************************************
 ...
 @(#) $RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.31 $) $Date: 2009-01-19 10:39:41 $
...
#ident "@(#) $RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.31 $) $Date: 2009-01-19 10:39:41 $"

static char const ident[] = "$RCSfile: strMIB.c,v $ $Name:  $($Revision: 0.9.2.31 $) $Date: 2009-01-19 10:39:41 $";
...
write_strModInfoMinpsz(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid * name, size_t name_len)
{
	...
	if ((StorageTmp = header_complex(strModInfoTableStorage, NULL, &name[15], &newlen, 1, NULL, NULL)) == NULL)
...
write_strApshModules(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid * name, size_t name_len)
{
	...
	if ((StorageTmp = header_complex(strApshTableStorage, NULL, &name[15], &newlen, 1, NULL, NULL)) == NULL)
		return SNMP_ERR_NOSUCHNAME;	/* remove if you support creation here */
	...
	case ACTION:
		...
		case RS_ACTIVE:
			...
			if (old_value != RS_ACTIVE) {
				...
				/* Ok, lets actually try to activate it... */
				goto probe_module;
			}
			break;
		case RS_NOTINSERVICE:
		...
		case RS_CREATEANDGO:
		{
			/* Creating a row in the strModTable has the effect of attempting to verify the module by
			   loading it.  We could use the sad(4) driver's ability to verify a module list; however, it
			   does not work for drivers.  So, we use the sc(4) modules' ability to tune a module or
			   driver, but don't really tune any of it (just for the side-effect of demand loading). */
			int fd;
			struct sc_tlist sc;
			struct sc_tune t[4];
		      ...
		      probe_module:
			if ((fd = open("/dev/sad", O_RDWR)) < 0) {
				snmp_log_perror("write_strModRowStatus: open(/dev/sad)");
				return SNMP_ERR_GENERR;
			}
			if (ioctl(fd, I_PUSH, "sc") < 0) {
				snmp_log_perror("write_strModRowStatus: ioctl(I_PUSH,\"sc\")");
				close(fd);
				return SNMP_ERR_GENERR;
			}
			memset(&sc, 0, sizeof(sc));
			sc.sc_major = 0;
			memcpy(sc.sc_name, StorageNew->strModName, StorageNew->strModNameLen);
			sc.sc_ntune = 4;
			sc.sc_tlist = t;
			memset(t, 0, 4 * sizeof(t[0]));
			if (ioctl(fd, SC_IOC_TUNE, &sc) < 0) {
				snmp_log_perror("write_strModRowStatus: ioctl(SC_IOC_TUNE)");
				close(fd);
				return SNMP_ERR_INCONSISTENTNAME;
			}
			close(fd);
	...
	case COMMIT:
		...
		case RS_NOTINSERVICE:
			...
			if (old_value != set_value) {
				switch (set_value) {
				case RS_ACTIVE:
					/* activate with underlying device: already done in ACTION phase */
					break;
				case RS_NOTINSERVICE:
					/* deactivate with underlying device: not possible,
					   precluded in RESERVE1 phase. */
					break;
				}
			}
			break;

...
static char const ident[] = "$RCSfile: sth.c,v $ $Name:  $($Revision: 0.9.2.206 $) $Date: 2008-10-21 03:06:48 $";
...
strwaitclose(struct stdata *sd, int oflag)
{
	...
	cred_t creds = {.cr_uid = current->euid,.cr_gid = current->egid,.cr_ruid = current->uid,.cr_rgid = current->gid,
	}, *crp = &creds;
#else

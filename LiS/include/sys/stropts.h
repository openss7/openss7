/*****************************************************************************
 ...
 @(#) $RCSfile: stropts.h,v $ $Name:  $($Revision: 1.1.1.3.4.11 $) $Date: 2008-04-29 08:33:15 $
 ...
 $Log: stropts.h,v $
 Revision 1.1.1.3.4.11  2008-04-29 08:33:15  brian
 ...
 * RCS Id          ; $Id: stropts.h,v 1.1.1.3.4.11 2008-04-29 08:33:15 brian Exp $
...
#ident "@(#) $RCSfile: stropts.h,v $ $Name:  $($Revision: 1.1.1.3.4.11 $) $Date: 2008-04-29 08:33:15 $"
...
/** @name LiS-2.6 Compatibility
  ...
  * following before including this file.  With this not defined we are
  * compatible with Solaris and UnixWare in our use of constant values.
  * 
...
/** @name Write Options
  *
  * As Solaris' guys say:
  * "These next three read options are added for the sake of user-level
  * transparency.  RPROTDAT will cause the stream head to treat the
  * contents of M_PROTO and M_PCPROTO message blocks as data.  RPROTDIS
  * will prevent the stream head from failing a read with EBADMSG if an
  * M_PROTO or M_PCPROTO message is on the front of the stream head read
  * queue.  Rather, the protocol blocks will be silently discarded and
  * the data associated with the message (in linked M_DATA blocks), if
  * any, will be delivered to the user.  RPROTNORM sets the default
  * behavior, where read will fail with EBADMSG if an M_PROTO or
  * M_PCPROTO are at the stream head."
  *
...
/** @name Events for SIGPOLL to be sent
  * @{ */
...
/** Stream buffer structure.
  * For putpmsg and getpmsg system calls.  */
...
/** @name Get and Put Message Flags
...
/** @name Get and Put Priority Message Flags
...
/** @name Get Messgage Return Flags
...
#define I_PIPE		(__SID |44)	/**< HP-UX, OSF, Mac OT (49), LiS (254 then 243),
					     connect two streams as a pipe. */
#define I_FIFO		(__SID |45)	/**< HP-UX, OSF, Mac OT (51), convert a stream
					     into a FIFO */
...
/** @name STREAMS System Call Emulation
  *
  * Note that, athough #I_LIS_GETPMSG and #I_LIS_PUTPMSG were removed in
  * 2.18.0, they need to go back in to support Linux kernel 2.6.11 on
  * FC4.  Attempting to overload the read()/write() system call was a
  * foolish way to go that does not work with the 2.6.11 kernel on FC4.
  * The proper way to go was to use an ioctl to emulate a system call as
  * is done on so many other UNIX(-like) systems. -bb Wed Jun 22
  * 22:31:13 MDT 2005
  * @{ */
...
/** getpmsg() argument structure.
  ...
  * structure... -bb Wed Jun 22 22:41:03 MDT 2005
  */
...
/* getpmsg()/putpmsg() argument structure.
 * For ioctl emulation of getmsg() getpmsg() putmsg() putpmsg(), matches
 * Mac OT, HP-UX, OSF: probably a Mentat thing...  This matches the
 * Linux Fast-STREAMS definition.
 */
struct strpmsg {
	struct strbuf ctlbuf;
	struct strbuf databuf;

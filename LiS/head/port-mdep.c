/*****************************************************************************
 ...
 @(#) $RCSfile: port-mdep.c,v $ $Name:  $($Revision: 1.1.1.4.4.5 $) $Date: 2008-04-29 08:33:13 $
 ...
 $Log: port-mdep.c,v $
 Revision 1.1.1.4.4.5  2008-04-29 08:33:13  brian
 - update headers for Affero release

 Revision 1.1.1.4.4.4  2007/08/14 10:47:01  brian
 - GPLv3 header update

 *****************************************************************************/

#ident "@(#) $RCSfile: port-mdep.c,v $ $Name:  $($Revision: 1.1.1.4.4.5 $) $Date: 2008-04-29 08:33:13 $"

static char const ident[] = "$RCSfile: port-mdep.c,v $ $Name:  $($Revision: 1.1.1.4.4.5 $) $Date: 2008-04-29 08:33:13 $";
 ...
 */
void
port_add_timer(struct timer_list *timer)
{
	lis_flags_t flags;
	struct timer_list *p;
 ...
 */
int
port_del_timer(struct timer_list *timer)
{
	lis_flags_t flags;
	struct timer_list *p;
 ...
 */
void
port_announce_time(long new_milli_secs)
{
	...
	while ((timer = timer_head.next) != &timer_head	/* list non-empty */
		...
		lis_spin_unlock_irqrestore(&port_timer_lock, &flags);
		if (fn != NULL)
			fn(data);	/* be careful of NULL ptr */
		lis_spin_lock_irqsave(&port_timer_lock, &flags);
	}
...
************************************************************************/
long
port_time_till(long target_time)
{
...
************************************************************************/
long
port_target_time(long milli_sec)
{
...
************************************************************************/
long
port_milli_to_ticks(long milli_sec)
{
	return (milli_sec);
}

 ...
 */
struct inode *
port_grab_inode(struct inode *i)
{
	(i->i_use)++;
	return i;
}
 ...
 */
struct inode *
port_set_up_inode(struct file *f, struct inode *inode)
{
	return (inode);
}
 ...
 */
int
port_is_stream_inode(struct inode *i)
{
	return (1);
}
 ...
 */
struct inode *
port_new_inode(struct file *fp, dev_t dev)
{
	struct inode *old;
	dir_t *dp;
	struct inode *new;
	...
	if (new != NULL) {	/* got a new one *//* allocate stores dev */
		ULOCK_INO(new);
 ...
 */
struct inode *
port_old_inode(struct file *f, struct inode *i)
{
 ...
 */
int
port_new_file_name(struct file *f, const char *name)
{
 ...
 */
void
port_cleanup_file_opening(struct file *f, stdata_t *head, int open_fail)
{
}

void
port_cleanup_file_closing(struct file *f, stdata_t *head)
{
}

void
port_new_stream_name(struct stdata *hd, struct file *f)
{
 ...
 */
int
port_get_pipe(struct file **f0, struct file **f1)
{
	return (user_get_pipe(f0, f1));
}

int
port_pipe(unsigned int *fd)
{
	return (user_pipe(fd));
}

int
port_fattach(struct file *f, const char *path)
{
	_return(-ENOSYS);
}

int
port_fdetach(const char *path)
{
	_return(-ENOSYS);
}

void
port_fdetach_stream(stdata_t *head)
{
}

/*
 ...
 */
int
port_sendfd(stdata_t *head, unsigned int fd, struct file *fp)
{
 ...
 */
int
port_recvfd(stdata_t *head, strrecvfd_t * recv, struct file *fp)
{

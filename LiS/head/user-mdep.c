/*****************************************************************************
 ...
 @(#) $RCSfile: user-mdep.c,v $ $Name:  $($Revision: 1.1.1.3.4.6 $) $Date: 2008-04-29 08:33:13 $
 ...
 $Log: user-mdep.c,v $
 Revision 1.1.1.3.4.6  2008-04-29 08:33:13  brian
...
#ident "@(#) $RCSfile: user-mdep.c,v $ $Name:  $($Revision: 1.1.1.3.4.6 $) $Date: 2008-04-29 08:33:13 $"

static char const ident[] = "$RCSfile: user-mdep.c,v $ $Name:  $($Revision: 1.1.1.3.4.6 $) $Date: 2008-04-29 08:33:13 $";
 ...
 */
int
user_get_pipe(struct file **f0, struct file **f1)
{
	stdata_t *hd0, *hd1;
	queue_t *wq0, *wq1;
	int error;
 ...
 */
int
user_sendfd(stdata_t *sendhd, unsigned int fd, struct file *fp)
{
	stdata_t *recvhd;
	mblk_t *mp;
	strrecvfd_t *send;
	...
	if (LIS_DEBUG_IOCTL) {
		printk("sendfd( ..., %d, 0x%x ) from %s%s to %s%s", fd, fp, lis_strm_name(sendhd),
		       lis_maj_min_name(sendhd), lis_strm_name(recvhd), lis_maj_min_name(recvhd));
		if (send->r.fp)
			printk(" as 0x%x\n", fp);
		else
			printk("\n");
 ...
 */
int
user_recvfd(stdata_t *recvhd, strrecvfd_t * recv, struct file *fp)
{
	mblk_t *mp;
	strrecvfd_t *sent;
	int error;
...
void
user_new_stream_name(struct stdata *hd, struct file *f)
{
	if (f->f_name)

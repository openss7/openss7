/*****************************************************************************
 ...
 @(#) $RCSfile: usrio.c,v $ $Name:  $($Revision: 1.1.1.1.12.4 $) $Date: 2007/08/14 10:47:05 $
 ...
 Last Modified $Date: 2007/08/14 10:47:05 $ by $Author: brian $

 -----------------------------------------------------------------------------

 $Log: usrio.c,v $
 Revision 1.1.1.1.12.4  2007/08/14 10:47:05  brian
 - GPLv3 header update

 *****************************************************************************/

#ident "@(#) $RCSfile: usrio.c,v $ $Name:  $($Revision: 1.1.1.1.12.4 $) $Date: 2007/08/14 10:47:05 $"

static char const ident[] = "$RCSfile: usrio.c,v $ $Name:  $($Revision: 1.1.1.1.12.4 $) $Date: 2007/08/14 10:47:05 $";
...
************************************************************************/
int
user_mknod(char *name, int mode, dev_t dev)
{
	int d;
	int m;
	inode_t *ip;
	...
	if ((m = major(dev)) > NMAJOR)
		_return(-EBADF);	/* range check major */
	if (dev != 0)		/* 0 is special case for a '.' entry */
		if (lis_fstr_sw[m].f_str == NULL)
			_return(-ENXIO);	/* ensure streamtab */
...
int
user_add_dir_entry(char *name, int mode, dev_t dev)
{
...
************************************************************************/
dir_t *
user_search_dir_inode(inode_t * i)
{
	int d;
	dir_t *dp;
...
user_alloc_file(void)
{
	...
	lis_mark_mem(fp, "file: ", fp->f_fdnr);
	if (!alloc_file_tbl()) {	/* ensure index slot *//* ooops, couldn't */
		FREE(fp);
		return (NULL);
	}
...
user_inode(dev_t dev)
{
	...
	for (ip = inodes; ip != NULL; ip = ip->i_link) {
		if (ip->i_use != 0 && ip->i_dev == dev) {
			LOCK_INO(ip);	/* lock it */
			if (ip->i_use == 0 || ip->i_dev != dev) {	/* lost race */
				ULOCK_INO(ip);
				continue;
			}
...
************************************************************************/
int
user_open(char *name, int flags, int mode)
{
	int m;
	int fd;
	int rslt = -1;
...
user_close(int fd)
{
	...
	if (ip->i_use > 0) {	/* inode usage count */
		...
		if ((ip->i_use == 1) && (ip->i_flags & REFERENCE_INODE)) {
			...
			if (ip->i_use <= 0) {	/* all usages gone *//* remove the directory entry */
				ip->i_str = NULL;
				lis_mark_mem(ip, "unused inode: ", major(ip->i_dev));
...
************************************************************************/
int
user_read(int fd, void *buf, int nbytes)
{
	inode_t *ip;
	file_t *fp = user_file_of_fd(fd);
	int err;
...
************************************************************************/
int
user_write(int fd, void *buf, int nbytes)
{
	inode_t *ip;
	file_t *fp = user_file_of_fd(fd);
	int err;
...
************************************************************************/
int
user_putpmsg(int fd, struct strbuf *ctlptr, struct strbuf *dataptr, int band, int flags)
{
	inode_t *ip;
	file_t *fp = user_file_of_fd(fd);
	int err;
...
************************************************************************/
int
user_getpmsg(int fd, struct strbuf *ctlptr, struct strbuf *dataptr, int *bandp, int *flagsp)
{
	inode_t *ip;
	file_t *fp = user_file_of_fd(fd);
	int err;
...
************************************************************************/
int
user_poll(struct pollfd *fds, int nfds, int time_out)
{
	int err;
	...
	for (i = 0, pdat_ptr = pdat_head, pfd_ptr = pfds; i < nfds; i++, pdat_ptr++, pfd_ptr++) {
		if ((fp = user_file_of_fd(pfd_ptr->fd)) == NULL || fp->f_op->pollfd == NULL) {	/* not 
												   an 
												   open 
												   file
												   descriptor 
	...
	for (done = 0; !done;) {
		for (i = 0, ready_fd_cnt = 0, pdat_ptr = pdat_head, pfd_ptr = pfds; i < nfds;
			...
			if (pdat_ptr->pd_headp == NULL || pdat_ptr->pd_headp->ph_list == pdat_ptr
			    || pdat_ptr->pd_next != NULL || pdat_ptr->pd_prev != NULL)
		...
		if (time_out != -1) {
			...
			ticks = (time_interval + ms_per_tick - 1) / ms_per_tick;
			timer_id =
			    timeout((timo_fcn_t *) user_poll_wakeup, (caddr_t) &poll_sem, ticks);
		}
...
************************************************************************/
int
port_register_chrdev(unsigned major, const char *name, struct file_operations *fops)
{
	maj_t *m;

	if (major == 0) {
		for (major = 1; major < NMAJOR; major++)
			if (majors[major].m_op == NULL)
				break;	/* available */
	}
...
************************************************************************/
int
port_unregister_chrdev(unsigned major, char *name)
{
	maj_t *m;
...
************************************************************************/
int
user_inode_stat(inode_t * ip, struct user_stat *buf)
{
...
************************************************************************/
int
user_stat(char *name, struct user_stat *buf)
{
	dir_t *dp;
	inode_t *ip;
	int rslt;
...
************************************************************************/
int
user_fstat(int fd, struct user_stat *buf)
{
	inode_t *ip;
	file_t *fp = user_file_of_fd(fd);
...
user_print_inode(inode_t * ip)
{
	...
	printk("\ti_mode     = %o %c%s%s%s\n", ip->i_mode, dcf[(ip->i_mode >> 12) & 0x07],
	       rwx[(ip->i_mode >> 6) & 0x07], rwx[(ip->i_mode >> 3) & 0x07],
	       rwx[(ip->i_mode) & 0x07]);
...
************************************************************************/
void
user_print_dir_entry(dir_t * dp, int optns)
{
	char timbuf[50];
	int refcnt;
	...
	printk("%3d %c%s%s%s %2d %2d,%2d %s  %s\n", dp->d_inum, dcf[(dp->d_mode >> 12) & 0x07],
	       rwx[(dp->d_mode >> 6) & 0x07], rwx[(dp->d_mode >> 3) & 0x07],
	       rwx[(dp->d_mode) & 0x07], refcnt, major(dp->d_dev), minor(dp->d_dev), timbuf,
	       dp->d_name);
...
************************************************************************/
void
user_print_dir(char *name, int optns)
{
	int d;
	dir_t *dp;
	...
	for (d = 0, dp = &directory[0]; d < NDIR; d++, dp++) {
		if (dp->d_name != NULL	/* name valid */
		    && (name == NULL || strcmp(dp->d_name, name) == 0	/* name match */
		    )
		    )
 ...
 */
int
user_isastream(unsigned int fd)
{
	return 1;
}
 ...
 */
int
user_fattach(unsigned int fd, const char *path)
{
	_return(-ENOSYS);
}

...
/*
 * Loadable module object names
 *
 * This table contains the object names of all configured loadable modules.
 * It is sorted on the module name field.
 *
 * This tells LiS how to ask kerneld(8) to load the kernel module that
 * contains the streams module.
 *
 * The file modconf.c provides such a table and is generated by the
 * strconf utility.  DO NOT EDIT modconf.c BY HAND.
 * 
 */
typedef struct module_obj_name {
...
/*
 *  Return the index into lis_mod_objnames[] of a module name,
 *  or -1 if not found.
 */
int find_mod_objname(const char *name)
{
#ifdef LIS_LOADABLE_SUPPORT
	int i;

	/* TODO: Use a binary search */
	for (i = 0; i < MOD_OBJNAMES_SIZE; ++i)
		if (!strcmp(name, lis_mod_objnames[i].name))
			return i;
#endif
	return -1;
}
...
static void apush_free(autopush_t *a)
{
	...
	for (i = 0; i < a->push.npush; ++i) {
		...
		ASSERT(lis_fmod_sw[a->push.mod[i]].f_str != NULL);
...
lis_findmod(const char *name)
{
	...
	configured = 0;
	if ((i = find_mod_objname(name)) >= 0) {
		objname = lis_mod_objnames[i].objname;
		configured = 1;
	}

	/* Ask kerneld to load the module and retry */
	sprintf(req, "streams-%s", objname);
	request_module(req);
	if (configured && !strcmp(lis_fmod_sw[i].f_name, name))
		return i;
...
#ifdef LIS_LOADABLE_SUPPORT
	/* Is this driver a configured loadable */
	if (find_drv_objname(major) < 0)
...
int lis_apushm(dev_t dev, const char *mods[])
{
	...
	for (i = 0; i < a->push.npush; ++i) {
		...
		if (lis_fmod_sw[mod].f_str != NULL)
			...
			ASSERT(mod <= MOD_OBJNAMES_SIZE);
			mods[i] = lis_mod_objnames[i+1].name;
		}
#else
...
int lis_apush_get(struct strapush *ap)
{
	...
	for (i = 0; i < a->push.npush; ++i) {
		...
		if (lis_fmod_sw[id].f_str != NULL)
			...
			ASSERT(id <= MOD_OBJNAMES_SIZE);
			strncpy(ap->sap_list[i],
				lis_mod_objnames[id + 1].name, LIS_NAMESZ + 1);
		}
#else
...
int lis_valid_mod_list(struct str_list ml)
{
	...
	for (i = 0, lp = ml.sl_modlist; i < ml.sl_nmods; ++i, ++lp) {
		/* Is this module a configured loadable? */
		if (find_mod_objname(lp->l_name) >= 0)
			continue;
		/* Nope, is it registered? */
		for (j = lis_fmodcnt; j > 0; j--)
...
void lis_init_mod(void)
{
	...
	for (i = 0; i < MOD_CONFIG_SIZE; ++i) {
		...
		if (lis_register_strmod(lis_module_config[i].cnf_str,
...
void lis_terminate_mod(void)
{
	...
	for (i = 0; i < MOD_CONFIG_SIZE; ++i)
		if (lis_unregister_strmod(lis_module_config[i].cnf_str) < 0)

/*****************************************************************************
 ...
 @(#) $RCSfile: dlpidriver.c,v $ $Name:  $($Revision: 1.1.1.2.4.7 $) $Date: 2008-04-29 08:33:11 $
...
#ident "@(#) $RCSfile: dlpidriver.c,v $ $Name:  $($Revision: 1.1.1.2.4.7 $) $Date: 2008-04-29 08:33:11 $"

static char const ident[] = "$RCSfile: dlpidriver.c,v $ $Name:  $($Revision: 1.1.1.2.4.7 $) $Date: 2008-04-29 08:33:11 $";
...
STATIC int
fropen(queue_t *q, dev_t *devp, int flag, int sflag, cred_t
       *crp)
{
	int i;
...
STATIC int
frclose(queue_t *q, int flag, cred_t *crp)
{
...
STATIC retval_t
fr_bind(struct fr * fr, mblk_t *mp)
{
	int i;
	dl_bind_req_t *reqp;
	dl_bind_ack_t *ackp;
...
STATIC retval_t
fr_unbind(struct fr * fr, mblk_t *mp)
{
	mblk_t *bp;
	dl_ok_ack_t *ackp;
...
STATIC void
fr_send(struct fr *fr, mblk_t *mp)
{
	int i;
	uchar_t *p, *dest;
	...
	/* Copy f and mp + plus all continuation blocks to board memory, and send the packet */
	frsendbusy = 1;
...
STATIC void
fr_wsched(struct fr *fr, int size)
{
...
STATIC retval_t
fr_errorack(struct fr *fr, ulong_t prim, ulong_t err, ulong_t uerr)
{
	dl_error_ack_t *errp;
	mblk_t *bp;
...
STATIC void
fr_uderr(struct fr *fr, uchar_t * dest, ulong_t destlen, ulong_t err, ulong_t uerr)
{
...
frint(int vec)
{
	...
	case RECV_PKT:
		...
		size = ? ?;
		if ((bp = esballoc(DPMem->? ?, size, BPRI_MED, &frfrtn)) == NULL) {
			freeb(mp);
			frrcvlost++;
			return;
		}
...
STATIC void
fr_loop(mblk_t *mp, struct frhdr *fhp, ushort_t srcsap)
{

...
putst_test(void)
{
    ...
    /***************************/
	cmn_err(CE_CONT, "Buffer before pullupmsg(6) -- " "rptr=%p wptr=%p %d bytes\n", mp1->b_rptr,
		mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	rslt = pullupmsg(mp1, 6);
	cmn_err(CE_CONT, "Buffer after pullupmsg(6) -- " "rptr=%p wptr=%p %d bytes\n", mp1->b_rptr,
		mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	if (rslt)
	...
	cmn_err(CE_CONT, "Buffer after pullupmsg(10) -- " "rptr=%p wptr=%p %d bytes\n", mp1->b_rptr,
		mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	if (rslt) {
    ...
    /***************************/
	mp1->b_rptr++;
	cmn_err(CE_CONT, "Buffer before pullupmsg(misaligned, 6) -- " "rptr=%p wptr=%p %d bytes\n",
		mp1->b_rptr, mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	rslt = pullupmsg(mp1, 6);
	cmn_err(CE_CONT, "Buffer after pullupmsg(misaligned, 6) -- " "rptr=%p wptr=%p %d bytes\n",
		mp1->b_rptr, mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	if (rslt)
		cmn_err(CE_CONT, "pullupmsg(\"%s\", 6) OK\n", mp1->b_rptr);
	...
	mp2->b_wptr += txt2_len;
	mp1->b_cont = mp2;
	cmn_err(CE_CONT, "Buffer before pullupmsg(8) -- " "rptr=%p wptr=%p %d bytes\n", mp1->b_rptr,
		mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	rslt = pullupmsg(mp1, 8);
	cmn_err(CE_CONT, "Buffer after pullupmsg(8) -- " "rptr=%p wptr=%p %d bytes\n", mp1->b_rptr,
		mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	if (rslt)
	...
	mp3->b_wptr += txt3_len;
	mp2->b_cont = mp3;
	cmn_err(CE_CONT, "Buffer before pullupmsg(21) -- " "rptr=%p wptr=%p %d bytes\n",
		mp1->b_rptr, mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	rslt = pullupmsg(mp1, 21);
	cmn_err(CE_CONT, "Buffer after pullupmsg(21) -- " "rptr=%p wptr=%p %d bytes\n", mp1->b_rptr,
		mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	if (rslt)
	...
	cmn_err(CE_CONT, "Original buffer before pullupmsg(22) -- " "rptr=%p wptr=%p %d bytes\n",
		mp1->b_rptr, mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	cmn_err(CE_CONT, "Dup'd buffer before pullupmsg(22) -- " "rptr=%p wptr=%p %d bytes\n",
		dup_mp->b_rptr, dup_mp->b_wptr, msgdsize(dup_mp));
	lis_print_msg(dup_mp, "", PRINT_DATA_ENTIRE);
	rslt = pullupmsg(mp1, 22);
	cmn_err(CE_CONT, "Original buffer after pullupmsg(22) -- " "rptr=%p wptr=%p %d bytes\n",
		mp1->b_rptr, mp1->b_wptr, msgdsize(mp1));
	lis_print_msg(mp1, "", PRINT_DATA_ENTIRE);
	cmn_err(CE_CONT, "Dup'd buffer after pullupmsg(22) -- " "rptr=%p wptr=%p %d bytes\n",
		dup_mp->b_rptr, dup_mp->b_wptr, msgdsize(dup_mp));
	lis_print_msg(dup_mp, "", PRINT_DATA_ENTIRE);
	if (rslt)
	...
	freemsg(mp1);
	cmn_err(CE_CONT, "Dup'd buffer after free original -- " "rptr=%p wptr=%p %d bytes\n",
		dup_mp->b_rptr, dup_mp->b_wptr, msgdsize(dup_mp));
	lis_print_msg(dup_mp, "", PRINT_DATA_ENTIRE);
	freemsg(dup_mp);

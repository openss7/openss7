/*****************************************************************************
 ...
 @(#) $RCSfile: ldlconfig.c,v $ $Name:  $($Revision: 1.1.1.1.4.2 $) $Date: 2003/12/15 23:35:06 $
...
#ident "@(#) $RCSfile: ldlconfig.c,v $ $Name:  $($Revision: 1.1.1.1.4.2 $) $Date: 2003/12/15 23:35:06 $"
...
int do_info(int fd)
{
	...
	case DL_INFO_ACK:
		...
		if (reply.ack.dl_addr_length && reply.ack.dl_addr_offset &&
		    reply.ack.dl_addr_offset + reply.ack.dl_addr_length <=
		    ctlbuf.len)
		...
		if (sap_first)
		{
		    memcpy(my_sap, &ctlbuf.buf[reply.ack.dl_addr_offset],
			   sap_len);
		    memcpy(my_addr,
			   &ctlbuf.buf[reply.ack.dl_addr_offset + sap_len],
			   addr_len);
		}
		else
		{
		    memcpy(my_addr, &ctlbuf.buf[reply.ack.dl_addr_offset],
			   addr_len);
		    memcpy(my_sap,
			   &ctlbuf.buf[reply.ack.dl_addr_offset + addr_len],
			   sap_len);
		}
	    }

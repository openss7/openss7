	...
	case DL_INFO_ACK:
		...
		if (verbose > 1) {
			...
			if (reply.ack.dl_addr_length && reply.ack.dl_addr_offset &&
				...
				dumpaddr("\tAddress", &ctlbuf.buf[reply.ack.dl_addr_offset], reply.ack.dl_addr_length);
			if (reply.ack.dl_brdcst_addr_length && reply.ack.dl_brdcst_addr_offset &&
				...
				dumpaddr("\tBroadcast address", &ctlbuf.buf[reply.ack.dl_brdcst_addr_offset],
					 reply.ack.dl_brdcst_addr_length);
...
int do_curr_phys_addr(int fd)
{
	...
	case DL_PHYS_ADDR_ACK:
		...
		if (ctlbuf.len < sizeof(dl_phys_addr_ack_t)) {
			fprintf(stderr, "do_curr_phys_addr: Bad DL_PHYS_ADDR_ACK length %d\n", ctlbuf.len);
			return -1;
		}
		if (verbose > 1) {
...
int do_bind_peer(int fd, char *sap, int saplen)
{
	...
	case DL_SUBS_BIND_ACK:
		...
		if (verbose > 1) {
			...
			dumpbytes("\tSAP",
				  (char *)&reply.ack + reply.ack.dl_subs_sap_offset,
				  reply.ack.dl_subs_sap_length);
		}
		break;
...
int process_arp_reply(int fd, char *databuf, int datalen, char *dlsap)
{
	...
	if (memcmp(pr + addr_len, &target_ip_addr, 4)) {
		if (verbose > 2) {
			...
			printf("process_arp_reply(): ARP reply is not for target IP address (%s)", inet_ntoa(target_ip_addr));
	...
	if (verbose > 2)
		dumpbytes("Got ARP reply: Target hardware address",
			   target_addr, addr_len);
...
/*	if (verbose > 2)*/
	...
	if (memcmp(pr + 2 * addr_len + 4, &my_ip_addr, 4)) {
		if (verbose > 2) {
			...
			printf("process_arp_request(): Not for my address (%s)", inet_ntoa(my_ip_addr));
...
int process_incoming_arp(int fd, char *databuf, int datalen, char *dlsap)
{
	...
	switch (ntohs(arp->ar_op)) {
	    ...
	    default:
		printf("process_incoming_arp(): Ignoring unknown ARP operation code 0x%04x\n", ntohs(arp->ar_op));
		return -1;
	}
}

int process_incoming_echo_request(int fd, char *databuf,
				  int datalen, char *dlsap)
{
	 ...
	 *  Sanity checks
	 */
	if (in_cksum(&pkt->icmp_hdr,
		     ntohs(pkt->ip_hdr.tot_len) - sizeof(pkt->ip_hdr)) != 0) {
		fprintf(stderr, "process_incoming_echo_request: Bad ICMP packet checksum\n");
		dumpbytes("process_incoming_echo_request: packet dump:",
			  databuf, datalen);
		return -1;
	}
	assert(pkt->icmp_hdr.type == ICMP_ECHO);
	if (pkt->icmp_hdr.code != 0) {
		fprintf(stderr, "process_incoming_echo_request: ICMP ECHOREQUEST packet is not code 0\n.");
		return -1;
	}
	...
	pkt->icmp_hdr.checksum = in_cksum(&pkt->icmp_hdr, ntohs(pkt->ip_hdr.tot_len) - sizeof(pkt->ip_hdr));
				  ...
				  unsigned char *dst_dlsap)
{
    ...
    if (verbose > 1)
	    printf("process_incoming_llc_request: "
		   "%d bytes: %02x %02x %02x\n",
			datalen,
			databuf[0]&0xFF, databuf[1]&0xFF, databuf[2]&0xFF) ;
...
int _do_rcv_unitdata(int fd, unsigned char *data, int *datalen,
		     unsigned char *src_dlsap, unsigned char *dst_dlsap)
{
	...
	if (ctlbuf.len <= 0 && framing == LDL_FRAME_RAW_LLC)
	{
		...
		memcpy(&databuf.buf[0], &databuf.buf[2*ETH_ALEN+2],
			databuf.len) ;
		addr_len = ETH_ALEN ;
		sap_len  = 1 ;
		break ;
	    case DL_TPR:		/* Token Ring */
		{
		    ...
		    if (tp->src_addr[0] & SADDR_0_RTE_PRES)
		    ...
		    llcp = (tr_llc_frm_hdr_t *)
				    (databuf.buf + sizeof(*tp)-2 + rtelgth) ;
	...
	else
	{
		...
		if (ctlbuf.len == 0)
		    ...
		    fprintf(stderr, "_do_rcv_unitdata(): "
			    ...
			    ctlbuf.len,
			    sizeof(dl_unitdata_ind_t) + 2*(addr_len + sap_len));
		return -1;
	    }
	    ...
	    memcpy(dst_dlsap, ctlbuf.buf + reply->dl_dest_addr_offset,
		   addr_len + sap_len);
	    memcpy(src_dlsap, ctlbuf.buf + reply->dl_src_addr_offset,
		   addr_len + sap_len);
...
int do_rcv_unitdata(int fd, unsigned char *data, int *datalen,
		    unsigned char *src_dlsap, unsigned char *dst_dlsap)
{
	int dlen;
	...
	for (;;) {
		...
		if (pkt_type == ETH_P_IP) {
			...
			if (memcmp(my_addr, dlsap_addr(dst_dlsap), addr_len)) {
				if (verbose > 2)
					printf("do_rcv_unitdata(): IP packet not for my MAC address.\n");
				continue;
			}
		...
		if (ntohs(pkt_type) < 1536) {
			...
			if (memcmp(my_addr, dlsap_addr(dst_dlsap), addr_len)) {
				if (verbose > 2)
					printf("do_rcv_unitdata(): LLC frame not for my MAC address.\n");
				continue;
			}
			if (process_incoming_llc_request(fd, data, dlen,
						 src_dlsap, dst_dlsap) <= 0)
...
int send_ping(int fd, __u16 id, __u16 sequence)
{
	...
	pkt.icmp_hdr.checksum = in_cksum(&pkt.icmp_hdr,
					 sizeof(pkt) - sizeof(pkt.ip_hdr));
...
int rcv_ping(int fd, __u16 id, __u16 *sequence)
{
	...
	if (pkt.ip_pkt.icmp_hdr.code != 0) {
		fprintf(stderr, "rcv_ping: ICMP ECHOREPLY packet from our target is not code 0\n.");
		return -1;
	}
...
int initialize(void)
{
	...
	if (   framing == LDL_FRAME_RAW_LLC
	    && do_promiscon(fd, DL_PROMISC_SAP) < 0)
		exit(1) ;
	if (do_bind(fd,
		    framing == LDL_FRAME_RAW_LLC ? ETH_P_RAW : ETH_P_IP) < 0)
		exit(1);
	if (verbose > 2)
	...
	if (target_ip_addr.s_addr != 0 || arp_respond != 0) {
		...
		*(unsigned short *)arp_sap = ETH_P_ARP;
		if (do_bind_peer(fd, arp_sap, sap_len) < 0)
			exit(1);
		if (target_ip_addr.s_addr != 0 &&
		    do_arp(fd, target_ip_addr, target_addr) < 0)
			exit(1);
	}
...
{
    printf("0x00000001 dump raw frame from net driver\n");
    printf("0x00000002 dump UNITDATA_IND upstream\n");
    printf("0x00000004 dump UNITDATA_REQ downstream\n");
    printf("0x00000008 dump Tx to net driver\n");
    printf("0x00000010 dump full buffer, not just header\n");
    printf("0x00000020 debug info for DL_ATTACH\n");
    printf("0x00000040 debug info for DL_BIND\n");
}
...
void usage(char *msg)
{
	...
	fprintf(stderr, "Usage: ldltest [options] interface local-IP\n");
	fprintf(stderr, "Options:\n");
	fprintf(stderr, "  -a              Respond to ARP queries\n");
	fprintf(stderr, "  -s              Respond to ping packets\n");
	fprintf(stderr, "  -S dsap:ssap    Set DSAP and SSAP for TEST/XID\n");
	fprintf(stderr, "  -r remote-IP    IP number to ping\n");
	fprintf(stderr, "  -R remote-MAC   Remote adapter addr\n");
	fprintf(stderr, "  -c packet-count Number of ping packets to send\n");
	fprintf(stderr, "  -v number       Verbose level\n");
	fprintf(stderr, "  -f framing      Link framing: RAW, EII or SNAP\n");
	fprintf(stderr, "  -d mask         Set LDL debug mask\n");
	fprintf(stderr, "  -H              Print debug mask bit meanings\n");
	fprintf(stderr, "  -g              Print global LDL statistics\n");
	fprintf(stderr, "  -T              Send LLC TEST cmnd\n");
	fprintf(stderr, "  -X              Send LLC XID cmnd\n");
	fprintf(stderr, "Example: ldltest -c 1 -r 10.0.0.2 eth0 10.0.0.1\n");
	exit(1);
}
...
int main( int argc, char *argv[] )
{
	...
	while ((c = getopt(argc, argv, "asgHTXr:R:c:v:f:d:S:")) != EOF) {
		switch (c) {
			case 'a':
				arp_respond = 1;
				break;
			...
			case 'r':
				if (inet_aton(optarg, &target_ip_addr) == 0)
					usage("Bad remote IP number");
				break;
			case 'R':
				if (get_mac_addr(optarg, target_dlsap) <= 0)
					usage("Bad remote MAC address");
				break;
			case 'c':
				pkt_count = strtoul(optarg, &endp, 0);
				if (endp == NULL || *endp != '\0')
					usage("packet-count must be a number");
				if (pkt_count == 0)
					usage("packet-count must be > 0");
				break;
			case 'v':
				l = strtol(optarg, &endp, 0);
				if (endp == NULL || *endp != '\0')
					usage("verbose level must be a number");
				if (l < 0)
					usage("verbose level must be non-negative");
				if (l > 10)
					verbose = 10;
				else
					verbose = l;
			...
			case 'f':
				if (!strcmp(optarg, "EII"))
					framing = LDL_FRAME_EII;
				else if (!strcmp(optarg, "SNAP"))
					framing = LDL_FRAME_SNAP;
				else if (!strcmp(optarg, "RAW"))
					framing = LDL_FRAME_RAW_LLC ;
				else
					usage("framing must be \"EII\" or \"SNAP\"");
				break;
			case 'd':
				l = strtol(optarg, &endp, 0);
				if (endp == NULL || *endp != '\0')
					usage("debug mask must be a number");
				do_set_debug_mask(l) ;
				exit(0);
			case 'H':
				print_debug_bits() ;
			...
			case 'g':
				...
				exit(0) ;
			case 'S':
				target_sap[0] = (unsigned char)
						    strtol(optarg, &endp, 0) ;
				if (*endp == ':')
				    my_llc_sap[0] = (unsigned char)
						    strtol(endp+1, &endp, 0) ;
				saps_set = 1 ;
				break;
			case 'T':
	...
	if (test_ping && xid_ping)
	    usage("-T and -X are exclusive options") ;
	...
	if (optind != argc - 2)
		usage("Wrong number of arguments after options");
	interface = argv[optind++];
	if (!test_xid)
	{
	    if (inet_aton(argv[optind], &my_ip_addr) == 0)
		    usage("Bad local IP number");

	    /* An extra sanity check */
	    if (target_ip_addr.s_addr == 0 &&
		    arp_respond == 0 && echo_respond == 0)
		usage("No remote to ping, and not responding to ARP or ping:\n"
	...
	for (i = 0; i < pkt_count; ++i)
	{
	    ...
	    if (i%1000 == 0) {
		    ...
		    rtt = ( 1000.0 * tv1.tv_sec + tv1.tv_usec / 1000.0 ) -

...
************************************************************************/
void
link_on_end(lookup_struct_t ** head, lookup_struct_t * item)
{
	lookup_struct_t *p;
...
************************************************************************/
driver_info_t *
mk_driver(char *name, char *prefix, int *maj, int nmaj, int nmin)
{
	driver_info_t *p;
...
************************************************************************/
module_info_t *
mk_module(char *name, char *prefix, int units)
{
	module_info_t *p;
...
************************************************************************/
node_info_t *
mk_node(char *name, int type, int maj, int min)
{
	node_info_t *p;
...
************************************************************************/
void *
lookup_name(void *list_head, char *name)
{
	lookup_struct_t *p;
...
************************************************************************/
driver_info_t *
find_driver_by_major(int major)
{
	driver_info_t *dp;
	int i;
...
get_next(void)
{
	...
	while (*nextp && *nextp <= ' ')
		nextp++;	/* skip white space */

	while (*nextp && *nextp > ' ' && n < sizeof(token) - 1) {	/* copy token */
		*p++ = *nextp++;
		n++;
	}

	while (*nextp && *nextp > ' ')
		nextp++;	/* skip excess chars */
	...
	*p = 0;			/* NUL terminate the token */
	...
	if (token[0] != '\0' && *endp == '\0' && val < LONG_MAX && val > LONG_MIN) {	/* valid
											   number */
		class = NUMBER;
		value = val;
	}
#endif
	else if (token[0] == 0) {	/* end of line */
...
process_line(void)
{
	...
	if (strcmp(token, "driver") == 0) {
		...
		else if (nmajors <= major(nminors - 1)) {
			err("Too few majors (%d) specified for %d minors.\n", nmajors, nminors);
		}

		if (nmajors == 1) {
			if (major0 < 0)
				major0 = get_free_major();
	...
	} else if (strcmp(token, "loadable") == 0) {
		object_info_t *op;
	...
	} else if (strcmp(token, "objname") == 0) {
		object_info_t *op;
	...
	} else if (strcmp(token, "autopush") == 0) {
		autopush_info_t *ap;
		driver_info_t *dp;
		...
		if (ap == NULL) {
			fprintf(stderr, "Cannot allocate autopush entry for device \"%s\"\n",
				dp->name);
			return;
		}

		if (get_next() != NUMBER || value < -1 || value > INT_MAX) {
			fprintf(stderr, "Invalid autopush entry (minor) for device \"%s\"\n",
				dp->name);
			free(ap);
			return;
		}
		...
		if (get_next() != NUMBER || value < 0 || value > INT_MAX) {
			fprintf(stderr, "Invalid autopush entry (lastminor) for device \"%s\"\n",
				dp->name);
			free(ap);
			return;
		}
...
#define INRANGE(x, lo, hi) ( (x) >= (lo) && (x) <= (hi) )
		if (ap->minor == -1) {	/* SAP_ALL */
			if (dp->ap != NULL) {
				fprintf(stderr, "Conflicting autopush entries for device \"%s\"\n",
					dp->name);
				free(ap);
				return;
			}
...
#define	p2(f,y,z)	fprintf(outfile, f,y,z)
	...
	p0("/* WARNING:  THIS FILE WAS GENERATED. " " MANUAL CHANGES MAY BE LOST. */\n");
	p0("\n");
	p0("\n");
	...
	for (dp = driver_head; dp != NULL; dp = dp->link) {
		...
		if (dp->init)
			p1("extern void _RP" " %sinit(void) ;\n", dp->prefix);
		if (dp->term)
			p1("extern void _RP" " %sterm(void) ;\n", dp->prefix);
		if (dp->intr) {
			p1("extern void _RP" " %sintr(void) ;\n", dp->prefix);
			p1("\nint _RP" "__%sintr(int vect,void *dev,struct pt_regs *reg)\n",
			   dp->prefix);
			p0("{\n");
	...
	for (mp = module_head; mp != NULL; mp = mp->link) {
		p1("	{\"%s\",", mp->name);
		if (mp->obj->loadable) {	/* no streamtab, incl obj name */
			p1(" NULL, \"%s\", ", mp->obj->name);
			p1("%d},\n", mp->qlock_option);
		} else {	/* streamtab, no obj name */
			p1(" &%sinfo,", mp->prefix);
			p1(" \"\", %d},\n", mp->qlock_option);
		}
	}
...
build_config(void)
{
	...
	for (drp = driver_head; drp != NULL; drp = drp->link) {
		...
		for (i = 0; i < drp->nmajors; i++)
			p3("#define\t%s_CMAJOR_%d\t%d\n", raise(drp->prefix), i + drp->inst_nr,
			   drp->major[i]);
...
build_drvrconf(void)
{
	...
	fprintf(f, "# WARNING:  THIS FILE WAS GENERATED. " " MANUAL CHANGES MAY BE LOST.\n\n");
...
get_options(int argc, char **argv)
{
	...
	if (infile_name == NULL || outfile_name == NULL || mkfile_name == NULL || mknod_name == NULL
	    || config_name == NULL) {
		print_options();
		exit(1);
	}

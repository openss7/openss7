...
float
 local_cpu_rate, remote_cpu_rate;
...
int
 ...
 local_send_offset = 0, local_recv_offset = 0, remote_recv_align,	/* alignment for remote
									   receives */
 remote_send_align,			/* alignment for remote sends */
 remote_send_offset = 0, remote_recv_offset = 0;

#ifdef INTERVALS
int
 interval_usecs, interval_wate, interval_burst;
...
#ifdef _GNU_SOURCE
	while ((c =
		getopt_long(argc, argv, GLOBAL_CMD_LINE_ARGS, long_options,
			    &option_index)) != EOF) {
#else				/* _GNU_SOURCE */

...
dnl =========================================================================
dnl
dnl @(#) $Id: streams.m4,v 0.9.2.2 2004/08/06 12:44:08 brian Exp $
dnl
dnl =========================================================================
dnl
...
# _STREAMS_ROOTDIR
# -------------------------------------------------------------------------
AC_DEFUN([_STREAMS_ROOTDIR], [dnl
    AC_ARG_VAR([DESTDIR], [Cross build root directory])
    AC_ARG_WITH([rootdir],
        AS_HELP_STRING([--with-rootdir=DIR],
            [specify the root directory for configure.
            @<:@default=${DESTDIR:-/}@:>@]),
        [with_rootdir="$withval"],
        [with_rootdir="$DESTDIR"])
    AC_MSG_CHECKING([for streams root directory])
    streams_cv_rootdir="$with_rootdir"
    AC_MSG_RESULT([${streams_cv_rootdir:-no}])
])# _STREAMS_ROOTDIR
# =========================================================================
...
# _STREAMS_PREFIX
# -------------------------------------------------------------------------
AC_DEFUN([_STREAMS_PREFIX], [dnl
    AC_ARG_WITH([prefix],
        AS_HELP_STRING([--with-prefix=DIR],
            [specify the directory prefix for install.
            @<:@default=/@:>@]),
        [with_prefix="$withval"],
        [with_prefix=])
    AC_MSG_CHECKING([for kernel root prefix])
    if test :"${with_prefix:-no}" != :no
    then
        streams_cv_prefix="$with_prefix"
    else
        if test :"$prefix" = :NONE
        then
            streams_cv_prefix="$ac_default_prefix"
        else
            streams_cv_prefix="$prefix"
        fi
        dnl strip trailing [/usr][/local]
        streams_cv_prefix=`echo $streams_cv_prefix | sed -e 's|/local$||;s|/usr$||'`
    fi
    AC_MSG_RESULT([${streams_cv_prefix:-no}])
])# _STREAMS_PREFIX
# =========================================================================
...
# _STREAMS_LFS_CHECK_HEADERS
# -------------------------------------------------------------------------
AC_DEFUN([_STREAMS_LFS_CHECK_HEADERS], [
...
dnl AC_MSG_CHECKING([for streams lfs include directory])
    ...
    then
        ...
        eval "streams_search_path=\"
            ...
            $streams_cv_rootdir$streams_cv_prefix/usr/src/streams/include
...
# _STREAMS_OUTPUT
# -------------------------------------------------------------------------
AC_DEFUN([_STREAMS_OUTPUT], [
    case "$streams_cv_package" in
        LiS)
            _STREAMS_LIS_DEFINES
            : ;;
        LfS)
            _STREAMS_LFS_DEFINES
            : ;;
    esac
])# _STREAMS_OUTPUT
# =========================================================================

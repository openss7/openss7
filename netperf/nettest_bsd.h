/*****************************************************************************
 ...
 @(#) $Id: nettest_bsd.h,v 1.1.1.7 2007/08/14 07:26:49 brian Exp $
 ...
 $Log: nettest_bsd.h,v $
 Revision 1.1.1.7  2007/08/14 07:26:49  brian
 - GPLv3 header update
...
#ident "@(#) $RCSfile: nettest_bsd.h,v $ $Name:  $($Revision: 1.1.1.7 $) Copyright (c) 2001-2007 OpenSS7 Corporation."
...
struct sctp_stream_request_struct {
	...
	int ipaddress;			/* the IP address to which the recv side should bind to
					   allow netperf to run through those evil firewall things */
};
...
struct sctp_maerts_request_struct {
	...
	int recv_buf_size;		/* how big does the client want it - the */
	/* receive socket buffer that is */
	int send_size;			/* how many bytes do we want netserver to send at one time? 
					 */

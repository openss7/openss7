/*****************************************************************************
 ...
 @(#) $RCSfile: netserver.c,v $ $Name:  $($Revision: 1.1.1.17 $) $Date: 2007/08/14 07:26:45 $
...
#ident "@(#) $RCSfile: netserver.c,v $ $Name:  $($Revision: 1.1.1.17 $) $Date: 2007/08/14 07:26:45 $"
...
#define IN6_CLEAR_IN6ADDR(x) 	memset(&(x), 0, sizeof(struct in6_addr))
		...
		fprintf(stderr, "netserver: IPv6 is not supported\n");
		fflush(stderr);
		exit(1);
	}
#endif
	...
	if (setsockopt(server_control,
		       SOL_SOCKET, SO_REUSEADDR, (char *) &on, sizeof(on)) == SOCKET_ERROR) {
...
#define SIGCLD SIGCHLD
		...
		for (;;) {
			if ((server_sock = accept(server_control,
						  &peeraddr, &sockaddr_len)) == INVALID_SOCKET) {
...
#ifdef WIN32
			{
				...
				if (verbosity > 1) {
					snprintf(&cmdline[strlen(cmdline)],
						 sizeof(cmdline) - strlen(cmdline), " -v %d",
						 verbosity);
				}
				for (i = 0; i < debug; i++) {
					snprintf(&cmdline[strlen(cmdline)],
						 sizeof(cmdline) - strlen(cmdline), " -d");
				}
				snprintf(&cmdline[strlen(cmdline)],
					 sizeof(cmdline) - strlen(cmdline), " -I %x",
  ...
  // child should close since they shouldn't have been inherited ("-i#"), and a single switch to
  // specify
...
/*  unlink(DEBUG_LOG_FILE); */
	...
	// is public and is used for more than just POSIX so hopefully it won't go away.
		...
		/* we are probably a child of inetd, or are being invoked via the VMS auxilliarly
		   server mechanism */
#if !defined(__VMS)
		server_sock = 0;
#else

/*****************************************************************************
 ...
 @(#) $RCSfile: nettest_bsd.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 1.1.1.19 $) $Date: 2007-08-14 07:26:45 $
 ...
 $Log: nettest_bsd.c,v $
 Revision 1.1.1.19  2007-08-14 07:26:45  brian
 - GPLv3 header update

 *****************************************************************************/

#ident "@(#) $RCSfile: nettest_bsd.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 1.1.1.19 $) $Date: 2007-08-14 07:26:45 $"
...
/*	recv_sctp_stream()					*/
/*	send_sctp_maerts()	perform a sctp stream test	*/
/*	recv_sctp_maerts()	in the other direction		*/
/*	send_sctp_rr()		perform a sctp request/response	*/
/*	recv_sctp_rr()						*/
/*	send_sctp_conn_rr()	an RR test including connect	*/
/*	recv_sctp_conn_rr()					*/
/*	send_sctp_cc()		a connect/disconnect test with	*/
/*	recv_sctp_cc()		no RR				*/
...
static char remote_cpu_method;
 ...
 /* or the SCTP_CRR and/or SCTP_TRR tests. */
...
#ifdef DO_SCTP
#ifdef SCTP_CORK
 loc_sctpcork = 0,			/* don't/do use SCTP_CORK locally */
 rem_sctpcork = 0,			/* don't/do use SCTP_CORK remotely */
#endif
				/* SCTP_CORK */
#endif
				/* DO_SCTP */
...
\n\
SCTP/TCP/UDP BSD Sockets Test Options:\n\
    -C                Set (SCTP|TCP)_CORK when available\n\
...

#ifdef DO_SCTP
 /* This routine is intended to retrieve interesting aspects of sctp */
...
static
    void
get_sctp_info(SOCKET socket, int *mss)
{
	...
	if (getsockopt(socket,
		       getprotobyname("sctp")->p_proto,
		       SCTP_MAXSEG, (char *) mss, &sock_opt_len) == SOCKET_ERROR) {
		fprintf(where, "netperf: get_sctp_info: getsockopt SCTP_MAXSEG: errno %d\n", errno);
		fflush(where);
		*mss = -1;
	}
#else
...
get_tcp_info(SOCKET socket, int *mss)
{
	...
	if (getsockopt(socket,
		       ...
		       TCP_MAXSEG, (char *) mss, &sock_opt_len) == SOCKET_ERROR) {
		fprintf(where, "netperf: get_tcp_info: getsockopt TCP_MAXSEG: errno %d\n", errno);
		fflush(where);
		*mss = -1;
	}
#else
...
complete_sockaddr(struct sockaddr_in *server, char *remote_host, int ip_address)
{
	...
	/* however, if the user provided a remote IP address with the -I option we will honour that 
	   above all else. raj 2004-06-15 */

	/* NOTE:- this should be changed for SCTP to return all IP addresses belonging to the host
	   in an array of IP addresses. */
...
static
 SOCKET
create_data_socket(int family, int type, int protocol, int address, unsigned short port)
{
	...
	temp_socket = socket(family, type, protocol);

	if (temp_socket == INVALID_SOCKET) {
		fprintf(where, "netperf: create_data_socket: socket: errno %d\n", errno);
		fflush(where);
		exit(1);
	}
	...
	/* Modify the local socket size. The reason we alter the send buffer size here rather than
	   when the connection is made is to take care of decreases in buffer size. Decreasing the
	   window size after connection establishment is a TCP no-no. Also, by setting the buffer
	   (window) size before the connection is established, we can control the TCP MSS (segment
	   size). The MSS is never (well, should never be) more that 1/2 the minimum receive buffer 
	   size at each half of the connection.  This is why we are altering the receive buffer
	   size on the sending size of a unidirectional transfer. If the user has not requested
	   that the socket buffers be altered, we will try to find-out what their values are. If we 
	   cannot touch the socket buffer in any way, we will set the values to -1 to indicate
	   that.  */

	/* all the oogy nitty gritty stuff moved from here into the routine being called below, per 
	   patches from davidm to workaround the bug in Linux getsockopt().  raj 2004-06-15 */
	...
	if (loc_rcvavoid) {
		if (setsockopt(temp_socket,
			       SOL_SOCKET,
			       SO_RCV_COPYAVOID, &loc_rcvavoid, sizeof(int)) == SOCKET_ERROR) {
			fprintf(where,
	...
	if (loc_sndavoid) {
		if (setsockopt(temp_socket,
			       SOL_SOCKET,
			       SO_SND_COPYAVOID, &loc_sndavoid, sizeof(int)) == SOCKET_ERROR) {
			fprintf(where,
	...
	switch ((type << 8) | protocol) {
#ifdef DO_SCTP
#ifdef SCTP_NODELAY
	case ((SOCK_STREAM << 8) | IPPROTO_SCTP):
	case ((SOCK_SEQPACKET << 8) | 0):
	case ((SOCK_SEQPACKET << 8) | IPPROTO_SCTP):
		/* Note:- for OpenSS7 SCTP, SCTP_NODELAY defaults to true instead of false. */
		if (!loc_nodelay) {
			one = 0;
			if (setsockopt(temp_socket,
				       SOL_SCTP,
				       SCTP_NODELAY, (char *) &one, sizeof(one)) == SOCKET_ERROR) {
				fprintf(where,
			...
			if (debug > 1) {
				fprintf(where,
					"netperf: create_data_socket: SCTP_NODELAY not requested...\n");
				fflush(where);
			}
		}
		break;
...
#endif				/* DO_SCTP */
#ifdef TCP_NODELAY
	case ((SOCK_STREAM << 8) | 0):
	case ((SOCK_STREAM << 8) | IPPROTO_TCP):
		if (loc_nodelay) {
			one = 1;
			if (setsockopt(temp_socket,
				       SOL_TCP,
				       TCP_NODELAY, (char *) &one, sizeof(one)) == SOCKET_ERROR) {
				fprintf(where,
					"netperf: create_data_socket: nodelay: errno %d\n", errno);
				fflush(where);
			}
	...
	default:
	case ((SOCK_DGRAM << 8) | 0):
	case ((SOCK_DGRAM << 8) | IPPROTO_UDP):
		loc_nodelay = 0;
		break;
	}

	/* since some of the UDP tests do not do anything to cause an implicit bind() call, we need 
	   to be rather explicit about our bind() call here. even if the address and/or the port
	   are zero (INADDR_ANY etc). raj 2004-07-20 */

	if (setsockopt(temp_socket, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) < 0) {
		fprintf(where, "netperf: create_data_socket: SO_REUSEADDR failled %d\n", errno);
		fflush(where);
	}
	...
	if (bind(temp_socket, (struct sockaddr *) &temp, sizeof(struct sockaddr_in)) < 0) {
		fprintf(where,
...
void
kludge_socket_options(int temp_socket, int level)
{
	...
	case SOL_SCTP:
		if (!loc_nodelay) {
			...
			if (setsockopt(temp_socket,
				       ...
				       SCTP_NODELAY, (char *) &one, sizeof(one)) == SOCKET_ERROR) {
				fprintf(where,
					"netperf: kludge_socket_options: nodelay: errno %d\n",
					errno);
				fflush(where);
			}
			...
			if (debug > 1) {
				fprintf(where,
					"netperf: kludge_socket_options: SCTP_NODELAY not requested...\n");
				fflush(where);
			}
		}
		break;
...
%6d\n";
	...
	if (print_headers) {
		...
		fprintf(where, "SCTP STREAM TEST");
		fprintf(where, " to %s", remote_host);
		if (iteration_max > 1) {
			fprintf(where,
				" : +/-%3.1f%% @ %2d%% conf.", interval / 0.02, confidence_level);
		}
		if (loc_nodelay || rem_nodelay) {
			fprintf(where, " : nodelay");
		}
		if (loc_sndavoid || loc_rcvavoid || rem_sndavoid || rem_rcvavoid) {
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		send_socket = create_data_socket(AF_INET,
						 SOCK_STREAM,
						 IPPROTO_SCTP, local_data_ip, local_data_port);
		...
		if (send_ring == NULL) {
			...
			send_ring = allocate_buffer_ring(send_width,
							 send_size,
							 local_send_align, local_send_offset);
		}
...
#endif				/* DIRTY */
		sctp_stream_request->port = (int) remote_data_port;
		sctp_stream_request->ipaddress = remote_data_ip;
		if (debug > 1) {
		...
		if (!netperf_response.content.serv_errno) {
			...
			/* network order */
			server.sin_port = (short) sctp_stream_response->data_port_number;
			...
			fprintf(where,
				"netperf: remote error %d", netperf_response.content.serv_errno);
			perror("");
			fflush(where);
		...
		if (connect(send_socket,
			    (struct sockaddr *) &server, sizeof(server)) == INVALID_SOCKET) {
		...
		if (sigprocmask(SIG_BLOCK, (sigset_t *) NULL, &signal_set) != 0) {
			fprintf(where, "send_sctp_stream: unable to get sigmask errno %d\n", errno);
			fflush(where);
			exit(1);
		}
#endif				/* INTERVALS */
		...
		while ((!times_up) || (bytes_remaining > 0)) {
			...
			if ((len = send(send_socket,
					send_ring->buffer_ptr, send_size, 0)) != send_size) {
		...
		if (shutdown(send_socket, 1) == SOCKET_ERROR) {
			perror("netperf: cannot shutdown sctp stream socket");
			exit(1);
		}
		...
		if (local_cpu_usage || remote_cpu_usage) {
			...
			if (local_cpu_usage) {
				...
				local_service_demand = calc_service_demand(bytes_sent, 0.0, 0.0, 0);
			} else {
	...
	if (local_cpu_usage || remote_cpu_usage) {
		...
		case 0:
			if (local_cpu_usage) {
				fprintf(where, cpu_fmt_0, local_service_demand, local_cpu_method);
			} else {
				fprintf(where, cpu_fmt_0, remote_service_demand, remote_cpu_method);
			}
			break;
		case 1:
		case 2:
			if (print_headers) {
				fprintf(where,
					cpu_title,
					format_units(), local_cpu_method, remote_cpu_method);
			}

			fprintf(where, cpu_fmt_1,	/* the format string */
		...
		switch (verbosity) {
		case 0:
			fprintf(where, tput_fmt_0, thruput);
			break;
		case 1:
		case 2:
			if (print_headers) {
			...
			fprintf(where, tput_fmt_1,	/* the format string */
				rsr_size,	/* remote recvbuf size */
				lss_size,	/* local sendbuf size */
				send_size,	/* how large were the sends */
	...
	if (verbosity > 1) {
		...
		fprintf(where, ksink_fmt2, sctp_mss);
		fflush(where);
#ifdef HISTOGRAM
...
%6d\n";
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		send_socket = create_data_socket(AF_INET,
						 SOCK_STREAM,
						 IPPROTO_TCP, local_data_ip, local_data_port);
		...
		/* get the signal set for the call to sigsuspend */
		if (sigprocmask(SIG_BLOCK, (sigset_t *) NULL, &signal_set) != 0) {
			fprintf(where, "send_tcp_stream: unable to get sigmask errno %d\n", errno);
			fflush(where);
			exit(1);
		}
#endif				/* INTERVALS */
		...
		while ((!times_up) || (bytes_remaining > 0)) {
			...
			if ((interval_burst) && (--interval_count == 0)) {
				...
				if (sigsuspend(&signal_set) == EFAULT) {
					fprintf(where, "send_tcp_stream: fault with sigsuspend.\n");
					fflush(where);
					exit(1);
				}
				interval_count = interval_burst;
	...
	if (verbosity > 1) {
		...
		fprintf(where, ksink_fmt2, tcp_mss);
		fflush(where);
#ifdef HISTOGRAM
		fprintf(where, "\n\nHistogram of time spent in send() call.\n");
		fflush(where);
...
/* This routine implements the netperf-side SCTP unidirectional data
...
void
send_sctp_maerts(char remote_host[])
{
...
%6d\n";
	...
	struct sctp_maerts_results_struct *sctp_maerts_result;

	sctp_maerts_request =
	    (struct sctp_maerts_request_struct *) netperf_request.content.test_specific_data;
	sctp_maerts_response =
	    (struct sctp_maerts_response_struct *) netperf_response.content.test_specific_data;
	sctp_maerts_result =
	    (struct sctp_maerts_results_struct *) netperf_response.content.test_specific_data;
	...
	if (print_headers) {
		...
		/* only print the test title here and will print the results */
		/* titles after the test is finished */
		fprintf(where, "SCTP MAERTS TEST");
		fprintf(where, " to %s", remote_host);
		if (iteration_max > 1) {
			fprintf(where,
				" : +/-%3.1f%% @ %2d%% conf.", interval / 0.02, confidence_level);
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (recv_socket == INVALID_SOCKET) {
			perror("netperf: send_sctp_maerts: sctp stream data socket");
			exit(1);
		}

		if (debug) {
			fprintf(where, "send_sctp_maerts: recv_socket obtained...\n");
		}
		...
		if (recv_ring == NULL) {
			...
			recv_ring = allocate_buffer_ring(recv_width,
							 recv_size,
							 local_recv_align, local_recv_offset);
		}
		...
		netperf_request.content.request_type = DO_SCTP_MAERTS;
		sctp_maerts_request->send_buf_size = rss_size;
		sctp_maerts_request->recv_buf_size = rsr_size;
		sctp_maerts_request->send_size = send_size;
		sctp_maerts_request->no_delay = rem_nodelay;
		sctp_maerts_request->send_alignment = remote_send_align;
		sctp_maerts_request->send_offset = remote_send_offset;
		sctp_maerts_request->measure_cpu = remote_cpu_usage;
		sctp_maerts_request->cpu_rate = remote_cpu_rate;
		if (test_time) {
			sctp_maerts_request->test_length = test_time;
		} else {
			sctp_maerts_request->test_length = test_bytes;
		}
		sctp_maerts_request->so_rcvavoid = rem_rcvavoid;
		sctp_maerts_request->so_sndavoid = rem_sndavoid;
#ifdef DIRTY
		sctp_maerts_request->dirty_count = rem_dirty_count;
		sctp_maerts_request->clean_count = rem_clean_count;
#endif				/* DIRTY */
		sctp_maerts_request->port = (int) remote_data_port;
		sctp_maerts_request->ipaddress = remote_data_ip;

		if (debug > 1) {
			fprintf(where, "netperf: send_sctp_maerts: requesting SCTP maerts test\n");
		}
		...
		if (!netperf_response.content.serv_errno) {
			if (debug)
				fprintf(where, "remote listen done.\n");
			rsr_size = sctp_maerts_response->recv_buf_size;
			rss_size = sctp_maerts_response->send_buf_size;
			rem_nodelay = sctp_maerts_response->no_delay;
			remote_cpu_usage = sctp_maerts_response->measure_cpu;
			remote_cpu_rate = sctp_maerts_response->cpu_rate;
			send_size = sctp_maerts_response->send_size;

			/* we have to make sure that the server port number is in */
			/* network order */
			server.sin_port = (short) sctp_maerts_response->data_port_number;
			server.sin_port = htons(server.sin_port);
			rem_rcvavoid = sctp_maerts_response->so_rcvavoid;
			rem_sndavoid = sctp_maerts_response->so_sndavoid;
		} else {
			Set_errno(netperf_response.content.serv_errno);
			fprintf(where,
				"netperf: remote error %d", netperf_response.content.serv_errno);
		...
		if (connect(recv_socket,
			    (struct sockaddr *) &server, sizeof(server)) == INVALID_SOCKET) {
			perror("netperf: send_sctp_maerts: data socket connect failed");
			printf(" port: %d\n", ntohs(server.sin_port));
			exit(1);
		}
		...
		if (test_time) {
			...
			/* The tester wanted to recv a number of bytes. we don't do that in a
			   SCTP_MAERTS test. sorry. raj 2002-06-21 */
			printf("netperf: send_sctp_maerts: test must be timed\n");
			exit(1);
		}
		...
		/* get the signal set for the call to sigsuspend */
		if (sigprocmask(SIG_BLOCK, (sigset_t *) NULL, &signal_set) != 0) {
			fprintf(where, "send_sctp_maerts: unable to get sigmask errno %d\n", errno);
			fflush(where);
			exit(1);
		}
#endif				/* INTERVALS */
		...
		/* the test will continue until we either get a zero-byte recv() on the socket or
		   our failsafe timer expires. most of the time we trust that we get a zero-byte
		   recieve from the socket. raj 2002-06-21 */
		...
		while ((len = recv(recv_socket, recv_ring->buffer_ptr, recv_size, 0)) > 0) {
#ifdef HISTOGRAM
			...
			if ((interval_burst) && (--interval_count == 0)) {
				...
				if (sigsuspend(&signal_set) == EFAULT) {
					fprintf(where,
						"send_sctp_maerts: fault with sigsuspend.\n");
					fflush(where);
					exit(1);
				}
				interval_count = interval_burst;
		...
		/* if we get here, it must mean we had a recv return of 0 before the watchdog timer 
		   expired */

		/* The test is over. Flush the buffers to the remote end. We do a graceful release
		   to tell the remote we have all the data. */

		/* but first, if the verbosity is greater than 1, find-out what */
		/* the SCTP maximum segment_size was (if possible) */
		if (verbosity > 1) {
			sctp_mss = -1;
			get_sctp_info(recv_socket, &sctp_mss);
		}

		if (shutdown(recv_socket, 1) == SOCKET_ERROR) {
			perror("netperf: cannot shutdown sctp maerts socket");
			exit(1);
		}

		stop_timer();

		/* this call will always give us the local elapsed time for the test, and will also 
		   store-away the necessaries for cpu utilization */
		...
		/* We now calculate what our thruput was for the test. In the future, */
		/* we may want to include a calculation of the thruput measured by */
		/* the remote, but it should be the case that for a SCTP maerts test, */
		/* that the two numbers should be *very* close... We calculate */
		/* bytes_sent regardless of the way the test length was controlled. */
		/* If it was time, we needed to, and if it was by bytes, the user may */
		/* have specified a number of bytes that wasn't a multiple of the */
		...
		bytes_sent = ntohd(sctp_maerts_result->bytes_sent);
		...
		if (local_cpu_usage || remote_cpu_usage) {
			...
			if (remote_cpu_usage) {
				...
				remote_cpu_utilization = sctp_maerts_result->cpu_util;
				remote_service_demand = calc_service_demand(bytes_sent,
									    0.0,
									    remote_cpu_utilization,
									    sctp_maerts_result->
	...
	if (local_cpu_usage || remote_cpu_usage) {
		local_cpu_method = format_cpu_method(cpu_method);
		remote_cpu_method = format_cpu_method(sctp_maerts_result->cpu_method);
	...
	if (verbosity > 1) {
		...
		fprintf(where,
			...
			bytes_sent / (double) nummessages,
			nummessages,
			bytes_sent / (double) sctp_maerts_result->send_calls,
			sctp_maerts_result->send_calls);
		fprintf(where, ksink_fmt2, sctp_mss);
		fflush(where);
#ifdef HISTOGRAM
...
%6d\n";
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (test_time) {
			...
			/* The tester wanted to recv a number of bytes. we don't do that in a
			   TCP_MAERTS test. sorry. raj 2002-06-21 */
		...
		/* get the signal set for the call to sigsuspend */
		if (sigprocmask(SIG_BLOCK, (sigset_t *) NULL, &signal_set) != 0) {
			fprintf(where, "send_tcp_maerts: unable to get sigmask errno %d\n", errno);
			fflush(where);
			exit(1);
		}
#endif				/* INTERVALS */
		...
		while ((len = recv(recv_socket, recv_ring->buffer_ptr, recv_size, 0)) > 0) {
			...
			if ((interval_burst) && (--interval_count == 0)) {
				...
				if (sigsuspend(&signal_set) == EFAULT) {
					fprintf(where, "send_tcp_maerts: fault with sigsuspend.\n");
					fflush(where);
					exit(1);
				}
				interval_count = interval_burst;
...
ssize_t
sendpath(int s, char *path, off_t offset, size_t nbytes, const struct iovec *hdtrl, int flags)
{
...
/* This routine implements the SCTP unidirectional data transfer test
   ...
   system call - SCTP_SENDFILE.  It receives its  parameters via global
   variables from the shell and writes its  output to the standard
   output. Basically,  this is the same test as the send_sctp_stream()
   logic and we even tell the remote to do a SCTP_STREAM test since for 
   all it knows, nothig is different. */

void
sendfile_sctp_stream(remote_host)
	char remote_host[];
{
...
%6d\n";
	...
	off_t scratch_offset;		/* the linux sendfile() call will update the offset
					   variable, which is something we do _not_ want to happen
					   to the value in the send_ring! so, we have to use a
					   scratch variable. */
	...
	if (print_headers) {
		...
		fprintf(where, "SCTP SENDPATH TEST");
#else
		fprintf(where, "SCTP SENDFILE TEST");
#endif				/* QUICK_SENDPATH */
		fprintf(where, " to %s", remote_host);
		if (iteration_max > 1) {
			fprintf(where,
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		send_socket = create_data_socket(AF_INET,
						 ...
						 IPPROTO_SCTP, local_data_ip, local_data_port);
		if (send_socket == INVALID_SOCKET) {
			perror("netperf: sendfile_sctp_stream: sctp stream data socket");
			exit(1);
		}

		if (debug) {
			fprintf(where, "sendfile_sctp_stream: send_socket obtained...\n");
		}
#if defined(SCTP_CORK)

		if (loc_sctpcork != 0) {
			/* the user wishes for us to set SCTP_CORK on the socket */
			int one = 1;

			if (setsockopt(send_socket,
				       SOL_SCTP,
				       SCTP_CORK, (char *) &one, sizeof(one)) == SOCKET_ERROR) {
				perror("netperf: sendfile_sctp_stream: sctp_cork");
				exit(1);
			}
			if (debug) {
				fprintf(where, "sendfile_sctp_stream: sctp_cork...\n");
			}
		}
#endif				/* SCTP_CORK */
		...
		/* set-up the data buffer ring with the requested alignment and offset. note also
		   that we have allocated a quantity of memory that is at least one send-size
		   greater than our socket buffer size. We want to be sure that there are at least
		   two buffers allocated - this can be a bit of a problem when the send_size is
		   bigger than the socket size, so we must check... the user may have wanted to
		   explicitly set the "width" of our send buffers, we should respect that wish... */
...
/*section of the file sent throughout the file*/
		...
		if (send_ring == NULL) {
			...
			/* only allocate the send ring once. this is a networking test, not a
			   memory allocation test. this way, we do not need a
			   deallocate_buffer_ring() routine, and I don't feel like writing one
			   anyway :) raj 11/94 */
		...
		/* If the user has requested cpu utilization measurements, we must calibrate the
		   cpu(s). We will perform this task within the tests themselves. If the user has
		   specified the cpu rate, then calibrate_local_cpu will return rather quickly as
		   it will have nothing to do. If local_cpu_rate is zero, then we will go through
		   all the "normal" calibration stuff and return the rate back. */
		...
		/* Tell the remote end to do a listen. The server alters the socket paramters on
		   the other side at this point, hence the reason for all the values being passed
		   in the setup message. If the user did not specify any of the parameters, they
		   will be passed as 0, which will indicate to the remote that no changes beyond
		   the system's default should be used. Alignment is the exception, it will default 
		   to 1, which will be no alignment alterations. */
		...
		/* The response from the remote will contain all of the relevant socket parameters
		   for this test type. We will put them back into the variables here so they can be 
		   displayed if desired. The remote will have calibrated CPU if necessary, and will 
		   have done all the needed set-up we will have calibrated the cpu locally before
		   sending the request, and will grab the counter value right after the connect
		   returns. The remote will grab the counter right after the accept call. This
		   saves the hassle of extra messages being sent for the SCTP tests.  */
		...
		/* Data Socket set-up is finished. If there were problems, either the connect would 
		   have failed, or the previous response would have indicated a problem. I failed
		   to see the value of the extra message after the accept on the remote. If it
		   failed, we'll see it here. If it didn't, we might as well start pumping data. */
		...
		if (test_time) {
			...
			/* in previous revisions, we had the same code repeated throught all the
			   test suites. this was unnecessary, and meant more work for me when I
			   wanted to switch to POSIX signals, so I have abstracted this out into a
			   routine in netlib.c. if you are experiencing signal problems, you might
			   want to look there. raj 11/94 */
		...
		/* We use an "OR" to control test execution. When the test is controlled by time,
		   the byte count check will always return false. When the test is controlled by
		   byte count, the time test will always return false. When the test is finished,
		   the whole expression will go false and we will stop sending data. */

		while ((!times_up) || (bytes_remaining > 0)) {

			/* the sendfile_sctp_stream test does not support making the buffers dirty. 
			   08/2000 */

#ifdef HISTOGRAM

			/* timestamp just before we go into sendfile() and then again just after we 
			   come out raj 08/2000 */

			HIST_timestamp(&time_one);
#endif				/* HISTOGRAM */

			/* you can look at netlib.h for a description of the fields we are passing
			   to sendfile(). 08/2000 */
#ifdef QUICK_SENDPATH
			if ((len = sendpath(send_socket,
					    ...
					    send_ring->offset,
					    send_ring->length,
					    send_ring->hdtrl, send_ring->flags)) != send_size)
#else
#if defined(__linux__)  || defined(__SunOS_5_9)
			scratch_offset = send_ring->offset;
			if ((len = sendfile(send_socket, send_ring->fildes, &scratch_offset,	/* modified 
												   after 
												   the 
												   call! 
												 */
			...
			if ((sendfile(send_ring->fildes,
				      ...
				      NULL,
				      (off_t *) & len,
				      send_ring->flags) != 0) || (len != send_size))
...
#endif				/* QUICK_SENDPATH */
			{
				/* the test was interrupted, must be the end of test. the
				   send_sctp_stream code has some WIN32 ifdefs that we do not need
				   here. */
				if ((len >= 0) || SOCKET_EINTR(len)) {
					break;
				}
...
/*	offset += len;*/
			...
			if ((interval_burst) && (--interval_count == 0)) {
				...
				if (sigsuspend(&signal_set) == EFAULT) {
					fprintf(where, "send_udp_stream: fault with sigsuspend.\n");
					fflush(where);
					exit(1);
				}
		...
		/* The test is over. Flush the buffers to the remote end. We do a graceful release
		   to insure that all data has been taken by the remote. */
		...
		/* since we are using sendfile() instead of send, we have no scratch buffer from
		   the send_ring to use for the receive. however, since we "know" that the recv
		   should be returning zero bytes (not that we are making the checks we should) we
		   can pass the address of the flags field. raj 08/2000 */

		recv(send_socket, &(send_ring->flags), sizeof(send_ring->flags), 0);
...
%6d\n";
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (loc_tcpcork != 0) {
			...
			if (setsockopt(send_socket,
				       SOL_TCP,
				       TCP_CORK, (char *) &one, sizeof(one)) == SOCKET_ERROR) {
...
/*section of the file sent throughout the file*/
		...
		/* The response from the remote will contain all of the relevant socket parameters
		   ...
		   sending the request, and will grab the counter value right after the connect
		   returns. The remote will grab the counter right after the accept call. This
		   saves the hassle of extra messages being sent for the TCP tests.  */
		...
		if (sigprocmask(SIG_BLOCK, (sigset_t *) NULL, &signal_set) != 0) {
			fprintf(where, "send_udp_stream: unable to get sigmask errno %d\n", errno);
			fflush(where);
			exit(1);
		}
#endif				/* INTERVALS */
		...
		while ((!times_up) || (bytes_remaining > 0)) {
			...
			/* the sendfile_tcp_stream test does not support making the buffers dirty.
			   08/2000 */
			...
			{
				/* the test was interrupted, must be the end of test. the
				   send_tcp_stream code has some WIN32 ifdefs that we do not need
				   here. */
				if ((len >= 0) || SOCKET_EINTR(len)) {
					break;
				}
...
recv_sctp_stream()
{
	...
	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM,
				      IPPROTO_SCTP,
				      sctp_stream_request->ipaddress, sctp_stream_request->port);
	...
	if (getsockname(s_listen, (struct sockaddr *) &myaddr_in, &addrlen) == SOCKET_ERROR) {
		netperf_response.content.serv_errno = errno;
		close(s_listen);
		send_response();
	...
	if (sctp_stream_request->measure_cpu) {
		...
		sctp_stream_response->cpu_rate = calibrate_local_cpu(sctp_stream_request->cpu_rate);
	} else {
	...
	if ((s_data = accept(s_listen,
			     (struct sockaddr *) &peeraddr_in, &addrlen)) == INVALID_SOCKET) {
	...
	kludge_socket_options(s_data, SOL_SCTP);
	...
	if (debug) {
		fprintf(where, "recv_sctp_stream: got %g bytes\n", bytes_received);
		fprintf(where, "recv_sctp_stream: got %d recvs\n", receive_calls);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_stream: test complete, sending results.\n");
		fprintf(where,
			"                 bytes_received %g receive_calls %d\n",
			bytes_received, receive_calls);
		fprintf(where, "                 len %d\n", len);
		fflush(where);
	}
...
recv_tcp_stream()
{
	...
	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM,
				      IPPROTO_TCP,
				      tcp_stream_request->ipaddress, tcp_stream_request->port);
	...
	kludge_socket_options(s_data, SOL_TCP);
	...
	if (debug) {
		fprintf(where, "recv_tcp_stream: got %g bytes\n", bytes_received);
		fprintf(where, "recv_tcp_stream: got %d recvs\n", receive_calls);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_tcp_stream: test complete, sending results.\n");
		fprintf(where,
			"                 bytes_received %g receive_calls %d\n",
			bytes_received, receive_calls);
		fprintf(where, "                 len %d\n", len);
...
/* This is the server-side routine for the sctp maerts test. It is
   implemented as one routine. I could break things-out somewhat, but
   didn't feel it was necessary. */
...
recv_sctp_maerts()
{
	...
	struct sctp_maerts_request_struct *sctp_maerts_request;
	struct sctp_maerts_response_struct *sctp_maerts_response;
	struct sctp_maerts_results_struct *sctp_maerts_results;
	...
	if (debug) {
		fprintf(where, "netserver: recv_sctp_maerts: entered...\n");
		fflush(where);
	}

	/* We want to set-up the listen socket with all the desired parameters and then let the
	   initiator know that all is ready. If socket size defaults are to be used, then the
	   initiator will have sent us 0's. If the socket sizes cannot be changed, then we will
	   send-back what they are. If that information cannot be determined, then we send-back
	   -1's for the sizes. If things go wrong for any reason, we will drop back ten yards and
	   punt. */

	/* If anything goes wrong, we want the remote to know about it. It would be best if the
	   error that the remote reports to the user is the actual error we encountered, rather
	   than some bogus unexpected response type message. */

	if (debug) {
		fprintf(where, "recv_sctp_maerts: setting the response type...\n");
		fflush(where);
	}

	netperf_response.content.response_type = SCTP_MAERTS_RESPONSE;

	if (debug) {
		fprintf(where, "recv_sctp_maerts: the response type is set...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_maerts: requested alignment of %d\n",
			sctp_maerts_request->send_alignment);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_maerts: grabbing a socket...\n");
		fflush(where);
	}
	...
	/* once the socket has been created, we will set the response values */
	/* based on the updated value of those globals. raj 7/94 */
	lss_size_req = sctp_maerts_request->send_buf_size;
	lsr_size_req = sctp_maerts_request->recv_buf_size;
	loc_nodelay = sctp_maerts_request->no_delay;
	loc_rcvavoid = sctp_maerts_request->so_rcvavoid;
	loc_sndavoid = sctp_maerts_request->so_sndavoid;

	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM,
				      IPPROTO_SCTP,
				      sctp_maerts_request->ipaddress, sctp_maerts_request->port);
	...
	/* what sort of sizes did we end-up with? */
	if (sctp_maerts_request->send_size == 0) {
		if (lss_size > 0) {
			send_size = lss_size;
		} else {
			send_size = 4096;
		...
		send_size = sctp_maerts_request->send_size;
	}
	...
	send_ring = allocate_buffer_ring(send_width,
					 send_size,
					 sctp_maerts_request->send_alignment,
					 sctp_maerts_request->send_offset);

	if (debug) {
		fprintf(where, "recv_sctp_maerts: receive alignment and offset set...\n");
		fflush(where);
	}
	...
	sctp_maerts_response->data_port_number = (int) ntohs(myaddr_in.sin_port);
	netperf_response.content.serv_errno = 0;
	...
	sctp_maerts_response->cpu_rate = (float) 0.0;	/* assume no cpu */
	if (sctp_maerts_request->measure_cpu) {
		sctp_maerts_response->measure_cpu = 1;
		sctp_maerts_response->cpu_rate = calibrate_local_cpu(sctp_maerts_request->cpu_rate);
	} else {
		sctp_maerts_response->measure_cpu = 0;
	}

	/* before we send the response back to the initiator, pull some of */
	/* the socket parms from the globals */
	sctp_maerts_response->send_buf_size = lss_size;
	sctp_maerts_response->recv_buf_size = lsr_size;
	sctp_maerts_response->no_delay = loc_nodelay;
	sctp_maerts_response->so_rcvavoid = loc_rcvavoid;
	sctp_maerts_response->so_sndavoid = loc_sndavoid;
	sctp_maerts_response->send_size = send_size;

	send_response();

	addrlen = sizeof(peeraddr_in);

	/* we will start the timer before the accept() to be somewhat analagous to the starting of
	   the timer before the connect() call in the SCTP_STREAM test. raj 2002-06-21 */

	start_timer(sctp_maerts_request->test_length);

	/* Now it's time to start receiving data on the connection. We will first grab the
	   apropriate counters and then start grabbing. */

	cpu_start(sctp_maerts_request->measure_cpu);
	...
	/* this is for those systems which *INCORRECTLY* fail to pass attributes across an accept() 
	   call. Including this goes against my better judgement :( raj 11/95 */
	...
	dirty_count = sctp_maerts_request->dirty_count;
	clean_count = sctp_maerts_request->clean_count;
	message_int_ptr = (int *) send_ring->buffer_ptr;
	for (i = 0; i < dirty_count; i++) {
		*message_int_ptr = rand();
		message_int_ptr++;
	...
	len = 0;		// nt-lint; len is not initialized (printf far below) if times_up
	// initially true.
	while (!times_up) {
		...
		if ((len = send(s_data, send_ring->buffer_ptr, send_size, 0)) != send_size) {
			if ((len >= 0) || SOCKET_EINTR(len)) {
				/* the test was interrupted, must be the end of test */
				break;
			}
	...
	/* hang a recv() off the socket to block until the remote has brought all the data up into
	   the application. it will do a shutdown to cause a FIN to be sent our way. We will assume 
	   that any exit from the recv() call is good... raj 4/93 */

	recv(s_data, send_ring->buffer_ptr, send_size, 0);

	cpu_stop(sctp_maerts_request->measure_cpu, &elapsed_time);

	/* send the results to the sender */

	if (debug) {
		fprintf(where, "recv_sctp_maerts: got %g bytes\n", bytes_sent);
		fprintf(where, "recv_sctp_maerts: got %d sends\n", send_calls);
		fflush(where);
	}

	sctp_maerts_results->bytes_sent = htond(bytes_sent);
	sctp_maerts_results->elapsed_time = elapsed_time;
	sctp_maerts_results->send_calls = send_calls;

	if (sctp_maerts_request->measure_cpu) {
		sctp_maerts_results->cpu_util = calc_cpu_util(0.0);
	};

	if (debug) {
		fprintf(where, "recv_sctp_maerts: test complete, sending results.\n");
		fprintf(where,
	...
	sctp_maerts_results->cpu_method = cpu_method;
	sctp_maerts_results->num_cpus = lib_num_loc_cpus;
	send_response();
...
recv_tcp_maerts()
{
	...
	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM,
				      IPPROTO_TCP,
				      tcp_maerts_request->ipaddress, tcp_maerts_request->port);
	...
	/* we will start the timer before the accept() to be somewhat analagous to the starting of
	   the timer before the connect() call in the TCP_STREAM test. raj 2002-06-21 */
	...
	if (debug) {
		fprintf(where, "recv_tcp_maerts: got %g bytes\n", bytes_sent);
		fprintf(where, "recv_tcp_maerts: got %d sends\n", send_calls);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_tcp_maerts: test complete, sending results.\n");
		fprintf(where,
			"                 bytes_sent %g send_calls %d\n", bytes_sent, send_calls);
		fprintf(where, "                 len %d\n", len);
		fflush(where);
...
%5d  %5d   %5d  %5d";
	...
	if (print_headers) {
		fprintf(where, "SCTP REQUEST/RESPONSE TEST");
		fprintf(where, " to %s", remote_host);
		if (iteration_max > 1) {
			fprintf(where,
				" : +/-%3.1f%% @ %2d%% conf.", interval / 0.02, confidence_level);
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		sctp_rr_request->recv_buf_size = rsr_size;
		sctp_rr_request->send_buf_size = rss_size;
		...
		sctp_rr_request->port = (int) remote_data_port;
		sctp_rr_request->ipaddress = remote_data_ip;
		...
		if (!netperf_response.content.serv_errno) {
			...
			server.sin_port = (short) sctp_rr_response->data_port_number;
			server.sin_port = htons(server.sin_port);
		} else {
			Set_errno(netperf_response.content.serv_errno);
			fprintf(where,
		...
		/* get the signal set for the call to sigsuspend */
		if (sigprocmask(SIG_BLOCK, (sigset_t *) NULL, &signal_set) != 0) {
			fprintf(where, "send_sctp_rr: unable to get sigmask errno %d\n", errno);
			fflush(where);
			exit(1);
		}
#endif				/* INTERVALS */
...
#ifdef DO_FIRST_BURST
		{
			...
			for (i = 0; i < first_burst_size; i++) {
				if ((len = send(send_socket,
						send_ring->buffer_ptr, req_size, 0)) != req_size) {
					/* we should never hit the end of the test in the first
					   burst */
		...
		while ((!times_up) || (trans_remaining > 0)) {
			...
			if ((interval_burst) && (--interval_count == 0)) {
				...
				if (sigsuspend(&signal_set) == EFAULT) {
					fprintf(where, "send_sctp_rr: fault with signal set!\n");
					fflush(where);
					exit(1);
				}
				interval_count = interval_burst;
		...
		if (local_cpu_usage || remote_cpu_usage) {
			...
			if (local_cpu_usage) {
				...
				local_service_demand =
				    calc_service_demand((double) nummessages * 1024, 0.0, 0.0, 0);
			} else {
				local_cpu_utilization = (float) -1.0;
				local_service_demand = (float) -1.0;
			}
			...
			if (remote_cpu_usage) {
				...
				remote_service_demand =
				    calc_service_demand((double) nummessages * 1024, 0.0,
							remote_cpu_utilization,
	...
	if (local_cpu_usage || remote_cpu_usage) {
		...
		case 2:
			if (print_headers) {
				fprintf(where, cpu_title, local_cpu_method, remote_cpu_method);
			}

			fprintf(where, cpu_fmt_1_line_1,	/* the format string */
				lss_size,	/* local sendbuf size */
				lsr_size, req_size,	/* how large were the requests */
				rsp_size,	/* guess */
				elapsed_time,	/* how long was the test */
				thruput, local_cpu_utilization,	/* local cpu */
				remote_cpu_utilization,	/* remote cpu */
				local_service_demand,	/* local service demand */
				remote_service_demand);	/* remote service demand */
			fprintf(where, cpu_fmt_1_line_2, rss_size, rsr_size);
			break;
		}
	} else {
		/* The tester did not wish to measure service demand. */
		...
		case 2:
			...
			fprintf(where, tput_fmt_1_line_1,	/* the format string */
				lss_size, lsr_size, req_size,	/* how large were the requests */
				rsp_size,	/* how large were the responses */
				elapsed_time,	/* how long did it take */
				thruput);
			fprintf(where, tput_fmt_1_line_2, rss_size,	/* remote recvbuf size */
				rsr_size);
	...
	if (verbosity > 1) {
		...
		fprintf(where,
			ksink_fmt,
			local_send_align,
			remote_recv_offset, local_send_offset, remote_recv_offset);
...
%5d  %5d   %5d  %5d";
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		/* get the signal set for the call to sigsuspend */
		if (sigprocmask(SIG_BLOCK, (sigset_t *) NULL, &signal_set) != 0) {
			fprintf(where, "send_tcp_rr: unable to get sigmask errno %d\n", errno);
			fflush(where);
			exit(1);
		}
#endif				/* INTERVALS */
		...
		while ((!times_up) || (trans_remaining > 0)) {
			...
			if ((interval_burst) && (--interval_count == 0)) {
				...
				if (sigsuspend(&signal_set) == EFAULT) {
					fprintf(where, "send_tcp_rr: fault with signal set!\n");
					fflush(where);
					exit(1);
				}
				interval_count = interval_burst;
...
%6d           %-7.2f   %7d           %7.1f     %-6.2f   %-6.3f\n\n";
	...
	float elapsed_time, local_cpu_utilization, remote_cpu_utilization;
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		data_socket = create_data_socket(AF_INET,
						 SOCK_DGRAM,
						 IPPROTO_UDP, local_data_ip, local_data_port);
		...
		while (!times_up) {
			...
			if ((interval_burst) && (--interval_count == 0)) {
				...
				if (sigsuspend(&signal_set) == EFAULT) {
					fprintf(where, "send_udp_stream: fault with signal set!\n");
					fflush(where);
					exit(1);
				}
				interval_count = interval_burst;
	...
	if (local_cpu_usage || remote_cpu_usage) {
		...
		case 0:
			if (local_cpu_usage) {
				...
				fprintf(where, cpu_fmt_0, remote_service_demand, local_cpu_method);
			}
			break;
		case 1:
		case 2:
			...
			fprintf(where, cpu_fmt_1,	/* the format string */
				...
				send_size,	/* how large were the sends */
				elapsed_time,	/* how long was the test */
				messages_sent, failed_sends, local_thruput,	/* what was the
										   xfer rate */
				local_cpu_utilization,	/* local cpu */
				local_service_demand,	/* local service demand */
				rsr_size, elapsed_time, messages_recvd, remote_thruput, remote_cpu_utilization,	/* remote 
														   cpu 
														 */
				remote_service_demand);	/* remote service demand */
			break;
		...
		switch (verbosity) {
		case 0:
			fprintf(where, tput_fmt_0, local_thruput);
			break;
		case 1:
		case 2:
			if (print_headers) {
			...
			fprintf(where, tput_fmt_1,	/* the format string */
				...
				send_size,	/* how large were the sends */
				elapsed_time,	/* how long did it take */
				messages_sent, failed_sends, local_thruput, rsr_size,	/* remote
											   recvbuf
											   size */
				elapsed_time, messages_recvd, remote_thruput);
			break;
		}
	}
...
recv_udp_stream()
{
	...
	s_data = create_data_socket(AF_INET,
				    SOCK_DGRAM,
				    IPPROTO_UDP,
				    udp_stream_request->ipaddress, udp_stream_request->port);
	...
	/* now get the port number assigned by the system */
	addrlen = sizeof(myaddr_in);
	if (getsockname(s_data, (struct sockaddr *) &myaddr_in, &addrlen) == SOCKET_ERROR) {
		netperf_response.content.serv_errno = errno;
		close(s_data);
		send_response();
	...
	while (!times_up) {
		...
		if ((len = recvfrom(s_data, recv_ring->buffer_ptr, message_size, 0, 0, 0)
		    ) != message_size)
#else
		if ((len = recv(s_data, recv_ring->buffer_ptr, message_size, 0)) != message_size)
#endif
		{
	...
	if (debug) {
		fprintf(where, "recv_udp_stream: got %d bytes\n", bytes_received);
		fflush(where);
	}
...
%-6d %-6d\n";
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		/* get the signal set for the call to sigsuspend */
		if (sigprocmask(SIG_BLOCK, (sigset_t *) NULL, &signal_set) != 0) {
			fprintf(where, "send_udp_rr: unable to get sigmask errno %d\n", errno);
			fflush(where);
			exit(1);
		}
#endif				/* INTERVALS */
		...
		while ((!times_up) || (trans_remaining > 0)) {
			...
			if ((interval_burst) && (--interval_count == 0)) {
				...
				if (sigsuspend(&signal_set) == EFAULT) {
					fprintf(where, "send_udp_rr: fault with signal set!\n");
					fflush(where);
					exit(1);
				}
				interval_count = interval_burst;
	...
	if (local_cpu_usage || remote_cpu_usage) {
		...
		case 2:
			...
			fprintf(where, cpu_fmt_1_line_1,	/* the format string */
				...
				rsp_size,	/* guess */
				elapsed_time,	/* how long was the test */
				nummessages / elapsed_time, local_cpu_utilization,	/* local
											   cpu */
				remote_cpu_utilization,	/* remote cpu */
				local_service_demand,	/* local service demand */
				remote_service_demand);	/* remote service demand */
		...
		switch (verbosity) {
		case 0:
			fprintf(where, tput_fmt_0, nummessages / elapsed_time);
			break;
		case 1:
		case 2:
			if (print_headers) {
...
recv_udp_rr()
{
	...
	s_data = create_data_socket(AF_INET,
				    SOCK_DGRAM, IPPROTO_UDP, INADDR_ANY, udp_rr_request->port);
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if (debug > 3) {
			fprintf(where, "recv_udp_rr: Transaction %d complete.\n", trans_received);
			fflush(where);
		}
	...
	if (debug) {
		fprintf(where, "recv_udp_rr: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_udp_rr: test complete, sending results.\n");
		fflush(where);
	}
...
recv_sctp_rr()
{
	...
	if (debug) {
		fprintf(where, "recv_sctp_rr: requested recv alignment of %d offset %d\n",
			sctp_rr_request->recv_alignment, sctp_rr_request->recv_offset);
		fprintf(where, "recv_sctp_rr: requested send alignment of %d offset %d\n",
			sctp_rr_request->send_alignment, sctp_rr_request->send_offset);
		fflush(where);
	}
	...
	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM,
				      IPPROTO_SCTP,
				      sctp_rr_request->ipaddress, sctp_rr_request->port);
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if ((bytes_sent = send(s_data,
				       send_ring->buffer_ptr,
				       sctp_rr_request->response_size, 0)) == SOCKET_ERROR) {
	...
	if (debug) {
		fprintf(where, "recv_sctp_rr: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_rr: test complete, sending results.\n");
		fflush(where);
	}
...
recv_tcp_rr()
{
	...
	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM,
				      IPPROTO_TCP, tcp_rr_request->ipaddress, tcp_rr_request->port);
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if ((bytes_sent = send(s_data,
				       send_ring->buffer_ptr,
				       tcp_rr_request->response_size, 0)) == SOCKET_ERROR) {
			if (SOCKET_EINTR(bytes_sent)) {
				/* the test timer has popped */
				timed_out = 1;
				fprintf(where, "yo6\n");
	...
	if (debug) {
		fprintf(where, "recv_tcp_rr: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_tcp_rr: test complete, sending results.\n");
		fflush(where);
	}
...
loc_cpu_rate()
{
	...
	fprintf(where, "%g", calibrate_local_cpu(0.0));
...
rem_cpu_rate()
{
	...
	fprintf(where, "%g", calibrate_remote_cpu());
...

 ...
 /* comparision of T/TCP with SCTP for transactional workloads. */
 /* it will also look (can look) much like the communication pattern */
 /* of http for www access. */

void
send_sctp_conn_rr(char remote_host[])
{
...
%5d  %5d   %5d  %5d\n";
	...
	struct sctp_conn_rr_request_struct *sctp_conn_rr_request;
	struct sctp_conn_rr_response_struct *sctp_conn_rr_response;
	struct sctp_conn_rr_results_struct *sctp_conn_rr_result;

	sctp_conn_rr_request =
	    (struct sctp_conn_rr_request_struct *) netperf_request.content.test_specific_data;
	sctp_conn_rr_response =
	    (struct sctp_conn_rr_response_struct *) netperf_response.content.test_specific_data;
	sctp_conn_rr_result =
	    (struct sctp_conn_rr_results_struct *) netperf_response.content.test_specific_data;
	...
	if (print_headers) {
		fprintf(where, "SCTP Connect/Request/Response TEST");
		fprintf(where, " to %s", remote_host);
		if (iteration_max > 1) {
			fprintf(where,
				" : +/-%3.1f%% @ %2d%% conf.", interval / 0.02, confidence_level);
	...
	send_ring = allocate_buffer_ring(send_width, req_size, local_send_align, local_send_offset);

	recv_ring = allocate_buffer_ring(recv_width, rsp_size, local_recv_align, local_recv_offset);

	if (debug) {
		fprintf(where, "send_sctp_conn_rr: send_socket obtained...\n");
	}
	...
	netperf_request.content.request_type = DO_SCTP_CRR;
	sctp_conn_rr_request->recv_buf_size = rsr_size;
	sctp_conn_rr_request->send_buf_size = rss_size;
	sctp_conn_rr_request->recv_alignment = remote_recv_align;
	sctp_conn_rr_request->recv_offset = remote_recv_offset;
	sctp_conn_rr_request->send_alignment = remote_send_align;
	sctp_conn_rr_request->send_offset = remote_send_offset;
	sctp_conn_rr_request->request_size = req_size;
	sctp_conn_rr_request->response_size = rsp_size;
	sctp_conn_rr_request->no_delay = rem_nodelay;
	sctp_conn_rr_request->measure_cpu = remote_cpu_usage;
	sctp_conn_rr_request->cpu_rate = remote_cpu_rate;
	sctp_conn_rr_request->so_rcvavoid = rem_rcvavoid;
	sctp_conn_rr_request->so_sndavoid = rem_sndavoid;
	if (test_time) {
		sctp_conn_rr_request->test_length = test_time;
	} else {
		sctp_conn_rr_request->test_length = test_trans * -1;
	}
	sctp_conn_rr_request->port = (int) remote_data_port;

	if (debug > 1) {
		fprintf(where, "netperf: send_sctp_conn_rr: requesting SCTP crr test\n");
	}
	...
	if (!netperf_response.content.serv_errno) {
		rsr_size = sctp_conn_rr_response->recv_buf_size;
		rss_size = sctp_conn_rr_response->send_buf_size;
		rem_nodelay = sctp_conn_rr_response->no_delay;
		remote_cpu_usage = sctp_conn_rr_response->measure_cpu;
		remote_cpu_rate = sctp_conn_rr_response->cpu_rate;
		/* make sure that port numbers are in network order */
		server.sin_port = (short) sctp_conn_rr_response->data_port_number;
		server.sin_port = htons(server.sin_port);
		if (debug) {
			fprintf(where, "remote listen done.\n");
		...
		Set_errno(netperf_response.content.serv_errno);
		fprintf(where, "netperf: remote error %d", netperf_response.content.serv_errno);
		perror("");
		fflush(where);
		exit(1);
	}
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if (send_socket == INVALID_SOCKET) {
			perror("netperf: send_sctp_conn_rr: sctp stream data socket");
			exit(1);
		}
		...
		if (setsockopt(send_socket, SOL_SOCKET, SO_REUSEADDR,
			...
			perror("netperf: send_sctp_conn_rr: so_reuseaddr");
			exit(1);
		}
		...
		/* broken SCTP's might treat the port number as a signed 16 bit */
		...
		if (myport == ntohs(server.sin_port))
			myport++;
		...
		if ((ret = bind(send_socket,
			...
			if (debug > 1) {
				fprintf(where,
					"send_sctp_conn_rr: tried to bind to port %d errno %d\n",
					ntohs(myaddr->sin_port), errno);
				fflush(where);
			}
		...
		if ((ret = connect(send_socket,
			...
			printf("\tattempted to connect on socket %d to port %d",
			       send_socket, ntohs(server.sin_port));
			printf(" from port %d \n", ntohs(myaddr->sin_port));
			exit(1);
		}

		/* send the request */
		if ((len = send(send_socket, send_ring->buffer_ptr, req_size, 0)) != req_size) {
			if (SOCKET_EINTR(len)) {
				/* we hit the end of a */
				/* timed test. */
				timed_out = 1;
			...
			perror("send_sctp_conn_rr: data send error");
			exit(1);
		}
		send_ring = send_ring->next;
		...
		while ((rsp_bytes_recvd = recv(send_socket,
					       temp_message_ptr, rsp_bytes_left, 0)) > 0) {
			rsp_bytes_left -= rsp_bytes_recvd;
		...
		if (rsp_bytes_recvd == 0) {
			...
			if (debug > 3) {
				fprintf(where,
					...
					nummessages, ntohs(myaddr->sin_port));
				fflush(where);
			}
			...
			perror("send_sctp_conn_rr: data recv error");
			exit(1);
		}
	...
	/* Kbytes/s as the units of thruput for a SCTP stream test, where K = */
	...
	if (local_cpu_usage || remote_cpu_usage) {
		...
		if (remote_cpu_usage) {
			...
			remote_cpu_utilization = sctp_conn_rr_result->cpu_util;
			/* since calc_service demand is doing ms/Kunit we will */
			/* multiply the number of transaction by 1024 to get */
			/* "good" numbers */
		...
		case 0:
			if (local_cpu_usage) {
				fprintf(where, cpu_fmt_0, local_service_demand);
			} else {
				fprintf(where, cpu_fmt_0, remote_service_demand);
			}
			break;
		case 1:
		case 2:
...
recv_sctp_conn_rr()
{
	...
	struct sctp_conn_rr_request_struct *sctp_conn_rr_request;
	struct sctp_conn_rr_response_struct *sctp_conn_rr_response;
	struct sctp_conn_rr_results_struct *sctp_conn_rr_results;

	sctp_conn_rr_request =
	    (struct sctp_conn_rr_request_struct *) netperf_request.content.test_specific_data;
	sctp_conn_rr_response =
	    (struct sctp_conn_rr_response_struct *) netperf_response.content.test_specific_data;
	sctp_conn_rr_results =
	    (struct sctp_conn_rr_results_struct *) netperf_response.content.test_specific_data;

	if (debug) {
		fprintf(where, "netserver: recv_sctp_conn_rr: entered...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_conn_rr: setting the response type...\n");
		fflush(where);
	}

	netperf_response.content.response_type = SCTP_CRR_RESPONSE;

	if (debug) {
		fprintf(where, "recv_sctp_conn_rr: the response type is set...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where,
			"recv_sctp_conn_rr: requested recv alignment of %d offset %d\n",
			sctp_conn_rr_request->recv_alignment, sctp_conn_rr_request->recv_offset);
		fprintf(where,
			"recv_sctp_conn_rr: requested send alignment of %d offset %d\n",
			sctp_conn_rr_request->send_alignment, sctp_conn_rr_request->send_offset);
		fflush(where);
	}

	recv_message_ptr =
	    ALIGN_BUFFER(message, sctp_conn_rr_request->recv_alignment,
			 sctp_conn_rr_request->recv_offset);

	send_message_ptr =
	    ALIGN_BUFFER(message, sctp_conn_rr_request->send_alignment,
			 sctp_conn_rr_request->send_offset);

	if (debug) {
		fprintf(where, "recv_sctp_conn_rr: receive alignment and offset set...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_conn_rr: grabbing a socket...\n");
		fflush(where);
	}
	...
	/* once the socket has been created, we will set the response values */
	/* based on the updated value of those globals. raj 7/94 */
	lss_size_req = sctp_conn_rr_request->send_buf_size;
	lsr_size_req = sctp_conn_rr_request->recv_buf_size;
	loc_nodelay = sctp_conn_rr_request->no_delay;
	loc_rcvavoid = sctp_conn_rr_request->so_rcvavoid;
	loc_sndavoid = sctp_conn_rr_request->so_sndavoid;

	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM,
				      IPPROTO_SCTP,
				      sctp_conn_rr_request->ipaddress, sctp_conn_rr_request->port);
	...
	sctp_conn_rr_response->data_port_number = (int) ntohs(myaddr_in.sin_port);
	if (debug) {
		fprintf(where, "telling the remote to call me at %d\n",
			sctp_conn_rr_response->data_port_number);
		fflush(where);
	}
	netperf_response.content.serv_errno = 0;
	...
	sctp_conn_rr_response->cpu_rate = (float) 0.0;	/* assume no cpu */
	if (sctp_conn_rr_request->measure_cpu) {
		sctp_conn_rr_response->measure_cpu = 1;
		sctp_conn_rr_response->cpu_rate =
		    calibrate_local_cpu(sctp_conn_rr_request->cpu_rate);
	}

	/* before we send the response back to the initiator, pull some of */
	/* the socket parms from the globals */
	sctp_conn_rr_response->send_buf_size = lss_size;
	sctp_conn_rr_response->recv_buf_size = lsr_size;
	sctp_conn_rr_response->no_delay = loc_nodelay;
	sctp_conn_rr_response->so_rcvavoid = loc_rcvavoid;
	sctp_conn_rr_response->so_sndavoid = loc_sndavoid;
	...
	cpu_start(sctp_conn_rr_request->measure_cpu);

	/* The loop will exit when the sender does a shutdown, which will */
	/* return a length of zero */

	if (sctp_conn_rr_request->test_length > 0) {
		times_up = 0;
		trans_remaining = 0;
		start_timer(sctp_conn_rr_request->test_length + PAD_TIME);
	} else {
		times_up = 1;
		trans_remaining = sctp_conn_rr_request->test_length * -1;
	}
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if ((s_data = accept(s_listen,
			...
			fprintf(where, "recv_sctp_conn_rr: accept: errno = %d\n", errno);
			fflush(where);
			close(s_listen);
		...
		if (debug) {
			fprintf(where, "recv_sctp_conn_rr: accepted data connection.\n");
			fflush(where);
		}
#ifdef WIN32
		/* this is used so the timer thread can close the socket out from */
		...
		request_bytes_remaining = sctp_conn_rr_request->request_size;
		...
		if ((bytes_sent = send(s_data,
				       send_message_ptr,
				       sctp_conn_rr_request->response_size, 0)) == SOCKET_ERROR) {
			if (errno == EINTR) {
				/* the test timer has popped */
				timed_out = 1;
				fprintf(where, "yo6\n");
	...
	cpu_stop(sctp_conn_rr_request->measure_cpu, &elapsed_time);
	...
	if (debug) {
		fprintf(where, "recv_sctp_conn_rr: got %d transactions\n", trans_received);
		fflush(where);
	}

	sctp_conn_rr_results->bytes_received = (trans_received *
						(sctp_conn_rr_request->request_size +
						 sctp_conn_rr_request->response_size));
	sctp_conn_rr_results->trans_received = trans_received;
	sctp_conn_rr_results->elapsed_time = elapsed_time;
	if (sctp_conn_rr_request->measure_cpu) {
		sctp_conn_rr_results->cpu_util = calc_cpu_util(elapsed_time);
	}

	if (debug) {
		fprintf(where, "recv_sctp_conn_rr: test complete, sending results.\n");
		fflush(where);
	}
...
recv_tcp_conn_rr()
{
	...
	recv_message_ptr =
	    ALIGN_BUFFER(message, tcp_conn_rr_request->recv_alignment,
			 tcp_conn_rr_request->recv_offset);

	send_message_ptr =
	    ALIGN_BUFFER(message, tcp_conn_rr_request->send_alignment,
	...
	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM,
				      IPPROTO_TCP,
				      tcp_conn_rr_request->ipaddress, tcp_conn_rr_request->port);
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if ((bytes_sent = send(s_data,
				       send_message_ptr,
				       tcp_conn_rr_request->response_size, 0)) == SOCKET_ERROR) {
			if (errno == EINTR) {
				/* the test timer has popped */
				timed_out = 1;
				fprintf(where, "yo6\n");
	...
	if (debug) {
		fprintf(where, "recv_tcp_conn_rr: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_tcp_conn_rr: test complete, sending results.\n");
		fflush(where);
	}
...
%5d  %5d   %5d  %5d\n";
	...
	struct sctp_tran_rr_response_struct *sctp_tran_rr_response;
	struct sctp_tran_rr_results_struct *sctp_tran_rr_result;

	sctp_tran_rr_request =
	    (struct sctp_tran_rr_request_struct *) netperf_request.content.test_specific_data;
	sctp_tran_rr_response =
	    (struct sctp_tran_rr_response_struct *) netperf_response.content.test_specific_data;
	sctp_tran_rr_result =
	    (struct sctp_tran_rr_results_struct *) netperf_response.content.test_specific_data;
	...
	if (debug) {
		fprintf(where, "send_sctp_tran_rr: send_socket obtained...\n");
	}
	...
	sctp_tran_rr_request->recv_buf_size = rsr_size;
	sctp_tran_rr_request->send_buf_size = rss_size;
	sctp_tran_rr_request->recv_alignment = remote_recv_align;
	sctp_tran_rr_request->recv_offset = remote_recv_offset;
	sctp_tran_rr_request->send_alignment = remote_send_align;
	sctp_tran_rr_request->send_offset = remote_send_offset;
	sctp_tran_rr_request->request_size = req_size;
	sctp_tran_rr_request->response_size = rsp_size;
	sctp_tran_rr_request->no_delay = rem_nodelay;
	sctp_tran_rr_request->measure_cpu = remote_cpu_usage;
	sctp_tran_rr_request->cpu_rate = remote_cpu_rate;
	sctp_tran_rr_request->so_rcvavoid = rem_rcvavoid;
	sctp_tran_rr_request->so_sndavoid = rem_sndavoid;
	if (test_time) {
		sctp_tran_rr_request->test_length = test_time;
	} else {
		sctp_tran_rr_request->test_length = test_trans * -1;
	}

	if (debug > 1) {
		fprintf(where, "netperf: send_sctp_tran_rr: requesting TCP_TRR test\n");
	}
	...
	if (!netperf_response.content.serv_errno) {
		rsr_size = sctp_tran_rr_response->recv_buf_size;
		rss_size = sctp_tran_rr_response->send_buf_size;
		rem_nodelay = sctp_tran_rr_response->no_delay;
		remote_cpu_usage = sctp_tran_rr_response->measure_cpu;
		remote_cpu_rate = sctp_tran_rr_response->cpu_rate;
		/* make sure that port numbers are in network order */
		server.sin_port = sctp_tran_rr_response->data_port_number;
		server.sin_port = htons(server.sin_port);
		if (debug) {
			fprintf(where, "remote listen done.\n");
			fprintf(where, "remote port is %d\n", ntohs(server.sin_port));
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if (setsockopt(send_socket, SOL_SOCKET, SO_REUSEADDR,
			       (char *) &one, sock_opt_len) == SOCKET_ERROR) {
			perror("netperf: send_sctp_tran_rr: so_reuseaddr");
			exit(1);
		}
		...
		/* we want to bind our socket to a particular port number. */
		if (bind(send_socket,
			 (struct sockaddr *) myaddr, sizeof(struct sockaddr_in)) == SOCKET_ERROR) {
			/* if the bind failed, someone else must have that port number */
			/* - perhaps in the listen state. since we can't use it, skip to */
			/* the next port number. we may have to do this again later, but */
			/* that's just too bad :) */
			if (debug > 1) {
				fprintf(where,
					"send_sctp_tran_rr: tried to bind to port %d errno %d\n",
					ntohs(myaddr->sin_port), errno);
				fflush(where);
			}
			/* yes, goto's are supposed to be evil, but they do have their */
		...
		if ((len = sendto(send_socket,
				  ...
				  req_size,
				  MSG_EOF,
				  (struct sockaddr *) &server, sizeof(server))) != req_size) {
			if (SOCKET_EINTR(len)) {
				/* we hit the end of a */
				/* timed test. */
				timed_out = 1;
			...
			perror("send_sctp_tran_rr: data send error");
			exit(1);
		}
		send_ring = send_ring->next;
		...
		while (rsp_bytes_left > 0) {
			if ((rsp_bytes_recvd = recv(send_socket,
				...
				perror("send_sctp_tran_rr: data recv error");
				exit(1);
			}
			rsp_bytes_left -= rsp_bytes_recvd;
			temp_message_ptr += rsp_bytes_recvd;
	...
	if (local_cpu_usage || remote_cpu_usage) {
		...
		if (remote_cpu_usage) {
			...
			remote_cpu_utilization = sctp_tran_rr_result->cpu_util;
			/* since calc_service demand is doing ms/Kunit we will */
			/* multiply the number of transaction by 1024 to get */
			/* "good" numbers */
			remote_service_demand = calc_service_demand((double) nummessages * 1024,
...
recv_sctp_tran_rr(void)
{
	...
	struct sctp_tran_rr_request_struct *sctp_tran_rr_request;
	struct sctp_tran_rr_response_struct *sctp_tran_rr_response;
	struct sctp_tran_rr_results_struct *sctp_tran_rr_results;

	sctp_tran_rr_request =
	    (struct sctp_tran_rr_request_struct *) netperf_request.content.test_specific_data;
	sctp_tran_rr_response =
	    (struct sctp_tran_rr_response_struct *) netperf_response.content.test_specific_data;
	sctp_tran_rr_results =
	    (struct sctp_tran_rr_results_struct *) netperf_response.content.test_specific_data;

	if (debug) {
		fprintf(where, "netserver: recv_sctp_tran_rr: entered...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_tran_rr: setting the response type...\n");
		fflush(where);
	}

	netperf_response.content.response_type = SCTP_TRR_RESPONSE;

	if (debug) {
		fprintf(where, "recv_sctp_tran_rr: the response type is set...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where,
			"recv_sctp_tran_rr: requested recv alignment of %d offset %d\n",
			sctp_tran_rr_request->recv_alignment, sctp_tran_rr_request->recv_offset);
		fprintf(where,
			"recv_sctp_tran_rr: requested send alignment of %d offset %d\n",
			sctp_tran_rr_request->send_alignment, sctp_tran_rr_request->send_offset);
		fflush(where);
	}

	recv_message_ptr =
	    ALIGN_BUFFER(message, sctp_tran_rr_request->recv_alignment,
			 sctp_tran_rr_request->recv_offset);

	send_message_ptr =
	    ALIGN_BUFFER(message, sctp_tran_rr_request->send_alignment,
			 sctp_tran_rr_request->send_offset);

	if (debug) {
		fprintf(where, "recv_sctp_tran_rr: receive alignment and offset set...\n");
		fflush(where);
	}
	...
	myaddr_in.sin_port = htons((unsigned short) sctp_tran_rr_request->port);

	/* Grab a socket to listen on, and then listen on it. */

	if (debug) {
		fprintf(where, "recv_sctp_tran_rr: grabbing a socket...\n");
		fflush(where);
	}
	...
	/* once the socket has been created, we will set the response values */
	/* based on the updated value of those globals. raj 7/94 */
	lss_size_req = sctp_tran_rr_request->send_buf_size;
	lsr_size_req = sctp_tran_rr_request->recv_buf_size;
	loc_nodelay = sctp_tran_rr_request->no_delay;
	loc_rcvavoid = sctp_tran_rr_request->so_rcvavoid;
	loc_sndavoid = sctp_tran_rr_request->so_sndavoid;

	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM,
				      IPPROTO_SCTP, INADDR_ANY, sctp_tran_rr_request->port);
	...
	if (bind(s_listen, (struct sockaddr *) &myaddr_in, sizeof(myaddr_in)) == SOCKET_ERROR) {
		netperf_response.content.serv_errno = errno;
		close(s_listen);
		send_response();
		if (debug) {
	...
	if (setsockopt(s_listen, SOL_SCTP, SCTP_NOPUSH, &NoPush, sizeof(int)) == SOCKET_ERROR) {
		fprintf(where, "recv_sctp_tran_rr: could not set SCTP_NOPUSH errno %d\n", errno);
		fflush(where);
		netperf_response.content.serv_errno = errno;
		close(s_listen);
		send_response();
	...
	sctp_tran_rr_response->data_port_number = (int) ntohs(myaddr_in.sin_port);
	if (debug) {
		fprintf(where, "telling the remote to call me at %d\n",
			sctp_tran_rr_response->data_port_number);
		fflush(where);
	}
	netperf_response.content.serv_errno = 0;
	...
	sctp_tran_rr_response->cpu_rate = 0.0;	/* assume no cpu */
	if (sctp_tran_rr_request->measure_cpu) {
		sctp_tran_rr_response->measure_cpu = 1;
		sctp_tran_rr_response->cpu_rate =
		    calibrate_local_cpu(sctp_tran_rr_request->cpu_rate);
	}

	/* before we send the response back to the initiator, pull some of */
	/* the socket parms from the globals */
	sctp_tran_rr_response->send_buf_size = lss_size;
	sctp_tran_rr_response->recv_buf_size = lsr_size;
	sctp_tran_rr_response->no_delay = loc_nodelay;
	sctp_tran_rr_response->so_rcvavoid = loc_rcvavoid;
	sctp_tran_rr_response->so_sndavoid = loc_sndavoid;
	...
	cpu_start(sctp_tran_rr_request->measure_cpu);

	/* The loop will exit when the sender does a shutdown, which will */
	/* return a length of zero */

	if (sctp_tran_rr_request->test_length > 0) {
		times_up = 0;
		trans_remaining = 0;
		start_timer(sctp_tran_rr_request->test_length + PAD_TIME);
	} else {
		times_up = 1;
		trans_remaining = sctp_tran_rr_request->test_length * -1;
	}
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if ((s_data = accept(s_listen,
			...
			fprintf(where, "recv_sctp_tran_rr: accept: errno = %d\n", errno);
			fflush(where);
			close(s_listen);
		...
		if (debug) {
			fprintf(where, "recv_sctp_tran_rr: accepted data connection.\n");
			fflush(where);
		}
#ifdef WIN32
		/* this is used so the timer thread can close the socket out from */
		...
		temp_message_ptr = recv_message_ptr;
		request_bytes_remaining = sctp_tran_rr_request->request_size;
		...
		if ((bytes_sent = sendto(s_data,
					 send_message_ptr,
					 sctp_tran_rr_request->response_size,
					 MSG_EOF,
					 &peeraddr_in,
					 sizeof(struct sockaddr_in))) == SOCKET_ERROR) {
			if (SOCKET_EINTR(bytes_sent)) {
	...
	cpu_stop(sctp_tran_rr_request->measure_cpu, &elapsed_time);
	...
	if (debug) {
		fprintf(where, "recv_sctp_tran_rr: got %d transactions\n", trans_received);
		fflush(where);
	}

	sctp_tran_rr_results->bytes_received = (trans_received *
						(sctp_tran_rr_request->request_size +
						 sctp_tran_rr_request->response_size));
	sctp_tran_rr_results->trans_received = trans_received;
	sctp_tran_rr_results->elapsed_time = elapsed_time;
	if (sctp_tran_rr_request->measure_cpu) {
		sctp_tran_rr_results->cpu_util = calc_cpu_util(elapsed_time);
	}

	if (debug) {
		fprintf(where, "recv_sctp_tran_rr: test complete, sending results.\n");
		fflush(where);
	}
...
recv_tcp_tran_rr(void)
{
	...
	if (debug) {
		fprintf(where,
			...
			tcp_tran_rr_request->recv_alignment, tcp_tran_rr_request->recv_offset);
		fprintf(where,
			"recv_tcp_tran_rr: requested send alignment of %d offset %d\n",
			tcp_tran_rr_request->send_alignment, tcp_tran_rr_request->send_offset);
		fflush(where);
	}

	recv_message_ptr =
	    ALIGN_BUFFER(message, tcp_tran_rr_request->recv_alignment,
			 tcp_tran_rr_request->recv_offset);

	send_message_ptr =
	    ALIGN_BUFFER(message, tcp_tran_rr_request->send_alignment,
	...
	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM,
				      IPPROTO_TCP, INADDR_ANY, tcp_tran_rr_request->port);
	...
	/* we want to disable the implicit PUSH on all sends. at some point, */
	/* this might want to be a parm to the test raj 3/95 */
	if (setsockopt(s_listen, SOL_TCP, TCP_NOPUSH, &NoPush, sizeof(int)) == SOCKET_ERROR) {
		fprintf(where, "recv_tcp_tran_rr: could not set TCP_NOPUSH errno %d\n", errno);
		fflush(where);
		netperf_response.content.serv_errno = errno;
		close(s_listen);
		send_response();
	...
	if (tcp_tran_rr_request->measure_cpu) {
		...
		tcp_tran_rr_response->cpu_rate = calibrate_local_cpu(tcp_tran_rr_request->cpu_rate);
	}
	...
	if (debug) {
		fprintf(where, "recv_tcp_tran_rr: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_tcp_tran_rr: test complete, sending results.\n");
		fflush(where);
	}
...
%5d  %5d   %5d  %5d";
	...
	if (print_headers) {
		fprintf(where, "SCTP Non-Blocking REQUEST/RESPONSE TEST");
		fprintf(where, " to %s", remote_host);
		if (iteration_max > 1) {
			fprintf(where,
				" : +/-%3.1f%% @ %2d%% conf.", interval / 0.02, confidence_level);
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (debug) {
			fprintf(where, "send_sctp_nbrr: send_socket obtained...\n");
		}
		...
		netperf_request.content.request_type = DO_SCTP_NBRR;
		sctp_rr_request->recv_buf_size = rsr_size;
		sctp_rr_request->send_buf_size = rss_size;
		sctp_rr_request->recv_alignment = remote_recv_align;
		sctp_rr_request->recv_offset = remote_recv_offset;
		...
		if (debug > 1) {
			fprintf(where, "netperf: send_sctp_nbrr: requesting SCTP rr test\n");
		}
		...
		if (!netperf_response.content.serv_errno) {
			...
			remote_cpu_rate = sctp_rr_response->cpu_rate;
			/* make sure that port numbers are in network order */
			server.sin_port = sctp_rr_response->data_port_number;
			server.sin_port = htons(server.sin_port);
		} else {
			Set_errno(netperf_response.content.serv_errno);
			fprintf(where,
		...
		/* get the signal set for the call to sigsuspend */
		if (sigprocmask(SIG_BLOCK, (sigset_t *) NULL, &signal_set) != 0) {
			fprintf(where, "send_sctp_nbrr: unable to get sigmask errno %d\n", errno);
			fflush(where);
			exit(1);
		}
#endif				/* INTERVALS */
		...
		while ((!times_up) || (trans_remaining > 0)) {
			...
			if ((len = send(send_socket,
				...
				perror("send_sctp_nbrr: data send error");
				exit(1);
			}
			send_ring = send_ring->next;
			...
			while (rsp_bytes_left > 0) {
				if ((rsp_bytes_recvd = recv(send_socket,
					...
					if (SOCKET_EINTR(rsp_bytes_recvd)) {
						...
						perror("send_sctp_nbrr: data recv error");
						exit(1);
					}
				}
				rsp_bytes_left -= rsp_bytes_recvd;
...
recv_sctp_nbrr(void)
{
	...
	if (debug) {
		fprintf(where, "netserver: recv_sctp_nbrr: entered...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_nbrr: setting the response type...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_nbrr: the response type is set...\n");
		fflush(where);
	}
	...
	/* and offsets. raj 7/94 */
	if (debug) {
		fprintf(where, "recv_sctp_nbrr: requested recv alignment of %d offset %d\n",
			sctp_rr_request->recv_alignment, sctp_rr_request->recv_offset);
		fprintf(where, "recv_sctp_nbrr: requested send alignment of %d offset %d\n",
			sctp_rr_request->send_alignment, sctp_rr_request->send_offset);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_nbrr: grabbing a socket...\n");
		fflush(where);
	}
	...
	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM, IPPROTO_SCTP, INADDR_ANY, sctp_rr_request->port);
	...
	sctp_rr_response->cpu_rate = 0.0;	/* assume no cpu */
	sctp_rr_response->measure_cpu = 0;
	...
	if (debug) {
		fprintf(where, "recv_sctp_nbrr: accept completes on the data connection.\n");
		fflush(where);
	}
#ifdef KLUDGE_SOCKET_OPTIONS
	/* this is for those systems which *INCORRECTLY* fail to pass */
	...
	if (debug) {
		fprintf(where, "recv_sctp_nbrr: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_nbrr: test complete, sending results.\n");
		fflush(where);
	}
...
%5d  %5d   %5d  %5d";
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		send_socket = create_data_socket(AF_INET,
						 SOCK_STREAM,
						 IPPROTO_TCP,
						 tcp_rr_request->ipaddress, tcp_rr_request->port);
		...
		/* get the signal set for the call to sigsuspend */
		if (sigprocmask(SIG_BLOCK, (sigset_t *) NULL, &signal_set) != 0) {
			fprintf(where, "send_tcp_nbrr: unable to get sigmask errno %d\n", errno);
			fflush(where);
			exit(1);
		}
#endif				/* INTERVALS */
...
recv_tcp_nbrr(void)
{
	...
	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM, IPPROTO_TCP, INADDR_ANY, tcp_rr_request->port);
	...
	if (debug) {
		fprintf(where, "recv_tcp_nbrr: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_tcp_nbrr: test complete, sending results.\n");
		fflush(where);
	}
...

 ...
 /* establishing a SCTP connection. that service demand could then be */
 /* compared with the sum of the service demands of a SCTP_CRR and */
 /* SCTP_RR test - presumeably, they would all relate */

void
send_sctp_cc(char remote_host[])
{
...
%5d  %5d   %5d  %5d\n";
	...
	struct sctp_cc_request_struct *sctp_cc_request;
	struct sctp_cc_response_struct *sctp_cc_response;
	struct sctp_cc_results_struct *sctp_cc_result;

	sctp_cc_request =
	    (struct sctp_cc_request_struct *) netperf_request.content.test_specific_data;
	sctp_cc_response =
	    (struct sctp_cc_response_struct *) netperf_response.content.test_specific_data;
	sctp_cc_result =
	    (struct sctp_cc_results_struct *) netperf_response.content.test_specific_data;
	...
	if (print_headers) {
		fprintf(where, "SCTP Connect/Close TEST");
		fprintf(where, " to %s", remote_host);
		if (iteration_max > 1) {
			fprintf(where,
				" : +/-%3.1f%% @ %2d%% conf.", interval / 0.02, confidence_level);
	...
	if (debug) {
		fprintf(where, "send_sctp_cc: send_socket obtained...\n");
	}
	...
	netperf_request.content.request_type = DO_SCTP_CC;
	sctp_cc_request->recv_buf_size = rsr_size;
	sctp_cc_request->send_buf_size = rss_size;
	sctp_cc_request->recv_alignment = remote_recv_align;
	sctp_cc_request->recv_offset = remote_recv_offset;
	sctp_cc_request->send_alignment = remote_send_align;
	sctp_cc_request->send_offset = remote_send_offset;
	sctp_cc_request->request_size = req_size;
	sctp_cc_request->response_size = rsp_size;
	sctp_cc_request->no_delay = rem_nodelay;
	sctp_cc_request->measure_cpu = remote_cpu_usage;
	sctp_cc_request->cpu_rate = remote_cpu_rate;
	sctp_cc_request->so_rcvavoid = rem_rcvavoid;
	sctp_cc_request->so_sndavoid = rem_sndavoid;
	if (test_time) {
		sctp_cc_request->test_length = test_time;
	} else {
		sctp_cc_request->test_length = test_trans * -1;
	}
	sctp_cc_request->port = (int) remote_data_port;

	if (debug > 1) {
		fprintf(where, "netperf: send_sctp_cc: requesting SCTP crr test\n");
	}
	...
	if (!netperf_response.content.serv_errno) {
		rsr_size = sctp_cc_response->recv_buf_size;
		rss_size = sctp_cc_response->send_buf_size;
		rem_nodelay = sctp_cc_response->no_delay;
		remote_cpu_usage = sctp_cc_response->measure_cpu;
		remote_cpu_rate = sctp_cc_response->cpu_rate;
		/* make sure that port numbers are in network order */
		server.sin_port = (unsigned short) sctp_cc_response->data_port_number;
		server.sin_port = htons(server.sin_port);
		if (debug) {
			fprintf(where, "remote listen done.\n");
			fprintf(where, "remote port is %d\n", ntohs(server.sin_port));
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if (send_socket == INVALID_SOCKET) {
			perror("netperf: send_sctp_cc: sctp stream data socket");
			exit(1);
		}
		...
		if (setsockopt(send_socket, SOL_SOCKET, SO_REUSEADDR,
			       (char *) &one, sock_opt_len) == SOCKET_ERROR) {
			perror("netperf: send_sctp_cc: so_reuseaddr");
			exit(1);
		}
		...
		if ((ret = bind(send_socket,
			...
			if (debug > 1) {
				fprintf(where,
					"send_sctp_cc: tried to bind to port %d errno %d\n",
					ntohs(myaddr->sin_port), errno);
				fflush(where);
			}
			/* yes, goto's are supposed to be evil, but they do have their */
		...
		rsp_bytes_recvd = recv(send_socket, temp_message_ptr, rsp_bytes_left, 0);
		...
		if (rsp_bytes_recvd == 0) {
			...
			perror("send_sctp_cc: data recv error");
			exit(1);
		}
	...
	if (local_cpu_usage || remote_cpu_usage) {
		...
		if (remote_cpu_usage) {
			...
			remote_cpu_utilization = sctp_cc_result->cpu_util;
			/* since calc_service demand is doing ms/Kunit we will */
			/* multiply the number of transaction by 1024 to get */
			/* "good" numbers */
			remote_service_demand = calc_service_demand((double) nummessages * 1024,
...
recv_sctp_cc()
{
	...
	struct sctp_cc_request_struct *sctp_cc_request;
	struct sctp_cc_response_struct *sctp_cc_response;
	struct sctp_cc_results_struct *sctp_cc_results;
	...
	if (debug) {
		fprintf(where, "netserver: recv_sctp_cc: entered...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sctp_cc: setting the response type...\n");
		fflush(where);
	}

	netperf_response.content.response_type = SCTP_CC_RESPONSE;

	if (debug) {
		fprintf(where, "recv_sctp_cc: the response type is set...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where,
			"recv_sctp_cc: requested recv alignment of %d offset %d\n",
			sctp_cc_request->recv_alignment, sctp_cc_request->recv_offset);
		fprintf(where,
			"recv_sctp_cc: requested send alignment of %d offset %d\n",
			sctp_cc_request->send_alignment, sctp_cc_request->send_offset);
		fflush(where);
	}

	recv_message_ptr =
	    ALIGN_BUFFER(message, sctp_cc_request->recv_alignment, sctp_cc_request->recv_offset);

	send_message_ptr =
	    ALIGN_BUFFER(message, sctp_cc_request->send_alignment, sctp_cc_request->send_offset);

	if (debug) {
		fprintf(where, "recv_sctp_cc: receive alignment and offset set...\n");
		fflush(where);
	}
	...
	myaddr_in.sin_family = AF_INET;
	myaddr_in.sin_addr.s_addr = INADDR_ANY;
	myaddr_in.sin_port = htons((unsigned short) sctp_cc_request->port);

	/* Grab a socket to listen on, and then listen on it. */

	if (debug) {
		fprintf(where, "recv_sctp_cc: grabbing a socket...\n");
		fflush(where);
	}
	...
	/* once the socket has been created, we will set the response values */
	/* based on the updated value of those globals. raj 7/94 */
	lss_size_req = sctp_cc_request->send_buf_size;
	lsr_size_req = sctp_cc_request->recv_buf_size;
	loc_nodelay = sctp_cc_request->no_delay;
	loc_rcvavoid = sctp_cc_request->so_rcvavoid;
	loc_sndavoid = sctp_cc_request->so_sndavoid;

	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM, IPPROTO_SCTP, INADDR_ANY, sctp_cc_request->port);
	...
	sctp_cc_response->data_port_number = (int) ntohs(myaddr_in.sin_port);
	if (debug) {
		fprintf(where, "telling the remote to call me at %d\n",
			sctp_cc_response->data_port_number);
		fflush(where);
	}
	netperf_response.content.serv_errno = 0;
	...
	sctp_cc_response->cpu_rate = (float) 0.0;	/* assume no cpu */
	if (sctp_cc_request->measure_cpu) {
		sctp_cc_response->measure_cpu = 1;
		sctp_cc_response->cpu_rate = calibrate_local_cpu(sctp_cc_request->cpu_rate);
	}

	/* before we send the response back to the initiator, pull some of */
	/* the socket parms from the globals */
	sctp_cc_response->send_buf_size = lss_size;
	sctp_cc_response->recv_buf_size = lsr_size;
	sctp_cc_response->no_delay = loc_nodelay;
	sctp_cc_response->so_rcvavoid = loc_rcvavoid;
	sctp_cc_response->so_sndavoid = loc_sndavoid;
	...
	cpu_start(sctp_cc_request->measure_cpu);

	/* The loop will exit when the sender does a shutdown, which will */
	/* return a length of zero */

	if (sctp_cc_request->test_length > 0) {
		times_up = 0;
		trans_remaining = 0;
		start_timer(sctp_cc_request->test_length + PAD_TIME);
	} else {
		times_up = 1;
		trans_remaining = sctp_cc_request->test_length * -1;
	}
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if ((s_data = accept(s_listen,
			...
			fprintf(where, "recv_sctp_cc: accept: errno = %d\n", errno);
			fflush(where);
			close(s_listen);
		...
		if (debug) {
			fprintf(where, "recv_sctp_cc: accepted data connection.\n");
			fflush(where);
		}
		...
		if (debug > 3) {
			fprintf(where, "recv_sctp_cc: Transaction %d complete\n", trans_received);
			fflush(where);
		}
	...
	cpu_stop(sctp_cc_request->measure_cpu, &elapsed_time);
	...
	if (debug) {
		fprintf(where, "recv_sctp_cc: got %d transactions\n", trans_received);
		fflush(where);
	}

	sctp_cc_results->bytes_received = (trans_received *
					   (sctp_cc_request->request_size +
					    sctp_cc_request->response_size));
	sctp_cc_results->trans_received = trans_received;
	sctp_cc_results->elapsed_time = elapsed_time;
	if (sctp_cc_request->measure_cpu) {
		sctp_cc_results->cpu_util = calc_cpu_util(elapsed_time);
	}

	if (debug) {
		fprintf(where, "recv_sctp_cc: test complete, sending results.\n");
		fflush(where);
	}
...
recv_tcp_cc()
{
	...
	recv_message_ptr =
	    ALIGN_BUFFER(message, tcp_cc_request->recv_alignment, tcp_cc_request->recv_offset);

	send_message_ptr =
	    ALIGN_BUFFER(message, tcp_cc_request->send_alignment, tcp_cc_request->send_offset);
	...
	s_listen = create_data_socket(AF_INET,
				      SOCK_STREAM, IPPROTO_TCP, INADDR_ANY, tcp_cc_request->port);
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if (debug > 3) {
			fprintf(where, "recv_tcp_cc: Transaction %d complete\n", trans_received);
			fflush(where);
		}
	...
	if (debug) {
		fprintf(where, "recv_tcp_cc: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_tcp_cc: test complete, sending results.\n");
		fflush(where);
	}
...
#define SOCKETS_ARGS "b:CDhI:m:M:p:P:r:s:S:Vw:W:z"
	...
	while ((c = getopt(argc, argv, SOCKETS_ARGS)) != EOF) {
		...
		case 'C':
			...
			/* set SCTP_CORK */
			loc_sctpcork = 1;
			rem_sctpcork = 1;	/* however, at first, we only have cork affect loc */
#else
			printf("WARNING: SCTP_CORK not available on this platform!\n");
#endif				/* SCTP_CORK */
#endif				/* DO_SCTP */
			break;
		case 'D':
		...
		case 'p':
			...
			break;
		case 'P':
			/* set the local and remote data port numbers for the tests to allow them
			   to run through those blankety blank end-to-end breaking firewalls. raj
			   2004-06-15 */
			break_args(optarg, arg1, arg2);
			if (arg1[0])

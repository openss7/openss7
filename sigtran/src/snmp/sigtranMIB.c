/*****************************************************************************
 ...
 @(#) $RCSfile: sigtranMIB.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2008-12-27 10:01:52 $
...
#ident "@(#) $RCSfile: sigtranMIB.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2008-12-27 10:01:52 $"

static char const ident[] = "$RCSfile: sigtranMIB.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2008-12-27 10:01:52 $";
...
/*
 * variable7 sigtranMIB_variables:
 *   this variable defines function callbacks and type return information
 *   for the sigtranMIB mib section
 */
...
var_sigtranMIB(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	case SIGTRANSCTPPROFILENEXTINDEX:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileNextIndex);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileNextIndex;
...
sigtranSctpProfileTable_add(struct sigtranSctpProfileTable_data *thedata)
{
	...
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->sigtranSctpProfileIndex, sizeof(thedata->sigtranSctpProfileIndex));
 ...
 *   The workings of this are basically the same as for var_sigtranMIB above.
 */
uint8_t *
var_sigtranSctpProfileTable(struct variable *vp, oid * name, size_t *length, int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	case SIGTRANSCTPPROFILENAME:
		...
		*var_len = StorageTmp->sigtranSctpProfileNameLen;
		return (uint8_t *) StorageTmp->sigtranSctpProfileName;
	case SIGTRANSCTPPROFILENODELAY:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileNodelay);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileNodelay;
	...
	case SIGTRANSCTPPROFILECSUMTYPE:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileCsumType);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileCsumType;
	case SIGTRANSCTPPROFILEMACTYPE:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileMacType);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileMacType;
	case SIGTRANSCTPPROFILEVALIDCOOKIELIFE:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileValidCookieLife);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileValidCookieLife;
	case SIGTRANSCTPPROFILECOOKIEINC:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileCookieInc);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileCookieInc;
	case SIGTRANSCTPPROFILEMAXISTREAMS:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileMaxIstreams);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileMaxIstreams;
	case SIGTRANSCTPPROFILEREQOSTREAMS:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileReqOstreams);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileReqOstreams;
	case SIGTRANSCTPECN:
		*write_method = write_sigtranSctpEcn;
		*var_len = sizeof(StorageTmp->sigtranSctpEcn);
		return (uint8_t *) &StorageTmp->sigtranSctpEcn;
	case SIGTRANSCTPADAPTATIONLAYERINFO:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpAdaptationLayerInfo);
		return (uint8_t *) &StorageTmp->sigtranSctpAdaptationLayerInfo;
	case SIGTRANSCTPPROFILEMAXINITRETRIES:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileMaxInitRetries);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileMaxInitRetries;
	case SIGTRANSCTPPROFILEMAXBURST:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileMaxBurst);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileMaxBurst;
	case SIGTRANSCTPPROFILEASSOCMAXRETRANS:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileAssocMaxRetrans);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileAssocMaxRetrans;
	case SIGTRANSCTPPROFILEMAXSACKDELAY:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileMaxSackDelay);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileMaxSackDelay;
	case SIGTRANSCTPPROFILERTOMIN:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileRtoMin);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileRtoMin;
	case SIGTRANSCTPPROFILERTOINITIAL:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileRtoInitial);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileRtoInitial;
	case SIGTRANSCTPPROFILERTOMAX:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileRtoMax);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileRtoMax;
	case SIGTRANSCTPPROFILEPATHMAXRETRANS:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfilePathMaxRetrans);
		return (uint8_t *) &StorageTmp->sigtranSctpProfilePathMaxRetrans;
	case SIGTRANSCTPPROFILEHEARTBEATITVL:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileHeartbeatItvl);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileHeartbeatItvl;
	case SIGTRANSCTPPROFILESTATUS:
		...
		*var_len = sizeof(StorageTmp->sigtranSctpProfileStatus);
		return (uint8_t *) &StorageTmp->sigtranSctpProfileStatus;
...
int
write_sigtranSctpProfileName(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	...
	case ACTION:		/* The variable has been stored in string for you to use, and you
		...
		tmpvar = StorageTmp->sigtranSctpProfileName;
		tmplen = StorageTmp->sigtranSctpProfileNameLen;
		memdup((void *) &StorageTmp->sigtranSctpProfileName, var_val, var_val_len);
		StorageTmp->sigtranSctpProfileNameLen = var_val_len;
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->sigtranSctpProfileName);
		StorageTmp->sigtranSctpProfileName = tmpvar;
		StorageTmp->sigtranSctpProfileNameLen = tmplen;
		break;
...
int
write_sigtranSctpProfileNodelay(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileNodelay;
		StorageTmp->sigtranSctpProfileNodelay = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileNodelay = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpPayloadProtocolId(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and
				   ...
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpPayloadProtocolId;
		StorageTmp->sigtranSctpPayloadProtocolId = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpPayloadProtocolId = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpLifetime(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpLifetime;
		StorageTmp->sigtranSctpLifetime = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpLifetime = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileCsumType(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileCsumType;
		StorageTmp->sigtranSctpProfileCsumType = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileCsumType = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileMacType(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileMacType;
		StorageTmp->sigtranSctpProfileMacType = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileMacType = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileValidCookieLife(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileValidCookieLife;
		StorageTmp->sigtranSctpProfileValidCookieLife = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileValidCookieLife = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileCookieInc(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileCookieInc;
		StorageTmp->sigtranSctpProfileCookieInc = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileCookieInc = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileMaxIstreams(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and
				   you have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileMaxIstreams;
		StorageTmp->sigtranSctpProfileMaxIstreams = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileMaxIstreams = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileReqOstreams(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and
				   you have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileReqOstreams;
		StorageTmp->sigtranSctpProfileReqOstreams = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileReqOstreams = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpEcn(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpEcn;
		StorageTmp->sigtranSctpEcn = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpEcn = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpAdaptationLayerInfo(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and
				   you have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpAdaptationLayerInfo;
		StorageTmp->sigtranSctpAdaptationLayerInfo = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpAdaptationLayerInfo = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileMaxInitRetries(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and
				   you have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileMaxInitRetries;
		StorageTmp->sigtranSctpProfileMaxInitRetries = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileMaxInitRetries = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileMaxBurst(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and
				   you have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileMaxBurst;
		StorageTmp->sigtranSctpProfileMaxBurst = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileMaxBurst = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileAssocMaxRetrans(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and
				   you have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileAssocMaxRetrans;
		StorageTmp->sigtranSctpProfileAssocMaxRetrans = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileAssocMaxRetrans = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileMaxSackDelay(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileMaxSackDelay;
		StorageTmp->sigtranSctpProfileMaxSackDelay = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileMaxSackDelay = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileRtoMin(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileRtoMin;
		StorageTmp->sigtranSctpProfileRtoMin = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileRtoMin = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileRtoInitial(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileRtoInitial;
		StorageTmp->sigtranSctpProfileRtoInitial = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileRtoInitial = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileRtoMax(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileRtoMax;
		StorageTmp->sigtranSctpProfileRtoMax = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileRtoMax = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfilePathMaxRetrans(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static uint32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and
				   you have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfilePathMaxRetrans;
		StorageTmp->sigtranSctpProfilePathMaxRetrans = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfilePathMaxRetrans = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileHeartbeatItvl(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->sigtranSctpProfileHeartbeatItvl;
		StorageTmp->sigtranSctpProfileHeartbeatItvl = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->sigtranSctpProfileHeartbeatItvl = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_sigtranSctpProfileStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	struct sigtranSctpProfileTable_data *StorageTmp = NULL;
	...
	case RESERVE2:
		...
		if (StorageTmp == NULL) {
			/* creation */
			vars = NULL;
			snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, NULL, 0);	/* sigtranSctpProfileIndex 
												 */
			...
			vp = vars;
			StorageNew = SNMP_MALLOC_STRUCT(sigtranSctpProfileTable_data);
	...
	case ACTION:
		...
		if (StorageTmp == NULL) {
			...
			StorageTmp->sigtranSctpProfileStatus = *((int *) var_val);
		} else {
	...
	case COMMIT:
		...
		if (StorageDel != NULL) {
			...
			if (StorageTmp && StorageTmp->sigtranSctpProfileStatus == RS_CREATEANDGO) {
				StorageTmp->sigtranSctpProfileStatus = RS_ACTIVE;
			} else if (StorageTmp && StorageTmp->sigtranSctpProfileStatus == RS_CREATEANDWAIT) {

/*****************************************************************************
 ...
 @(#) $RCSfile: m3uaSgpMIB.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2008-12-27 10:01:52 $
...
#ident "@(#) $RCSfile: m3uaSgpMIB.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2008-12-27 10:01:52 $"

static char const ident[] = "$RCSfile: m3uaSgpMIB.c,v $ $Name: OpenSS7-0_9_2 $($Revision: 0.9.2.1 $) $Date: 2008-12-27 10:01:52 $";
...
/*
 * variable7 m3uaSgpMIB_variables:
 *   this variable defines function callbacks and type return information
 *   for the m3uaSgpMIB mib section
 */
...
m3uaSgpAgTable_add(struct m3uaSgpAgTable_data *thedata)
{
	...
	/* add the index variables to the varbind list, which is used by header_complex to index
	   the data */
	/* m3uaSgpAgIndex */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->m3uaSgpAgIndex, sizeof(thedata->m3uaSgpAgIndex));
...
m3uaSgpAspTable_add(struct m3uaSgpAspTable_data *thedata)
{
	...
	/* add the index variables to the varbind list, which is used by header_complex to index
	   the data */
	/* m3uaSgpAspIndex */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->m3uaSgpAspIndex, sizeof(thedata->m3uaSgpAspIndex));
...
m3uaSgpTable_add(struct m3uaSgpTable_data *thedata)
{
	...
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->m3uaSgpAgIndex, sizeof(thedata->m3uaSgpAgIndex));
	/* m3uaSgpSgIndex */
	snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, (uint8_t *) &thedata->m3uaSgpSgIndex, sizeof(thedata->m3uaSgpSgIndex));
 ...
 *   The workings of this are basically the same as for var_m3uaSgpMIB above.
 */
uint8_t *
...
int
write_m3uaSgpAgName(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct m3uaSgpAgTable_data *StorageTmp = NULL;
	...
	case ACTION:		/* The variable has been stored in string for you to use, and you
		...
		tmpvar = StorageTmp->m3uaSgpAgName;
		tmplen = StorageTmp->m3uaSgpAgNameLen;
		memdup((void *) &StorageTmp->m3uaSgpAgName, var_val, var_val_len);
		StorageTmp->m3uaSgpAgNameLen = var_val_len;
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->m3uaSgpAgName);
		StorageTmp->m3uaSgpAgName = tmpvar;
		StorageTmp->m3uaSgpAgNameLen = tmplen;
		break;
...
int
write_m3uaSgpAgProtocolVersion(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static oid *tmpvar;
	struct m3uaSgpAgTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in objid for you to use, and you
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAgProtocolVersion;
		tmplen = StorageTmp->m3uaSgpAgProtocolVersionLen;
		memdup((void *) &StorageTmp->m3uaSgpAgProtocolVersion, var_val, var_val_len);
		StorageTmp->m3uaSgpAgProtocolVersionLen = var_val_len / sizeof(oid);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->m3uaSgpAgProtocolVersion);
		StorageTmp->m3uaSgpAgProtocolVersion = tmpvar;
		StorageTmp->m3uaSgpAgProtocolVersionLen = tmplen;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
		SNMP_FREE(tmpvar);
...
int
write_m3uaSgpAgOptions(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct m3uaSgpAgTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in string for you to use, and you
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAgOptions;
		tmplen = StorageTmp->m3uaSgpAgOptionsLen;
		memdup((void *) &StorageTmp->m3uaSgpAgOptions, var_val, var_val_len);
		StorageTmp->m3uaSgpAgOptionsLen = var_val_len;
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->m3uaSgpAgOptions);
		StorageTmp->m3uaSgpAgOptions = tmpvar;
		StorageTmp->m3uaSgpAgOptionsLen = tmplen;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
		SNMP_FREE(tmpvar);
...
int
write_m3uaSgpAgRegistrationPolicy(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAgRegistrationPolicy;
		StorageTmp->m3uaSgpAgRegistrationPolicy = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAgRegistrationPolicy = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAgAspIdPolicy(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct m3uaSgpAgTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAgAspIdPolicy;
		StorageTmp->m3uaSgpAgAspIdPolicy = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAgAspIdPolicy = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAgProtocolPayloadId(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct m3uaSgpAgTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAgProtocolPayloadId;
		StorageTmp->m3uaSgpAgProtocolPayloadId = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAgProtocolPayloadId = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAgIpPort(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct m3uaSgpAgTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAgIpPort;
		StorageTmp->m3uaSgpAgIpPort = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAgIpPort = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAgMinOstreams(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct m3uaSgpAgTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAgMinOstreams;
		StorageTmp->m3uaSgpAgMinOstreams = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAgMinOstreams = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAgMaxIstreams(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct m3uaSgpAgTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAgMaxIstreams;
		StorageTmp->m3uaSgpAgMaxIstreams = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAgMaxIstreams = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpSgName(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct m3uaSgpSgTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in string for you to use, and you
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpSgName;
		tmplen = StorageTmp->m3uaSgpSgNameLen;
		memdup((void *) &StorageTmp->m3uaSgpSgName, var_val, var_val_len);
		StorageTmp->m3uaSgpSgNameLen = var_val_len;
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->m3uaSgpSgName);
		StorageTmp->m3uaSgpSgName = tmpvar;
		StorageTmp->m3uaSgpSgNameLen = tmplen;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
		SNMP_FREE(tmpvar);
...
int
write_m3uaSgpAspName(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct m3uaSgpAspTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in string for you to use, and you
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAspName;
		tmplen = StorageTmp->m3uaSgpAspNameLen;
		memdup((void *) &StorageTmp->m3uaSgpAspName, var_val, var_val_len);
		StorageTmp->m3uaSgpAspNameLen = var_val_len;
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->m3uaSgpAspName);
		StorageTmp->m3uaSgpAspName = tmpvar;
		StorageTmp->m3uaSgpAspNameLen = tmplen;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
		SNMP_FREE(tmpvar);
...
int
write_m3uaSgpAspAdministrativeState(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct m3uaSgpAspTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAspAdministrativeState;
		StorageTmp->m3uaSgpAspAdministrativeState = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAspAdministrativeState = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAspAlarmStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static char *tmpvar;
	struct m3uaSgpAspTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in string for you to use, and you
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAspAlarmStatus;
		tmplen = StorageTmp->m3uaSgpAspAlarmStatusLen;
		memdup((void *) &StorageTmp->m3uaSgpAspAlarmStatus, var_val, var_val_len);
		StorageTmp->m3uaSgpAspAlarmStatusLen = var_val_len;
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->m3uaSgpAspAlarmStatus);
		StorageTmp->m3uaSgpAspAlarmStatus = tmpvar;
		StorageTmp->m3uaSgpAspAlarmStatusLen = tmplen;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
		SNMP_FREE(tmpvar);
...
int
write_m3uaSgpAspAspState(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct m3uaSgpAspTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAspAspState;
		StorageTmp->m3uaSgpAspAspState = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAspAspState = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAspSctpProfile(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static oid *tmpvar;
	struct m3uaSgpAspTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in objid for you to use, and you
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAspSctpProfile;
		tmplen = StorageTmp->m3uaSgpAspSctpProfileLen;
		memdup((void *) &StorageTmp->m3uaSgpAspSctpProfile, var_val, var_val_len);
		StorageTmp->m3uaSgpAspSctpProfileLen = var_val_len / sizeof(oid);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		SNMP_FREE(StorageTmp->m3uaSgpAspSctpProfile);
		StorageTmp->m3uaSgpAspSctpProfile = tmpvar;
		StorageTmp->m3uaSgpAspSctpProfileLen = tmplen;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
		SNMP_FREE(tmpvar);
...
int
write_m3uaSgpAspMaxPathRetrans(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	...
	case ACTION:		/* The variable has been stored in ulong_ret for you to use, and
				   ...
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAspMaxPathRetrans;
		StorageTmp->m3uaSgpAspMaxPathRetrans = *((ulong *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAspMaxPathRetrans = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAspRtoMin(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct m3uaSgpAspTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAspRtoMin;
		StorageTmp->m3uaSgpAspRtoMin = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAspRtoMin = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAspMaxLifeTime(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct m3uaSgpAspTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAspMaxLifeTime;
		StorageTmp->m3uaSgpAspMaxLifeTime = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAspMaxLifeTime = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAspTimerDivert(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	static int32_t tmpvar;
	struct m3uaSgpAspTable_data *StorageTmp = NULL;
	static size_t tmplen;
	...
	case ACTION:		/* The variable has been stored in long_ret for you to use, and you 
				   have just been asked to do something with it.  Note that
				   anything done here must be reversable in the UNDO case */
		tmpvar = StorageTmp->m3uaSgpAspTimerDivert;
		StorageTmp->m3uaSgpAspTimerDivert = *((long *) var_val);
		break;
	case UNDO:		/* Back out any changes made in the ACTION case */
		StorageTmp->m3uaSgpAspTimerDivert = tmpvar;
		break;
	case COMMIT:		/* Things are working well, so it's now safe to make the change
				   permanently.  Make sure that anything done here can't fail! */
...
int
write_m3uaSgpAgStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	struct m3uaSgpAgTable_data *StorageTmp = NULL;
	...
	case RESERVE2:
		...
		if (StorageTmp == NULL) {
			/* creation */
			vars = NULL;
			snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, NULL, 0);	/* m3uaSgpAgIndex 
												 */
			...
			vp = vars;
			StorageNew = SNMP_MALLOC_STRUCT(m3uaSgpAgTable_data);
	...
	case ACTION:
		...
		if (StorageTmp == NULL) {
			...
			StorageTmp->m3uaSgpAgStatus = *((int *) var_val);
		} else {
	...
	case COMMIT:
		...
		if (StorageDel != NULL) {
			...
			if (StorageTmp && StorageTmp->m3uaSgpAgStatus == RS_CREATEANDGO) {
				StorageTmp->m3uaSgpAgStatus = RS_ACTIVE;
			} else if (StorageTmp && StorageTmp->m3uaSgpAgStatus == RS_CREATEANDWAIT) {
...
int
write_m3uaSgpSgStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	struct m3uaSgpSgTable_data *StorageTmp = NULL;
	...
	case RESERVE2:
		...
		if (StorageTmp == NULL) {
			...
			vp = vars;
			StorageNew = SNMP_MALLOC_STRUCT(m3uaSgpSgTable_data);
			StorageNew->m3uaSgpAgIndex = *vp->val.integer;
			vp = vp->next_variable;
	...
	case ACTION:
		...
		if (StorageTmp == NULL) {
			...
			StorageTmp->m3uaSgpSgStatus = *((int *) var_val);
		} else {
			/* destroy...  extract it for now */
	...
	case COMMIT:
		...
		if (StorageDel != NULL) {
			...
			if (StorageTmp && StorageTmp->m3uaSgpSgStatus == RS_CREATEANDGO) {
				StorageTmp->m3uaSgpSgStatus = RS_ACTIVE;
			} else if (StorageTmp && StorageTmp->m3uaSgpSgStatus == RS_CREATEANDWAIT) {
...
int
write_m3uaSgpAspStatus(int action, uint8_t *var_val, uint8_t var_val_type, size_t var_val_len, uint8_t *statP, oid * name, size_t name_len)
{
	struct m3uaSgpAspTable_data *StorageTmp = NULL;
	...
	case RESERVE2:
		...
		if (StorageTmp == NULL) {
			/* creation */
			vars = NULL;
			snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, NULL, 0);	/* m3uaSgpAspIndex 
												 */
			...
			vp = vars;
			StorageNew = SNMP_MALLOC_STRUCT(m3uaSgpAspTable_data);
	...
	case ACTION:
		...
		if (StorageTmp == NULL) {
			...
			StorageTmp->m3uaSgpAspStatus = *((int *) var_val);
		} else {
			/* destroy...  extract it for now */
	...
	case COMMIT:
		...
		if (StorageDel != NULL) {
			...
			if (StorageTmp && StorageTmp->m3uaSgpAspStatus == RS_CREATEANDGO) {
				StorageTmp->m3uaSgpAspStatus = RS_ACTIVE;
			} else if (StorageTmp && StorageTmp->m3uaSgpAspStatus == RS_CREATEANDWAIT) {

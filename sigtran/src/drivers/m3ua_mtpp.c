/*****************************************************************************
 ...
 @(#) $RCSfile: m3ua_mtpp.c,v $ $Name:  $($Revision: 0.9.2.4 $) $Date: 2008-04-29 01:52:21 $
...
#ident "@(#) $RCSfile: m3ua_mtpp.c,v $ $Name:  $($Revision: 0.9.2.4 $) $Date: 2008-04-29 01:52:21 $"
 ...
 */
static int
mtpp_info_ack(queue_t *q, mblk_t *mp)
{
	/* 
 ...
 */
static int
mtpp_bind_ack(queue_t *q, mblk_t *mp)
{
	/* 
 ...
 */
static int
mtpp_error_ack(queue_t *q, mblk_t *mp)
{
	/* 
	 *  FIXME: might want to look at this one to echo the state of the
	 *  interface and then pass it on.
 ...
 */
static int
mtpp_ok_ack(queue_t *q, mblk_t *mp)
{
	/* 
	 *  FIXME: might want to look at this one to echo the state of the
	 *  interface and then pass it on.
 ...
 */
static mblk_t *
mtpp_uderror_ind(queue_t *q, mblk_t *msg)
{
	mblk_t *mp;
	mtpp_t *mtp = Q_MTP(q);
 ...
 */
static int
mtpp_datack_ind(queue_t *q, mblk_t *msg)
{
}
 ...
 */
static int
mtpp_reset_ind(queue_t *q, mblk_t *msg)
{
	mblk_t *mp;
	mtpp_t *mtp = Q_MTP(q);
	size_t mlen = FIXME;
 ...
 */
static mblk_t *
mtpp_unitdata_req(queue_t *q, mblk_t *msg)
{
	mblk_t *mp, *db = msg->b_cont;
	mtpp_t *mtp = Q_MTP(q);
	static const size_t mlen =
 ...
 */
static int
mtpp_w_data(queue_t *q, mblk_t *mp)
{
	int err;
	mblk_t *np;
...
static int
mtpp_r_data(queue_t *q, mblk_t *mp)
{
	int err;
	mblk_t *np;
 ...
 */
static int
mtpp_w_proto(queue_t *q, mblk_t *mp)
{

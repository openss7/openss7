/*****************************************************************************
 ...
 @(#) $RCSfile: m3ua_asp.c,v $ $Name:  $($Revision: 0.9.2.2 $) $Date: 2007/06/17 02:00:50 $
...
#ident "@(#) $RCSfile: m3ua_asp.c,v $ $Name:  $($Revision: 0.9.2.2 $) $Date: 2007/06/17 02:00:50 $"
 ...
 *  =========================================================================
 *
 *  M3UA Peer --> M3UA (ASP) Received Messages
 *
 *  =========================================================================
 */
/*
 ...
 *  -------------------------------------------------------------------------
 */
static int m3ua_recv_data(asp_t * asp, mblk_t * msg)
{
 ...
 */
static int m3ua_distribute(asp_t * asp, mblk_t * mp, parm_t * rc)
{
	int i;
	mblk_t *dp, *ep;
...
static int m3ua_snmm_parm_check(asp_t * asp, mblk_t * msg, m3ua_parms_t * parms)
{
	mblk_t *ep;
...
static int m3ua_not_sg(asp_t * asp, mblk_t * msg)
{
	mblk_t *ep;
 ...
 */
static int m3ua_recv_duna(asp_t * asp, mblk_t * msg)
{
	int err;
	mblk_t *mp;
	m3ua_parms_t parms;
 ...
 */
static int m3ua_recv_dava(asp_t * asp, mblk_t * msg)
{
	(void) q;
	(void) msg;
	/* 
 ...
 *  -------------------------------------------------------------------------
 */
static int m3ua_recv_daud(asp_t * asp, mblk_t * msg)
{
	mblk_t *ep;
 ...
 *  -------------------------------------------------------------------------
 */
static int m3ua_recv_scon(asp_t * asp, mblk_t * msg)
{
	mblk_t *ep;
	size_t mlen = msgdsize(msg);
	if (Q_MODE(q) & Q_MODE_SG) {
 ...
 *  -------------------------------------------------------------------------
 */
static int m3ua_recv_dupu(asp_t * asp, mblk_t * msg)
{
	mblk_t *ep;
	size_t mlen = msgdsize(msg);
	if (Q_MODE(q) & Q_MODE_SG) {
 ...
 *  -------------------------------------------------------------------------
 */
static int m3ua_recv_drst(asp_t * asp, mblk_t * msg)
{
	mblk_t *ep;
	size_t mlen = msgdsize(msg);
	if (Q_MODE(q) & Q_MODE_SG) {
 ...
 *  -------------------------------------------------------------------------
 */
static int m3ua_recv_reg_rsp(asp_t * asp, mblk_t * msg)
{
	(void) q;
	(void) msg;
	return (-EOPNOTSUPP);
 ...
 *  -------------------------------------------------------------------------
 */
static int m3ua_recv_dereg_rsp(asp_t * asp, mblk_t * msg)
{
	(void) q;
	(void) msg;
	return (-EOPNOTSUPP);
...
static int (*m3ua_xfer[]) (asp_t *, mblk_t *) = {
	NULL,			/* (reserved) 0x0 */
	    m3ua_recv_data	/* M3UA_XFER_DATA 0x1 */
};
static int (*m3ua_snmm[]) (asp_t *, mblk_t *) = {
	NULL,			/* (reserved) 0x0 */
...
static int (*m3ua_rkmm[]) (asp_t *, mblk_t *) = {
	NULL,			/* (reserved) 0x0 */
...
int m3ua_asp_recv_msg(asp_t * asp, mblk_t * mp)
{

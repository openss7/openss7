/*****************************************************************************
 ...
 @(#) $RCSfile: m2ua_as.c,v $ $Name:  $($Revision: 0.9.2.8 $) $Date: 2007/02/10 22:32:06 $
 ...
 Last Modified $Date: 2007/02/10 22:32:06 $ by $Author: brian $
...
#ident "@(#) $RCSfile: m2ua_as.c,v $ $Name:  $($Revision: 0.9.2.8 $) $Date: 2007/02/10 22:32:06 $"
...
#define M2UA_AS_REVISION	"OpenSS7 $RCSfile: m2ua_as.c,v $ $Name:  $($Revision: 0.9.2.8 $) $Date: 2007/02/10 22:32:06 $"
 ...
 */
static fastcall int
gp_send_asps_hbeat_ack(struct gp *gp, struct tp *tp, queue_t *q, mblk_t *msg, caddr_t hptr,
		       size_t hlen)
{
		  ...
		  struct tcap_addr *dest, uint8_t pri, uint8_t sls, mblk_t *dp)
{
	...
	size_t mlen =
	    UA_MHDR_SIZE + (as->as.asid ? UA_PHDR_SIZE + sizeof(uint32_t) : 0) + UA_PHDR_SIZE;
...
#endif
#if SUA
		p[0] = UA_PHDR(SUA_PARM_PROT_DATA3, dlen);
#endif
#if TUA
		p[0] = UA_PHDR(TUA_PARM_PROT_DATA3, dlen);
#endif
		p[1] = htonl(orig->pc);
		p[2] = htonl(dest->pc);
		p[3] = htonl(((uint32_t) dest->si << 24) |
...
rp_lookup(struct gp *gp, queue_t *q, mblk_t *mp, int *errp)
{
	...
	if (ua_dec_parm(mp->b_rptr, mp->b_wptr, &asid, UA_PARM_IID_TEXT)) {
		for (rp = gp->rplist; rp; rp = rp->sp.next)
			if (strncmp(asid.cp, (caddr_t) &rp->as.as->as.asid, min(asid.len, 32)) == 0)
				goto found;
	}
...
#endif
#if M3UA
		/* TS 102 142 says that if a routing key management message is received to return

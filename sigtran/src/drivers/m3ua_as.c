...
static char const ident[] = "$RCSfile: m3ua_as.c,v $ $Name:  $($Revision: 0.9.2.3 $) $Date: 2007/02/03 03:07:46 $";
...
struct df {
	...
	struct lmi_notify events;
	struct { struct up *list; } up;
	struct { struct as *list; } as;
	struct { struct sg *list; } sg;
	struct { struct xp *list; } xp;
} ua_defaults = {
	/* *INDENT-OFF* */
	{
	},
 ...
 */
static inline fastcall int
mtp_optmgmt_ack(struct up *up, queue_t *q, mblk_t *msg, caddr_t optr, size_t olen,
		mtp_ulong flags)
{
...
as_recv_aspt_aspia_ack(struct as *as, queue_t *q, mblk_t *mp)
{
	...
	case ASP_WACK_ASPIA:	/* solicited */
	{
		...
		if (!asp) {
			...
			case MTPS_WACK_BREQ:
			case MTPS_WACK_CREQ:
				if ((err = mtp_error_ack(up, q, NULL, MTP_BIND_REQ, -ECONNRESET)))
					return (err);
				break;
			case MTPS_WCON_CREQ:
			case MTPS_CONNECTED:
			...
			case MTPS_WRES_CIND:
				if ((err = mtp_discon_ind(up, q, NULL)))
					return (err);
				break;
			case MTPS_WACK_UREQ:
				if ((err = mtp_ok_ack(up, q, NULL, MTP_UNBIND_REQ)))
					return (err);
				break;
			case MTPS_WACK_DREQ6:
			case MTPS_WACK_DREQ9:
				if ((err = mtp_ok_ack(up, q, NULL, MTP_DISCON_REQ)))
					return (err);
				break;
			default:
			case MTPS_UNBND:
...
as_recv_xfer_data(struct as *as, queue_t *q, mblk_t *mp)
{
	...
	return (err);
      missing:
	strlog(as->tp.tp->mid, as->tp.tp->sid, 0, SL_ERROR, "XFER DATA missing PROT DATA3 parameter");
...
tp_recv_msg_slow(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	case UA_CLASS_RKMM:
		...
		if (sg->sp.options.options.pvar == UA_VERSION_TS102142)
			goto enoprotoopt;
...
t_discon_ind(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	write_lock(&ua_mux_lock);
	for (up = tp->up.list; up; up = up->tp.next) {
		if (tp->sg.sg->sp.options.options.pvar != UA_VERSION_TS102142
		    && (tp->sg.sg->sg.options.options.popt & UA_SG_DYNAMIC)) {
			/* ETSI TS 102 142 does not permit dynamic configuration. */

/*****************************************************************************
 ...
 @(#) $RCSfile: ua_as.c,v $ $Name:  $($Revision: 0.9.2.3 $) $Date: 2007/02/03 03:07:46 $
...
#ident "@(#) $RCSfile: ua_as.c,v $ $Name:  $($Revision: 0.9.2.3 $) $Date: 2007/02/03 03:07:46 $"
...
#define UA_AS_REVISION	"OpenSS7 $RCSfile: ua_as.c,v $ $Name:  $ ($Revision: 0.9.2.3 $) $Date: 2007/02/03 03:07:46 $"
...
#ifdef LFS
#define UA_AS_DRV_ID	CONFIG_STREAMS_UA_MODID
#define UA_AS_DRV_NAME	CONFIG_STREAMS_UA_NAME
#define UA_AS_CMAJORS	CONFIG_STREAMS_UA_NMAJORS
#define UA_AS_CMAJOR_0	CONFIG_STREAMS_UA_MAJOR
#define UA_AS_UNITS	CONFIG_STREAMS_UA_NMINORS
#endif				/* LFS */
		...
		case AS_WACK_ASPAC:
			for (up = as->up.list; up; up = up->as.next)
				if (up_get_t_state(up) == AS_WACK_ASPAC)
					if ((err = up_aspac_con(up, q, NULL)))
						return (err);
			as_set_t_state(as, AS_ACTIVE);
#if 0				/* needs to be in up_aspac_con */
			/* Inform management (UP or LM). */
			switch (ua_profiles[as->sp.sp->sp.options.pvar].protocol) {
			...
			case UA_PROTOCOL_TUA:
				for (up = ap->up.list; up; up = up->as.next) {
					switch (up_get_state(up)) {
					case TS_WACK_BREQ:
 ...
 * @errno: set to non-zero when state changes results from:
 *	[ECONNRESET]:	SCTP association failure or shutdown,
 *	[EACCES]:	management blocking at the SG.
 */
static int
gp_set_state(struct gp *gp, queue_t *q, uint newstate, int errno)
{
	...
	if (newstate != oldstate) {
		...
		struct sp *sp = sg->sp.sp;
		bool dynamic =
		    ((sg->gp.options.proto.popt & UA_DYNAMIC) &&
		     (sp->sp.options.proto.pvar != UA_PROFILE_TS102141));
		...
		case ASP_DOWN:
			...
			case ASP_WACK_ASPDN:
				/* Management needs a confirmation. */
				if ((lm = gp->sg.sg->sp.sp->lm.lm ? : lm_ctrl)) {
					if ((err = ua_aspdn_con(lm, q, NULL, gp)))
						return (err);
				} else
			...
			case ASP_WACK_ASPUP:
				...
				if ((tp = gp->xp.xp) && tp_get_state(tp) == TS_DATA_XFER)
					/* Note: this will reenter this function! */
					return gp_send_asps_aspup_req(gp, q, NULL, NULL, 0);
				break;
			}
			break;
		case ASP_WACK_ASPUP:
		...
		case ASP_UP:
			...
			case ASP_WACK_ASPUP:
				/* Management needs a confirmation. */
				if ((lm = gp->sg.sg->sp.sp->lm.lm ? : lm_ctrl)) {
					if ((err = ua_aspup_con(lm, q, NULL, gp)))
						return (err);
				} else
					gp_set_t_state(gp, ASP_UP);
				/* fall through */
			case ASP_UP:
				/* When the SGP comes ASP_UP, and management wants it up, check if
				   there are keys that need to be registered and AS that need to be 
				   activated. */
				for (rp = gp->rp.list; rp; rp = rp->gp.next) {
					struct as *as = rp->as.as;
			...
			case ASP_WACK_ASPDN:
				...
				/* Note: this will reenter this function! */
				return gp_send_asps_aspdn_req(gp, q, mp, NULL, 0);
				break;
			}
		}
...
static void __unlikely
ua_term_object_xp(struct tp *xp)
{
	int b;
...
static void __unlikely
ua_free_object_xp(struct tp *xp)
{
...
static struct lm *ua_create_object_lm(int *errp, struct up *lm, uint id, struct sp *sp);
 ...
 */
static inline fastcall int
ua_sctp_ind(struct tp *tp, queue_t *q, mblk_t *msg)
{
	struct UA_sctp_ind *p;
	mblk_t *mp;
	struct up *lm;

	if (!(lm = tp->sp.sp ? (tp->sp.sp->lm.lm ? : lm_ctrl) : lm_ctrl)) {
		freemsg(msg);
		return (0);
	}
...
gp_send_asps_aspup_req(struct gp *gp, queue_t *q, mblk_t *msg, caddr_t iptr, size_t ilen)
{
	...
	if (likely((mp = ua_allocb(q, mlen, BPRI_MED)) != NULL)) {
		...
		if ((err = gp_set_state(gp, q, ASP_WACK_ASPUP, 0)) ||
...
gp_send_asps_aspdn_req(struct gp *gp, queue_t *q, mblk_t *msg, caddr_t iptr, size_t ilen)
{
	...
	if (likely((mp = ua_allocb(q, mlen, BPRI_MED)) != NULL)) {
		...
		/* send unordered and expedited on stream 0 */
		if ((err = gp_set_state(gp, q, ASP_WACK_ASPDN, 0)) ||
		    (err = t_optdata_req(tp, q, msg, T_ODF_EX, 0, mp)))
			freeb(mp);
		return (err);
	}
...
rp_send_aspt_aspac_ack(struct rp *rp, queue_t *q, mblk_t *msg, caddr_t iptr, size_t ilen)
{
	...
	mblk_t *mp;
	uint popt = sp->sp.options.proto.popt;
	uint plev = UA_PROTOCOL_LEVEL(sp->sp.options.proto.pvar);
	size_t tlen = (popt & UA_TEXTIID) ? strnlen(as->as.addr.iid_text, 32) : 0;
	size_t mlen = UA_MHDR_SIZE +
	    (as->as.addr.tmode ? UA_SIZE(UA_PARM_TMODE) : 0) +
	    (as->as.asid ? UA_PHDR_SIZE + sizeof(uint32_t) : 0) +
		  ...
		  struct mtp_addr *dest, uint8_t pri, uint8_t sls, mblk_t *dp)
{
	...
	if (likely((mp = ua_allocb(q, mlen, BPRI_MED)))) {
		...
		if (unlikely((err = t_optdata_req(tp, q, msg, 0, rp->rp.streamid, 0, mp))))
			freeb(mp);
		return (err);
	}
	return (-ENOBUFS);
...
#endif
#ifdef SUA
		p[2] = UA_PHDR(SUA_PARM_CONCERN_DEST, sizeof(uint32_t));
#endif
#ifdef TUA
		p[2] = UA_PHDR(TUA_PARM_CONCERN_DEST, sizeof(uint32_t));
#endif
		p[3] = htonl(cpc);
		p += 4;
		if (cong) {
...
#endif
#if SUA
			p[0] = UA_PHDR(SUA_PARM_CONG_IND, sizeof(uint32_t));
#endif
#if TUA
			p[0] = UA_PHDR(TUA_PARM_CONG_IND, sizeof(uint32_t));
#endif
			p[1] = htonl(*cong);
			p += 2;
		}
...
{
	uint plev = UA_PROTOCOL_LEVEL(tp->sp.sp->options.proto.pvar);
...
tp_recv_asps_aspup_ack(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	if (gp_get_state(gp) == ASP_WACK_ASPUP) {
		/* Solicited */
		if ((err = gp_set_state(gp, q, ASP_UP, 0)))
			return (err);
...
tp_recv_asps_aspdn_ack(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	if (gp_get_state(gp) != ASP_DOWN) {
		if ((err = gp_set_state(gp, q, ASP_DOWN, 0)))
			return (err);
		mi_timer_stop(gp->gp.wack_aspup);
...
rp_recv_aspt_aspac_req(struct rp *rp, struct tp *tp, queue_t *q, mblk_t *mp, uint tmode)
{
	...
	if (as->as.addr.tmode == UA_TMODE_NONE)
		as->as.addr.tmode = tmode;
	if (as->as.addr.tmode != tmode)
...
tp_recv_aspt_aspac_req(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	uint pvar = tp->sp.sp->options.proto.pvar;
	uint popt = tp->sp.sp->options.proto.popt;
	uint plev = UA_PROTOCOL_LEVEL(pvar);
	int err = 0;
	...
	for (; rp; rp = rp_lookup_next(tp, mp->b_wptr, &parm, rp, &err)) {
		...
		if ((err = rp_recv_aspt_aspac_req(rp, tp, q, mp)))
			return (err);
	}
	if (err)
		return (err);
...
tp_recv_aspt_aspac_ack(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	mi_timer_stop(tp->gp.gp->gp.wack_aspac);
	goto protocol_error;
...
as_recv_xfer_data(struct as *as, queue_t *q, mblk_t *mp)
{
	...
	struct up *up = as->up.list;	/* only one user part */
...
#if M3UA || SUA
/**
 * rp_recv_snmm_duna: - receive SNMM DUNA message
 * @rp: AS-SGP relation pointer
 * @tp: TP private structure
 * @q: active queue (lower read queue)
 * @mp: the message
 */
static int
rp_recv_snmm_duna(struct rp *rp, struct tp *tp, queue_t *q, mblk_t *mp)
{
	int err;
	...
	return mtp_pause_ind(up, q, mp, &addr);
      missing:
	strlog(tp->mid, tp->sid, 0, SL_ERROR, "SNMM DUNA missing APC parameter");
	err = -ENXIO;		/* missing mandatory parameter */
	goto error;
      outstate:
	strlog(tp->mid, tp->sid, 0, SL_ERROR, "SNMM DUNA in unexpected AS state %s",
	       as_state_name(rp_get_state(rp)));
	err = UA_ECODE_UNEXPECTED_MESSAGE;
	goto error;
      error:
	freemsg(mp);
...
#endif
#if SUA
	if (!ua_dec_parm(mp->b_rptr, mp->b_wptr, &cause, SUA_PARM_USER_CAUSE))
		goto missing_user_cause;
#endif
#if TUA
	if (!ua_dec_parm(mp->b_rptr, mp->b_wptr, &cause, TUA_PARM_USER_CAUSE))
		goto missing_user_cause;
#endif
	addr.pc = apc.val;
...
tp_recv_snmm(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	if (!(rp = rp_lookup(tp->gp.gp, mp, &err)) && err)
		return (err);

	switch (UA_MSG_TYPE(p[0])) {
	case UA_SNMM_DUNA:
		strlog(tp->mid, tp->sid, UALOGRX, SL_TRACE, "SNMM DUNA <-");
		err = as_recv_snmm_duna(rp, tp, q, mp);
		break;
	case UA_SNMM_DAVA:
		strlog(tp->mid, tp->sid, UALOGRX, SL_TRACE, "SNMM DAVA <-");
		err = as_recv_snmm_dava(rp, tp, q, mp);
		break;
	case UA_SNMM_DAUD:
	...
	case UA_SNMM_SCON:
		...
		err = as_recv_snmm_scon(rp, tp, q, mp);
		break;
	case UA_SNMM_DUPU:
		strlog(tp->mid, tp->sid, UALOGRX, SL_TRACE, "SNMM DUPU <-");
		err = as_recv_snmm_dupu(rp, tp, q, mp);
		break;
	case UA_SNMM_DRST:
		strlog(tp->mid, tp->sid, UALOGRX, SL_TRACE, "SNMM DRST <-");
		err = as_recv_snmm_drst(rp, tp, q, mp);
		break;
	default:
		err = -EOPNOTSUPP;
		break;
...
#if SUA
	/* FIXME: message can contain multiple registration results (but probably won't) */
	if (ua_dec_parm(mp->b_rptr, mp->b_wptr, &request_id, SUA_PARM_REG_RESULT)) {
		uint32_t status = 0;
		uint32_t asid = 0;
...
#if TUA
	/* FIXME: message can contain multiple registration results (but probably won't) */
	if (ua_dec_parm(mp->b_rptr, mp->b_wptr, &request_id, TUA_PARM_REG_RESULT)) {
		uint32_t status = 0;
		uint32_t asid = 0;
...
tp_recv_msg_slow(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	if (!(gp = tp->gp.gp))
		return (-EAGAIN);
	class = UA_MSG_CLAS(p[0]);
	switch (ua_profile[tp->sp.sp->sp.options.proto.pvar].protocol) {
	case UA_PROTOCOL_IUA:
	case UA_PROTOCOL_V5UA:
	case UA_PROTOCOL_DUA:
	case UA_PROTOCOL_GUA:
...
lmi_attach_req(struct up *up, queue_t *q, mblk_t *mp)
{
	...
	if (ppa_len == 0) {
		...
		if ((spid = up->as.as->as.addr.spid) == 0)
			goto badppa;
		asid =
		    ((up->as.as->as.addr.sdti << 16) | up->as.as->as.addr.sdli) ? : up->as.as->as.
		    addr.asid;
		iid_ptr =
...
ua_add_object_sg(struct up *lm, queue_t *q, uint id, struct ua_conf_sg *c)
{
	...
	if ((sp = ua_find_object_sp(&err, lm, c->spid, NULL))) {
		...
		if (!(sg = ua_create_object_sg(&err, lm, sp, c->position))) {
			sp_unlock(sp);
			return (err);
		}
		sp_unlock(sp);
...
lm_i_plink(struct up *lm, queue_t *q, mblk_t *mp)
{
	...
	if (!(tp = ua_link_alloc(l->l_qtop, l->l_index, ioc->ioc_cr, lm->as.addr.sgid))) {
		err = ENOMEM;
		goto error;
	}
...
up_w_data(queue_t *q, mblk_t *mp)
{
	...
	case MTPS_IDLE:
		...
		err = rp_send_xfer_data(rp, tp, q, &up->orig, &up->dest, 0, 0, mp);
		break;
	case MTPS_CONNECTED:
...
t_discon_ind(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	if ((err = gp_set_state(tp->gp.gp, q, ASP_DOWN, ECONNRESET)))
		return (err);
	tp_set_state(tp, TS_IDLE);
      idle:
	...
	case TS_WCON_CREQ:
	case TS_DATA_XFER:
		/* If the TP target state is TS_WCON_CREQ or TS_DATA_XFER, then attempt a
		   reconnect. */
...
t_info(struct tp *tp, queue_t *q, mblk_t *mp, struct T_info_ack *p)
{
	...
	case TS_WREQ_ORDREL:
		if ((err = t_ordrel_req(tp, q, NULL)))
			return (err);
		if ((gp = tp->gp.gp) && (err = gp_set_state(gp, q, ASP_DOWN, ECONNRESET)))
			return (err);
		if (tp_get_t_state(tp) == TS_DATA_XFER)
			return t_conn_req(tp, q, mp);
		break;
...
t_ordrel_ind(struct tp *tp, queue_t *q, mblk_t *mp)
{
	...
	if (mp->b_wptr < mp->b_rptr + sizeof(*p))
		goto tooshort;
	if ((gp = tp->gp.gp)) {
		if ((err = gp_set_state(gp, q, ASP_DOWN, ECONNRESET)))
			return (err);
	}
	switch (tp_get_state(tp)) {
	case TS_DATA_XFER:
